...
<p align="center">
  <strong>在一个 Windows 桌面应用里，管理 Codex 账号、线程、用量与 Skills。</strong><br>
  <strong>Manage Codex accounts, threads, usage, and Skills in one Windows desktop app.</strong>
</p>
...
NexCode 是一款面向 Codex 用户的原生 Windows 桌面软件。它把分散在本机文件、
...
Windows 10/11 x64 用户可从 [Releases](https://github.com/jasonlee539/NexCode/releases)
获取已发布的安装包或便携版。

从源码构建需要 Node.js 18+、Visual Studio 2019 或更高版本、.NET Framework 4.7.2
targeting pack，以及 Microsoft Edge WebView2 Runtime：
...
- `NexCode-windows-x64/NexCode.exe`：便携应用
- `NexCode-windows-x64-portable.zip`：便携版压缩包
- `NexCode-Setup-<version>-x64.exe`：当前用户安装包

如不需要安装包，可运行 `npm run desktop:portable`。

---

## English

NexCode is a native Windows desktop app for Codex users. It brings accounts,
...
- Run in a dedicated WebView2 desktop window instead of redirecting the dashboard to a browser.
...
- Choose between an installer and a portable package.
...
Windows 10/11 x64 users can download published installers and portable packages
from [Releases](https://github.com/jasonlee539/NexCode/releases).

Building from source requires Node.js 18+, Visual Studio 2019 or newer, the
.NET Framework 4.7.2 targeting pack, and Microsoft Edge WebView2 Runtime:
...
- `NexCode-windows-x64/NexCode.exe` — portable application
- `NexCode-windows-x64-portable.zip` — portable archive
- `NexCode-Setup-<version>-x64.exe` — per-user installer

Run `npm run desktop:portable` when the installer is not needed.

---

## Project structure / 项目结构

- `desktop/` — Windows host, installer, branding assets, and packaging scripts
