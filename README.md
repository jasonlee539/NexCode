...
- 将线程以 Markdown 保存到用户选择的任意可写目录。
- 已安装的 Windows 应用启动后自动检查 GitHub Release，并在验证 RSA 签名清单和 SHA-256 后通过 `Windows-Ota-Updata.exe` 完成 OTA 更新。
...
`dist/NexCode-Setup-<version>-x64.exe` 安装包。正式构建还会生成固定名称的
`dist/Windows-Ota-Updata.exe`、SHA-256、签名清单与签名文件，供 GitHub Release OTA 使用。应用包内包含 Bun、管理服务源码、生产 GUI 和
