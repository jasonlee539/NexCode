...
function interceptConfig(): NxcConfig {
  return {
    port: 0,
    defaultProvider: "xai",
    providers: {
...
async function withTempHome<T>(run: () => Promise<T>): Promise<T> {
  ...
  const dir = mkdtempSync(join(tmpdir(), "nxc-shadow-"));
...
async function shadowApi(config: NxcConfig, method: string, body?: unknown): Promise<Record<string, unknown>> {
...
describe("shadow-call settings API reports the intercepted source models", () => {
  test("GET reports the 0.145.0+ helper-model default", async () => {
    await withTempHome(async () => {
      const body = await shadowApi({ port: 0, defaultProvider: "xai", providers: {} } as NxcConfig, "GET");
