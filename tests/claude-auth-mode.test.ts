...
function cfg(claudeCode?: NxcConfig["claudeCode"], apiKeys?: { key: string }[]): NxcConfig {
  return {
    port: 10100,
...
test("a stale marker never suppresses the admission key", () => {
  ...
  // nexcode really does own authentication here, so the host flag is correct.
...
//
// Bun auto-loads `.env`/`.env.local` before any nexcode code runs, so process.env alone
...
const PRE_BUN = "NXC_PRE_BUN_ANTHROPIC_ENV";
...
// The admission key is nexcode's own gate, not user auth: it is injected after the strip.
...
test("a stale generated admission key is still recognized after key rotation", () => {
  const env = buildClaudeEnv(
    cfg(undefined, [{ key: "nxc_data_current" }]), 10100,
    {
      ANTHROPIC_BASE_URL: "https://trusted-gateway.example",
      ANTHROPIC_AUTH_TOKEN: "  nxc_data_rotated  ",
    },
    {},
    {
      authDetect: fileAuth("present"),
...
test("a proxy admission secret is never preserved in the API-key slot", () => {
  const external = buildClaudeEnv(
    ...
    {
      ANTHROPIC_BASE_URL: "https://trusted-gateway.example",
      ANTHROPIC_API_KEY: "  nxc_data_rotated  ",
    },
    {},
    {
      authDetect: fileAuth("present"),
  ...
  expect(local.ANTHROPIC_AUTH_TOKEN).toBe("nxc_data_current");
});
...
// therefore correct exactly when nexcode owns authentication, and the auto path has
// to reach that conclusion on its own.
// ---------------------------------------------------------------------------

test("auto-resolved proxy emits the host-managed assertion with its token", () => {
  const env = buildClaudeEnv(cfg(), 10100, {}, {}, { authDetect: fileAuth("absent") });
  // Auto found no Claude auth, so nexcode owns authentication here.
  expect(env.ANTHROPIC_AUTH_TOKEN).toBe(PROXY_MARKER);
  expect(env.CLAUDE_CODE_PROVIDER_MANAGED_BY_HOST).toBe("1");
});
...
// what keeps nexcode routing. We model the merge and assert the strip fires exactly
