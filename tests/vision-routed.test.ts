...
const originalEnvToken = process.env.NEXCODE_API_AUTH_TOKEN;

beforeEach(() => {
  previousHome = process.env.NEXCODE_HOME;
  isolatedCodexHome = installIsolatedCodexHome("nxc-vision-routed-codex-");
  testDir = mkdtempSync(join(tmpdir(), "nxc-vision-routed-"));
...
afterEach(() => {
  ...
  if (originalEnvToken === undefined) delete process.env.NEXCODE_API_AUTH_TOKEN;
  else process.env.NEXCODE_API_AUTH_TOKEN = originalEnvToken;
...
describe("describeImageRouted unit", () => {
    ...
    const server = Bun.serve({
      ...
      async fetch(req) {
        seen = {
          ...
          apiKey: req.headers.get("x-nexcode-api-key"),
  ...
  test("admission ladder: env token first, then first apiKeys entry, as x-nexcode-api-key", () => {
    ...
    process.env.NEXCODE_API_AUTH_TOKEN = "env-token";
    expect(routedDescribeAdmissionToken({
      apiKeys: [{ id: "a", name: "a", key: "key-1", createdAt: "" }],
    })).toBe("env-token");
    delete process.env.NEXCODE_API_AUTH_TOKEN;
