...
function generatedRoutedEntry(slug: string, marker?: string): RawEntry {
  ...
  return {
    ...
    slug,
    display_name: slug,
    description: `Routed via nexcode → ${provider} (${provider}).`,
    owned_by: provider,
...
function config(pickerEnabled: boolean, disabledModels: string[] = []): NxcConfig {
  return {
    port: 10100,
    providers: {
      openai: {
...
function seedObservedRuntimeSupport(efforts = ["low", "medium", "high", "xhigh"]): void {
  rmSync(join(nexcodeHome, "codex-runtime.json"), { force: true });
  const runtime = {
    command: "/tmp/codex",
    version: "0.145.0",
...
async function convergeCatalog(nextConfig: NxcConfig): Promise<RawCatalog> {
...
async function convergeCatalogDisposition(nextConfig: NxcConfig) {
  saveConfig(nextConfig);
...
beforeEach(() => {
  ...
  root = realpathSync.native(mkdtempSync(join(tmpdir(), "nxc-convergence-accounts-")));
  codexHome = join(root, "codex");
  nexcodeHome = join(root, "nexcode");
  catalogPath = join(codexHome, "nexcode-catalog.json");
  mkdirSync(codexHome);
  mkdirSync(nexcodeHome);
...
test("convergence renders account-qualified rows and preserves only non-generated foreign rows", async () => {
  writeCatalog([
    ...
    foreignEntry(),
    {
      ...
      description: "Routed via nexcode → removed-provider (removed-provider).",
    },
  ]);
...
test("disabling the picker removes generated rows, restores bare rows, and retains foreign rows", async () => {
  ...
  expect(models.some(entry => entry.nexcode_catalog_kind === CODEX_ACCOUNT_BOUND_CATALOG_KIND)).toBe(false);
  expect(models.some(entry => entry.slug === "external/vendor-model")).toBe(true);
});
...
function legacyCustomDeletionConfig(): NxcConfig {
  const nextConfig = config(false);
  nextConfig.providers.offline = {
    adapter: "openai-chat",
    baseUrl: "https://offline.example.test/v1",
...
test("a missing supported native is backfilled and restored when the picker is disabled", async () => {
  ...
  expect(disabled.models?.some(entry => (
    entry.nexcode_catalog_kind === CODEX_ACCOUNT_BOUND_CATALOG_KIND
  ))).toBe(false);
});
