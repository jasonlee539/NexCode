...
const previousImagesApiKey = process.env.NEXCODE_TEST_IMAGES_API_KEY;
...
beforeEach(() => {
  ...
  process.env.NEXCODE_TEST_IMAGES_API_KEY = "custom-images-key";
  isolatedCodexHome = installIsolatedCodexHome("nxc-server-images-codex-");
...
afterEach(() => {
  ...
  if (previousImagesApiKey === undefined) delete process.env.NEXCODE_TEST_IMAGES_API_KEY;
  else process.env.NEXCODE_TEST_IMAGES_API_KEY = previousImagesApiKey;
...
function forwardConfig(_baseUrl = ""): NxcConfig {
  return {
    port: 0,
  ...
  saveConfig({
    ...
    providers: {
      "custom-images": {
        ...
        authMode: "key",
        apiKey: "${NEXCODE_TEST_IMAGES_API_KEY}",
...
test("an explicit Images provider accepts bearer admission without leaking the proxy secret", async () => {
  process.env.NEXCODE_API_AUTH_TOKEN = "proxy-admission-secret";
  const captured: CapturedRequest[] = [];
  const upstream = Bun.serve({
    port: 0,
    async fetch(req) {
...
test("an image body that stalls after headers retains the 504 deadline", async () => {
  ...
  saveConfig({ ...forwardConfig(), images: { timeoutMs: 50 } } as NxcConfig);
...
test("CCA image fallback generates images via Google Antigravity when no OpenAI upstream exists", async () => {
  ...
  try {
    ...
    // bespoke "nexcode-images/1.0"), so the request fingerprint matches the
    // OAuth credential.
...
test("CCA fetch network failure returns 502 without leaking the timeout timer", async () => {
  ...
  saveConfig({ ...ccaConfig(), images: { timeoutMs: 10_000 } } as NxcConfig);
  await saveCredential("google-antigravity", { ...CCA_CREDENTIAL });
...
test("CCA body-read timeout returns 504 when upstream stalls after sending headers", async () => {
  ...
  saveConfig({ ...ccaConfig(), images: { timeoutMs: 100 } } as NxcConfig);
  await saveCredential("google-antigravity", { ...CCA_CREDENTIAL });
...
test("CCA body-read client cancellation returns 499, not 504", async () => {
  ...
  const cfg = { ...ccaConfig(), images: { timeoutMs: 30_000 } } as NxcConfig;
  saveConfig(cfg);
  await saveCredential("google-antigravity", { ...CCA_CREDENTIAL });
...
test("CCA deadline expiry during OAuth preflight returns 504, not a hung response", async () => {
  ...
  const cfg = { ...ccaConfig(), images: { timeoutMs: 100 } } as NxcConfig;
  saveConfig(cfg);
  await saveCredential("google-antigravity", { ...CCA_CREDENTIAL_EXPIRED });
...
test("GET /v1/nexcode/artifacts/:id serves opaque artifacts with API auth", async () => {
  process.env.NEXCODE_API_AUTH_TOKEN = "proxy-admission-secret";
  ...
  try {
    ...
    const traversal = await fetch(`http://127.0.0.1:${server.port}/v1/nexcode/artifacts/../package.json`, {
      headers: { authorization: "Bearer proxy-admission-secret" },
    });
