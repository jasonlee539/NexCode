...
function ctx(config: NxcConfig = LOOPBACK): ExportContext {
...
describe("no client config ever carries a credential", () => {
  ...
  const withKey = { ...LOOPBACK, apiKeys: [{ key: SENTINEL }] } as NxcConfig;
...
describe("hermes", () => {
  ...
  test("adds the admission header only on a non-loopback bind", () => {
    ...
    const remote = buildClientConfig("hermes", ctx({ ...LOOPBACK, hostname: "0.0.0.0" } as NxcConfig)) as HermesGeneratedConfig;
    expect(remote.providers[OPENCODE_PROVIDER_ID]!.extra_headers).toEqual({
      "x-nexcode-api-key": HERMES_API_KEY_ENV_REF,
    });
  });
});
