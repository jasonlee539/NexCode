...
describe("invalidateCodexModelsCache write gate (#476 / #518)", () => {
  ...
  beforeEach(() => {
    ...
    codexHome = mkdtempSync(join(tmpdir(), "nxc-invalidate-codex-"));
    nexcodeHome = mkdtempSync(join(tmpdir(), "nxc-invalidate-nxc-"));
  ...
  test("returns true and writes models_cache when catalog.json is readable", () => {
    writeFileSync(join(codexHome, "nexcode-catalog.json"), JSON.stringify({
      models: [{ slug: "gpt-5.5" }],
    }, null, 2) + "\n");
  ...
  test("refuses the cache rewrite when desired state flipped OFF between commit and reacquisition", () => {
    ...
    mkdirSync(join(nexcodeHome, ".nexcode"), { recursive: true });
  ...
  test("returns false for a missing catalog and does not warn/restart app-servers", () => {
    ...
    // Mirrors nxc sync-cache: only call the handler when invalidate wrote.
  ...
  test("returns false for invalid catalog JSON and does not warn/restart app-servers", () => {
    writeFileSync(join(codexHome, "nexcode-catalog.json"), "{ not-json");
    const errors: string[] = [];
    const logs: string[] = [];
    let listed = 0;
  ...
  test("nxc sync --restart-codex neither warns nor restarts when catalog exists but is unreadable", async () => {
    ...
    // Mirrors `nxc sync --restart-codex`: only handle app-servers after a real write.
  ...
  test("nxc sync --restart-codex neither warns nor restarts when catalog JSON is malformed", async () => {
    writeFileSync(join(codexHome, "config.toml"), 'model_catalog_json = "broken.json"\n', "utf8");
