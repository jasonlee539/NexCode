...
beforeEach(() => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("nxc-server-live-codex-");
...
function forwardConfig(): NxcConfig {
  return {
    port: 0,
...
test("standalone realtime upgrade keeps the auth and origin guards", async () => {
  saveConfig(forwardConfig());
  process.env.NEXCODE_API_AUTH_TOKEN = "required-secret";
...
// The env-gated frame forensic log (NXC_LIVE_FRAME_LOG) records per-frame metadata and
...
test("sideband frame log records direction, kind, and U+FFFD context without full payloads", async () => {
  ...
  process.env.NXC_LIVE_FRAME_LOG = frameLogPath;
  ...
  try {
    ...
    delete process.env.NXC_LIVE_FRAME_LOG;
    globalThis.WebSocket = RealWebSocket;
    await server.stop(true);
    await upstream.stop(true);
  }
...
describe("GET /readyz", () => {
  ...
  test("fresh gate is pending; /healthz 200 while /readyz 503 with Retry-After", async () => {
    ...
    try {
      ...
      expect(healthzBody.service).toBe("nexcode");
      ...
      expect(readyzBody.service).toBe("nexcode");
  ...
  test("/readyz is 200 with status ready only after gate.markReady()", async () => {
    ...
    try {
      ...
      expect(body.service).toBe("nexcode");
    } finally {
      await server.stop(true);
    }
  });
