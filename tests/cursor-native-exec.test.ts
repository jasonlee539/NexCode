...
describe("Cursor native exec bridge", () => {
  test("fails closed if synthetic Responses client tools arrive on native MCP exec channel", async () => {
    ...
    const reply = decode((await handleCursorNativeExec(execMessage({
      ...
      value: create(McpArgsSchema, {
        ...
        providerIdentifier: "nexcode-responses",
      }),
    }), {
      mcp: () => {
        called = true;
  ...
  test("blocks built-in local fs, shell, and fetch execution by default", async () => {
    const dir = mkdtempSync(join(tmpdir(), "nxc-cursor-exec-"));
    ...
    if (deniedRead.message.value.result.case === "error") {
      ...
      expect(deniedRead.message.value.result.value.error).not.toContain("disabled by NexCode policy");
    ...
    if (deniedShell.message.value.result.case === "failure") {
      ...
      expect(deniedShell.message.value.result.value.stderr).toContain("mcp_nexcode-responses_*");
      ...
      expect(deniedShell.message.value.result.value.stderr).not.toContain("disabled by NexCode policy");
    ...
    expect(streamText).toContain("mcp_nexcode-responses_*");
    ...
    if (deniedFetch.message.value.result.case === "error") {
      ...
      expect(deniedFetch.message.value.result.value.error).not.toContain("disabled by NexCode policy");
    }
  });
  ...
  test("keeps the removed allowNativeLocalExec key inert", async () => {
    const dir = mkdtempSync(join(tmpdir(), "nxc-cursor-alias-"));
    ...
    if (shell.message.value.result.case === "failure") {
      ...
      expect(shell.message.value.result.value.stderr).toContain("mcp_nexcode-responses_*");
  ...
  test("rejects native write and delete when apply_patch is available", async () => {
    const dir = mkdtempSync(join(tmpdir(), "nxc-cursor-patch-policy-"));
  ...
  test("deletes only the requested temp file", async () => {
    const dir = mkdtempSync(join(tmpdir(), "nxc-cursor-delete-"));
  ...
  test("runs harmless shell commands", async () => {
    const dir = mkdtempSync(join(tmpdir(), "nxc-cursor-shell-"));
    const shell = decode((await handleCursorNativeExec(execMessage({
      case: "shellArgs",
  ...
  test("returns shell stream events for shellStreamArgs", async () => {
    const dir = mkdtempSync(join(tmpdir(), "nxc-cursor-stream-"));
  ...
  test("background shell spawn and stdin receive the same native exec session owner", async () => {
    const dir = mkdtempSync(join(tmpdir(), "nxc-cursor-bg-"));
  ...
  test("greps temp files with content, file, and count output modes", async () => {
    const dir = mkdtempSync(join(tmpdir(), "nxc-cursor-grep-"));
  ...
  test("opens MCP and computer-use through executor hooks", async () => {
    const synthetic = decode((await handleCursorNativeExec(execMessage({
      case: "mcpArgs",
      value: create(McpArgsSchema, { name: "read_file", toolName: "read_file", providerIdentifier: "nexcode-responses" }),
    }), {
      mcp: async () => {
