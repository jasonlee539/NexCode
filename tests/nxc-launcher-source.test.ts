...
/**
 * bin/nxc.mjs is the Node bin launcher — it executes top-level logic on import, so it
 * cannot be imported by tests. Guard its Windows-critical invariants at the source level.
 */
const source = readFileSync(join(import.meta.dir, "..", "bin", "nxc.mjs"), "utf8");
...
describe("nxc.mjs npm launcher (source invariants)", () => {
  test("the Bun child receives the runtime provenance the launcher actually selected (#848)", () => {
    ...
    expect(source).toContain('const BUN_RUNTIME_SOURCE_ENV = "NXC_BUN_RUNTIME_SOURCE";');
    ...
    expect(runtimeSource).toContain('export const BUN_RUNTIME_SOURCE_ENV = "NXC_BUN_RUNTIME_SOURCE";');
  });
  ...
  test("the Bun child receives proof-bound pre-Bun Anthropic provenance", () => {
    ...
    expect(source).not.toContain("NXC_PRE_BUN_ANTHROPIC_ENV: preBunAnthropicSlots");
  ...
  test("valid Bun overrides are selected before the bundled runtime", () => {
    expect(source).toContain('const BUN_OVERRIDE_ENV = "NEXCODE_BUN_PATH";');
