...
describe("reclaimListenPort", () => {
  test("does not kill any nxc listener by default (healthy proxy / stale pid files)", async () => {
    const killed: number[] = [];
  ...
  test("does not kill when killNxcHolders is true but allowlist is empty", async () => {
    const killed: number[] = [];
    await expect(reclaimListenPort(10100, "127.0.0.1", {
      timeoutMs: 80,
      intervalMs: 20,
  ...
  test("concurrent pinned-start shape: second start never kills the first nxc listener", async () => {
    const killed: number[] = [];
    await expect(reclaimListenPort(10100, "127.0.0.1", {
      timeoutMs: 80,
      intervalMs: 20,
  ...
  test("kills only the allowlisted nxc pid after revalidation", async () => {
    const killed: number[] = [];
  ...
  test("unknown old PID: update-style reclaim kills no nxc listener", async () => {
    const killed: number[] = [];
    await expect(reclaimListenPort(10100, "127.0.0.1", {
      timeoutMs: 80,
      intervalMs: 20,
  ...
  test("does not kill foreign (non-nxc) listeners and does not drop their TCP rows", async () => {
    const killed: number[] = [];
  ...
  test("allowlisted pid with failing nxc revalidation is still killed (trusted teardown PID)", async () => {
    const killed: number[] = [];
    let available = false;
    let checks = 0;
    await expect(reclaimListenPort(10100, "127.0.0.1", {
      ...
      verifyNxcFn: pid => {
        checks += 1;
        // First pass (scan identity) succeeds; later scans reclassify as non-nxc.
  ...
  test("does not drop TCP rows while allowlisted non-nxc survives kill", async () => {
    const killed: number[] = [];
    const dropped: number[] = [];
    await expect(reclaimListenPort(10100, "127.0.0.1", {
      timeoutMs: 80,
  ...
  test("does not drop TCP rows while allowlisted nxc survives kill", async () => {
    const killed: number[] = [];
    const dropped: number[] = [];
    await expect(reclaimListenPort(10100, "127.0.0.1", {
      timeoutMs: 80,
  ...
  test("drops TCP rows only after allowlisted nxc is confirmed dead", async () => {
    let alive = true;
    let available = false;
    const dropped: number[] = [];
    await expect(reclaimListenPort(10100, "127.0.0.1", {
  ...
  test("killAllNxcOnPort kills nxc listeners absent from the allowlist snapshot", async () => {
    const killed: number[] = [];
    let holder = 9001;
    let available = false;
    await expect(reclaimListenPort(10100, "127.0.0.1", {
  ...
  test("foreign non-nxc claimant survives reclaim without allowlist or nxc identity", async () => {
    const killed: number[] = [];
    const dropped: number[] = [];
    await expect(reclaimListenPort(10100, "127.0.0.1", {
      timeoutMs: 80,
  ...
  test("allowlisted PID that fails nxc verify still gets killed and does not block TCP drop", async () => {
    const killed: number[] = [];
    const dropped: number[] = [];
    let alive = true;
    let available = false;
  ...
  test("dead ghost then same PID reused is killed again under killAllNxcOnPort", async () => {
    const killed: number[] = [];
