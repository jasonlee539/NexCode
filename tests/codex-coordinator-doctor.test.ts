...
beforeEach(() => {
  ...
  codexHome = mkdtempSync(join(tmpdir(), "nxc-coordinator-doctor-codex-"));
  nexcodeHome = mkdtempSync(join(tmpdir(), "nxc-coordinator-doctor-nxc-"));
...
test("doctor distinguishes unversioned, rowless, and authoritative coordinators", () => {
  ...
  const transaction = nexCodeCoordinatorTransaction(coordinatorPath);
  transaction.commit();
  transaction.close();
