...
function applyPatchTool(): NxcTool {
  return {
    name: "apply_patch",
...
function execCommandTool(): NxcTool {
  return {
    name: "exec_command",
...
describe("cursor protobuf event translation", () => {
  ...
  test("translates a non-identity wire-name mapping (Cursor display name -> Codex tool name) for multi_edit", () => {
    // Cursor advertises the Responses tool as `mcp_nexcode-responses_multi_edit`; the adapter must
    ...
    const toolCall = mcpToolCall(`mcp_nexcode-responses_${CURSOR_MULTI_EDIT_TOOL}`, {
