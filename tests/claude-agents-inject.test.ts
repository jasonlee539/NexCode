...
function tempDir(): string {
  const d = mkdtempSync(join(tmpdir(), "nxc-agents-"));
  dirs.push(d);
  return d;
}
...
function cfg(extra?: Partial<NxcConfig>): NxcConfig {
  return { port: 10100, defaultProvider: "mock", providers: {}, ...extra } as NxcConfig;
}

function generatedBodies(config: NxcConfig, dir: string): string[] {
...
describe("buildClaudeAgentDefs (devlog 070 + audit 071)", () => {
  test("roster + pinned self mark only authoritative 1M windows; name collision suffix", () => {
    const windows = { "claude-nxc-native--gpt-5.6-sol": 372_000, "claude-nxc-cursor--gpt-5.6-sol": 1_000_000 };
    const dir = tempDir();
    writeFileSync(join(dir, "settings.json"), JSON.stringify({ model: "claude-nxc-native--gpt-5.6-sol[1m]" }));
    ...
    expect(byName["nxc-gpt-5-6-sol"]!.model).toBe("claude-nxc-native--gpt-5.6-sol");
    expect(byName["nxc-gpt-5-6-sol-2"]!.model).toBe("claude-nxc-cursor--gpt-5.6-sol[1m]"); // collision suffix
    // Self pins the picker-saved default but cannot inherit an unsafe auto-context marker.
    expect(byName["nxc-self"]!.model).toBe("claude-nxc-native--gpt-5.6-sol");
  ...
  test("generated profiles retain catalog-derived 1M markers for Claude 4.6 and 4.7", async () => {
    ...
    expect(models).toEqual({
      "nxc-claude-sonnet-4-6": "claude-sonnet-4-6[1m]",
      "nxc-claude-opus-4-6": "claude-opus-4-6[1m]",
      "nxc-claude-opus-4-7": "claude-opus-4-7[1m]",
    });
  });
  ...
  test("generated profiles preserve genuine routed [1m] ids and honor provider caps", async () => {
    ...
    const windows = buildClaudeContextWindows([], catalog);
    const dir = tempDir();
    writeFileSync(join(dir, "settings.json"), JSON.stringify({ model: "claude-nxc-kimi--k3[1m]" }));
    const defs = buildClaudeAgentDefs(config, windows, dir);
    const models = Object.fromEntries(defs.map(def => [def.name, def.model]));

    expect(windows["claude-nxc-kimi--k3"]).toBe(262_144);
    expect(windows["claude-nxc-kimi--k3[1m]"]).toBe(1_048_576);
    expect(models).toEqual({
      "nxc-k3-1m": "claude-nxc-kimi--k3[1m]",
      "nxc-self": "claude-nxc-kimi--k3[1m]",
    });
    ...
    writeFileSync(join(cappedDir, "settings.json"), JSON.stringify({ model: "claude-nxc-kimi--k3[1m]" }));
    const cappedDefs = buildClaudeAgentDefs(config, cappedWindows, cappedDir);

    expect(cappedWindows["claude-nxc-kimi--k3[1m]"]).toBe(350_000);
    expect(Object.fromEntries(cappedDefs.map(def => [def.name, def.model]))).toEqual({
      "nxc-k3-1m": "claude-nxc-kimi--k3",
      "nxc-self": "claude-nxc-kimi--k3",
    });
  });

  test("marker case is honored and unknown windows keep the selector as-was", () => {
    const windows = { "claude-nxc-cursor--gpt-5.6-sol": 1_000_000 };
    const dir = tempDir();
    // Uppercase [1M] spelling is a genuine marker (the CLI matches /\[1m\]/i).
    writeFileSync(join(dir, "settings.json"), JSON.stringify({ model: "claude-nxc-cursor--gpt-5.6-sol[1M]" }));
    const defs = buildClaudeAgentDefs(cfg({ subagentModels: ["cursor/gpt-5.6-sol", "cursor/unknown-model"] }), windows, dir);
    const byName = Object.fromEntries(defs.map(d => [d.name, d]));
    expect(byName["nxc-gpt-5-6-sol"]!.model).toBe("claude-nxc-cursor--gpt-5.6-sol[1m]");
    // Incomplete metadata: no window entry -> selector preserved, never unmarked.
    expect(byName["nxc-unknown-model"]!.model).toBe("claude-nxc-cursor--unknown-model");
    expect(byName["nxc-self"]!.model).toBe("claude-nxc-cursor--gpt-5.6-sol[1M]");
  });

  test("placeholder guidance recommends haiku, never sonnet (issue #252)", () => {
    const dir = tempDir();
    writeFileSync(join(dir, "settings.json"), JSON.stringify({ model: "claude-nxc-native--gpt-5.6-sol" }));
  ...
  test("unset roster seeds the defaults; explicit [] respected; no default model -> no self", () => {
    ...
    expect(explicit.map(d => d.name)).toEqual(["nxc-self"]);
  ...
  test("rendered frontmatter quotes every scalar and parses back", () => {
    ...
    expect(body).toContain("generated-by: nexcode");
    expect(body).toContain(`nxc-route: ${def!.model}`);
  ...
  test("configured Claude Code subagent effort is rendered for roster and self definitions", () => {
    ...
    for (const effort of levels) {
      ...
      for (const def of defs) {
        ...
        expect(body).toContain(`<!-- nxc-effort: ${effort} -->`);
      }
    }
  });
  ...
  test("regeneration removes a previously configured effort when the option is cleared", () => {
    ...
    const target = join(dir, "agents", "nxc-gpt-5-6-sol.md");
  ...
  test("generated routed agents refuse the default blocked skill before its bundle expands", () => {
    ...
    expect(bodies).toHaveLength(2); // roster + nxc-self
  ...
  test("generated blocked-skill guard mirrors custom names and honors explicit opt-out", () => {
    ...
    writeFileSync(join(customDir, "settings.json"), JSON.stringify({ model: "claude-nxc-native--gpt-5.6-sol" }));
    ...
    writeFileSync(join(offDir, "settings.json"), JSON.stringify({ model: "claude-nxc-native--gpt-5.6-sol" }));
...
describe("syncClaudeAgentDefs ownership contract (audit 071 #2/#3)", () => {
  test("writes, overwrites, and prunes ONLY marker-verified nxc files", () => {
    const dir = tempDir();
    writeFileSync(join(dir, "settings.json"), JSON.stringify({ model: "claude-nxc-native--gpt-5.6-sol" }));
    const defs = buildClaudeAgentDefs(cfg({ subagentModels: ["gpt-5.6-sol"] }), {}, dir);
    ...
    writeFileSync(join(agentsDir, "nxc-custom.md"), "---\nname: nxc-custom\n---\nuser file");
    writeFileSync(join(agentsDir, "nxc-gpt-5-6-sol.md"), "user replaced this — no marker");
    const second = syncClaudeAgentDefs(buildClaudeAgentDefs(cfg({ subagentModels: [] }), {}, dir), dir)!;
    expect(second).toEqual(["nxc-self.md"]);
    const remaining = readdirSync(agentsDir).sort();
    // nxc-self rewritten; unowned nxc-custom + user-replaced sol file both preserved.
    expect(remaining).toEqual(["nxc-custom.md", "nxc-gpt-5-6-sol.md", "nxc-self.md"]);
    expect(readFileSync(join(agentsDir, "nxc-gpt-5-6-sol.md"), "utf8")).toBe("user replaced this — no marker");
  });
  ...
  test.skipIf(!canSymlink)("symlinks are never followed or pruned", () => {
    ...
    try {
      symlinkSync(victim, join(agentsDir, "nxc-linked.md"));
    } catch (err) {
    ...
    expect(readdirSync(agentsDir)).toContain("nxc-linked.md");
  });
