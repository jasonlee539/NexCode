...
describe("Codex reset-credit recovery coordinator", () => {
  ...
  test("refuses process-state reset without the repository test preload", async () => {
    const originalGuard = process.env.NXC_TEST_HOME_GUARD;
    process.env.NXC_TEST_HOME_GUARD = "0";
    try {
      ...
      if (originalGuard === undefined) delete process.env.NXC_TEST_HOME_GUARD;
      else process.env.NXC_TEST_HOME_GUARD = originalGuard;
    }
  });
