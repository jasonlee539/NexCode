...
function config(overrides: {
  ...
  const clientIntegrations: NonNullable<NxcConfig["clientIntegrations"]> = {};
...
function harness(initial: NxcConfig) {
  let current = initial;
  const grokActions: Array<{ action: "strip" | "sync"; config: NxcConfig; hostname?: string }> = [];
...
async function runSpawnedBranch(h: ReturnType<typeof harness>, next: NxcConfig): Promise<void> {
  const stale = h.deps.loadConfig();
  void stale;
  h.flip(next);
