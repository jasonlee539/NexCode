...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), "nxc-write-lock-"));
  cleanup.push(root);
...
describe("two real processes contend for one lock", () => {
  ...
  function spawnChild(payload: Record<string, unknown>) {
    return Bun.spawn(["bun", childPath], {
      env: { ...process.env, CODEX_HOME: codexHome, NXC_LOCK_CHILD_PAYLOAD: JSON.stringify(payload) },
      stdout: "pipe",
      stderr: "pipe",
    });
  }
  ...
  function spawnChildWithEnv(payload: Record<string, unknown>, env: Record<string, string>) {
    return Bun.spawn(["bun", childPath], {
      env: {
        ...
        ...env,
        NXC_LOCK_CHILD_PAYLOAD: JSON.stringify(payload),
      },
      stdout: "pipe",
      stderr: "pipe",
    });
