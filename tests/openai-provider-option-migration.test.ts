...
import type { NxcConfig, NxcProviderConfig, ProviderCostOverlay } from "../src/types";

const forward: NxcProviderConfig = {
  adapter: "openai-responses",
  baseUrl: "https://chatgpt.com/backend-api/codex",
  authMode: "forward",
};

function cfg(overrides: Partial<NxcConfig> = {}): NxcConfig {
  return {
    port: 10100,
...
describe("OpenAI provider option migration matrix", () => {
  ...
  test("leaves unknown passthrough values unchanged and emits only their paths", () => {
    ...
    const result = projectOpenAiTierMigration(input);
    expect((result.config as NxcConfig & { customMigrationNote: string }).customMigrationNote).toContain("openai-multi");
    expect((result.config as NxcConfig & { nestedUnknown: { model: string } }).nestedUnknown.model).toBe("openai-multi/gpt-unknown");
