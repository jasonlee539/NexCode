...
// 260702_windows-deploy-stability close: the nxc.cmd shell-less restart (A), F9 systemd no-DBUS
...
// nxc-launcher-source.test.ts / service.test.ts).
...
describe("update-job restart avoids the shell-less .cmd EINVAL (Windows, bun/source)", () => {
  ...
  test("no nxc.cmd shim is spawned for restart", () => {
    expect(src).not.toContain('"nxc.cmd"');
    expect(src).not.toMatch(/function nxcBin/);
  });
  ...
  test("service update restart bakes NXC_BAKE_PORT so wrappers hard-pin the captured port", () => {
    expect(src).toContain("NXC_BAKE_PORT");
  ...
  test("Windows GUI update worker is launched without inheriting the proxy LISTEN socket", () => {
    ...
    // Foreign listeners must stay fail-closed; npm rename is covered by nxc identity.
    ...
    expect(src).toContain('process.env.NXC_SERVICE === "1" && refreshRegisters');
