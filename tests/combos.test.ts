...
function rrConfig(stickyLimit: number, weights: number[]): NxcConfig {
...
function successfulPicks(config: NxcConfig, count: number): string[] {
...
async function withTempHome<T>(run: (dir: string) => Promise<T> | T): Promise<T> {
  ...
  const dir = mkdtempSync(join(tmpdir(), "nxc-combos-"));
...
async function comboApiRaw(config: NxcConfig, method: string, path: string, body: string): Promise<Response | null> {
  const req = new Request(`http://localhost${path}`, {
    method,
...
describe("combo validation and normalization", () => {
  ...
  test("normalizes valid values and returns defensive default efforts", () => {
    ...
    const corrupt = baseConfig() as NxcConfig & { combos: Record<string, { defaultEffort: string; targets: [] }> };
  ...
  test("preserves a physical provider named combo while no combos are configured", () => {
    ...
    const inheritedProviders = Object.create({ combo: config.providers.combo }) as NxcConfig["providers"];
...
describe("persisted combo config parity", () => {
  test("reports malformed maps and exact domain messages for policy-independent rows", async () => {
    await withTempHome(() => {
      ...
      const rows: Array<{ id: string; combo: unknown; providers?: NxcConfig["providers"] }> = [
