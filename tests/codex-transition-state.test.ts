...
beforeEach(() => {
  ...
  codexHome = mkdtempSync(join(tmpdir(), "nxc-transition-state-codex-home-"));
  nexcodeHome = mkdtempSync(join(tmpdir(), "nxc-transition-state-nexcode-home-"));
...
test("a missing database with legacy JSON transition fields is legacy-ambiguous", () => {
  const integrations = join(nexcodeHome, "integrations");
...
test("the row validator refuses every whitespace-only txId", () => {
  ...
  for (const [label, txId] of trimRemovedCodePoints) {
    ...
    expect(() => nexCodeCoordinatorTransaction(coordinatorPath), label)
...
test("the opaque coordinator capability cannot reach a second connection", () => {
  ...
  const controller = nexCodeCoordinatorTransaction(coordinatorPath);
  try {
    expect(() => {
      const second = nexCodeCoordinatorTransaction(coordinatorPath);
      second.close();
    }).toThrow();
  } finally {
    controller.close();
