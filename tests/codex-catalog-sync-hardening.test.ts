...
/** Row shape NexCode itself generates for routed models (ownership signature). */
function nxcAuthoredEntry(slug: string, priority: number): Record<string, unknown> {
  return {
    ...routedEntry(slug, priority),
    description: `Routed via nexcode → ${slug} (test-owner).`,
  };
}

/** Legacy generated shape (June–July 2026): provider name, not the full slug. */
function nxcLegacyAuthoredEntry(slug: string, priority: number): Record<string, unknown> {
  const provider = slug.slice(0, slug.indexOf("/"));
  return {
    ...routedEntry(slug, priority),
    description: `Routed via nexcode → ${provider} (test-owner).`,
  };
}

describe("Codex catalog sync hardening", () => {
  let codexHome: string;
  let nexcodeHome: string;

  beforeEach(() => {
    codexHome = mkdtempSync(join(tmpdir(), "nxc-sync-home-"));
    nexcodeHome = mkdtempSync(join(tmpdir(), "nxc-sync-nxc-"));
  });

  afterEach(() => {
    if (existsSync(codexHome)) rmSync(codexHome, { recursive: true, force: true });
    if (existsSync(nexcodeHome)) rmSync(nexcodeHome, { recursive: true, force: true });
  });
  ...
  test("Gap B: drops legacy OpenAI-family natives but keeps supported + user natives", () => {
    ...
    const r = runScript(codexHome, nexcodeHome, `
  ...
  test("native-alias suppression preserves authoritative metadata on account-qualified rows", () => {
    ...
    expect(rows.find(row => row.slug === "team/gpt-5.6-sol")).toMatchObject({
      ...
      model_messages: { instructions_template: "Native Sol instructions" },
      tool_mode: "code_mode_only",
      nexcode_catalog_kind: "account-selector-v1",
    });
  });
  ...
  test("account rows reconcile idempotently and independently from authoritative provider empties", () => {
    const catalogPath = join(codexHome, "catalog.json");
    const firstCatalogPath = join(nexcodeHome, "first-catalog.json");
    writeFileSync(join(codexHome, "config.toml"), 'model_catalog_json = "catalog.json"\n', "utf8");
    ...
    writeFileSync(catalogPath, JSON.stringify({
      models: [
        ...
        },
        {
          ...
          nexcode_catalog_kind: accountMarker,
        },
      ],
    }, null, 2) + "\n");
  ...
  test("explicit Codex-forward Daybreak survives sync with Sol metadata while account picker is off", () => {
    ...
    expect(daybreak).toMatchObject({
      ...
      nexcode_catalog_kind: "custom-model-v1",
    });
  ...
  test("a live provider row shadowed by an account selector warns once per runtime generation", () => {
    ...
    expect(rows.filter(row => row.slug === "team/gpt-5.5")).toEqual([
      expect.objectContaining({ nexcode_catalog_kind: "account-selector-v1" }),
    ]);
  });
  ...
  test("default catalog path merges from disk instead of replacing it with bundled rows", () => {
    ...
    const codexCliPath = createCodexCatalogFixture(nexcodeHome);
    const r = runScript(codexHome, nexcodeHome, `
      const { syncCatalogModels } = require("./src/codex/catalog");
      syncCatalogModels({ providers: {} }).then(res => console.log(JSON.stringify(res)));
  ...
  /*
   * #855. Deleting a provider must remove the rows NexCode generated for it
   ...
   */
  test("drops NexCode-authored rows of a deleted provider, keeps foreign rows", () => {
    const catalogPath = join(codexHome, "catalog.json");
    writeFileSync(join(codexHome, "config.toml"), 'model_catalog_json = "catalog.json"\n', "utf8");
    writeFileSync(catalogPath, JSON.stringify({
      models: [
        nativeEntry("gpt-5.5", 0),
        nxcAuthoredEntry("future-grok/old-model", 5),
  ...
  test("authoritative empty providers drop their own rows and deleted-provider ghosts", () => {
    ...
    writeFileSync(catalogPath, JSON.stringify({
      models: [
        nativeEntry("gpt-5.5", 0),
        nxcAuthoredEntry("future-grok/old-model", 5),
        nxcAuthoredEntry("openai/keep-model", 6),
  ...
  test("a degraded provider preserves only its own prior rows", () => {
    ...
    writeFileSync(catalogPath, JSON.stringify({
      models: [
        nativeEntry("gpt-5.5", 0),
        nxcAuthoredEntry("offline/keep-model", 5),
        nxcAuthoredEntry("offline/disabled-model", 6),
        nxcAuthoredEntry("removed/ghost", 7),
  ...
  test("drops legacy-signature ghost rows in both gather branches", () => {
    ...
    writeFileSync(catalogPath, JSON.stringify({
      models: [
        nativeEntry("gpt-5.5", 0),
        nxcLegacyAuthoredEntry("future-grok/legacy-model", 5),
        routedEntry("cursor/composer-2.5", 6),
      ],
    }, null, 2) + "\n");

    // Partial-gather branch: another provider is configured and gathers rows.
    const partial = runScript(codexHome, nexcodeHome, `
      const { syncCatalogModels } = require("./src/codex/catalog");
      syncCatalogModels({
        providers: {
          openai: {
    ...
    const empty = runScript(codexHome, nexcodeHome, `
      const { syncCatalogModels } = require("./src/codex/catalog");
      syncCatalogModels({ providers: {} }).then(res => console.log(JSON.stringify(res)));
    `);
  ...
  test("an identical resync leaves the catalog file untouched, a real change still writes", () => {
    ...
    // verdict withholds nexcode's model guidance for the rest of that Codex's
