...
describe("ZCode client config", () => {
  test("adds only provider.nexcode in ZCode's observed v2 schema", () => {
    ...
    expect(provider.name).toBe("NexCode");
  ...
  test("native JSON round-trips and never carries a credential", () => {
    ...
    const withKey = { ...CONFIG, apiKeys: [{ key: sentinel }] } as NxcConfig;
  ...
  test("the contribution owns exactly the provider.nexcode path", () => {
