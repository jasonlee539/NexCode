...
beforeEach(() => {
  ...
  testRoot = realpathSync.native(mkdtempSync(join(tmpdir(), "nxc-catalog-writer-")));
  ...
  for (const path of [codexHome, otherCodexHome, targetDir, join(testRoot, "nexcode-home")]) {
    mkdirSync(path, { recursive: true });
  }
  process.env.CODEX_HOME = codexHome;
  process.env.NEXCODE_HOME = join(testRoot, "nexcode-home");
});
