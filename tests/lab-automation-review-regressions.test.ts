...
function tempHome(): string {
  const dir = join(tmpdir(), `nxc-lab-cl08-review-${process.pid}-${Math.random().toString(16).slice(2)}`);
...
function providerConfig(baseUrl = "https://api.example.com/v1", providerName = "fixture-provider"): NxcConfig {
  return {
    providers: {
      [providerName]: {
...
function twoRouteConfig(): NxcConfig {
  return {
    providers: {
      "route-a": {
        adapter: "openai-responses",
