 ...
 */
const canSymlink = (() => {
  const probeDir = mkdtempSync(join(tmpdir(), "nxc-state-symlink-probe-"));
  try {
...
describe("Responses previous_response_id state", () => {
  // Sandbox NEXCODE_HOME: the state store now snapshots to disk, and these tests must never
  // touch the real ~/.nexcode.
  let home: string;
  const priorHome = process.env["NEXCODE_HOME"];

  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "nxc-state-test-"));
  ...
  afterEach(() => {
    ...
    if (priorHome === undefined) delete process.env["NEXCODE_HOME"];
    else process.env["NEXCODE_HOME"] = priorHome;
  });
  ...
  test("validates reserved continuation ownership separately from provider spill state", () => {
    ...
    const valid = writeResponseSpillDurably("resp_valid_spill_owner", {
      createdAt: Date.now(),
      items: ["valid"],
      providers: { __nxcOwner: owner, kiro: { conversationId: "kiro-valid" } },
    });
  ...
  test("persistent snapshot I/O failure does not schedule background retry passes", async () => {
    ...
    process.env["NEXCODE_HOME"] = blockedHome;
  ...
  test("recovers only old response-state temps owned by dead processes", () => {
    ...
    const stale = join(home, `responses-state.json.nxc.${deadPid}.1.tmp`);
    const live = join(home, "responses-state.json.nxc.5252.2.tmp");
    const current = join(home, `responses-state.json.nxc.${process.pid}.3.tmp`);
    const young = join(home, "responses-state.json.nxc.6262.4.tmp");
    const unrelated = join(home, "responses-state.json.nxc.7272.tmp");
    const directory = join(home, "responses-state.json.nxc.8282.5.tmp");
  ...
  test.skipIf(!canSymlink)("load sweeps stale temps in a symlinked snapshot's real directory", () => {
    ...
    const realDir = mkdtempSync(join(tmpdir(), "nxc-state-real-"));
    ...
    const stranded = join(realDir, `responses-state.json.nxc.${deadPid}.1.tmp`);
  ...
  test("stale temp recovery is best-effort when unlink fails", () => {
    const deadPid = process.pid === 4242 ? 4243 : 4242;
    const path = join(home, `responses-state.json.nxc.${deadPid}.1.tmp`);
  ...
  test("stale temp recovery stops at injectable enumeration and cleanup caps", () => {
    const old = new Date(Date.now() - 60 * 60 * 1_000);
    const first = "responses-state.json.nxc.7001.1.tmp";
    const second = "responses-state.json.nxc.7002.2.tmp";
  ...
  test("boot floor reclaims a pre-boot temp whose pid has been reused", () => {
    ...
    const old = new Date(Date.now() - 60 * 60 * 1_000);
    const path = join(home, "responses-state.json.nxc.9101.1.tmp");
    writeFileSync(path, "private state");
    utimesSync(path, old, old);
  ...
  test("the 15-minute grace outranks the boot floor", () => {
    ...
    const path = join(home, "responses-state.json.nxc.9102.1.tmp");
    writeFileSync(path, "private state");
  ...
  test("this process's own temps are never reclaimed, even before boot", () => {
    const old = new Date(Date.now() - 60 * 60 * 1_000);
    const path = join(home, `responses-state.json.nxc.${process.pid}.1.tmp`);
    writeFileSync(path, "private state");
    utimesSync(path, old, old);
  ...
  test("a future or non-finite boot time disables the floor instead of trusting it", () => {
    const old = new Date(Date.now() - 60 * 60 * 1_000);
    const path = join(home, "responses-state.json.nxc.9103.1.tmp");
    writeFileSync(path, "private state");
    utimesSync(path, old, old);
  ...
  test("a temp another process already removed counts as reclaimed, not failed", () => {
    ...
    const old = new Date(Date.now() - 60 * 60 * 1_000);
    const path = join(home, "responses-state.json.nxc.9104.1.tmp");
    writeFileSync(path, "private state");
    utimesSync(path, old, old);
  ...
  test("a dry run reports exactly what a reclaim then removes", () => {
    ...
    const stale = join(home, `responses-state.json.nxc.${deadPid}.1.tmp`);
    const live = join(home, "responses-state.json.nxc.5252.2.tmp");
    const young = join(home, "responses-state.json.nxc.6262.3.tmp");
  ...
  test("a dry run is not truncated by the cleanup budget", () => {
    ...
    const old = new Date(Date.now() - 60 * 60 * 1_000);
    const names = [7301, 7302, 7303].map(pid => `responses-state.json.nxc.${pid}.1.tmp`);
  ...
  test("the periodic scan stops at its wall-clock deadline", () => {
    const old = new Date(Date.now() - 60 * 60 * 1_000);
    const names = ["responses-state.json.nxc.9201.1.tmp", "responses-state.json.nxc.9202.2.tmp"];
    for (const name of names) {
      const path = join(home, name);
      writeFileSync(path, "private state");
      utimesSync(path, old, old);
  ...
  test("a truncated scan closes the directory iterator", () => {
    const old = new Date(Date.now() - 60 * 60 * 1_000);
    const names = ["responses-state.json.nxc.9301.1.tmp", "responses-state.json.nxc.9302.2.tmp"];
    for (const name of names) {
      const path = join(home, name);
      writeFileSync(path, "private state");
      utimesSync(path, old, old);
...
describe("Responses state admission boundary (oversized direct-spill)", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "nxc-state-admission-"));
    process.env["NEXCODE_HOME"] = home;
    clearResponseStateMemoryForTests();
  });
