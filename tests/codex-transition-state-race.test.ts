...
import { nexCodeCoordinatorTransaction } from "../src/codex/transition-state";
import {
...
const transitionProbe = `
  ...
  const payload = JSON.parse(process.env.NXC_TEST_PAYLOAD);
...
interface Sandbox {
  root: string;
  codexHome: string;
  nexcodeHomes: [string, string];
  coordinatorPath: string;
}

function createSandbox(label: string): Sandbox {
  const root = mkdtempSync(join(tmpdir(), `nxc-transition-race-${label}-`));
  const codexHome = join(root, "codex");
  const nexcodeHomes: [string, string] = [join(root, "nxc-a"), join(root, "nxc-b")];
  mkdirSync(codexHome);
  for (const path of nexcodeHomes) mkdirSync(path);
  const coordinatorPath = resolveCodexCoordinatorDatabasePath(
    resolveEffectiveUserIdentity(),
    realpathSync.native(codexHome),
  );
  return { root, codexHome, nexcodeHomes, coordinatorPath };
}
...
function spawnProbe(sandbox: Sandbox, nexcodeHome: string, payload: Record<string, unknown>) {
  return Bun.spawn([process.execPath, "--eval", transitionProbe], {
    env: {
      ...process.env,
      CODEX_HOME: sandbox.codexHome,
      NEXCODE_HOME: nexcodeHome,
      NXC_TEST_PAYLOAD: JSON.stringify({ ...payload, codexHome: sandbox.codexHome }),
    },
    stdin: "ignore",
    stdout: "pipe",
    stderr: "pipe",
...
test("different NEXCODE_HOME claimants advance the row under one CODEX_HOME", async () => {
  const sandbox = createSandbox("shared-codex-home");
  try {
    const first = await collectProbe(spawnProbe(sandbox, sandbox.nexcodeHomes[0], {
      action: "begin",
    ...
    const integrations = join(sandbox.nexcodeHomes[1], "integrations");
    mkdirSync(integrations);
    writeFileSync(join(integrations, "codex.json"), JSON.stringify({
      version: 1,
      nativeGeneration: 91,
      currentTxId: "nexcode-home-local-claimant",
      history: { status: "pending", txId: "nexcode-home-local-claimant" },
    }));

    const second = await collectProbe(spawnProbe(sandbox, sandbox.nexcodeHomes[1], {
      action: "begin",
    ...
    const observed = await collectProbe(spawnProbe(sandbox, sandbox.nexcodeHomes[0], {
      action: "read",
    }));
...
test("a locked coordinator returns the exact typed busy outcome", async () => {
  ...
  let controller: ReturnType<typeof nexCodeCoordinatorTransaction> | undefined;
  try {
    const initialized = await collectProbe(spawnProbe(sandbox, sandbox.nexcodeHomes[0], {
      action: "read",
    }));
    expect(initialized.outcome?.kind).toBe("ready");

    controller = nexCodeCoordinatorTransaction(sandbox.coordinatorPath);
    const blocked = await collectProbe(spawnProbe(sandbox, sandbox.nexcodeHomes[1], {
      action: "read",
    }));
...
test("an unsafe coordinator path returns the exact typed unsafe-path outcome", async () => {
  ...
  try {
    ...
    const refused = await collectProbe(spawnProbe(sandbox, sandbox.nexcodeHomes[0], {
      action: "read",
    }));
