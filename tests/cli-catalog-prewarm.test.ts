...
describe("catalog prewarm on handleStart bind", () => {
  test("scheduleCatalogPrewarm calls gatherRoutedModels(loadConfig()) once", async () => {
    const config = { port: 9_001, providers: {}, defaultProvider: "fixture" } as NxcConfig;
    const gatherRoutedModels = mock(async (_config: NxcConfig) => []);
  ...
  test("scheduleCatalogPrewarm swallows gather failures", async () => {
    ...
    scheduleCatalogPrewarm({
      loadConfig: () => ({ port: 9_002, providers: {}, defaultProvider: "fixture" }) as NxcConfig,
  ...
  test("catalog busy maps startup prewarm to warn-skip", async () => {
    ...
    try {
      scheduleCatalogPrewarm({
        loadConfig: () => ({ port: 9_003, providers: {}, defaultProvider: "fixture" }) as NxcConfig,
