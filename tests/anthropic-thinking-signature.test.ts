...
import { encodeReasoningEnvelope, decodeReasoningEnvelope, NXC_REASONING_PREFIX } from "../src/responses/reasoning-envelope";
import type { AdapterEvent, NxcProviderConfig, NxcThinkingContent } from "../src/types";
...
describe("bridge nxcr1 envelope emission", () => {
...
describe("parser nxcr1 decode + anthropic replay", () => {
  test("reasoning input with nxcr1 envelope restores the real signature", async () => {
    ...
    const thinking = (assistant as unknown as { content: NxcThinkingContent[] }).content.find(p => p.type === "thinking");
  ...
  test("native (non-nxcr1) encrypted_content keeps the placeholder signature", async () => {
    const parsed = parseRequest({
      model: "anthropic/claude-x",
      input: [
...
describe("passthrough scrub of nxcr1 envelopes", () => {
  test("sanitize strips nxcr1 encrypted_content even with empty content", async () => {
    ...
    const body = {
      model: "gpt-5.5",
      input: [
        { type: "reasoning", id: "rs_1", summary: [], encrypted_content: NXC_REASONING_PREFIX + Buffer.from(JSON.stringify({ sig: "RealSig1234567890==" })).toString("base64") },
      ],
    };
    // Build the outgoing request the adapter would send; the nxcr1 envelope must be stripped.
    const req = await adapter.buildRequest({ _rawBody: body, model: "gpt-5.5", messages: [], options: {} } as never) as { body?: string };
    expect(req.body ?? "").not.toContain(NXC_REASONING_PREFIX);
