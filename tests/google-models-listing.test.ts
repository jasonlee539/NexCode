...
const originalNexcodeHome = process.env.NEXCODE_HOME;

afterEach(() => {
  globalThis.fetch = originalFetch;
  clearModelCache();
  if (originalNexcodeHome === undefined) delete process.env.NEXCODE_HOME;
  else process.env.NEXCODE_HOME = originalNexcodeHome;
});

function configWith(name: string, prov: Partial<NxcProviderConfig>): NxcConfig {
  return {
    providers: { [name]: prov },
...
describe("buildModelsRequest google routing", () => {
  test("ai-studio google uses x-goog-api-key + /v1beta/models", () => {
    const prov = { adapter: "google", authMode: "key", baseUrl: "https://generativelanguage.googleapis.com" } as NxcProviderConfig;
  ...
  test("custom google-adapter provider without googleMode defaults to ai-studio", () => {
    const prov = { adapter: "google", authMode: "key", baseUrl: "https://example.com" } as NxcProviderConfig;
  ...
  test("Antigravity uses its authenticated CCA model-discovery RPC", () => {
    ...
    const prov = { adapter: "google", authMode: "oauth", baseUrl: "https://daily-cloudcode-pa.googleapis.com", liveModels: true } as NxcProviderConfig;
  ...
  test("google-vertex without googleMode resolves vertex via registry, not ai-studio", () => {
    const prov = { adapter: "google", authMode: "key", baseUrl: "https://aiplatform.googleapis.com" } as NxcProviderConfig;
...
describe("Antigravity live model discovery", () => {
  test("uses the CCA agent list and applies CCA metadata", async () => {
    const home = mkdtempSync(join(tmpdir(), "nxc-antigravity-discovery-"));
  ...
  test("degrades malformed CCA agent IDs to the configured static catalog", async () => {
    const home = mkdtempSync(join(tmpdir(), "nxc-antigravity-malformed-discovery-"));
    process.env.NEXCODE_HOME = home;
    writeFileSync(join(home, "auth.json"), JSON.stringify({
      "google-antigravity": {
        activeAccountId: "active",
  ...
  test("does not register wire mappings from a stale CCA discovery", async () => {
    const home = mkdtempSync(join(tmpdir(), "nxc-antigravity-stale-discovery-"));
    process.env.NEXCODE_HOME = home;
    writeFileSync(join(home, "auth.json"), JSON.stringify({
      "google-antigravity": {
        activeAccountId: "active",
