...
describe("nxc claude env assembly", () => {
  test("injects base URL, discovery flag and model slots — NO auth token by default (subscription mode)", () => {
    const env = buildClaudeEnv(cfg({
      claudeCode: { model: "claude-nxc-gemini--gemini-3-pro", smallFastModel: "gemini/gemini-3-flash" },
    }), 10123, {}, {}, AUTH_PRESENT);
    ...
    expect(env.ANTHROPIC_MODEL).toBe("claude-nxc-gemini--gemini-3-pro");
  ...
  test("configured API key becomes the auth token (admission required)", () => {
    const env = buildClaudeEnv(cfg({
      apiKeys: [{ id: "1", name: "main", key: "sk-nxc-123", createdAt: "2026-01-01" }],
    }), 10100, {});
    expect(env.ANTHROPIC_AUTH_TOKEN).toBe("sk-nxc-123");
  });
  ...
  test("proxy-owned authentication sets CLAUDE_CODE_PROVIDER_MANAGED_BY_HOST=1", () => {
    ...
    expect(proxy.ANTHROPIC_AUTH_TOKEN).toBe("nexcode-proxy");
...
describe("nxc claude Windows launch (devlog 260715_cross_platform_audit/020)", () => {
