...
test("one corrupt origin marker does not discard later valid purge provenance", () => {
  const home = configDir("nxc-cl10-origin-salvage-");
  const publisherA = configDir("nxc-cl10-origin-publisher-a-");
  const publisherB = configDir("nxc-cl10-origin-publisher-b-");
...
test("origin pressure preserves provenance while the matching local export exists", () => {
  const home = configDir("nxc-cl10-origin-export-retain-");
...
test("operator import of an own verified bundle restores missing local-origin provenance", () => {
  const home = configDir("nxc-cl10-origin-rehydrate-");
  const bundle = signedBundle(home);
...
test("operator import of a third-party verified bundle does not create local-origin provenance", () => {
  const home = configDir("nxc-cl10-origin-third-party-home-");
  const publisher = configDir("nxc-cl10-origin-third-party-publisher-");
  signedBundle(home);
...
test("failed own-origin commit rolls back a newly imported community copy", () => {
  const home = configDir("nxc-cl10-origin-rollback-");
  const bundle = signedBundle(home);
  const dir = labPublicOriginDir(home);
...
test.skipIf(process.platform === "win32")(
  ...
  () => {
    const home = configDir("nxc-cl10-origin-recovery-");
    const publisher = configDir("nxc-cl10-community-recovery-publisher-");
    const identity = {
...
test("V1 revocations are bounded to one already-verified anchor bundle", () => {
  const publisher = configDir("nxc-cl10-revocation-anchor-");
