...
async function putSidecarSettings(config: NxcConfig, webSearch: Record<string, unknown>): Promise<Response> {
  const url = new URL("http://localhost/api/sidecar-settings");
...
describe("sidecar-settings webSearch.streamRoutedModelOutput", () => {
  ...
  beforeEach(() => {
    ...
    isolatedHome = mkdtempSync(join(tmpdir(), "nxc-sidecar-ws-stream-"));
