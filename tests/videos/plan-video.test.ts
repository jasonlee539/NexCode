...
function makeParsed(): NxcParsedRequest {
  return { stream: true, context: { messages: [] } } as unknown as NxcParsedRequest;
}

function makeProvider(host: string): NxcProviderConfig {
  return { baseUrl: `https://${host}`, authMode: "key", apiKey: "other-key" } as unknown as NxcProviderConfig;
}

describe("planVideoBridge", () => {
  test("returns undefined when videoBridgeEnabled is not true", async () => {
    const config = makeConfig({ images: { videoBridgeEnabled: false } } as unknown as NxcConfig);
  ...
  test("returns undefined when videoBridgeEnabled is missing", async () => {
    const config = makeConfig({ images: {} } as unknown as NxcConfig);
    const plan = await planVideoBridge(config, makeParsed(), makeProvider("api.anthropic.com"));
    expect(plan).toBeUndefined();
  });

  test("returns plan when enabled with valid xAI provider", async () => {
    const config = makeConfig({ images: { videoBridgeEnabled: true } } as unknown as NxcConfig);
    const plan = await planVideoBridge(config, makeParsed(), makeProvider("api.anthropic.com"));
  ...
  test("respects custom videoBridgeModel", async () => {
    const config = makeConfig({ images: { videoBridgeEnabled: true, videoBridgeModel: "custom-video-model" } } as unknown as NxcConfig);
    const plan = await planVideoBridge(config, makeParsed(), makeProvider("api.anthropic.com"));
    expect(plan).toBeDefined();
