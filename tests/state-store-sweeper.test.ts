...
import {
  ...
  setNxcStartProcessProbeForTests,
  sweepDeadNxcStartProcessCache,
} from "../src/config";
...
// tick, so any test that drives a real tick performs filesystem work under NEXCODE_HOME.
// Without this isolation the suite would scan (and could unlink inside) a developer's real
// ~/.nexcode as a side effect of a unit test.
let sweeperHome: string;
let previousSweeperHome: string | undefined;

beforeEach(() => {
  previousSweeperHome = process.env.NEXCODE_HOME;
  sweeperHome = mkdtempSync(join(tmpdir(), "nxc-sweeper-home-"));
  process.env.NEXCODE_HOME = sweeperHome;
  resetStateStoreSweeperForTests();
  resetAppOwnedMemoryForTests();
  clearResponseStateMemoryForTests();
  __resetAntigravityReplayCache();
...
afterEach(() => {
  ...
  __resetAntigravityReplayCache();
  setNxcStartProcessCacheForTests([]);
  setNxcStartProcessProbeForTests(null);
  if (previousSweeperHome === undefined) delete process.env.NEXCODE_HOME;
  else process.env.NEXCODE_HOME = previousSweeperHome;
...
describe("state-store sweeper", () => {
  ...
  test("partial reconcile failure keeps live-key writes accepted for new owners", async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), "nxc-sweeper-gcp-live-"));
  ...
  test("provider-quota late completion cannot resurrect a deleted provider or account row", async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), "nxc-sweeper-quota-"));
    ...
    try {
      ...
      const staleConfig = { ...config, providers: { ...config.providers } } as NxcConfig;
  ...
  test("PID liveness probes at most 64, rotates, and deletes only ESRCH", () => {
    setNxcStartProcessCacheForTests(Array.from({ length: 70 }, (_, index) => [index + 1, true] as const));
    const probed: number[] = [];
    setNxcStartProcessProbeForTests(pid => {
      probed.push(pid);
      if (pid === 1) {
    ...
    expect(sweepDeadNxcStartProcessCache()).toBe(1);
    expect(probed).toHaveLength(64);
    expect(nxcStartProcessCacheSizeForTests()).toBe(69);
    probed.length = 0;
    sweepDeadNxcStartProcessCache();
  ...
  test("PID liveness discards invalid keys without probing them", () => {
    setNxcStartProcessCacheForTests([[0, true], [-1, true], [1.5, true], [42, true]]);
    const probed: number[] = [];
    setNxcStartProcessProbeForTests(pid => { probed.push(pid); });

    expect(sweepDeadNxcStartProcessCache()).toBe(3);
    expect(probed).toEqual([42]);
    expect(nxcStartProcessCacheSizeForTests()).toBe(1);
  });
});
