...
describe("MiniMax Code client config", () => {
  test("adds only custom_provider.nexcode, syncs model capabilities, and never changes the selected model", () => {
  ...
  test("launcher reads only the managed provider destination", () => {
    ...
    expect(mcodeNexCodeBaseUrl(text)).toBe("http://127.0.0.1:10100");
    expect(mcodeNexCodeBaseUrl("not: [valid")).toBeNull();
  });
});
...
describe("MiniMax CLI wrapper", () => {
    ...
    const upstream = Bun.serve({
      ...
      async fetch(req) {
        ...
        seen.push({
          ...
          dedicated: req.headers.get("x-nexcode-api-key"),
    ...
    try {
      const messages = await fetch(`${bridge.baseUrl}/anthropic/v1/messages?beta=true`, {
        ...
        headers: {
          ...
          "x-nexcode-api-key": "must-not-forward",
  ...
  test("returns 502 when the selected NexCode proxy address is unavailable", async () => {
  ...
  test("bounds the wait for response headers from a stalled NexCode proxy", async () => {
    const upstream = Bun.serve({
      hostname: "127.0.0.1",
      port: 0,
      async fetch() {
