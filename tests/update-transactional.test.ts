...
function writeTree(packageDir: string, version: string): void {
  ...
  writeFileSync(join(packageDir, "bin", "nxc.mjs"), "#!/usr/bin/env node\n" + "x".repeat(2048));
...
function stagingNpm(version: string, opts: { fail?: boolean; truncate?: boolean } = {}) {
  return (args: string[]) => {
    ...
    if (opts.truncate) rmSync(join(staged, "bin", "nxc.mjs"));
    return { status: 0 };
  };
}
...
describe("#1942 transactional update", () => {
  ...
  beforeEach(() => {
    scopeDir = mkdtempSync(join(tmpdir(), "nxc-tx-update-"));
    packageDir = join(scopeDir, "nexcode");
  ...
  test("manifest verifies a complete tree and rejects a truncated one", () => {
    ...
    rmSync(join(packageDir, "bin", "nxc.mjs"));
  ...
  test("double fault writes the recovery marker with a one-line restore (D4 last row)", () => {
    ...
    const marker = JSON.parse(readFileSync(join(scopeDir, ".nxc-recovery.json"), "utf8"));
    expect(marker.restore).toContain("nexcode");
  });
  ...
  test("boot probe restores the backup over a broken live tree (D4 power-loss rows)", () => {
    ...
    const backupRoot = join(scopeDir, ".nxc-backup-2026");
    mkdirSync(backupRoot, { recursive: true });
    const { renameSync } = require("node:fs");
    renameSync(packageDir, join(backupRoot, "nexcode"));
  ...
  test("boot probe reaps stale backups when live is healthy", () => {
    const backupRoot = join(scopeDir, ".nxc-backup-2026");
    mkdirSync(join(backupRoot, "nexcode"), { recursive: true });
    writeFileSync(join(backupRoot, "nexcode", "package.json"), JSON.stringify({ version: "0.9.0" }));
    const probe = bootRestoreProbe(packageDir);
