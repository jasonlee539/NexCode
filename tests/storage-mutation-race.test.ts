...
beforeEach(async () => {
  ...
  resetArchivedCleanupJobForTests();
  resetStorageMutationCoordinatorForTests();
  isolatedCodexHome = installIsolatedCodexHome("nxc-storage-mutation-race-codex-");
  testDir = mkdtempSync(join(tmpdir(), "nxc-storage-mutation-race-"));
...
describe("storage mutation coordinator", () => {
  ...
  test("manual cleanup and restore are rejected while policy job holds the shared mutation slot", async () => {
    ...
    try {
      ...
      // Windows holding NEXCODE_HOME (SQLite/job handles) and afterEach rmSync fails EBUSY.
