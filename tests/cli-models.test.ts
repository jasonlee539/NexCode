...
import type { NxcProviderConfig } from "../src/types";
...
function freshConfig(extra?: Record<string, unknown>) {
  const dir = mkdtempSync(join(tmpdir(), "nxc-models-"));
  const config = {
    port: 10100,
    providers: {
      openai: {
...
describe("nxc models", () => {
  test("models lists all provider models", () => {
    ...
    try {
      const result = runCli(["models"], { NEXCODE_HOME: dir });
  ...
  test("models --provider filters to one provider", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["models", "--provider", "test"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
      expect(result.stdout).toContain("test-model-1");
  ...
  test("models --provider rejects unknown provider", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["models", "--provider", "nonexistent"], { NEXCODE_HOME: dir });
  ...
  test("models --json returns valid JSON", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["models", "--json"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
  ...
  test("models --provider X --json combines flags", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["models", "--provider", "test", "--json"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
      const parsed = JSON.parse(result.stdout);
  ...
  test("models --help prints usage", () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain("nxc models");
  });
...
describe("nxc models richer metadata", () => {
  test("models --json includes contextWindow and inputModalities", () => {
    const dir = mkdtempSync(join(tmpdir(), "nxc-models-rich-"));
    const config = {
      port: 10100,
      providers: {
        test: {
  ...
  test("a family entry classifies its tagged siblings, as the runtime does", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "nxc-models-family-"));
    const provider = {
      adapter: "openai-chat",
      baseUrl: "http://localhost:8080/v1",
      allowPrivateNetwork: true,
    ...
    try {
      ...
      expect(isModelTextOnly(provider as unknown as NxcProviderConfig, "gpt-oss:120b")).toBe(true);
  ...
  test("a noVision family entry beats an exact modality entry, as the runtime does", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "nxc-models-novision-"));
    const provider = {
      adapter: "openai-chat",
      baseUrl: "http://localhost:8080/v1",
      allowPrivateNetwork: true,
  ...
  test("an exact entry still wins over the family entry", () => {
    const dir = mkdtempSync(join(tmpdir(), "nxc-models-exact-"));
    const provider = {
      adapter: "openai-chat",
      baseUrl: "http://localhost:8080/v1",
      allowPrivateNetwork: true,
  ...
  test("models rejects unknown flags", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["models", "--bogus"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(1);
...
describe("nxc models custom slash ids", () => {
  test("models add accepts slash model ids", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["models", "add", "test", "openai/gpt-5.5"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
  ...
  test("models remove accepts raw and encoded slash selectors", () => {
    for (const target of ["test/openai/gpt-5.5", "test/openai-gpt-5.5"]) {
      const { dir } = freshConfig();
      try {
        const add = runCli(["models", "add", "test", "openai/gpt-5.5"], { NEXCODE_HOME: dir });
        expect(add.status).toBe(0);
        const remove = runCli(["models", "remove", target, "--yes"], { NEXCODE_HOME: dir });
  ...
  test("models add rejects a slash id that encodes to an existing native id", () => {
    ...
    try {
      const slash = runCli(["models", "add", "test", "openai/gpt-5.5"], { NEXCODE_HOME: dir });
      expect(slash.status).toBe(1);
      expect(slash.stderr).toContain("ambiguous");
      const multi = runCli(["models", "add", "test", "a/b-c"], { NEXCODE_HOME: dir });
  ...
  test("models remove rejects an encoded selector that matches more than one custom model", () => {
    ...
    try {
      const result = runCli(["models", "remove", "test/openai-gpt-5.5", "--yes"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(1);
      expect(result.stderr).toContain("ambiguous");
