...
async function getVision(config: NxcConfig): Promise<Response> {
...
async function putVision(config: NxcConfig, vision: Record<string, unknown>): Promise<Response> {
  const url = new URL("http://localhost/api/sidecar-settings");
...
describe("vision reasoning capability contracts", () => {
  test("native management rows expose vision-safe reasoning ladders", async () => {
    const config: NxcConfig = { port: 10100, defaultProvider: "none", providers: {} };
  ...
  test("management normalizes native model/effort pairs on every relevant partial update", async () => {
    ...
    const isolatedHome = mkdtempSync(join(tmpdir(), "nxc-vision-reasoning-contract-"));
    process.env.NEXCODE_HOME = isolatedHome;

    try {
      const direct = { port: 10100, defaultProvider: "none", providers: {} } as NxcConfig;
      ...
      const custom = { port: 10100, defaultProvider: "none", providers: {} } as NxcConfig;
  ...
  test("CLI import normalizes reasoning against omitted and blank runtime model defaults", async () => {
    const previousHome = process.env.NEXCODE_HOME;
    const isolatedHome = mkdtempSync(join(tmpdir(), "nxc-vision-reasoning-cli-"));
    process.env.NEXCODE_HOME = isolatedHome;
  ...
  test("invalid effort is rejected without mutation and unrelated patches preserve reasoning", async () => {
    const previousHome = process.env.NEXCODE_HOME;
    const isolatedHome = mkdtempSync(join(tmpdir(), "nxc-vision-reasoning-invalid-"));
    process.env.NEXCODE_HOME = isolatedHome;
    const config = {
      port: 10100,
      defaultProvider: "none",
