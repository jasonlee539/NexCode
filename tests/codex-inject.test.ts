...
describe("Codex config injection", () => {
  test("omits provider-level Responses WebSocket support by default", () => {
    ...
    expect(block).toContain("[model_providers.nexcode]");
  ...
  test("non-loopback proxy mode injects the modern env_key admission line (#2073)", () => {
    ...
    expect(block).toContain('env_key = "NEXCODE_API_AUTH_TOKEN"');
  ...
  test("strips stale root context-window overrides on injection so the catalog drives model context (gpt-5.5 regression)", () => {
    ...
    expect(cleaned).toContain('model_provider = "nexcode"');
  ...
  test("preserves user root context-window overrides when restoring native Codex", () => {
    const stripped = stripNexcodeConfig([
      'model = "gpt-5.5"',
      'model_context_window = 1000000',
      'model_auto_compact_token_limit = 900000',
      'model_catalog_json = "/tmp/nexcode-catalog.json"',
      'model_provider = "nexcode"',
      "",
      "[features]",
      "fast_mode = true",
  ...
  test("preserves non-nexcode routed model names during fallback restore", () => {
    const stripped = stripNexcodeConfig([
  ...
  test("loopback fallback file uses the Design B root override (no provider table)", () => {
    ...
    expect(profile).not.toContain('model_provider = "nexcode"');
    expect(profile).not.toContain("[model_providers.nexcode]");
  ...
  test("non-loopback fallback profile keeps the legacy provider-table shape with the injected host", () => {
    ...
    expect(profile).toContain('model_provider = "nexcode"');
    expect(profile).toContain("[model_providers.nexcode]");
  });

  test("non-loopback fallback profile mirrors websocket and API auth provider options", () => {
    const profile = buildProfileFile(10100, "/tmp/nexcode-catalog.json", true, true);

    expect(profile).toContain('model_catalog_json = "/tmp/nexcode-catalog.json"');
    expect(profile).toContain("supports_websockets = true");
    expect(profile).toContain('env_key = "NEXCODE_API_AUTH_TOKEN"');
    expect(profile).not.toContain("env_http_headers");
  });

  test("honors an explicit unavailable catalog decision", () => {
    const path = chooseCatalogPathForInjection('model_catalog_json = "/tmp/nexcode-catalog.json"\n', null);
  ...
  test("strips injected TOML sections without swallowing later indented tables", () => {
    ...
    expect(stripped).toContain('model = "gpt-5.5"');
    expect(stripped).not.toContain("[model_providers.nexcode]");
    expect(stripped).not.toContain("[profiles.nexcode]");
  });
...
describe("Design B openai_base_url injection", () => {
  ...
  test("inserts marker + root key before the first table header", () => {
    ...
    const markerIdx = lines.findIndex(l => l.includes("Auto-injected by nexcode"));
  ...
  test("strip removes only the marker-owned pair; a user's own line survives", () => {
    ...
    expect(stripped).not.toContain("Auto-injected by nexcode");
  ...
  test("stripNexcodeConfig removes the Design B form including routed root models", () => {
    ...
    const stripped = stripNexcodeConfig(injected);
  ...
  test("upgrade path: legacy table + root re-tag coexisting with Design B form all strip cleanly", () => {
    ...
    const stripped = stripNexcodeConfig(legacy);

    expect(stripped).not.toContain("nexcode");
    expect(stripped).not.toContain("openai_base_url");
    expect(stripped).toContain('model = "gpt-5.5"');
  });

  test("legacy marker directly before the provider table survives the root strip order (removeNxcSection keeps its anchor)", () => {
    // No Design B form present — stripInjectedOpenaiBaseUrl must not eat the legacy EOF marker
    // in a way that leaves the [model_providers.nexcode] table behind.
    const legacyOnly = [
      'model_provider = "nexcode"',
      'model = "gpt-5.5"',
      "",
    ...
    const stripped = stripNexcodeConfig(legacyOnly);
  ...
  test("app-rewritten env_http_headers sub-table strips fully: no nameless provider survives", () => {
    ...
    // into a separate [model_providers.nexcode.env_http_headers] sub-table. Cleanup must
    ...
    const rewritten = [
      ...
      'wire_api = "responses"',
      "",
      "[model_providers.nexcode.env_http_headers]",
      '"x-nexcode-api-key" = "NEXCODE_API_AUTH_TOKEN"',
      "",
      "[agents]",
      "max_concurrent_threads_per_session = 8",
      "",
    ].join("\n");
    const stripped = stripNexcodeConfig(rewritten);
  ...
  test("an orphaned env_http_headers sub-table alone is removed (recurrence breaker)", () => {
    ...
    const stripped = stripNexcodeConfig(orphan);
  ...
  test("a user's similarly named provider table is preserved while nexcode sub-tables strip", () => {
    const content = [
      "[model_providers.nexcode.env_http_headers]",
      '"x-nexcode-api-key" = "NEXCODE_API_AUTH_TOKEN"',
      "",
      "[model_providers.nexcode_backup]",
      'name = "user backup"',
      "",
    ].join("\n");
    const stripped = stripNexcodeConfig(content);

    expect(stripped).not.toContain("env_http_headers");
    expect(stripped).toContain("[model_providers.nexcode_backup]");
  ...
  test("a trailing comment on the root provider header is still recognized (TOML allows `[table] # comment`)", () => {
    const commented = [
      'model = "gpt-5.5"',
      "",
      "[model_providers.nexcode] # managed provider",
      'name = "NexCode Proxy"',
      'base_url = "http://127.0.0.1:10100/v1"',
      "",
    ].join("\n");
    const stripped = stripNexcodeConfig(commented);

    expect(stripped).not.toContain("model_providers.nexcode");
    expect(stripped).not.toContain("NexCode Proxy");
    expect(stripped).toContain('model = "gpt-5.5"');
  });
  ...
  test("a trailing comment on the sub-table header is still recognized", () => {
    const commented = [
      'model = "gpt-5.5"',
      "",
      "[model_providers.nexcode.env_http_headers] # managed sub-table",
      '"x-nexcode-api-key" = "NEXCODE_API_AUTH_TOKEN"',
      "",
    ].join("\n");
    const stripped = stripNexcodeConfig(commented);
