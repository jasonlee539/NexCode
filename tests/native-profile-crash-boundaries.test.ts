...
const roots: string[] = [];
const oldNxc = process.env.NEXCODE_HOME;
...
afterEach(() => {
  restoreEnv("NEXCODE_HOME", oldNxc);
...
async function fixture() {
  const root = mkdtempSync(join(tmpdir(), "nxc-native-crash-"));
  roots.push(root);
  const home = join(root, "home");
  const codexHome = join(home, ".codex");
  const configDir = join(home, ".nexcode");
...
function spawnStartup(
  ...
  return Bun.spawn([process.execPath, join(import.meta.dir, "helpers", "native-profile-startup-child.ts")], {
    cwd: join(import.meta.dir, ".."),
    env: {
      ...process.env, HOME: f.home, USERPROFILE: f.home, CODEX_HOME: f.codexHome, NEXCODE_HOME: f.configDir,
      NEXCODE_ADMIN_AUTH_TOKEN: "crash-test-admin",
...
describe("native profile NexCode process-exit phases", () => {
  test("hard NexCode process exit after each published transaction phase converges exact auth, vault, journal, gate, and runtime bearer", async () => {
  ...
  test("a stalled startup child is killed by the bounded teardown instead of hanging", async () => {
    const f = await fixture();
    const p = startupPaths(f);
    const child = spawnStartup(f, p, { NXC_TEST_STALL_ON_STOP: "1" });
    try {
      await waitFor(p.port);
  ...
  test("waitForJson holds out for a complete document instead of parsing a partial write", async () => {
    const dir = mkdtempSync(join(tmpdir(), "nxc-settled-race-"));
