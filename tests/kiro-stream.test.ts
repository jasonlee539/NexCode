...
import type { NxcParsedRequest, NxcProviderConfig, NxcUsage } from "../src/types";
...
const origDebugFrames = process.env.NXC_DEBUG_FRAMES;
...
beforeEach(() => {
  ...
  delete process.env.NXC_DEBUG_FRAMES;
});
afterEach(() => {
  ...
  if (origDebugFrames === undefined) delete process.env.NXC_DEBUG_FRAMES; else process.env.NXC_DEBUG_FRAMES = origDebugFrames;
...
async function doneUsage(adapter: ReturnType<typeof createKiroAdapter>, ...frames: Uint8Array[]): Promise<NxcUsage> {
  let done: NxcUsage | undefined;
...
describe("kiro adapter — parseStream", () => {
  ...
  test("an event-stream profileArn-required exception classifies as kiro_profile_required (#993)", async () => {
    ...
    expect(errors[0]).toContain("nxc account login kiro --reauth");
  });
  ...
  test("buildRequest emits only redacted Kiro diagnostic breadcrumbs when enabled", async () => {
    process.env.NXC_DEBUG_FRAMES = "1";
    ...
    try {
      ...
      expect(line).toContain("[nxc:kiro:request]");
