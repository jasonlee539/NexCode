 ...
 * three branches; nexcode only implemented the third and hardcoded the macOS
...
describe("Claude Desktop configLibrary resolution", () => {
  ...
  test("NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR wins over every branch and is used verbatim", () => {
    // Returned without join(): tests point this straight at a temp dir.
    const dir = resolveConfigLibraryDir({
      env: {
        NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR: "/tmp/nxc-override",
    ...
    expect(dir).toBe("/tmp/nxc-override");
  });
...
/**
 * Desktop reads ONLY the profile named by `_meta.json`'s appliedId, so an nexcode
...
describe("Claude Desktop status reports whether our profile is the active one", () => {
  async function statusWithMeta(meta: unknown): Promise<{ activeProfile: boolean | null }> {
    const dir = mkdtempSync(join(tmpdir(), "nxc-desktop-active-"));
    const previous = process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR;
    process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR = dir;
    ...
    try {
      ...
      server.stop(true);
      if (previous === undefined) delete process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR;
      else process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR = previous;
  ...
  test("appliedId pointing at another profile reports activeProfile false", async () => {
    const body = await statusWithMeta({
      ...
      entries: [
        ...
        { id: "22222222-2222-4222-8222-222222222222", name: "nexcode" },
      ],
    });
  ...
  test("a readable appliedId with no nexcode entry is a known false, not unknown", async () => {
  ...
  test("metadata without appliedId leaves activeProfile undeterminable", async () => {
    const body = await statusWithMeta({
      entries: [{ id: "22222222-2222-4222-8222-222222222222", name: "nexcode" }],
    });
