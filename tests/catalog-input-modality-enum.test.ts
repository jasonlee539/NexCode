 ...
 * back to the GUI and CLI as if it were real, while the offline `nxc models add` path already
...
describe("custom-model API rejects out-of-enum input modalities", () => {
  ...
  async function callCustomModels(
    ...
    return handleModelRoutes({
      ...
      // fixture must NEVER reach the process-global NEXCODE_HOME; that exact bug
  ...
  // `nxc models edit --modalities -` sends an empty array. That must clear the field, not 400.
