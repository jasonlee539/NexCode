...
function start(home = codexHome): ReturnType<typeof startServer> {
  ...
  const server = startServer(0, {
    // The fixture builds its own CODEX_HOME/NEXCODE_HOME under a temp root, so
    ...
    // service instead: a machine running nxc as a launchd/systemd job reports
...
async function request(port: number, kind: string): Promise<{ status: number; text: string }> {
  ...
  if (kind === "management-list" || kind === "management-doctor") {
    ...
    const response = await fetch(`http://127.0.0.1:${port}${path}`, {
      headers: { "x-nexcode-api-key": process.env.NEXCODE_ADMIN_AUTH_TOKEN ?? "" },
    });
    return { status: response.status, text: await response.text() };
  }
  if (kind === "direct") {
