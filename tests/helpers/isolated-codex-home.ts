...
export function installIsolatedCodexHome(prefix = "nxc-codex-home-"): IsolatedCodexHome {
  const previousCodexHome = process.env.CODEX_HOME;
  const path = mkdtempSync(join(tmpdir(), prefix));
  writeFileSync(join(path, "config.toml"), 'model_catalog_json = "nexcode-catalog.json"\n', "utf8");
