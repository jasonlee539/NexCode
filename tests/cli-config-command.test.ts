...
const isolatedCodexHome = mkdtempSync(join(tmpdir(), "nxc-config-codex-home-"));
...
function freshConfig() {
  const dir = mkdtempSync(join(tmpdir(), "nxc-config-"));
  const config = {
    port: 10100,
    providers: {
      openai: {
...
describe("nxc config display redaction", () => {
  test("config show --json never prints secret-shaped modelCosts keys", () => {
    const dir = freshConfig();
    try {
      const result = runCli(["config", "show", "--json"], { NEXCODE_HOME: dir });
  ...
  test("config get providers.<name>.modelCosts --json drops secret-shaped keys", () => {
    const dir = freshConfig();
    try {
      const result = runCli(["config", "get", "providers.blsc.modelCosts", "--json"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
      expect(result.stdout).not.toContain("sk-abcdef1234567890");
      const parsed = JSON.parse(result.stdout);
