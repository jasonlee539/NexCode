...
const HOMES: string[] = [];
function tempHome(): string { const dir = join(tmpdir(), `nxc-lab-probe-${process.pid}-${Math.random().toString(16).slice(2)}`); mkdirSync(dir, { recursive: true, mode: 0o700 }); HOMES.push(dir); return dir; }
afterEach(() => { for (const dir of HOMES.splice(0)) { try { rmSync(dir, { recursive: true, force: true }); } catch { /* ignore */ } } delete process.env.NEXCODE_HOME; clearMcpStub(); });
...
function mockRoute(overrides: Partial<LabRouteContext> = {}): LabRouteContext {
  const base = {
    ...
    baseUrl: "https://api.example.com/v1", nexcodeCompatibilityVersion: "a".repeat(64), labRunApproval: true, allowPrivateNetwork: false,
...
describe("CL-03 live probe harness", () => {
  ...
  test("runs only applicable live suite scenarios with an injected test transport", async () => {
    const home = tempHome(); process.env.NEXCODE_HOME = home; const authority = loadLiveCaseAuthority();
  ...
  test("test transports can exercise normalization but cannot create live evidence", async () => {
    const home = tempHome(); process.env.NEXCODE_HOME = home; const authority = loadLiveCaseAuthority(); const caseRecord = authority.cases.find((c) => c.id === "responses-core.live.basic-turn")!;
  ...
  test("JSONL persistence and SQLite rebuild for trusted live evidence", async () => {
    const home = tempHome(); process.env.NEXCODE_HOME = home; const authority = loadLiveCaseAuthority(); const caseRecord = authority.cases.find((c) => c.id === "chat-core.live.basic-turn")!;
  ...
  test("live verification fails closed without route identity or validated claims", () => {
    ...
    const protocolSubject: ProtocolSubjectV1 = { subjectSchemaVersion: 1, subjectKind: "protocol", nexcodeCompatibilityVersion: "a".repeat(64), effectiveAdapter: "openai-responses", inboundProtocol: "openai-responses", upstreamProtocol: "openai-responses", surface: "responses-http", behaviorFingerprint: "b".repeat(64) };
  ...
  test("timeouts classify as blockers", async () => {
    const home = tempHome(); process.env.NEXCODE_HOME = home; const scenario = loadLiveCaseAuthority().cases[0]!;
  ...
  test("reasoning replay persists no private reasoning material", async () => {
    const home = tempHome(); process.env.NEXCODE_HOME = home; const authority = loadLiveCaseAuthority(); const scenario = authority.cases.find((c) => c.id === "reasoning-core.live.replay")!;
