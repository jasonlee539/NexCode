 ...
 * but the CLI paths carry their own parsing and validation copies: `nxc models add`
 * validates offline before writing config.json, and `nxc models edit` maps flags onto
 * the PUT body ("-" -> null). These tests pin that mapping so CLI and API cannot drift.
 */
describe("nxc models add --reasoning-efforts parsing", () => {
...
describe("nxc models edit reasoning flag mapping onto the PUT body", () => {
...
describe("nxc models add persists reasoning metadata into config.json", () => {
  const home = mkdtempSync(join(tmpdir(), "nxc-cli-test-"));
