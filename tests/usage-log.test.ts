...
beforeEach(() => {
  ...
  testDir = mkdtempSync(join(tmpdir(), "nxc-usage-"));
...
describe("usage log", () => {
  ...
  test("persists the empty-completion recovery kind on attempts", () => {
    const entry: PersistedUsageEntry = {
      requestId: "nxc-empty-completion-kind",
      timestamp: 1,
      provider: "fixture",
      model: "fixture/model",
      status: 200,
  ...
  test("persists the opaque-blob rejection recovery kind on attempts", () => {
    const entry: PersistedUsageEntry = {
      requestId: "nxc-opaque-blob-kind",
      timestamp: 1,
      provider: "openai",
      model: "gpt-5.6-sol",
      status: 200,
  ...
  test("persists an absolute context checkpoint for stateful providers", () => {
    ...
    appendUsageEntry({
      requestId: "nxc-context-checkpoint",
      timestamp: 1,
      provider: "kiro",
      model: "claude-opus-5",
      status: 200,
  ...
  test("omits malformed optional attempt reasoning metadata without dropping the attempt", () => {
    appendUsageEntry({
      requestId: "nxc-attempt-reasoning",
      timestamp: 1,
      provider: "combo",
      model: "combo/free",
      status: 200,
  ...
  test("keeps boolean reasoning values only for reasoning.enabled", () => {
    const base = {
      requestId: "nxc-boolean-reasoning",
      timestamp: 1,
      provider: "combo",
      model: "combo/free",
      status: 200,
  ...
  test("omits malformed parent firstOutputMs without dropping the entry (direct input)", () => {
    ...
    for (const bad of [Number.NaN, Number.POSITIVE_INFINITY, -5]) {
      ...
      const [entry] = readUsageEntries();
      expect(entry?.requestId).toBe("nxc-ttft-bad");
  ...
  test("uses NEXCODE_HOME for the append-only JSONL path", () => {
  ...
  test("appends secret-safe usage entries and reads them back", () => {
    ...
    const raw = readFileSync(usageLogPath(), "utf-8");
    expect(raw).toContain("\"requestId\":\"nxc-1\"");
