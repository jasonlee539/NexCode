 ...
 *
 * The toggle writes one field of nexcode's own config, so there is nothing to
...
function baseConfig(overrides: Partial<NxcConfig> = {}): NxcConfig {
  return { port: 10100, providers: [], ...overrides } as NxcConfig;
}

/**
 * `saveConfigPreservingClaudeCode` is injected as a no-op spy on purpose: the
 * production function writes the developer's real NEXCODE_HOME, and
 * ManagementApiDeps carries this seam precisely so a fixture config cannot
 * overwrite it (src/server/management/context.ts).
 */
function dispatch(config: NxcConfig, path: string, init?: RequestInit) {
  const url = new URL(`http://127.0.0.1:10100${path}`);
  const saved: NxcConfig[] = [];
...
async function put(config: NxcConfig, enabled: boolean) {
...
test("a held REAL config transaction refuses 409 config_busy, and release lets a retry through", async () => {
   ...
   * against a fixture NEXCODE_HOME.
   */
  const { Database } = await import("bun:sqlite");
  const { mkdtempSync, rmSync } = await import("node:fs");
  const { tmpdir } = await import("node:os");
  const { join } = await import("node:path");
  const fixtureRoot = mkdtempSync(join(tmpdir(), "nxc-lock-"));
  ...
  try {
    ...
    const putReal = (config: NxcConfig) => {
      const url = new URL("http://127.0.0.1:10100/api/native-integrations/claude");
