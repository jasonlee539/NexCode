...
const prevHome = process.env.NEXCODE_HOME;
let dir: string;

beforeEach(() => {
  dir = join(tmpdir(), `nxc-update-job-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  mkdirSync(dir, { recursive: true });
  process.env.NEXCODE_HOME = dir;
});

afterEach(() => {
  if (prevHome === undefined) delete process.env.NEXCODE_HOME;
  else process.env.NEXCODE_HOME = prevHome;
...
describe("GUI update check", () => {
  test("surfaces an npm update with the launcher-safe command", () => {
    ...
    expect(result.command).toContain("nxc.mjs update --tag latest");
  });
...
describe("GUI update execution decisions", () => {
  test("the persistence boundary redacts profile/cache paths and UID/GID from every field", () => {
    ...
    expect(() => startUpdateJob("latest", true, {
      checkForUpdateFn: () => ({
        ...
        releaseNotesUrl: "https://github.com/lidge-jun/nexcode/releases/latest",
      }),
  ...
  test("a failed cache pre-flight leaves the install command unrun", async () => {
    ...
    await runGuiUpdateWorker("gate-job", "latest", false, {
      ...
      checkForUpdateFn: () => ({
        ...
        updateAvailable: true,
        canUpdate: true,
        command: "npm i -g nexcode@latest",
        releaseNotesUrl: "https://github.com/lidge-jun/nexcode/releases/latest",
      }),
  ...
  test("an error message naming a person is never persisted, path or not", () => {
    ...
    expect(() => startUpdateJob("latest", false, {
      checkForUpdateFn: () => ({
        ...
        updateAvailable: true,
        canUpdate: true,
        command: "npm install -g nexcode@2.7.41",
        releaseNotesUrl: "https://github.com/lidge-jun/nexcode/releases/latest",
      }),
    ...
    expect(persisted).toContain("nexcode@2.7.41");
  });
  ...
  test("an allowlisted field name does not make its value safe", () => {
    ...
    // can encode anything (`@bitkyc08/nexcode@99.99.99-JaneDoe`). `code: ETARGET` plus the
    // bare fact is the diagnostic that matters.
    for (const line of [
      "npm error notarget No matching version found for jane.doe@example.com",
      "npm error notarget No matching version found for @bitkyc08/nexcode@99.99.99-JaneDoe",
    ]) {
    ...
    expect(() => startUpdateJob("latest", false, {
      checkForUpdateFn: () => ({
        ...
        updateAvailable: true, canUpdate: true, command: "npm install -g nexcode@2.7.41",
        releaseNotesUrl: "https://github.com/lidge-jun/nexcode/releases/latest",
      }),
  ...
  test("npm worker uses the Node launcher update path", () => {
    const cmd = updateExecutionCommand("npm", "preview", "/pkg/bin/nxc.mjs");
    expect(cmd.bin).toMatch(/^node/);
    expect(cmd.args).toEqual(["/pkg/bin/nxc.mjs", "update", "--tag", "preview"]);
  });

  test("restart command separates service and direct proxy modes", () => {
    expect(restartCommand(true, "npm", "/pkg/bin/nxc.mjs")).toMatchObject({
      mode: "service",
      args: ["/pkg/bin/nxc.mjs", "service", "repair"],
    });
    expect(restartCommand(false, "npm", "/pkg/bin/nxc.mjs")).toMatchObject({
      mode: "proxy",
      args: ["/pkg/bin/nxc.mjs", "start"],
    });
  });
  ...
  test("proxy restart pins --port so post-update start does not hop to an ephemeral port", () => {
    const proxy = restartCommand(false, "npm", "/pkg/bin/nxc.mjs", 10100);
    expect(proxy.mode).toBe("proxy");
    expect(proxy.args).toEqual(["/pkg/bin/nxc.mjs", "start", "--port", "10100"]);
    expect(proxy.display).toContain("start --port 10100");
    // The service refresh takes no --port at the argv level; wrappers bake it via NXC_BAKE_PORT.
    expect(restartCommand(true, "npm", "/pkg/bin/nxc.mjs", 10100).args).toEqual([
      "/pkg/bin/nxc.mjs", "service", "repair",
    ]);
  });
  ...
  test("restart waits on the captured pre-update port unconditionally and pins the spawn to it", async () => {
    ...
    const waited: Array<{ port: number; hostname: string; opts?: { killNxcHolders?: boolean; onlyKillPids?: number[]; killAllNxcOnPort?: boolean } }> = [];
    ...
    await restartAfterUpdateForTests(job, { port: 12345, hostname: "127.0.0.1" }, {
      ...
      waitForPort: async (port, hostname, opts) => {
        waited.push({
          ...
          hostname: hostname ?? "",
          opts: {
            killNxcHolders: opts?.killNxcHolders,
            onlyKillPids: opts?.onlyKillPids,
            killAllNxcOnPort: (opts as { killAllNxcOnPort?: boolean } | undefined)?.killAllNxcOnPort,
          },
        });
        return true;
      },
    ...
    expect(waited).toEqual([{
      port: 12345,
      hostname: "127.0.0.1",
      opts: { killNxcHolders: true, onlyKillPids: [], killAllNxcOnPort: true },
    }]);
    expect(spawned).toEqual([{ port: 12345 }]);
  });

  test("restart reclaim allowlists the trusted oldPid and kills any nxc on the port", async () => {
    const optsSeen: Array<{ killNxcHolders?: boolean; onlyKillPids?: number[]; killAllNxcOnPort?: boolean }> = [];
    const job: UpdateJobState = {
      id: "restart-oldpid",
      status: "restarting",
      startedAt: new Date().toISOString(),
    ...
    expect(optsSeen).toEqual([{ killNxcHolders: true, onlyKillPids: [4242], killAllNxcOnPort: true }]);
  });

  test("restart reclaim also allowlists leftover nxc listeners on the captured port", async () => {
    const optsSeen: number[][] = [];
    const job: UpdateJobState = {
      id: "restart-leftover-nxc",
      status: "restarting",
      startedAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
      currentVersion: "2.7.39",
    ...
    await restartAfterUpdateForTests(job, { port: 19111, hostname: "127.0.0.1", oldPid: 100 }, {
      ...
      verifyNxcFn: (pid) => (pid === 100 || pid === 200 ? pid : null),
      waitForPort: async (_port, _hostname, opts) => {
  ...
  test("service restart waits on the captured port and clears NXC_BAKE_PORT after install", async () => {
    ...
    try {
      await restartAfterUpdateForTests(job, { port: 18765, hostname: "127.0.0.1" }, {
        ...
        // fallback. This test is about the NXC_BAKE_PORT lifecycle, not the recovery
        ...
        waitForPort: async (port, hostname) => {
          ...
          expect(process.env.NXC_BAKE_PORT).toBeUndefined();
          return true;
        },
        runService: () => {
          bakeDuringInstall.push(process.env.NXC_BAKE_PORT ?? "");
          return { status: 0 };
        },
      });
  ...
  test("service reinstall failure falls back to a direct proxy start", async () => {
    ...
    writeFileSync(updateJobPath(job.id), JSON.stringify(job));
    const prevService = process.env.NXC_SERVICE;
    delete process.env.NXC_SERVICE;
    try {
      ...
      if (prevService === undefined) delete process.env.NXC_SERVICE;
      else process.env.NXC_SERVICE = prevService;
    }
  });

  // 260804 #970: the Windows GUI update worker (NXC_SERVICE=1, never elevated) used to
  ...
  test("npm finish skips the soft probe for direct installs and restarts immediately", async () => {
    ...
    const ok = await finishGuiUpdateRestart(job, { port: 10100, hostname: "127.0.0.1" }, "npm", {
      serviceInstalledFn: () => false,
      probeProxy: async () => {
        // Only becomes healthy after the explicit restart (launcher printed `nxc start` only).
  ...
  test("a running job prevents a second update job", () => {
    ...
    const job: UpdateJobState = {
      ...
      installer: "npm",
      restart: true,
      command: "node /pkg/bin/nxc.mjs update --tag latest",
      releaseNotesUrl: "https://github.com/lidge-jun/nexcode/releases/latest",
      log: [],
    };
    writeFileSync(updateJobPath(), `${JSON.stringify(job)}\n`);
  ...
  test("recovers a dead worker and persists the replacement worker PID", () => {
    ...
    const oldJob: UpdateJobState = {
      ...
      installer: "bun",
      restart: true,
      command: "bun add -g @bitkyc08/nexcode@2.7.41",
      releaseNotesUrl: "https://github.com/lidge-jun/nexcode/releases/latest",
      log: [],
      pid: 777,
    };
...
describe("immutable update target (WP160)", () => {
  test("a resolved version pins the install target instead of the movable tag", () => {
    expect(updateCommand("bun", "latest", "2.7.24").args).toEqual(["add", "-g", "@bitkyc08/nexcode@2.7.24"]);
    expect(updateCommand("npm", "latest", "2.7.24").args).toEqual(["install", "-g", "@bitkyc08/nexcode@2.7.24"]);
    expect(updateCommandStr("bun", "latest", "2.7.24")).toContain("@bitkyc08/nexcode@2.7.24");
    // Unknown version falls back to the tag (best-effort lane).
    expect(updateCommand("bun", "latest").args).toEqual(["add", "-g", "@bitkyc08/nexcode@latest"]);
    expect(updateCommand("bun", "latest", null).args).toEqual(["add", "-g", "@bitkyc08/nexcode@latest"]);
  });

  test("bun worker execution pins the resolved version through updateExecutionCommand", () => {
    const cmd = updateExecutionCommand("bun", "latest", "/pkg/bin/nxc.mjs", "2.7.24");
    expect(cmd.bin).toBe(process.platform === "win32" ? process.execPath : "bun");
    expect(cmd.args).toEqual(["add", "-g", "@bitkyc08/nexcode@2.7.24"]);
