...
const entry: WindowsTrayEntry = {
  ...
  nexcodeHome: "C:\\사용자 공간\\%TEMP% ! ^ ( ) & 검증\\.nexcode",
};
...
describe("Windows tray packaging and command safety", () => {
  test("owned-file temp cleanup forgets successful ACL memos and retains failed removals", () => {
    const root = mkdtempSync(join(tmpdir(), "nxc-tray-acl-"));
  ...
  test("quotes metacharacter and Unicode paths without shell interpolation", () => {
    ...
    expect(powershellCommand).toContain(`-NexCodeHome "${entry.nexcodeHome}"`);
    ...
    const runCommand = buildWindowsTrayRunCommand({
      ...entry,
      launcherPath: `${entry.nexcodeHome}\\nexcode-tray.vbs`,
    });
  ...
  test("rejects quote and control-character path injection", () => {
    expect(() => windowsTrayProcessArgs({ ...entry, nexcodeHome: 'C:\\bad" -Command whoami' })).toThrow();
    expect(() => windowsTrayProcessArgs({ ...entry, cli: "C:\\bad\r\nwhoami" })).toThrow();
  });

  test("never trusts state-selected executable or deletion paths", () => {
    const home = "C:\\Users\\Test\\.nexcode";
    expect(windowsTrayStatePathsOwned({
      nexcodeHome: home,
      script: join(home, "nexcode-tray.ps1"),
      launcherPath: join(home, "nexcode-tray.vbs"),
    }, home)).toBe(true);
    expect(windowsTrayStatePathsOwned({
      nexcodeHome: home,
  ...
  test("normalizes equivalent homes to one owned Run value", () => {
    expect(windowsTrayRunValue("C:\\Users\\Test\\.nexcode"))
      .toBe(windowsTrayRunValue("C:\\Users\\Test\\.nexcode\\."));
  });

  test("treats an unexpected registry type or unreadable value as foreign", () => {
    const value = "NexCodeTray-test";
  ...
  test("PowerShell controller uses mutex/event shutdown and bans command evaluation", () => {
    ...
    expect(source).toContain('Start-NxcCommand @("__tray-restart")');
    ...
    expect(cli).toContain("delete env.NXC_SERVICE");
    expect(cli).not.toContain("NXC_KEEP_ROUTING");
    expect(source).toContain('Load-TrayIcon "nexcode-tray-online.ico"');
    expect(source).toContain('Load-TrayIcon "nexcode-tray-warning.ico"');
    expect(source).toContain('Load-TrayIcon "nexcode-tray-offline.ico"');
  ...
  test("launches the detached tray host without retaining the proxy listen socket", async () => {
    ...
    const directory = mkdtempSync(join(tmpdir(), "nxc-tray-inheritance-"));
    ...
    const previousPidPath = process.env.NXC_TRAY_TEST_PID_FILE;
    ...
    try {
      process.env.NXC_TRAY_TEST_PID_FILE = pidPath;
      launchWindowsTrayHost({
        ...entry,
      ...
      if (previousPidPath === undefined) delete process.env.NXC_TRAY_TEST_PID_FILE;
      else process.env.NXC_TRAY_TEST_PID_FILE = previousPidPath;
  ...
  test("ships branded multi-size Windows tray icons", () => {
    ...
    for (const name of ["online", "warning", "offline"]) {
      const path = join(assets, `nexcode-tray-${name}.ico`);
  ...
  test("serves tray status without blocking the proxy event loop", async () => {
    ...
    const responsePromise = handleManagementAPI(
      new Request(url),
      url,
      { port: 10100, providers: {}, defaultProvider: "openai" } as NxcConfig,
    );
    await Bun.sleep(50);
  ...
  test("copies the tray script into the hardened home and gates all update lanes", () => {
    ...
    expect(tray).toContain('join(getConfigDir(), "nexcode-tray.ps1")');
  ...
  test("a non-ASCII profile path round-trips through the registry reader (#1933)", () => {
    ...
    const runValue = "NexCodeTray-c856edd2e06f";
    const command = [
      String.raw`"C:\WINDOWS\System32\wscript.exe" //B //NoLogo `,
      String.raw`"C:\Users\M\u00f6tz\.nexcode\nexcode-tray.vbs"`,
    ].join("").replace("\\u00f6", "\u00f6");
    const rendered = [
      "",
