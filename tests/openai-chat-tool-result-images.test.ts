...
import type { NxcContentPart, NxcMessage, NxcParsedRequest, NxcProviderConfig } from "../src/types";
...
function wire(messages: NxcMessage[]): ChatMsg[] {
...
function user(text: string): NxcMessage {
  return { role: "user", content: text, timestamp: 0 };
}

function assistantWithCalls(calls: { id: string; name: string }[]): NxcMessage {
  return {
    role: "assistant",
...
function toolResult(callId: string, name: string, content: string | NxcContentPart[]): NxcMessage {
...
function isImageCarrier(msg: ChatMsg): boolean {
  ...
  return head?.type === "text" && typeof head.text === "string" && head.text.startsWith("[nxc]")
