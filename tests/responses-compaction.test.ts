...
describe("compaction envelope", () => {
  test("round-trips a summary", () => {
    ...
    expect(enc.startsWith("nxc1:")).toBe(true);
...
describe("parser compaction handling", () => {
  ...
  test("nxc1 compaction input item decodes into a summary user message", () => {
    const parsed = parseRequest({
      model: "anthropic/claude-sonnet-4-6",
      input: [
...
describe("forward-path nxc1 compaction scrub", () => {
  const provider = {
  ...
  test("nxc1 compaction items become plain user messages before ChatGPT forwarding", () => {
    ...
    expect(JSON.stringify(body)).not.toContain("nxc1:");
  });

  test("nxc1 context_compaction items are scrubbed the same way", () => {
    const body = forwardedBody({
      model: "gpt-5.5",
      input: [
