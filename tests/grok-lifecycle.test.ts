...
describe("Grok fence lifecycle wiring", () => {
  ...
  test("a refused Grok strip makes nxc stop fail instead of reporting success", () => {
    const restoreFn = sliceFn(CLI_SOURCE, "async function restoreSharedClientStateAfterStop(", "async function handleStop(");
    const stopFn = sliceFn(CLI_SOURCE, "async function handleStop(", "async function handleUninstall(");
  ...
  test("the daemon's exit cleanup keeps the NXC_SERVICE exclusion and adds the ownership check", () => {
    ...
    expect(startFn).toContain('process.env.NXC_SERVICE === "1"');
    expect(startFn).not.toContain("NXC_KEEP_ROUTING");
