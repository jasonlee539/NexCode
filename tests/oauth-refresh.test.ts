...
describe("oauth refresh hardening", () => {
  ...
  test("stale Grok generation refreshes once and detaches to NexCode ownership", async () => {
    await saveCredential("xai", {
      access: "xai-old", refresh: "rt-old", expires: Date.now() - 1, accountId: "user-1", source: "local-cli",
    });
    seedGrokAuth({
    ...
    expect(warnings).toEqual([[
      "[oauth:xai] Grok CLI credential was stale; refreshed into NexCode ownership. Grok CLI may require login again.",
    ]]);
