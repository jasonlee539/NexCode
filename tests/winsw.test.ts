...
const entry = { bun: "C:\\NexCode\\bun.exe", bunRuntimeSource: "bundled" as const, cli: "C:\\Open Codex\\cli & co\\index.ts" };
...
describe("winsw xml", () => {
  ...
  test("carries service env: NXC_SERVICE, token file pointer, and escaped PATH parity", () => {
    const xml = buildWinswXml(entry, env);

    expect(xml).toContain('<env name="NXC_SERVICE" value="1"/>');
    expect(xml).toContain('<env name="NXC_API_TOKEN_FILE"');
    ...
    expect(winswEnvValue(xml, "NEXCODE_HOME")).toBe(getConfigDir());
    // Token VALUES never land in the XML — only file pointers / non-secret budgets.
    expect(xml).not.toContain("NEXCODE_API_AUTH_TOKEN");
    expect(xml).not.toContain("NEXCODE_ADMIN_AUTH_TOKEN");
  });

  test("carries the Bun provenance paired with the executable it baked (#848)", () => {
    expect(buildWinswXml(entry, env)).toContain('<env name="NXC_BUN_RUNTIME_SOURCE" value="bundled"/>');
    ...
    expect(overrideXml).toContain('<env name="NXC_BUN_RUNTIME_SOURCE" value="override"/>');
  ...
  test("bakes install-time ACL timeout and never embeds the admin token (#764)", () => {
    const xml = buildWinswXml(entry, {
      ...env,
      NEXCODE_API_AUTH_TOKEN: "api-secret-value",
      NEXCODE_ADMIN_AUTH_TOKEN: "admin-secret & more",
      NEXCODE_ACL_TIMEOUT_MS: "10000",
    });

    expect(xml).toContain('<env name="NEXCODE_ACL_TIMEOUT_MS" value="10000"/>');
    expect(winswEnvValue(xml, "NEXCODE_HOME")).toBe(getConfigDir());
    expect(xml).not.toContain("NEXCODE_ADMIN_AUTH_TOKEN");
    expect(xml).not.toContain("NEXCODE_API_AUTH_TOKEN");
  ...
  test("escapes executable/arguments and configures restart + graceful stop", () => {
    ...
    expect(xml).toContain("<executable>C:\\NexCode\\bun.exe</executable>");
  ...
  test("honors NXC_BAKE_PORT when building WinSW arguments", () => {
    const xml = buildWinswXml(entry, { ...env, NXC_BAKE_PORT: "14444" });
...
describe("app-side service token loading", () => {
  test("loads the token from NXC_API_TOKEN_FILE only when the env token is empty", () => {
    const dir = mkdtempSync(join(tmpdir(), "nxc-token-"));
    const file = join(dir, "service-api-token");
    writeFileSync(file, "  tok-123  \n");
    try {
      expect(loadServiceTokenFromFile({ NXC_API_TOKEN_FILE: file })).toBe("tok-123");
      expect(loadServiceTokenFromFile({ NXC_API_TOKEN_FILE: file, NEXCODE_API_AUTH_TOKEN: "already" })).toBeNull();
      expect(loadServiceTokenFromFile({})).toBeNull();
      expect(loadServiceTokenFromFile({ NXC_API_TOKEN_FILE: join(dir, "missing") })).toBeNull();
    } finally {
