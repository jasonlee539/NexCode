...
beforeEach(() => {
  ...
  testDir = mkdtempSync(join(tmpdir(), "nxc-auth-ctx-"));
...
function guardianConfig(): NxcConfig {
  const cfg = config();
...
const forwardProvider: NxcProviderConfig = {
  adapter: "openai-responses",
...
describe("Codex auth context", () => {
  ...
  test("Direct admission-bearer substitution checks the stored main account grant", async () => {
    ...
    let callerChecks = 0;
    await expect(resolveCodexAuthContext(
      new Headers({ authorization: "Bearer nxc-admission" }),
      config(),
      "direct",
      {
        modelId: "gpt-daybreak-blue-latest",
  ...
  test("an admission bearer on main substitutes the stored credential, never forwards it (#1686)", () => {
    ...
    const admissionSecret = "nxc_data_localsecret";
    writeFileSync(join(testDir, "auth.json"), JSON.stringify({
  ...
  test("substitution fails closed when no usable main credential exists (#1686)", () => {
    ...
    expect(() => materializeCodexUpstreamAuth(
      new Headers({ authorization: "Bearer nxc_data_localsecret" }),
      { kind: "main", accountId: null },
      { substituteMainCredential: true },
...
describe("cooldown error surface", () => {
  test("native-main maintenance identifies NexCode instead of upstream capacity", async () => {
  ...
  test("message names the account, deadline, source, and escape command", () => {
    ...
    expect(message).toContain("nxc account clear-cooldown openai <id>");
  ...
  test("HTTP form carries a Retry-After the client can act on", async () => {
    ...
    expect(body.error?.message).toContain("nxc account clear-cooldown");
  });
...
// #2108: a Windows reboot can leave the native-main fence closed until `nxc restart`, and the
