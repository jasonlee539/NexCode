...
function fixture() {
  const root = mkdtempSync(join(tmpdir(), "nxc-native-profile-"));
  roots.push(root);
...
describe("native main profile transactions", () => {
  ...
  test("the same canonical CODEX_HOME serializes different NexCode config roots", async () => {
    const f = fixture();
    const secondConfigDir = join(f.root, "nexcode-second");
  ...
  test("shares one vault while preventing another NEXCODE_HOME from finishing or cancelling a stage", async () => {
    const f = fixture();
  ...
  test("shares journal quarantine and manual recovery state across NEXCODE_HOME roots", async () => {
  ...
  test("pending recovery blocks register before auth or vault mutation", async () => {
    ...
    expect(caught).toBeInstanceOf(NativeProfileError);
    expect((caught as NativeProfileError).code).toBe("RECOVERY_REQUIRED");
    expect((caught as NativeProfileError).message).toContain("nxc account main recover");
    expect(readFileSync(f.manager.context.authPath, "utf8")).toBe(authBefore);
    expect(readFileSync(f.manager.context.vaultPath, "utf8")).toBe(vaultBefore);
