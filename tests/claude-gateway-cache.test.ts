...
function tempDir(): string {
  const d = mkdtempSync(join(tmpdir(), "nxc-gwcache-"));
  dirs.push(d);
  return d;
}
afterEach(() => {
...
describe("Claude Code gateway-model cache pre-write (devlog 260712 030)", () => {
  ...
  test("proxy refresh pins the readable id family with ?ids=cli (audit 051 #5)", async () => {
    ...
    try {
      globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        expect(headers.get("x-nexcode-api-key")).toBe("env-admission");
        expect(headers.get("authorization")).toBeNull();
        return new Response(JSON.stringify({ data: [{ id: "claude-nxc-native--gpt-5.6-sol", display_name: "gpt-5.6-sol (native)" }] }), {
      ...
      const path = await refreshGatewayModelCacheFromProxy(10100, {
        ...
        env: { NEXCODE_API_AUTH_TOKEN: " env-admission " },
      });
      expect(requestedUrl).toContain("ids=cli");
      const body = JSON.parse(readFileSync(path!, "utf8"));
      expect(body.models[0].id).toBe("claude-nxc-native--gpt-5.6-sol");
    } finally {
  ...
  test("proxy refresh falls back to a configured admission key", async () => {
    ...
    try {
      globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
        const headers = new Headers(init?.headers);
        expect(headers.get("x-nexcode-api-key")).toBe("configured-admission");
  ...
  test("proxy refresh uses the hardened service token file before a configured key", async () => {
    ...
    try {
      globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
        seen = new Headers(init?.headers).get("x-nexcode-api-key");
        return new Response(JSON.stringify({ data: [] }), {
          headers: { "content-type": "application/json" },
        });
      }) as typeof fetch;
      await refreshGatewayModelCacheFromProxy(10100, {
        ...
        env: { NXC_API_TOKEN_FILE: tokenFile },
      });
