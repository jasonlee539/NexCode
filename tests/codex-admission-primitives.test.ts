...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), "nxc-admission-primitives-"));
  cleanup.push(root);
...
describe("absence is one state, and being unable to look is another", () => {
  ...
  test("an unreadable parent directory is not absent", () => {
    ...
    process.env.NEXCODE_HOME = nested;
    try {
      const observed = observeConfigGeneration();
...
describe("the journal has one owner", () => {
  ...
  test("the exported constant is the file journal.ts actually uses", () => {
    // The old hand-derived path was NEXCODE_HOME/codex-journal.json — wrong
    ...
    expect(basename(JOURNAL_PATH)).toBe("nexcode-journal.json");
    expect(dirname(JOURNAL_PATH).endsWith(".codex")).toBeTrue();
    expect(dirname(JOURNAL_PATH).endsWith(".nexcode")).toBeFalse();
  });
...
function snapshotWith(overrides: Partial<AdmissionSnapshot>): AdmissionSnapshot {
  return {
    ...
    canonicalTargets: {
      ...
      profile: "/codex/nexcode.config.toml",
      catalog: "/codex/nexcode-catalog.json",
      cache: "/codex/models_cache.json",
      journal: JOURNAL_PATH,
      integrationRecord: "/nexcode/integrations/codex.json",
      catalogBackups: [],
      historyDb: "/codex/state_5.sqlite",
      historyManifest: "/codex/state_5.sqlite.nxc-backup.json",
      historyRollouts: [],
    },
