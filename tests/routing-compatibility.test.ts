...
function routed(provider: NxcConfig["providers"][string]) {
  return { ...provider };
}
...
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), "nxc-cl06-"));
...
describe("CL-06 routing compatibility", () => {
  ...
  test("routing subject resolution never creates a missing Lab salt", () => {
    const freshHome = mkdtempSync(join(tmpdir(), "nxc-cl06-nosalt-"));
    try {
  ...
  test("incompatible projection is treated as unavailable without rebuild", () => {
    const missingHome = mkdtempSync(join(tmpdir(), "nxc-cl06-missing-"));
    try {
  ...
  test("subject id is stable for frozen route subject", () => {
    const subject = {
      ...
      upstreamProtocol: "openai-responses",
      surface: "responses-http",
      nexcodeCompatibilityVersion: COMPAT_VERSION,
