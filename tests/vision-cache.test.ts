...
import type { NxcConfig, NxcContentPart, NxcProviderConfig } from "../src/types";
import {
...
const openaiProvider: NxcProviderConfig = {
...
const textOnlyProvider: NxcProviderConfig = {
  adapter: "openai-chat",
...
test("vision sidecar auth stays lazy for no-image and disabled branches", () => {
  const cfg: NxcConfig = { port: 10100, defaultProvider: "routed", providers: { routed: textOnlyProvider } };
...
function textParts(request: ReturnType<typeof parsed>, messageIndex = 0): string[] {
  ...
  return (content as NxcContentPart[]).filter(part => part.type === "text").map(part => part.text);
}
