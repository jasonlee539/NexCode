...
function tempHome(): string {
  const home = join(tmpdir(), `nxc-cl10-surfaces-${process.pid}-${Math.random().toString(16).slice(2)}`);
...
function config(home: string): NxcConfig {
  void home;
  return { port: 0, defaultProvider: "openai-apikey", providers: {} } as NxcConfig;
}
