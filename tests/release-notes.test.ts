...
describe("stripCarriedReleaseNotes", () => {
  test("keeps PR categories and drops npm blurb, commits, and compare link", () => {
    const body = [
      "Published to npm as `@bitkyc08/nexcode@2.7.39-preview.20260724` with dist-tag `preview`.",
      "",
      ...
      "",
      "**Full Changelog**: https://github.com/lidge-jun/nexcode/compare/v2.7.38-preview.20260724...v2.7.39-preview.20260724",
      "",
    ].join("\n");
...
describe("rewriteTakeoverCredits", () => {
  test("credits original author and keeps landing PR link", async () => {
    const body = [
      "## What's Changed",
      "### New Features",
      "* feat(images): Grok image bridge (maintainer takeover of #424) by @Wibias in https://github.com/lidge-jun/nexcode/pull/577",
      "* feat(other): normal change by @Alice in https://github.com/lidge-jun/nexcode/pull/100",
    ].join("\n");
    ...
    expect(rewritten).toContain(
      "* feat(images): Grok image bridge (maintainer takeover of #424) by @tizerluo (takeover by @Wibias) in https://github.com/lidge-jun/nexcode/pull/577",
    );
    expect(rewritten).toContain(
      "* feat(other): normal change by @Alice in https://github.com/lidge-jun/nexcode/pull/100",
    );
  ...
  test("falls back to landing lookup when title says takeover without #N", async () => {
    const line =
      "* feat x (takeover) by @Wibias in https://github.com/lidge-jun/nexcode/pull/577";
    const rewritten = await rewriteTakeoverCredits(
      line,
      async (pr) => {
    ...
    expect(rewritten).toContain(
      "* feat x (takeover) by @tizerluo (takeover by @Wibias) in https://github.com/lidge-jun/nexcode/pull/577",
    );
  });
  ...
  test("leaves line unchanged when original author matches landing author", async () => {
    const line =
      "* feat x (takeover #9) by @Wibias in https://github.com/lidge-jun/nexcode/pull/10";
    const rewritten = await rewriteTakeoverCredits(
      line,
      async () => ({
...
describe("renderReleaseNotes", () => {
  const carried = [
    ...
    "## What's Changed",
    "### New Features",
    "* feat(providers): add Baseten Model APIs preset by @olddonkey in https://github.com/lidge-jun/nexcode/pull/653",
    "### Bug Fixes",
    "* fix(providers): keep Antigravity catalog static by @luvs01 in https://github.com/lidge-jun/nexcode/pull/744",
    "",
    "## New Contributors",
  ...
  const delta = [
    "## What's Changed",
    "### New Features",
    "* feat(server): advertise reasoning-effort ladders on the raw /v1/models list by @n3wr1ch in https://github.com/lidge-jun/nexcode/pull/853",
    "### Documentation",
    "* docs(codex): clarify pool routing and account continuity by @luvs01 in https://github.com/lidge-jun/nexcode/pull/862",
  ].join("\n");

  test("renders OpenAI-style sections, scope bullets, and a full PR changelog", () => {
    const notes = renderReleaseNotes({
      npmMetadata: "Published to npm as `@bitkyc08/nexcode@2.10.0` with dist-tag `latest`.",
      carriedPreviewNotes: carried,
      deltaPrNotes: delta,
      compareFrom: "v2.9.1",
      compareTo: "v2.10.0",
      repository: "lidge-jun/nexcode",
    });

    expect(notes).toBe([
      "Published to npm as `@bitkyc08/nexcode@2.10.0` with dist-tag `latest`.",
      "",
      "## New Features",
      "",
      ...
      "## Changelog",
      "",
      "Full Changelog: https://github.com/lidge-jun/nexcode/compare/v2.9.1...v2.10.0",
      "",
  ...
  test("carries already-rendered preview bodies losslessly into stable notes", () => {
    const preview = renderReleaseNotes({
      npmMetadata: "Published to npm as `@bitkyc08/nexcode@2.10.0-preview.1` with dist-tag `preview`.",
      carriedPreviewNotes: carried,
      deltaPrNotes: delta,
      compareFrom: "v2.9.1",
    ...
    expect(stable).toContain("Full Changelog: https://github.com/lidge-jun/nexcode/compare/v2.9.1...v2.10.0");
