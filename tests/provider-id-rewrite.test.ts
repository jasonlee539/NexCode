...
test("moves a providerContextCaps entry by key, not by prefix", () => {
  const config = { providerContextCaps: { [FROM]: 500_000, anthropic: 200_000 } } as unknown as NxcConfig;
...
test("reports a providerContextCaps collision instead of overwriting it", () => {
  const config = { providerContextCaps: { [FROM]: 500_000, [TO]: 900_000 } } as unknown as NxcConfig;
...
test("re-points combo targets so the migrated config still validates", () => {
  const providers = { [TO]: { adapter: "openai-chat" } } as unknown as Record<string, NxcProviderConfig>;
  const combo = { targets: [{ provider: FROM, model: "qwen3.7-max" }] };
  const config = { providers, combos: { fast: combo } } as unknown as NxcConfig;
...
function customModelsConfig(models: Array<{ id: string; provider: string; modelId: string }>): NxcConfig {
  return {
    providers: {
...
test("an absent customModels key is left absent", () => {
  const config = { providers: { huggingface: { adapter: "openai-chat" } } } as unknown as NxcConfig;
  expect(dropProviderCustomModels(config, "huggingface")).toBe(0);
  expect(Object.hasOwn(config, "customModels")).toBe(false);
});
