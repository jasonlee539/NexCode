...
function cfg(extra?: Partial<NxcConfig>): NxcConfig {
  return {
    port: 10100,
    hostname: "127.0.0.1",
...
describe("relocated OpenCode serializer (accept criterion 1)", () => {
  ...
  test("buildClientConfig('opencode') emits the same entries as the golden", () => {
    const block = opencodeConfig().provider.nexcode!;
  ...
  test("carries the V1 schema and only the nexcode provider key", () => {
    const config = opencodeConfig();
    expect(config.$schema).toBe("https://opencode.ai/config.json");
    expect(Object.keys(config.provider)).toEqual(["nexcode"]);
  });

  test("a non-loopback bind moves admission to the header branch", () => {
    const block = opencodeConfig(ctx({ config: cfg({ hostname: "0.0.0.0" }) })).provider.nexcode!;
    expect(block.options.apiKey).toBeUndefined();
    expect(block.options.headers).toEqual({ "x-nexcode-api-key": OPENCODE_API_KEY_ENV_REF });
  });

  test("a loopback bind keeps the apiKey branch", () => {
    const block = opencodeConfig(ctx({ config: cfg({ hostname: "127.0.0.1" }) })).provider.nexcode!;
  ...
  test("the label carries the provider suffix, not a bare display name", () => {
    const models = opencodeConfig().provider.nexcode!.models;
...
describe("Pi serializer (accept criterion 2)", () => {
  test("models is an array keyed by id, not a keyed object", () => {
    const provider = piConfig().providers.nexcode!;
  ...
  test("cost is omitted on every entry — zeros would assert routed models are free", () => {
    for (const model of piConfig().providers.nexcode!.models) {
  ...
  test("reasoning is emitted only for rows with a non-empty effort ladder", () => {
    ...
    const models = config.providers.nexcode!.models;
    ...
    expect(ultraOnly.providers.nexcode!.models[0]!.thinkingLevelMap).toMatchObject({
      max: "ultra",
      off: null,
      minimal: null,
    });
  ...
  test("contextWindow and maxTokens are omitted when the context window is unknown", () => {
    const entry = piConfig().providers.nexcode!.models.find(model => model.id === "custom/no-context")!;
  ...
  test("maxTokens uses the schema budget and clamps to a smaller context window", () => {
    const models = piConfig().providers.nexcode!.models;
  ...
  test("Pi reuses the OpenCode label rule verbatim", () => {
    const piNames = piConfig().providers.nexcode!.models.map(model => model.name).sort();
    const opencodeNames = Object.values(opencodeConfig().provider.nexcode!.models)
...
describe("OMP serializer", () => {
  test("writes the full routed catalog in OMP's models.yml provider shape", () => {
    ...
    expect(document.providers.nexcode!.models.map(model => model.id)).toEqual([
      "anthropic/claude-opus-5",
      "custom/no-context",
      "gpt-5.6-luna",
      "tiny/small-ctx",
    ...
    expect(built.text).toContain("apiKey: nexcode-loopback");
  });
  ...
  test("keeps the provider on completions while opting native OpenAI models into Responses", () => {
    ...
    const document = buildClientConfig("omp", ctx({ models })) as {
      providers: { nexcode: { api: string; models: Array<Record<string, unknown>> } };
    };
    const provider = document.providers.nexcode;
    expect(provider.api).toBe("openai-completions");
  ...
  test("emits OMP reasoning metadata only for the accepted effort vocabulary", () => {
    const document = buildClientConfig("omp", ctx({
      ...
      providers: { nexcode: { models: Array<Record<string, unknown>> } };
    };
    const model = document.providers.nexcode.models[0]!;
  ...
  test("does not emit reasoning or thinking when no supported efforts are declared", () => {
    ...
    expect(document.providers.nexcode.models[0]).not.toHaveProperty("reasoning");
    expect(document.providers.nexcode.models[0]).not.toHaveProperty("thinking");
  });
});

describe("DSH rc.6 serializer", () => {
  test("owns only llm-pi-ai.providers.nexcode with a loopback Responses profile", () => {
    const document = dshConfig();
    expect(Object.keys(document)).toEqual(["llm-pi-ai"]);
    expect(Object.keys(document["llm-pi-ai"].providers)).toEqual(["nexcode"]);
    const provider = document["llm-pi-ai"].providers.nexcode!;
    expect(provider.displayName).toBe("NexCode");
    expect(provider.api).toBe("openai-responses");
    expect(provider.baseURL).toBe(BASE_URL);
    expect(provider.headers).toEqual({ Authorization: "Bearer nxc_data_dsh" });
  ...
  test("exports only representable modalities and authoritative context windows", () => {
    ...
    const models = document["llm-pi-ai"].providers.nexcode!.models;
  ...
  test("Direct drops native OpenAI and OpenAI-backed combos, failing closed on unknown combos", () => {
    ...
    const ids = dshConfig(ctx({ models, config: direct }))["llm-pi-ai"].providers.nexcode!.models.map(model => model.id);
    ...
    expect(dshConfig(ctx({ models, config: pool }))["llm-pi-ai"].providers.nexcode!.models).toHaveLength(models.length);
  });
});

describe("no credential ever reaches the output (accept criterion 3)", () => {
  const LIVE_KEY = "nxc_live_do_not_serialize_0123456789";

  test("neither serializer emits an nxc_ key even when one exists in config", () => {
    const withKey = cfg({ apiKeys: [{ key: LIVE_KEY }] } as Partial<NxcConfig>);
    ...
    for (const client of EXPORT_CLIENT_IDS) {
      ...
      expect(serialized.replaceAll("nxc_data_dsh", "")).not.toContain("nxc_");
...
describe("stable ordering (accept criterion 4)", () => {
  ...
  test("a duplicate namespaced row is emitted once by both serializers", () => {
    ...
    expect(Object.keys(opencodeConfig(dupes).provider.nexcode!.models)).toEqual(["gpt-5.6-luna"]);
    expect(piConfig(dupes).providers.nexcode!.models).toHaveLength(1);
    expect(piConfig(dupes).providers.nexcode!.models[0]!.name).toBe("gpt-5.6-luna (native)");
  });
});
...
`);
  ...
  test("the OMP destination follows its global agent directory and profile selectors", () => {
    const home = mkdtempSync(join(tmpdir(), "nxc-omp-destination-"));
    try {
  ...
  test("apiKeyEnv and exportHint name the variable the config references", () => {
    ...
    for (const id of EXPORT_CLIENT_IDS) {
      expect(EXPORT_CLIENTS[id].exportHint).not.toContain("nxc_");
    }
  });
  ...
  test("an empty catalog still yields a structurally valid document", () => {
    ...
    expect(opencodeConfig(empty).provider.nexcode!.models).toEqual({});
    expect(piConfig(empty).providers.nexcode!.models).toEqual([]);
  });
});
