...
beforeEach(() => {
  ...
  testDir = mkdtempSync(join(tmpdir(), "nxc-overlay-delete-"));
...
describe("provider deletion with disk-only preservation", () => {
  test("an owner that previously owned a provider can delete it without stale owners resurrecting it", () => {
    ...
    const externallyEdited = JSON.parse(readFileSync(getConfigPath(), "utf8")) as NxcConfig;
    ...
    let persisted = JSON.parse(readFileSync(getConfigPath(), "utf8")) as NxcConfig;
    ...
    persisted = JSON.parse(readFileSync(getConfigPath(), "utf8")) as NxcConfig;
