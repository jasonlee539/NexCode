...
beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), "nxc-issue-702-"));
  process.env.NEXCODE_HOME = testHome;
  delete process.env.NEXCODE_API_AUTH_TOKEN;
  clearResponseStateMemoryForTests();
  resetSubagentModelFallbackStateForTests();
  isolatedCodexHome = installIsolatedCodexHome("nxc-issue-702-codex-");
});
...
describe("Issue #702 expired forward replay state", () => {
  test("known continuation spill failure returns terminal structured previous_response_not_found before upstream I/O", async () => {
    ...
    const routeClasses: Array<{ config: NxcConfig; model: string }> = [
