...
beforeEach(() => {
  ...
  testDir = join(tmpdir(), `nxc-proj-warn-${Date.now()}`);
...
describe("isGlobalNexcodeRoutingActive", () => {
...
model_provider = "nexcode"
`;
    expect(isGlobalNexcodeRoutingActive("unused", text)).toBe(true);
  });

  test("does not treat dormant model_providers.nexcode table as active routing", () => {
    const text = `
[model_providers.nexcode]
name = "nexcode"
base_url = "http://127.0.0.1:10100/v1"
`;
    expect(isGlobalNexcodeRoutingActive("unused", text)).toBe(false);
  });
});
...
describe("collectProjectCodexConfigWarnings", () => {
  test("does not discover the global config when walking through its parent directory", () => {
    ...
    writeFileSync(codexConfigPath, `model_provider = "nexcode-retry"`);
  ...
  test("does not discover a project candidate that aliases the global config through a symlink", () => {
    ...
    writeFileSync(candidatePath, `model_provider = "nexcode-retry"`);
...
describe("explainProjectConfigBypass", () => {
  ...
  test("does not mislabel NexCode-prefixed provider ids as OpenCode", () => {
    expect(explainProjectConfigBypass(warningFor("nexcode"))).toContain("uses NexCode ");
    expect(explainProjectConfigBypass(warningFor("nexcode-retry")))
      .toContain("uses nexcode-retry ");
