...
const originalHome = process.env.NEXCODE_HOME;
const homes: string[] = [];

afterEach(() => {
  if (originalHome === undefined) delete process.env.NEXCODE_HOME;
  else process.env.NEXCODE_HOME = originalHome;
...
describe("OAuth provider reconciliation", () => {
  test("heals a stale Cursor all-models noVisionModels stamp down to the curated list", () => {
    const home = mkdtempSync(join(tmpdir(), "nxc-cursor-novision-reconcile-"));
    homes.push(home);
  ...
  test("refreshes a saved Antigravity 3.5 preset without touching credentials or user fields", async () => {
    const home = mkdtempSync(join(tmpdir(), "nxc-gemini-36-reconcile-"));
    homes.push(home);
    process.env.NEXCODE_HOME = home;
  ...
  test("preserves explicit Antigravity live discovery when authMode is omitted or non-OAuth", () => {
    const home = mkdtempSync(join(tmpdir(), "nxc-antigravity-authmode-reconcile-"));
    homes.push(home);
    process.env.NEXCODE_HOME = home;
  ...
  test("refreshes Grok 4.6 levels while runtime fills the default without overwriting user intent", () => {
    const home = mkdtempSync(join(tmpdir(), "nxc-grok-46-reconcile-"));
    homes.push(home);
    process.env.NEXCODE_HOME = home;
