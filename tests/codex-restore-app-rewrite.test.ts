 ...
 * the proxy URL stops being recognized as ours and survives `nxc stop` / `nxc restore`
...
const REINJECT_AND_READ_JOURNAL = [
  ...
  '  const journal = JSON.parse(fs.readFileSync(path.join(process.env.CODEX_HOME, "nexcode-journal.json"), "utf8"));',
...
const REINJECT_AFTER_USER_EDIT_RESTORE = [
  ...
  '  const profileExistsAfterRestore = fs.existsSync(path.join(process.env.CODEX_HOME, "nexcode.config.toml"));',
...
describe("#1798 restore after the Codex app rewrites the config", () => {
  ...
  beforeEach(() => {
    testDir = mkdtempSync(join(tmpdir(), "nxc-1798-"));
  });
