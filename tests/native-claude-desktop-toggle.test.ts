...
async function dispatch(path: string, init?: RequestInit, deps: ManagementApiDeps = {}, inputConfig: NxcConfig = config()) {
...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), "nxc-desktop-toggle-"));
  library = join(root, "desktop-library");
  previousHome = process.env.NEXCODE_HOME;
  previousLibrary = process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR;
  process.env.NEXCODE_HOME = root;
  process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR = library;
...
afterEach(() => {
  ...
  if (previousLibrary === undefined) delete process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR;
  else process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR = previousLibrary;
...
test("OFF on a missing or empty library is an idempotent no-op with no footprint", async () => {
  ...
  mkdirSync(empty);
  process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR = empty;
...
test("OFF removes an owned drifted gateway even without a saved fingerprint", async () => {
  ...
  writeFileSync(join(library, "_meta.json"), JSON.stringify({ appliedId: id, entries: [{ id, name: "nexcode" }] }));
...
test("POST /apply enables from a stale OFF server snapshot instead of cancelling itself", async () => {
  ...
  const staleSnapshot = { ...config(), clientIntegrations: { "claude-desktop": false } } as NxcConfig;
