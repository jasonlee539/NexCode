/**
 * Injected tests for `nxc ready` (parseReadyArgs + runReady).
 *
...
// `nxc ready` must reject invalid arguments with exit 64 BEFORE the global
...
// #764 follow-up: in NXC_SERVICE context a healthy proxy from ANY source must
// end handleStart with exit 0, so the nexcode-service.cmd `:loop` wrapper
...
describe("handleStart NXC_SERVICE exit guard (source-level)", () => {
  const cliSource = readFileSync(join(import.meta.dir, "../src/cli/index.ts"), "utf8");

  test("an already-live proxy exits 0 in NXC_SERVICE context", () => {
    expect(cliSource).toMatch(/process\.env\.NXC_SERVICE === "1"/);
  ...
  test("wrapper kill matches the canonical paths of THIS installation, not bare filenames", () => {
    ...
    // wrapper from another NexCode home (or any process whose command line
    ...
    expect(killBody![0]).not.toMatch(/\$pats = @\('nexcode-service\.cmd'\)/);
  });
