...
function installTempHomes(): { codexHome: string; nexcodeHome: string; restore(): void } {
  const previousCodexHome = process.env.CODEX_HOME;
  const previousNexCodeHome = process.env.NEXCODE_HOME;
  const codexHome = mkdtempSync(join(tmpdir(), "nxc-refresh-codex-"));
  const nexcodeHome = mkdtempSync(join(tmpdir(), "nxc-refresh-nxc-"));
  tempHomes.push(codexHome, nexcodeHome);
...
describe("Codex catalog refresh", () => {
  ...
  test("invalidateCodexModelsCache reports real cache write success and failure cases", () => {
    ...
    try {
      writeFileSync(join(success.codexHome, "config.toml"), 'model_catalog_json = "nexcode-catalog.json"\n', "utf8");
      writeFileSync(join(success.codexHome, "nexcode-catalog.json"), nativeCatalogFixture("gpt-5.6-sol"), "utf8");
    ...
    try {
      writeFileSync(join(malformedCatalog.codexHome, "config.toml"), 'model_catalog_json = "nexcode-catalog.json"\n', "utf8");
      writeFileSync(join(malformedCatalog.codexHome, "nexcode-catalog.json"), "{not-json", "utf8");
    ...
    try {
      writeFileSync(join(unwritableCache.codexHome, "config.toml"), 'model_catalog_json = "nexcode-catalog.json"\n', "utf8");
      writeFileSync(join(unwritableCache.codexHome, "nexcode-catalog.json"), nativeCatalogFixture(), "utf8");
