...
describe("anthropic-flavor ModelInfo discovery entries (devlog 130 B4b)", () => {
  ...
  test("readable id style serves claude-nxc ids with hash fallback + readable [1m] variants (devlog 050)", () => {
    const auto = { enabled: true, compactWindow: 350_000 };
    ...
    const ids = infos.map(i => i.id);
    expect(ids).toContain("claude-nxc-native--gpt-5.5");
    // 272k native: NO [1m] variant under the authoritative-window contract.
    expect(ids).not.toContain("claude-nxc-native--gpt-5.5[1m]");
    expect(ids).toContain("claude-nxc-cursor--gpt-5.6-luna");
    expect(ids).toContain("claude-nxc-cursor--gpt-5.6-luna[1m]");
    ...
    expect(hashed.map(i => i.id).some(id => id.startsWith("claude-nxc-"))).toBe(false);
  });
});
