...
function fixture(): Fixture {
  const root = mkdtempSync(join(tmpdir(), "nxc-start-owner-"));
  roots.push(root);
  const codexHome = join(root, "codex");
  const nxcHome = join(root, "nxc");
  const home = join(root, "home");
  const runtime = join(root, "runtime");
  for (const path of [codexHome, nxcHome, home, runtime]) mkdirSync(path, { recursive: true });
  const configPath = join(codexHome, "config.toml");
  const journalPath = join(codexHome, "nexcode-journal.json");
  const pidPath = join(nxcHome, "nxc.pid");
...
async function runCli(fx: Fixture, argv: string[]): Promise<{ exitCode: number; stdout: string; stderr: string }> {
  ...
  const completed = await Promise.race([
    ...
    new Promise<never>((_, reject) => setTimeout(() => reject(new Error(`CLI watchdog: nxc ${argv.join(" ")}`)), 10_000)),
  ]);
...
async function startOwner(fx: Fixture): Promise<ReturnType<typeof Bun.spawn>> {
  ...
  children.push(child);
  const runtimePath = join(fx.nxcHome, "runtime-port.json");
