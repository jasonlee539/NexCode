...
beforeEach(() => {
  ...
  testDir = mkdtempSync(join(tmpdir(), "nxc-profile-editor-"));
...
describe("routing profile management editor API", () => {
  test("GET exposes the configured alias for editor round-trips", async () => {
    ...
    expect(body.profiles?.[0]).toMatchObject({ id: "fast", alias: "nxc/fast" });
  });
  ...
  test("PUT creates a validated normalized profile and refreshes the catalog", async () => {
    ...
    expect(body.profile?.alias).toBe("nxc/balanced");
  ...
  test("PUT update replaces an existing profile, persists once, and refreshes the catalog", async () => {
    ...
    expect(body.success).toBe(true);
    expect(body.profile?.alias).toBe("nxc/faster");
  ...
  test("PUT update migrates config references when the profile alias changes", async () => {
    const config = baseConfig();
    config.disabledModels = ["nxc/fast"];
    config.subagentModels = ["nxc/fast", "a/m1"];
    config.subagentModelFallback = ["nxc/fast", "a/m1"];
    config.injectionModel = "nxc/fast";
    config.shadowCallIntercept = { model: "nxc/fast" };
    config.claudeCode = {
      enabled: true,
      model: "nxc/fast",
      smallFastModel: "a/m1",
      modelMap: { "nxc/fast": "a/m1", "a/m2": "nxc/fast" },
    };
    let saves = 0;
    const req = new ManagementRequest("http://localhost/api/routing-profiles", {
      method: "PUT",
    ...
    expect(response?.status).toBe(200);
    expect(config.disabledModels).toEqual(["nxc/faster"]);
    expect(config.subagentModels).toEqual(["nxc/faster", "a/m1"]);
    expect(config.subagentModelFallback).toEqual(["nxc/faster", "a/m1"]);
    expect(config.injectionModel).toBe("nxc/faster");
    expect(config.shadowCallIntercept?.model).toBe("nxc/faster");
    expect(config.claudeCode?.model).toBe("nxc/faster");
    expect(config.claudeCode?.smallFastModel).toBe("a/m1");
    expect(config.claudeCode?.modelMap).toEqual({ "nxc/faster": "a/m1", "a/m2": "nxc/faster" });
    expect(saves).toBe(1);
  });
  ...
  test("PUT update rejects an account-selector namespace before reference migration", async () => {
    ...
    config.codexAccounts = [{ id: privateAccountId, email: privateEmail, isMain: false }];
    config.codexAccountNamespaces = { side: privateAccountId };
    const disabledModels = ["nxc/fast"];
    const subagentModels = ["nxc/fast", "a/m1"];
  ...
  test("PUT update rejects a modelMap key collision instead of silently dropping a mapping", async () => {
    ...
    expect(config.routingProfiles?.fast).toMatchObject({ alias: "nxc/fast" });
    expect(config.claudeCode?.modelMap).toEqual({ "nxc/fast": "a/m1", "nxc/faster": "a/m2" });
    expect(saves).toBe(0);
  });
