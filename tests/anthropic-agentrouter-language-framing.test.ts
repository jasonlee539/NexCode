...
function providerAt(baseUrl: string): NxcProviderConfig {
  return { adapter: "anthropic", baseUrl, apiKey: "key", authMode: "key" };
}

function requestWith(messages: NxcMessage[]): NxcParsedRequest {
  return {
...
async function bodyFor(baseUrl: string, messages: NxcMessage[]): Promise<WireBody> {
