...
const BOUNDED_READ_TEST_SEAM = Symbol.for("nexcode.native-profile-store.bounded-read-test-seam");
...
function context(): NativeProfileContext {
  const base = mkdtempSync(join(tmpdir(), "nxc-native-profile-store-"));
  roots.push(base);
  const requestedCodexHome = join(base, "codex");
  const configDir = join(base, "nexcode");
  mkdirSync(requestedCodexHome, { mode: 0o700 });
  const codexHome = realpathSync.native(requestedCodexHome);
  const rootDir = join(codexHome, ".nexcode-native-main-profiles");
...
describe("native-profile path ownership", () => {
  test("shares authoritative metadata by canonical CODEX_HOME but isolates staging by NEXCODE_HOME", () => {
    const root = mkdtempSync(join(tmpdir(), "nxc-native-profile-layout-"));
    roots.push(root);
    const codexHome = join(root, "codex");
    const configA = join(root, "nexcode-a");
    const configB = join(root, "nexcode-b");
    mkdirSync(codexHome);
    mkdirSync(configA);
    mkdirSync(configB);
    ...
    expect(a.rootDir).toBe(join(a.codexHome, ".nexcode-native-main-profiles"));
    ...
    const requestedMissing = join(root, "missing-parent", "nexcode");
...
describe("native-profile recovery journal storage", () => {
  ...
  test.skipIf(process.platform === "win32")("rejects a vault replaced by a FIFO after layout validation", () => {
    const base = mkdtempSync(join(tmpdir(), "nxc-native-profile-fifo-race-"));
    roots.push(base);
    ...
    const childSource = `
      ...
      const codexHome = process.env.NXC_NATIVE_PROFILE_CODEX_HOME;
      const configDir = process.env.NXC_NATIVE_PROFILE_CONFIG_DIR;
      ...
      store[Symbol.for("nexcode.native-profile-store.bounded-read-test-seam")] = {
        beforeOpen(path) {
    ...
    const child = spawnSync(process.execPath, ["--eval", childSource], {
      ...
      env: {
        ...process.env,
        NXC_NATIVE_PROFILE_CODEX_HOME: codexHome,
        NXC_NATIVE_PROFILE_CONFIG_DIR: configDir,
      },
      timeout: 2_000,
    });
