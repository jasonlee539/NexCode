...
function baseConfig(storageCleanupPolicy?: NxcConfig["storageCleanupPolicy"]): NxcConfig {
  return {
    port: 0,
    hostname: "127.0.0.1",
...
async function expectLivePolicyJobSink(server: StartedServer): Promise<void> {
  isolatedCodexHome = installIsolatedCodexHome("nxc-server-background-sink-");
  const policy = normalizeStorageCleanupPolicy({
    enabled: true,
...
beforeEach(async () => {
  ...
  testDir = mkdtempSync(join(tmpdir(), "nxc-server-background-lifecycle-"));
  process.env.NEXCODE_HOME = testDir;
  process.env.NEXCODE_API_AUTH_TOKEN = "server-background-test-token";
});
...
describe("server background lifecycle", () => {
  ...
  test("last-server stop aborts and drains a startup policy Worker", async () => {
    isolatedCodexHome = installIsolatedCodexHome("nxc-server-background-worker-");
