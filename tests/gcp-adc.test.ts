...
function parsed(modelId = "gemini-3-pro"): NxcParsedRequest {
  return {
    modelId,
    stream: true,
    context: { messages: [{ role: "user", content: "hi" }], systemPrompt: [], tools: [] },
    options: {},
  } as unknown as NxcParsedRequest;
}

beforeAll(async () => {
  tmp = mkdtempSync(join(tmpdir(), "nxc-gcp-adc-"));
...
describe("google adapter vertex mode", () => {
  test("vertex + api key -> aiplatform host + x-goog-api-key (no ADC fetch)", async () => {
    const provider = { adapter: "google", baseUrl: "https://generativelanguage.googleapis.com", googleMode: "vertex", apiKey: "real-key" } as NxcProviderConfig;
  ...
  test("vertex + ADC -> regional host + Authorization Bearer", async () => {
    setEnv("GOOGLE_APPLICATION_CREDENTIALS", saPath);
    const provider = { adapter: "google", baseUrl: "https://x", googleMode: "vertex", project: "proj-1", location: "us-central1" } as NxcProviderConfig;
    const req = await createGoogleAdapter(provider).buildRequest(parsed());
  ...
  test("vertex + ADC + location global -> global aiplatform host", async () => {
    setEnv("GOOGLE_APPLICATION_CREDENTIALS", saPath);
    const provider = { adapter: "google", baseUrl: "https://x", googleMode: "vertex", project: "proj-1", location: "global" } as NxcProviderConfig;
    const req = await createGoogleAdapter(provider).buildRequest(parsed());
  ...
  test("ai-studio default mode is unchanged (no regression)", async () => {
    const provider = { adapter: "google", baseUrl: "https://generativelanguage.googleapis.com", apiKey: "ai-key" } as NxcProviderConfig;
    const req = await createGoogleAdapter(provider).buildRequest(parsed());
