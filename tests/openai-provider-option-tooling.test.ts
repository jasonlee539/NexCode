...
function validArtifacts(): { root: string; paths: string[] } {
  const root = mkdtempSync(join(tmpdir(), "nxc-provider-option-evidence-"));
  roots.push(root);
...
describe("OpenAI provider-option live policy and runtime isolation", () => {
  ...
  test("removes credential and proxy sentinels while preserving safe process state", () => {
    const source = {
      ...
      NEXCODE_HOME: "sentinel",
      nexcode_base_url: "sentinel",
      HTTP_PROXY: "sentinel",
      https_proxy: "sentinel",
      ALL_PROXY: "sentinel",
      all_proxy: "sentinel",
    };
    const env = buildSanitizedRuntimeEnv(source, "/tmp/nxc", "/tmp/codex");
    expect(env.PATH).toBe("/bin");
    expect(env.OPENAI_API_KEY).toBeUndefined();
    expect(env.openai_base_url).toBeUndefined();
    expect(env.codex_api_key).toBeUndefined();
    expect(env.nexcode_base_url).toBeUndefined();
    expect(env.HTTP_PROXY).toBeUndefined();
    expect(env.https_proxy).toBeUndefined();
    expect(env.ALL_PROXY).toBeUndefined();
    expect(env.all_proxy).toBeUndefined();
    expect(env.NEXCODE_HOME).toBe("/tmp/nxc");
    expect(env.CODEX_HOME).toBe("/tmp/codex");
    expect(env.NO_PROXY).toBe("127.0.0.1,localhost,::1");
    expect(env.no_proxy).toBe("127.0.0.1,localhost,::1");
    expect(env.NXC_SHIM_BYPASS).toBe("1");
  });

  test("keeps the fixture admission token through an installed Unix shim without reading its token file", () => {
    if (process.platform === "win32") return;
    const root = mkdtempSync(join(tmpdir(), "nxc-runtime-shim-isolation-"));
    roots.push(root);
    ...
    const env = buildSanitizedRuntimeEnv({ PATH: process.env.PATH }, "/tmp/nxc", "/tmp/codex");
