...
const TEST_NEXCODE_HOME = join(TEST_DIR, "nexcode");
let prevNexcodeHome: string | undefined;
...
describe("doctor", () => {
  beforeEach(() => {
    prevNexcodeHome = process.env.NEXCODE_HOME;
    prevCodexHome = process.env.CODEX_HOME;
    prevHttpsProxy = process.env.HTTPS_PROXY;
    prevLowerHttpsProxy = process.env.https_proxy;
    prevProxyRef = process.env.NXC_TEST_PROXY_REF;
    prevAdminToken = process.env.NEXCODE_ADMIN_AUTH_TOKEN;
    if (existsSync(TEST_DIR)) rmSync(TEST_DIR, { recursive: true });
    mkdirSync(TEST_CODEX_HOME, { recursive: true });
    mkdirSync(TEST_NEXCODE_HOME, { recursive: true });
    process.env.NEXCODE_HOME = TEST_NEXCODE_HOME;
    process.env.CODEX_HOME = TEST_CODEX_HOME;
    delete process.env.HTTPS_PROXY;
    delete process.env.https_proxy;
    delete process.env.NXC_TEST_PROXY_REF;
  });

  afterEach(() => {
    if (prevNexcodeHome === undefined) delete process.env.NEXCODE_HOME;
    else process.env.NEXCODE_HOME = prevNexcodeHome;
    ...
    if (prevProxyRef === undefined) delete process.env.NXC_TEST_PROXY_REF;
    else process.env.NXC_TEST_PROXY_REF = prevProxyRef;
    if (prevAdminToken === undefined) delete process.env.NEXCODE_ADMIN_AUTH_TOKEN;
    else process.env.NEXCODE_ADMIN_AUTH_TOKEN = prevAdminToken;
    if (existsSync(TEST_DIR)) rmSync(TEST_DIR, { recursive: true });
  });
  ...
  test("path report flips auth.json/config.json from absent to present", () => {
    ...
    const cfg = () => rows.find(r => r.label === "NEXCODE_HOME/config.json")!;
    expect(auth().exists).toBe(false);
    expect(cfg().exists).toBe(false);

    writeFileSync(join(TEST_CODEX_HOME, "auth.json"), "{}");
    writeFileSync(join(TEST_NEXCODE_HOME, "config.json"), "{}");
    rows = collectPaths();
  ...
  test("Orca home diagnostic warns only for the Windows Orca runtime mismatch", () => {
    ...
    expect(mismatch.warning).toContain("NexCode injection will not reach that app");
    expect(mismatch.effectiveCodexHome).toContain("C:\\Users\\[USER]\\");
    expect(mismatch.effectiveCodexHome).not.toContain("alice");
    expect(mismatch.action).toContain("nxc service uninstall");
    expect(mismatch.action).toContain("nxc service install");
  ...
  test("resolveCodexHomeDir discovers a single Windows Codex Desktop home from WSL", () => {
    ...
    writeFileSync(join(windowsCodexHome, "config.toml"), "model_provider = \"nexcode\"\n");
  ...
  test("detectFsType flags /mnt drvfs mounts and leaves ext4 home alone", () => {
    ...
    const c = detectFsType("/mnt/c/Users/test/.nexcode", mounts);
    expect(c.isDrvfs).toBe(true);
    expect(c.isMntDrive).toBe(true);
    expect(c.fstype).toBe("drvfs");

    const home = detectFsType("/home/test/.nexcode", mounts);
  ...
  test("collectConfiguredProxy reports effective config proxy without leaking values", () => {
    writeFileSync(join(TEST_NEXCODE_HOME, "config.json"), JSON.stringify({ proxy: "${NXC_TEST_PROXY_REF}" }));

    let diagnostic = collectConfiguredProxy();
    expect(diagnostic.configured).toBe(true);
    expect(diagnostic.present).toBe(false);
    expect(diagnostic.detail).toContain("NXC_TEST_PROXY_REF");

    process.env.NXC_TEST_PROXY_REF = "http://user:secret@proxy.example.test:8080";
...
describe("service memory section (#314 WP4)", () => {
  ...
  test("fetchServiceMemory: ok / unauthorized / unreachable / malformed", async () => {
    process.env.NEXCODE_ADMIN_AUTH_TOKEN = "admin-token-must-not-leave-doctor";
    ...
    const ok = await fetchServiceMemory(target, {
      ...
      fetchImpl: (async (_input, init) => {
        ...
        expect(headers.get("x-nexcode-api-key")).toBeNull();
  ...
  test("interpretation: high RSS with large JS counters asks for corroboration", () => {
    ...
    expect(lines.some(l => l.includes("likely an nexcode bug"))).toBe(false);
  });
  ...
  test("guidance gating: win32 + auto-known-bad prints version-claiming guidance", () => {
    // A bundled runtime is the case where "set NEXCODE_BUN_PATH" is still the right advice.
    const lines = formatServiceMemoryLines({ status: "ok", data: { ...baseData, bunRuntimeSource: "bundled" } });
    expect(lines.some(l => l.includes("NEXCODE_BUN_PATH"))).toBe(true);
    // Version-claiming, never binary-claiming.
    expect(lines.join("\n")).not.toContain("bundled binary");
  });

  test("guidance gating: an active override is never told to set NEXCODE_BUN_PATH again (#848)", () => {
    const lines = formatServiceMemoryLines({
      status: "ok",
      data: { ...baseData, bunRuntimeSource: "override" },
    });
    const text = lines.join("\n");
    expect(text).toContain("NEXCODE_BUN_PATH is already active");
    expect(text).not.toContain("set NEXCODE_BUN_PATH to a runtime you trust");
  ...
  test("guidance gating: a process-provenance runtime is not described as bundled", () => {
    ...
    expect(text).toContain("set NEXCODE_BUN_PATH to a runtime you trust");
  });
  ...
  test("guidance gating: darwin auto-off or fixed Windows runtime prints no override guidance", () => {
    ...
    expect(darwin.some(l => l.includes("NEXCODE_BUN_PATH"))).toBe(false);

    const fixedRuntime = formatServiceMemoryLines({
      status: "ok",
      data: { ...baseData, eagerRelay: { useEagerRelay: true, reason: "auto-fixed-runtime" } },
    });
    expect(fixedRuntime.some(l => l.includes("NEXCODE_BUN_PATH"))).toBe(false);
  });
  ...
  test("proxyDownRestartHint names the symptom and both restart paths", () => {
    ...
    expect(hint).toContain("nxc start");
    expect(hint).toContain("nxc service install");
  });

  test("proxyDownRestartHint prefers 'nxc service start' when a service is installed", () => {
    const hint = proxyDownRestartHint({ proxyRunning: false, port: 12000, serviceViable: true });
    expect(hint).toContain("nxc service start");
    expect(hint).toContain("127.0.0.1:12000");
    expect(hint).not.toContain("nxc service install");
  });
  ...
  test("an installed but unhealthy service is pointed at repair, not install", () => {
    ...
    expect(broken).toContain("nxc service repair");
    expect(broken).not.toContain("nxc service install");

    const absent = proxyDownRestartHint({ proxyRunning: false, port: 10100, serviceViable: false, serviceInstalled: false });
    expect(absent).toContain("nxc service install");
    ...
    expect(conflict).toContain("nxc service install");
  });
});
...
describe("doctor abandoned response-state temps", () => {
  ...
  test("reports reclaimable files without removing them, and names the opt-in flag", () => {
    ...
    expect(lines.join("\n")).toContain("nxc doctor --reclaim-response-temps");
  });
...
describe("doctor reclaim wiring (end to end)", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = join(tmpdir(), `nxc-doctor-temps-${Date.now()}-${Math.random().toString(16).slice(2)}`);
    mkdirSync(tempHome, { recursive: true });
    process.env.NEXCODE_HOME = tempHome;
    logged = [];
  ...
  const seedStaleTemp = (): string => {
    ...
    const path = join(tempHome, `responses-state.json.nxc.${deadPid}.1.tmp`);
  ...
  const runOfflineDoctor = (args: string[]) => runDoctor(args, {
    probeWhamFn: async () => ({
      ok: false,
      status: null,
      durationMs: 0,
  ...
  test("the opt-in flag removes it", async () => {
    const path = seedStaleTemp();
    await runOfflineDoctor(["--reclaim-response-temps"]);
  ...
  test("a mistyped flag warns instead of silently reporting", async () => {
    const path = seedStaleTemp();
    await runOfflineDoctor(["--reclaim-response-temp"]);
    expect(existsSync(path)).toBe(true);
