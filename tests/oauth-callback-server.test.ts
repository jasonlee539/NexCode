...
import { OAuthCallbackFlow, oauthCallbackSuccessHtml } from "../src/oauth/callback-server";
...
describe("OAuth callback server defaults", () => {
  test("desktop callback page returns to the native app without claiming validation is finished", () => {
    const html = oauthCallbackSuccessHtml(true);

    expect(html).toContain("nexcode://oauth-complete");
    expect(html).toContain("正在完成账号验证");
    expect(html).not.toContain("You can close this tab and return to nexcode");
  });

  test("non-desktop callback page does not register a native-app redirect", () => {
    expect(oauthCallbackSuccessHtml(false)).not.toContain("nexcode://oauth-complete");
  });
