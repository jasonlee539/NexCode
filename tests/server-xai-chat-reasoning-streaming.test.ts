...
beforeEach(async () => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("nxc-xai-chat-reasoning-codex-");
  testDir = mkdtempSync(join(tmpdir(), "nxc-xai-chat-reasoning-"));
...
describe("xAI OAuth Chat reasoning streaming", () => {
  test("bridges reasoning_content before content and response.completed", async () => {
    ...
    try {
      ...
      expect(outboundHeaders?.get("x-grok-client-identifier")).toBe("nexcode");
