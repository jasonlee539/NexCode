...
/** These cases sandbox CODEX_HOME/NEXCODE_HOME, so the installed service is not their evidence. */
...
beforeEach(() => {
  nexcodeHome = mkdtempSync(join(tmpdir(), "nxc-main-drain-server-"));
  codexHome = mkdtempSync(join(tmpdir(), "nxc-main-drain-codex-"));
...
afterEach(() => {
  ...
  clearMainAccountInfoCache();
  resetMainCodexAccountIdentityTrackingForTests();
  if (nexcodeHome) rmSync(nexcodeHome, { recursive: true, force: true });
