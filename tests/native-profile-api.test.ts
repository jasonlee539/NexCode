...
describe("native main profile management API", () => {
  test("long Direct/non-main work does not block switch while new main selection is fenced", async () => {
    ...
    try {
      const response = await handleNativeProfileAPI(request, new URL(request.url), {} as NxcConfig, { manager });
  ...
  test("constructor failures remain inside the redacted management error boundary", async () => {
    const missingHome = join(tmpdir(), `nxc-native-profile-missing-${crypto.randomUUID()}`);
  ...
  test("pending-recovery errors retain actionable public recovery commands", async () => {
    const message = "A native-profile recovery journal is pending. Run `nxc account main recover` or `nxc account main recover --rollback --yes` before registering or adding profiles.";
    const manager = {
  ...
  test("finish returns a typed non-200 after server-owned stage cleanup without a cleanup warning", async () => {
    const message = "A native-profile recovery journal is pending. Run `nxc account main recover` before adding profiles.";
    const manager = {
  ...
  test("retained manual markers and failed recovery never complete the gate", async () => {
    ...
    const retained = await handleNativeProfileAPI(retainedRequest, new URL(retainedRequest.url), {} as NxcConfig, {
    ...
    const failed = await handleNativeProfileAPI(failedRequest, new URL(failedRequest.url), {} as NxcConfig, {
