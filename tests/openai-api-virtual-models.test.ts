...
const moduleOriginalHome = process.env.NEXCODE_HOME;
afterEach(() => {
  globalThis.fetch = moduleOriginalFetch;
  if (moduleOriginalHome === undefined) delete process.env.NEXCODE_HOME;
  else process.env.NEXCODE_HOME = moduleOriginalHome;
});
...
describe("OpenAI API compact transport", () => {
  test("maps every Pro id to base, strips reasoning, buffers failures, caps bodies, and logs exactly once", async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), "nxc-openai-api-compact-"));
...
describe("OpenAI API Pro transport identities", () => {
  test("HTTP JSON, HTTP SSE, and real WebSocket keep base wire/client identity and virtual logs", async () => {
    const originalFetch = globalThis.fetch;
    const home = mkdtempSync(join(tmpdir(), "nxc-openai-api-pro-"));
    process.env.NEXCODE_HOME = home;
    saveConfig({
      port: 0,
      websockets: true,
