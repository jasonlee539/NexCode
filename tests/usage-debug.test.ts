...
beforeEach(() => {
  ...
  testDir = mkdtempSync(join(tmpdir(), "nxc-usage-debug-"));
...
describe("appendUsageDebug", () => {
  function sample(extra: Partial<{ requestId: string; ts: number }> = {}) {
    return {
      ts: extra.ts ?? 1,
      requestId: extra.requestId ?? "nxc-debug-1",
      provider: "chatgpt",
      model: "gpt-5.5",
  ...
  test("appends one JSON line with the expected shape and 0o600 perms", () => {
    ...
    expect(parsed.requestId).toBe("nxc-debug-1");
  ...
  test("rotates to the most recent USAGE_DEBUG_KEEP_LINES once USAGE_DEBUG_MAX_LINES is exceeded", () => {
    ...
    for (let i = 0; i < total; i++) {
      appendUsageDebug(sample({ requestId: `nxc-${i}`, ts: i }));
    }
    const path = usageDebugPath();
    const lines = readFileSync(path, "utf-8").split(/\r?\n/).filter(Boolean);
    ...
    expect(last.requestId).toBe(`nxc-${total - 1}`);
    expect(first.requestId).toBe(`nxc-${total - USAGE_DEBUG_KEEP_LINES}`);
  });
