...
function runStatusJson(nexcodeHome: string) {
...
describe("CLI status JSON", () => {
  test("status --json prints valid read-only diagnostics without secrets", () => {
    const nexcodeHome = mkdtempSync(join(tmpdir(), "nxc-status-json-"));
    try {
      ...
      const beforeFiles = readdirSync(nexcodeHome).sort();
      const result = runStatusJson(nexcodeHome);
      const afterFiles = readdirSync(nexcodeHome).sort();

      expect(result.status).toBe(0);
      expect(result.stderr).toBe("");
      expect(afterFiles).toEqual(beforeFiles);
      expect(existsSync(join(nexcodeHome, "nxc.pid"))).toBe(false);
      ...
      expect(parsed.paths?.pid).toBe(join(nexcodeHome, "nxc.pid"));
  ...
  test("status --json reports catalogClamp.runtimeVersion when clamp is active", async () => {
    ...
    const nexcodeHome = mkdtempSync(join(tmpdir(), "nxc-status-clamp-"));
    try {
      ...
      const fakeCodex = process.platform === "win32"
        ? join(nexcodeHome, "bin", "codex.cmd")
        : join(nexcodeHome, "bin", "codex");
      mkdirSync(join(nexcodeHome, "bin"), { recursive: true });
  ...
  test("status rejects unknown flags instead of silently printing human text", () => {
    ...
    try {
      ...
      expect(result.stderr).toContain("Usage: nxc status [--json]");
