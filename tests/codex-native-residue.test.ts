...
beforeEach(() => {
  ...
  codexHome = mkdtempSync(join(tmpdir(), "nxc-native-residue-codex-"));
  nexcodeHome = mkdtempSync(join(tmpdir(), "nxc-native-residue-nexcode-"));
...
function historyBackupPath(): string {
  ...
  return join(nexcodeHome, `codex-history-backup-${id}.json`);
}
...
const residueFixtures: Array<{
  ...
  },
  {
    ...
    arrange: () => writeFileSync(
      pathInCodexHome("nexcode.config.toml"),
  ...
  },
  {
    name: "routed catalog",
    surface: "catalog",
    arrange: () => writeFileSync(pathInCodexHome("nexcode-catalog.json"), routedCatalog()),
  },
  {
  ...
  },
  {
    ...
    surface: "journal",
    arrange: () => writeFileSync(pathInCodexHome("nexcode-journal.json"), JSON.stringify({
      version: 1,
  ...
  },
  {
    ...
    surface: "history",
    arrange: () => createHistoryDatabase("nexcode"),
  },
  {
...
test("an NexCode atomic-write artifact is indeterminate", () => {
  writeFileSync(pathInCodexHome("config.toml.nxc.123.1.tmp"), "partial");
  expect(classifyNativeRoutedResidue()).toMatchObject({
    kind: "indeterminate",
...
for (const shape of catalogPathShapes) {
  test(`configured catalog classification follows the ${shape.name} path`, () => {
    const outsideRoot = mkdtempSync(join(tmpdir(), "nxc-native-residue-catalog-outside-"));
  ...
  test(`production writer routes the ${shape.name} configured catalog ${configuredLeaf}`, async () => {
    ...
    const routedRows = catalog.models.filter(model =>
      typeof model.description === "string"
        && model.description.startsWith("Routed via nexcode → ")
    );
...
test("an atomic-write artifact beside the configured catalog is indeterminate", () => {
  const catalogPath = canonicalPathInCodexHome("nested/custom-catalog.json");
  const artifactPath = `${catalogPath}.nxc.42.7.tmp`;
  mkdirSync(pathInCodexHome("nested"));
  writeFileSync(pathInCodexHome("config.toml"), 'model_catalog_json = "nested/custom-catalog.json"\n');
  writeFileSync(catalogPath, JSON.stringify({ models: [] }));
...
test("the default catalog is still inspected when a custom catalog is configured", () => {
  const defaultCatalogPath = canonicalPathInCodexHome("nexcode-catalog.json");
  mkdirSync(pathInCodexHome("nested"));
  writeFileSync(pathInCodexHome("config.toml"), 'model_catalog_json = "nested/custom-catalog.json"\n');
...
for (const location of ["inside", "outside"] as const) {
  test(`the default catalog remains inspected with an absolute ${location} configured path`, () => {
    const outsideRoot = mkdtempSync(join(tmpdir(), "nxc-native-residue-default-outside-"));
...
test(`production-generated arbitrary bare combo alias ${arbitraryComboAlias} is routed residue`, async () => {
  const catalogPath = canonicalPathInCodexHome("nexcode-catalog.json");
  writeFileSync(catalogPath, JSON.stringify({ models: [] }));
  const config: NxcConfig = {
    port: 10100,
    defaultProvider: "fixture",
  ...
  expect(routedRows).toEqual([
    expect.objectContaining({
      ...
      description: "Routed via nexcode → combo (combo).",
      owned_by: "combo",
    }),
  ]);
  expect(classifyNativeRoutedResidue()).toMatchObject({
...
test(`arbitrary foreign row ${arbitraryForeignSlug} described as ${arbitraryForeignDescription} is indeterminate`, () => {
  writeFileSync(pathInCodexHome("nexcode-catalog.json"), JSON.stringify({
...
test("a native-tagged history row with routed latest rollout metadata refuses coordinator initialization", () => {
  createHistoryDatabase("openai", ["openai", "nexcode"]);
...
test("routed first rollout metadata is residue even when the latest metadata is native", () => {
  createHistoryDatabase("openai", ["nexcode", "openai"]);
...
test("an nexcode first rollout with invalid latest metadata is indeterminate", () => {
  createHistoryDatabase("openai");
  writeFileSync(
    pathInCodexHome("rollout.jsonl"),
    sessionMeta("thread-1", "nexcode") + "\n" + sessionMeta("thread-2", "openai") + "\n",
  );
...
test("a routed rollout without a trailing newline is residue", () => {
  createHistoryDatabase("openai");
  writeFileSync(pathInCodexHome("rollout.jsonl"), sessionMeta("thread-1", "nexcode"));
...
test("a routed rollout with a non-ASCII id split across the read chunk is residue", () => {
  ...
  const suffix = `","id":"thread-1","model_provider":"nexcode","source":"cli"}}\n`;
...
const indeterminateFixtures: Array<{
  ...
  },
  {
    ...
    surface: "profile",
    arrange: () => writeFileSync(pathInCodexHome("nexcode.config.toml"), "[features\n"),
  },
  {
    name: "malformed catalog JSON",
    surface: "catalog",
    arrange: () => writeFileSync(pathInCodexHome("nexcode-catalog.json"), "{not-json"),
  },
  {
  ...
  },
  {
    ...
    surface: "journal",
    arrange: () => writeFileSync(pathInCodexHome("nexcode-journal.json"), "{not-json"),
  },
  {
    name: "partial write",
    surface: "partial-write",
    arrange: () => writeFileSync(pathInCodexHome("nexcode-catalog.json.nxc.42.7.tmp"), ""),
  },
  {
...
test("user-owned non-NexCode content is clean and coordinator initialization succeeds", () => {
...
test("CODEX_HOME is resolved at call time", () => {
  const secondHome = mkdtempSync(join(tmpdir(), "nxc-native-residue-second-codex-"));
  try {
    writeFileSync(join(secondHome, "nexcode.config.toml"), buildProfileFile(10100, null));
    expect(classifyNativeRoutedResidue()).toEqual({ kind: "clean" });
...
test("a missing coordinator with only the generated profile refuses initialization", () => {
  writeFileSync(join(codexHome, "nexcode.config.toml"), buildProfileFile(10100, null));
