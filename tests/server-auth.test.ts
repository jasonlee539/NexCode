...
// recreate it while pointing NEXCODE_HOME there. `bun test --isolate` gives each file
...
const TEST_DIR = mkdtempSync(join(tmpdir(), "nxc-server-auth-"));
let isolatedCodexHome: IsolatedCodexHome | null = null;

function config(hostname?: string): NxcConfig {
  return {
    port: 10100,
    hostname,
...
function managementHeaders(initial?: HeadersInit): Headers {
  ...
  headers.set("x-nexcode-api-key", token);
  return headers;
}
...
function poolProviders(): NxcConfig["providers"] {
  return {
...
beforeEach(() => {
  isolatedCodexHome = installIsolatedCodexHome("nxc-server-auth-codex-");
});
...
describe("server local API auth", () => {
  ...
  test("loopback hostnames do not require nexcode API auth", () => {
  ...
  test("non-loopback binding requires env token before startup", () => {
    ...
    expect(() => assertServerAuthConfig(config("0.0.0.0"))).toThrow("NEXCODE_API_AUTH_TOKEN");
  ...
  test("auth header must match env token when non-loopback auth is required", () => {
    ...
    expect(hasValidApiAuth(new Request("http://localhost/api/config", {
      headers: { "x-nexcode-api-key": "wrong" },
    }), cfg)).toBe(false);
    expect(hasValidApiAuth(new Request("http://localhost/api/config", {
      headers: { "x-nexcode-api-key": "local-secret" },
    }), cfg)).toBe(true);
  });
  ...
  test("CORS preflight permits the nexcode API key header", () => {
    const allowed = corsHeaders()["Access-Control-Allow-Headers"];
    expect(allowed).toContain("X-NexCode-API-Key");
  ...
  test("safeConfigDTO strips URL-embedded provider secrets", () => {
    ...
    } as NxcConfig) as { providers: Record<string, { baseUrl: string }> };
  ...
  test("/v1/models requires API auth and local Origin on non-loopback bindings", async () => {
    ...
    try {
      ...
      const badOrigin = await fetch(modelsUrl, {
        headers: { "x-nexcode-api-key": "local-secret", origin: "https://attacker.test" },
      });
      ...
      const sameOrigin = await fetch(modelsUrl, {
        headers: { "x-nexcode-api-key": "local-secret", origin: new URL(modelsUrl).origin },
      });
  ...
  test("safeConfigDTO exposes the freeTier badge flag (WP040)", async () => {
    ...
    } as NxcConfig) as { providers: Record<string, { freeTier?: boolean }> };
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:${server.port}/api/config`, {
        headers: {
          ...
          "x-nexcode-api-key": configuredAdminToken() ?? "missing-admin-token",
        },
      });
      expect(response.status).toBe(403);
      expect(await response.json()).toMatchObject({ error: "cross-origin request blocked" });
      ...
      const response = await new Promise<{ status: number; body: string }>((resolve, reject) => {
        const req = httpRequest({
          ...
          headers: {
            ...
            "x-nexcode-api-key": "local-secret",
          },
        }, incoming => {
          let body = "";
  ...
  test("POST /v1/responses/compact on a routed model returns v1 replacement history", async () => {
    ...
    try {
      ...
      // No nxc1 envelope may leak into v1 output.
      expect(JSON.stringify(json)).not.toContain("nxc1:");
    } finally {
      await server.stop(true);
      await upstream.stop(true);
    }
  ...
  /**
   * This matrix points NEXCODE_HOME at its own temp dir, so the service
  ...
  test("OpenAI option auth matrix keeps direct, pool, and API credentials independent", async () => {
    ...
    const request = (server: ReturnType<typeof startServer>, headers?: HeadersInit, model = "gpt-test") => {
      ...
      requestHeaders.set("x-nexcode-api-key", "local-secret");
    ...
    const wsTurn = (server: ReturnType<typeof startServer>, headers?: Record<string, string>, model = "gpt-test") => {
      ...
      url.protocol = "ws:";
      const ws = new WebSocket(url, { headers: { "x-nexcode-api-key": "local-secret", ...(headers ?? {}) } } as unknown as string[]);
    ...
    try {
      ...
      const mainOnlyConfig = (): NxcConfig => ({
        port: 0,
        websockets: true,
        defaultProvider: "openai",
        openaiProviderTierVersion: 2,
      ...
      } as NxcConfig, "pool-a", 429, { retryAfter: "60" });
  ...
  test("internal web-search and vision never forward a non-ChatGPT bearer as Direct sidecar auth", async () => {
    ...
    mkdirSync(TEST_DIR, { recursive: true });
    process.env.NEXCODE_HOME = TEST_DIR;
    process.env.NEXCODE_API_AUTH_TOKEN = "dedicated-x-key";
      ...
      for (const authorization of [
        ...
        for (const body of [
          ...
          const response = await originalGlobalFetch(`http://127.0.0.1:${server.port}/v1/responses`, {
            ...
            headers: {
              "content-type": "application/json",
              "x-nexcode-api-key": "dedicated-x-key",
              authorization,
  ...
  test("Activation A: allow-listed 400 retries once on another eligible pool account", async () => {
    ...
    try {
      ...
      const affinity = getDebugLogEntries()
        ...
        .filter(line => line.startsWith("[nxc:codex:affinity] "))
        .map(line => JSON.parse(line.slice("[nxc:codex:affinity] ".length)) as {
          status: number;
          authKind: string;
