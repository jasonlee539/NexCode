...
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), "nxc-integrations-state-"));
...
describe("the five states, each triggered directly", () => {
  ...
  test("conflict: an owned fragment changed after we wrote it", () => {
    ...
    editedDocument.providers.nexcode!.baseUrl = "http://user-edited.invalid/v1";
    writeFileSync(
...
describe("classifier unit behavior", () => {
  ...
  test("parseConfig refuses duplicate json members a rewrite would delete", () => {
    ...
    expect(parseConfig("{\"providers\": {\"mine\": 1}, \"providers\": {\"nxc\": 2}}", "json"))
      .toBe(PARSE_FAILED);
...
describe("ownership is scoped to recorded fragments", () => {
  ...
  const ownedContribution = {
    ...
    fragments: [{ path: ["providers", "nexcode"], value: ownedValue }],
  };
  const extraValue = {
  ...
  const documentWithExtra = {
    providers: { nexcode: ownedValue, freebuff: extraValue },
  };
  ...
  const record: OwnershipRecord = {
    ...
    fragmentPaths: [["providers", "nexcode"]],
    appliedAt: "2026-08-02T00:00:00.000Z",
    opId: "seeded-op",
  };
  ...
  test("DSH also ignores whole-file edits outside its registry-declared fragment", () => {
    const dshContribution = {
      ...
      fragments: [{
        path: ["llm-pi-ai", "providers", "nexcode"],
        value: ownedValue,
      }],
    };
    const dshDocument = {
      "agent-default-model": "user-edited-after-apply",
      "llm-pi-ai": { providers: { nexcode: ownedValue } },
    };
    const dshRecord: OwnershipRecord = {
      ...
      fragmentPaths: [["llm-pi-ai", "providers", "nexcode"]],
    };
  ...
  test("modifying an owned fragment remains a conflict", () => {
    const editedDocument = {
      providers: {
        nexcode: { ...ownedValue, baseUrl: "http://user-edited.invalid/v1" },
        freebuff: extraValue,
      },
    };
    const result = classifyIntegration({
