 ...
 */
const canSymlink = (() => {
  const probeDir = mkdtempSync(join(tmpdir(), "nxc-cache-preflight-symlink-probe-"));
  try {
...
function tempRoot(name: string): string {
  const root = join(tmpdir(), `nxc-cache-preflight-${name}-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`);
