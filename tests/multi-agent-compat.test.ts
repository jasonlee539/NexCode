 ...
 * models are no longer v1-pinned by nxc, but legacy/v1-surface requests still need
...
import type { NxcParsedRequest } from "../src/types";
...
const savedCatalogStateOverride = process.env.NEXCODE_APP_SERVER_CATALOG_STATE_OVERRIDE;

// Hermetic default: the host machine may run a real Codex app-server whose
// process state must not leak into these tests (#857).
process.env.NEXCODE_APP_SERVER_CATALOG_STATE_OVERRIDE = "fresh";
...
afterAll(() => {
  if (savedCatalogStateOverride === undefined) delete process.env.NEXCODE_APP_SERVER_CATALOG_STATE_OVERRIDE;
  else process.env.NEXCODE_APP_SERVER_CATALOG_STATE_OVERRIDE = savedCatalogStateOverride;
});

function codexHomeFixture(configToml: string): string {
  const dir = mkdtempSync(join(tmpdir(), "nxc-v1pin-"));
...
function catalogFixture(dir: string, models: CatalogFixtureModel[]): void {
  writeFileSync(join(dir, "nexcode-catalog.json"), JSON.stringify({
    models: models.map((model, index) => ({
      ...
      ...(model.accountBound ? { nexcode_catalog_kind: CODEX_ACCOUNT_BOUND_CATALOG_KIND } : {}),
...
describe("multiAgentGuidanceText", () => {
  ...
  test("the v2 default catalog path uses the request collector, not the synchronous one (#1852)", async () => {
    ...
    const fakeDir = mkdtempSync(join(tmpdir(), "nxc-collab-ps-"));
    ...
    // real default path.
    delete process.env.NEXCODE_APP_SERVER_CATALOG_STATE_OVERRIDE;
  ...
  test("v2 guidance suppresses positive model claims while the app-server catalog is stale or unknown (#857)", async () => {
    ...
    for (const state of ["stale", "unknown"] as const) {
      ...
      // #1395: withhold NexCode's disk-derived claims, but do not prohibit
  ...
  test("stale and unknown withhold NexCode's own catalog claims (#1354, #1395)", async () => {
    const dir = codexHomeFixture(V2_ON);
    catalogFixture(dir, [{
      slug: "anthropic/claude-sonnet-5",
      efforts: ["low", "medium", "high", "xhigh"],
...
describe("injectDeveloperMessage", () => {
  ...
  test("keeps raw and parsed stateful placement aligned across reconstructed compaction history", () => {
    const rawInput = [
      { type: "message", role: "user", content: "current turn" },
      { type: "compaction", encrypted_content: "nxc1:c3VtbWFyeQ==" },
    ];
    const parsed = parseRequest({ model: "gpt-5.5", input: rawInput, previous_response_id: "resp_remote" });
