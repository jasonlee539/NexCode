...
const BEGIN_MARKER = "# >>> nexcode managed block — do not edit (removed by `nxc stop`) >>>";
const END_MARKER = "# <<< nexcode managed block <<<";
...
describe("Grok config injection", () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "nxc-grok-inject-"));
  ...
  test("backs up once, appends to user config, and restores user bytes", () => {
    const configPath = join(grokHome, "config.toml");
    const backupPath = join(grokHome, "config.toml.bak-nexcode");
  ...
  test("replaces the managed region idempotently", () => {
    ...
    expect(content).toContain("[model.nxc-new-model]");
    expect(content).toContain("[model.nxc-newer-model]");
  });
  ...
  test("emits per-model direct fields (grok 0.2.101 ignores model_providers inheritance)", () => {
    ...
    const table = block.slice(block.indexOf("[model.nxc-cursor-grok-4-5]"));
  ...
  test("reserves user-owned [model.*] aliases outside the fence", () => {
    ...
    // a duplicate [model.nxc-mine] header would invalidate the whole TOML.
    expect(content.match(/\[model\.nxc-mine\]/g) ?? []).toHaveLength(1);
    expect(content).toContain("[model.nxc-mine-2]");
  ...
  test("recognizes quoted and whitespace-padded user model headers (TOML-equivalent forms)", () => {
    ...
    expect(content).toContain("[model.nxc-quoted-2]");
    expect(content).toContain("[model.nxc-spaced-2]");
    expect(content).toContain("[model.nxc-single-2]");
    // Exactly one bare-form [model.nxc-quoted] must NOT exist (only the user's quoted header).
    expect(content).not.toContain("[model.nxc-quoted]");
  ...
  test("decodes TOML unicode escapes in user model headers", () => {
    const configPath = join(grokHome, "config.toml");
    // \U0000006E and \u006E are both "n" — these headers canonically define model.nxc-esc*.
    const userContent = '[model."\\U0000006Exc-esc"]\nmodel = "user/esc"\n[model."\\u006Exc-esc4"]\nmodel = "user/esc4"\n';
    writeFileSync(configPath, userContent, "utf8");

    injectGrokConfig(10100, [{ id: "esc" }, { id: "esc4" }], { grokHome });
    const content = readFileSync(configPath, "utf8");
    expect(content).not.toContain("[model.nxc-esc]");
    expect(content).not.toContain("[model.nxc-esc4]");
    expect(content).toContain("[model.nxc-esc-2]");
    expect(content).toContain("[model.nxc-esc4-2]");
    expect(() => Bun.TOML.parse(content)).not.toThrow();
  });
  ...
  test("sanitizes aliases, suffixes collisions, and escapes TOML strings", () => {
    ...
    expect(block).toContain("[model.nxc-anthropic-claude-opus-4-8]");
    expect(block).toContain("[model.nxc-same-path]");
    expect(block).toContain("[model.nxc-same-path-2]");
  ...
  describe("user table reservation across TOML header spellings", () => {
    ...
    // Every spelling below addresses the SAME table as a generated [model.nxc-mine]. grok's TOML
    ...
    const collidingSpellings: Array<[label: string, header: string]> = [
      ["quoted first segment", '["model"."nxc-mine"]'],
      ["single-quoted first segment", "['model'.nxc-mine]"],
      ["mixed quoting with whitespace", `[ "model" . 'nxc-mine' ]`],
      ["bare (baseline)", "[model.nxc-mine]"],
      ["array of tables", "[[model.nxc-mine]]"],
      ["sub-table", "[model.nxc-mine.extra]"],
      ["trailing comment", '[model."nxc-mine"] # mine'],
    ];
    ...
    for (const [label, header] of collidingSpellings) {
      test(`reserves a user alias written as ${label}`, () => {
        ...
        expect(generated).toContain("[model.nxc-mine-2]");
        expect(generated).not.toContain("[model.nxc-mine]\n");
      });
    }
    ...
    test("does not reserve aliases from unrelated tables", () => {
      ...
      writeFileSync(
        configPath(),
        '[models.nxc-mine]\nx = 1\n\n[model_providers.nxc-mine]\ny = 2\n\n[auth_provider.nxc-mine]\nz = 3\n',
        "utf8",
      );
      ...
      const written = readFileSync(configPath(), "utf8");
      expect(written.slice(written.indexOf(BEGIN_MARKER))).toContain("[model.nxc-mine]");
    });
    ...
    test("an unclosed header inside a multiline string does not swallow the next real header", () => {
      ...
      // scan reserve "a" and miss nxc-mine entirely, so we emitted a duplicate [model.nxc-mine]
      ...
      expect(generated.slice(generated.indexOf(BEGIN_MARKER))).toContain("[model.nxc-mine-2]");
      expect(generated.match(/^\[model\.nxc-mine\]$/gm)).toHaveLength(1);
    });
  ...
  describe("non-loopback binds refuse auto-registration", () => {
    ...
    for (const hostname of [undefined, "", "127.0.0.1", "localhost", "::1", "[::1]"]) {
      test(`still registers when bound to ${hostname === undefined ? "(unset)" : hostname || "(empty)"}`, () => {
        ...
        expect(result).toMatchObject({ ok: true, changed: true });
        expect(readFileSync(configPath(), "utf8")).toContain("[model.nxc-gpt-5-6-sol]");
      });
    }
