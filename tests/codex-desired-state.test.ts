 ...
 *
 * The switch was never the hard part — `nxc restore` already unroutes Codex
 * without stopping the proxy. What was missing is that the decision did not
 * survive a restart, because `nxc start` force-synced unconditionally. These
...
describe("absence means ON", () => {
   ...
   */
  const onCases: { name: string; config: NxcConfig }[] = [
...
describe("the startup gate", () => {
  /**
   * This is the defect the whole phase exists for. `nxc start` called
...
describe("Grok has the same durability, because it shipped without it", () => {
   ...
   * nothing, so `nxc start` called `syncGrokConfig` unconditionally and wrote
