...
function tempRoot(): string {
  const root = mkdtempSync(join(tmpdir(), "nxc-lab-paths-"));
  ROOTS.push(root);
  return root;
}
...
test("restricted Lab paths allow a symlinked infrastructure ancestor above the Lab boundary", () => {
  ...
  const lab = join(infrastructureAlias, "nexcode", "lab");
  expect(() => ensureRestrictedDir(lab, lab)).not.toThrow();
  expect(realpathSync.native(lab)).toBe(realpathSync.native(join(actualInfrastructure, "nexcode", "lab")));
});
