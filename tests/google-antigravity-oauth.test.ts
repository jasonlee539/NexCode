...
describe("antigravity refresh", () => {
  test("fails closed before a token request when OAuth client credentials are missing", async () => {
    delete process.env.GOOGLE_ANTIGRAVITY_CLIENT_ID;
    delete process.env.GOOGLE_ANTIGRAVITY_CLIENT_SECRET;
    const fetchSpy = spyOn(globalThis, "fetch");

    await expect(refreshAntigravityToken("refresh-tok")).rejects.toThrow(
