...
beforeEach(() => {
  ...
  testDir = mkdtempSync(join(tmpdir(), "nxc-profile-"));
...
describe("routing profiles (RI-04)", () => {
  test("normalizes a valid profile with deterministic weights and revision", () => {
    ...
    expect(profile.alias).toBe("nxc/fast");
  ...
  test("policy id/alias resolution follows canonical-id-first", () => {
    ...
    expect(resolvePolicyProfileId(config, "nxc/fast")).toBe("fast");
    ...
    expect(policyPublicModelId("fast", getRoutingProfile(config, "fast")!)).toBe("nxc/fast");
  });
