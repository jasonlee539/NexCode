...
function manifestWithoutCatalogArtifacts(entries: Record<string, string>): Record<string, string> {
  return Object.fromEntries(
    Object.entries(entries).filter(([key]) => !key.includes("nexcode-catalog") && key !== "models_cache.json"),
  );
}
...
async function waitFor<T>(
  ...
  label: string,
  // These wait on a REAL `nxc start` child: spawn a Bun runtime, load the CLI, read config,
...
class Fixture {
  readonly root = mkdtempSync(join(tmpdir(), "nxc-composed-"));
  readonly codex = join(this.root, "codex");
  readonly nxc = join(this.root, "nxc");
  ...
  constructor() {
    for (const path of [this.codex, this.nxc, this.homeA, this.homeB, this.userprofileA, this.userprofileB, this.runtime, this.provider]) {
    ...
    this.serviceManagerEnv = claimOwnedServiceHome(this.codex, this.nxc, this.homeA).env;
  }
  ...
  env(home = this.homeA, userprofile = this.userprofileA): Record<string, string> {
    ...
    return {
      ...
      NEXCODE_HOME: this.nxc,
      XDG_RUNTIME_DIR: this.runtime,
      NEXCODE_API_AUTH_TOKEN: this.dataToken,
      ...
      NEXCODE_ADMIN_AUTH_TOKEN: this.managementToken,
  ...
  writeConfig(overrides: Record<string, unknown> = {}): void {
    writeFileSync(join(this.nxc, "config.json"), JSON.stringify({
      port: 0,
      hostname: "127.0.0.1",
  ...
  async runCli(
    ...
    const completed = await Promise.race([
      ...
      new Promise<never>((_, reject) => setTimeout(() => reject(new Error(`CLI watchdog: nxc ${argv.join(" ")}`)), timeoutMs)),
    ]);
  ...
  async start(): Promise<StartedServer> {
    ...
    const runtimePath = join(this.nxc, "runtime-port.json");
  ...
  async request(
    ...
    const response = await fetch(`http://127.0.0.1:${runtime.port}${path}`, {
      ...init,
      headers: {
        "x-nexcode-api-key": this.managementToken,
  ...
  async cleanup(): Promise<void> {
    ...
    // `nxc start`, and if the wait below throws — or an earlier child refuses SIGTERM — the
...
afterEach(async () => {
  ...
  // fixture, then report. Without this, a throw here leaves live `nxc start` processes for
...
describe("WP13 composed toggle acceptance", () => {
  ...
  test("#1802: /api/sync applies the on-disk config, not the server's startup snapshot", async () => {
    ...
    try {
      ...
      const configPath = join(fx.nxc, "config.json");
  ...
  test("B-reduced: a held local provider cannot commit after the HTTP route persists OFF", async () => {
    ...
    try {
      ...
      const server = await fx.start();
      try {
        writeFileSync(join(fx.codex, "nexcode-catalog.json"), JSON.stringify({ models: [] }));
  ...
  test("D-reduced: foreign service-home evidence refuses real CLI and HTTP writers before artifacts", async () => {
    ...
    writeFileSync(join(fx.codex, "nexcode-catalog.json"), JSON.stringify({
      models: [{ slug: "foreign-sentinel" }],
    }));
    writeFileSync(join(fx.codex, "models_cache.json"), "foreign-cache-sentinel\n");
    writeFileSync(join(fx.nxc, "service-state.json"), JSON.stringify({
      version: 2,
      codexHome: join(fx.root, "foreign-codex"),
      nexcodeHome: join(fx.root, "foreign-nxc"),
      backend: "scheduler",
    }));
    const before = manifest(fx.codex);
    const server = await fx.start();
  ...
  test("D-unknown: unprovable service-home ownership refuses native reads and cache writes", async () => {
    ...
    writeFileSync(join(fx.nxc, "service-state.json"), "{malformed-service-state\n");
    const before = manifest(fx.codex);
    const server = await fx.start();
    try {
      const nativeRead = await fx.request(server.runtime, "/v1/responses", {
  ...
  test("E: separate fake homes share the effective-user Codex lock", async () => {
    ...
    const holder = Bun.spawn([process.execPath, lockChildPath], {
      ...
      env: {
        ...
        NXC_LOCK_CHILD_PAYLOAD: JSON.stringify({
          timeoutMs: 5_000,
          holdMarker: held,
    ...
    const contender = Bun.spawn([process.execPath, lockChildPath], {
      cwd: repoRoot,
      env: { ...fx.env(fx.homeB, fx.userprofileB), NXC_LOCK_CHILD_PAYLOAD: JSON.stringify({ timeoutMs: 0 }) },
      stdout: "pipe", stderr: "pipe",
    });
  ...
  test("Grok E2E: route-disabled Grok stays absent across a real startup", async () => {
    ...
    try {
      expect(readFileSync(join(grokHome, "config.toml"), "utf8")).not.toContain("nexcode managed block");
    } finally {
  ...
  test("Restore truth: JSON distinguishes a busy history restore from native artifact recovery", async () => {
    ...
    const injected = `${original}# Auto-injected by nexcode\nopenai_base_url = "http://127.0.0.1:45678/v1"\n`;
    const profile = "# nexcode profile\n";
    writeFileSync(join(fx.codex, "config.toml"), injected);
    writeFileSync(join(fx.codex, "nexcode.config.toml"), profile);
    writeFileSync(join(fx.codex, "nexcode-journal.json"), JSON.stringify({
      version: 1,
    ...
    writeFileSync(rollout, `${JSON.stringify({ type: "session_meta", payload: { id: "restore-1", model_provider: "nexcode", source: "cli" } })}\n`);
    ...
    seeded.run("INSERT INTO threads VALUES ('restore-1', ?, 'nexcode', 'cli', 'hello', 1)", [rollout]);
    seeded.close();
