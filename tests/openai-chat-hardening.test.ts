...
import type { AdapterEvent, NxcConfig, NxcParsedRequest, NxcProviderConfig } from "../src/types";
import { withTestTranslatorBudget } from "./helpers/translator-budget";

const createOpenAIChatAdapter = (...args: Parameters<typeof createOpenAIChatAdapterProduction>) =>
  withTestTranslatorBudget(createOpenAIChatAdapterProduction(...args));

const previousDebug = process.env.NXC_DEBUG;

afterEach(() => {
  resetDebugSettingsForTests();
  resetDebugLogBufferForTests();
  if (previousDebug === undefined) delete process.env.NXC_DEBUG;
  else process.env.NXC_DEBUG = previousDebug;
});
...
function provider(overrides: Partial<NxcProviderConfig> = {}): NxcProviderConfig {
  return {
    adapter: "openai-chat",
...
function routedProvider(name: "litellm" | "ollama", apiKey?: string): NxcProviderConfig {
  const config = {
    port: 10100,
...
describe("openai-chat non-stream response hardening", () => {
  ...
  test("debug mode records only the non-stream tool-call shape failure", async () => {
    ...
    expect(lines).toContain("[nxc:openai-chat:invalid-tool-calls]");
...
describe("openai-chat response_format emission", () => {
  ...
  test("omits response_format only for an explicitly opted-out model", () => {
    ...
    const options: NxcParsedRequest["options"] = {
