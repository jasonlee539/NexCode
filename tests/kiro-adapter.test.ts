...
import type { NxcParsedRequest, NxcProviderConfig } from "../src/types";
...
const origNxcHome = process.env.NEXCODE_HOME;
let tmp: string;
...
afterEach(() => {
  ...
  if (origNxcHome === undefined) delete process.env.NEXCODE_HOME; else process.env.NEXCODE_HOME = origNxcHome;
  rmSync(tmp, { recursive: true, force: true });
});

const provider = { adapter: "kiro", baseUrl: "https://runtime.us-east-1.kiro.dev", authMode: "oauth", apiKey: "tok-123" } as unknown as NxcProviderConfig;
const bashTool = { name: "bash", description: "Run a shell command", parameters: { type: "object" } };

function parsedWith(messages: unknown[], tools?: unknown[], modelId = "claude-sonnet-4.5"): NxcParsedRequest {
  return { modelId, stream: true, options: {}, context: { messages, tools } } as unknown as NxcParsedRequest;
}
...
describe("kiro adapter — buildRequest", () => {
  ...
  test("Kiro API keys force the CLI token type and ignore unrelated profile metadata", async () => {
    const apiKeyProvider = { ...provider, authMode: "key", apiKey: "ksk_example" } as unknown as NxcProviderConfig;
  ...
  test("validates Kiro request capabilities explicitly", async () => {
    for (const options of [
      ...
      await expect(createKiroAdapter(provider).buildRequest({
        ...parsedWith([{ role: "user", content: "hi" }], [bashTool]),
        options,
      } as NxcParsedRequest)).rejects.toThrow(/Kiro (supports only|does not support)/);
    }

    await expect(createKiroAdapter(provider).buildRequest({
      ...parsedWith([{ role: "user", content: "hi" }], [bashTool]),
      _structuredOutput: true,
    } as NxcParsedRequest)).rejects.toThrow("Kiro does not support Responses text controls or structured output");

    const none = { ...parsedWith([{ role: "user", content: "hi" }], [bashTool]), options: { toolChoice: "none" } } as NxcParsedRequest;
...
describe("kiro adapter — native and emulated reasoning effort", () => {
  const kiro = PROVIDER_REGISTRY.find(p => p.id === "kiro") as unknown as NxcProviderConfig;
