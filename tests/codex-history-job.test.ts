...
function makeFixture(prefix: string): Fixture {
  ...
  writeFileSync(rollout, `${JSON.stringify({
    type: "session_meta",
    payload: { id: "thread-1", model_provider: "nexcode", source: "exec" },
  })}\n`);
  ...
  db.run("INSERT INTO threads VALUES ('thread-1', ?, 'nexcode', 'exec', 1, 'hi')", [rollout]);
  db.close();
...
test("the operation is derived from admitted intent, not chosen by a caller", () => {
  ...
  // Legacy mode is the only case that routes history TO nexcode; the ordinary
...
test("the failure wording names the real reason instead of always blaming the Codex app", () => {
  ...
  expect(describeHistoryJobFailure(busy, "apply", true)).toContain("Close it and rerun 'nxc start'");
  ...
  expect(unsafeText).toContain("'nxc doctor'");
  ...
  expect(describeHistoryJobFailure(permission, "apply")).toContain("'nxc doctor'");
  ...
  expect(describeHistoryJobFailure(workerError, "apply")).toContain("'nxc doctor'");
...
test("skip resolves without spawning a thread and writes nothing", async () => {
  const fixture = makeFixture("nxc-history-job-skip-");
  ...
  db.close();
  expect(row?.model_provider).toBe("nexcode");
});
...
test("a real Worker performs the transition and the parent joins it", async () => {
  const fixture = makeFixture("nxc-history-job-run-");
...
test("the history job does not resolve before its Worker closes", async () => {
  const fixture = makeFixture("nxc-history-job-close-");
...
test("an overrun Worker returns a typed timeout rather than hanging", async () => {
  const fixture = makeFixture("nxc-history-job-timeout-");
...
test("a hard history error reaches the caller with its real message", async () => {
  const fixture = makeFixture("nxc-history-job-hard-error-");
...
test("the synchronous restore body is gated on skipHistory", () => {
  ...
  try {
    ...
    db.run(`INSERT INTO threads VALUES ('thread-1', ?, 'nexcode', 'cli', 'hello', 1)`, rollout);
    db.close();
    ...
    const runRestore = (optionsLiteral: string) => spawnSync(process.execPath, ["--eval", [
      ...
      cwd: repoRoot,
      env: { ...process.env, CODEX_HOME: fixtureCodexHome, NEXCODE_HOME: fixtureNxcHome },
      encoding: "utf8",
    });
    const provider = () => {
