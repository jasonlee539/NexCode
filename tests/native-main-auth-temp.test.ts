...
function fixture() {
  const root = mkdtempSync(join(tmpdir(), "nxc-auth-temp-"));
  roots.push(root);
...
describe("native-main auth temp startup scrub", () => {
  test("truncates and removes exact atomic-writer residues without touching near-miss names", () => {
    ...
    const exact = ["auth.json.nxc.123.1.tmp", "auth.json.nxc.456.27.tmp"];
    const near = [
      "auth.json.nxc.0.1.tmp",
      "auth.json.nxc.01.1.tmp",
      "auth.json.nxc.1.0.tmp",
      "auth.json.nxc.1.01.tmp",
      "auth.json.nxc.1.1.TMP",
      "auth.json.nxc.1.1.tmp.bak",
    ];
  ...
  test("fails closed without following an exact-name symlink or truncating its target", () => {
    ...
    const residue = join(f.codexHome, "auth.json.nxc.123.1.tmp");
  ...
  test("fails closed before modifying an excessive exact-residue set", () => {
    ...
    for (let sequence = 1; sequence <= 129; sequence += 1) {
      writeFileSync(join(f.codexHome, `auth.json.nxc.123.${sequence}.tmp`), "private-value");
    }

    expectCleanupRequired(() => scrubNativeMainAuthTempResidues(f.context));
    expect(readFileSync(join(f.codexHome, "auth.json.nxc.123.1.tmp"), "utf8")).toBe("private-value");
    expect(readFileSync(join(f.codexHome, "auth.json.nxc.123.129.tmp"), "utf8")).toBe("private-value");
  });
});
