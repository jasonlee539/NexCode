...
function parsedWith(messages: unknown[], tools?: unknown[]): NxcParsedRequest {
  return { modelId: "gemini-3-pro", stream: false, options: {}, context: { messages, tools } } as unknown as NxcParsedRequest;
}

async function geminiContents(parsed: NxcParsedRequest): Promise<{ role: string; parts: Record<string, unknown>[] }[]> {
...
async function geminiBody(parsed: NxcParsedRequest): Promise<Record<string, unknown>> {
  const { body } = await createGoogleAdapter(provider).buildRequest(parsed);
...
describe("google adapter — tool_choice on the wire", () => {
  ...
  function parsedWithChoice(toolChoice: unknown, tools: unknown[] | null = TOOLS): NxcParsedRequest {
    return {
...
describe("google adapter — direct -tiered wire renames", () => {
  function renamedParsed(modelId: string): NxcParsedRequest {
    return {
      modelId,
      stream: false,
      options: {},
  ...
  function identityParsed(modelId: string): NxcParsedRequest {
    return {
      modelId,
      stream: false,
      options: {},
