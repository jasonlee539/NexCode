...
beforeEach(() => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("nxc-server-search-codex-");
  clearCodexUpstreamHealth();
  clearThreadAccountMap();
...
function exactSearchConfig(): NxcConfig {
  return {
    ...forwardConfig(),
    providers: {
      openai: {
...
test("a search body that stalls after headers retains the total 504 deadline", async () => {
  ...
  saveConfig({ ...forwardConfig(), search: { timeoutMs: 50 } } as NxcConfig);
