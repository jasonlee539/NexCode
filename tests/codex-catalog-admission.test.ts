...
beforeEach(() => {
  ...
  testRoot = realpathSync.native(mkdtempSync(join(tmpdir(), "nxc-catalog-admission-")));
  codexHome = join(testRoot, "codex-home");
  nexcodeHome = join(testRoot, "nexcode-home");
...
test("captures the given config reference, generation, and catalog target identities", () => {
  ...
  writeFileSync(join(codexHome, "nexcode-catalog.json"), "{}\n");
  ...
  expect(JSON.parse(snapshot.targets.catalog)).toMatchObject({
    path: join(codexHome, "nexcode-catalog.json"),
...
test("ignores process-local symbol tags when binding the config snapshot identity", () => {
  ...
  Object.defineProperty(tagged, Symbol("nexcode.user-cost-overlay-preservation-owner"), {
