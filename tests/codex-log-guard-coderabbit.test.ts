...
function fixture(): { codexHome: string; databasePath: string } {
  const root = mkdtempSync(join(tmpdir(), "nxc-log-guard-cr-protect-"));
  roots.push(root);
...
function reservedTriggers(databasePath: string): Array<{ name: string; sql: string }> {
  ...
  try {
    return db.query<{ name: string; sql: string }, []>(
      "SELECT name, sql FROM sqlite_master WHERE type='trigger' AND name LIKE 'nexcode_log_guard_%' ORDER BY name",
    ).all();
  } finally {
    db.close();
  }
...
describe("CodeRabbit protection regressions", () => {
  test("compatible but unsafe trigger path reports unknown protection state", () => {
    const root = mkdtempSync(join(tmpdir(), "nxc-log-guard-cr-symlink-"));
    roots.push(root);
    const codexHome = join(root, "codex-home");
  ...
  test.skipIf(process.platform !== "darwin")(
    ...
    () => {
      expect(sameLogGuardPathIdentity(
        "/private/tmp/nexcode-log-guard/logs_2.sqlite",
        "/tmp/nexcode-log-guard/logs_2.sqlite",
      )).toBe(true);
      expect(sameLogGuardPathIdentity(
        "/private/var/tmp/nexcode-log-guard/logs_2.sqlite",
        "/var/tmp/nexcode-log-guard/logs_2.sqlite",
      )).toBe(true);

      const root = mkdtempSync(join(tmpdir(), "nxc-log-guard-cr-path-"));
      roots.push(root);
  ...
  test.skipIf(process.platform !== "darwin")(
    ...
    () => {
      expect(codexLogGuardLockDigest(
        "/tmp/nexcode-home",
        "/var/tmp/nexcode-home/logs_2.sqlite",
      )).toBe(codexLogGuardLockDigest(
        "/private/tmp/nexcode-home",
        "/private/var/tmp/nexcode-home/logs_2.sqlite",
      ));
    },
  );
  ...
  test("unprotect recovers when both exact NexCode-owned triggers are present", () => {
    const { codexHome, databasePath } = fixture();
    ...
    const db = new Database(databasePath);
    db.exec(`
      CREATE TRIGGER nexcode_log_guard_quiet_v1
...
describe("log guard path identity survives OS canonicalization", () => {
  test("a path that only differs by the OS's own canonical spelling is the same file", () => {
    const root = mkdtempSync(join(tmpdir(), "nxc-log-guard-identity-"));
    roots.push(root);
