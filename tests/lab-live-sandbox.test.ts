...
const HOMES: string[] = [];
function tempHome(): string { const dir = join(tmpdir(), `nxc-lab-live-${process.pid}-${Math.random().toString(16).slice(2)}`); mkdirSync(dir, { recursive: true, mode: 0o700 }); HOMES.push(dir); return dir; }
afterEach(() => { for (const dir of HOMES.splice(0)) { try { rmSync(dir, { recursive: true, force: true }); } catch { /* ignore */ } } delete process.env.NEXCODE_HOME; });
...
describe("CL-03 live sandbox security seams", () => {
  ...
  test("DNS/address mismatch and lease scope mismatch fail closed", async () => {
    const home = tempHome(); process.env.NEXCODE_HOME = home;
  ...
  test("host/SNI mismatch fails closed", async () => { const home = tempHome(); process.env.NEXCODE_HOME = home; const destination = await createLabDestination({ baseUrl: "https://api.example.com/v1", labRunApproval: true, resolve: async () => [{ address: "93.184.216.34", family: 4 }], configDir: home }); expect(() => assertHostSniMatch(destination, "evil.example.com")).toThrow(LabDestinationError); });
  test("metadata and link-local destinations are blocked", async () => { const home = tempHome(); process.env.NEXCODE_HOME = home; await expect(createLabDestination({ baseUrl: "http://169.254.169.254/latest/meta-data", labRunApproval: true, configDir: home })).rejects.toThrow(LabDestinationError); });
  test("private network requires permission and lab approval", async () => { const home = tempHome(); process.env.NEXCODE_HOME = home; await expect(createLabDestination({ baseUrl: "http://127.0.0.1:11434/v1", configDir: home })).rejects.toThrow(LabDestinationError); const ok = await createLabDestination({ baseUrl: "http://127.0.0.1:11434/v1", allowPrivateNetwork: true, labRunApproval: true, resolve: async () => [{ address: "127.0.0.1", family: 4 }], configDir: home }); expect(ok.privateNetwork).toBe(true); });
