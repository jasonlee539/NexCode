...
beforeEach(() => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("nxc-api-storage-codex-");
  testDir = mkdtempSync(join(tmpdir(), "nxc-api-storage-"));
  ...
  test("returns the scan report with buckets and totals, no error field", async () => {
    ...
    try {
      ...
      const coordinationDatabases = [
        ".nexcode-native-main.claim.sqlite",
        ".nexcode-native-main.owner.sqlite",
        ".nexcode-native-profile.lock.sqlite",
      ];
