...
import type { NxcConfig, NxcParsedRequest, TierObservationContext } from "../src/types";
...
describe("FastWire logging and persistence", () => {
  ...
  test("adapter metadata lands on its attempt and final-attempt summary", () => {
    ...
    addFinalRequestLog("nxc-tier", Date.now(), logCtx, 200, undefined, entry => {
      logged = entry;
    });
