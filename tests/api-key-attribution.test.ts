...
function remoteConfig(): NxcConfig {
  return {
    ...
    apiKeys: [
      { id: "key-one", name: "one", key: "nxc_data_attributionone", createdAt: "2026-07-31T00:00:00.000Z" },
      { id: "key-two", name: "two", key: "nxc_data_attributiontwo", createdAt: "2026-07-31T00:00:00.000Z" },
    ],
  };
}
...
async function keysGet(server: { url: URL }): Promise<Record<string, unknown>> {
  const res = await fetch(new URL("/api/keys", server.url), {
    headers: { "x-nexcode-api-key": ADMIN_TOKEN },
  });
  return await res.json() as Record<string, unknown>;
}

beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), "nxc-attribution-"));
...
describe("attribution reaches usage.jsonl", () => {
  test("an authed request is attributed to the key that opened it, all the way to disk", async () => {
    ...
    try {
      await fetch(new URL("/v1/chat/completions", server.url), {
        method: "POST",
        headers: { "content-type": "application/json", "x-nexcode-api-key": "nxc_data_attributiontwo" },
  ...
  test("the same request appears in the GET /api/keys rollup", async () => {
    ...
    try {
      await fetch(new URL("/v1/chat/completions", server.url), {
        method: "POST",
        headers: { "content-type": "application/json", "x-nexcode-api-key": "nxc_data_attributionone" },
        body: JSON.stringify({ model: "test/gpt-test", messages: [{ role: "user", content: "hi" }] }),
      });
  ...
  test("the environment token records its own kind", async () => {
    process.env.NEXCODE_API_AUTH_TOKEN = "env-data-secret";
    saveConfig(remoteConfig());
    const server = startServer(0);
    try {
      await fetch(new URL("/v1/chat/completions", server.url), {
        method: "POST",
        headers: { "content-type": "application/json", "x-nexcode-api-key": "env-data-secret" },
        body: JSON.stringify({ model: "test/gpt-test", messages: [{ role: "user", content: "hi" }] }),
      });
      const row = usageRows().at(-1);
    ...
    try {
      ...
      const settled = await new Promise<boolean>(resolve => {
        const socket = new WebSocket(target, {
          headers: { "X-NexCode-API-Key": "nxc_data_attributiontwo" },
  ...
  test("a long key id survives the round trip intact", async () => {
    ...
    config.apiKeys = [{ id: longId, name: "long", key: "nxc_data_longid", createdAt: "2026-07-31T00:00:00.000Z" }];
    saveConfig(config);
    const server = startServer(0);
    try {
      await fetch(new URL("/v1/chat/completions", server.url), {
        method: "POST",
        headers: { "content-type": "application/json", "x-nexcode-api-key": "nxc_data_longid" },
        body: JSON.stringify({ model: "test/gpt-test", messages: [{ role: "user", content: "hi" }] }),
      });
