...
describe("Codex shim install readiness", () => {
  test("keeps a clean install green for native and managed routing", () => {
    ...
    expect(codexShimReadinessWarnings({
      ...ready,
      routingKind: "nexcode-local",
    })).toEqual([]);
  });

  test("warns when an external provider is not routed through NexCode", () => {
    ...
    expect(warnings[0]).toContain("live NexCode /v1 endpoint");
  ...
  test("distinguishes user-owned local and remote routes", () => {
    ...
    expect(local[0]).toContain("nxc doctor");
  ...
  test("the install command surfaces readiness warnings without leaking the proxy URL", () => {
    ...
    const root = mkdtempSync(join(tmpdir(), "nxc-shim-readiness-"));
  ...
  test("keeps install advisory when the Codex config cannot be read", () => {
    const root = mkdtempSync(join(tmpdir(), "nxc-shim-unreadable-config-"));
    const codexHome = join(root, "codex-home");
    const nexcodeHome = join(root, "nexcode-home");
    const binDir = join(root, "bin");
    mkdirSync(codexHome);
