...
import { classifyRecoverableHistoryError, countPendingNexcodeHistory, historyBackupPathFor, isRecoverableHistoryError, migrateHistoryToOpenai, restoreLegacyOpenaiHistory, setAfterNoopPendingCountForTests, setHistoryDbBusyTimeoutForTests, snapshotCodexHistoryNoop, syncCodexHistoryProvider, withHistoryRetry } from "../src/codex/history-provider";
...
function makeFixture({ includeExec = false, includeLegacy = false } = {}) {
  const dir = join(tmpdir(), `nxc-history-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`);
  ...
  writeFileSync(execRollout, [
    JSON.stringify({
      type: "session_meta",
      payload: { id: "thread-2", model_provider: "nexcode", source: "exec", cwd: dir },
    }),
  ...
  writeFileSync(legacyRollout, [
    JSON.stringify({
      type: "session_meta",
      payload: { id: "thread-3", model_provider: "nexcode", source: "cli", cwd: dir },
    }),
  ...
  if (includeExec) {
    db.run(`
      INSERT INTO threads (id, rollout_path, model_provider, source, first_user_message, has_user_event)
      VALUES ('thread-2', ?, 'nexcode', 'exec', 'hello from exec', 0)
    `, execRollout);
  }
  if (includeLegacy) {
    db.run(`
      INSERT INTO threads (id, rollout_path, model_provider, source, first_user_message, has_user_event)
      VALUES ('thread-3', ?, 'nexcode', 'cli', 'legacy remapped row', 1)
    `, legacyRollout);
  }
  db.close();
...
describe("Codex history provider sync", () => {
  test("maps resumable Codex threads to nexcode via the latest session_meta", () => {
    const { dbPath, backupPath, rollout } = makeFixture();

    const result = syncCodexHistoryProvider("nexcode", dbPath, backupPath);

    expect(result).toEqual({ rows: 1, files: 1 });
    const db = new Database(dbPath);
    expect(db.query("SELECT model_provider FROM threads WHERE id = 'thread-1'").get()).toEqual({ model_provider: "nexcode" });
    expect(db.query("SELECT has_user_event FROM threads WHERE id = 'thread-1'").get()).toEqual({ has_user_event: 1 });
    db.close();
    expect(latestSessionMetaPayload(rollout).model_provider).toBe("nexcode");
  });
  ...
  test("rewrites line 1 in place (length-preserving) when reverting an nexcode-origin rollout, so a later first-line clone cannot resurrect nexcode", () => {
    const { dbPath, backupPath, legacyRollout } = makeFixture({ includeLegacy: true });
    // thread-3 / legacyRollout is an nexcode-origin row with no backup -> eject path (revert to openai).
  ...
  test("patches line 1 even when the first session_meta line is larger than the read chunk (big base_instructions)", () => {
    const dir = join(tmpdir(), `nxc-bighead-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`);
    mkdirSync(dir, { recursive: true });
    const rollout = join(dir, "rollout.jsonl");
    const big = "x".repeat(200_000); // > 64KiB read chunk, forces the probe to grow
    writeFileSync(rollout, [
      JSON.stringify({ type: "session_meta", payload: { id: "big-1", model_provider: "nexcode", source: "cli", cwd: dir, base_instructions: big } }),
    ...
    db.run(`INSERT INTO threads VALUES ('big-1', ?, 'nexcode', 'cli', 'hi', 1)`, rollout);
    db.close();
  ...
  test("maps resumable Codex threads back to openai", () => {
    const { dbPath, backupPath, rollout } = makeFixture();
    syncCodexHistoryProvider("nexcode", dbPath, backupPath);
  ...
  test("does not consume a history backup written for a different Codex state DB", () => {
    ...
    syncCodexHistoryProvider("nexcode", first.dbPath, first.backupPath);
  ...
  test("promotes nexcode exec threads to app-visible cli source and restores from backup", () => {
  ...
  test("ejects no-backup nexcode interactive rows to openai during native restore", () => {
  ...
  test("explicitly recovers legacy nexcode user rows to openai", () => {
...
describe("history lock retry", () => {
  ...
  test("syncCodexHistoryProvider reports why the retry budget died", () => {
    // A pending nexcode row makes the eject path actually write; with no rows
...
describe("Design B migration helpers", () => {
  test("strict no-op snapshots distinguish absence from manifest uncertainty", () => {
    const dir = join(tmpdir(), `nxc-history-noop-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`);
    mkdirSync(dir, { recursive: true });
    const dbPath = join(dir, "state_5.sqlite");
  ...
  test("a missing database with a valid nonempty manifest remains pending", () => {
    const dir = join(tmpdir(), `nxc-history-noop-pending-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`);
    mkdirSync(dir, { recursive: true });
    const dbPath = join(dir, "state_5.sqlite");
    const backupPath = historyBackupPathFor(dbPath);
    noopSnapshotArtifacts.add(backupPath);
  ...
  test("a WAL commit after the pending count invalidates a no-op snapshot", () => {
    const dir = join(tmpdir(), `nxc-history-noop-wal-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`);
    mkdirSync(dir, { recursive: true });
    const dbPath = join(dir, "state_5.sqlite");
    const backupPath = historyBackupPathFor(dbPath);
    noopSnapshotArtifacts.add(backupPath);
    ...
    setAfterNoopPendingCountForTests(() => {
      ...
      try {
        ...
        writer.run(
          "INSERT INTO threads VALUES (?, ?, 'nexcode', 'cli', 1, 'raced')",
          ["raced-nexcode-row", join(dir, "raced-rollout.jsonl")],
        );
      } finally {
        writer.close();
      }
  ...
  test("countPendingNexcodeHistory mirrors the eject predicate and reaches 0 after migration", () => {
    const { dbPath, backupPath } = makeFixture({ includeExec: true, includeLegacy: true });

    const before = countPendingNexcodeHistory(dbPath, backupPath);
    ...
    const after = countPendingNexcodeHistory(dbPath, backupPath);
  ...
  test("countPendingNexcodeHistory returns zeros for a missing DB", () => {
    const missing = join(tmpdir(), `nxc-none-${Date.now()}`, "state_5.sqlite");
    const result = countPendingNexcodeHistory(missing, join(tmpdir(), "no-backup.json"));
  ...
  test("a missing DB with a leftover backup manifest does not satisfy the steady-state gate", () => {
    const dir = join(tmpdir(), `nxc-reinstall-${process.pid}-${Date.now()}`);
    mkdirSync(dir, { recursive: true });
    ...
    const pending = countPendingNexcodeHistory(missingDb, backupPath);
