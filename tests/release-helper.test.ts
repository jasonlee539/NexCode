...
const sshTarget = `${"git"}@${"github.com"}:lidge-jun/nexcode.git`;
...
if (args[0] === "remote" && args[1] === "get-url") {
  stdout((process.env.FAKE_GIT_ORIGIN_URL ?? "https://github.com/lidge-jun/nexcode.git") + "\\n");
  process.exit(0);
}
...
function runRelease(version: string, scenario: ReleaseScenario = {}) {
  const shimDir = mkdtempSync(join(tmpdir(), "nxc-release-helper-"));
  ...
  const inheritedEnv = Object.fromEntries(
    Object.entries(process.env).filter(([key]) => key.toLowerCase() !== "path"
      ...
      && key !== "NXC_RELEASE_SSH_KEY" && key !== "NXC_RELEASE_SSH_REPO"),
  );
  ...
  const result = spawnSync(process.execPath, [releaseScriptPath, version], {
    ...
    env: {
      ...
      ...(scenario.releaseSshKey ? { NXC_RELEASE_SSH_KEY: scenario.releaseSshKey } : {}),
      ...(scenario.releaseSshRepo ? { NXC_RELEASE_SSH_REPO: scenario.releaseSshRepo } : {}),
...
function executeGitSshCommand(gitSshCommand: string): { calls: SshInvocation[]; result: ReturnType<typeof spawnSync> } {
  const shimDir = mkdtempSync(join(tmpdir(), "nxc-release-ssh-"));
...
describe("release helper", () => {
  ...
  test("the protected push uses the release deploy key only when one is configured", () => {
    const { calls, result } = runRelease("9.9.9", {
      releaseSshKey: "/tmp/nxc-release-key",
    ...
    expect(push?.gitSshCommand).toBe('ssh -i "/tmp/nxc-release-key" -o IdentitiesOnly=yes');
  });
  ...
  test("a key path with spaces and backslashes stays a single ssh argument", () => {
    const { calls } = runRelease("9.9.9", {
      releaseSshKey: "C:\\Users\\Jun Kim\\.ssh\\nxc release key",
      pendingBump: true,
    });

    const push = calls.find(call => call.name === "git" && call.args[0] === "push");
    expect(push?.gitSshCommand).toBe('ssh -i "C:\\\\Users\\\\Jun Kim\\\\.ssh\\\\nxc release key" -o IdentitiesOnly=yes');
  });

  test("Git passes the emitted deploy-key path to SSH as one literal argument", () => {
    const keyPath = 'C:\\Users\\Jun Kim\\.ssh\\nxc "quoted" $HOME $(not-run) `not-run`; key';
  ...
  test("the ssh push target follows the configured origin remote", () => {
    const { calls } = runRelease("9.9.9", {
      releaseSshKey: "/tmp/k",
      originUrl: "https://github.com/someone-else/nexcode.git",
      pendingBump: true,
    });

    const push = calls.find(call => call.name === "git" && call.args[0] === "push");
    expect(push?.args[1]).toBe(`${"git"}@${"github.com"}:someone-else/nexcode.git`);
  });
  ...
  test("an origin carrying credentials is refused rather than transplanted", () => {
    const { calls, result } = runRelease("9.9.9", {
      releaseSshKey: "/tmp/k",
      originUrl: `https://x-access-token:SECRET@${"github.com"}/lidge-jun/nexcode.git`,
      pendingBump: true,
    });
  ...
  test("a malformed NXC_RELEASE_SSH_REPO override is refused instead of pushed to", () => {
    const { calls, result } = runRelease("9.9.9", {
      releaseSshKey: "/tmp/k",
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr + result.stdout).toContain("NXC_RELEASE_SSH_REPO");
    expect(calls.find(call => call.name === "git" && call.args[0] === "push")).toBeUndefined();
  });
  ...
  test("an ssh origin is reused verbatim rather than rewritten", () => {
    const { calls } = runRelease("9.9.9", {
      releaseSshKey: "/tmp/k",
      originUrl: `${"git"}@${"github.com"}:lidge-jun/nexcode.git`,
      pendingBump: true,
    });

    const push = calls.find(call => call.name === "git" && call.args[0] === "push");
    expect(push?.args[1]).toBe(`${"git"}@${"github.com"}:lidge-jun/nexcode.git`);
  });

  test("an origin that yields no ssh target aborts instead of guessing one", () => {
    const { calls, result } = runRelease("9.9.9", {
      releaseSshKey: "/tmp/k",
      originUrl: "/srv/git/nexcode.git",
      pendingBump: true,
    });
