...
describe("optional DSH rc.6 compatibility E2E helpers", () => {
  ...
  test("renders only a loopback NexCode endpoint into the checked-in template", () => {
    const template = [
      "agent-default-model:",
    ...
    expect(rendered).not.toContain("__NXC_BASE_URL__");
    expect(() => renderDshSettings(template, "https://gateway.example/v1"))
      .toThrow("loopback NexCode URL");
    expect(() => renderDshSettings(template.replace("__NXC_BASE_URL__", "already-set"), "http://localhost:18181/v1"))
      .toThrow("exactly one __NXC_BASE_URL__ placeholder");
  });
