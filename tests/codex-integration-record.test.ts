...
function integrationRecordPath(): string {
  return join(nexcodeHome, "integrations", "codex.json");
}

function writeRecord(value: unknown): void {
  mkdirSync(join(nexcodeHome, "integrations"), { recursive: true });
...
beforeEach(() => {
  ...
  nexcodeHome = mkdtempSync(join(tmpdir(), "nxc-integration-record-"));
