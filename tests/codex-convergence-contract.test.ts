...
function config(port = 10100): NxcConfig {
...
beforeEach(() => {
  ...
  root = realpathSync.native(mkdtempSync(join(tmpdir(), "nxc-convergence-")));
  ...
  resetCodexRuntimeResolveCacheForTests();
  saveConfig(config());
  writeFileSync(join(codexHome, "nexcode-catalog.json"), sourceCatalog());
});
...
test("same-inode source drift rejects before every catalog target write", async () => {
  const gathered = await candidate();
  const path = join(codexHome, "nexcode-catalog.json");
...
test("management convergence restores omitted natives and retains a configured native alias", async () => {
  ...
  persistCodexRuntime(runtime, { configDir: nexcodeHome, now: () => 0 });
  setCodexRuntimeResolveCacheForTests({ runtime, failures: [] }, { discoverAlternatives: false });
  setBundledCatalogCacheForTests(runtime, bundled, { nexcodeHome });
  ...
  const written = JSON.parse(readFileSync(join(codexHome, "nexcode-catalog.json"), "utf8")) as {
