 ...
 */
const REAL_LOOKING_KEY = ["nxc", "live", "9f3c7a2b41d84e6fa05c8e17b3d92764"].join("_");
...
let config: NxcConfig;

beforeEach(() => {
  base = mkdtempSync(join(tmpdir(), "nxc-management-integrations-"));
  home = join(base, "home");
...
describe("PUT /api/client-integrations/:clientId", () => {
  test("applies and disables through the writer", async () => {
    ...
    expect(readFileSync(configPath, "utf8")).toContain("nexcode");
    ...
    expect(readFileSync(configPath, "utf8")).not.toContain("nexcode");
  ...
  test("MCode apply and stale refresh write context and reasoning capabilities", async () => {
    ...
    const applied = Bun.YAML.parse(readFileSync(configPath, "utf8")) as {
      custom_provider: { nexcode: { models: Record<string, unknown> } };
    };
    expect(applied.custom_provider.nexcode.models["a/m1"]).toEqual({
    ...
    expect(afterRefresh.custom_provider.nexcode.models["a/m1"]).toEqual({
...
describe("refusals", () => {
  test("invalid OMP alias removal returns unsafe without changing bytes or journal", async () => {
    ...
    const edited = readFileSync(configPath, "utf8")
      .replace("    baseUrl:", "    baseUrl: &nexcode_url")
      .concat("settings:\n  inheritedBase: *nexcode_url\n");
    expect(edited).toContain("    baseUrl: &nexcode_url");
  ...
  test("conflict rejects disable without changing a managed-field edit", async () => {
    ...
    expect(body.message).toBe(
      `${configPath} changed after nexcode wrote it; disabling would discard that edit`,
    );
    expect(readFileSync(configPath, "utf8")).toBe(edited);
...
describe("admission", () => {
  test("the real server admits before it dispatches an integration mutation", async () => {
    ...
    const previousAdmin = process.env.NEXCODE_ADMIN_AUTH_TOKEN;
    const serverHome = join(base, "server-home");
    mkdirSync(serverHome, { recursive: true });
    process.env.NEXCODE_HOME = serverHome;
    process.env.NEXCODE_ADMIN_AUTH_TOKEN = ["admission", "secret", "fixture"].join("-");
    installHermes();
    ...
    try {
      ...
      expect(await unauthenticated.json()).toEqual({ error: "nexcode admin token required" });
      ...
      const authenticated = await fetch(target, {
        ...
        headers: {
          ...
          "x-nexcode-api-key": process.env.NEXCODE_ADMIN_AUTH_TOKEN ?? "",
        },
        body: JSON.stringify({ enabled: true }),
      });
      ...
      if (previousAdmin === undefined) delete process.env.NEXCODE_ADMIN_AUTH_TOKEN;
      else process.env.NEXCODE_ADMIN_AUTH_TOKEN = previousAdmin;
    }
  });
  ...
  test("a whole transaction stays inside the bound store", async () => {
     ...
     * proves nothing about the developer's own `~/.nexcode/integrations`,
  ...
  test("a GUI-session mutation without CSRF is rejected before integration dispatch", async () => {
    ...
    try {
      ...
      const token = meta("nexcode-session-token");
      const csrf = meta("nexcode-session-csrf");
      if (!token || !csrf) {
        /*
      ...
      const guiHeaders = {
        Origin: origin,
        "Content-Type": "application/json",
        "x-nexcode-api-key": token,
        "x-nexcode-gui-origin": origin,
      };
      ...
      const withCsrf = await fetch(target, {
        method: "PUT",
        headers: { ...guiHeaders, "x-nexcode-csrf-token": csrf },
        body: JSON.stringify({ enabled: true }),
      });
      ...
      if (previousAdmin !== undefined) process.env.NEXCODE_ADMIN_AUTH_TOKEN = previousAdmin;
    }
  });
});
