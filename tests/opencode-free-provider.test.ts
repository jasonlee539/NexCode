...
function minimalRequest(model = "kimi-k2.7-code"): NxcParsedRequest {
  return {
    modelId: model,
    stream: false,
...
describe("opencode-free provider", () => {
  ...
  test("adapter sends no auth header with no apiKey configured", () => {
    const provider: NxcProviderConfig = providerConfigSeed(entry!);
  ...
  describe("existing installs receive newly added static headers", () => {
    const persisted = (headers?: Record<string, string>): NxcProviderConfig => ({
      adapter: "openai-chat",
  ...
  test("Zen-bound tool schemas are normalized to an object root", () => {
    const provider: NxcProviderConfig = providerConfigSeed(entry!);
    const request: NxcParsedRequest = {
