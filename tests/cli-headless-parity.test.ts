...
describe("nxc agent sidecar --list (#2188)", () => {
...
describe("headless GUI parity CLI", () => {
  test("every GUI management endpoint belongs to a documented CLI resource", () => {
    ...
    const coverage: Array<[string, string]> = [
      ["/api/claude-code", "nxc claude config"],
      ...
      ["/api/oauth", "nxc account"],
      ["/api/providers/keys", "nxc account"],
      ["/api/providers", "nxc provider"],
      ["/api/provider-", "nxc provider/models"],
      ["/api/selected-models", "nxc models"],
      ["/api/custom-models", "nxc models"],
      ["/api/model", "nxc models"],
      ["/api/combos", "nxc combo"],
      ["/api/client-config", "nxc export"],
      ["/api/client-integrations", "nxc integration client"],
      ...
      // `nxc grok apply` regenerates the fence and `nxc stop` strips it, and
      // the Claude flag flips through `nxc claude config` — so a dedicated
      // `nxc integration native` verb would duplicate existing commands rather
      ...
      ["/api/keys", "nxc access"],
      ["/api/logs", "nxc observe"],
      ["/api/lab", "nxc lab"],
      ["/api/config", "nxc config"],
      ["/api/settings", "nxc system"],
      // Routing Intelligence (RI-04..RI-10): profiles + dry-run are mirrored by
      // `nxc route policy`. Analytics is GUI-first for now; the same request
      // history remains available through observe/index tooling.
      ["/api/routing-profiles", "nxc route policy"],
      ["/api/routing-analytics", "(none — GUI analytics surface; history via nxc observe/logs)"],
      ["/api/shadow", "nxc models"],
      ["/api/sidecar", "nxc agent"],
      ["/api/startup", "nxc system"],
      ["/api/stop", "nxc stop"],
      ["/api/storage", "nxc observe"],
      ["/api/subagent", "nxc agent"],
      ["/api/sync", "nxc system sync"],
      ["/api/system", "nxc observe/system"],
      ["/api/update", "nxc system update"],
      ["/api/usage", "nxc observe usage"],
      ["/api/v2", "nxc v2/agent"],
      ["/api/windows-tray", "nxc tray"],
    ];
  ...
  test("API key create returns the one-time key through the access command", async () => {
    const runtime = fakeRuntime((req) => new URL(req.url).pathname === "/api/keys"
      ? { id: "key-1", name: "deploy", key: "nxc_secret" }
      : undefined);
  ...
  test("config set validates the complete candidate before the atomic write", async () => {
    const home = mkdtempSync(join(tmpdir(), "nxc-cli-config-"));
    const previous = process.env.NEXCODE_HOME;
    ...
    try {
      ...
      if (previous === undefined) delete process.env.NEXCODE_HOME;
      else process.env.NEXCODE_HOME = previous;
  ...
  test("config set and import reject an invalid app-owned memory budget without persisting the normalized default", async () => {
    const home = mkdtempSync(join(tmpdir(), "nxc-cli-memory-budget-"));
    const previous = process.env.NEXCODE_HOME;
    process.env.NEXCODE_HOME = home;
    try {
  ...
  test("config set applies onto the disk state, not a snapshot read before the lock (#1835)", async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), "nxc-cli-set-race-"));
    const previous = process.env.NEXCODE_HOME;
    process.env.NEXCODE_HOME = home;
    const configPath = join(home, "config.json");
    const base = {
  ...
  test("config unset actually removes the key through the mutation primitive (#1835)", async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), "nxc-cli-unset-"));
    const previous = process.env.NEXCODE_HOME;
    process.env.NEXCODE_HOME = home;
    const configPath = join(home, "config.json");
    try {
  ...
  test("config set releases the manual pin when it writes the selection order", async () => {
    const home = mkdtempSync(join(tmpdir(), "nxc-cli-priority-pin-"));
    const previous = process.env.NEXCODE_HOME;
    process.env.NEXCODE_HOME = home;
    const configPath = join(home, "config.json");
    const base = {
