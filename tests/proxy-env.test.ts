...
const PROXY_ENV_KEYS = ["HTTP_PROXY", "HTTPS_PROXY", "NO_PROXY", "http_proxy", "https_proxy", "no_proxy", "NXC_TEST_PROXY_REF"] as const;
...
function configWithProxy(proxy?: string): NxcConfig {
  return { proxy, providers: {} } as unknown as NxcConfig;
}
...
describe("applyProxyEnv", () => {
  ...
  test("resolves ${VAR}-style env references like other config secrets", () => {
    process.env.NXC_TEST_PROXY_REF = "http://ref-proxy:9999";
    applyProxyEnv(configWithProxy("${NXC_TEST_PROXY_REF}"));
