...
function provider(baseUrl: string, overrides: Partial<NxcProviderConfig> = {}): NxcProviderConfig {
  return { adapter: "openai-chat", baseUrl, apiKey: "test-key", ...overrides };
}

function parsed(modelId: string, stream = false): NxcParsedRequest {
  return {
    modelId,
    stream,
...
function body(baseUrl: string, modelId: string, overrides: Partial<NxcProviderConfig> = {}, stream = false): Record<string, unknown> {
...
describe("OpenRouter configurable provider routing", () => {
  ...
  test("preserves provider routing after native Chat key rotation", () => {
    ...
    const home = mkdtempSync(join(tmpdir(), "nxc-openrouter-routing-"));
  ...
  for (const [label, overrides, message] of [
    ...
    test(`rejects ${label}`, () => {
      const error = openRouterRoutingConfigError(provider(
        ...
        overrides as Partial<NxcProviderConfig>,
      ));
