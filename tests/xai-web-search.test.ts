...
const xaiProvider: NxcProviderConfig = { adapter: "openai-chat", baseUrl: "https://api.x.ai/v1", authMode: "oauth" };
...
describe("credential pinning + loop fail-closed (review blockers)", () => {
  test("lookalike origin https://api.x.ai.evil never receives the bearer (falls back to canonical)", async () => {
    ...
    try {
      const evil: NxcProviderConfig = { adapter: "openai-chat", baseUrl: "https://api.x.ai.evil/v1", authMode: "oauth" };
