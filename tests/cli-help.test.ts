...
const binPath = join(repoRoot, "bin", "nxc.mjs");
...
describe("CLI subcommand help", () => {
  test("version commands print a single script-friendly line", () => {
    for (const args of [["--version"], ["-v"], ["version"]]) {
      ...
      expectSpawnFinished(result, `nxc ${args.join(" ")}`);
      expect(result.status).toBe(0);
      expect(result.stderr).toBe("");
      expect(result.stdout.trim()).toMatch(/^nexcode \d+\.\d+\.\d+/);
    ...
    expectSpawnFinished(binResult, "bin/nxc.mjs --version");
    expect(binResult.status).toBe(0);
    expect(binResult.stdout.trim()).toMatch(/^nexcode \d+\.\d+\.\d+/);
    expect(binResult.stdout.trim().split("\n")).toHaveLength(1);
  });

  test("help command routes to subcommand help", () => {
    const result = runCli(["help", "start"]);
    expectSpawnFinished(result, "nxc help start");
    expect(result.status).toBe(0);
    expect(result.stderr).toBe("");
  ...
  test("top-level help counts every export client and export help names them", () => {
    ...
    // machinery on the `nxc --help` path — so this assertion is what
    ...
    expectSpawnFinished(exportHelp, "nxc help export");
  ...
  test("top-level help forms exit before Codex shim auto-restore can mutate launchers", () => {
    const nexcodeHome = mkdtempSync(join(tmpdir(), "nxc-help-shim-home-"));
    const binDir = mkdtempSync(join(tmpdir(), "nxc-help-shim-bin-"));
    try {
      ...
      const statePath = join(nexcodeHome, "codex-shim.json");
      ...
      for (const args of [[], ["help"], ["--help"], ["-h"]]) {
        const result = runCli(args, { NEXCODE_HOME: nexcodeHome, PATH: binDir });
        expectSpawnFinished(result, `nxc ${args.join(" ") || "(no args)"}`);
        expect(result.status).toBe(0);
        expect(result.stdout).toContain("nexcode (nxc)");
  ...
  test("tray help documents the install-only no-start flag", () => {
    ...
    expectSpawnFinished(result, "nxc help tray");
    expect(result.status).toBe(0);
    expect(result.stderr).toBe("");
  ...
  test("unknown command with help flag remains an error", () => {
    ...
    expectSpawnFinished(result, "nxc foobar --help");
  ...
  test("status prints diagnostics without starting the proxy", () => {
    const nexcodeHome = mkdtempSync(join(tmpdir(), "nxc-status-"));
    try {
      const configPath = join(nexcodeHome, "config.json");
      ...
      const result = spawnSync(process.execPath, [cliPath, "status"], {
        cwd: repoRoot,
        env: { ...process.env, NEXCODE_HOME: nexcodeHome },
        encoding: "utf8",
        timeout: SPAWN_TIMEOUT_MS,
      });
      ...
      expect(result.stdout).toContain(`PID file: ${join(nexcodeHome, "nxc.pid")}`);
      ...
      expect(result.stdout).toContain(join(nexcodeHome, "service.log"));
  ...
  test("restore --help prints usage without mutating Codex config", () => {
    const codexHome = mkdtempSync(join(tmpdir(), "nxc-help-"));
    try {
      ...
      expectSpawnFinished(result, "nxc restore --help");
      expect(result.status).toBe(0);
      expect(result.stdout).toContain("Usage: nxc restore");
  ...
  test("mutating command help exits before local state changes", () => {
    const nexcodeHome = mkdtempSync(join(tmpdir(), "nxc-help-state-"));
    const codexHome = mkdtempSync(join(tmpdir(), "nxc-help-codex-"));
    try {
      const configPath = join(codexHome, "config.toml");
      const markerPath = join(nexcodeHome, "service-state.json");
      const before = 'model_provider = "nexcode"\n';
      writeFileSync(configPath, before, "utf8");
      writeFileSync(markerPath, '{"installed":true}', "utf8");

      const cases = [
        { args: ["stop", "--help"], expected: "Usage: nxc stop" },
        { args: ["uninstall", "--help"], expected: "Usage: nxc uninstall" },
        { args: ["service", "uninstall", "--help"], expected: "Usage: nxc service" },
        { args: ["codex-shim", "uninstall", "--help"], expected: "Usage: nxc codex-shim" },
      ];
      ...
      for (const testCase of cases) {
        ...
        expectSpawnFinished(result, `nxc ${testCase.args.join(" ")}`);
        expect(result.status).toBe(0);
  ...
  test("recover-history --help prints usage without opening history database", () => {
    ...
    try {
      ...
      expectSpawnFinished(result, "nxc recover-history --help");
      expect(result.status).toBe(0);
      expect(result.stdout).toContain("Usage: nxc recover-history --legacy-openai");
  ...
  test("start rejects unknown and partially numeric port arguments", () => {
    const cases = [
      ...
      { args: ["start", "--bad"], expected: "Usage: nxc start [--port <port>]" },
      { args: ["start", "--port", "1234", "--extra"], expected: "Usage: nxc start [--port <port>]" },
    ];
  ...
  test("start help wins before port validation", () => {
    ...
    expectSpawnFinished(result, "nxc start --port 123abc --help");
    expect(result.status).toBe(0);
    expect(result.stderr).toBe("");
    expect(result.stdout).toContain("Usage: nxc start [--port <port>]");
  });
  ...
  test("invalid service and codex-shim usage include remove alias", () => {
    const cases = [
      { args: ["service", "nope"], expected: "Usage: nxc service [install|repair|restart|start|stop|status|uninstall|remove]" },
      { args: ["codex-shim", "nope"], expected: "Usage: nxc codex-shim <install|status|uninstall|remove>" },
    ];
