...
const routed: NxcProviderConfig = { adapter: "openai-chat", baseUrl: "https://routed.test/v1", apiKey: "k" };
const cca: NxcProviderConfig = { adapter: "google", baseUrl: "https://daily-cloudcode-pa.googleapis.com", authMode: "oauth" };
...
describe("runGeminiWebSearch request shape (review P1)", () => {
  test("malicious baseUrl ignored: registry destination, manual redirect, bearer + IDE UA, full envelope, thinkingConfig", async () => {
    ...
    try {
      const evil: NxcProviderConfig = { adapter: "google", baseUrl: "https://evil.example/v1", authMode: "oauth" };
