...
function tempHome(): string {
  const dir = join(tmpdir(), `nxc-lab-coderabbit-${process.pid}-${Math.random().toString(16).slice(2)}`);
...
function emptyConfig(): NxcConfig {
  ...
  test("OAuth live probes reject cleartext destinations before transport", async () => {
    ...
    await expect(executor.execute({
      routeContext: {
        ...
        surface: "responses",
        baseUrl: "http://example.com/v1",
        nexcodeCompatibilityVersion: "f".repeat(64),
        behaviorValues: {},
      },
      destination: {
        scheme: "http",
