 ...
 * with -like '*name*'. On a machine with two NexCode homes under one account,
...
const HOME_A = "C:\\Users\\nxc\\.nexcode";
const HOME_B = "C:\\Users\\nxc\\other-home\\.nexcode";
const script = (home: string) => join(home, "nexcode-service.cmd");
const launcher = (home: string) => join(home, "nexcode-service-launcher.vbs");
...
describe("which command lines the wrapper killer stops", () => {
  ...
  test("another NexCode home under the same account survives", () => {
    // The defect this replaces: -like '*nexcode-service.cmd*' matched here.
  ...
  test("an unrelated process merely naming the file is not killed", () => {
    expect(killsCommandLine("notepad.exe nexcode-service.cmd", patterns)).toBe(false);
    expect(killsCommandLine('findstr /c:"nexcode-service-launcher.vbs" log.txt', patterns)).toBe(false);
  });
...
describe("the generated script still implements that rule", () => {
  ...
  test("the script carries this home's canonical paths, not bare filenames", () => {
    ...
    expect(ps).not.toContain("@('nexcode-service.cmd'");
  });
  ...
  test("a path containing a quote is escaped, not injected", () => {
    const odd = "C:\\Users\\o'brien\\.nexcode\\nexcode-service.cmd";
    expect(windowsWrapperKillScript([odd])).toContain("C:\\Users\\o''brien\\.nexcode\\nexcode-service.cmd");
  });
});
...
describe("both teardown paths use the shared killer", () => {
  test("neither file keeps a private matcher", () => {
    for (const rel of ["src/service.ts", "src/update/job.ts"]) {
      ...
      expect(src).not.toContain("$pats = @('nexcode-service.cmd'");
    }
  });
});
