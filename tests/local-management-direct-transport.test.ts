...
describe("local management direct transport", () => {
  ...
  test("bypasses configured environment proxies for liveness, readiness, and capability reads", async () => {
    ...
    const reply = (
      ...
      if (pathname === "/healthz") {
        write(200, { service: "nexcode", status: "ok", version: "test", uptime: 1, pid: PID, port: targetPort });
        return;
      }
      if (pathname === "/readyz") {
        write(200, { service: "nexcode", status: "ready", version: "test", uptime: 1, pid: PID, port: targetPort });
        return;
      }
    ...
    const target = createServer((request, response) => {
      ...
      const capability = request.headers["x-nexcode-local-capability"];
