...
let runTurnCalls = 0;
let httpCalls = 0;
let parsedAttempts: NxcParsedRequest[] = [];
...
function fixtureAdapter(provider: NxcProviderConfig): ProviderAdapter {
  ...
  return {
    ...
    ...(runTurn ? {
      async runTurn(parsed: NxcParsedRequest, _incoming: unknown, emit: (event: AdapterEvent) => void) {
        if (customRunTurn) {
...
function config(adapter: "test-run-turn" | "test-http", extra: Partial<NxcConfig> = {}): NxcConfig {
  return {
    port: 0,
...
afterEach(() => {
  resetProviderRequestPacingForTest();
  delete process.env.NXC_EMPTY_COMPLETION_RETRY;
});
