...
function remoteConfig(): NxcConfig {
  return {
    port: 0,
    hostname: "0.0.0.0",
...
function websocketHandshakeOpens(url: URL, token: string): Promise<boolean> {
  return new Promise(resolve => {
    ...
    const socket = new WebSocket(target, {
      headers: { "X-NexCode-API-Key": token },
...
beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), "nxc-management-auth-"));
...
describe("management and data-plane credential separation", () => {
  ...
  test("management-token temp cleanup forgets successful ACL memos and retains failed removals", () => {
    ...
    process.env.USERNAME = "nxc-test-user";
    resetHardenedStateForTests();
  ...
  test("config salvage does not weaken the management auth boundary (#1785)", async () => {
    ...
    try {
      ...
      const withWrongAdminToken = await fetch(new URL("/api/config", server.url), {
        headers: { "x-nexcode-api-key": "not-the-admin-secret" },
      });
  ...
  test("a management token that matches the data environment token closes only the management plane", async () => {
    process.env.NEXCODE_ADMIN_AUTH_TOKEN = "data-secret";
    saveConfig(remoteConfig());
    const server = startServer(0);
    try {
  ...
  test("a protected management token file is generated and remains management-only", async () => {
    ...
    try {
      ...
      expect(adminToken).toMatch(/^nxc_admin_[A-Za-z0-9_-]{43}$/);

      const management = await fetch(new URL("/api/config", server.url), {
        headers: { "x-nexcode-api-key": adminToken },
      });
  ...
  test("an icacls timeout keeps the management plane closed without stopping the data plane", async () => {
    ...
    try {
      ...
      const management = await fetch(new URL("/api/config", server.url), {
        headers: { "x-nexcode-api-key": "nxc_admin_unhardened" },
      });
      expect(management.status).toBe(503);
      ...
      expect(body.hint).toContain("NEXCODE_ADMIN_AUTH_TOKEN");
  ...
  test("a configured data key satisfies the remote data-plane startup requirement", async () => {
    ...
    try {
      const data = await fetch(new URL("/v1/models", server.url), {
        headers: { "x-nexcode-api-key": "nxc_data_configured-secret" },
      });
      expect(data.status).toBe(200);
  ...
  test("management browser origins must match the request origin exactly", async () => {
    ...
    try {
      const crossPort = await fetch(new URL("/api/config", server.url), {
        headers: {
          "x-nexcode-api-key": "admin-secret",
  ...
  test("a local GUI page receives an origin-bound session with CSRF protection", async () => {
    ...
    expect(html).toContain(`name="nexcode-session-token" content="${session?.token}"`);
    expect(html).toContain(`name="nexcode-session-csrf" content="${session?.csrfToken}"`);

    // The dev GUI fetches /nexcode-session through Vite so the app shell stays
    ...
    expect(bootstrapHtml).toContain(`name="nexcode-session-origin" content="${session?.origin}"`);
    expect(bootstrapHtml).toContain(`name="nexcode-session-token" content="${session?.token}"`);

    const sameOriginRead = new Request("http://localhost:10100/api/config", {
      headers: {
        Host: "localhost:10100",
        "x-nexcode-api-key": session?.token ?? "",
        "x-nexcode-gui-origin": "http://localhost:10100",
      },
    });
    ...
    const crossPortRead = new Request("http://localhost:10100/api/config", {
      headers: {
        ...
        "x-nexcode-api-key": session?.token ?? "",
        "x-nexcode-gui-origin": "http://localhost:20100",
      },
    });
    ...
    const mutationWithCsrf = new Request("http://localhost:10100/api/config", {
      ...
      headers: {
        ...
        "x-nexcode-api-key": session?.token ?? "",
        "x-nexcode-gui-origin": "http://localhost:10100",
        "x-nexcode-csrf-token": session?.csrfToken ?? "",
      },
    });
  ...
  test("GET /nexcode-session serves the bootstrap document from a live server", async () => {
    const config = remoteConfig();
    config.hostname = "127.0.0.1";
    saveConfig(config);
    const server = startServer(0);
    try {
      const response = await fetch(new URL("/nexcode-session", server.url), {
      ...
      expect(html).toContain('name="nexcode-session-token"');
      expect(html).toContain('name="nexcode-session-csrf"');
      expect(html).toContain('name="nexcode-session-origin"');
    } finally {
      await server.stop(true);
    }
  });
  ...
  test("all local credential shapes are rejected by the upstream-forwarding guard", () => {
    ...
    for (const secret of [
      ...
      "nxc_data_generated",
      "nxc_admin_generated",
      "nxc_session_generated",
    ]) {
      expect(isProxyAdmissionSecret(secret, config)).toBe(true);
    }
    expect(isProxyAdmissionSecret("nxc_provider_upstream", config)).toBe(false);
  });
  ...
  test("Responses authentication and WebSocket handshakes accept data credentials only", async () => {
    ...
    const server = startServer(0);
    try {
      for (const rejected of ["admin-secret", "nxc_session_browser-secret"]) {
        const response = await fetch(new URL("/v1/responses", server.url), {
          method: "POST",
          headers: {
            "content-type": "application/json",
            "x-nexcode-api-key": rejected,
          },
  ...
  test("an invalid existing management token file keeps management unavailable", async () => {
    ...
    try {
      const management = await fetch(new URL("/api/config", server.url), {
        headers: { "x-nexcode-api-key": "corrupt-token" },
      });
      expect(management.status).toBe(503);
  ...
  test("an existing management token ACL hardening failure keeps management unavailable", async () => {
    delete process.env.NEXCODE_ADMIN_AUTH_TOKEN;
    saveConfig(remoteConfig());
    const adminToken = `nxc_admin_${"b".repeat(43)}`;
  ...
  test("directory ACL timeout keeps management unavailable and names NEXCODE_ADMIN_AUTH_TOKEN", () => {
    delete process.env.NEXCODE_ADMIN_AUTH_TOKEN;
    saveConfig(remoteConfig());
    const adminToken = `nxc_admin_${"d".repeat(43)}`;
    writeFileSync(join(testHome, "admin-api-token"), `${adminToken}\n`, { mode: 0o600 });
    ...
    expect(state.reason).toContain("NEXCODE_ADMIN_AUTH_TOKEN");
  });

  test("required management harden retries after a soft loadConfig directory timeout", async () => {
    delete process.env.NEXCODE_ADMIN_AUTH_TOKEN;
    saveConfig(remoteConfig());
    const adminToken = `nxc_admin_${"f".repeat(43)}`;
    writeFileSync(join(testHome, "admin-api-token"), `${adminToken}\n`, { mode: 0o600 });
    process.env.USERNAME ??= "tester";
    setPlatformForTests("win32");
  ...
  test("NEXCODE_ADMIN_AUTH_TOKEN bypasses file-backed ACL hardening", async () => {
    process.env.NEXCODE_ADMIN_AUTH_TOKEN = "env-admin-secret";
    saveConfig(remoteConfig());
    process.env.USERNAME ??= "tester";
    setPlatformForTests("win32");
    ...
    try {
      const management = await fetch(new URL("/api/config", server.url), {
        headers: { "x-nexcode-api-key": "env-admin-secret" },
      });
      expect(management.status).toBe(200);
    } finally {
      await server.stop(true);
