...
function parsed(stream = false): NxcParsedRequest {
  return {
...
function antigravityProvider(overrides: Partial<NxcProviderConfig> = {}): NxcProviderConfig {
  return provider({
...
describe("google provider hardening", () => {
  ...
  test("Antigravity rejects a blank OAuth token", async () => {
    ...
    await expect(adapter.buildRequest(parsed())).rejects.toThrow(
      "google-antigravity oauth token missing — run nxc login google-antigravity",
    );
  });
  ...
  test("garbage stays debug-dropped while comment keepalives are excluded", async () => {
    ...
    try {
      ...
      const dropped = getDebugLogEntries().filter(entry => entry.line.includes("[nxc:frame-drop] google"));
