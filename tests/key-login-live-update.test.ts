...
/**
 * Regression: `nxc login <key-provider>` used to POST the unmerged preset row
...
function umansKeyConfig(port = 0): NxcConfig {
  return {
    port,
    hostname: "127.0.0.1",
...
beforeEach(() => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("nxc-key-login-live-");
  testDir = mkdtempSync(join(tmpdir(), "nxc-key-login-live-"));
...
describe("CLI key-login live-update overlay preservation", () => {
  test("notify after key login pushes the merged row and keeps modelCosts on live and disk", async () => {
    ...
    try {
      ...
      const disk = JSON.parse(readFileSync(join(testDir, "config.json"), "utf-8")) as NxcConfig;
