...
const TEST_NXC_HOME = join(TEST_DIR, "nxc");
...
function claimTempHome(codexHome: string, nxcHome: string, home: string): void {
  claimOwnedServiceHome(codexHome, nxcHome, home);
}
...
describe("GUI/CLI Codex sync backend", () => {
  beforeEach(() => {
    ...
    mkdirSync(TEST_NXC_HOME, { recursive: true });
    mkdirSync(TEST_HOME, { recursive: true });
    process.env.CODEX_HOME = TEST_CODEX_HOME;
    process.env.NEXCODE_HOME = TEST_NXC_HOME;
    process.env.HOME = TEST_HOME;
    process.env.USERPROFILE = TEST_HOME;
    writeFileSync(join(TEST_CODEX_HOME, "config.toml"), 'model = "gpt-5.5"\n', "utf8");
    writeFileSync(join(TEST_NXC_HOME, "config.json"), JSON.stringify(config));
    claimTempHome(TEST_CODEX_HOME, TEST_NXC_HOME, TEST_HOME);
  });
  ...
  test("returns the structured sync result used by POST /api/sync", async () => {
    ...
    expect(injectedCatalogPath).toBe("/tmp/nexcode-catalog.json");
    expect(result).toEqual({
      status: "applied",
      ok: true,
      added: 3,
      catalogPath: "/tmp/nexcode-catalog.json",
      catalogExists: true,
      catalogWritten: true,
      cacheSynced: true,
      message: "injected",
  ...
  test("the real successful injection preflight writes no Codex artifacts", () => {
    ...
    const profilePath = join(TEST_CODEX_HOME, "nexcode.config.toml");
    const journalPath = join(TEST_CODEX_HOME, "nexcode-journal.json");
  ...
  test("returns a policy skip without touching the catalog or config", async () => {
    let refreshed = false;
    let injected = false;
    writeFileSync(join(TEST_NXC_HOME, "config.json"), JSON.stringify({
      ...config,
  ...
  test("a competing OFF during catalog discovery becomes the discriminated skip", async () => {
    const raceRoot = mkdtempSync(join(tmpdir(), "nxc-sync-lost-transition-"));
    const raceCodexHome = join(raceRoot, ".codex");
    const raceNxcHome = join(raceRoot, ".nexcode");
    const raceHome = join(raceRoot, "home");
    mkdirSync(raceCodexHome, { recursive: true });
    mkdirSync(raceNxcHome, { recursive: true });
    mkdirSync(raceHome, { recursive: true });
    try {
      writeFileSync(join(raceCodexHome, "config.toml"), 'model = "gpt-5"\n', "utf8");
      writeFileSync(join(raceNxcHome, "config.json"), JSON.stringify(config));
      claimTempHome(raceCodexHome, raceNxcHome, raceHome);
      const script = [
  ...
  test("surfaces combo catalog omissions in sync result and CLI stderr (#484)", async () => {
    ...
    const omission = {
      ...
      message: "[nexcode] Combo \"k3k3\" is omitted from the catalog because member capabilities are incomplete: kimi/k3, xianyu/kimi-k3.",
    };
    const result = await syncModelsToCodex(12345, config, { log: line => logs.push(String(line)), error: line => errors.push(String(line)) }, {
      admitCodexWrite: admittedSync,
      refreshCodexModelCatalog: async () => ({
  ...
  test("CLI sync summary uses incompatible_modalities reason, not incomplete (#516)", async () => {
    ...
    const omission = {
      ...
      message: "[nexcode] Combo \"disjoint\" is omitted from the catalog because members have no common input modalities: a/m1, b/m2.",
    };
  ...
  test("POST /api/sync exposes an actionable error when native defaults are ambiguous", () => {
    const nxcHome = join(TEST_DIR, "nexcode");
    ...
    `], {
      ...
      env: { ...process.env, CODEX_HOME: TEST_CODEX_HOME, NEXCODE_HOME: nxcHome },
      encoding: "utf8",
    });
