...
function baseConfig(loopbackPort: number | null): NxcConfig {
  return {
    port: 0,
    hostname: "0.0.0.0",
...
beforeEach(() => {
  testDir = mkdtempSync(join(tmpdir(), "nxc-loopback-listener-"));
  process.env.NEXCODE_HOME = testDir;
  process.env.NEXCODE_API_AUTH_TOKEN = "public-secret";
});
...
describe("unauthenticated loopback listener", () => {
  ...
  test("serves only the four allowlisted routes, using each route's real method", async () => {
    ...
    try {
      ...
      const denied: Array<{ method: string; path: string; body?: string }> = [
        ...
        { method: "GET", path: "/v1/nexcode/artifacts/x" },
  ...
  test("upgrades a Responses WebSocket on the listener that received it", async () => {
    const loopbackPort = await freePort();
    saveConfig({ ...baseConfig(loopbackPort), websockets: true } as unknown as NxcConfig);
    const server = startServer(0);
    try {
...
describe("Codex injection targets the loopback listener", () => {
  test("the written config points at the loopback port with no auth header", () => {
    ...
    const root = mkdtempSync(join(tmpdir(), "nxc-loopback-inject-"));
    const codexHome = join(root, ".codex");
    const nxcHome = join(root, ".nexcode");
    mkdirSync(codexHome, { recursive: true });
    mkdirSync(nxcHome, { recursive: true });
    writeFileSync(join(codexHome, "config.toml"), 'model = "gpt-5"\n', "utf-8");
    const config = baseConfig(10_200) as Record<string, unknown>;
    config.port = 10_100;
    writeFileSync(join(nxcHome, "config.json"), JSON.stringify(config), "utf-8");
    try {
      const child = spawnSync(process.execPath, [
        ...
        env: {
          ...
          NXC_INJECT_RACE_PAYLOAD: JSON.stringify({ port: 10_100 }),
        },
      });
