...
describe("GET /api/system/memory", () => {
  ...
  test("serializes only an allowlisted Bun runtime provenance, omitting it otherwise (#848)", async () => {
    const inherited = process.env.NXC_BUN_RUNTIME_SOURCE;
    ...
    try {
      for (const source of ["override", "bundled", "process"]) {
        process.env.NXC_BUN_RUNTIME_SOURCE = source;
      ...
      process.env.NXC_BUN_RUNTIME_PATH = "/usr/local/bin/definitely-not-this-bun";
      expect((await read()).bunRuntimeSource).toBeUndefined();
      delete process.env.NXC_BUN_RUNTIME_PATH;
      ...
      process.env.NXC_BUN_RUNTIME_SOURCE = "system";
      expect((await read()).bunRuntimeSource).toBeUndefined();
    } finally {
      if (inherited === undefined) delete process.env.NXC_BUN_RUNTIME_SOURCE;
      else process.env.NXC_BUN_RUNTIME_SOURCE = inherited;
      delete process.env.NXC_BUN_RUNTIME_PATH;
    }
