...
beforeEach(() => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("nxc-api-debug-codex-");
  testDir = mkdtempSync(join(tmpdir(), "nxc-api-debug-"));
  ...
  delete process.env.NEXCODE_USAGE_DEBUG;
});
...
describe("management API /api/debug/logs", () => {
  ...
  test("caps limit query param at 2000", async () => {
    for (let i = 0; i < 5; i += 1) appendDebugLogLine(`[nxc:test:${i}]`);
    const server = startServer(0);
    try {
...
describe("management API /api/debug/injection-logs", () => {
  test("returns buffered injection lines with seq cursor and limit", async () => {
    ...
    injectionDebugLog("[nexcode] gpt-5.4: multi-agent guidance injected (surface=collab, 128 chars)");
    injectionDebugLog("[nexcode] gpt-5.4: effort cap applied (ultra -> high, main turn)");
  ...
  test("caps limit query param at 2000", async () => {
    for (let i = 0; i < 5; i += 1) injectionDebugLog(`[nexcode] inj:${i}`);
    const server = startServer(0);
    try {
...
describe("management API /api/debug/usage-logs", () => {
  test("tails usage-debug.jsonl from the running proxy home", async () => {
    ...
    try {
      ...
      expect(entries[0]!.seq).toBe(1);
      expect(entries[0]!.line).toContain("nxc-usage-wire");
