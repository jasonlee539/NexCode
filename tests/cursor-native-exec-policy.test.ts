...
const baseProvider: NxcProviderConfig = {
  adapter: "cursor",
  baseUrl: "https://api2.cursor.sh",
};

const baseParsed: NxcParsedRequest = {
  modelId: "cursor/auto",
  context: { messages: [] },
  stream: false,
  options: {},
...
describe("Cursor native exec sandbox policy", () => {
  ...
  async function assertNativeSinksDenied(unsafeAllowNativeLocalExec: boolean) {
    const dir = mkdtempSync(join(tmpdir(), "nxc-cursor-policy-"));
    ...
    expect(deniedText).toContain("mcp_nexcode-responses_*");
    ...
    expect(deniedText).not.toContain("disabled by NexCode policy");
    ...
    expect(deniedShellText).toContain("mcp_nexcode-responses_*");
    ...
    expect(deniedShellText).not.toContain("disabled by NexCode policy");
    ...
    expect(deniedFetchText).toContain("mcp_nexcode-responses_shell_command");
    expect(deniedFetchText).not.toContain("disabled by NexCode policy");
  ...
  test("activates a real read only when nativeLocalExec is explicitly on", async () => {
    ...
    const content = "C-ACTIVATION-GROUNDING-01 allowed content";
    writeFileSync(path, content);
    const provider = { ...baseProvider, nativeLocalExec: "on" } satisfies NxcProviderConfig;
  ...
  test("runTurn passes the developer declaration decision to the transport factory", async () => {
    const captured: CursorTransportFactoryInput[] = [];
    const provider = { ...baseProvider, nativeLocalExec: "codex-sandbox" } satisfies NxcProviderConfig;
    const adapter = createCursorAdapter(provider, {
      createTransport(input) {
        captured.push(input);
        return {
