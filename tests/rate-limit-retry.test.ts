...
describe("rateLimitRetryPolicyFor", () => {
  test("null when absent or explicitly disabled", () => {
    expect(rateLimitRetryPolicyFor({} as NxcProviderConfig)).toBeNull();
    expect(rateLimitRetryPolicyFor({ retryOn429: { enabled: false } } as NxcProviderConfig)).toBeNull();
  });
  ...
  test("null for OAuth, forward, local, and unknown auth modes (fail closed)", () => {
    ...
    expect(rateLimitRetryPolicyFor({
      authMode: "key",
      retryOn429: {},
    } as NxcProviderConfig)).not.toBeNull();
  });

  test("applies defaults when the object is present", () => {
    expect(rateLimitRetryPolicyFor({ retryOn429: {} } as NxcProviderConfig)).toEqual({
      enabled: true,
      attempts: 3,
      intervalMs: 5_000,
...
describe("rateLimitRetryDelayMs", () => {
  const policy = rateLimitRetryPolicyFor({ retryOn429: {} } as NxcProviderConfig)!;
