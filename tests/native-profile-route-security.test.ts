...
beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), "nxc-native-profile-route-security-"));
  process.env.HOME = testHome;
  process.env.NEXCODE_HOME = testHome;
  process.env.CODEX_HOME = join(testHome, "codex-home");
  process.env.NEXCODE_ADMIN_AUTH_TOKEN = "native-main-admin-token";
  ...
  test("rejects missing or wrong credentials and hostile origins before every native operation dispatches", async () => {
    ...
    try {
      for (const operation of operations) {
        ...
        expect((await request({ "x-nexcode-api-key": "wrong-admin-token" })).status).toBe(401);
        expect((await request({
          "x-nexcode-api-key": "native-main-admin-token",
          origin: "https://attacker.example",
        })).status).toBe(403);
        expect(calls).toEqual([]);

        expect((await request({ "x-nexcode-api-key": "native-main-admin-token" })).status).toBe(200);
      ...
      for (const operation of operations.filter(operation => operation.method === "POST")) {
        const request = (csrf?: string) => fetch(new URL(operation.path, server.url), {
          ...
          headers: {
            ...
            "x-nexcode-gui-origin": server.url.origin,
            ...(csrf === undefined ? {} : { "x-nexcode-csrf-token": csrf }),
          },
          body: operation.body ? JSON.stringify(operation.body) : undefined,
        });
