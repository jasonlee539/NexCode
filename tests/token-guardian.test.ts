...
function kimiProvider(refreshPolicy?: NxcProviderConfig["refreshPolicy"]): NxcProviderConfig {
  return { adapter: "openai-chat", baseUrl: "https://api.moonshot.ai/v1", authMode: "oauth", ...(refreshPolicy ? { refreshPolicy } : {}) };
}

function writeConfig(partial: Partial<NxcConfig>): void {
  ...
  const cfg: NxcConfig = { port: 10100, ...partial, providers, defaultProvider };
...
beforeEach(() => {
  ...
  mkdirSync(join(tmp, "nxc"), { recursive: true });
...
describe("token guardian", () => {
  ...
  test("direct mode warms main only and never enumerates the added-account store", async () => {
    const accountStore = join(tmp, "nxc", "codex-accounts.json");
