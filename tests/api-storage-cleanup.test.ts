...
beforeEach(() => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("nxc-api-storage-cleanup-codex-");
  testDir = mkdtempSync(join(tmpdir(), "nxc-api-storage-cleanup-"));
...
describe("POST /api/storage/cleanup", () => {
  ...
  test("partial permanent purge returns relative trashDir on the wire", async () => {
    seedArchived(isolatedCodexHome!.path);
    const previous = process.env.NEXCODE_CLEANUP_TEST_HOOKS;
    ...
    try {
      ...
      await server.stop(true);
      if (previous === undefined) delete process.env.NEXCODE_CLEANUP_TEST_HOOKS;
      else process.env.NEXCODE_CLEANUP_TEST_HOOKS = previous;
    }
  });
});
