...
function configFor(providerName: string, provider: NxcProviderConfig): NxcConfig {
  return {
    port: 10100,
...
function routeCapturingWarnings(config: NxcConfig, model: string, times = 1): string[] {
...
function routePinned(baseUrl: unknown, times = 1): string[] {
  return routeCapturingWarnings(
    configFor(PINNED_PROVIDER, { adapter: "google", baseUrl } as NxcProviderConfig),
...
for (const { label, id, adapter, baseUrl } of [
  ...
  test(`stays silent for ${label}, whose baseUrl is honored`, () => {
    const config = configFor(id, { adapter, baseUrl } as NxcProviderConfig);
