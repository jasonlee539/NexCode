...
describe("migration memo release gate", () => {
  test("both residual classes retain the destination memo", () => {
    expect(isAtomicResidualError(new AtomicWriteResidualTempError("/tmp/x.nxc.1.1.tmp", true))).toBe(true);
    expect(isAtomicResidualError(new AtomicWriteSecretResidualError("/tmp/x.nxc.1.1.tmp"))).toBe(true);
