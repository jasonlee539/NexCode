...
function ownedEnvironment(codexHome: string, nxcHome: string): Record<string, string> {
  const home = join(nxcHome, "home");
  mkdirSync(home, { recursive: true });
  return { HOME: home, USERPROFILE: home, ...claimOwnedServiceHome(codexHome, nxcHome, home).env };
}
...
describe("nxc restore back", () => {
  test("restore durably disables Codex in an isolated home", () => {
    const codexHome = mkdtempSync(join(tmpdir(), "nxc-cli-restore-codex-"));
    const nxcHome = mkdtempSync(join(tmpdir(), "nxc-cli-restore-home-"));
    try {
      writeFileSync(join(codexHome, "config.toml"), 'model = "gpt-5"\n', "utf8");
      writeFileSync(join(nxcHome, "config.json"), JSON.stringify({ providers: {}, defaultProvider: "openai", checkForUpdates: false }), "utf8");
      const result = runCli(["restore"], {
        ...ownedEnvironment(codexHome, nxcHome),
        CODEX_HOME: codexHome,
        NEXCODE_HOME: nxcHome,
        CI: "1",
      });
      expect(result.status).toBe(0);
      expect(JSON.parse(readFileSync(join(nxcHome, "config.json"), "utf8")).clientIntegrations.codex).toBe(false);
  ...
  test("restore --json emits a schema-complete envelope on the already-OFF no-op path", () => {
    const codexHome = mkdtempSync(join(tmpdir(), "nxc-cli-json-noop-codex-"));
    const nxcHome = mkdtempSync(join(tmpdir(), "nxc-cli-json-noop-home-"));
    try {
      writeFileSync(join(codexHome, "config.toml"), 'model = "gpt-5"\n', "utf8");
      writeFileSync(join(nxcHome, "config.json"), JSON.stringify({
  ...
  test("sync treats durable OFF as a successful no-write policy result", () => {
    const codexHome = mkdtempSync(join(tmpdir(), "nxc-cli-sync-off-codex-"));
    const nxcHome = mkdtempSync(join(tmpdir(), "nxc-cli-sync-off-home-"));
    try {
      const configPath = join(codexHome, "config.toml");
      writeFileSync(configPath, 'model = "gpt-5"\n', "utf8");
      writeFileSync(join(nxcHome, "config.json"), JSON.stringify({ providers: {}, defaultProvider: "openai", clientIntegrations: { codex: false }, checkForUpdates: false }), "utf8");
      ...
      expect(result.status).toBe(0);
      // #1931: explicit sync now refreshes the nxc-side catalog/cache while OFF when a
  ...
  test("sync exits nonzero when managed-default cleanup is ambiguous", () => {
    const codexHome = mkdtempSync(join(tmpdir(), "nxc-cli-sync-codex-"));
    const nxcHome = mkdtempSync(join(tmpdir(), "nxc-cli-sync-home-"));
    try {
      writeFileSync(join(codexHome, "config.toml"), [
        "# Managed by nexcode: native subagent defaults table",
        "[agents]",
        "# Managed by nexcode: native subagent default",
        "",
      ...
      const catalogPath = join(codexHome, "nexcode-catalog.json");
  ...
  test("help documents both directions of the switch", () => {
    const codexHome = mkdtempSync(join(tmpdir(), "nxc-cli-help-codex-"));
    const nxcHome = mkdtempSync(join(tmpdir(), "nxc-cli-help-home-"));
    try {
      writeFileSync(join(codexHome, "config.toml"), 'model = "gpt-5"\n', "utf8");
      writeFileSync(join(nxcHome, "config.json"), JSON.stringify({
        providers: {}, defaultProvider: "openai", checkForUpdates: false,
