...
const anthropicOAuth: NxcProviderConfig = { adapter: "anthropic", baseUrl: "https://api.anthropic.com", authMode: "oauth" };
...
describe("isCodexAuth is login-shaped, not provider-shaped", () => {
  ...
  test("credential without a canonical forward provider -> false", () => {
    usableCodexAccounts.add(MAIN_CODEX_ACCOUNT_ID);
    const keyed: NxcProviderConfig = { adapter: "openai-responses", baseUrl: "https://other.test/v1", authMode: "key", apiKey: "k" };
...
describe("isAnthropicAuth mirrors the stored-OAuth predicate", () => {
  ...
  test.each([
    ...
    ["wrong adapter", { ...anthropicOAuth, adapter: "openai-chat" } as NxcProviderConfig, active],
    ["key auth", { ...anthropicOAuth, authMode: "key", apiKey: "k" } as NxcProviderConfig, active],
