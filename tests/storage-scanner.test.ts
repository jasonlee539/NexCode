 ...
 */
function buildFixtureHome(home: string = mkdtempSync(join(tmpdir(), "nxc-storage-fixture-"))): string {
...
describe("scanStorage", () => {
  ...
  test("counts DB rows correctly when CODEX_HOME contains URI-reserved characters", () => {
   ...
   const parent = mkdtempSync(join(tmpdir(), "nxc-storage-uri-"));
  ...
  test("reports empty buckets for a missing or empty home without throwing", () => {
    fixtureHome = mkdtempSync(join(tmpdir(), "nxc-storage-empty-"));
  ...
  test("throws when the home exists but is not a directory", () => {
    fixtureHome = mkdtempSync(join(tmpdir(), "nxc-storage-notdir-"));
