...
function liveKimiProvider(onFetch: () => void): NxcProviderConfig {
  return {
...
async function runCatalogGather(
  ...
  const config: NxcConfig = { providers: { kimi: liveKimiProvider(onFetch) } };
...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), "nxc-catalog-auth-observe-"));
  nexcodeHome = join(root, "nexcode");
  mkdirSync(nexcodeHome, { recursive: true, mode: 0o700 });
...
describe("catalog gather OAuth observation", () => {
  test("expired active token stays typed and gather does not refresh or touch the auth store", async () => {
    ...
    const listingBefore = readdirSync(nexcodeHome).sort();
    ...
    expect(readdirSync(nexcodeHome).sort()).toEqual(listingBefore);
    expect(readdirSync(nexcodeHome).some(name => name.startsWith("auth.json.invalid-"))).toBe(false);
