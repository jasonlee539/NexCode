...
const previousApiAuthToken = process.env.NEXCODE_API_AUTH_TOKEN;
...
afterEach(() => {
  ...
  if (previousApiAuthToken === undefined) delete process.env.NEXCODE_API_AUTH_TOKEN;
  else process.env.NEXCODE_API_AUTH_TOKEN = previousApiAuthToken;
...
describe("service listen-port bake", () => {
  test("resolveServiceListenPort prefers override, then NXC_BAKE_PORT, then config", () => {
    process.env.NEXCODE_HOME = TEST_DIR;
    mkdirSync(TEST_DIR, { recursive: true });
    saveConfig({ port: 10100, hostname: "127.0.0.1", defaultProvider: "openai", providers: {} } as NxcConfig);
    expect(resolveServiceListenPort(18765)).toBe(18765);
    const prev = process.env.NXC_BAKE_PORT;
    try {
      process.env.NXC_BAKE_PORT = "15555";
      expect(resolveServiceListenPort()).toBe(15555);
      delete process.env.NXC_BAKE_PORT;
      expect(resolveServiceListenPort()).toBe(10100);
      saveConfig({ port: 0, hostname: "127.0.0.1", defaultProvider: "openai", providers: {} } as NxcConfig);
      expect(resolveServiceListenPort()).toBe(10100);
    } finally {
      if (prev === undefined) delete process.env.NXC_BAKE_PORT;
      else process.env.NXC_BAKE_PORT = prev;
    }
  });
  ...
  test("Windows batch and launchd/systemd shell commands bake start --port", () => {
    process.env.NEXCODE_HOME = TEST_DIR;
    mkdirSync(TEST_DIR, { recursive: true });
    saveConfig({ port: 13337, hostname: "127.0.0.1", defaultProvider: "openai", providers: {} } as NxcConfig);
    const script = buildWindowsServiceScript({ bun: "C:\\NexCode\\bun.exe", bunRuntimeSource: "bundled", cli: "C:\\NexCode\\cli.ts" });
...
describe("systemd service unit", () => {
  ...
  test("the Windows wrapper bakes proxy env the same way the unit and plist do (#2107)", () => {
    ...
    const script = buildWindowsServiceScript(
      { bun: "C:\\NexCode\\bun.exe", bunRuntimeSource: "bundled", cli: "C:\\NexCode\\cli.ts" },
      10100,
  ...
  test("preserves custom Codex and NexCode homes", () => {
    const oldCodexHome = process.env.CODEX_HOME;
    const oldCodexSqliteHome = process.env.CODEX_SQLITE_HOME;
    const oldNexCodeHome = process.env.NEXCODE_HOME;
    const oldApiAuthToken = process.env.NEXCODE_API_AUTH_TOKEN;
    try {
      process.env.CODEX_HOME = "/tmp/codex-home";
      process.env.CODEX_SQLITE_HOME = "/tmp/codex-sqlite-home";
      process.env.NEXCODE_HOME = "/tmp/nexcode-home";
      process.env.NEXCODE_API_AUTH_TOKEN = "local-secret";
      const unit = buildUnit();
      ...
      expect(unit).toContain('Environment="NEXCODE_HOME=/tmp/nexcode-home"');
      expectTextToContainPath(unit, serviceApiTokenFilePath());
      expect(unit).not.toContain("local-secret");
      expect(unit).not.toContain("Environment=\"NEXCODE_API_AUTH_TOKEN=");
    } finally {
      ...
      if (oldNexCodeHome === undefined) delete process.env.NEXCODE_HOME;
      else process.env.NEXCODE_HOME = oldNexCodeHome;
      if (oldApiAuthToken === undefined) delete process.env.NEXCODE_API_AUTH_TOKEN;
      else process.env.NEXCODE_API_AUTH_TOKEN = oldApiAuthToken;
    }
  });
  ...
  test("service start checks for the systemd user unit before shelling out", async () => {
    ...
    expect(startSystemd).toContain("nxc service install");
    ...
    expect(installSystemd).not.toContain("nxc service install");
...
describe("service install auth preflight", () => {
  test("rejects non-loopback service install without a persisted API token", () => {
    ...
    expect(() => assertServiceAuthEnvironment()).toThrow("NEXCODE_API_AUTH_TOKEN");
  });
...
describe("Windows service task", () => {
  test("builds schtasks create args from XML instead of runtime flags", () => {
    const script = "C:\\Users\\a&b\\.nexcode\\nexcode-service.cmd";
  ...
  test("builds service-like Task Scheduler XML settings", () => {
    const script = "C:\\Users\\a&b\\.nexcode\\nexcode-service.cmd";
    const launcher = "C:\\Users\\a&b\\.nexcode\\nexcode-service-launcher.vbs";
    ...
    expect(xml).toContain('<Arguments>/b /nologo &quot;C:\\Users\\a&amp;b\\.nexcode\\nexcode-service-launcher.vbs&quot;</Arguments>');
    expect(xml).not.toContain("<Command>C:\\Users\\a&amp;b\\.nexcode\\nexcode-service.cmd</Command>");
  });

  test("validates the registered scheduler action, trigger, principal, and settings", () => {
    const wscript = "C:\\Windows\\System32\\wscript.exe";
    const launcher = "C:\\Users\\Test\\.nexcode\\service-launcher.vbs";
  ...
  test("accepts a canonicalized export whose launcher path contains an ampersand", () => {
    const wscript = "C:\\Windows\\System32\\wscript.exe";
    const launcher = "C:\\Users\\a&b\\.nexcode\\service-launcher.vbs";
    const xml = buildWindowsTaskXml("ignored.cmd", launcher)
      .replace(/<Command>.*?<\/Command>/, `<Command>${wscript}</Command>`);
    // `&` stays `&amp;` (it must, or the XML is malformed); only the quotes flip.
    const canonical = xml.replace(
      "<Arguments>/b /nologo &quot;C:\\Users\\a&amp;b\\.nexcode\\service-launcher.vbs&quot;</Arguments>",
      "<Arguments>/b /nologo \"C:\\Users\\a&amp;b\\.nexcode\\service-launcher.vbs\"</Arguments>",
    );
    expect(windowsTaskRegistrationHealthy(canonical, wscript, launcher)).toBe(true);
  });
  ...
  test("hidden launcher VBS stays resident and escapes quotes in the wrapper path", () => {
    const vbs = buildWindowsLauncherVbs('C:\\Users\\quo"te\\.nexcode\\nexcode-service.cmd');

    // windowStyle 0 (hidden) + bWaitOnReturn True (resident, so IgnoreNew and /end keep working).
    expect(vbs).toContain(", 0, True");
    expect(vbs).toContain('shell.Run """C:\\Users\\quo""te\\.nexcode\\nexcode-service.cmd""", 0, True');
    expect(vbs).toContain('CreateObject("WScript.Shell")');
  });

  test("hidden launcher VBS carries non-ASCII profile paths verbatim", () => {
    const vbs = buildWindowsLauncherVbs("C:\\Users\\한글사용자\\.nexcode\\nexcode-service.cmd");

    expect(vbs).toContain("C:\\Users\\한글사용자\\.nexcode\\nexcode-service.cmd");
  });
  ...
  test("escapes environment values that would break out of set quotes", () => {
    ...
    try {
      ...
      process.env.NEXCODE_HOME = 'C:\\nxc" & del C:\\important & rem "';
      process.env.NEXCODE_API_AUTH_TOKEN = 'token" & echo LEAK & rem "';
      const script = buildWindowsServiceScript();
      expect(script).toContain('set "PATH=C:\\safe & echo PWNED & rem "');
      expect(script).toContain('set "NEXCODE_HOME=C:\\nxc & del C:\\important & rem "');
      ...
      expect(script).not.toContain('set "NEXCODE_HOME=C:\\nxc" & del');
  ...
  test("escapes service executable paths through variables", () => {
    const script = buildWindowsServiceScript({
      ...
      cli: "C:\\NexCode&Dir\\cli.ts",
    });

    expect(script).toContain('set "NXC_BUN=C:\\Bun&Dir\\100%%bun^^\\bun.exe"');
    expect(script).toContain('set "NXC_CLI=C:\\NexCode&Dir\\cli.ts"');
    expect(script).toContain('"%NXC_BUN%" "%NXC_CLI%" start --port');
  ...
  test("switches the wrapper console to UTF-8 and sleeps via ping (timeout dies without console stdin)", () => {
    ...
    expect(script.indexOf("chcp 65001 >nul")).toBeLessThan(script.indexOf('set "NXC_SERVICE=1"'));
  ...
  test("stops instead of restart-looping when an update removed the baked runtime or CLI (#1849)", () => {
    const script = buildWindowsServiceScript({
      bun: "C:\\NexCode\\bun.exe",
      bunRuntimeSource: "bundled",
      cli: "C:\\NexCode\\cli.ts",
    });
    const loopAt = script.indexOf(":loop");
    const bunCheckAt = script.indexOf('if not exist "%NXC_BUN%"');
    const cliCheckAt = script.indexOf('if not exist "%NXC_CLI%"');
    const launchAt = script.indexOf('"%NXC_BUN%" "%NXC_CLI%" start --port');
    ...
    expect(script).toContain(".nxc-backup-*");
  });
  ...
  test("rewrites profile-relative paths to env indirection so non-ASCII usernames survive OEM-codepage batch parsing", () => {
    ...
    try {
      ...
      expect(script).toContain('set "NXC_CLI=%APPDATA%\\npm\\node_modules\\nexcode\\src\\cli.ts"');
      expect(script).not.toContain('set "NXC_BUN=C:\\Users\\한글사용자');
    } finally {
  ...
  test("writes token-safe startup identity and child output to the service log", () => {
    ...
    try {
      ...
      expect(script).toContain('set "NXC_SERVICE_LOG=');
      expect(script).toContain("nexcode service wrapper start");
      expect(script).toContain('echo bun="%NXC_BUN%"');
      expect(script).toContain('echo bun_source="');
      expect(script).toContain('echo cli="%NXC_CLI%"');
      expect(script).toContain('echo nexcode_home="%NEXCODE_HOME%"');
      expect(script).toContain('echo codex_home="%CODEX_HOME%"');
      expect(script).toContain('set "CODEX_SQLITE_HOME=C:\\codex-sqlite-home"');
      expect(script).toContain('echo token_file="%NXC_API_TOKEN_FILE%"');
      expect(script).toMatch(/"%NXC_BUN%" "%NXC_CLI%" start --port \d+ >>"%NXC_SERVICE_LOG%" 2>&1/);
      expect(script).toContain("child exited with code %ERRORLEVEL%");
      expect(script).not.toContain("local-secret");
      expect(script).not.toContain('set "NEXCODE_API_AUTH_TOKEN=');
    } finally {
      if (oldCodexHome === undefined) delete process.env.CODEX_HOME;
      else process.env.CODEX_HOME = oldCodexHome;
      if (oldCodexSqliteHome === undefined) delete process.env.CODEX_SQLITE_HOME;
...
describe("launchd service plist", () => {
  test("every durable launcher stamps the Bun provenance paired with the binary it baked (#848)", () => {
    const inheritedOverride = process.env.NEXCODE_BUN_PATH;
    const inheritedSource = process.env.NXC_BUN_RUNTIME_SOURCE;
    const inheritedPath = process.env.NXC_BUN_RUNTIME_PATH;
    const overrideBun = join(TEST_DIR, "provenance-override-bun.exe");
    mkdirSync(TEST_DIR, { recursive: true });
    writeFileSync(overrideBun, "x".repeat(2 * 1024 * 1024));
    try {
      // NEXCODE_BUN_PATH is consumed by the Node launcher before Bun can load a
      // project dotenv. Once Bun is running, an unpaired value is untrusted and
      // must never be persisted into a durable launcher.
      delete process.env.NXC_BUN_RUNTIME_SOURCE;
      delete process.env.NXC_BUN_RUNTIME_PATH;
      process.env.NEXCODE_BUN_PATH = overrideBun;
      const plist = buildPlist();
      expect(plist).not.toContain("<key>NXC_BUN_RUNTIME_SOURCE</key><string>override</string>");
      expect(plist).not.toContain(overrideBun);

      const unit = buildUnit();
      expect(unit).not.toContain('Environment="NXC_BUN_RUNTIME_SOURCE=override"');
      expect(unit).not.toContain(overrideBun);

      const script = buildWindowsServiceScript();
      expect(script).not.toContain('set "NXC_BUN_RUNTIME_SOURCE=override"');
      ...
      process.env.NXC_BUN_RUNTIME_SOURCE = "override";
      process.env.NXC_BUN_RUNTIME_PATH = process.execPath;
      const trustedPlist = buildPlist();
      expect(trustedPlist).toContain("<key>NXC_BUN_RUNTIME_SOURCE</key><string>override</string>");
      expectTextToContainPath(trustedPlist, process.execPath);
      expect(buildUnit()).toContain('Environment="NXC_BUN_RUNTIME_SOURCE=override"');
      expect(buildWindowsServiceScript()).toContain('set "NXC_BUN_RUNTIME_SOURCE=override"');
    } finally {
      if (inheritedOverride === undefined) delete process.env.NEXCODE_BUN_PATH;
      else process.env.NEXCODE_BUN_PATH = inheritedOverride;
      if (inheritedSource === undefined) delete process.env.NXC_BUN_RUNTIME_SOURCE;
      else process.env.NXC_BUN_RUNTIME_SOURCE = inheritedSource;
      if (inheritedPath === undefined) delete process.env.NXC_BUN_RUNTIME_PATH;
      else process.env.NXC_BUN_RUNTIME_PATH = inheritedPath;
    }
  });
  ...
  test("preserves custom Codex and NexCode homes", () => {
    ...
    try {
      ...
      expect(plist).toContain("<key>NEXCODE_HOME</key><string>/tmp/nexcode-home</string>");
      expectTextToContainPath(plist, serviceApiTokenFilePath());
      expect(plist).not.toContain("local-secret");
      expect(plist).not.toContain("<key>NEXCODE_API_AUTH_TOKEN</key>");
    } finally {
      if (oldCodexHome === undefined) delete process.env.CODEX_HOME;
      else process.env.CODEX_HOME = oldCodexHome;
      if (oldCodexSqliteHome === undefined) delete process.env.CODEX_SQLITE_HOME;
  ...
  test("carries an absolute POSIX sqlite home into POSIX units without host anchoring", () => {
    ...
    try {
      process.env.CODEX_SQLITE_HOME = "/var/lib/nexcode/codex-sqlite";

      expect(buildPlist()).toContain(
        "<key>CODEX_SQLITE_HOME</key><string>/var/lib/nexcode/codex-sqlite</string>",
      );
      expect(buildUnit()).toContain(
        'Environment="CODEX_SQLITE_HOME=/var/lib/nexcode/codex-sqlite"',
      );
    } finally {
...
describe("service lifecycle cleanup ordering", () => {
  ...
  test("rollback preserves a task owned by another install attempt and reports residual state", async () => {
    ...
    const result = await rollbackOwned("attempt-a", "nexcode-proxy", {
    ...
    expect(result).toContain("Residual scheduler state: task nexcode-proxy remains registered");
  });
  ...
  test("fresh registration elevates only the fixed create after a structured denial", async () => {
    const calls: string[] = [];
    const parent = mkdtempSync(join(tmpdir(), "nxc-service-fixed-create-"));
  ...
  test("fresh registration UAC denial returns before task probing or cleanup", async () => {
    const calls: string[] = [];
    const parent = mkdtempSync(join(tmpdir(), "nxc-service-uac-denial-"));
    const stagedXml = join(parent, "attempt.xml");
    try {
  ...
  test("fresh registration never elevates an unstructured scheduler failure", async () => {
    const calls: string[] = [];
    const parent = mkdtempSync(join(tmpdir(), "nxc-service-unstructured-"));
    const stagedXml = join(parent, "attempt.xml");
    try {
      writeFileSync(stagedXml, `\uFEFF${buildWindowsTaskXml(undefined, undefined, registrationAttemptNonce)}`, "utf16le");
      await expect(registerFreshWindowsSchedulerTask(stagedXml, registrationAttemptNonce, {
  ...
  test("create success followed by proven absence does not request a pointless rollback UAC", async () => {
    const calls: string[] = [];
    const parent = mkdtempSync(join(tmpdir(), "nxc-service-proven-absence-"));
    const stagedXml = join(parent, "attempt.xml");
    try {
      writeFileSync(stagedXml, `\uFEFF${buildWindowsTaskXml(undefined, undefined, registrationAttemptNonce)}`, "utf16le");
      await expect(registerFreshWindowsSchedulerTask(stagedXml, registrationAttemptNonce, {
  ...
  test("fresh registration requires the live Task Scheduler XML before cleanup can begin", async () => {
    const calls: string[] = [];
    const parent = mkdtempSync(join(tmpdir(), "nxc-service-live-xml-"));
  ...
  test("fresh registration elevation uses captured XML bytes after the staged file changes", async () => {
    const parent = mkdtempSync(join(tmpdir(), "nxc-service-elevated-xml-"));
    ...
    try {
      ...
      await registerFreshWindowsSchedulerTask(xmlPath, registrationAttemptNonce, {
        ...
        elevate: async (taskName, xml) => {
          expect(taskName).toBe("nexcode-proxy");
          elevatedXml = xml;
        },
        probe: () => ({ status: "present", detail: "present" }),
  ...
  test("fresh registration rejects a staged definition owned by another attempt before create", async () => {
    const parent = mkdtempSync(join(tmpdir(), "nxc-service-foreign-stage-"));
    const xmlPath = join(parent, "attempt.xml");
    const calls: string[] = [];
    try {
      writeFileSync(
  ...
  test("fresh scheduler staging hardens its private directory and XML before registration", async () => {
    const parent = mkdtempSync(join(tmpdir(), "nxc-service-stage-order-"));
  ...
  test("fresh scheduler staging removes a partially-written private directory on failure", () => {
    const parent = mkdtempSync(join(tmpdir(), "nxc-service-stage-failure-"));
    const stageDir = join(parent, "private-stage");
    try {
  ...
  test("fresh scheduler install removes staging before initializing config ownership", async () => {
    const parent = mkdtempSync(join(tmpdir(), "nxc-service-fresh-ownership-"));
  ...
  test("fresh scheduler install rolls back before cleanup when a new config root cannot be claimed", async () => {
    const parent = mkdtempSync(join(tmpdir(), "nxc-service-ownership-race-"));
    const home = join(parent, "config");
    ...
    try {
      await expect(installFreshWindowsSchedulerSafely({
        ...
        writeState: () => { calls.push("write-state"); },
        rollbackTask: async () => { calls.push("rollback-task"); return null; },
      })).rejects.toThrow(/fresh NexCode config root could not be claimed/);
...
describe("service diagnostics", () => {
  ...
  test("flags stale baked service paths recorded at install time", () => {
    ...
    try {
      process.env.NEXCODE_HOME = stateDir;
      ...
      writeFileSync(statePath, JSON.stringify({ version: 1, codexHome: stateDir, nexcodeHome: stateDir }), "utf8");
      expect(bakedServicePathsDiagnostic()).toBeNull();
    } finally {
      if (oldNexCodeHome === undefined) delete process.env.NEXCODE_HOME;
      else process.env.NEXCODE_HOME = oldNexCodeHome;
 ...
 * written one sat unused, which is the 2026-08-02 report: `nxc service` prints a
 * checkmark, `launchctl list` shows the job, and the port answers nothing.
 *
 * Measured on macOS 27.0:
 *   $ launchctl load -w ~/Library/LaunchAgents/com.nexcode.proxy.plist
...
describe("launchctl load verification", () => {
  ...
  describe("launchdJobMatchesPlist", () => {
    // Shape captured from a real `launchctl print gui/$(id -u)/com.nexcode.proxy`
    ...
    const printed = (command: string) => [
      ...
      "\t\t/bin/sh",
      "\t\t-lc",
      `\t\tif [ -f '/h/.nexcode/service-api-token' ]; then NEXCODE_API_AUTH_TOKEN="$(cat '/h/.nexcode/service-api-token')"; export NEXCODE_API_AUTH_TOKEN; fi; ${command}`,
      "\t}",
    ].join("\n");
  ...
  describe("startLaunchd", () => {
     ...
     * correct one, so `nxc service start` on a healthy service hits it every time.
...
describe("auth preflight retry command (260804 #970 follow-up)", () => {
  ...
  test("serviceRetryCommand picks the command that can actually succeed", () => {
    ...
    expect(serviceRetryCommand({ installed: true, conflict: false })).toBe("nxc service repair");
    // Nothing registered: repairService() would refuse, so install is the only option.
    expect(serviceRetryCommand({ installed: false, conflict: false })).toBe("nxc service install");
    ...
    expect(serviceRetryCommand({ installed: true, conflict: true })).toBe("nxc service install");
  });
});
...
describe("service serving confirmation", () => {
  ...
  /**
   * `nxc service status` printed raw `launchctl list` output, which reports a
  ...
  describe("serviceStatusReport", () => {
    ...
    test("names the log path and the repair command when nothing answers", async () => {
      ...
      expect(out).toContain("nxc service repair");
      expect(out).toContain("nxc start");
    });
   ...
   * change the definition systemd has loaded until `daemon-reload`, so `nxc service
   ...
   * scheduler wrapper (`nexcode-service.cmd`) and the WinSW XML. Both must be
  ...
  describe("windowsListenPort", () => {
    test("reads the port baked into the scheduler wrapper", () => {
      expect(windowsListenPort({
        readScript: () => '"%NXC_BUN%" "%NXC_CLI%" start --port 18222 >>"%LOG%" 2>&1',
      })).toBe(18222);
    });
    ...
    test("prefers the argument tail over a path that looks like one", () => {
      expect(windowsListenPort({
        readScript: () => 'set "NXC_BUN=C:\\start --port 9999\\bun.exe"\r\n"%NXC_BUN%" "%NXC_CLI%" start --port 18222\r\n',
      })).toBe(18222);
    });
...
describe("service definitions are not world-readable", () => {
  ...
  posixOnly("a freshly written definition is owner-only", () => {
    const dir = mkdtempSync(join(tmpdir(), "nxc-service-mode-"));
    try {
