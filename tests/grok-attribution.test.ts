...
/**
 * D3 activation evidence: the fence stamps `x-nexcode-grok` on every registered
...
test("the managed fence stamps the grok attribution header on every model", () => {
  ...
  expect(block).toContain('extra_headers = { "x-nexcode-grok" = "1" }');
  ...
  expect(modelSections[0]).toContain("x-nexcode-grok");
});

test("the fence survives a write and keeps the header line parseable", () => {
  const root = mkdtempSync(join(tmpdir(), "nxc-grok-attr-"));
  ...
  try {
    ...
    expect(content).toContain('extra_headers = { "x-nexcode-grok" = "1" }');
  } finally {
...
const config = { port: 0, defaultProvider: "mock", providers: {} } as unknown as NxcConfig;
...
test("a request carrying the header is tagged grok; one without is not", async () => {
  ...
  await handleChatCompletions(chatReq({ "x-nexcode-grok": "1" }), config, tagged).catch(() => {});
