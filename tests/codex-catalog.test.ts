...
describe("combo catalog capability intersection", () => {
  ...
  test("provider allowlists do not remove a current slashed combo alias", () => {
    const row = (slug: string, ownedBy: string, sourceProvider = ownedBy) => ({
      ...
      slug,
      owned_by: ownedBy,
      description: `Routed via nexcode → ${sourceProvider} (${ownedBy}).`,
  ...
  test("an empty routed gather preserves a configured native-alias row from disk", () => {
    ...
    try {
      ...
      expect(merged.find(entry => entry.slug === alias)).toMatchObject({
        ...
        owned_by: "combo",
        nexcode_catalog_kind: CODEX_NATIVE_ALIAS_CATALOG_KIND,
      });
    ...
    for (const generated of [
      {
        ...nativeTemplate(),
        slug,
        description: "Routed via nexcode → managed/model (managed).",
      },
      {
        ...nativeTemplate(),
        slug,
        nexcode_catalog_kind: CODEX_CUSTOM_MODEL_CATALOG_KIND,
      },
    ]) {
      const merged = mergeObservedForTest({
  ...
  test("a fresh account row wins without a false unrestorable-shadow warning", () => {
    ...
    try {
      const account = {
        ...nativeTemplate(),
        slug,
        nexcode_catalog_kind: CODEX_ACCOUNT_BOUND_CATALOG_KIND,
      };
      const combo = {
        ...nativeTemplate(),
        slug,
      ...
      expect(merged.filter(entry => entry.slug === slug)).toEqual([
        expect.objectContaining({ nexcode_catalog_kind: CODEX_ACCOUNT_BOUND_CATALOG_KIND }),
      ]);
  ...
  test("retains configured combo targets when authoritative live discovery omits them (NXC-111)", async () => {
    // Repro from #1308 / NXC-111: live /models returns a different roster than the
  ...
  test("warm cache still retains configured combo targets added inside the TTL (NXC-111)", async () => {
...
describe("configured CatalogModel displayName -> catalog display_name", () => {
  ...
  test("a configured customModel displayName propagates end-to-end through gatherRoutedModels", async () => {
    ...
    try {
      ...
      expect(row?.nexcode_catalog_kind).toBe(CODEX_CUSTOM_MODEL_CATALOG_KIND);
    } finally {
      globalThis.fetch = originalFetch;
      clearModelCache("custom-provider");
    }
...
describe("legacy custom-model catalog ownership", () => {
  ...
  test("deletion evidence removes only an old unmarked NexCode custom row", () => {
  ...
  test("fresh provider rediscovery acknowledges a removed slug before a later outage", () => {
    ...
    expect(acknowledged?.nexcode_catalog_kind).toBe(CODEX_PROVIDER_MODEL_CATALOG_KIND);
    ...
    expect(degraded).toContainEqual(expect.objectContaining({
      slug: "custom-provider/removed-model",
      nexcode_catalog_kind: CODEX_PROVIDER_MODEL_CATALOG_KIND,
    }));
  });
  ...
  test("unknown catalog kinds fail closed under legacy deletion evidence", () => {
    const unknownKind = {
      ...
      nexcode_catalog_kind: "future-model-v2",
    };
    const merged = mergeObservedForTest({
...
function openAiApiCatalogConfig(overrides: Record<string, unknown> = {}): NxcConfig {
  return {
    port: 10100,
...
describe("Codex catalog routed normalization", () => {
  test("does not reuse a routed native alias as the native catalog template", () => {
    const routedAlias = {
      ...
      slug: "gpt-5.6-sol",
      owned_by: "combo",
      description: "Routed via nexcode → Nova1 (openai).",
      nexcode_catalog_kind: CODEX_NATIVE_ALIAS_CATALOG_KIND,
    };
  ...
  test("materializes bundled Codex catalog when no on-disk source exists", () => {
    const dir = mkdtempSync(join(tmpdir(), "nxc-catalog-"));
    const path = join(dir, "nested", "nexcode-catalog.json");
    try {
  ...
  test("gpt-5.6 natives come from the pinned upstream snapshot (PR #31684) with exact per-slug specs", () => {
    ...
    // nxc adaptations: client-version gate stripped; ws preference gated off by default.
  ...
  test("catalog sync upgrades fallback-quality gpt-5.6 entries but preserves genuine ones", () => {
    // Fallback-quality: display_name stamped with the bare slug (nxc synthesis signature),
    // wrong ladder (ultra on luna) left by an older nxc version.
    const synthesizedLuna = {
      ...nativeTemplate(),
      slug: "gpt-5.6-luna",
      display_name: "gpt-5.6-luna",
  ...
  test("backfilled natives and account clones use the pristine native baseline", () => {
    const accountRows = buildCatalogEntries(
      ...
      new Set(),
      ["team"],
    ).filter(entry => entry.nexcode_catalog_kind === CODEX_ACCOUNT_BOUND_CATALOG_KIND);
    ...
    for (const priority of [41, 9]) {
      ...
      expect(account?.nexcode_catalog_kind).toBe(CODEX_ACCOUNT_BOUND_CATALOG_KIND);
    }
  });
  ...
  test("a custom-named provider on a known vendor endpoint still gets the opt-in (#1100)", () => {
    ...
    const reported: NxcConfig["providers"][string] = {
      adapter: "openai-chat",
    ...
    const custom: NxcConfig["providers"][string] = {
      adapter: "openai-chat",
      baseUrl: "https://api.z.ai/api/coding/paas/v4",
      authMode: "key",
    };
    ...
    const renamed: NxcConfig["providers"][string] = {
      adapter: "openai-chat",
      baseUrl: "https://open.bigmodel.cn/api/paas/v4",
      authMode: "key",
    };
  ...
  test("the destination fallback never claims an unrelated custom endpoint (#1100)", () => {
    ...
    const unknown: NxcConfig["providers"][string] = {
      adapter: "openai-chat",
    ...
    const opinionated: NxcConfig["providers"][string] = {
      adapter: "openai-chat",
      baseUrl: "https://api.z.ai/api/coding/paas/v4",
      authMode: "key",
  ...
  test("registry summary defaults are never persisted into saved config (#1100)", () => {
    ...
    const created: NxcConfig["providers"][string] = {
      adapter: "openai-chat",
      baseUrl: "https://api.deepseek.com",
      authMode: "key",
    };
    ...
    const submitted: NxcConfig["providers"][string] = {
      adapter: "openai-chat",
      baseUrl: "https://api.deepseek.com",
      authMode: "key",
  ...
  test("explicit per-model overrides survive registry backfill", () => {
    const provider: NxcConfig["providers"][string] = {
      adapter: "openai-chat",
      baseUrl: "https://api.deepseek.com",
      authMode: "key",
      modelSupportsReasoningSummaries: { "deepseek-v4-flash": false },
...
describe("OpenAI API trusted catalog augmentation", () => {
  ...
  test("trusted OpenAI API reconstruction is authoritative after discovery failure", async () => {
    ...
    try {
      ...
      const ghost = {
        ...
        description: "Routed via nexcode → openai-apikey (openai-apikey).",
      };
  ...
  test("is a no-op when API tier is absent or disabled", () => {
    ...
    const absent: NxcConfig = { port: 10100, defaultProvider: "other", providers: { other: { adapter: "openai-chat", baseUrl: "https://example.test/v1" } } };
