...
function backupPathForTestCatalog(codexHome: string, nexcodeHome: string, catalogName: string): string {
  ...
  return join(nexcodeHome, `catalog-backup-${backupId}.json`);
}

function runScript(codexHome: string, nexcodeHome: string, script: string): { stdout: string; status: number } {
  const result = spawnSync(process.execPath, ["--eval", script], {
    cwd: repoRoot,
    env: { ...process.env, CODEX_HOME: codexHome, NEXCODE_HOME: nexcodeHome },
    encoding: "utf8",
  });
...
describe("Codex catalog restore", () => {
  ...
  beforeEach(() => {
    codexHome = mkdtempSync(join(tmpdir(), "nxc-catalog-home-"));
    nexcodeHome = mkdtempSync(join(tmpdir(), "nxc-catalog-nxc-"));
  });
  ...
  test("fallback restore repairs only enabled natives with unanimously visible account clones", () => {
    const catalogPath = join(codexHome, "catalog.json");
    writeFileSync(join(codexHome, "config.toml"), 'model_catalog_json = "catalog.json"\n', "utf8");
    writeFileSync(join(nexcodeHome, "config.json"), JSON.stringify({
  ...
  test("backup restore repairs only later native additions with trusted visible clones", () => {
    const catalogPath = join(codexHome, "catalog.json");
    const backupPath = backupPathForTestCatalog(codexHome, nexcodeHome, "catalog.json");
    writeFileSync(join(codexHome, "config.toml"), 'model_catalog_json = "catalog.json"\n', "utf8");
  ...
  test("does not apply generic legacy backup to a custom catalog path", () => {
    ...
    writeFileSync(join(nexcodeHome, "catalog-backup.json"), JSON.stringify({
