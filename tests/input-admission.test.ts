...
import type { NxcMessage, NxcParsedRequest, NxcProviderConfig, NxcTool } from "../src/types";

const CANONICAL_NATIVE: NxcProviderConfig = {
  adapter: "openai-responses",
  baseUrl: "https://chatgpt.com/backend-api/codex",
  authMode: "forward",
};

function request(messages: NxcMessage[], tools?: NxcTool[]): NxcParsedRequest {
  return {
    modelId: "test-model",
...
function userText(text: string): NxcMessage {
...
describe("resolveInputCeiling", () => {
  ...
  test("returns null when nothing is configured", () => {
    const provider: NxcProviderConfig = { adapter: "openai-chat", baseUrl: "https://example.test/v1" };
  ...
  test("a custom provider merely NAMED openai does not inherit native limits", () => {
    const impostor: NxcProviderConfig = {
      adapter: "openai-chat",
  ...
  test("an explicit user window still wins over native metadata", () => {
    const pinned: NxcProviderConfig = { ...CANONICAL_NATIVE, modelContextWindows: { "gpt-5.6-sol": 50_000 } };
...
describe("checkInputAdmission", () => {
  ...
  test("admits everything when no ceiling resolves", () => {
    const unknown: NxcProviderConfig = { adapter: "openai-chat", baseUrl: "https://example.test/v1" };
  ...
  test("the estimator CJK sampling alias does not trip the gate", () => {
    ...
    const aliased: NxcProviderConfig = {
      adapter: "openai-chat",
      baseUrl: "https://example.test/v1",
