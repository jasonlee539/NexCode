...
function provider(requestPacing: NxcProviderConfig["requestPacing"]): NxcProviderConfig {
...
describe("provider request pacing queue", () => {
  test("spaces concurrent starts in one provider FIFO and exposes queue state", async () => {
    ...
    const configured = {
      ...
      fetch: fetchImpl,
    } as NxcProviderConfig & { fetch: typeof globalThis.fetch };
  ...
  test("Google AI Studio providerFetch paces each attempt through waitForPacing", async () => {
    let pacingWaited = 0;
    const configured: NxcProviderConfig = {
      adapter: "google",
