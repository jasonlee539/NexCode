...
const isolatedCodexHome = mkdtempSync(join(tmpdir(), "nxc-prov-codex-home-"));
...
function freshConfig(extra?: Record<string, unknown>) {
  const dir = mkdtempSync(join(tmpdir(), "nxc-prov-"));
  const config = {
    port: 10100,
    providers: {
      openai: {
...
describe("nxc provider", () => {
  test("provider --help prints usage", () => {
    const result = runCli(["provider", "--help"]);
    expect(result.status).toBe(0);
    expect(result.stdout).toContain("Usage: nxc provider");
  ...
  test("provider list shows configured providers", () => {
    ...
    try {
      const result = runCli(["provider", "list"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
  ...
  test("provider list --json returns valid JSON", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["provider", "list", "--json"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
  ...
  test("provider add registry provider seeds config", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["provider", "add", "deepseek", "--api-key", "sk-test"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
  ...
  test.each(["xai", "deepseek"])("login %s rejects a configured Codex account namespace before prompting", provider => {
    ...
    try {
      const before = readFileSync(configPath, "utf8");
      const result = runCli(["login", provider], { NEXCODE_HOME: dir });
  ...
  test("provider add custom provider requires --adapter and --base-url", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["provider", "add", "my-custom"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(1);
  ...
  test("provider show --json never prints secret-shaped modelCosts keys", () => {
    ...
    try {
      const result = runCli(["provider", "show", "blsc", "--json"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
  ...
  test("provider add rejects duplicate without --force", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["provider", "add", "openai"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(1);
  ...
  test("provider add with --force overwrites", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["provider", "add", "openai", "--force"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  ...
  test("provider add --set-default changes defaultProvider", () => {
    const { dir } = freshConfig();
    try {
      runCli(["provider", "add", "deepseek", "--api-key", "k", "--set-default"], { NEXCODE_HOME: dir });
      const config = readConfig(dir);
  ...
  test("provider remove works for non-default provider", () => {
    ...
    try {
      const result = runCli(["provider", "remove", "deepseek"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
  ...
  test("provider remove drops that provider's custom models (#1273)", () => {
    ...
    try {
      const result = runCli(["provider", "remove", "huggingface", "--json"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
  ...
  test("provider remove rejects default provider", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["provider", "remove", "openai"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(1);
  ...
  test("provider show displays config with masked secret", () => {
    ...
    try {
      const result = runCli(["provider", "show", "deepseek"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
      expect(result.stdout).toContain("deepseek");
  ...
  test("provider show --json returns valid JSON", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["provider", "show", "openai", "--json"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
      const parsed = JSON.parse(result.stdout);
  ...
  test("provider set-default changes default", () => {
    ...
    try {
      const result = runCli(["provider", "set-default", "deepseek"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
  ...
  test("provider set-default rejects unconfigured provider", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["provider", "set-default", "nonexistent"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(1);
  ...
  test("provider add warns on --api-key for oauth provider", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["provider", "add", "anthropic", "--api-key", "test"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
...
describe("nxc provider strict args", () => {
  test("provider list rejects unknown flags", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["provider", "list", "--bogus"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(1);
  ...
  test("provider add rejects unknown flags", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["provider", "add", "deepseek", "--unknown-thing"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(1);
      expect(result.stderr).toContain("Unknown flag");
    } finally {
      rmSync(dir, { recursive: true, force: true });
  ...
  test("provider show rejects unknown flags", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["provider", "show", "openai", "--bogus"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(1);
      expect(result.stderr).toContain("Unknown flag");
    } finally {
      rmSync(dir, { recursive: true, force: true });
...
describe("nxc provider mutating --json", () => {
  test("provider add --json returns structured output", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["provider", "add", "deepseek", "--api-key", "sk-test", "--json"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
      const parsed = JSON.parse(result.stdout);
  ...
  test("provider remove --json returns structured output", () => {
    ...
    try {
      const result = runCli(["provider", "remove", "deepseek", "--json"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
      const parsed = JSON.parse(result.stdout);
  ...
  test("provider set-default --json returns structured output", () => {
    ...
    try {
      const result = runCli(["provider", "set-default", "deepseek", "--json"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
      const parsed = JSON.parse(result.stdout);
...
describe("nxc provider add --sync", () => {
  test("provider add --sync flag is accepted without error", () => {
    ...
    try {
      ...
      const result = runCli(["provider", "add", "deepseek", "--api-key", "sk-test", "--sync"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
      expect(result.stdout).toContain("deepseek");
    } finally {
      rmSync(dir, { recursive: true, force: true });
  ...
  test("provider add --sync --json reports needsSync false", () => {
    const { dir } = freshConfig();
    try {
      const result = runCli(["provider", "add", "deepseek", "--api-key", "sk-test", "--sync", "--json"], { NEXCODE_HOME: dir });
      expect(result.status).toBe(0);
      const parsed = JSON.parse(result.stdout);
