...
const config = { providers: [] } as unknown as NxcConfig;

let testDir = "";
let previousHome: string | undefined;

beforeEach(() => {
  // addRequestLog persists to usage.jsonl; without a scratch NEXCODE_HOME a bare
  // `bun test <file>` run from outside the repo (no bunfig preload) writes these
  // fixture rows into the real ~/.nexcode log and poisons the GUI Usage page.
  previousHome = process.env.NEXCODE_HOME;
  testDir = mkdtempSync(join(tmpdir(), "nxc-logs-metrics-"));
...
describe("GET /api/logs display metrics", () => {
  ...
  test("fixture usage rows land in the scratch home, never the default location", () => {
    ...
    // repo writes fixture rows into the developer's real ~/.nexcode log.
    ...
    // The default location (what the resolver returns with no NEXCODE_HOME
