...
beforeEach(() => {
  root = join(mkdtempSync(join(tmpdir(), "nxc-integrations-journal-")), "integrations");
...
describe("store isolation", () => {
  test("everything a store writes stays under its own root", () => {
    const other = join(mkdtempSync(join(tmpdir(), "nxc-other-")), "integrations");
    try {
  ...
  test("a temp-rooted store leaves the real ownership manifest untouched", () => {
    // atomicWriteFile records writes in the nexcode uninstall manifest, but
    ...
    const manifest = join(getConfigDir(), ".nexcode-ownership.json");
  ...
  test("records written through the io() seam land in the bound store", () => {
    ...
    const other = join(mkdtempSync(join(tmpdir(), "nxc-io-seam-")), "integrations");
    try {
  ...
  test("priorRecord round-trips through the journal unchanged", () => {
    const priorRecord: OwnershipRecord = {
      ...
      fragmentPaths: [["providers", "nexcode"], ["models", "nexcode/x"]],
