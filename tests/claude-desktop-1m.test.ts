...
test("supports1m is true at and above the threshold, false below it", async () => {
  const home = mkdtempSync(join(tmpdir(), "nxc-desktop-1m-"));
  const prev = process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR;
  process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR = home;
  try {
    ...
    if (prev === undefined) delete process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR;
    else process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR = prev;
