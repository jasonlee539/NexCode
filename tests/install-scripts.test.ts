...
describe("install scripts", () => {
  ...
  test("Node can import the package main without executing the CLI", () => {
    const result = spawnSync("node", [
      "-e",
      "import('./bin/package-main.mjs').then(m => { if (m.cliCommand !== 'nxc') process.exit(2); })",
    ], {
      cwd: repoRoot,
      encoding: "utf8",
    });
  ...
  test("npmignore keeps GUI development docs out of the package", async () => {
    ...
    expect(guiReadme).toContain("nexcode dashboard");
  ...
  test("POSIX installer matches the Node launcher prerequisite", async () => {
    ...
    expect(script).toContain("npm install -g @bitkyc08/nexcode");
    expect(script).toContain("command -v nxc");
    expect(script).toContain("nxc help");
    expect(script).not.toContain("bun install -g @bitkyc08/nexcode");
  ...
  test("PowerShell installer matches the Node launcher prerequisite", async () => {
    ...
    expect(script).toContain("Node.js 18+ is required");
    expect(script).toContain("& $npm.Source install -g @bitkyc08/nexcode");
    expect(script).toContain("$LASTEXITCODE");
    expect(script).toContain("Get-Command nxc.cmd");
    expect(script).toContain("Get-Command nxc");
    expect(script).toContain("& $nxc.Source help");
    expect(script).not.toContain("bun install -g @bitkyc08/nexcode");
    expect(script).not.toContain("bun.sh/install.ps1");
  });

  test("Node launcher handles npm self-update before starting Bun", async () => {
    const launcher = await readText("bin/nxc.mjs");
  ...
  test.skipIf(process.platform === "win32")(
    ...
    () => {
      const fixtureRoot = mkdtempSync(join(tmpdir(), "nxc-package-modes-"));
      ...
      try {
        ...
        writeFileSync(join(packageRoot, "bin", "nxc.mjs"), "launcher");
        ...
        chmodSync(join(packageRoot, "bin", "nxc.mjs"), 0o600);
        ...
        expect(mode(join(packageRoot, "bin", "nxc.mjs"))).toBe(0o755);
  ...
  test("package mode normalization does not traverse a linked output root", () => {
    const fixtureRoot = mkdtempSync(join(tmpdir(), "nxc-package-root-link-"));
    const packageRoot = join(fixtureRoot, "package");
