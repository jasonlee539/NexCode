...
beforeEach(() => {
  ...
  testRoot = mkdtempSync(join(tmpdir(), "nxc-config-generation-"));
...
test("observe-only generation reports a missing database without creating or chmodding paths", () => {
  ...
  const absentHome = join(absentParent, "nexcode-home");
  const rootBefore = statSync(testRoot, { bigint: true });
  process.env.CODEX_HOME = absentHome;
  process.env.NEXCODE_HOME = absentHome;
  ...
  process.env.NEXCODE_HOME = existingHome;
  ...
  const children = ids.map((id, index) => {
    ...
    return Bun.spawn([process.execPath, "--eval", generationGuardRaceScript], {
      ...
      env: {
        ...process.env,
        HOME: testRoot,
        NEXCODE_HOME: testRoot,
        NXC_TEST_PAYLOAD: JSON.stringify(payload),
      },
      stdin: "ignore",
      stdout: "pipe",
      stderr: "pipe",
...
test("busy and unavailable databases return typed outcomes instead of throwing", () => {
  ...
  process.env.NEXCODE_HOME = unavailableHome;
