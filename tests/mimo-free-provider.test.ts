...
function minimalRequest(model = "mimo-auto"): NxcParsedRequest {
  return {
    modelId: model,
    stream: false,
...
describe("mimo-free client id", () => {
  ...
  test("random UUID persisted under NEXCODE_HOME and stable across cache resets", () => {
    const home = mkdtempSync(joinPath(tmpdir(), "nxc-mimo-id-"));
  ...
  test("client id is not derived from machine attributes (two homes differ)", () => {
    const homeA = mkdtempSync(joinPath(tmpdir(), "nxc-mimo-a-"));
    const homeB = mkdtempSync(joinPath(tmpdir(), "nxc-mimo-b-"));
    const prevHome = process.env["NEXCODE_HOME"];
    try {
      process.env["NEXCODE_HOME"] = homeA;
      resetMimoClientIdCache();
      const idA = getMimoClientId();
      process.env["NEXCODE_HOME"] = homeB;
      resetMimoClientIdCache();
...
describe("mimo-free auth retry predicate", () => {
  ...
  function adapterForRetry(): ReturnType<typeof createMimoFreeAdapter> {
    const provider: NxcProviderConfig = providerConfigSeed(PROVIDER_REGISTRY.find(e => e.id === "mimo-free")!);
