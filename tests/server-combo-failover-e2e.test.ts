...
import type { AdapterEvent, NxcConfig, NxcProviderConfig, NxcProviderContinuationState } from "../src/types";
...
mock.module("../src/server/adapter-resolve", () => ({
  ...actualResolver,
  resolveAdapter(provider: NxcProviderConfig, cacheRetention?: "none" | "short" | "long") {
...
beforeEach(() => {
  ...
  previousCursorToken = process.env.NEXCODE_CURSOR_TEST_TOKEN;
  delete process.env.NEXCODE_CURSOR_TEST_TOKEN;
  isolatedCodexHome = installIsolatedCodexHome("nxc-combo-030-codex-");
  testDir = mkdtempSync(join(tmpdir(), "nxc-combo-030-"));
...
afterEach(async () => {
  ...
  try {
    ...
    if (previousCursorToken === undefined) delete process.env.NEXCODE_CURSOR_TEST_TOKEN;
    else process.env.NEXCODE_CURSOR_TEST_TOKEN = previousCursorToken;
...
function provider(
  ...
  url: string,
  apiKey: string,
  extra: Partial<NxcProviderConfig> = {},
): NxcProviderConfig {
  return {
    adapter,
    baseUrl: url,
...
function comboConfig(
  providers: NxcConfig["providers"],
  targets = Object.keys(providers).map((name, index) => ({ provider: name, model: `m${index + 1}` })),
  extra: Partial<NonNullable<NxcConfig["combos"]>[string]> = {},
): NxcConfig {
  return {
    port: 0,
...
async function latestAttemptReceipts(config: NxcConfig) {
...
describe("server combo failover 030 activation matrix", () => {
  ...
  test("combo keeps an explicitly empty provider-state snapshot across failover", async () => {
    ...
    expect(ownedState?.__nxcOwner?.providerName).toBe("b");
  ...
  test("combo response state deep-merges provider-private payloads generically", async () => {
    ...
    expect(stored?.__nxcOwner?.providerName).toBe("a");
  });
  ...
  test("combo rejects malformed provider-continuation owner metadata", async () => {
    ...
    rememberResponseState(
      ...
      } as unknown as NxcProviderContinuationState,
    );
  ...
  test("backup 401 never triggers xAI refresh or receives an xAI bearer", async () => {
    for (const includeC of [false, true]) {
      ...
      const providers: NxcConfig["providers"] = {
        xai: { adapter: "openai-chat", baseUrl: "https://api.x.ai/v1", authMode: "oauth" },
  ...
  test("existence gate preserves physical combo provider and unknown routeModel 404", async () => {
    ...
    const physicalConfig: NxcConfig = {
      port: 0,
...
describe("cursor conversation continuity across store:false chains", () => {
  ...
  async function postCursor(config: NxcConfig, raw: Record<string, unknown>): Promise<Response> {
    return handleResponses(new Request("http://localhost/v1/responses", {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  function cursorConfig(): NxcConfig {
    return {
      port: 0,
