 ...
 * SENTINEL home handed over via NXC_REAL_HOME, exercising the same capture path the
 ...
 * yields `/D:/a/nexcode/nexcode/`, whose leading slash makes the directory
...
/**
 * Run a probe in a child process so we control NXC_REAL_HOME at STARTUP — the guard
...
function runProbe(source: string, env: Record<string, string | undefined>): { code: number; stdout: string; stderr: string } {
  const dir = mkdtempSync(join(tmpdir(), "nxc-guard-probe-"));
...
function sentinelHome(): { realHome: string; nexcodeHome: string } {
  const realHome = mkdtempSync(join(tmpdir(), "nxc-sentinel-home-"));
  const nexcodeHome = join(realHome, ".nexcode");
  mkdirSync(nexcodeHome, { recursive: true });
  return { realHome, nexcodeHome };
}
 ...
 */
const canSymlink = (() => {
  const probeDir = mkdtempSync(join(tmpdir(), "nxc-home-guard-symlink-probe-"));
  try {
  ...
  test("armed + the protected home: all three writers throw", () => {
    const { realHome, nexcodeHome } = sentinelHome();
    const probe = runProbe(`
      ...
      const threw: string[] = [];
      const REFUSAL = "refusing to write the real NexCode home";
    ...
    `, { NXC_TEST_HOME_GUARD: "1", NXC_REAL_HOME: realHome, NEXCODE_HOME: nexcodeHome });
    ...
    expect(() => readFileSync(join(nexcodeHome, "config.json"))).toThrow();
    expect(() => readFileSync(join(nexcodeHome, "auth.json"))).toThrow();
    expect(() => readFileSync(join(nexcodeHome, "codex-accounts.json"))).toThrow();
  });
  ...
  test.skipIf(!canSymlink)("armed + a symlink escaping a temp home into the protected home: refused", () => {
    ...
    const { realHome, nexcodeHome } = sentinelHome();
    const protectedFile = join(nexcodeHome, "config.json");
    writeFileSync(protectedFile, '{"sentinel":true}', "utf8");
    const dir = mkdtempSync(join(tmpdir(), "nxc-escape-home-"));
    ...
    `, { NXC_TEST_HOME_GUARD: "1", NXC_REAL_HOME: realHome, NEXCODE_HOME: dir });
  ...
  test("armed + an unregistered temp home: writers succeed", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "nxc-plain-home-"));
    const probe = runProbe(`
      import { saveConfig } from "${REPO_ROOT_URL}src/config";
      saveConfig({ providers: {}, defaultProvider: "openai", port: 10100 } as never);
      console.log("wrote");
    `, { NXC_TEST_HOME_GUARD: "1", NXC_REAL_HOME: join(tmpdir(), "nxc-nonexistent-real-home"), NEXCODE_HOME: dir });
  ...
  test.skipIf(!canSymlink)("armed + a first write beneath a symlinked PARENT escaping into the protected home: refused", () => {
    ...
    const { realHome, nexcodeHome } = sentinelHome();
    const dir = mkdtempSync(join(tmpdir(), "nxc-parent-escape-"));
    const linkDir = join(dir, "home-link");
    symlinkSync(nexcodeHome, linkDir);
    const modeBefore = statSync(nexcodeHome).mode;
    ...
    `, { NXC_TEST_HOME_GUARD: "1", NXC_REAL_HOME: realHome, NEXCODE_HOME: linkDir });
    ...
    expect(() => readFileSync(join(nexcodeHome, "never-created.json"))).toThrow();
    expect(() => readFileSync(join(nexcodeHome, "nxc.pid"))).toThrow();
    // The protected directory's mode is untouched by the refused write.
    expect(statSync(nexcodeHome).mode).toBe(modeBefore);
  });
  ...
  test("disarmed: the protected home is allowed (production stays inert)", () => {
    ...
    const probe = runProbe(`
      ...
      saveConfig({ providers: {}, defaultProvider: "openai", port: 10100 } as never);
      console.log("wrote");
    `, { NXC_TEST_HOME_GUARD: undefined, NXC_REAL_HOME: realHome, NEXCODE_HOME: nexcodeHome });

    expect(probe.stdout).toContain("wrote");
  });

  test("the protected path comes from NXC_REAL_HOME, not the sandboxed HOME", () => {
    ...
    const decoyHome = mkdtempSync(join(tmpdir(), "nxc-decoy-home-"));
    const probe = runProbe(`
      import { protectedHomeForTests } from "${REPO_ROOT_URL}src/lib/test-home-guard";
      console.log(protectedHomeForTests());
    `, { NXC_TEST_HOME_GUARD: "1", NXC_REAL_HOME: realHome, HOME: decoyHome });

    expect(probe.stdout).toContain(".nexcode");
    expect(probe.stdout).not.toContain("nxc-decoy-home-");
  });

  test.skipIf(!canSymlink)("a symlink pointing at the protected home is rejected", () => {
    const { realHome, nexcodeHome } = sentinelHome();
    const linkDir = mkdtempSync(join(tmpdir(), "nxc-symlink-"));
    const link = join(linkDir, "looks-like-temp");
    symlinkSync(nexcodeHome, link);
    const probe = runProbe(`
    ...
    `, { NXC_TEST_HOME_GUARD: "1", NXC_REAL_HOME: realHome });
  ...
  test("/var and /private/var spellings of one path agree", () => {
    ...
    const probe = runProbe(`
      ...
      for (const path of [${JSON.stringify(join(realHome, ".nexcode"))}, ${JSON.stringify(join(aliased, ".nexcode"))}]) {
  ...
  test("the preload sandboxes this very process", () => {
    ...
    expect(process.env.NXC_TEST_PRELOAD_PID).toBe(String(process.pid));
    // NEXCODE_HOME is redirected for this process, so ordinary resolution sandboxes.
    expect(process.env.NEXCODE_HOME).toBeDefined();
    expect(process.env.NEXCODE_HOME).not.toBe(protectedHomeForTests());
