...
const origAdminToken = process.env.NEXCODE_ADMIN_AUTH_TOKEN;
let tmp: string;
...
beforeEach(() => {
  ...
  process.env.NEXCODE_HOME = join(tmp, "nxc");
  clearCodexUpstreamHealth();
});
...
afterEach(() => {
  ...
  if (origNxcHome === undefined) delete process.env.NEXCODE_HOME;
  else process.env.NEXCODE_HOME = origNxcHome;
  if (origAdminToken === undefined) delete process.env.NEXCODE_ADMIN_AUTH_TOKEN;
  else process.env.NEXCODE_ADMIN_AUTH_TOKEN = origAdminToken;
  clearCodexUpstreamHealth();
...
describe("collectOAuthHealthEntries", () => {
  test("projects needsReauth account with reauth action", async () => {
    ...
    expect(entry).toEqual({
      ...
      action: "run `nxc login kimi`",
    });
  });

  test("Codex reauth action points at the dashboard pool, not nxc login codex", () => {
    ...
    expect(entry!.action).not.toContain("nxc login codex");
  });
...
describe("collectOAuthHealthEntriesForCli", () => {
  test("uses management API Codex health and does not read CLI process maps", async () => {
    markCodexAccountNeedsReauth(MAIN_CODEX_ACCOUNT_ID);
    process.env.NEXCODE_ADMIN_AUTH_TOKEN = "nxc-admin-health-test";
    ...
    const report = await collectOAuthHealthEntriesForCli(Date.now(), {
      ...
      fetchImpl: async (_input, init) => {
        ...
        apiKey = headers.get("x-nexcode-api-key");
...
describe("getCodexAccountHealthSnapshot", () => {
  test("exposes active cooldown source without changing write policy", () => {
    const config = { providers: {} } as NxcConfig;
