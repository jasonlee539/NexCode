...
const ADMISSION_SECRET = "nxc_data_2132secret";
 ...
 */
function mixedConfig(): NxcConfig {
  return {
    port: 0,
    hostname: "0.0.0.0",
...
beforeEach(() => {
  nxcHome = mkdtempSync(join(tmpdir(), "nxc-2132-home-"));
  codexHome = mkdtempSync(join(tmpdir(), "nxc-2132-codex-"));
...
describe("an admission bearer never reaches a canonical ChatGPT transport, whatever the row is named", () => {
  function customNamedCanonicalConfig(): NxcConfig {
