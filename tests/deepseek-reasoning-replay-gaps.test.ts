...
/**
 * Regression coverage for nexcode issue #950: OpenCode Go DeepSeek V4 Flash
...
function wireFor(
  input: unknown[],
  scope: NxcReasoningReplayScopeRef | null = REPLAY_SCOPE,
  ...
  const req = createOpenAIChatAdapter(route.provider).buildRequest(parsed as NxcParsedRequest);
...
describe("issue #950 — tool-call reasoning replay invariant (openai-chat wire)", () => {
  ...
  test("documented non-bug: opaque encrypted-only reasoning degrades to the placeholder, not invented plaintext", () => {
    // Native (non-nxcr1) encrypted reasoning has no readable text; the parser
...
describe("issue #950 — reasoning replay cache bounds", () => {
  ...
  test("unscoped entries are rejected instead of sharing a process-wide namespace", () => {
    ...
    expect(peekReasoningForCallRaw(
      "call_collision",
      "global" as unknown as NxcReasoningReplayScopeRef,
    )).toBeUndefined();
  });
