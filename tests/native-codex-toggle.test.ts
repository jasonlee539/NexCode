 ...
 * turning Codex OFF is not `nxc stop`: the proxy keeps serving, `/healthz` keeps
...
async function put(config: NxcConfig, body: unknown, deps?: ManagementApiDeps) {
...
beforeEach(() => {
  ...
  fixtureRoot = mkdtempSync(join(tmpdir(), "nxc-codex-toggle-"));
...
describe("turning Codex off", () => {
  test("persists the decision so it survives the next start", async () => {
    ...
    // `nxc start` re-syncs over it, which is the defect this phase exists for.
