...
beforeEach(() => {
  ...
  testDir = mkdtempSync(join(tmpdir(), "nxc-overlay-live-"));
...
afterEach(() => {
  stopUserCostOverlayReconciler();
  resetUserCostOverlayReconcilerForTests();
  refreshUserCostOverlays({ providers: {} } as unknown as NxcConfig);
  resetPreservedDiskOnlyProvidersForTests();
...
describe("cross-process user cost overlay reconciliation", () => {
  test("a CLI-process saveConfig edit becomes live in the running server registry", async () => {
    ...
    // Separate writer process: exactly what `nxc config set` does — a fresh
    // module instance calling saveConfig() under the same NEXCODE_HOME.
  ...
  test("a direct config.json edit becomes live without running saveConfig", async () => {
    ...
    // Separate writer process: raw file edit, no nxc code at all.
    const { exitCode, stderr } = await runChild(`
      import { readFileSync, writeFileSync } from "node:fs";
      import { join } from "node:path";
      const path = join(process.env.NEXCODE_HOME, "config.json");
