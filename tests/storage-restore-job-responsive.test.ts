...
beforeEach(async () => {
  ...
  previousCleanupTestHooks = process.env.NEXCODE_CLEANUP_TEST_HOOKS;
  process.env.NEXCODE_CLEANUP_TEST_HOOKS = "1";
  // Join leftover Workers before allocating homes / mutating NEXCODE_HOME.
  stopStorageCleanupScheduler();
  await resetRestoreTrashJobForTestsAsync();
  await drainStorageWorkers();
  isolatedCodexHome = installIsolatedCodexHome("nxc-restore-job-responsive-codex-");
  testDir = mkdtempSync(join(tmpdir(), "nxc-restore-job-responsive-"));
...
afterEach(async () => {
  ...
  if (previousCleanupTestHooks === undefined) delete process.env.NEXCODE_CLEANUP_TEST_HOOKS;
  else process.env.NEXCODE_CLEANUP_TEST_HOOKS = previousCleanupTestHooks;
...
describe("storage trash restore job responsiveness", () => {
  async function withTestStreamRoute(
    ...
    const previousHooksEnv = process.env.NEXCODE_CLEANUP_TEST_HOOKS;
    setup();
    ...
    try {
      ...
      try {
        ...
        setRestoreTrashJobTestHooks(null);
        if (previousHooksEnv === undefined) delete process.env.NEXCODE_CLEANUP_TEST_HOOKS;
        else process.env.NEXCODE_CLEANUP_TEST_HOOKS = previousHooksEnv;
      }
    }
  }

  test("test-stream route is absent without NEXCODE_CLEANUP_TEST_HOOKS", async () => {
    await withTestStreamRoute(
      () => {
        delete process.env.NEXCODE_CLEANUP_TEST_HOOKS;
