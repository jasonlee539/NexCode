...
function togetherConfig(apiKey = "together-authority-secret"): NxcConfig {
...
describe("catalog gather discovery-policy authority", () => {
  ...
  test("a provider field outside the legacy fingerprint cannot join another admission's flight", async () => {
    ...
    const withEfforts = (efforts: readonly string[]): NxcConfig => {
      const config = togetherConfig();
  ...
  test("different combo retention sets cannot join another admission's flight (NXC-111)", async () => {
