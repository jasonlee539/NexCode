...
describe("startup star prompt", () => {
  ...
  test("nxc start performs sync/injection without an external promotion prompt", async () => {
    ...
    expect(cli).not.toContain("await maybeShowStarPrompt()");
  ...
  test("an agent driving nxc is told to ask the user instead of answering", async () => {
    const prompt = await readText("src/cli/star-prompt.ts");
  ...
  test("the relayed question is a real choice asked once, never decided by silence", async () => {
    ...
    expect(prompt).toContain("at most once per nexcode version");
  });
  ...
  test("the management star endpoint refuses agent callers too", async () => {
    ...
    expect(routes).not.toMatch(/hasBrowserSessionEvidence[\s\S]*?headers\.get\("x-nexcode-csrf-token"\)/);
  });

  test("the independent build documents that it never requests external promotion", async () => {
    const install = await readText("AGENTS_INSTALL.md");
    const readme = await readText("README.md");

    const normalizedInstall = install.replace(/\s+/g, " ");
    expect(normalizedInstall).toContain("does not ask an agent or user to star");
    expect(normalizedInstall).toContain("must never be interpreted as permission");
    expect(normalizedInstall).toContain("OAuth and provider-account actions remain user decisions");
    expect(readme).not.toContain("POST /api/github/star");
    expect(readme).not.toContain("lidge-jun/nexcode");
  });
  ...
  test("nxc init offers the Codex autostart shim by default", async () => {
  ...
  test("nxc service install completes without an external promotion prompt", async () => {
  ...
  test("the service-installed proxy still cannot prompt", async () => {
    ...
    // The supervised child always carries NXC_SERVICE=1; that guard is what makes
    // the `service install` call site the only interactive moment for those users.
    expect(prompt).toContain("if (process.env.NXC_SERVICE ||");
  });
});
