...
import type { NxcParsedRequest, NxcProviderConfig, NxcTool } from "../src/types";
...
async function toolsOf(tools: NxcTool[]): Promise<Array<{ name: string; input_schema: Record<string, unknown> }>> {
...
async function toolSchema(parameters: unknown): Promise<Record<string, unknown>> {
  const [tool] = await toolsOf([{ name: "sample_tool", description: "Sample", parameters } as NxcTool]);
