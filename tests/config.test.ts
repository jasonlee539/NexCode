...
import {
  ...
  removePid,
  removeRuntimePort,
  nxcStartProcessCacheSizeForTests,
  setNxcStartProcessCacheForTests,
 ...
 */
const canSymlink = (() => {
  const dir = mkdtempSync(join(tmpdir(), "nxc-config-symlink-probe-"));
  try {
...
beforeEach(() => {
  testDir = mkdtempSync(join(tmpdir(), "nxc-config-"));
  process.env.NEXCODE_HOME = testDir;
});
...
describe("nexcode config defaults", () => {
  ...
  test("an invalid persisted Claude Code subagent effort is ignored without wiping config or logging its value", () => {
    ...
    writeConfig({
      ...
      apiKeys: [{ id: "key-1", name: "default", key: "nxc_persisted", createdAt: "2026-07-28T00:00:00.000Z" }],
    ...
    expect(config).toMatchObject({
      ...
      apiKeys: [expect.objectContaining({ id: "key-1", key: "nxc_persisted" })],
    });
  ...
  test("loads valid config from NEXCODE_HOME", () => {
    writeConfig({
      port: 12345,
      providers: {
  ...
  test("resolves relative NEXCODE_HOME once to an absolute config directory", () => {
    const parent = mkdtempSync(join(tmpdir(), "nxc-config-parent-"));
    const oldCwd = process.cwd();
    try {
      process.env.NEXCODE_HOME = "relative-home";
      ...
      expect(getPidPath()).toBe(join(expectedConfigDir, "nxc.pid"));
    } finally {
  ...
  test("uses the default home when NEXCODE_HOME is unset", () => {
    delete process.env.NEXCODE_HOME;

    expect(getConfigPath()).toBe(join(homedir(), ".nexcode", "config.json"));
    expect(getPidPath()).toBe(join(homedir(), ".nexcode", "nxc.pid"));
  });
  ...
  test("backs up invalid JSON config before falling back to defaults", () => {
    ...
    try {
      ...
      expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining("Could not load nexcode config"));
    } finally {
  ...
  test("recognizes nexcode start command lines", () => {
    expect(isNxcStartCommandLine('bun run src/cli.ts start')).toBe(true);
    expect(isNxcStartCommandLine('"C:/tools/bun/bin/bun.exe" "run" "src/cli/index.ts" "start"')).toBe(true);
    expect(isNxcStartCommandLine('bun C:/tools/bun/install/global/node_modules/@bitkyc08/nexcode/src/cli.ts start')).toBe(true);
    // npm's in-place rename during `npm install -g` (Windows service wrapper respawn mid-update).
    expect(isNxcStartCommandLine(
      'bun C:/nvm/node_modules/@bitkyc08/.nexcode-1JejBqbZ/src/cli/index.ts start --port 10100',
    )).toBe(true);
    expect(isNxcStartCommandLine("nexcode start")).toBe(true);

    expect(isNxcStartCommandLine("bun run src/cli.ts status")).toBe(false);
    expect(isNxcStartCommandLine("bun test C:/work/nexcode/tests/config.test.ts")).toBe(false);
    expect(isNxcStartCommandLine("notepad.exe")).toBe(false);
  });
  ...
  test("pid validation does not execute ps from PATH", () => {
    ...
    expect(nxcStartProcessCacheSizeForTests()).toBe(0);
  });
  ...
  test("pid validation selects only trusted Windows process probes", () => {
    ...
    try {
      ...
      setProcessCommandLineExecForTests((executable) => {
        ...
        if (executable === trustedWmic) return "CommandLine=nxc start\r\n";
      ...
      setProcessCommandLineExecForTests((executable) => {
        ...
        if (executable === trustedPowerShell) return "nxc start\n";
        throw new Error(`unexpected process probe: ${executable}`);
      });
      expect(readPid()).toBe(process.pid);
...
describe("config.ts – atomic writes preserve symlinked destinations", () => {
  ...
  test.skipIf(!canSymlink)("no temp file is left beside the link or its target", () => {
    ...
    expect(readdirSync(repoDir).filter(name => name.includes(".nxc."))).toEqual([]);
    expect(readdirSync(testDir).filter(name => name.includes(".nxc."))).toEqual([]);
  });
