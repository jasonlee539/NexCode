...
describe("Cursor live transport", () => {
  ...
  test("fails before network when no Cursor credential is configured", () => {
    const prev = process.env.NEXCODE_CURSOR_TEST_TOKEN;
    ...
    try {
      ...
      if (prev === undefined) delete process.env.NEXCODE_CURSOR_TEST_TOKEN;
      else process.env.NEXCODE_CURSOR_TEST_TOKEN = prev;
    }
  });
