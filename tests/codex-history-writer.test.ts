...
test("every history writer refuses a permit that is not live", () => {
  const { codexHome, target } = makeTarget("nxc-history-writer-dead-");
...
test("a live permit for another state database cannot write this one", () => {
  const first = makeTarget("nxc-history-writer-a-");
  const second = makeTarget("nxc-history-writer-b-");
...
test("a writer holding a live permit performs the real transition", () => {
  const { codexHome, target } = makeTarget("nxc-history-writer-live-");
  ...
  db.run(
    "INSERT INTO threads (id, rollout_path, model_provider, source, has_user_event, first_user_message) VALUES (?, ?, 'nexcode', 'exec', 1, 'hi')",
    ["thread-1", rollout],
  );
  db.close();
