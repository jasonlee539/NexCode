...
import type { NxcConfig, NxcParsedRequest } from "../src/types";

const savedHome = process.env.NEXCODE_HOME;
const savedCodexHome = process.env.CODEX_HOME;
let tempHome: string | null = null;
let tempCodexHome: string | null = null;

afterEach(() => {
  if (savedHome === undefined) delete process.env.NEXCODE_HOME;
  else process.env.NEXCODE_HOME = savedHome;
...
function makeConfig(overrides: Partial<NxcConfig> = {}): NxcConfig {
  return { port: 10100, providers: {}, defaultProvider: "openai", ...overrides } as NxcConfig;
}

function makeParsed(reasoning?: string): NxcParsedRequest {
  return {
    modelId: "gpt-5.6-sol",
...
describe("supportedLadderFor (real routeModel routes)", () => {
  ...
  test("custom key-mode providers cannot capture a bare native-looking OpenAI id", () => {
    tempCodexHome = mkdtempSync(join(tmpdir(), "nxc-effort-catalog-"));
    process.env.CODEX_HOME = tempCodexHome;
    writeFileSync(join(tempCodexHome, "nexcode-catalog.json"), JSON.stringify({
...
describe("effortCapAppliesTo (caps are a v2-feature gate)", () => {
  function parsedWithTools(tools: Array<{ name: string; namespace?: string }>, reasoning?: string): NxcParsedRequest {
    return {
      modelId: "gpt-5.6-sol",
      context: {
...
describe("/api/effort-caps", () => {
  function isolatedHome(): void {
    tempHome = mkdtempSync(join(tmpdir(), "nxc-effort-caps-"));
    process.env.NEXCODE_HOME = tempHome;
  }

  async function put(config: NxcConfig, body: unknown): Promise<Response> {
