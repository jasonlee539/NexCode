...
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), "nxc-probe-hardening-"));
  configDir = join(home, "custom-nexcode");
...
function writeSchedulerChain(
  ...
  const launcher = join(dir, "nexcode-service-launcher.vbs");
  const taskXml = join(dir, "nexcode-service-task.xml");
...
function writeWinsw(
  ...
  writeFileSync(join(winswDir, "nexcode-proxy-native.exe"), "not-executable-test-placeholder");
  writeFileSync(join(winswDir, "nexcode-proxy-native.xml"), [
    ...
    `  <env name="NEXCODE_HOME" value="${xmlEscape(nexcodeHome)}"/>`,
...
describe("Windows ownership probe hardening regressions", () => {
  test("registered CP949 task XML preserves a Korean profile path", () => {
    const koreanConfigDir = join(home, "한글", ".nexcode");
    ...
    expect(result.claims[0].homes).toEqual({ codexHome, nexcodeHome: koreanConfigDir });
  });

  test("ownership inspects the effective current NEXCODE_HOME without an injected configDir", () => {
    ...
    const result = inspectNativeCodexOwnership({
      ...
      currentHomes: { codexHome: currentCodexHome, nexcodeHome: configDir },
    });
  ...
  test("the default WinSW registration probe uses bounded trusted sc.exe instead of executing the WinSW binary", () => {
    ...
    const runRaw: RawProbeRunner = (file, args) => {
      calls.push({ file, args });
      if (file.toLowerCase().endsWith("sc.exe")) return raw(0, "SERVICE_NAME: nexcode-proxy-native");
    ...
    expect(calls.some(call => call.file.toLowerCase().endsWith("sc.exe")
      && call.args[0] === "query"
      && call.args[1] === "nexcode-proxy-native")).toBe(true);
  ...
  test("a scheduler definition cannot make the probe follow a launcher outside the generated config chain", () => {
    ...
    writeFileSync(join(configDir, "nexcode-service-task.xml"), schedulerXml(foreign.launcher));
    const calls: Array<{ file: string; args: readonly string[] }> = [];
    const runRaw = taskAbsentRunner(calls);
  ...
  test("legacy v1 service state means scheduler and cannot authorize a WinSW manager", () => {
    ...
    const result = inspectNativeCodexOwnership({
      ...
      winswStatus: () => "started",
      statePaths: [statePath],
      currentHomes: { codexHome, nexcodeHome: configDir },
    });
