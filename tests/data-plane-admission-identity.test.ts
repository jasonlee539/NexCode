...
function remoteConfig(): NxcConfig {
  return {
    ...
    apiKeys: [
      { id: "first-key", name: "first", key: "nxc_data_firstsecret", createdAt: "2026-07-31T00:00:00.000Z" },
      { id: "second-key", name: "second", key: "nxc_data_secondsecret", createdAt: "2026-07-31T00:00:00.000Z" },
    ],
  };
}

function loopbackConfig(): NxcConfig {
...
describe("resolveDataPlaneAdmissionSecret", () => {
  test("names the configured key that actually matched", () => {
    ...
    expect(resolveDataPlaneAdmissionSecret("nxc_data_firstsecret", config)).toEqual({
      kind: "configured",
      keyId: "first-key",
      source: "dedicated",
    });
  ...
  test("resolves the SECOND key to its own id, not the first", () => {
    ...
    expect(resolveDataPlaneAdmissionSecret("nxc_data_secondsecret", config)).toEqual({
      kind: "configured",
      keyId: "second-key",
      source: "dedicated",
    });
  ...
  test("the environment token has no configured key to name", () => {
    process.env.NEXCODE_API_AUTH_TOKEN = "env-secret";
  ...
  test("a key that only shares a prefix does not match", () => {
    expect(resolveDataPlaneAdmissionSecret("nxc_data_first", remoteConfig())).toBeNull();
  });
});

describe("no admission decision changed", () => {
  test.each([
    ["configured key", "nxc_data_firstsecret", true],
    ["second configured key", "nxc_data_secondsecret", true],
  ...
  test("the never-forward-upstream guard still recognizes a configured key", () => {
    const config = remoteConfig();
    expect(isProxyAdmissionSecret("nxc_data_firstsecret", config)).toBe(true);
    // Legacy 40-hex shape and the other proxy prefixes are unaffected.
    expect(isProxyAdmissionSecret(`nxc_${"a".repeat(40)}`, config)).toBe(true);
...
describe("the two wrappers still differ", () => {
  test("bearer admission is accepted on both paths and names its source (#1686)", () => {
    const config = remoteConfig();
    const bearer = request({ authorization: "Bearer nxc_data_firstsecret" });
    ...
    expect(resolveResponsesApiAuth(request({ authorization: "Bearer nxc_data_firstsecret" }), config))
      .toEqual({ kind: "configured", keyId: "first-key", source: "bearer" });
    expect(requireResponsesApiAuth(request({ authorization: "Bearer nxc_data_firstsecret" }), config)).toBeNull();
  });
  ...
  test("the dedicated header still wins over a bearer (#1686)", () => {
    const config = remoteConfig();
    const both = request({
      "x-nexcode-api-key": "nxc_data_secondsecret",
      authorization: "Bearer nxc_data_firstsecret",
    });
  ...
  test("x-api-key is accepted only by the broad path", () => {
    const config = remoteConfig();
    expect(resolveApiAuth(request({ "x-api-key": "nxc_data_firstsecret" }), config)).not.toBeNull();
    expect(resolveResponsesApiAuth(request({ "x-api-key": "nxc_data_firstsecret" }), config)).toBeNull();
  });

  test("the dedicated header works on both", () => {
    const config = remoteConfig();
    const dedicated = () => request({ "x-nexcode-api-key": "nxc_data_secondsecret" });
...
describe("the Responses WebSocket handshake", () => {
  test("opens for a configured key", async () => {
    const home = mkdtempSync(join(tmpdir(), "nxc-admission-ws-"));
    const previousHome = process.env.NEXCODE_HOME;
    process.env.NEXCODE_HOME = home;
    process.env.NEXCODE_ADMIN_AUTH_TOKEN = "admin-secret-for-ws";
    const config = remoteConfig();
    ...
    try {
      ...
      const opened = await new Promise<boolean>(resolve => {
        const socket = new WebSocket(target, {
          headers: { "X-NexCode-API-Key": "nxc_data_secondsecret" },
