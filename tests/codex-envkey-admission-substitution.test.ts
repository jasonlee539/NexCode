...
const previousNxcHome = process.env.NEXCODE_HOME;
...
const ADMISSION_SECRET = "nxc_data_envkeysecret";
...
function directConfig(): NxcConfig {
  return {
    port: 0,
...
beforeEach(() => {
  nxcHome = mkdtempSync(join(tmpdir(), "nxc-1686-home-"));
  codexHome = mkdtempSync(join(tmpdir(), "nxc-1686-codex-"));
...
afterEach(() => {
  ...
  resetDebugSettingsForTests();
  resetDebugLogBufferForTests();
  if (previousNxcHome === undefined) delete process.env.NEXCODE_HOME;
  else process.env.NEXCODE_HOME = previousNxcHome;
  ...
  if (nxcHome) rmSync(nxcHome, { recursive: true, force: true });
  ...
  test("an admission bearer is served and the stored main credential goes upstream", async () => {
    ...
    try {
      ...
      const affinityLine = getDebugLogEntries()
        ...
        .find(line => line.startsWith("[nxc:codex:affinity] "));
      expect(affinityLine).toBeDefined();
      expect(JSON.parse(affinityLine!.slice("[nxc:codex:affinity] ".length))).toMatchObject({
        authKind: "main",
        accountMode: "direct",
