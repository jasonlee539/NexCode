...
describe("acceptSystemRestart", () => {
  ...
  test("NXC_SERVICE with non-viable Background Service uses detached start", async () => {
    const calls: string[] = [];
    let scheduled: (() => void | Promise<void>) | null = null;
    const prev = process.env.NXC_SERVICE;
    ...
    try {
      ...
      if (prev === undefined) delete process.env.NXC_SERVICE;
      else process.env.NXC_SERVICE = prev;
    }
  });

  test("NXC_SERVICE with viable Background Service exits 1 for supervisor respawn", async () => {
    const calls: string[] = [];
    let scheduled: (() => void | Promise<void>) | null = null;
    const prev = process.env.NXC_SERVICE;
    process.env.NXC_SERVICE = "1";
  ...
  test("spawn failure clears NXC_SERVICE so exit cleanup can restore fences", async () => {
    const calls: string[] = [];
    let scheduled: (() => void | Promise<void>) | null = null;
    const prev = process.env.NXC_SERVICE;
    process.env.NXC_SERVICE = "1";
    ...
    try {
      ...
      await scheduled!();
      expect(calls).toEqual(["drain", "start", "exit:1"]);
      expect(process.env.NXC_SERVICE).toBeUndefined();
    } finally {
      if (prev === undefined) delete process.env.NXC_SERVICE;
      else process.env.NXC_SERVICE = prev;
    }
