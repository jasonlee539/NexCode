...
const previousNxcDebug = process.env.NXC_DEBUG;
...
beforeEach(() => {
  isolatedCodexHome = installIsolatedCodexHome("nxc-issue-452-");
...
afterEach(() => {
  ...
  if (previousNxcDebug === undefined) delete process.env.NXC_DEBUG;
  else process.env.NXC_DEBUG = previousNxcDebug;
  resetDebugSettingsForTests();
  resetDebugLogBufferForTests();
...
describe("openai-chat provider debug (#452)", () => {
  test("buildRequest emits debugProviderDiagnostic when NXC_DEBUG=1", () => {
    ...
    expect(lines.some(line => line.includes("[nxc:openai-chat:request]"))).toBe(true);
  ...
  test("tenant-scoped baseUrl logs host only — account id never appears", () => {
    ...
    expect(joined).toContain("[nxc:openai-chat:request]");
