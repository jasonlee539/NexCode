...
beforeEach(() => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("nxc-claude-mgmt-");
  testDir = mkdtempSync(join(tmpdir(), "nxc-claude-mgmt-"));
...
test("GET /api/claude-code returns defaults + available + aliases", async () => {
  ...
  try {
    ...
    expect(d.aliases.some((a: { id: string }) => a.id === "claude-nxc-mock--test-model")).toBe(true);
...
test("PUT immediately restores generated agents after re-enable and roster changes", async () => {
  ...
  try {
    ...
    expect(readdirSync(agentsDir).some(name => name === "nxc-gpt-5-6-sol.md")).toBe(true);
    ...
    expect(readdirSync(agentsDir)).toEqual(["nxc-gpt-5-6-terra.md"]);
  } finally {
    await server.stop(true);
  }
});
...
test("Claude Desktop profile GET, PUT and apply round-trip four-family assignments", async () => {
  ...
  try {
    ...
    expect(result.path.startsWith(process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR!)).toBe(true);
