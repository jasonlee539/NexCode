...
beforeEach(() => {
  previousAclTimeout = process.env.NEXCODE_ACL_TIMEOUT_MS;
  delete process.env.NEXCODE_ACL_TIMEOUT_MS;
  testDir = mkdtempSync(join(tmpdir(), "nxc-acl-test-"));
});

afterEach(() => {
  if (previousAclTimeout === undefined) delete process.env.NEXCODE_ACL_TIMEOUT_MS;
  else process.env.NEXCODE_ACL_TIMEOUT_MS = previousAclTimeout;
...
describe("ephemeral harden success memo lifecycle", () => {
  test("forgetHardenedSecretPath releases only the actual temp and a second temp hardens again", () => {
    ...
    resetHardenedStateForTests();
    const tempA = join(testDir, "config.json.nxc.1.1.tmp");
    const tempB = join(testDir, "config.json.nxc.1.2.tmp");
...
describe("icacls failure paths (injected seams)", () => {
  ...
  test("all icacls steps share one deadline and a timed-out path is not retried this process", () => {
    ...
    process.env.NEXCODE_ACL_TIMEOUT_MS = "5000";
    const filePath = secretFile();
    let now = 0;
  ...
  test("NEXCODE_ACL_TIMEOUT_MS overrides the total budget with clamping", () => {
    const budgets: number[] = [];
    let now = 0;
    setNowForTests(() => now);
    setIcaclsRunnerForTests((_args, timeoutMs) => {
    ...
    const prev = process.env.NEXCODE_ACL_TIMEOUT_MS;
    try {
      process.env.NEXCODE_ACL_TIMEOUT_MS = "10000";
      ...
      budgets.length = 0;
      process.env.NEXCODE_ACL_TIMEOUT_MS = "50"; // below floor → clamped to 1000
      ...
      budgets.length = 0;
      process.env.NEXCODE_ACL_TIMEOUT_MS = "5000ms"; // malformed → default 30000 (#1156)
      ...
      if (prev === undefined) delete process.env.NEXCODE_ACL_TIMEOUT_MS;
      else process.env.NEXCODE_ACL_TIMEOUT_MS = prev;
    }
  });
...
describe("async hardenSecretPath (issue #612)", () => {
  ...
  test("timeoutMemoKey shares the timeout cache across distinct temp paths", async () => {
    ...
    const tempA = join(testDir, "responses-state.json.nxc.1.1.tmp");
    const tempB = join(testDir, "responses-state.json.nxc.1.2.tmp");
...
describe("ephemeral ACL memo release (#840 refinement)", () => {
  ...
  test("ephemeral release clears temp-keyed timeout memos in BOTH namespaces", () => {
    ...
    const previousUsername = process.env.USERNAME;
    process.env.USERNAME = "nxc-test-user";
    const tempA = join(testDir, "dest.nxc.1.1.tmp");
    const tempB = join(testDir, "dest.nxc.1.2.tmp");
    writeFileSync(tempA, "a", "utf-8");
    writeFileSync(tempB, "b", "utf-8");
    try {
...
describe("the production default hardener is reached, with the resolved platform", () => {
  ...
  const forcedWindows = async (
    ...
    const codexHome = mkdtempSync(join(tmpdir(), "nxc-default-harden-"));
    try {
      await run(codexHome);
    } finally {
      if (previousUsername === undefined) delete process.env.USERNAME;
...
describe("a required hardening failure stops the operation it protects", () => {
  ...
  const forcedWindowsFailure = async (
    ...
    const codexHome = mkdtempSync(join(tmpdir(), "nxc-harden-fail-"));
    try {
      await run(codexHome);
    } finally {
      if (previousUsername === undefined) delete process.env.USERNAME;
