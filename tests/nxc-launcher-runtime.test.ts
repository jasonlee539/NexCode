...
const BIN_NXC = join(import.meta.dir, "..", "bin", "nxc.mjs");
...
async function effectiveRuntime(override: string): Promise<string> {
  const root = mkdtempSync(join(tmpdir(), "nxc-launcher-runtime-"));
  ...
  try {
    port = await freePort();
    launcher = spawn("node", [BIN_NXC, "start", "--port", String(port)], {
      stdio: "ignore",
      windowsHide: true,
  ...
  if (port !== null) {
    ...
    if (lingeringProxy) {
      cleanupErrors.push(`NexCode proxy PID ${lingeringProxy.pid} remained on owned port ${port}`);
    }
  }
  try {
    removeTree(root);
...
function isolatedLauncherEnv(root: string, override: string): NodeJS.ProcessEnv {
  ...
  return {
    ...
    GROK_HOME: grokHome,
    NEXCODE_BUN_PATH: override,
  };
}

describe.skipIf(!nodeAvailable)("nxc npm launcher relative Bun override", () => {
  test("resolves a valid bare relative override before spawning", () => {
    const root = mkdtempSync(join(tmpdir(), "nxc-launcher-relative-"));
    try {
      ...
      const result = spawnSync("node", [BIN_NXC, "--version"], {
        cwd: root,
        encoding: "utf8",
        timeout: 30_000,
        windowsHide: true,
      ...
      expect(result.stderr).not.toContain("NEXCODE_BUN_PATH is missing");
    } finally {
      removeTree(root);
    }
  }, 60_000);
  ...
  test("warns without exposing the rejected override path before bundled fallback", () => {
    const root = mkdtempSync(join(tmpdir(), "nxc-launcher-invalid-"));
    try {
      ...
      expect(result.status).toBe(0);
      expect(result.stderr).toContain("NEXCODE_BUN_PATH is missing, unreadable, or not a complete Bun binary");
...
describe.skipIf(!runnable)("nxc npm launcher effective Bun runtime", () => {
  test("uses a valid NEXCODE_BUN_PATH for the actual proxy process", async () => {
    const root = mkdtempSync(join(tmpdir(), "nxc-launcher-runtime-copy-"));
    try {
  ...
  test("falls back to bundled Bun for a sub-1MB override stub", async () => {
    const root = mkdtempSync(join(tmpdir(), "nxc-launcher-runtime-stub-"));
    try {
