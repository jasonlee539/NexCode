...
// Full injectCodexConfig runs in a subprocess with isolated CODEX_HOME/NEXCODE_HOME so
// module-level path constants bind to the temp dirs (same pattern as codex-journal.test.ts).
function runInject(codexHome: string, nxcHome: string, configJson = "{}"): { stdout: string; status: number } {
  const script = `
    const { injectCodexConfig } = require("./src/codex/inject");
    injectCodexConfig(10100, JSON.parse(process.env.TEST_NXC_CONFIG)).then(r => {
      console.log(JSON.stringify(r));
    });
  `;
  const result = spawnSync(process.execPath, ["--eval", script], {
    cwd: repoRoot,
    env: { ...process.env, CODEX_HOME: codexHome, NEXCODE_HOME: nxcHome, TEST_NXC_CONFIG: configJson },
    encoding: "utf8",
...
function runRestore(codexHome: string, nxcHome: string): { stdout: string; status: number } {
  const script = `
  ...
  const result = spawnSync(process.execPath, ["--eval", script], {
    cwd: repoRoot,
    env: { ...process.env, CODEX_HOME: codexHome, NEXCODE_HOME: nxcHome },
    encoding: "utf8",
    timeout: SPAWN_BUDGET_MS - 5_000,
  });
  return { stdout: result.stdout?.trim() ?? "", status: result.status ?? 1 };
...
describe("injectCodexConfig integration (Design B)", () => {
  ...
  beforeEach(() => {
    codexHome = mkdtempSync(join(tmpdir(), "nxc-inject-codex-"));
    nxcHome = mkdtempSync(join(tmpdir(), "nxc-inject-home-"));
  });

  afterEach(() => {
    rmSync(codexHome, { recursive: true, force: true });
    rmSync(nxcHome, { recursive: true, force: true });
  });
  ...
  test("upgrade path: a legacy-injected config converts to the Design B form in one inject", () => {
    ...
    const r = runInject(codexHome, nxcHome);
    ...
    expect(config).toContain("# Auto-injected by nexcode");
    expect(config).not.toContain("[model_providers.nexcode]");
    expect(config).not.toContain('model_provider = "nexcode"');
    expect(config).toContain('model = "gpt-5.5"');
    // Exactly one marker survives (the Design B one) — no duplicate accumulation.
    expect(config.match(/Auto-injected by nexcode/g)?.length).toBe(1);
  });
  ...
  test("upgrade path: a non-loopback legacy env_http_headers config converts to env_key (#2073)", () => {
    writeFileSync(join(codexHome, "config.toml"), [
      ...
      'wire_api = "responses"',
      "requires_openai_auth = true",
      'env_http_headers = { "x-nexcode-api-key" = "NEXCODE_API_AUTH_TOKEN" }',
      "",
    ].join("\n"), "utf8");

    const r = runInject(codexHome, nxcHome, JSON.stringify({ hostname: "192.168.1.50" }));
    expect(r.status).toBe(0);
    expect(JSON.parse(r.stdout).success).toBe(true);

    const config = readFileSync(join(codexHome, "config.toml"), "utf8");
    expect(config).toContain('env_key = "NEXCODE_API_AUTH_TOKEN"');
    expect(config).not.toContain("env_http_headers");
    // Still exactly one provider block, no duplicate accumulation.
    expect(config.match(/\[model_providers\.nexcode]/g)?.length).toBe(1);
  });

  test("re-inject over a Design B config is idempotent", () => {
    writeFileSync(join(codexHome, "config.toml"), 'model = "gpt-5.5"\n', "utf8");

    expect(runInject(codexHome, nxcHome).status).toBe(0);
    ...
    expect(second.match(/Auto-injected by nexcode/g)?.length).toBe(1);
  ...
  test.each([
    ...
    (assignment) => {
      ...
      const result = runInject(codexHome, nxcHome);
      ...
      const profile = readFileSync(join(codexHome, "nexcode.config.toml"), "utf8");
  ...
  test("repairs an owned duplicate without replacing a commented user catalog", () => {
    ...
    writeFileSync(join(codexHome, "config.toml"), [
      userAssignment,
      'model_catalog_json = "nexcode-catalog.json"',
      "",
      "[features]",
      "fast_mode = true",
      "",
    ...
    expect(config).not.toContain('model_catalog_json = "nexcode-catalog.json"');
    expect(() => Bun.TOML.parse(config)).not.toThrow();
  ...
  test("removes a stale NexCode catalog assignment with a trailing comment", () => {
    writeFileSync(
      join(codexHome, "config.toml"),
      'model_catalog_json = "nexcode-catalog.json" # stale catalog\n',
      "utf8",
    );
  ...
  test("does not strip a catalog-shaped assignment from a user table", () => {
    const nestedAssignment = '"model_catalog_json" = "nexcode-catalog.json" # user table value';
    writeFileSync(join(codexHome, "config.toml"), [
      "[user_metadata]",
      nestedAssignment,
      "",
  ...
  test("fastMode=false forces fast_mode=false in both config and profile", () => {
    ...
    const r = runInject(codexHome, nxcHome, JSON.stringify({ fastMode: false }));
    expect(r.status).toBe(0);
    expect(JSON.parse(r.stdout).success).toBe(true);
  ...
  test("fastMode=true adds fast_mode=true to a config without a [features] table", () => {
    ...
    const r = runInject(codexHome, nxcHome, JSON.stringify({ fastMode: true }));
    expect(r.status).toBe(0);
    expect(JSON.parse(r.stdout).success).toBe(true);
  ...
  test("opt-in injects native subagent defaults, removes them when disabled, and restores the native config", () => {
    ...
    expect(runInject(codexHome, nxcHome, enabled).status).toBe(0);
    ...
    expect(runInject(codexHome, nxcHome, enabled).status).toBe(0);
    expect(runRestore(codexHome, nxcHome).status).toBe(0);
  ...
  test("opt-in preserves a user-owned native default pair and reports the conflict", () => {
    ...
    const result = runInject(codexHome, nxcHome, JSON.stringify({
      syncCodexSubagentDefaults: true,
      injectionModel: "gpt-5.6-sol",
      injectionEffort: "high",
    }));
  ...
  test("sync-disabled injection cleans managed-default residue before journaling and restore", () => {
    ...
    const injectedResult = runInject(codexHome, nxcHome, "{}");
    ...
    const restoredResult = runRestore(codexHome, nxcHome);
  ...
  test("ambiguous managed-default residue refuses injection without changing files", () => {
    ...
    expect(existsSync(join(codexHome, "nexcode-journal.json"))).toBe(false);
  });
  ...
  test("kept-user-base-url: reports routing NOT injected and leaves the user's override alone", () => {
    ...
    const r = runInject(codexHome, nxcHome, JSON.stringify({
      syncCodexSubagentDefaults: true,
      injectionModel: "gpt-5.6-sol",
      injectionEffort: "high",
    }));
    ...
    expect(result.message).not.toContain("All models now route through nexcode proxy");
    ...
    expect(config).not.toContain("# Auto-injected by nexcode\nopenai_base_url");
  ...
  test("external model provider stays byte-for-byte unchanged so its session history remains visible", () => {
    ...
    const profilePath = join(codexHome, "nexcode.config.toml");
    ...
    const journalPath = join(codexHome, "nexcode-journal.json");
  ...
  // root model_provider, a coexisting [model_providers.nexcode] table, and a [windows]
  ...
  test("#1090: CRLF Windows config with external deepseek provider and nexcode table stays byte-for-byte unchanged", () => {
    const original = [
  ...
  test("restoreNativeCodex removes a stale journal without changing external provider state", () => {
    ...
    const r = runRestore(codexHome, nxcHome);
    expect(r.status).toBe(0);
    const result = JSON.parse(r.stdout);
    expect(result.success).toBe(true);
  ...
  test("external provider guidance includes the admission header for non-loopback binds", () => {
    ...
    const r = runInject(codexHome, nxcHome, JSON.stringify({ hostname: "192.168.1.20" }));
    expect(r.status).toBe(0);
    const message = JSON.parse(r.stdout).message;
    expect(message).toContain("http://192.168.1.20:10100/v1");
    expect(message).toContain("x-nexcode-api-key from NEXCODE_API_AUTH_TOKEN");
    expect(readFileSync(join(codexHome, "config.toml"), "utf8")).toBe(original);
  });
  ...
  test("non-loopback hostname still uses the legacy provider-table injection", () => {
    ...
    const config = readFileSync(join(codexHome, "config.toml"), "utf8");
    expect(config).toContain('model_provider = "nexcode"');
    expect(config).toContain("[model_providers.nexcode]");
