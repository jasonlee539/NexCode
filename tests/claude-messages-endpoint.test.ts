...
beforeEach(() => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("nxc-claude-endpoint-");
  testDir = mkdtempSync(join(tmpdir(), "nxc-claude-endpoint-"));
  process.env.NEXCODE_HOME = testDir;
  previousDesktopConfigDir = process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR;
  process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR = join(testDir, "claude-desktop");
...
afterEach(() => {
  ...
  if (previousDesktopConfigDir === undefined) delete process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR;
  else process.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR = previousDesktopConfigDir;
...
function mockConfig(baseUrl: string, claudeCode?: NxcConfig["claudeCode"]): NxcConfig {
  return {
    port: 0,
  ...
  try {
    const response = await fetch(new URL("/v1/messages", server.url), {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 16,
        system: [
          { type: "text", text: "<!-- nxc-route: claude-haiku-4-5 -->" },
          { type: "text", text: "<!-- nxc-effort: max -->" },
        ],
...
test("A6: body-guard config normalization — 0 disables, negatives fall back, sub-second clamps to 1s", () => {
  const guardFor = (claudeCode: NxcConfig["claudeCode"]) =>
...
test("routed Claude requests give OpenAI sidecars main auth without leaking it to the routed provider", async () => {
  ...
  const visionCaption = "A red NEXCODE logo on a white background.";
  ...
  const forward = Bun.serve({
    ...
    async fetch(req) {
      ...
      const text = kind === "vision" ? visionCaption : "NexCode search results are available.";
      return new Response([
  ...
  const routed = Bun.serve({
    ...
    async fetch(req) {
      ...
      const frames = choosesWebSearch
        ? [
            { choices: [{ index: 0, delta: { tool_calls: [{ index: 0, id: "call_search", function: { name: "web_search", arguments: '{"query":"latest nexcode"}' } }] } }] },
  ...
  const requestBody = {
    ...
    messages: [{
      role: "user",
      content: [
        { type: "text", text: "Search for NexCode and inspect this logo." },
  ...
  try {
    for (const effort of ["xhigh", "max"]) {
      const response = await postMessages(server.url.toString(), {
        ...
        stream: true,
        system: [
          { type: "text", text: "<!-- nxc-route: claude-nxc-mock--test-model -->" },
          { type: "text", text: `<!-- nxc-effort: ${effort} -->` },
        ],
        thinking: { type: "enabled", budget_tokens: 31999 },
        messages: [{ role: "user", content: "hi" }],
      });
...
test("generated agent effort directive preserves routed Anthropic structured output", async () => {
  ...
  try {
    const response = await postMessages(server.url.toString(), {
      ...
      stream: true,
      system: [
        { type: "text", text: "<!-- nxc-route: claude-nxc-mock-anthropic--claude-sonnet-5 -->" },
        { type: "text", text: "<!-- nxc-effort: max -->" },
      ],
      thinking: { type: "enabled", budget_tokens: 31999 },
      output_config: {
