...
import type { NxcMessage, NxcParsedRequest, NxcProviderConfig } from "../src/types";
...
function developer(text: string): NxcMessage {
  return { role: "developer", content: text, timestamp: 0 };
}

function assistantWithCalls(calls: { id: string; name: string }[], text = ""): NxcMessage {
  return {
    role: "assistant",
    content: [
...
function toolResult(callId: string, name: string, output = "ok"): NxcMessage {
...
describe("openai-chat dangling tool_calls hardening", () => {
  ...
  test("T8 empty call id is minted so the call can be answered", () => {
    ...
    expect(minted).toMatch(/^call_nxc_minted_\d+$/);
