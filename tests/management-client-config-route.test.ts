...
/**
 * A key that looks exactly like a real one. Every assertion about `nxc_` absence is
 * worthless unless the running config actually holds a serializable secret (030 §Security).
 */
const REAL_LOOKING_KEY = "nxc_live_9f3c7a2b41d84e6fa05c8e17b3d92764";
...
async function clientConfigApi(config: NxcConfig, query: string): Promise<Response> {
...
async function modelRows(config: NxcConfig): Promise<ModelRow[]> {
...
describe("GET /api/client-config", () => {
  test("opencode envelope carries the shared builder's exact bytes", async () => {
    ...
    // for the same input, so the GUI download and `nxc export` can never disagree.
  ...
  test("an accepted override still resolves through the route", async () => {
    ...
    const overrideDir = "/tmp/nexcode-pi-route-fixture";
