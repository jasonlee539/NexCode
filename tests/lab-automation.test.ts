...
function tempHome(): string {
  const dir = join(tmpdir(), `nxc-lab-cl08-${process.pid}-${Math.random().toString(16).slice(2)}`);
...
describe("CL-08 lab automation", () => {
  ...
  test("production factory yields host-issued trusted executor", () => {
    const executor = createProductionLabRouteExecutor({
      loadConfig: () => ({ providers: {} } as NxcConfig),
    });
...
function fixtureProviderConfig(): NxcConfig {
  return {
    providers: {
      "fixture-provider": {
