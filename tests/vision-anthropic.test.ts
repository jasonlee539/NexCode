...
const anthropicProvider: NxcProviderConfig = {
  adapter: "anthropic",
  authMode: "oauth",
  baseUrl: "https://api.anthropic.test/v1/",
};
const settings = { model: "claude-sonnet-5", timeoutMs: 5000 };
const AUTH_ERROR_CANARY = "\\\\server\\share\\nexcode\\auth.json.nxc-tmp /home/alice/.nexcode/auth.json.nxc-tmp";
...
describe("Anthropic vision planning and management config", () => {
  test("explicit anthropic backend fails closed without a usable stored credential", async () => {
    const routed: NxcProviderConfig = {
      adapter: "openai-chat",
  ...
  test("GET/PUT persists valid vision backend and cap and rejects invalid values", async () => {
    ...
    const isolatedHome = mkdtempSync(join(tmpdir(), "nxc-vision-management-"));
  ...
  test("PUT rejects malformed body shapes with 400 and never persists them (review F2)", async () => {
    const previousHome = process.env.NEXCODE_HOME;
    const isolatedHome = mkdtempSync(join(tmpdir(), "nxc-vision-management-malformed-"));
    process.env.NEXCODE_HOME = isolatedHome;
    const config: NxcConfig = { port: 10100, defaultProvider: "none", providers: {} };
    try {
