...
beforeEach(() => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("nxc-worker-teardown-codex-");
  testDir = mkdtempSync(join(tmpdir(), "nxc-worker-teardown-"));
...
test.skipIf(skipNonWindowsWorkerSpawn)("repeated Windows-style spawn/reset cycles leave no live workers", async () => {
  ...
  for (let i = 0; i < cycles; i++) {
    ...
    isolatedCodexHome?.restore();
    isolatedCodexHome = installIsolatedCodexHome(`nxc-worker-teardown-cycle-${i}-`);
...
test.skipIf(skipNonWindowsWorkerSpawn)("async beforeEach-style join between cycles leaves no live workers", async () => {
  ...
  for (let i = 0; i < cycles; i++) {
    ...
    isolatedCodexHome?.restore();
    isolatedCodexHome = installIsolatedCodexHome(`nxc-worker-teardown-beforeeach-${i}-`);
