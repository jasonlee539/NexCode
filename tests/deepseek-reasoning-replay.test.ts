 ...
 * still stripping proxy-minted `nxcr1` envelopes no upstream can decrypt.
 */
...
import { NXC_REASONING_PREFIX } from "../src/responses/reasoning-envelope";
...
describe("sanitizeReasoningInputContent scoping", () => {
  ...
  test("preservation still strips an nxcr1 envelope but keeps the plaintext content", () => {
    const item = reasoningItem({ encrypted_content: `${NXC_REASONING_PREFIX}Zm9v` });
...
describe("DeepSeek Responses replay keeps reasoning on the wire", () => {
  function buildBody(provider: NxcProviderConfig): Record<string, unknown> {
