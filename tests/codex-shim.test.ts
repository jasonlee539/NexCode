...
const SHIM_MARKER = "nexcode codex autostart shim";
const UNIX_SHIM_REVISION_MARKER = "nexcode unix codex shim revision 2";
 ...
 * `NXC_SHIM_ACTIVE_DEPTH=1` — so the guard fires a level early and the test measures whatever
...
function shimChildEnv(overrides: Record<string, string> = {}): NodeJS.ProcessEnv {
  ...
  delete env.NXC_SHIM_ACTIVE_PID;
  delete env.NXC_SHIM_ACTIVE_DEPTH;
  delete env.NXC_SHIM_PROBE_ACTIVE;
  return env;
}
...
function withInstalledShim(run: (paths: {
  ...
  const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-bin-"));
  const home = mkdtempSync(join(tmpdir(), "nxc-shim-home-"));
  const oldPath = process.env.PATH;
  const oldHome = process.env.NEXCODE_HOME;
  ...
  try {
    ...
    if (oldHome === undefined) delete process.env.NEXCODE_HOME;
    else process.env.NEXCODE_HOME = oldHome;
...
describe("Codex autostart shim", () => {
  test("builds a Unix shim that starts nxc before execing Codex", () => {
    const script = buildUnixCodexShim("/usr/local/bin/codex-real", "/usr/local/bin/bun", "/opt/nexcode/src/cli.ts", "bundled");
    ...
    expect(script).toContain("NEXCODE_API_AUTH_TOKEN");
  });

  test("every shim flavor exports the Bun provenance it was built with (#848)", () => {
    // The shim reaches the daemon through `nxc ensure`, which inherits this env;
    // without it a Codex-autostarted service reports no provenance at all.
    const unix = buildUnixCodexShim("/usr/local/bin/codex-real", "/usr/local/bin/bun", "/opt/nexcode/src/cli.ts", "override", "/tmp/token");
    // Source and the binary it describes are stamped as a pair.
    expect(unix).toContain("NXC_BUN_RUNTIME_SOURCE='override' NXC_BUN_RUNTIME_PATH='/usr/local/bin/bun' '/usr/local/bin/bun' '/opt/nexcode/src/cli.ts' ensure");

    expect(buildWindowsCodexShim("C:\\Tools\\codex-real.exe", "C:\\Bun\\bun.exe", "C:\\nxc\\cli.ts", "override"))
      .toContain('set "NXC_BUN_RUNTIME_SOURCE=override"');

    expect(buildWindowsPowerShellCodexShim("C:\\Tools\\codex-real.exe", "C:\\Bun\\bun.exe", "C:\\nxc\\cli.ts", "process"))
      .toContain("$env:NXC_BUN_RUNTIME_SOURCE = 'process'");
  });
  ...
  test("the provenance marker never leaks into the real Codex process (#848 scoping)", () => {
    ...
    expect(unix).not.toContain("export NXC_BUN_RUNTIME_SOURCE");
    // The only occurrence is the one-shot assignment prefixed onto `ensure`.
    expect((unix.match(/NXC_BUN_RUNTIME_SOURCE/g) ?? []).length).toBe(1);
    expect((unix.match(/NXC_BUN_RUNTIME_PATH/g) ?? []).length).toBe(1);
    expect(unix.indexOf("NXC_BUN_RUNTIME_SOURCE")).toBeGreaterThan(unix.indexOf("nxc_subcommand"));
    ...
    const ensureBlock = cmd.slice(cmd.indexOf(":ensure_nxc"), cmd.indexOf(":run_codex"));
    expect(ensureBlock).toContain("setlocal");
    expect(ensureBlock).toContain('set "NXC_BUN_RUNTIME_SOURCE=override"');
    expect(ensureBlock).toContain("endlocal");
    expect(ensureBlock).toContain("NXC_BUN_RUNTIME_PATH");
    expect((cmd.match(/NXC_BUN_RUNTIME_SOURCE/g) ?? []).length).toBe(1);
    expect((cmd.match(/NXC_BUN_RUNTIME_PATH/g) ?? []).length).toBe(1);
    ...
    expect(ps).toContain("$priorRuntimeSource = $env:NXC_BUN_RUNTIME_SOURCE");
    expect(ps).toContain("Remove-Item Env:\\NXC_BUN_RUNTIME_SOURCE");
    expect(ps).toContain("$env:NXC_BUN_RUNTIME_SOURCE = $priorRuntimeSource");
    expect(ps.indexOf("NXC_BUN_RUNTIME_SOURCE")).toBeGreaterThan(ps.indexOf("$skipEnsure"));
  });

  test("builds a Windows shim that starts nxc before running Codex", () => {
    const script = buildWindowsCodexShim("C:\\Tools\\codex-real.exe", "C:\\Bun\\bun.exe", "C:\\nxc\\cli.ts", "bundled");

    expect(script).toContain(SHIM_MARKER);
    expect(script).toContain("ensure");
    expect(script).not.toContain("sync-cache");
    expect(script).toContain('set "NXC_REAL_CODEX=C:\\Tools\\codex-real.exe"');
    expect(script).toContain('set "NXC_API_TOKEN_FILE=');
    expect(script).toContain('set /p NEXCODE_API_AUTH_TOKEN=<"%NXC_API_TOKEN_FILE%"');
    expect(script).toContain('"%NXC_REAL_CODEX%" %*');
  });
  ...
  test("Windows cmd shim escapes executable paths through variables", () => {
    ...
    expect(script).toContain('set "NXC_REAL_CODEX=C:\\Tools&A\\100%%codex^^\\codex-real.exe"');
    expect(script).toContain('set "NXC_BUN=C:\\Bun&Dir\\100%%bun^^\\bun.exe"');
    expect(script).toContain('set "NXC_CLI=C:\\nxc&Dir\\cli.ts"');
    expect(script).toContain('"%NXC_BUN%" "%NXC_CLI%" ensure >nul 2>nul');
  ...
  test("Windows cmd shim rewrites profile paths to env indirection (OEM-codepage batch parsing vs non-ASCII usernames)", () => {
    ...
    try {
      ...
      const script = buildWindowsCodexShim(
        "C:\\Users\\한글사용자\\AppData\\Roaming\\npm\\codex.nexcode-real.cmd",
        "C:\\Users\\한글사용자\\AppData\\Roaming\\npm\\node_modules\\bun\\bin\\bun.exe",
        "C:\\Users\\한글사용자\\AppData\\Roaming\\npm\\node_modules\\nexcode\\src\\cli.ts",
        "bundled",
      );

      expect(script).toContain('set "NXC_REAL_CODEX=%APPDATA%\\npm\\codex.nexcode-real.cmd"');
      expect(script).toContain('set "NXC_BUN=%APPDATA%\\npm\\node_modules\\bun\\bin\\bun.exe"');
  ...
  test("Unix shim accepts an injected token-file path (Git-Bash shims need forward slashes everywhere)", () => {
    const script = buildUnixCodexShim(
      "C:/Users/한글사용자/AppData/Roaming/npm/codex.nexcode-real",
      "C:/Users/한글사용자/AppData/Roaming/npm/node_modules/bun/bin/bun.exe",
      "C:/Users/한글사용자/AppData/Roaming/npm/node_modules/nexcode/src/cli.ts",
      "bundled",
      "C:/Users/한글사용자/.nexcode/service-api-token",
    );

    expect(script).toContain("exec 'C:/Users/한글사용자/AppData/Roaming/npm/codex.nexcode-real' \"$@\"");
    expect(script).toContain("[ -f 'C:/Users/한글사용자/.nexcode/service-api-token' ]");
  ...
  test("shim builder output contains the marker that isShim() checks", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "nxc-shim-test-"));
  ...
  test("Unix shim uses bypass env var to skip proxy start", () => {
    ...
    expect(script).toContain("NXC_SHIM_BYPASS");
  });

  test("Unix shim stops same-process re-entry through a dynamic launcher", () => {
    if (process.platform === "win32") return;

    const dir = mkdtempSync(join(tmpdir(), "nxc-shim-reentry-"));
    const bunPath = join(dir, "bun");
    const misePath = join(dir, "mise");
    const realCodexPath = join(dir, "codex.nexcode-real");
...
`, "utf8");
      ...
      const env = shimChildEnv({ PATH: prependPath(dir, process.env.PATH) ?? "", NXC_SHIM_BYPASS: "1" });
      ...
      expect(result.stderr).toContain("nxc codex-shim uninstall");
    } finally {
  ...
  test("Unix install rejects a recursive dynamic launcher and restores the original", () => {
    ...
    const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-install-reentry-bin-"));
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-install-reentry-home-"));
    const oldPath = process.env.PATH;
    const oldHome = process.env.NEXCODE_HOME;
...
`, "utf8");
      ...
      expect(existsSync(`${codexPath}.nexcode-real`)).toBe(false);
  ...
  test("Unix runtime guard stops argument-dependent child-process redispatch", () => {
    ...
    const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-runtime-child-reentry-bin-"));
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-runtime-child-reentry-home-"));
    const oldPath = process.env.PATH;
    const oldHome = process.env.NEXCODE_HOME;
    const codexPath = join(binDir, "codex");
    const original = `#!/bin/sh
...
`;
    try {
      ...
      const result = spawnSync(codexPath, ["--help"], {
        encoding: "utf8",
        env: { ...process.env, NXC_SHIM_BYPASS: "1" },
        timeout: 3_000,
      });
      expect(result.error).toBeUndefined();
      expect(result.status).toBe(126);
  ...
  test("Unix install drains an immediate recursive diagnostic before classifying the probe", () => {
    ...
    const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-install-immediate-reentry-bin-"));
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-install-immediate-reentry-home-"));
    const oldPath = process.env.PATH;
    const oldHome = process.env.NEXCODE_HOME;
    const codexPath = join(binDir, "codex");
    const original = `#!/bin/sh
printf '%s\\n' 'nexcode: saved Codex launcher resolved back to the autostart shim; run nxc codex-shim uninstall and reinstall Codex before enabling codexAutoStart.' >&2
exit 126
`;
    try {
      process.env.PATH = prependPath(binDir, oldPath);
  ...
  test.skipIf(process.platform === "win32" || !existsSync("/bin/dash"))(
    ...
    () => {
      const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-install-dash-bin-"));
      const home = mkdtempSync(join(tmpdir(), "nxc-shim-install-dash-home-"));
      const oldPath = process.env.PATH;
      const oldHome = process.env.NEXCODE_HOME;
      const codexPath = join(binDir, "codex");
      try {
        ...
        expect(readFileSync(`${codexPath}.nexcode-real`, "utf8")).toBe(successfulLauncher("dash-valid-launcher"));
  ...
  test("Unix install honors the injected probe shell path", () => {
    ...
    const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-install-missing-shell-bin-"));
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-install-missing-shell-home-"));
    const oldPath = process.env.PATH;
    const oldHome = process.env.NEXCODE_HOME;
    const codexPath = join(binDir, "codex");
  ...
  test.skipIf(process.platform === "win32" || !existsSync("/usr/bin/true"))(
    ...
    () => {
      const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-install-native-bin-"));
      const home = mkdtempSync(join(tmpdir(), "nxc-shim-install-native-home-"));
      const oldPath = process.env.PATH;
      const oldHome = process.env.NEXCODE_HOME;
      const codexPath = join(binDir, "codex");
      try {
        ...
        expect(installed.installed).toBe(true);
        expect(readFileSync(codexPath, "utf8")).toContain(SHIM_MARKER);
        expect(lstatSync(`${codexPath}.nexcode-real`).isFile()).toBe(true);
      } finally {
        if (oldPath === undefined) delete process.env.PATH;
        else process.env.PATH = oldPath;
        if (oldHome === undefined) delete process.env.NEXCODE_HOME;
  ...
  test.skipIf(process.platform === "win32" || !existsSync("/usr/bin/true"))(
    ...
    () => {
      const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-install-native-link-bin-"));
      const home = mkdtempSync(join(tmpdir(), "nxc-shim-install-native-link-home-"));
      const oldPath = process.env.PATH;
      const oldHome = process.env.NEXCODE_HOME;
      const codexPath = join(binDir, "codex");
      try {
        ...
        expect(installed.installed).toBe(true);
        expect(readFileSync(codexPath, "utf8")).toContain(SHIM_MARKER);
        expect(lstatSync(`${codexPath}.nexcode-real`).isSymbolicLink()).toBe(true);
      } finally {
        if (oldPath === undefined) delete process.env.PATH;
        else process.env.PATH = oldPath;
        if (oldHome === undefined) delete process.env.NEXCODE_HOME;
  ...
  test("Unix install rejects a launcher that leaves a background descendant", () => {
    ...
    const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-install-child-reentry-bin-"));
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-install-child-reentry-home-"));
    const oldPath = process.env.PATH;
    const oldHome = process.env.NEXCODE_HOME;
    const codexPath = join(binDir, "codex");
...
`;
  ...
  test.skipIf(process.platform === "win32" || !python3Path)(
    ...
    () => {
      const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-install-detached-reentry-bin-"));
      const home = mkdtempSync(join(tmpdir(), "nxc-shim-install-detached-reentry-home-"));
      const oldPath = process.env.PATH;
      const oldHome = process.env.NEXCODE_HOME;
      const codexPath = join(binDir, "codex");
...
`;
  ...
  test("Unix install rolls back when launcher validation times out", () => {
    ...
    const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-install-timeout-bin-"));
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-install-timeout-home-"));
    const oldPath = process.env.PATH;
    const oldHome = process.env.NEXCODE_HOME;
    const codexPath = join(binDir, "codex");
...
`;
  ...
  test("Unix install preserves an existing backup without probing or mutation", () => {
    ...
    const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-install-backup-bin-"));
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-install-backup-home-"));
    const oldPath = process.env.PATH;
    const oldHome = process.env.NEXCODE_HOME;
    const codexPath = join(binDir, "codex");
    const backupPath = `${codexPath}.nexcode-real`;
    const original = "#!/bin/sh\nprintf '%s\\n' original\n";
    const backup = "#!/bin/sh\nprintf '%s\\n' preserved-backup\n";
    try {
      process.env.PATH = prependPath(binDir, oldPath);
  ...
  test("Unix install rolls back when the saved launcher fails its version probe", () => {
    ...
    const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-install-failed-bin-"));
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-install-failed-home-"));
    const oldPath = process.env.PATH;
    const oldHome = process.env.NEXCODE_HOME;
    const codexPath = join(binDir, "codex");
    const original = "#!/bin/sh\nexit 127\n";
  ...
  test("Unix install rolls back when probe infrastructure throws", () => {
    ...
    const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-install-probe-error-bin-"));
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-install-probe-error-home-"));
    const oldPath = process.env.PATH;
    const oldHome = process.env.NEXCODE_HOME;
    const codexPath = join(binDir, "codex");
  ...
  test("Unix fresh install removes its marker-bearing partial wrapper before rollback", () => {
    ...
    const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-install-partial-write-bin-"));
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-install-partial-write-home-"));
    const oldPath = process.env.PATH;
    const oldHome = process.env.NEXCODE_HOME;
    const codexPath = join(binDir, "codex");
  ...
  test("Unix fresh install rolls an unprobeable original back into place", () => {
    ...
    // stranding the launcher at codex.nexcode-real. Identity is metadata, not
    // content, so the fingerprint must not depend on the file having bytes.
    const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-install-unprobeable-bin-"));
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-install-unprobeable-home-"));
    const oldPath = process.env.PATH;
    const oldHome = process.env.NEXCODE_HOME;
    const codexPath = join(binDir, "codex");
    const backupPath = `${codexPath}.nexcode-real`;
  ...
  test("Unix rollback restore refuses to replace a launcher published in the restore window", () => {
    ...
    const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-restore-window-bin-"));
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-restore-window-home-"));
    const oldPath = process.env.PATH;
    const oldHome = process.env.NEXCODE_HOME;
    const codexPath = join(binDir, "codex");
    const backupPath = `${codexPath}.nexcode-real`;
  ...
  test("Unix fresh install preserves a concurrent wrapper replacement after a successful probe", () => {
    ...
    const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-install-concurrent-bin-"));
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-install-concurrent-home-"));
    const oldPath = process.env.PATH;
    const oldHome = process.env.NEXCODE_HOME;
    const codexPath = join(binDir, "codex");
...
`;
    try {
      ...
      // A stray `codex.nexcode-real` is recoverable; a deleted launcher is not.
      expect(existsSync(`${codexPath}.nexcode-real`)).toBe(true);
      expect(existsSync(join(home, "codex-shim.json"))).toBe(false);
    } finally {
      if (oldPath === undefined) delete process.env.PATH;
      else process.env.PATH = oldPath;
  ...
  test("Unix fresh install refuses to adopt a wrapper replaced between the write and the fingerprint", () => {
    ...
    // afterwards. A replacement that lands in that window carries the NexCode
    // markers by coincidence or by design; adopting it means our rollback later
    // unlinks an executable we never wrote.
    const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-adopt-bin-"));
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-adopt-home-"));
    const oldPath = process.env.PATH;
    const oldHome = process.env.NEXCODE_HOME;
    const codexPath = join(binDir, "codex");
    const original = "#!/bin/sh\nexit 0\n";
    ...
    try {
      ...
      expect(existsSync(`${codexPath}.nexcode-real`)).toBe(true);
      expect(readFileSync(`${codexPath}.nexcode-real`, "utf8")).toBe(original);
      // No install state is published for a refused install.
      expect(existsSync(join(home, "codex-shim.json"))).toBe(false);
      // No staging artifact leaked into the user's PATH directory.
      expect(readdirSync(binDir).filter(name => name.includes("nexcode-staging"))).toEqual([]);
    } finally {
      setCodexShimFreshWriteHookForTests(null);
      if (oldPath === undefined) delete process.env.PATH;
      else process.env.PATH = oldPath;
  ...
  test("Unix shim permits a real Codex process to start a new child invocation", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "nxc-shim-child-"));
    const bunPath = join(dir, "bun");
    const realCodexPath = join(dir, "codex.nexcode-real");
    const shimPath = join(dir, "codex");
    try {
      writeFileSync(bunPath, "#!/usr/bin/env sh\nexit 0\n", "utf8");
      writeFileSync(realCodexPath, `#!/usr/bin/env sh
if [ -z "$NXC_TEST_CHILD" ]; then
  NXC_TEST_CHILD=1
  export NXC_TEST_CHILD
  "${shimPath}" --version
  exit $?
fi
printf '%s\\n' child-codex
`, "utf8");
      ...
      chmodSync(realCodexPath, 0o755);
      chmodSync(shimPath, 0o755);
      const env = shimChildEnv({ NXC_SHIM_BYPASS: "1" });
  ...
  test("Unix shim treats executable paths as literals instead of shell interpolation", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "nxc-shim-quote-"));
  ...
  test("Unix shim exports persisted service API token before running Codex", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "nxc-shim-token-"));
    const logPath = join(dir, "calls.log");
    const bunPath = join(dir, "bun");
    const realCodexPath = join(dir, "codex-real");
    const shimPath = join(dir, "codex");
    const oldHome = process.env.NEXCODE_HOME;
    const oldToken = process.env.NEXCODE_API_AUTH_TOKEN;
    try {
      process.env.NEXCODE_HOME = dir;
      delete process.env.NEXCODE_API_AUTH_TOKEN;
      writeFileSync(join(dir, "service-api-token"), "local-secret\n", "utf8");
      writeFileSync(bunPath, `#!/usr/bin/env sh\nexit 0\n`, "utf8");
      writeFileSync(realCodexPath, `#!/usr/bin/env sh\necho "token:$NEXCODE_API_AUTH_TOKEN" >> "${logPath}"\n`, "utf8");
      writeFileSync(shimPath, buildUnixCodexShim(realCodexPath, bunPath, "/opt/nexcode/src/cli.ts", "bundled"), "utf8");
      chmodSync(bunPath, 0o755);
      chmodSync(realCodexPath, 0o755);
      chmodSync(shimPath, 0o755);
      ...
      if (oldToken === undefined) delete process.env.NEXCODE_API_AUTH_TOKEN;
      else process.env.NEXCODE_API_AUTH_TOKEN = oldToken;
    }
  });

  test("Unix shim skips nxc startup only for Codex management commands", () => {
    if (process.platform === "win32") return;
    ...
    expect(readFileSync(logPath, "utf8")).toBe(
      "codex:doctor\ncodex:-s read-only -a untrusted app-server\nbun:/opt/nexcode/src/cli.ts ensure\ncodex:exec hello\n",
    );
  ...
  test("Windows shim skips nxc startup only for Codex management commands", () => {
    const script = buildWindowsCodexShim("C:\\Tools\\codex-real.exe", "C:\\Bun\\bun.exe", "C:\\nxc\\cli.ts", "bundled");
  ...
  test("Windows install backs up cmd, ps1, and the bare Git-Bash launcher", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "nxc-shim-bin-"));
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-home-"));
    const oldPath = process.env.PATH;
    const oldHome = process.env.NEXCODE_HOME;
    ...
    try {
      ...
      expect(readFileSync(join(dir, "codex.nexcode-real.cmd"), "utf8")).toBe(cmdOriginal);
      expect(readFileSync(join(dir, "codex.nexcode-real.ps1"), "utf8")).toBe(ps1Original);
      expect(readFileSync(join(dir, "codex.nexcode-real"), "utf8")).toBe(bareOriginal);
...
`;
  ...
  test("an aged lock held by a live restore owner is never reclaimed", async () => {
    const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-concurrent-bin-"));
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-concurrent-home-"));
    ...
    const backup = join(binDir, process.platform === "win32" ? "codex.nexcode-real.cmd" : "codex.nexcode-real");
    ...
    try {
      ...
      `;
      const childEnv = { ...process.env, PATH: binDir, NEXCODE_HOME: home };
  ...
  test("mixed launcher siblings defer the whole restore without piecemeal mutation", () => {
    withInstalledShim(({ binDir, wrappers, backups, statePath }) => {
      if (wrappers.length === 1) {
        ...
        const siblingBackup = join(binDir, "codex.nexcode-real.ps1");
  ...
  test("multi-wrapper restore rolls back when a later sibling fingerprint changes", () => {
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-transaction-home-"));
    const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-transaction-bin-"));
    const oldHome = process.env.NEXCODE_HOME;
    try {
      process.env.NEXCODE_HOME = home;
      const wrappers = [join(binDir, "codex.cmd"), join(binDir, "codex.ps1")];
      const backups = [join(binDir, "codex.nexcode-real.cmd"), join(binDir, "codex.nexcode-real.ps1")];
