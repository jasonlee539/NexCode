...
import type { AdapterEvent, NxcParsedRequest, NxcProviderConfig } from "../src/types";
import { withTestTranslatorBudget } from "./helpers/translator-budget";

const createGoogleAdapter = (...args: Parameters<typeof createGoogleAdapterProduction>) =>
  withTestTranslatorBudget(createGoogleAdapterProduction(...args));

function parsed(text = "hello world", stream = false, modelId = "gemini-3-pro"): NxcParsedRequest {
  return {
    modelId,
    stream,
...
function parsedWithEffort(modelId: string, effort?: string): NxcParsedRequest {
  return {
    modelId,
    stream: false,
...
describe("antigravity CCA envelope", () => {
  ...
  test("throws when no project id is available", async () => {
    const noProj = { ...provider, project: undefined } as NxcProviderConfig;
  ...
  function threaded(text: string, threadId?: string): NxcParsedRequest {
    const base = parsed(text) as NxcParsedRequest & { _clientThreadId?: string };
  ...
  test("#1297: promptCacheKey does not influence the id", () => {
    ...
    const withKey = threaded("same text", "thread-a") as NxcParsedRequest;
    (withKey.options as Record<string, unknown>).promptCacheKey = "cohort-1";
    const otherKey = threaded("same text", "thread-a") as NxcParsedRequest;
  ...
  test("claude-on-antigravity forces toolConfig.functionCallingConfig.mode=VALIDATED", async () => {
    const claudeProvider = { ...provider } as NxcProviderConfig;
    const withTools = {
