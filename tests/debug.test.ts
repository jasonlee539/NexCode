...
describe("debug frame logging", () => {
  const previous = process.env.NXC_DEBUG;

  afterEach(() => {
    resetDebugSettingsForTests();
    resetDebugLogBufferForTests();
    resetInjectionDebugLogBufferForTests();
    if (previous === undefined) delete process.env.NXC_DEBUG;
    else process.env.NXC_DEBUG = previous;
  });

  test("debugDroppedFrame redacts payload content", () => {
    process.env.NXC_DEBUG = "1";
  ...
  test("debugProviderDiagnostic emits under NXC_DEBUG with provider prefix and redacts secrets", () => {
    process.env.NXC_DEBUG = "1";
    const error = spyOn(console, "error").mockImplementation(() => {});
    try {
      ...
      expect(error).toHaveBeenCalledTimes(1);
      const line = String(error.mock.calls[0]?.[0] ?? "");
      expect(line).toContain("[nxc:cursor:dial]");
  ...
  test("legacy NXC_DEBUG_FRAMES still enables provider diagnostics", () => {
    delete process.env.NXC_DEBUG;
    process.env.NXC_DEBUG_FRAMES = "1";
    const error = spyOn(console, "error").mockImplementation(() => {});
    try {
      debugProviderDiagnostic("cursor", "connected", { connectMs: 12 });
      expect(error).toHaveBeenCalledTimes(1);
      expect(String(error.mock.calls[0]?.[0] ?? "")).toContain("[nxc:cursor:connected]");
    } finally {
      error.mockRestore();
    }
  });
  ...
  test("appendDebugLogLine supports after/limit queries with monotonic seq", () => {
    appendDebugLogLine("[nxc:test:one]");
    appendDebugLogLine("[nxc:test:two]");
  ...
  test("same-millisecond bursts keep every line via seq cursor", () => {
    ...
    try {
      appendDebugLogLine("[nxc:test:a]");
      appendDebugLogLine("[nxc:test:b]");
      const all = getDebugLogEntries();
      const tail = getDebugLogEntries({ after: all[0]!.seq, limit: 10 });
      expect(tail).toHaveLength(1);
