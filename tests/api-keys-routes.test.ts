...
async function keysRequest(
  ...
  const res = await fetch(new URL("/api/keys", server.url), {
    method,
    headers: { "Content-Type": "application/json", "x-nexcode-api-key": ADMIN_TOKEN },
...
beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), "nxc-api-keys-routes-"));
  process.env.NEXCODE_HOME = testHome;
  delete process.env.NEXCODE_API_AUTH_TOKEN;
  process.env.NEXCODE_ADMIN_AUTH_TOKEN = ADMIN_TOKEN;
});
...
describe("POST /api/keys", () => {
  test("persists a key and returns the full secret exactly once", async () => {
    ...
    try {
      ...
      expect(created.json.key).toMatch(/^nxc_data_[0-9a-f]{40}$/);
  ...
  test("two keys differ in the eight random hex the list actually shows", async () => {
    ...
    try {
      ...
      // fixed `nxc_data` literal for every key ever generated.
...
describe("apiKeys config compatibility", () => {
  ...
  test("one malformed entry costs only itself", () => {
    ...
    const raw = readRawConfig();
    raw.apiKeys = [
      { id: "good", name: "usable", key: "nxc_data_usable", createdAt: "2026-07-31T00:00:00.000Z" },
      { id: "", name: 7 },
    ];
    writeRawConfig(raw);
  ...
  test("malformed metadata never revokes a usable credential", () => {
    ...
    raw.apiKeys = [
      { id: "still-live", name: 7, key: "nxc_data_stilllive", createdAt: 1234 },
    ];
    writeRawConfig(raw);
    ...
    const kept = loaded.apiKeys ?? [];
    expect(kept).toHaveLength(1);
    expect(kept[0]!.key).toBe("nxc_data_stilllive");
    expect(isDataPlaneAdmissionSecret("nxc_data_stilllive", loaded)).toBe(true);
  });
  ...
  test("a salvaged credential stays manageable: it gets a real id", async () => {
    ...
    raw.apiKeys = [
      { id: 7, name: "unmanageable", key: "nxc_data_needsid", createdAt: "2026-07-31T00:00:00.000Z" },
    ];
    writeRawConfig(raw);
    ...
    try {
      ...
      expect(removed.status).toBe(200);
      expect(isDataPlaneAdmissionSecret("nxc_data_needsid", loadConfig())).toBe(false);
    } finally {
      await server.stop(true);
    }
  });
  ...
  test("a whitespace-only key is dropped, not floated to the front", () => {
    ...
    raw.apiKeys = [
      ...
      { id: "real", name: "usable", key: "nxc_data_realkey", createdAt: "2026-07-31T00:00:00.000Z" },
    ];
    writeRawConfig(raw);
    ...
    const kept = loaded.apiKeys ?? [];
    expect(kept).toHaveLength(1);
    expect(kept[0]!.key).toBe("nxc_data_realkey");
    expect(ownAdmissionTokens(loaded)).toEqual(["nxc_data_realkey"]);
  });
  ...
  test("a duplicate id is reported too", () => {
    ...
    try {
      ...
      const raw = readRawConfig();
      raw.apiKeys = [
        { id: "same", name: "one", key: "nxc_data_dupwarnone", createdAt: "2026-07-31T00:00:00.000Z" },
        { id: "same", name: "two", key: "nxc_data_dupwarntwo", createdAt: "2026-07-31T00:00:00.000Z" },
      ];
      writeRawConfig(raw);
      loadConfig();
  ...
  test("metadata repair is reported, not silent", () => {
    ...
    try {
      ...
      const raw = readRawConfig();
      raw.apiKeys = [
        { id: "keeps-working", name: 7, key: "nxc_data_repaired", createdAt: "2026-07-31T00:00:00.000Z" },
      ];
      writeRawConfig(raw);
      loadConfig();
  ...
  test("a credential salvaged from bad metadata survives a later save", async () => {
    ...
    const raw = readRawConfig();
    raw.apiKeys = [
      { id: "still-live", name: 7, key: "nxc_data_stilllive", createdAt: "2026-07-31T00:00:00.000Z" },
    ];
    writeRawConfig(raw);
    ...
    try {
      ...
      expect(persisted.map(k => k.key)).toContain("nxc_data_stilllive");
    } finally {
      await server.stop(true);
    }
  });
  ...
  test("a legacy over-long name still loads", () => {
    ...
    const raw = readRawConfig();
    raw.apiKeys = [
      { id: "legacy", name: "n".repeat(200), key: "nxc_data_legacy", createdAt: "2026-07-31T00:00:00.000Z" },
    ];
    writeRawConfig(raw);
  ...
  test("a repaired id is stable across loads, not reminted every parse", () => {
    ...
    const raw = readRawConfig();
    raw.apiKeys = [
      { id: 7, name: "needs-an-id", key: "nxc_data_stableid", createdAt: "2026-07-31T00:00:00.000Z" },
    ];
    writeRawConfig(raw);
  ...
  test("diagnostics see the same repaired ids as loadConfig", () => {
    ...
    const raw = readRawConfig();
    raw.apiKeys = [
      { id: 7, name: "needs-an-id", key: "nxc_data_diagnostics", createdAt: "2026-07-31T00:00:00.000Z" },
    ];
    writeRawConfig(raw);
  ...
  test("a synthesized id never steals an id another key already owns", () => {
    ...
    raw.apiKeys = [
      { id: 7, name: "needs-an-id", key: "nxc_data_needsid", createdAt: "2026-07-31T00:00:00.000Z" },
      { id: "salvaged-1", name: "already-owns-it", key: "nxc_data_ownsit", createdAt: "2026-07-31T00:00:00.000Z" },
    ];
    writeRawConfig(raw);

    const kept = loadConfig().apiKeys ?? [];
    const ownsIt = kept.find(k => k.key === "nxc_data_ownsit")!;
    const needsId = kept.find(k => k.key === "nxc_data_needsid")!;
  ...
  test("the first holder of a duplicate id keeps it", () => {
    ...
    const raw = readRawConfig();
    raw.apiKeys = [
      { id: "shared", name: "first", key: "nxc_data_firstdup", createdAt: "2026-07-31T00:00:00.000Z" },
      { id: "shared", name: "second", key: "nxc_data_seconddup", createdAt: "2026-07-31T00:00:00.000Z" },
    ];
    writeRawConfig(raw);

    const kept = loadConfig().apiKeys ?? [];
    expect(kept.find(k => k.key === "nxc_data_firstdup")!.id).toBe("shared");
    expect(kept.find(k => k.key === "nxc_data_seconddup")!.id).not.toBe("shared");
  });
  ...
  test("duplicate ids are separated so each key stays individually revocable", () => {
    ...
    const raw = readRawConfig();
    raw.apiKeys = [
      { id: "same", name: "one", key: "nxc_data_dupone", createdAt: "2026-07-31T00:00:00.000Z" },
      { id: "same", name: "two", key: "nxc_data_duptwo", createdAt: "2026-07-31T00:00:00.000Z" },
    ];
    writeRawConfig(raw);
  ...
  test("a key with surrounding whitespace is dropped: it can never admit", () => {
    ...
    // verbatim, so " nxc_data_spaced " matches neither form of itself.
    raw.apiKeys = [
      { id: "spaced", name: "unusable", key: " nxc_data_spaced ", createdAt: "2026-07-31T00:00:00.000Z" },
      { id: "real", name: "usable", key: "nxc_data_realkey", createdAt: "2026-07-31T00:00:00.000Z" },
    ];
    writeRawConfig(raw);

    const loaded = loadConfig();
    expect((loaded.apiKeys ?? []).map(k => k.key)).toEqual(["nxc_data_realkey"]);
    expect(ownAdmissionTokens(loaded)).toEqual(["nxc_data_realkey"]);
  });
});
