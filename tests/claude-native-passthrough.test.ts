...
beforeEach(() => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("nxc-claude-native-");
  testDir = mkdtempSync(join(tmpdir(), "nxc-claude-native-"));
...
function cfg(anthropicBaseUrl: string, extraClaude?: Record<string, unknown>): NxcConfig {
  return {
    port: 0,
...
test("exposed native passthrough requires dedicated admission and never forwards admission credentials", async () => {
  ...
  try {
    ...
    const bearerProvider = await globalThis.fetch(messagesUrl, {
      ...
      headers: {
        "content-type": "application/json",
        "x-nexcode-api-key": admissionSecret,
    ...
    expect(captured[0].headers.get("x-nexcode-api-key")).toBeNull();
    ...
    expect(captured[1].headers.get("x-nexcode-api-key")).toBeNull();
...
test("alias/mapped models and non-anthropic credentials do NOT pass through", async () => {
  ...
  try {
    ...
    const alias = await fetch(new URL("/v1/messages", server.url), {
      method: "POST",
      headers: OAUTH_HEADERS,
      body: JSON.stringify({ model: "claude-nxc-mock--test-model", max_tokens: 10, messages: [{ role: "user", content: "x" }] }),
    });
    ...
    const placeholder = await fetch(new URL("/v1/messages", server.url), {
      method: "POST",
      headers: { "content-type": "application/json", "authorization": "Bearer nexcode-local" },
