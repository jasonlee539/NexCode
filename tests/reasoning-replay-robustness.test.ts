...
const SCOPE: NxcReasoningReplayScopeRef = {
...
describe("reasoning replay empty-delta robustness", () => {
  ...
  test("the memory-only cache writes no reasoning state to disk even when persistence env vars are set", async () => {
    const scratch = mkdtempSync(join(tmpdir(), "nxc-reasoning-memory-only-"));
    ...
    try {
      const child = Bun.spawn([process.execPath, "-e", script], {
        ...
        env: {
          ...process.env,
          NEXCODE_HOME: scratch,
          NEXCODE_REASONING_REPLAY_PERSIST: "1",
          NEXCODE_REASONING_REPLAY_FILE: spill,
        },
        stdout: "pipe",
        stderr: "pipe",
      });
