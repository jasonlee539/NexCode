...
function providerWithFamilyEntries(): NxcProviderConfig {
  return {
    adapter: "openai-chat",
...
function configFor(provider: NxcProviderConfig): NxcConfig {
  return { providers: { custom: provider } } as unknown as NxcConfig;
}
...
describe("candidateCapabilityEvidence model matching", () => {
  ...
  test("a registry entry covers its tagged siblings with no provider configured", () => {
    ...
    const emptyConfig = { providers: {} } as unknown as NxcConfig;
