...
function parsed(modelId: string, adapter: string, extra?: Partial<NxcParsedRequest>): NxcParsedRequest {
  return {
...
describe("identity neutralization — central helper", () => {
  ...
  test("never emits the nexcode proxy identity", () => {
    const out = neutralizeIdentity(`${SYS}\n\nmore context`);
    expect(out).not.toMatch(/nexcode proxy/i);
...
describe("identity neutralization — adapters never leak proxy identity", () => {
  test("openai-chat: system message is neutralized, no proxy mention", async () => {
    const provider = { adapter: "openai-chat", baseUrl: "https://api.example.invalid", apiKey: "key" } as unknown as NxcProviderConfig;
    ...
    expect(sys.content).not.toMatch(/nexcode proxy/i);
  ...
  test("openai-chat: identity names the model id actually sent on the wire", async () => {
    ...
    } as unknown as NxcProviderConfig;
  ...
  test("google/antigravity: systemInstruction is neutralized, no proxy mention", async () => {
    ...
    expect(sysText).not.toMatch(/nexcode proxy/i);
  ...
  describe("kiro", () => {
    ...
    test("system prefix is neutralized, no proxy mention", async () => {
      ...
      expect(serialized).not.toMatch(/nexcode proxy/i);
