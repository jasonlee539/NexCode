...
function parsed(modelId: string, providerOptions: NxcParsedRequest["options"]): NxcParsedRequest {
  return {
    modelId,
...
function buildBody(provider: NxcProviderConfig, modelId: string, options: NxcParsedRequest["options"]): Record<string, unknown> {
...
function buildChatRequest(
  ...
  modelId: string,
  options: NxcParsedRequest["options"],
): AdapterRequest {
...
describe("thinking-toggle models (260707)", () => {
  const toggleProvider: NxcProviderConfig = {
    adapter: "openai-chat",
...
describe("Qwen reasoning wire contracts", () => {
  const budgetProvider: NxcProviderConfig = {
    adapter: "openai-chat",
    baseUrl: "https://api.neuralwatt.com/v1",
  ...
  test("opencode-go Qwen models are no longer pinned to the Anthropic wire", () => {
    const provider: NxcProviderConfig = { adapter: "openai-chat", baseUrl: "https://opencode.ai/zen/go/v1" };
...
describe("ultra reasoning effort (upstream codex-rs parity)", () => {
  const base: NxcProviderConfig = { adapter: "openai-chat", baseUrl: "https://provider.example/v1" };
...
describe("stale reasoning-ladder self-heal", () => {
  const base: NxcProviderConfig = { baseUrl: "https://x", apiKey: "k" };

  test("ladder stopping at xhigh gains max when the wire map routes xhigh -> max", () => {
    const prov: NxcProviderConfig = {
      ...base,
  ...
  test("no wire map means no heal — an xhigh-top ladder without max evidence is preserved", () => {
    const prov: NxcProviderConfig = { ...base, modelReasoningEfforts: { m: ["low", "medium", "high", "xhigh"] } };
