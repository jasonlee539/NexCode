...
function migratableConfig(): NxcConfig {
  return {
    port: 10100,
...
function namespaceCollidingConfig(): NxcConfig {
  return {
    ...migratableConfig(),
...
test("moves a Beijing entry holding an international endpoint", () => {
  ...
  // Beijing catalog fields, as `nxc provider add` would have persisted them.
...
test("the migrated config survives a reload", () => {
  ...
  const home = mkdtempSync(join(tmpdir(), "nxc-alibaba-"));
  const prev = process.env.NEXCODE_HOME;
  ...
  try {
    ...
    if (prev === undefined) delete process.env.NEXCODE_HOME;
    else process.env.NEXCODE_HOME = prev;
...
test("a namespace-blocked migration remains valid across reload", () => {
  ...
  const home = mkdtempSync(join(tmpdir(), "nxc-alibaba-namespace-"));
  const prev = process.env.NEXCODE_HOME;
  process.env.NEXCODE_HOME = home;
  try {
    saveConfig(projection.config);
