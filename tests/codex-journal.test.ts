...
describe("codex-journal", () => {
  ...
  beforeEach(() => {
    testDir = mkdtempSync(join(tmpdir(), "nxc-journal-"));
  ...
  test("writeJournal creates journal file", () => {
    const r = runScript(testDir, `
      ...
      const journalPath = path.join(process.env.CODEX_HOME, "nexcode-journal.json");
  ...
  test("reconcileJournal restores config when journaled PID is dead", () => {
    const journalPath = join(testDir, "nexcode-journal.json");
  ...
  test("reconcileJournal skips when journaled PID is alive", () => {
    const journalPath = join(testDir, "nexcode-journal.json");
    const modified = "# modified by nexcode\n";
    writeFileSync(join(testDir, "config.toml"), modified, "utf8");
    writeFileSync(journalPath, JSON.stringify({
      version: 1,
  ...
  test("removeJournal cleans up", () => {
    ...
    const r = runScript(testDir, `
      ...
      const fs = require("fs");
      const path = require("path");
      console.log(JSON.stringify({ exists: fs.existsSync(path.join(process.env.CODEX_HOME, "nexcode-journal.json")) }));
    `);
    expect(r.status).toBe(0);
  ...
  test("removeCodexConfig is a successful no-op when Codex is not installed", () => {
    writeFileSync(join(testDir, "nexcode.config.toml"), 'openai_base_url = "http://127.0.0.1:10100/v1"\n', "utf8");
    ...
    expect(existsSync(join(testDir, "nexcode.config.toml"))).toBe(false);
  });
  ...
  test("removeCodexConfig reports damaged managed-default cleanup and preserves the ambiguous value", () => {
    ...
    expect(after).toContain("# Managed by nexcode: native subagent default");
  ...
  test("restoreNativeCodex restores an exact unchanged journal snapshot with managed defaults", () => {
    ...
    const result = JSON.parse(r.stdout);
    expect(result.success).toBe(true);
    expect(result.message).toContain("restored from nexcode journal");
    expect(readFileSync(join(testDir, "config.toml"), "utf8")).toBe(original);
    expect(existsSync(join(testDir, "nexcode-journal.json"))).toBe(false);
  });
  ...
  test("restoreNativeCodex reports damaged managed-default cleanup during fallback restore", () => {
    ...
    expect(after).toContain("# Managed by nexcode: native subagent default");
    expect(after).toContain('default_subagent_model = "gpt-5.6-sol"');
    expect(existsSync(join(testDir, "nexcode-journal.json"))).toBe(true);
  });
  ...
  test("restoreNativeCodex uses journal snapshot for normal stop without losing custom defaults", () => {
    ...
    writeFileSync(join(testDir, "nexcode.config.toml"), originalProfile, "utf8");
    ...
    const r = runScript(testDir, `
      ...
      fs.writeFileSync(path.join(process.env.CODEX_HOME, "nexcode.config.toml"), 'model_provider = "nexcode"\\n', "utf8");
    ...
    expect(readFileSync(join(testDir, "nexcode.config.toml"), "utf8")).toBe(originalProfile);
    expect(existsSync(join(testDir, "nexcode-journal.json"))).toBe(false);
  });
  ...
  test("restoreNativeCodex does not clobber user config edits made after injection", () => {
    ...
    expect(restored).not.toContain("[model_providers.nexcode]");
    expect(restored).not.toContain("Managed by nexcode: native subagent");
  ...
  test("full lifecycle: write → crash → reconcile restores", () => {
    ...
    writeFileSync(join(testDir, "config.toml"), "# injected nexcode config\n", "utf8");
  ...
  test("a stale journal is superseded once the config is native again (#477)", () => {
    const r = runScript(testDir, `
      ...
      (async () => {
        ...
        restoreNativeCodex();
        // Day four: the user installs a plugin while nexcode is not running.
    ...
    expect(recovered).not.toContain("[model_providers.nexcode]");
    expect(recovered).not.toContain("Auto-injected by nexcode");
  });
   ...
   * restore would then replay nexcode routing as if the user had written it.
   */
  test("re-injecting over an injected config never captures it as the original (#477)", () => {
    ...
    `);
    expect(r.status).toBe(0);
    const journal = JSON.parse(readFileSync(join(testDir, "nexcode-journal.json"), "utf8"));
  ...
  test("an injected config with no journal is never captured as the original (#477)", () => {
    ...
    const after = readFileSync(join(testDir, "config.toml"), "utf8");
    expect(after).not.toContain("[model_providers.nexcode]");
    expect(after).not.toContain("Auto-injected by nexcode");
  });

  /**
   * Documents why no PID-based transaction guard is needed. `nxc sync` and the
   * `nxc ensure` parent legitimately inject in a process that did not write the
