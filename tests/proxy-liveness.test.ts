...
const OURS = { status: "ok", service: "nexcode", version: "2.6.17", uptime: 12, pid: 4242, port: 10100 };

describe("isNexcodeHealthz", () => {
  test("accepts the explicit service marker", () => {
    expect(isNexcodeHealthz(OURS)).toBe(true);
  });

  test("accepts the legacy pre-identity body (still-running old proxy after update)", () => {
    expect(isNexcodeHealthz({ status: "ok", version: "2.6.16", uptime: 5 })).toBe(true);
  });

  test("rejects foreign bodies", () => {
    expect(isNexcodeHealthz(null)).toBe(false);
    expect(isNexcodeHealthz({ status: "ok" })).toBe(false);
    expect(isNexcodeHealthz({ service: "something-else", status: "ok", version: "1", uptime: 1 })).toBe(false);
    expect(isNexcodeHealthz({ healthy: true } as never)).toBe(false);
  });
});
...
describe("findLiveProxy", () => {
  ...
  test("an orphaned record backed by a pidless legacy proxy yields pid null (never a killable stale pid)", async () => {
    ...
    // The record's pid 1111 may be dead/reused — synthesizing it would let `nxc stop`
...
const VALID_BODY = { service: "nexcode", version: "2.6.17", uptime: 12, pid: 4242, port: 10100, status: "ready" as const };
...
describe("validateReadyzBody strict contract", () => {
  ...
  test("rejects missing version", () => {
    expect(validateReadyzBody({ service: "nexcode", uptime: 1, pid: 4242, port: 10100, status: "ready" }, 10100)).toBeNull();
  });
  ...
  test("rejects missing pid", () => {
    expect(validateReadyzBody({ service: "nexcode", version: "v", uptime: 1, port: 10100, status: "ready" }, 10100)).toBeNull();
  });
  ...
  test("rejects missing port", () => {
    expect(validateReadyzBody({ service: "nexcode", version: "v", uptime: 1, pid: 4242, status: "ready" }, 10100)).toBeNull();
  });
  ...
  test("rejects missing/malformed status (not a fixed enum value)", () => {
    expect(validateReadyzBody({ service: "nexcode", version: "v", uptime: 1, pid: 4242, port: 10100 }, 10100)).toBeNull();
  ...
  test("non-object / null bodies are rejected", () => {
    ...
    expect(validateReadyzBody("nexcode", 10100)).toBeNull();
...
const READY_BODY = { service: "nexcode", version: "2.6.17", uptime: 12, pid: 4242, port: 10100, status: "ready" };
const PENDING_BODY = { service: "nexcode", version: "2.6.17", uptime: 1, pid: 4242, port: 10100, status: "pending" };
const FAILED_BODY = { service: "nexcode", version: "2.6.17", uptime: 1, pid: 4242, port: 10100, status: "failed" };
...
describe("probeReadiness adversarial contract (never counts ready)", () => {
  ...
  test("rejects a body missing version", async () => {
    const probe = await probeReadiness(10100, {}, {
      fetchFn: (async () => readyz({ service: "nexcode", uptime: 1, pid: 4242, port: 10100, status: "ready" }, 200)) as typeof fetch,
    });
    expect(probe).toBeNull();
  });
  ...
  test("rejects a body missing pid", async () => {
    const probe = await probeReadiness(10100, {}, {
      fetchFn: (async () => readyz({ service: "nexcode", version: "v", uptime: 1, port: 10100, status: "ready" }, 200)) as typeof fetch,
    });
    expect(probe).toBeNull();
  });
  ...
  test("rejects a body missing port", async () => {
    const probe = await probeReadiness(10100, {}, {
      fetchFn: (async () => readyz({ service: "nexcode", version: "v", uptime: 1, pid: 4242, status: "ready" }, 200)) as typeof fetch,
    });
    expect(probe).toBeNull();
  });
