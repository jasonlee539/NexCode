...
import type { NxcContentPart, NxcMessage, NxcParsedRequest } from "../src/types";
...
function assistant(calls: Array<{ id: string; name: string }>): NxcMessage {
  return {
    role: "assistant",
...
function result(id: string, name: string, content: string | NxcContentPart[] = "ok"): NxcMessage {
  return {
    role: "toolResult",
    toolCallId: id,
    toolName: name,
...
async function wire(messages: NxcMessage[]): Promise<GeminiTurn[]> {
  const parsed = {
