...
const launcherSource = readFileSync(join(import.meta.dir, "..", "bin", "nxc.mjs"), "utf8");
...
describe("update stops the running proxy before replacing files", () => {
  ...
  test("npm launcher update path stops via its own launcher path before npm install", () => {
    ...
    expect(launcherSource).toContain('existsSync(join(configDir(), "nxc.pid"))');
  ...
  test("both paths abort when the stop fails, and REPAIR a managed service after success", () => {
    ...
    // parsing a failure message; bin/nxc.mjs is plain Node and cannot import
    ...
    expect(updateSource).toContain("NXC_BAKE_PORT");
    expect(launcherSource).toContain("NXC_BAKE_PORT");
  ...
  test("both update paths surface a skipped history restore after the stop", () => {
    // A codex-history-backup-*.json surviving `nxc stop` means the native-history restore
  ...
  test("GUI worker update children use pipe stdio so background updates do not open consoles", () => {
    ...
    expect(updateSource).toContain('process.env.NXC_SERVICE === "1"');
...
describe("nxc update --help has no side effects (#168)", () => {
...
describe("/healthz identity fields", () => {
  test("healthz advertises service identity, pid, and port", () => {
    expect(serverSource).toContain('service: "nexcode"');
