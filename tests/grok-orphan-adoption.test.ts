...
describe("Grok orphan adoption (#511)", () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "nxc-grok-orphan-"));
  ...
  test("does not adopt our api_key when the base_url is remote", () => {
    ...
    injectGrokConfig(10100, MODELS, { grokHome });
    expect(readFileSync(configPath, "utf8")).toContain("[model.nxc-remote]");
  });
  ...
  test("leaves an array-of-table model reserved", () => {
    ...
    injectGrokConfig(10100, MODELS, { grokHome });
    const content = readFileSync(configPath, "utf8");
    expect(content).toContain("[[model.nxc-arr]]");
  ...
  test("keeps an orphan whose model is no longer in the catalog", () => {
    ...
    injectGrokConfig(10100, MODELS, { grokHome });
    const content = readFileSync(configPath, "utf8");
    expect(content).toContain('default = "nxc-retired"');
  });
  ...
  test("still adopts safely when Grok has dropped the markers entirely", () => {
    writeFileSync(configPath, [
      ...
      'default = "nxc-gpt-5-6-sol"',
      "",
      "[model.nxc-gpt-5-6-sol]",            // stale: no context_window
      'model = "gpt-5.6-sol"',
      'base_url = "http://127.0.0.1:10100/v1"',
      'api_key = "nexcode-loopback"',
      "",
      "[model.nxc-gpt-5-6-sol-2]",          // the correct duplicate, also unfenced now
      'model = "gpt-5.6-sol"',
      'base_url = "http://127.0.0.1:10100/v1"',
      'api_key = "nexcode-loopback"',
      "context_window = 372000",
      "",
      "[model.nxc-gpt-5-6-sol-2.extra_headers]",
      'x-nexcode-grok = "1"',
      "",
    ...
    const content = readFileSync(configPath, "utf8");
    // Both nexcode duplicates collapse into the single regenerated entry.
    expect(modelTables(content).filter(alias => alias.startsWith("nxc-"))).toHaveLength(1);
    expect(content).toContain("context_window = 372000");
  ...
  test("adopts our own entries left on a port the proxy no longer listens on", () => {
    writeFileSync(configPath, [
      ...
      'default = "nxc-gpt-5-6-sol"',
      "",
      "[model.nxc-gpt-5-6-sol]",            // stale generation: dead port
      'model = "gpt-5.6-sol"',
      ...
      "context_window = 372000",
      "",
      "[model.nxc-gpt-5-6-sol.extra_headers]",
      'x-nexcode-grok = "1"',
      "",
    ...
    const content = readFileSync(configPath, "utf8");
    // No nexcode-owned entry may still point at the dead port.
    expect(content).not.toContain("127.0.0.1:4179/v1\"\napi_backend");
    expect(modelTables(content).filter(alias => alias.startsWith("nxc-"))).toHaveLength(1);
    // Its orphaned sub-table went with it, or the alias stays reserved forever.
    expect(content).not.toContain("[model.nxc-gpt-5-6-sol.extra_headers]");
...
describe("Grok orphan adoption — fence boundary (#511 follow-up)", () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "nxc-grok-fence-"));
    grokHome = join(root, ".grok");
    mkdirSync(grokHome);
    configPath = join(grokHome, "config.toml");
  });
  ...
  test("an orphan directly above the fence does not swallow the BEGIN marker", () => {
    writeFileSync(configPath, [
      ...
      'default = "nxc-gpt-5-6-sol"',
      "",
      ...orphan("nxc-gpt-5-6-sol"),
      "",
      ...fence("nxc-gpt-5-6-sol-2"),
      "",
    ].join("\n"));
    ...
    expect(tables(content)).toEqual(["nxc-gpt-5-6-sol"]);
    ...
    expect(content).toContain('default = "nxc-gpt-5-6-sol"');
    expect(content).not.toContain('default = "nxc-gpt-5-6-sol-2"');
  });
  ...
  test("a below-fence orphan still gets its sub-tables swept", () => {
    ...
    expect(content).not.toContain("nxc-gpt-5-6-sol-2");
  });
  ...
  test("a comment trailing an adopted orphan goes with it, and the sync converges", () => {
    ...
    expect(readFileSync(`${configPath}.bak-nexcode`, "utf8")).toContain("# this note trails the orphan");
  });
  ...
  test("adopting an orphan backs the user's config up first", () => {
    ...
    const backup = readFileSync(`${configPath}.bak-nexcode`, "utf8");
    expect(backup).toContain("[model.nxc-gpt-5-6-sol]");
    expect(backup).toContain("[model.nxc-gpt-5-6-sol-2]");
  });
});
