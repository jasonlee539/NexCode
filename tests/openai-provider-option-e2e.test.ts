...
describe("OpenAI provider-option integration spine", () => {
  test("keeps Pool, Direct, and API ownership stable across transports and management", async () => {
    const root = mkdtempSync(join(tmpdir(), "nxc-provider-option-e2e-"));
    ...
    const previousEnv = {
      NEXCODE_HOME: process.env.NEXCODE_HOME,
    ...
    try {
      for (const dir of [nexcodeHome, codexHome, claudeConfigDir]) {
      ...
      const directBaseline = {
        config: hashTree(join(nexcodeHome, "config.json")),
        accounts: hashTree(join(nexcodeHome, "codex-accounts.json")),
      ...
      expect(logs.some(row => row.provider === "openai-apikey"
        ...
        && row.resolvedModel === "gpt-5.6-sol")).toBe(true);
      const usageLines = existsSync(join(nexcodeHome, "usage.jsonl"))
        ? readFileSync(join(nexcodeHome, "usage.jsonl"), "utf8").trim().split("\n").filter(Boolean)
      ...
      const migrationRoot = mkdtempSync(join(tmpdir(), "nxc-provider-option-migration-"));
      try {
        const child = Bun.spawn([
          process.execPath,
          join(import.meta.dir, "fixtures/openai-provider-option-migration-child.ts"),
          join(migrationRoot, "nexcode"),
      ...
      const evidenceDir = process.env.NXC_EVIDENCE_DIR;
      if (evidenceDir) {
      ...
      try {
        ...
        restoreEnv("NEXCODE_HOME", previousEnv.NEXCODE_HOME);
