...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), "nxc-claude-hook-"));
...
describe("Claude Code shell-hook reconciliation", () => {
  ...
  test("removes only the stale NexCode hook when Claude Code is absent", () => {
    writeFileSync(zshrcPath, [
      "export USER_SETTING=1",
      "# nexcode claude-env hook",
      "[ -f ~/.nexcode/claude-env.sh ] && source ~/.nexcode/claude-env.sh",
      "alias keep-me='yes'",
      "",
    ].join("\n"));
    ...
    expect(content).not.toContain("nexcode claude-env hook");
  ...
  test("installs the hook only for an executable Claude Code CLI and stays idempotent", () => {
    ...
    expect(readFileSync(zshrcPath, "utf8").match(/nexcode claude-env hook/g)).toHaveLength(1);
  });
  ...
  test("removes the hook when system environment integration is inactive", () => {
    ...
    expect(readFileSync(zshrcPath, "utf8")).not.toContain("nexcode claude-env hook");
  });
  ...
  test("a marker block this pattern does not own is reported failed, not removed", () => {
    ...
    writeFileSync(zshrcPath, "# nexcode claude-env hook\n# hand-edited by the user\n", "utf8");
