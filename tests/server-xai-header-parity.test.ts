...
beforeEach(() => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("nxc-xai-parity-codex-");
  testDir = mkdtempSync(join(tmpdir(), "nxc-xai-parity-"));
...
function config(connectTimeoutMs = 1_000): NxcConfig {
  return {
    port: 0,
    hostname: "127.0.0.1",
    connectTimeoutMs,
