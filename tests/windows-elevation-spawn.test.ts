...
import {
  NXC_ELEVATED_CREATE_FAILED,
  NXC_ELEVATED_PROTOCOL_CODES,
  NXC_ELEVATED_PROTOCOL_FAILED,
  NXC_ELEVATED_RUN_FAILED_ROLLBACK_FAILED,
  NXC_ELEVATED_RUN_FAILED_ROLLED_BACK,
  NXC_ELEVATED_SUCCESS,
  NXC_ELEVATED_UAC_CANCELLED,
...
describe("runWindowsElevated spawn contract", () => {
  ...
  test("PowerShell script treats a missing ExitCode as protocol failure", async () => {
    ...
    expect(commandScript).toContain(`if ($null -eq $p.ExitCode) { exit ${NXC_ELEVATED_PROTOCOL_FAILED} }`);
  ...
  test("scheduled-task registration embeds immutable XML bytes instead of a file path", async () => {
    ...
    await expect(runWindowsElevatedScheduledTaskRegistration("nexcode-proxy", xml)).resolves.toBe(0);
  ...
  test("maps exit 1223 to cancelled", async () => {
    fakeChild({ code: NXC_ELEVATED_UAC_CANCELLED });
  ...
  test("request timeout does not kill the launcher; late close still settles once", async () => {
    ...
    child.emit("close", NXC_ELEVATED_SUCCESS, null);
    await lateWait;
    expect(late).toEqual({ exitCode: NXC_ELEVATED_SUCCESS, stdout: "", stderr: "" });
    child.emit("close", 1, null); // second close must not double-settle
    await Promise.resolve();
    expect(late?.exitCode).toBe(NXC_ELEVATED_SUCCESS);
  });
  ...
  test("startElevatedSchtasksCreateAndRun exposes completion after request race timeout", async () => {
    ...
    const started = startElevatedSchtasksCreateAndRun(
      "schtasks.exe",
      ["/create", "/tn", "nexcode-proxy", "/f"],
      ["/run", "/tn", "nexcode-proxy"],
      ["/delete", "/tn", "nexcode-proxy", "/f"],
    );
    const raced = await raceWithTimeout(started.completion, 20);
    expect(raced.status).toBe("timed-out");
    expect(child.kill).not.toHaveBeenCalled();
    child.emit("close", NXC_ELEVATED_CREATE_FAILED, null);
    await expect(started.completion).resolves.toMatchObject({
      outcome: "create-failed",
      exitCode: NXC_ELEVATED_CREATE_FAILED,
    });
  });
...
describe("elevated scheduler protocol codes", () => {
  test("reserved codes are unique and exclude UAC cancellation", () => {
    const set = new Set<number>(NXC_ELEVATED_PROTOCOL_CODES);
    expect(set.size).toBe(NXC_ELEVATED_PROTOCOL_CODES.length);
    expect(set.has(NXC_ELEVATED_UAC_CANCELLED)).toBe(false);
    expect(classifyElevatedSchedulerExitCode(NXC_ELEVATED_SUCCESS)).toBe("success");
    expect(classifyElevatedSchedulerExitCode(NXC_ELEVATED_CREATE_FAILED)).toBe("create-failed");
    expect(classifyElevatedSchedulerExitCode(NXC_ELEVATED_RUN_FAILED_ROLLED_BACK)).toBe("run-failed-rolled-back");
    expect(classifyElevatedSchedulerExitCode(NXC_ELEVATED_RUN_FAILED_ROLLBACK_FAILED)).toBe("run-failed-rollback-failed");
    expect(classifyElevatedSchedulerExitCode(NXC_ELEVATED_PROTOCOL_FAILED)).toBe("protocol-failed");
    ...
    expect(classifyElevatedSchedulerExitCode(NXC_ELEVATED_UAC_CANCELLED)).toBe("protocol-failed");
  });
});
...
describe("one-UAC create/run/rollback elevated script", () => {
  test("embeds create, run, and delete rollback without a second RunAs or temp file writes", () => {
    const script = buildElevatedSchtasksCreateAndRunScript(
      ...
      ["/create", "/tn", "nexcode-proxy", "/xml", "C:\\Users\\Jane Doe\\task.xml", "/f"],
      ["/run", "/tn", "nexcode-proxy"],
      ["/delete", "/tn", "nexcode-proxy", "/f"],
    );
    expect(script).toContain("Invoke-NxcSchtasks");
    expect(script).toContain(`exit ${NXC_ELEVATED_CREATE_FAILED}`);
    expect(script).toContain(`exit ${NXC_ELEVATED_SUCCESS}`);
    expect(script).toContain(`exit ${NXC_ELEVATED_RUN_FAILED_ROLLED_BACK}`);
    expect(script).toContain(`exit ${NXC_ELEVATED_RUN_FAILED_ROLLBACK_FAILED}`);
    ...
    expect(script).not.toMatch(/TEMP|tmpdir|nxc-elev/i);
  });
});
...
describe("finalizeWindowsSchedulerServiceRegistration", () => {
  ...
  test("successful create+run uses exactly one elevated launcher and writes install state", async () => {
    setFinalizeWindowsSchedulerHooksForTests({
      elevateCreateAndRun: async () => {
        elevateLaunches += 1;
        return { outcome: "success", exitCode: NXC_ELEVATED_SUCCESS, stdout: "", stderr: "" };
      },
      verify: okVerify,
      writeInstallState: () => { writeCount += 1; },
    });

    const result = await finalizeWindowsSchedulerServiceRegistration("C:\\Users\\x\\.nexcode\\nexcode-service.cmd");
  ...
  test("create failure does not write install state or parent-rollback", async () => {
    setFinalizeWindowsSchedulerHooksForTests({
      elevateCreateAndRun: async () => {
        elevateLaunches += 1;
        return { outcome: "create-failed", exitCode: NXC_ELEVATED_CREATE_FAILED, stdout: "", stderr: "" };
      },
      writeInstallState: () => { writeCount += 1; },
    });
    ...
    setFinalizeWindowsSchedulerHooksForTests({
      elevateCreateAndRun: async () => {
        ...
        return {
          ...
          exitCode: NXC_ELEVATED_RUN_FAILED_ROLLED_BACK,
          stdout: "",
          stderr: "",
        };
      },
    ...
    setFinalizeWindowsSchedulerHooksForTests({
      elevateCreateAndRun: async () => {
        ...
        return {
          ...
          exitCode: NXC_ELEVATED_RUN_FAILED_ROLLBACK_FAILED,
          stdout: "",
          stderr: "",
        };
      },
  ...
  test("late protocol-failed with unknown task probe stays blocked-partial and never writes state", async () => {
    ...
    resolveCompletion({
      outcome: "protocol-failed",
      exitCode: NXC_ELEVATED_PROTOCOL_FAILED,
      stdout: "",
      stderr: "",
    });
    if (result.kind !== "indeterminate") throw new Error("expected indeterminate");
  ...
  test("protocol-failed with task present attempts parent cleanup and does not write state", async () => {
    ...
    setFinalizeWindowsSchedulerHooksForTests({
      elevateCreateAndRun: async () => {
        elevateLaunches += 1;
        return { outcome: "protocol-failed", exitCode: NXC_ELEVATED_PROTOCOL_FAILED, stdout: "", stderr: "" };
      },
      writeInstallState: () => { writeCount += 1; },
      taskInstalled: () => {
        calls += 1;
  ...
  test("unknown WinSW status fails closed without claiming conflict and without rollback", async () => {
    setFinalizeWindowsSchedulerHooksForTests({
      ...
      verify: () => ({
        ...
        conflict: false,
        ok: false,
        detail: "The Task Scheduler task was created, but NexCode could not verify that the native WinSW service is absent.",
      }),
      writeInstallState: () => { writeCount += 1; },
    });
  ...
  function succeedingElevation() {
    return async () => {
      elevateLaunches += 1;
      return { outcome: "success" as const, exitCode: NXC_ELEVATED_SUCCESS, stdout: "", stderr: "" };
    };
  }
  ...
  test("runElevatedSchtasksCreateAndRun launches PowerShell once and classifies protocol exit", async () => {
    ...
    setWindowsElevationSpawnForTests((() => {
      ...
      child.stderr.setEncoding = () => undefined;
      child.kill = mock(() => true);
      queueMicrotask(() => child.emit("close", NXC_ELEVATED_SUCCESS, null));
      return child as never;
    }) as never);
    ...
    expect(result.exitCode).toBe(NXC_ELEVATED_SUCCESS);
  });
});
