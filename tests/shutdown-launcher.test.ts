...
/**
 * Regression: `nxc start` + Ctrl-C must NOT orphan the Bun proxy.
 *
 * The bin/nxc.mjs launcher used a blocking spawnSync that did not forward signals,
 * so a signal delivered only to the launcher killed it and left the Bun child
 * serving forever (port bound, nxc.pid/runtime-port.json left behind, Codex config
...
describe.skipIf(!runnable)("nxc launcher graceful shutdown", () => {
  for (const signal of ["SIGINT", "SIGTERM", "SIGHUP"] as const) {
    test(
      `${signal} to the launcher tears down the Bun proxy and restores Codex config (no orphan)`,
      async () => {
        const home = mkdtempSync(join(tmpdir(), "nxc-shutdown-"));
        tmpHomes.push(home);
        ...
        const child = spawn("node", [BIN_NXC, "start", "--port", String(port)], {
          stdio: "ignore",
          env: {
            ...process.env,
        ...
        expect(up).toBe(true);
        expect(existsSync(join(home, "nxc.pid"))).toBe(true);
        const injected = readFileSync(codexConfig, "utf8");
        expect(injected).toContain("# Auto-injected by nexcode");
        expect(injected).toContain(`openai_base_url = "http://127.0.0.1:${port}/v1"`);
        expect(injected).not.toContain("model_providers.nexcode");
        ...
        expect(existsSync(join(home, "nxc.pid"))).toBe(false);
        expect(existsSync(join(home, "runtime-port.json"))).toBe(false);
        expect(readFileSync(codexConfig, "utf8")).not.toContain("nexcode");
      },
      45_000,
    );
  }
