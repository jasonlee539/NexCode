...
import { MODEL_ADAPTER_OVERRIDE_ALLOWED, type NxcParsedRequest, type NxcProviderConfig } from "../src/types";
...
function providerFixture(adapterId: string, wire: AdapterWire): NxcProviderConfig {
  ...
  } satisfies NxcProviderConfig;
}

function prepareForWire(parsed: NxcParsedRequest, wire: AdapterWire): NxcParsedRequest {
  if (wire !== "kiro") return parsed;
  return { ...parsed, _kiroAuthContext: { apiRegion: "us-east-1" } };
}

function codeModeParsed(wire: AdapterWire): NxcParsedRequest {
...
function freeformParsed(wire: AdapterWire): NxcParsedRequest {
  return prepareForWire(parseRequest({
...
function namespacedCollisionParsed(wire: AdapterWire): NxcParsedRequest {
  return prepareForWire(parseRequest({
    model: WIRE_MODELS[wire],
...
function toolChoiceParsed(wire: AdapterWire, toolChoice?: "none"): NxcParsedRequest {
  return prepareForWire(parseRequest({
    model: WIRE_MODELS[wire],
...
function continuationParsed(wire: AdapterWire): NxcParsedRequest {
  return prepareForWire(parseRequest({
    model: WIRE_MODELS[wire],
    input: [
      {
...
async function outbound(adapterId: string, parsed: NxcParsedRequest): Promise<string> {
