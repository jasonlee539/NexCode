...
const PUBLIC_OAUTH_AUTHENTICATION_ERROR = "OAuth authentication failed. Check the NexCode account status and retry.";
const WINDOWS_PATH_CANARY = "C:\\Users\\Alice\\.nexcode\\auth.json.nxc-tmp";
const UNC_PATH_CANARY = "\\\\server\\share\\nexcode\\auth.json.nxc-tmp";
const POSIX_PATH_CANARY = "/home/alice/.nexcode/auth.json.nxc-tmp";
...
beforeEach(() => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("nxc-xai-401-codex-");
  testDir = mkdtempSync(join(tmpdir(), "nxc-xai-401-"));
...
function xaiConfig(authMode: "oauth" | "key" = "oauth"): NxcConfig {
  return {
    port: 0,
    hostname: "127.0.0.1",
    defaultProvider: "xai",
