...
import type { AdapterEvent, NxcConfig, NxcProviderConfig } from "../src/types";
import type { AdapterFetchContext, ProviderAdapter } from "../src/adapters/base";
import type { NxcMessage, NxcParsedRequest } from "../src/types";
...
const routedProvider: NxcProviderConfig = {
  adapter: "openai-chat",
...
function config(overrides: Partial<NxcConfig> = {}): NxcConfig {
  return {
    port: 10100,
...
describe("web-search sidecar planning", () => {
  test("canonical sidecar discovery defaults only an omitted OpenAI auth mode to forward", () => {
    const canonicalWithoutAuthMode: NxcConfig = {
      port: 10100,
  ...
  test("central Direct sidecar selection never treats a proxy admission bearer as Codex auth", async () => {
    ...
    const resolved = await resolveFirstUsableOpenAiSidecar(
      ...
      new Headers({ authorization: "Bearer proxy-secret", "x-nexcode-api-key": "proxy-secret" }),
      cfg,
    );
...
describe("web-search sidecar native web_search_call emission", () => {
  ...
  test("signed thinking before a web_search call survives into the replayed assistant turn", async () => {
    ...
    const seenBodies: NxcMessage[][] = [];
    let pass = 0;
    const adapter: ProviderAdapter = {
      name: "mock",
      buildRequest: (p: NxcParsedRequest) => {
...
function capturingAdapter(firstPass: AdapterEvent[]): { adapter: ProviderAdapter; messagesPerPass: NxcMessage[][] } {
  const messagesPerPass: NxcMessage[][] = [];
  let pass = 0;
  const adapter: ProviderAdapter = {
    name: "mock",
