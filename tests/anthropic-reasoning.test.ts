...
const provider = { adapter: "anthropic", baseUrl: "https://api.anthropic.com", apiKey: "sk-x", authMode: "apiKey" } as unknown as NxcProviderConfig;

function parsed(reasoning?: string, extraOpts: Record<string, unknown> = {}, modelId = "anthropic/claude-sonnet-4.5"): NxcParsedRequest {
  return {
    modelId,
    stream: false,
...
async function bodyOf(p: NxcParsedRequest, configuredProvider = provider): Promise<Record<string, unknown>> {
