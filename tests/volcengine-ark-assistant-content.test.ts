...
function providerFor(baseUrl: string): NxcProviderConfig {
...
function wire(provider: NxcProviderConfig, messages: NxcMessage[]): ChatMsg[] {
...
function assistantToolCall(): NxcMessage {
  return {
    role: "assistant",
...
describe("Volcengine Ark empty assistant content (#796)", () => {
  ...
  test("a reasoning-only assistant takes the same path", () => {
    ...
    const reasoningHistory: NxcMessage[] = [
      { role: "user", content: "hi", timestamp: 0 },
