 ...
 *
 * The defect: with `hostname: "0.0.0.0"`, every caller needs `x-nexcode-api-key`, but a
...
const wildcardConfig = {
  hostname: "0.0.0.0",
  apiKeys: [{ id: "k1", key: "nxc_data_realsecret", name: "test" }],
...
describe("loopback listener policy view", () => {
  ...
  test("a valid configured key is still attributed to that key, not collapsed to loopback", () => {
    ...
    expect(resolveResponsesApiAuth(
      request("/v1/responses", { "x-nexcode-api-key": "nxc_data_realsecret" }),
      wildcardConfig,
...
describe("injected Codex provider block", () => {
  ...
  test("enabling the loopback listener drops the header", () => {
    // The directly-spawned app-server has no NEXCODE_API_AUTH_TOKEN, so emitting the header
