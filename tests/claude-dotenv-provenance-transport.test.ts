...
/**
 * Project dotenv can write environment variables before NexCode evaluates,
...
describe("Node launcher context transport", () => {
  const dir = mkdtempSync(join(tmpdir(), "nxc-launch-context-"));
  ...
  writeFileSync(
    ...
    `import { initializeNodeLauncherContext } from ${JSON.stringify(moduleUrl)};\n`
      ...
      + "process.stdout.write(JSON.stringify({ context, args: process.argv.slice(2), contextEnv: process.env.NXC_NODE_LAUNCH_CONTEXT ?? null }));\n",
  );
  ...
  function run(args: string[], contextEnv: string | undefined) {
    ...
    if (contextEnv === undefined) delete env.NXC_NODE_LAUNCH_CONTEXT;
    else env.NXC_NODE_LAUNCH_CONTEXT = contextEnv;
  ...
  test("matching argv proof authenticates and consumes the parent snapshot", () => {
    const seen = run([`--nxc-internal-launch-proof=${proof}`, "claude"], context);
  ...
  test("duplicate internal proofs fail closed and are removed from user argv", () => {
    const seen = run([
      `--nxc-internal-launch-proof=${proof}`,
      `--nxc-internal-launch-proof=${proof}`,
      "claude",
    ], context);
    expect(seen.context).toBeNull();
