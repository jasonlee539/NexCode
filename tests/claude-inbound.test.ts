...
import { AnthropicRequestError, anthropicToResponsesBody, anthropicToResponsesTranslation, effortForThinkingBudget, extractNxcEffortDirective, resolveInboundModel } from "../src/claude/inbound";
...
describe("claude inbound translation", () => {
  ...
  test("Claude Code Auto Mode classifier routing uses only operator-declared targets (#1697)", () => {
    ...
    expect(resolveInboundModel("claude-opus-5", { model: "claude-nxc-RelayA--claude-fable-5" })).toBe("claude-opus-5");
...
describe("prompt cache key provenance (devlog 130 B3)", () => {
  ...
  test("[1m] strip works for both alias families before decode", () => {
    // Legacy claude-nxc-* (pure decode, no registry needed).
    expect(resolveInboundModel("claude-nxc-cursor--gpt-5.6-luna[1m]")).toBe("cursor/gpt-5.6-luna");
  });
});
...
describe("nxc-route directive (devlog 072)", () => {
  const { extractNxcRouteDirective } = require("../src/claude/inbound") as typeof import("../src/claude/inbound");

  test("extracts from string and block-array system; first directive wins", () => {
    expect(extractNxcRouteDirective({ system: "intro\n<!-- nxc-route: claude-nxc-native--gpt-5.6-sol[1m] -->\nrest" }))
      .toBe("claude-nxc-native--gpt-5.6-sol[1m]");
    expect(extractNxcRouteDirective({
      system: [
        { type: "text", text: "You are a delegated worker" },
        { type: "text", text: "<!-- nxc-route: gemini/gemini-3-pro --> and <!-- nxc-route: other -->" },
      ],
    })).toBe("gemini/gemini-3-pro");
  });

  test("extracts only supported generated-agent effort values", () => {
    expect(extractNxcEffortDirective({ system: "<!-- nxc-effort: max -->" })).toBe("max");
    expect(extractNxcEffortDirective({
      system: [{ type: "text", text: "<!-- nxc-effort: xhigh -->" }],
    })).toBe("xhigh");
    expect(extractNxcEffortDirective({ system: "<!-- nxc-effort: ultra -->" })).toBeNull();
  });

  test("absent or malformed directives return null", () => {
    expect(extractNxcRouteDirective({ system: "no directive here" })).toBeNull();
    expect(extractNxcRouteDirective({ system: [{ type: "text", text: "<!-- nxc-route: -->" }] })).toBeNull();
    expect(extractNxcRouteDirective({})).toBeNull();
    expect(extractNxcRouteDirective(null)).toBeNull();
    expect(extractNxcEffortDirective(null)).toBeNull();
  });
});
