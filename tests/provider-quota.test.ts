...
beforeEach(() => {
  nexcodeHome = mkdtempSync(join(tmpdir(), "nxc-quota-"));
...
describe("fetchProviderQuotaReports", () => {
  ...
  function kimiOnlyConfig(baseUrl = "https://api.kimi.com/coding/v1"): NxcConfig {
    return {
  ...
  function a6apiOnlyConfig(baseUrl = "https://api.a6api.com/v1"): NxcConfig {
    return {
  ...
  test("malformed API-key fields do not break unrelated quota reports", async () => {
    ...
    config.providers.broken = {
      ...
      apiKey: 42,
    } as unknown as NxcConfig["providers"][string];
  ...
  function keyQuotaConfig(name: string, baseUrl: string, apiKey = `${name}-secret`): NxcConfig {
    return {
    ...
    const result = await fetchProviderQuotaReports({
      ...
      providers: {
        "kimi-code": {
          ...
          authMode: "key",
          baseUrl: "https://api.kimi.com/coding/v1",
          apiKey: "${NXC_TEST_MISSING_KIMI_KEY}",
  ...
  test("direct mode reports main without reading or repairing the added-account store", async () => {
    const accountStore = join(nexcodeHome, "codex-accounts.json");
  ...
  function cursorOnlyConfig(): NxcConfig {
    return {
