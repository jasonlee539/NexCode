...
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), "nxc-probe-"));
  cleanup.push(home);
...
function writePlist(codexHome: string | null, nexcodeHome: string | null): string {
  const dir = join(home, "Library", "LaunchAgents");
  mkdirSync(dir, { recursive: true });
  const path = join(dir, "com.nexcode.proxy.plist");
  writeFileSync(path, [
    "<plist><dict><key>EnvironmentVariables</key><dict>",
    codexHome ? `<key>CODEX_HOME</key><string>${codexHome}</string>` : "",
    nexcodeHome ? `<key>NEXCODE_HOME</key><string>${nexcodeHome}</string>` : "",
    "</dict></dict></plist>",
  ].join("\n"));
  return path;
}

function writeUnit(codexHome: string, nexcodeHome: string): string {
  const dir = join(home, ".config", "systemd", "user");
  mkdirSync(dir, { recursive: true });
  const path = join(dir, "nexcode-proxy.service");
  writeFileSync(path, [
    "[Service]",
...
describe("the probe only ever asks", () => {
  ...
  test("macOS asks launchctl only with print, in both domains", () => {
    ...
    expect(calls.map(c => c.args[1])).toEqual([
      "gui/501/com.nexcode.proxy",
      "user/501/com.nexcode.proxy",
    ]);
...
describe("absence has to be proven twice", () => {
  ...
  test("no registration but a definition on disk is NOT absent", () => {
    const path = writePlist("/somewhere/.codex", "/somewhere/.nexcode");
    const { run } = recorder(() => ({ status: 113 }));
    const result = inspectServiceManagerInstallation({ run, platform: "darwin", uid: 501, home });
    ...
    expect(result.claims[0].homes).toEqual({ codexHome: "/somewhere/.codex", nexcodeHome: "/somewhere/.nexcode" });
  });
...
describe("could not ask is not an answer", () => {
  ...
  test("exit 112 WITH a plist staged is unknown", () => {
    ...
    writeFileSync(join(home, "Library", "LaunchAgents", "com.nexcode.proxy.plist"), "<plist/>");
    const { run } = recorder(() => ({ status: 112, stderr: "Could not find domain for user" }));
    const result = inspectServiceManagerInstallation({ run, platform: "darwin", uid: 501, home });
  ...
  const canSymlink = (() => {
    const dir = mkdtempSync(join(tmpdir(), "nxc-symlink-probe-"));
    try {
  ...
  test.skipIf(!canSymlink)("a dangling plist symlink does not read as a clean machine", () => {
    ...
    symlinkSync(join(home, "nothing-here.plist"), join(agents, "com.nexcode.proxy.plist"));
    expect(existsSync(join(agents, "com.nexcode.proxy.plist"))).toBeFalse();
  ...
  test("a non-zero systemctl status is unknown even though a missing unit exits zero", () => {
    ...
    const { run } = recorder(() => ({ status: 1, stderr: "Job for nexcode-proxy.service failed" }));
...
describe("a definition that cannot supply homes is not present", () => {
  test("an unreadable plist is unknown", () => {
    ...
    mkdirSync(join(dir, "com.nexcode.proxy.plist"));
    const { run } = recorder(() => ({ status: 113 }));
    expect(inspectServiceManagerInstallation({ run, platform: "darwin", uid: 501, home }).kind).toBe("unknown");
  });
  ...
  test("an omitted home is null, not an empty string", () => {
    writePlist(null, "/somewhere/.nexcode");
    const { run } = recorder(() => ({ status: 113 }));
    const result = inspectServiceManagerInstallation({ run, platform: "darwin", uid: 501, home });
    expect(result.kind).toBe("present");
    if (result.kind !== "present") return;
    expect(result.claims[0].homes.codexHome).toBeNull();
    expect(result.claims[0].homes.nexcodeHome).toBe("/somewhere/.nexcode");
  });
});

describe("the Windows chain walk", () => {
  function writeWindowsTask(launcherPath: string): string {
    const dir = join(home, ".nexcode");
    mkdirSync(dir, { recursive: true });
    const path = join(dir, "nexcode-service-task.xml");
  ...
  function writeWindowsLauncher(wrapperPath: string): string {
    const path = join(home, ".nexcode", "nexcode-service-launcher.vbs");
    mkdirSync(join(home, ".nexcode"), { recursive: true });
    writeFileSync(path, [
  ...
  function writeWindowsWrapper(codexHome?: string, nexcodeHome?: string): string {
    const path = join(home, ".nexcode", "nexcode-service.cmd");
    mkdirSync(join(home, ".nexcode"), { recursive: true });
    const lines = ["@echo off", "setlocal"];
    if (codexHome) lines.push(`set "CODEX_HOME=${codexHome}"`);
    if (nexcodeHome) lines.push(`set "NEXCODE_HOME=${nexcodeHome}"`);
    ...
    lines.push(
      'set "NXC_BUN=C:\\bun\\bun.exe"',
      'set "NXC_CLI=C:\\nexcode\\src\\cli\\index.ts"',
      ":loop",
      '>>"%NXC_SERVICE_LOG%" echo start',
      '"%NXC_BUN%" "%NXC_CLI%" start --port 10100',
    );
  ...
  test("the full chain is walked and the homes are extracted", () => {
    const wrapper = writeWindowsWrapper("C:\\Users\\ws\\.codex", "C:\\Users\\ws\\.nexcode");
    ...
    expect(result.claims[0].homes).toEqual({
      ...
      nexcodeHome: "C:\\Users\\ws\\.nexcode",
    });
    ...
    expect(calls[0].args).toEqual(["/query", "/tn", "nexcode-proxy", "/xml"]);
  });
  ...
  test("a registered task whose chain disagrees with the staged definition is unknown", () => {
    ...
    const foreignWrapper = join(home, ".nexcode", "foreign-wrapper.cmd");
    ...
    const foreignLauncher = join(home, ".nexcode", "foreign-launcher.vbs");
  ...
  test("batch env-token homes are decoded before they are compared", () => {
    ...
    const dir = join(home, ".nexcode");
    mkdirSync(dir, { recursive: true });
    const wrapper = join(dir, "nexcode-service.cmd");
    writeFileSync(wrapper, [
      "@echo off",
      "setlocal",
      'set "CODEX_HOME=%USERPROFILE%\\.codex"',
      'set "NEXCODE_HOME=%%PROFILE_VAR%%\\custom"',
      'set "NXC_BUN=C:\\bun\\bun.exe"',
      'set "NXC_CLI=C:\\nexcode\\src\\cli\\index.ts"',
      ":loop",
      '"%NXC_BUN%" "%NXC_CLI%" start --port 10100',
    ...
    expect(result.claims[0].homes.nexcodeHome).toBe(`%PROFILE_VAR%\\custom`);
  });
  ...
  test("escaped and lowercase env tokens decode correctly with a controlled env", () => {
    ...
    writeFileSync(wrapper, [
      ...
      'set "NEXCODE_HOME=%userprofile%\\lower"',
      'set "NXC_BUN=C:\\bun\\bun.exe"',
      'set "NXC_CLI=C:\\nexcode\\src\\cli\\index.ts"',
      ":loop",
      '"%NXC_BUN%" "%NXC_CLI%" start --port 10100',
    ...
    expect(result.claims[0].homes.nexcodeHome).toBe(`${process.env.USERPROFILE ?? ""}\\lower`);
  });
  ...
  test("scheduler assets are located under the effective NEXCODE_HOME", () => {
    // A decoy chain in the DEFAULT <home>/.nexcode location must NOT win over
    // the customized NEXCODE_HOME passed as configDir.
    const decoyWrapper = writeWindowsWrapper("C:\\decoy\\.codex", "C:\\decoy\\.nexcode");
    const decoyLauncher = writeWindowsLauncher(decoyWrapper);
    writeWindowsTask(decoyLauncher);

    // The real custom config dir (customized NEXCODE_HOME) holds the actual chain.
    const custom = join(home, "custom-home");
    const wrapper = join(custom, "nexcode-service.cmd");
    ...
    const launcher = join(custom, "nexcode-service-launcher.vbs");
    writeFileSync(launcher, `shell.Run """${wrapper}""", 0, True\r\n`);
    writeFileSync(join(custom, "nexcode-service-task.xml"), [
      '<?xml version="1.0" encoding="UTF-16"?>',
  ...
  test("a malformed wrapper is unknown, not a deliberate omission", () => {
    ...
    expect(result.kind === "unknown" && result.reason)
      .toContain("the launcher wrapper does not look like a generated nexcode service wrapper");
  });
  ...
  test("a staged task that was never registered is present but registration=absent", () => {
    const wrapper = writeWindowsWrapper("C:\\a\\.codex", "C:\\a\\.nexcode");
    const launcher = writeWindowsLauncher(wrapper);
    writeWindowsTask(launcher);
    const { runRaw } = recorder(() => ({ status: 1, stderr: "ERROR: The system cannot find the file specified." }));
  ...
  test("a UTF-16LE task XML on disk is decoded before the chain is walked", () => {
    ...
    writeFileSync(join(home, ".nexcode", "nexcode-service-task.xml"), Buffer.from(`\uFEFF${xml}`, "utf16le"));
    const { runRaw } = recorder(() => ({ status: 1, stderr: "ERROR: The system cannot find the file specified." }));

    const result = inspectServiceManagerInstallation({ platform: "win32", home, runRaw, winswStatus: () => "nonexistent" });
    expect(result.kind).toBe("present");
    if (result.kind !== "present") return;
    expect(result.claims[0].homes).toEqual({ codexHome: "C:\\a\\.codex", nexcodeHome: "C:\\a\\.nexcode" });
  });
  ...
  test("a UTF-16LE VBS launcher is decoded before the chain is walked", () => {
    ...
    const launcher = join(home, ".nexcode", "nexcode-service-launcher.vbs");
    writeFileSync(launcher, Buffer.from(
      `\uFEFF' launcher\r\nSet shell = CreateObject("WScript.Shell")\r\nshell.Run """${wrapper}""", 0, True\r\n`,
      "utf16le",
    ));
    const xml = join(home, ".nexcode", "nexcode-service-task.xml");
    writeFileSync(xml, [
      '<?xml version="1.0" encoding="UTF-16"?>',
      `<Arguments>/b /nologo &quot;${launcher.replace(/&/g, "&amp;").replace(/"/g, "&quot;")}&quot;</Arguments>`,
    ].join("\n"));
  ...
  test("a registered task whose launcher is missing is unknown", () => {
    ...
    const missingLauncher = join(home, ".nexcode", "no-such.vbs");
  ...
  function writeWinswXml(dir: string, codexHome: string, nexcodeHome: string): void {
    // The probe resolves the winsw dir under the effective config dir
    // (default <home>/.nexcode/winsw), matching winswDir() in src/lib/winsw.ts.
    const winswDir = join(dir, ".nexcode", "winsw");
    mkdirSync(winswDir, { recursive: true });
    // The exe must be present for the SCM claim to be verifiable (fail-closed).
    writeFileSync(join(winswDir, "nexcode-proxy-native.exe"), "placeholder");
    writeFileSync(join(winswDir, "nexcode-proxy-native.xml"), [
      '<?xml version="1.0" encoding="UTF-8"?>',
      "<service>",
      "  <id>nexcode-proxy-native</id>",
      `  <env name="CODEX_HOME" value="${codexHome}"/>`,
      `  <env name="NEXCODE_HOME" value="${nexcodeHome}"/>`,
  ...
  test("WinSW and Task Scheduler both present is a conflict", () => {
    ...
    const launcher = writeWindowsLauncher(wrapper);
    writeWindowsTask(launcher);
    writeWinswXml(home, "C:\\winsw\\.codex", "C:\\winsw\\.nexcode");
    ...
    expect(result.claims[1].homes).toEqual({ codexHome: "C:\\a\\.codex", nexcodeHome: "C:\\a\\.nexcode" });
  });
  ...
  test("a broken scheduler chain alongside WinSW is unknown, not a fabricated conflict", () => {
    ...
    writeWindowsTask(join(home, ".nexcode", "no-such.vbs"));
    writeWinswXml(home, "C:\\winsw\\.codex", "C:\\winsw\\.nexcode");
    const { runRaw } = recorder(() => ({ status: 0, stdout: windowsTaskXmlFor(join(home, ".nexcode", "no-such.vbs")) }));
  ...
  test("WinSW homes are parsed from the XML env entries", () => {
    ...
    if (result.kind !== "present") return;
    expect(result.claims[0].backend).toBe("winsw");
    expect(result.claims[0].homes).toEqual({ codexHome: "C:\\winsw\\.codex", nexcodeHome: "C:\\winsw\\.nexcode" });
  });
  ...
  test("WinSW XML present but exe missing is unknown (fail closed)", () => {
    ...
    const winswDir = join(home, ".nexcode", "winsw");
    mkdirSync(winswDir, { recursive: true });
    writeFileSync(join(winswDir, "nexcode-proxy-native.xml"), [
      '<?xml version="1.0" encoding="UTF-8"?>',
      "<service>",
...
describe("ownership refuses what it cannot prove", () => {
  ...
  function own(extra: { run: ProbeRunner }) {
    ...
    const nexcodeHome = join(home, ".nexcode");
    return {
      ...extra,
      platform: "darwin" as const,
      uid: 501,
      home,
      statePaths: [join(nexcodeHome, "service-state.json")],
      currentHomes: { codexHome, nexcodeHome },
    };
  }

  function useHomes(): { codexHome: string; nexcodeHome: string } {
    const codexHome = join(home, ".codex");
    const nexcodeHome = join(home, ".nexcode");
  ...
  function writeState(dir: string, codexHome: string, nexcodeHome: string): void {
    writeFileSync(join(dir, "service-state.json"), JSON.stringify({
      version: 1, codexHome, nexcodeHome,
    }));
  }
  ...
  test("state naming another home is foreign", () => {
    const { nexcodeHome } = useHomes();
    writeState(nexcodeHome, "/elsewhere/.codex", "/elsewhere/.nexcode");
    const { run } = recorder(() => ({ status: 113 }));
  ...
  test("state says here, definition says elsewhere — unknown, not owned", () => {
    const { codexHome, nexcodeHome } = useHomes();
    writeState(nexcodeHome, codexHome, nexcodeHome);
    writePlist("/elsewhere/.codex", "/elsewhere/.nexcode");
    const { run } = recorder(() => ({ status: 113 }));
  ...
  test("state and definition agreeing is owned", () => {
    const { codexHome, nexcodeHome } = useHomes();
    writeState(nexcodeHome, codexHome, nexcodeHome);
    writePlist(codexHome, nexcodeHome);
    const { run } = recorder(() => ({ status: 113 }));
    expect(inspectNativeCodexOwnership(own({ run })).ownership).toBe("owned");
  });
  ...
  test("a malformed state file is unknown, where the teardown helper says fine", () => {
    const { nexcodeHome } = useHomes();
    writeFileSync(join(nexcodeHome, "service-state.json"), "{ not json");
    const { run } = recorder(() => ({ status: 113 }));
    const result = inspectNativeCodexOwnership(own({ run }));
    expect(result.ownership).toBe("unknown");
  ...
  test("a state backend that disagrees with the installed manager is unknown", () => {
    ...
    writeFileSync(join(nexcodeHome, "service-state.json"), JSON.stringify({
      version: 2, codexHome, nexcodeHome, backend: "native",
    }));
    // A scheduler claim whose homes agree with the state.
    const wrapper = join(nexcodeHome, "nexcode-service.cmd");
    writeFileSync(wrapper, [
      "@echo off",
      "setlocal",
      `set "CODEX_HOME=${codexHome}"`,
      `set "NEXCODE_HOME=${nexcodeHome}"`,
      'set "NXC_BUN=C:\\bun\\bun.exe"',
      'set "NXC_CLI=C:\\nexcode\\src\\cli\\index.ts"',
      ":loop",
      '"%NXC_BUN%" "%NXC_CLI%" start --port 10100',
    ].join("\r\n"));
    const launcher = join(nexcodeHome, "nexcode-service-launcher.vbs");
    writeFileSync(launcher, `shell.Run """${wrapper}""", 0, True\r\n`);
    writeFileSync(join(nexcodeHome, "nexcode-service-task.xml"), [
      '<?xml version="1.0" encoding="UTF-16"?>',
      `<Arguments>/b /nologo &quot;${launcher}&quot;</Arguments>`,
    ].join("\n"));
  ...
  test("a state backend that agrees with the installed manager is owned", () => {
    ...
    mkdirSync(nexcodeHome, { recursive: true });
    writeFileSync(join(nexcodeHome, "service-state.json"), JSON.stringify({
      version: 2, codexHome, nexcodeHome, backend: "scheduler",
    }));
    const wrapper = join(nexcodeHome, "nexcode-service.cmd");
    writeFileSync(wrapper, [
      "@echo off",
...
describe("systemd probe: the bus is unreachable (#2114)", () => {
  ...
  test("a unit naming THIS home is still ours, read off disk", () => {
    const definitionPath = writeUnit(join(home, ".codex"), join(home, ".nexcode"));
    const { run } = recorder(() => ({ status: 1, stderr: BUS_DOWN }));
  ...
  test("a unit naming a FOREIGN home still blocks", () => {
    writeUnit("/other/.codex", "/other/.nexcode");
    const { run } = recorder(() => ({ status: 1, stderr: BUS_DOWN }));
...
/*
 * #2108: a reboot leaves native-main fenced until `nxc restart`.
 *
...
describe("WinSW probe: a timed-out query with no assets (#2108)", () => {
  ...
  test("an unaskable query with WinSW assets present is still unknown", () => {
    const dir = join(home, ".nexcode", "winsw");
    mkdirSync(dir, { recursive: true });
    writeFileSync(join(dir, "nexcode-proxy.xml"), "<service><id>nexcode-proxy</id></service>");
    writeFileSync(join(dir, "nexcode-proxy.exe"), "MZ");
...
describe("bus-down absence must mean absence everywhere systemd looks (#2114)", () => {
  ...
  test("a foreign unit in the other user search dir still blocks", () => {
    ...
    mkdirSync(dir, { recursive: true });
    writeFileSync(join(dir, "nexcode-proxy.service"), [
      "[Service]",
      'Environment="CODEX_HOME=/other/.codex"',
      'Environment="NEXCODE_HOME=/other/.nexcode"',
    ].join("\n"));
    const { run } = recorder(() => ({ status: 1, stderr: BUS_DOWN }));
  ...
  test("XDG_CONFIG_HOME is honored the way systemd honors it", () => {
    ...
    mkdirSync(dir, { recursive: true });
    writeFileSync(join(dir, "nexcode-proxy.service"), '[Service]\nEnvironment="CODEX_HOME=/other/.codex"');
  ...
  test("two units in different search dirs refuse rather than choose", () => {
    ...
    writeFileSync(join(a, "nexcode-proxy.service"), `[Service]\nEnvironment="CODEX_HOME=${join(home, ".codex")}"`);
    writeFileSync(join(b, "nexcode-proxy.service"), '[Service]\nEnvironment="CODEX_HOME=/other/.codex"');
    const { run } = recorder(() => ({ status: 1, stderr: BUS_DOWN }));
