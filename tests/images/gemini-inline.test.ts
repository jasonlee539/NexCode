...
import type { AdapterEvent, NxcProviderConfig } from "../../src/types";
...
beforeAll(() => {
  savedHome = process.env.NEXCODE_HOME;
  tempHome = mkdtempSync(join(tmpdir(), "nxc-test-"));
  process.env.NEXCODE_HOME = tempHome;
  artifactsDir = join(tempHome, "artifacts");
});

afterAll(() => {
  if (savedHome !== undefined) process.env.NEXCODE_HOME = savedHome;
  else delete process.env.NEXCODE_HOME;
...
async function collectStream(provider: NxcProviderConfig, chunks: unknown[]): Promise<AdapterEvent[]> {
...
const aiStudioProvider = { adapter: "google", baseUrl: "https://generativelanguage.googleapis.com", apiKey: "key" } as NxcProviderConfig;
...
describe("markdown emits authenticated opaque artifact URLs", () => {
  test("streaming: markdown uses /v1/nexcode/artifacts/<id>, not file: or host paths", async () => {
    const events = await collectStream(aiStudioProvider, [
      { candidates: [{ content: { parts: [{ inlineData: { mimeType: "image/png", data: TINY_PNG } }] } }] },
      { candidates: [{ finishReason: "STOP" }], usageMetadata: { promptTokenCount: 1, candidatesTokenCount: 1 } },
    ]);
    ...
    expect(mdPath.startsWith("/v1/nexcode/artifacts/")).toBe(true);
    ...
    const id = mdPath.slice("/v1/nexcode/artifacts/".length);
    expect(existsSync(join(artifactsDir, id))).toBe(true);
  });

  test("non-streaming: markdown uses /v1/nexcode/artifacts/<id>, not file: or host paths", async () => {
    const adapter = createGoogleAdapter(aiStudioProvider);
    const events = await adapter.parseResponse(jsonResponse({
...
describe("artifact markdown never leaks NEXCODE_HOME paths", () => {
  let underHome: string;
  let savedHome: string | undefined;

  beforeAll(() => {
    savedHome = process.env.NEXCODE_HOME;
    underHome = mkdtempSync(join(homedir(), ".nxc-test-leak-"));
    process.env.NEXCODE_HOME = underHome;
  });
  ...
  test("streaming: no home/username path segments in markdown", async () => {
    ...
    expect(md).toContain("/v1/nexcode/artifacts/");
