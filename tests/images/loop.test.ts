...
import type { AdapterEvent, NxcParsedRequest } from "../../src/types";
...
function makeParsed(): NxcParsedRequest {
  return { modelId: "test-model", context: { messages: [], tools: [] }, stream: true, options: {} } as NxcParsedRequest;
}
...
describe("runWithImageBridge — runTurn adapter", () => {
  ...
  const runTurnAdapter: ProviderAdapter = {
    ...mockAdapter,
    runTurn: async (_parsed: NxcParsedRequest, _incoming: IncomingMeta, emit: (e: AdapterEvent) => void) => {
