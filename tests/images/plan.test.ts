...
import type { NxcConfig, NxcProviderConfig, NxcParsedRequest } from "../../src/types";

const PREV_HOME = process.env.NEXCODE_HOME;
...
beforeAll(async () => {
  process.env.NEXCODE_HOME = join(tmpdir(), "nxc-test-" + randomUUID());
...
afterAll(() => { if (PREV_HOME === undefined) delete process.env.NEXCODE_HOME; else process.env.NEXCODE_HOME = PREV_HOME; mock.restore(); });
...
function makeConfig(
  providers: Record<string, Partial<NxcProviderConfig>>,
...
function makeParsed(withImageGen: boolean): NxcParsedRequest {
  return {
    modelId: "test-model",
...
const routed = { adapter: "openai-chat", baseUrl: "https://api.anthropic.com" } as NxcProviderConfig;
const openaiRouted = { adapter: "openai-chat", baseUrl: "https://api.openai.com" } as NxcProviderConfig;
