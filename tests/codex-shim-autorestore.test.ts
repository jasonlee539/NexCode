...
describe("Codex shim CLI auto-restore policy", () => {
  ...
  test("restore failure -> warning only, command succeeds", () => {
    ...
    expect(warnings[0]).toContain("nxc codex-shim install");
  });
  ...
  test("opt-out config and environment are evaluated lazily only for a candidate", () => {
    for (const [configValue, env] of [[false, {}], [true, { NEXCODE_CODEX_SHIM_AUTO_RESTORE: "0" }]] as const) {
  ...
  test("oversized shim state is bounded, skipped, and warned without loading config", () => {
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-oversized-state-"));
  ...
  test("shim replaced -> next nxc command auto-restores and warns", async () => {
    if (process.platform === "win32") return;
    const binDir = mkdtempSync(join(tmpdir(), "nxc-shim-activation-bin-"));
    const home = mkdtempSync(join(tmpdir(), "nxc-shim-activation-home-"));
    const wrapper = join(binDir, "codex");
    const backup = join(binDir, "codex.nexcode-real");
    ...
    try {
      ...
      const result = spawnSync(process.execPath, [join(import.meta.dir, "..", "src", "cli", "index.ts"), "codex-shim", "status"], {
        encoding: "utf8",
        env: { ...process.env, PATH: binDir, NEXCODE_HOME: home },
      });
