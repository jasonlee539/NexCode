 ...
 * `nexcode_capability_provenance` block instead of the compatibility-shaped
...
beforeEach(() => {
  ...
  codexHome = mkdtempSync(join(tmpdir(), "nxc-capability-catalog-"));
...
function writeCatalog(models: CatalogModel[]): void {
  ...
  writeFileSync(join(codexHome, "nexcode-catalog.json"), JSON.stringify({ models: entries }));
}

function configFor(providers: string[]): NxcConfig {
  ...
  return { providers: Object.fromEntries(entries) } as unknown as NxcConfig;
}
...
describe("catalog-sourced capability evidence (#1796)", () => {
  ...
  test("synthesized combo rows are not stamped with capability provenance", () => {
    ...
    expect(entry.nexcode_capability_provenance).toBeUndefined();
...
describe("provenance sources beyond the CatalogModel (#1796)", () => {
  test("captures generated metadata for an identity-only model", () => {
    ...
    const provenance = entry.nexcode_capability_provenance as Record<string, unknown> | undefined;
