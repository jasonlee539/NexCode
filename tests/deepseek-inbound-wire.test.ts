...
function deepseekProvider(): NxcProviderConfig {
  return { ...providerConfigSeed(getProviderRegistryEntry("deepseek")!), apiKey: "sk-test" };
}

function deepseekReasoningProvider(): NxcProviderConfig {
...
describe("the inbound scope survives the handleResponses replay", () => {
  ...
  async function drive(
    ...
    const config = { providers: { deepseek: deepseekProvider() } } as unknown as NxcConfig;
  ...
  test("a streamed DeepSeek turn repairs UUID item ids on the live SSE path (#938)", async () => {
    ...
    expect(text).toMatch(/"id":"msg_nxc_[0-9a-f]+/);
    expect(text).toMatch(/"id":"rs_nxc_[0-9a-f]+/);
    expect(text).toContain("data: [DONE]");
  });
  ...
  test("streaming terminal repair composes with canonical item-id repair", async () => {
    ...
    try {
      ...
      expect(reasoningId).toMatch(/^rs_nxc_/);
      expect(messageId).toMatch(/^msg_nxc_/);
...
describe("the bounded-JSON mechanism stays alive behind a synthetic registry entry", () => {
  ...
  function fixtureProvider(overrides?: Partial<NxcProviderConfig>): NxcProviderConfig {
    return {
      adapter: "openai-responses",
      baseUrl: FIXTURE_BASE,
      authMode: "key",
  ...
  function repairingFixtureProvider(): NxcProviderConfig {
    return fixtureProvider({
      responsesItemIdRepair: { message: ["msg_placeholder"], reasoning: ["rs_placeholder"] },
    } as Partial<NxcProviderConfig>);
  }
  ...
  async function driveFixture(
    ...
    const config = { providers: { [FIXTURE_ID]: provider } } as unknown as NxcConfig;
  ...
  test("the synthesized terminal SSE carries repaired item ids, not the upstream placeholders", async () => {
    ...
    expect(text).toMatch(/"id":"msg_nxc_[0-9a-f]{8}/);
    expect(text).toMatch(/"id":"rs_nxc_[0-9a-f]{8}/);
  });
...
describe("stateless Responses upstreams get no stateful parameters", () => {
  function buildBody(provider: NxcProviderConfig, rawBody: Record<string, unknown>): Record<string, unknown> {
