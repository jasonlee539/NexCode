...
import type { NxcConfig, NxcParsedRequest, NxcProviderConfig } from "../src/types";

let home: string;

function makeConfig(provider: Partial<NxcProviderConfig>): NxcConfig {
  return {
    port: 10199,
    defaultProvider: "p",
    providers: {
...
function pool3(): NxcProviderConfig["apiKeyPool"] {
  return [
...
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), "nxc-keyfailover-"));
...
describe("hasKeyPoolFailover", () => {
  test("true only for key-auth providers with 2+ pool entries", () => {
    expect(hasKeyPoolFailover({ adapter: "openai-chat", baseUrl: "x", apiKeyPool: pool3() } as NxcProviderConfig)).toBe(true);
    expect(hasKeyPoolFailover({ adapter: "openai-chat", baseUrl: "x", apiKeyPool: [pool3()![0]] } as NxcProviderConfig)).toBe(false);
    expect(hasKeyPoolFailover({ adapter: "openai-chat", baseUrl: "x" } as NxcProviderConfig)).toBe(false);
    expect(hasKeyPoolFailover({ adapter: "anthropic", baseUrl: "x", authMode: "oauth", apiKeyPool: pool3() } as NxcProviderConfig)).toBe(false);
    expect(hasKeyPoolFailover({ adapter: "openai-responses", baseUrl: "x", authMode: "forward", apiKeyPool: pool3() } as NxcProviderConfig)).toBe(false);
  });
});
