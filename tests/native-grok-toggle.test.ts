...
const BEGIN = "# >>> nexcode managed block — do not edit (removed by `nxc stop`) >>>";
const END = "# <<< nexcode managed block <<<";
...
function writeForeignInstallState(): void {
  writeFileSync(join(fixtureRoot, "service-state.json"), JSON.stringify({
    ...
    nexcodeHome: "/foreign/nexcode-home",
    backend: "scheduler",
  }));
}

function baseConfig(overrides: Partial<NxcConfig> = {}): NxcConfig {
  return { port: 10100, hostname: "127.0.0.1", providers: [], ...overrides } as NxcConfig;
}
...
function dispatch(config: NxcConfig, path: string, init?: RequestInit, deps: ManagementApiDeps = testDeps()) {
...
async function put(config: NxcConfig, enabled: boolean, deps?: ManagementApiDeps) {
...
async function get(config: NxcConfig, deps?: ManagementApiDeps) {
...
test("enable regenerates the fence with the catalog's aliases", async () => {
  ...
  // The writer allocates its own `nxc-`-prefixed alias and carries the aliased
...
test("enable re-inspects AFTER the catalog fetch (audit r7)", async () => {
  ...
  const deps = testDeps({
    fetchAllModels: (async () => {
      // The file becomes orphaned INSIDE the awaiting window — by `nxc
...
test("a foreign fence between strip and read is superseded, never absent (audit r9)", async () => {
  ...
  const deps = nonLoopbackDeps({
    injectGrokConfig: (() => {
      // `nxc ensure` regenerated a well-formed fence in the window.
      writeConfig(fencedConfig());
      return { ok: true, changed: true, message: "policy skip", skippedReason: "non-loopback" } as const;
    }) as never,
  });
...
test("the route's model list is byte-identical to syncGrokConfig's", async () => {
  ...
  expect(fence).not.toContain("nxc-stub-m2");
});
  ...
  return (async () => {
    ...
    const deps = testDeps({
      injectGrokConfig: (() => ({
        ...
        message: "Grok config contains an nexcode begin marker without its end marker; refusing to guess where the managed block ends.",
