...
async function getSidecarSettings(config: NxcConfig): Promise<Response> {
...
async function putSidecarSettings(config: NxcConfig, vision: Record<string, unknown>): Promise<Response> {
  const url = new URL("http://localhost/api/sidecar-settings");
...
async function putClaudeCode(config: NxcConfig, body: Record<string, unknown>): Promise<Response> {
...
function emptyConfig(overrides: Partial<NxcConfig> = {}): NxcConfig {
  return {
    port: 10100,
...
describe("sidecar-settings vision model filter", () => {
  ...
  beforeEach(() => {
    ...
    isolatedHome = mkdtempSync(join(tmpdir(), "nxc-sidecar-vision-filter-"));
