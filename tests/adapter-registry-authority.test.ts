...
function provider(adapter: string): NxcProviderConfig {
  return {
    adapter,
...
const ANTHROPIC_CACHE_REQUEST: NxcParsedRequest = {
...
describe("adapter registry authority", () => {
  ...
  test("rejects non-string persisted adapter ids before registry lookup", () => {
    for (const adapterId of [null, 42, ["azure"]]) {
      ...
      const malformed = { ...provider("anthropic"), adapter: adapterId } as unknown as NxcProviderConfig;
