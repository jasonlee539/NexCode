...
const MANUAL_IMPORT_ENV = "NEXCODE_ENABLE_UNVERIFIED_CODEX_IMPORT";
let previousNexcodeHome: string | undefined;
let previousCodexHome: string | undefined;
let previousManualImportEnv: string | undefined;
let previousFetch: typeof fetch;

function jwtWithExp(exp: number, claims: Record<string, unknown> = {}): string {
  const enc = (value: unknown) => Buffer.from(JSON.stringify(value)).toString("base64url");
  return enc({ alg: "RS256", typ: "JWT" }) + "." + enc({ ...claims, exp }) + ".sig";
}

function makeConfig(overrides: Partial<NxcConfig> = {}): NxcConfig {
  return {
    port: 10100,
    providers: {},
...
describe("codex-auth API", () => {
  ...
  test("main account keeps its last valid quota when the usage endpoint is unreachable", async () => {
    writeFileSync(join(TEST_CODEX_HOME, "auth.json"), JSON.stringify({
      tokens: {
        access_token: jwtWithExp(Math.floor(Date.now() / 1000) + 3600, { email: "main@example.test" }),
        account_id: "acct-main",
      },
    }));
    updateAccountQuota(MAIN_CODEX_ACCOUNT_ID, 37, 1_783_000_000);
    setMainAccountPlan("pro");
    globalThis.fetch = (async () => { throw new TypeError("network unavailable"); }) as typeof fetch;

    const listed = await listCodexAuthAccounts(makeConfig(), true);
    const main = listed.find(account => account.id === MAIN_CODEX_ACCOUNT_ID);

    expect(main?.hasCredential).toBe(true);
    expect(main?.needsReauth).toBe(false);
    expect(main?.plan).toBe("pro");
    expect(main?.quota?.weeklyPercent).toBe(37);
  });
  ...
  test("main account display falls back to the masked local ID-token email", async () => {
    writeFileSync(join(TEST_CODEX_HOME, "auth.json"), JSON.stringify({
      tokens: {
        access_token: jwtWithExp(Math.floor(Date.now() / 1000) + 3600),
        id_token: jwtWithExp(Math.floor(Date.now() / 1000) + 3600, { email: "local-main@example.test" }),
        account_id: "acct-main-local-label",
      },
    }));
    ...
    expect(data.accounts.find(account => account.id === MAIN_CODEX_ACCOUNT_ID)?.email)
      .toBe("l***n@example.test");
  });
  ...
  test("GET /api/codex-auth/accounts omits a malformed persisted plan", async () => {
    const config = makeConfig({
      ...
      ] as unknown as NxcConfig["codexAccounts"],
    });
  ...
  test("enabled picker deletion retains its selector across an OAuth account re-add", async () => {
    ...
    const convergeCodexCatalog = async (): Promise<CatalogDisposition> => {
      convergences += 1;
      const persisted = JSON.parse(readFileSync(getConfigPath(), "utf8")) as NxcConfig;
  ...
  async function putPriority(config: NxcConfig, body: unknown): Promise<Response> {
  ...
  test("the account list reports selection order, defaulting to zero", async () => {
    ...
    seedPoolAccount(config, { id: "work", email: "work@example.test" });
    seedPoolAccount(config, { id: "side", email: "side@example.test" });
    // This is a DTO/priority contract, not a live WHAM integration test. Fresh
    // cache rows keep it deterministic and prevent a real network probe.
  ...
  test("Codex OAuth login responses project raw provider errors", async () => {
    ...
    const startSpy = spyOn(oauth, "startLoginFlow").mockImplementation(async () => {
      throw new Error("already in progress at C:\\Users\\Alice\\.nexcode\\auth.json.nxc-tmp sk-secret-provider-key");
    });
    try {
      const req = new Request("http://localhost/api/codex-auth/login", {
        method: "POST",
      ...
      expect(data.error).toBe("OAuth authentication failed. Check the NexCode account status and retry.");
    ...
    const cases: Array<{ error: Error; expected: string }> = [
      {
        ...
        expected: "Not logged in to chatgpt. Run: nxc login chatgpt",
      },
      {
  ...
  test("Codex OAuth login status projects late provider errors", async () => {
    ...
    oauth.OAUTH_PROVIDERS.chatgpt.login = async (controller) => {
      ...
      throw new Error("late failure at /home/alice/.nexcode/auth.json.nxc-tmp sk-secret-provider-key");
    };
    const openSpy = spyOn(openUrlMod, "openUrl").mockImplementation(() => {});
    const timeoutSpy = spyOn(globalThis, "setTimeout").mockImplementation(((
      callback: (...args: unknown[]) => void,
    ...
    try {
      ...
      expect(state).toMatchObject({
        status: "error",
        error: "OAuth authentication failed. Check the NexCode account status and retry.",
      });
  ...
  test("OAuth creation keeps an authorized account when warmup is temporarily unavailable", async () => {
    const accountId = "oauth-warmup-deferred";
    const config = makeConfig();
    const result = await completeMockCodexOAuth({
      config,
      requestBody: { id: accountId },
      oauthAccountId: "acct-oauth-warmup-deferred",
      email: "oauth-warmup-deferred@example.test",
      onWarmup: () => {},
      warmupResponse: () => new Response("temporarily unavailable", { status: 503 }),
    });
  ...
  test("OAuth creation still rejects an explicit Codex authentication failure", async () => {
    const accountId = "oauth-warmup-unauthorized";
    const config = makeConfig();
    const result = await completeMockCodexOAuth({
      config,
      requestBody: { id: accountId },
      oauthAccountId: "acct-oauth-warmup-unauthorized",
      email: "oauth-warmup-unauthorized@example.test",
      onWarmup: () => {},
      warmupResponse: () => new Response("unauthorized", { status: 401 }),
    });

    expect(result.state).toMatchObject({ status: "error" });
    expect(result.state.error).toContain("http_status:401");
    expect(config.codexAccounts).toEqual([]);
    expect(getCodexAccountCredential(accountId)).toBeNull();
  });
