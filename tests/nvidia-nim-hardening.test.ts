...
import type { NxcConfig, NxcParsedRequest, NxcTool } from "../src/types";
import { planVisionSidecar } from "../src/vision";

const tools: NxcTool[] = [{ name: "shell", description: "run", parameters: { type: "object" } }];

function nvidiaConfig(): NxcConfig {
  return {
    port: 10100,
    defaultProvider: "nvidia",
    providers: {
      // Bare persisted config, like `nxc init` writes: registry seeds must backfill the flags.
      nvidia: { adapter: "openai-chat", baseUrl: "https://integrate.api.nvidia.com/v1", apiKey: "k" },
    },
  };
}

function parsedFor(modelId: string, options: Partial<NxcParsedRequest["options"]> = {}): Parameters<ReturnType<typeof createOpenAIChatAdapter>["buildRequest"]>[0] {
  return {
    modelId,
    context: {
