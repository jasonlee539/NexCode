...
let adapterFactory: ((provider: NxcProviderConfig) => ProviderAdapter) | undefined;
...
function captureAdapter(captured: NxcParsedRequest[]): ProviderAdapter {
  return {
    ...
    async runTurn(parsed: NxcParsedRequest, _incoming, emit) {
...
}): Promise<NxcParsedRequest> {
  const captured: NxcParsedRequest[] = [];
