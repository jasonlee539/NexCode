...
function tempGrokHome(): { root: string; grokHome: string } {
  const root = mkdtempSync(join(tmpdir(), "nxc-grok-sel-"));
...
// THE audit regression: kimi/k3 and kimi/k3[1m] both sanitize toward nxc-kimi-k3-….
...
test("excluding a colliding model leaves the survivor's alias unchanged", () => {
  ...
  const survivorAlias = /\[model\.(nxc-[^\]]+)\][^[]*model = "kimi-k3"/.exec(full)?.[1];
  expect(survivorAlias).toBe("nxc-kimi-k3-2");
...
test("excluding the first model keeps the TOML shape valid", () => {
  ...
  const afterMarker = block.split("do not edit (removed by `nxc stop`) >>>\n")[1]!;
...
test("excluding everything writes a marker-only fence that strip removes", () => {
  ...
  try {
    ...
    expect(content).toContain(">>> nexcode managed block");
...
test("a user-reserved alias still pushes generated aliases past it while exclusions are active", () => {
  const { root, grokHome } = tempGrokHome();
  try {
    writeFileSync(join(grokHome, "config.toml"), '[model.nxc-kimi-k3]\nmodel = "mine"\n', "utf8");
    ...
    expect(content).toContain('[model.nxc-kimi-k3]\nmodel = "mine"');
    // And the survivor's generated alias starts AFTER the reservation, not at -1.
    expect(content).not.toContain("[model.nxc-kimi-k3]\nmodel = \"kimi-k3\"");
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});

const baseConfig = { port: 10100, defaultProvider: "openai", providers: {} } as unknown as NxcConfig;
...
test("syncGrokConfig with grokExcludedModels omits that model's table end to end", async () => {
  ...
  try {
    ...
    const config = { ...baseConfig, grokExcludedModels: ["cursor/grok-4.5"] } as NxcConfig;
    ...
    expect(content).toContain("[model.nxc-gpt-");
