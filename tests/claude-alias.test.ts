...
describe("claudeCodeAlias — readable-or-hash shared helper (devlog 050 / audit 051 #2)", () => {
  test("readable form when representable; both forms decode to the same route", () => {
    expect(claudeCodeAlias("gemini", "gemini-3-pro")).toBe("claude-nxc-gemini--gemini-3-pro");
    expect(claudeCodeNativeAlias("gpt-5.6-sol")).toBe("claude-nxc-native--gpt-5.6-sol");
    ...
    expect(resolveInboundModel("claude-nxc-native--gpt-5.6-sol[1m]", undefined)).toBe("gpt-5.6-sol");
    expect(resolveInboundModel("claude-nxc-gemini--gemini-3-pro[1M]", undefined)).toBe("gemini/gemini-3-pro");
  });
  ...
  test("slash-containing model ids stay readable under v2 (no desktop-3p hash)", () => {
    expect(claudeCodeAlias("openrouter", "anthropic/claude-opus-4-8")).toBe(
      "claude-nxc2-openrouter--anthropic~sclaude-opus-4-8",
    );
    expect(claudeCodeAlias("mock", "path/model")).toBe("claude-nxc2-mock--path~smodel");
    expect(resolveInboundModel("claude-nxc2-openrouter--anthropic~sclaude-opus-4-8", undefined)).toBe(
  ...
  test("unrepresentable shapes fall back to the desktop-3p hash — model never disappears", () => {
    ...
    expect(claudeCodeAlias("mock", "has~tilde")).toBe("claude-nxc2-mock--has~ttilde");
  });
});
