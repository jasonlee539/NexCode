...
const config = (claudeCode: NxcConfig["claudeCode"] = {}): NxcConfig => ({
  providers: [],
  claudeCode,
} as NxcConfig);
...
describe("Claude agent roster proxy-start synchronization (#2200)", () => {
  ...
  test("uses the live proxy context-window map for an enabled roster", async () => {
    ...
    const result = await syncClaudeAgentDefsAtProxyStartup(config(), 10100, {
      ...
      injectAgentDefs: (_cfg, windows) => {
        ...
        return ["nxc-google-gemini-3-7-flash.md"];
      },
    });

    expect(result).toEqual(["nxc-google-gemini-3-7-flash.md"]);
  ...
  test("catalog failure still runs the real injector with an unmarked roster", async () => {
    const dir = mkdtempSync(join(tmpdir(), "nxc-startup-roster-"));
    try {
      const configured = {
        providers: [],
      ...
      expect(result?.sort()).toEqual(["nxc-gpt-5-6-sol.md", "nxc-self.md"]);
