...
import type { AdapterEvent, NxcReasoningReplayScopeRef } from "../src/types";
...
function scope(
  overrides: Partial<NonNullable<NxcReasoningReplayScopeRef["current"]>> = {},
...
async function drain(events: AsyncIterable<AdapterEvent>, replayScope: NxcReasoningReplayScopeRef): Promise<void> {
...
describe("reasoning replay provider and credential identity", () => {
  ...
  test("incomplete, unscoped, and legacy thread-only namespaces fail closed", () => {
    const incomplete: NxcReasoningReplayScopeRef[] = [
      { clientThreadId: THREAD },
    ...
    rememberReasoningForCall(CALL_ID, REASONING, THREAD as unknown as NxcReasoningReplayScopeRef);
    expect(peekReasoningForCall(CALL_ID, THREAD as unknown as NxcReasoningReplayScopeRef)).toBeUndefined();
  });
