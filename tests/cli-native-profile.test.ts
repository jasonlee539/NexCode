...
describe("nxc account main", () => {
  test("cleanup guidance requires a literal true signal", () => {
    ...
    expect(apiError({ error: "validation failed", cleanupRequired: true }, "fallback")).toBe(1);
    expect(errors.at(-1)).toBe("Warning: native-login staging cleanup is still required; run 'nxc account main doctor'.");
  });
  ...
  test("official login honors an explicit Windows runtime outside PATH through ComSpec", () => {
    ...
    const invocation = nativeMainCodexLoginInvocation("win32", {
      ...
      configDir: tempConfigDir("nxc-native-profile-explicit-runtime-"),
    });
  ...
  test("official login honors a persisted runtime outside PATH", () => {
    ...
    const configDir = tempConfigDir("nxc-native-profile-persisted-runtime-");
  ...
  test("mutating human output uses the server's effective home while add keeps the auth envelope off HTTP", async () => {
    const stagingHome = join(tmpdir(), "nxc-native-profile-stage");
    const effectiveHome = join(tmpdir(), "nxc-native-profile-effective-home");
    const clientHome = join(tmpdir(), "nxc-native-profile-client-home");
  ...
  test("JSON mutating output preserves the server's canonical effective home", async () => {
    const effectiveHome = join(tmpdir(), "nxc-native-profile-json-effective-home");
    process.env.CODEX_HOME = join(tmpdir(), "nxc-native-profile-json-client-home");
  ...
  test("add sends an idempotent cancel fallback after a non-200 finish response", async () => {
    const stagingHome = join(tmpdir(), "nxc-native-profile-stage-non-200");
    ...
    expect(errors).not.toContain("Warning: native-login staging cleanup is still required; run 'nxc account main doctor'.");
  });

  test("add sends an idempotent cancel fallback when the finish response disconnects", async () => {
    const stagingHome = join(tmpdir(), "nxc-native-profile-stage-disconnect");
    const requests: string[] = [];
    const fetchImpl: typeof fetch = async input => {
      const path = new URL(String(input)).pathname;
      requests.push(path);
  ...
  test("add cancels server staging when official login aborts", async () => {
    const stagingHome = join(tmpdir(), "nxc-native-profile-stage-abort");
    const effectiveHome = join(tmpdir(), "nxc-native-profile-effective-home-abort");
    const requests: string[] = [];
    const errors: string[] = [];
    console.error = (...values: unknown[]) => errors.push(values.join(" "));
    const fetchImpl: typeof fetch = async input => {
  ...
  test("heartbeats only while the configured Codex login child is alive", async () => {
    const stagingHome = join(tmpdir(), "nxc-native-profile-stage-heartbeat-child");
  ...
  test("a stalled heartbeat is aborted before lease expiry, kills login, and cancels staging", async () => {
    const stagingHome = join(tmpdir(), "nxc-native-profile-stage-heartbeat-deadline");
    const requests: string[] = [];
    let now = 10_000;
