...
describe("Codex Log Guard CLI", () => {
  test("nxc storage codex-logs status uses the dedicated diagnostics endpoint", async () => {
    const seen: string[] = [];
  ...
  test("nxc storage remains an alias for the existing storage report", async () => {
    const seen: string[] = [];
    const fetchImpl: typeof fetch = async input => {
      seen.push(String(input));
