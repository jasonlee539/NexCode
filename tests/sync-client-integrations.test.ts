...
/**
 * `nxc sync` used to write the Codex catalog and stop, so a Grok fence or a Desktop profile
 * kept whatever context windows it was created with until the next `nxc start`. That gap is
 ...
 */
describe("nxc sync fans out to enabled native clients and owned file integrations", () => {
  const base = { port: 10100, defaultProvider: "x", providers: {} } as NxcConfig;
  ...
  test("an explicit false is the only thing that takes a client out of the fan-out", () => {
    const grokOff = { ...base, clientIntegrations: { grok: false } } as NxcConfig;
    ...
    const desktopOff = { ...base, clientIntegrations: { "claude-desktop": false } } as NxcConfig;
...
describe("nxc sync refreshes an already-owned MCode integration", () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "nxc-mcode-auto-sync-"));
    home = join(root, "home");
  ...
  test("updates context and the full max/ultra effort ladder through the real writer", async () => {
    ...
    expect(document.custom_provider.nexcode.models["openai/gpt-5.6-sol"]).toEqual({
  ...
  test("refuses a foreign edit without changing bytes or appending a journal row", async () => {
    ...
    expect(outcome?.reason).toContain("changed after nexcode wrote it");
  ...
  test("serializes a CLI refresh racing a server disable across the process boundary", async () => {
    ...
    expect(refreshed.custom_provider.nexcode.models["openai/gpt-5.6-sol"]).toEqual({
      limit: { context: 922_000 },
      thinking: { effortOptions: ["low", "medium", "high", "xhigh", "max", "ultra"] },
    });
    ...
    const finalDocument = Bun.YAML.parse(readFileSync(configPath, "utf8")) as {
      custom_provider?: { nexcode?: unknown };
    };
    expect(finalDocument.custom_provider?.nexcode).toBeUndefined();
  });
});

test("the direct nxc sync command refreshes MCode instead of relying on /api/sync", async () => {
