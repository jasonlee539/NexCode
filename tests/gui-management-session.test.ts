...
describe("GUI management session bootstrap", () => {
  test("management requests use the injected session while data requests remain untouched", async () => {
    ...
    const meta = new Map([
      ["nexcode-session-token", "nxc_session_browser-secret"],
      ["nexcode-session-csrf", "csrf-browser-secret"],
      ["nexcode-session-origin", "http://localhost:10100"],
    ]);
    ...
    expect(seen[0]?.headers.get("x-nexcode-api-key")).toBe("nxc_session_browser-secret");
    expect(seen[0]?.headers.get("x-nexcode-gui-origin")).toBe("http://localhost:10100");
    expect(seen[0]?.headers.get("x-nexcode-csrf-token")).toBeNull();
    expect(seen[1]?.headers.get("x-nexcode-api-key")).toBe("nxc_session_browser-secret");
    expect(seen[1]?.headers.get("x-nexcode-csrf-token")).toBe("csrf-browser-secret");
    expect(seen[2]?.headers.get("x-nexcode-api-key")).toBeNull();
    expect(seen[2]?.headers.get("x-nexcode-gui-origin")).toBeNull();
  });
});
