...
function baseRoutes(extra: MockRoute[]): MockRoute[] {
  return [
    { path: "/repos/lidge-jun/nexcode", body: { default_branch: "main" } },
    {
      path: "/repos/lidge-jun/nexcode/contents/.github/workflows?ref=main",
      body: [
...
describe("orphaned GitHub Actions cleanup", () => {
  ...
  test("deletes runs only for local workflows absent from the default branch", async () => {
    ...
    const routes = baseRoutes([
      {
        path: "/repos/lidge-jun/nexcode/actions/workflows?per_page=100&page=1",
        body: {
          workflows: [
      ...
      {
        path: "/repos/lidge-jun/nexcode/actions/workflows/2/runs?per_page=100&page=1",
        body: { workflow_runs: [{ id: 201, status: "completed", pull_requests: [] }] },
      },
      { method: "DELETE", path: "/repos/lidge-jun/nexcode/actions/runs/201", status: 204 },
    ]);
      ...
      },
      {
        ...
        body: {
          workflow_runs: [{
            ...
            head_repository: { full_name: "lidge-jun/nexcode" },
          }],
        },
      },
      {
        path: "/repos/lidge-jun/nexcode/contents/.github/workflows/tmp-pr.yml?ref=agent%2Fpr-77",
  ...
  test("cleans an orphan after its run head branch no longer contains the workflow file", async () => {
    ...
    const routes = baseRoutes([
      ...
      { method: "DELETE", path: "/repos/lidge-jun/nexcode/actions/runs/202", status: 204 },
    ]);
  ...
  test("preflights every candidate before issuing the first delete", async () => {
    ...
    const routes = baseRoutes([
      ...
      {
        path: "/repos/lidge-jun/nexcode/actions/workflows/3/runs?per_page=100&page=1",
        status: 503,
  ...
  test("caps deletion volume and prefers clearing smaller histories first", async () => {
    ...
    const routes = baseRoutes([
      ...
      { method: "DELETE", path: "/repos/lidge-jun/nexcode/actions/runs/207", status: 204 },
    ]);
    ...
    expect(calls.at(-1)).toBe("DELETE /repos/lidge-jun/nexcode/actions/runs/207");
    expect(routes).toHaveLength(0);
  });
});
