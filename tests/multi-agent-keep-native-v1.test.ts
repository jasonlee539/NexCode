...
describe("keepNativeChatGptOnV1", () => {
  ...
  test("native alias rows count as native; routed providers do not", () => {
    const routedAlias: RawEntry = { slug: "sol", nexcode_catalog_kind: CODEX_NATIVE_ALIAS_CATALOG_KIND };
    ...
    const stamped: RawEntry[] = [
      { slug: "sol", nexcode_catalog_kind: CODEX_NATIVE_ALIAS_CATALOG_KIND },
      { slug: "gpt-5.6-sol" },
    ];
...
const savedNxcHome = process.env.NEXCODE_HOME;
const savedCodexHome = process.env.CODEX_HOME;

afterEach(() => {
  if (savedNxcHome === undefined) delete process.env.NEXCODE_HOME;
  else process.env.NEXCODE_HOME = savedNxcHome;
  if (savedCodexHome === undefined) delete process.env.CODEX_HOME;
  else process.env.CODEX_HOME = savedCodexHome;
});

function isolateHomes(): void {
  process.env.NEXCODE_HOME = mkdtempSync(join(tmpdir(), "nxc-keep-native-"));
...
describe("nxc v2 keep-native-v1", () => {
...
describe("/api/v2 keepNativeChatGptOnV1", () => {
  test("GET/PUT persist the flag, warn by mode, and restamp via catalog convergence", async () => {
    isolateHomes();
    const config: NxcConfig = { providers: {}, hostname: "127.0.0.1", port: 10100, defaultProvider: "openai" } as NxcConfig;
