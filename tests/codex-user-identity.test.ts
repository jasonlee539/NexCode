...
const identityProbe = `
  ...
  const canonicalCodexHome = realpathSync.native(process.env.NXC_TEST_CANONICAL_CODEX_HOME);
...
beforeEach(() => {
  ...
  codexHome = mkdtempSync(join(tmpdir(), "nxc-user-identity-codex-home-"));
});

test("samePathIdentity is case-insensitive on Windows and exact elsewhere", () => {
  const winPath = "C:\\Users\\Alice\\AppData\\Local\\NexCode\\Runtime\\v1\\S-1-5-21\\history-write-locks\\abc.sqlite";
  ...
  expect(samePathIdentity(winPath, "D:\\Users\\Alice\\AppData\\Local\\NexCode\\Runtime\\v1\\S-1-5-21\\history-write-locks\\abc.sqlite", "win32")).toBe(false);
...
test("the coordinator namespace probe is read-only", () => {
  ...
  const root = join(realpathSync.native("/tmp"), `nexcode-runtime-v1-${uid}`);
...
test("real processes resolve one identity and coordinator path across every home/runtime environment", async () => {
  ...
  const environmentRoots = ["a", "b"].map(label => {
    const root = mkdtempSync(join(tmpdir(), `nxc-user-identity-env-${label}-`));
    const paths = {
      home: join(root, "home"),
      ...
      nexcodeHome: join(root, "ambient-nexcode"),
  ...
  try {
    const probes = await Promise.all(environmentRoots.map(({ paths }, index) => {
      ...
      return runIdentityProbe({
        ...
        NEXCODE_HOME: paths.nexcodeHome,
        NXC_TEST_CANONICAL_CODEX_HOME: canonicalHome,
        ...accountEnvironment,
