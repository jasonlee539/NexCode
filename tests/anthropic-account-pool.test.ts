...
import type { NxcAccountPoolRotationStrategy, NxcConfig } from "../src/types";

const originalHome = process.env.NEXCODE_HOME;
let home: string;

beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), "nxc-anthropic-pool-"));
...
function cfg(
  enabled: boolean,
  threshold = 80,
  pool: { strategy?: NxcAccountPoolRotationStrategy; stickyLimit?: number } = {},
): NxcConfig {
  return {
    port: 0,
