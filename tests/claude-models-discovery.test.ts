...
beforeEach(() => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("nxc-claude-discovery-");
  testDir = mkdtempSync(join(tmpdir(), "nxc-claude-discovery-"));
...
function configWithStaticModels(claudeCode?: NxcConfig["claudeCode"]): NxcConfig {
  return {
    port: 0,
...
test("per-surface id style: ?ids= wins, claude-code UA gets readable, unknown UA stays hashed (devlog 050)", async () => {
  ...
  const server = startServer(0);
  try {
    const readable = "claude-nxc-mock--test-model";
