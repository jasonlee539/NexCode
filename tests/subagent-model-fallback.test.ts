...
const savedNexcodeHome = process.env.NEXCODE_HOME;
let testDir: string;
...
function codexHomeFixture(): string {
  const dir = mkdtempSync(join(tmpdir(), "nxc-subagent-fallback-"));
...
beforeEach(() => {
  testDir = mkdtempSync(join(tmpdir(), "nxc-subagent-fb-"));
...
afterEach(() => {
  ...
  if (savedNexcodeHome === undefined) delete process.env.NEXCODE_HOME;
  else process.env.NEXCODE_HOME = savedNexcodeHome;
  clearAccountQuota();
  resetSubagentModelFallbackStateForTests();
  clearAccountNeedsReauth("pool-a");
  clearAccountNeedsReauth("account-a");
