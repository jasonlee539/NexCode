...
test("the default claude.json reader stays inside the overridden profile", () => {
  const dir = mkdtempSync(join(tmpdir(), "nxc-authdetect-"));
...
// into ANTHROPIC_AUTH_TOKEN. Counting it would make nexcode's own output look like
// proof the user can authenticate natively — the marker feedback loop, one variable over.
test("nexcode's own admission key is not counted as user auth", () => {
  const base = {
    readClaudeJson: () => undefined,
    credentialsFileExists: () => false,
  ...
  const own = detectClaudeAuth({
    ...base,
    env: () => ({ ANTHROPIC_AUTH_TOKEN: "sk-nxc-admission" }),
    ownTokens: ["sk-nxc-admission"],
  });
  ...
  expect(detectClaudeAuth({
    ...base,
    env: () => ({ ANTHROPIC_API_KEY: "sk-nxc-admission" }),
    ownTokens: ["sk-nxc-admission"],
