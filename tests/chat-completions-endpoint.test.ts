...
beforeEach(() => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("nxc-chat-completions-");
  testDir = mkdtempSync(join(tmpdir(), "nxc-chat-completions-"));
...
function mockConfig(baseUrl: string, providerOverrides: Partial<NxcProviderConfig> = {}): NxcConfig {
  return {
    port: 0,
...
async function driveChatFallbackServiceTier(
  providerOverrides: Partial<NxcProviderConfig>,
...
test("chat-native consumes pacing before the response-header timeout starts", async () => {
  ...
  } as Partial<NxcProviderConfig> & { fetch: typeof globalThis.fetch });
...
function dualWireConfig(baseUrl: string): NxcConfig {
  return {
    port: 0,
    defaultProvider: "mock",
    providers: {
