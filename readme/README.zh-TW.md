...
nxc start        # 代理 + 儀表板: localhost:10100
```

<p align="center">
  <img src="../assets/claude-code-models.gif" alt="透過 nexcode 執行路由模型的 Claude Code —— 狀態列顯示 gpt-5.6-luna-medium 為目前模型" width="820"><br>
  <sub><b>Claude Code 可以用任何模型。</b>選擇器是原生 Claude Code，跑起來的模型隨你選。</sub>
</p>

<p align="center">
  <img src="../assets/demo.gif" alt="nexcode 示範 —— 在 Codex 應用中用路由的非 OpenAI 模型執行任務" width="820"><br>
  <sub><b>Codex 可以用任何模型。</b>選好 provider 直接開跑 —— 同樣的 Codex 工作流，換個大腦。</sub>
</p>

<p align="center">
  <a href="../README.md">English</a> · <a href="README.fr.md">Français</a> · <a href="README.ko.md">한국어</a> · <a href="README.zh-CN.md">简体中文</a> · <b>繁體中文</b> · <a href="README.ru.md">Русский</a> · <a href="README.ja.md">日本語</a> · <a href="README.tr.md">Türkçe</a> · 📖 <a href="https://nexcode.me/zh-tw/"><b>完整文件 →</b></a>
</p>

<p align="center">
  <img src="../assets/architecture.png" alt="nexcode 架構 — Codex CLI 透過 nexcode 代理路由到任意 LLM 供應商" width="820">
</p>

在 Codex 中 —— 以及在 **Claude Code** 中 —— 使用 Claude、Gemini、Grok、GLM、DeepSeek、Kimi、Qwen、Ollama 或任意其他 LLM，無需等待官方新增支援。

nexcode 是一個輕量級本機代理，把 Codex 的 Responses API 翻譯成你的 provider 所講的協議。streaming、tool 呼叫、reasoning token、圖片 —— 全部雙向工作。
...
```mermaid
flowchart LR
  codex[Codex 會話<br/>CLI, App, SSH, 行動端] --> proxy[nexcode]
...
nexcode 把 Bun 執行時作為依賴打包，並透過 Node 啟動器執行，因此你**不必**自己安裝 Bun。如果看到 "bundled Bun runtime is missing" 錯誤，代表安裝時略過了 lifecycle 腳本（包括 npm 透過 `allowScripts` 攔截 bun postinstall 的情況）或 optional 依賴。請允許 bun 安裝腳本後再重裝：
...
- **在 Claude 中也能使用任意 LLM。** `nxc claude` 可透過代理啟動 Claude Code。Claude 儀表板還提供獨立的 Desktop 配置，可管理 Opus、Fable、Sonnet、Haiku 四個系列，並支援拖放、鍵盤操作和 JSON 匯入/匯出。
...
- **委派給合適的模型。** 在儀表板或 config 中把最多 5 個路由/原生模型放進 Codex 的 subagent 選擇器 —— 複雜任務交給 reasoning 模型，快速任務交給便宜模型。在 v2 多智慧體表面（GPT-5.6 Sol/Terra）上，代理會注入精簡的委派指引：首選子智慧體模型與 effort（`injectionModel` / `injectionEffort`）、featured 模型清單及各自支援的 effort 階梯，以及讓跨模型 `spawn_agent` 覆蓋得以應用的 `fork_turns` 規則。已知限制：原生父代理 spawn 路由子代理時，任務本文可能以後端加密形式到達而丟失（[#92](https://github.com/lidge-jun/nexcode/issues/92)）—— 需要可靠的跨 provider 委派請使用 v1 表面。想自訂文案，可在 `injectionPrompt` 中使用 `{{model}}` / `{{effort}}` / `{{roster}}` 預留位置。
...
- **乾淨退出，零殘留。** `nxc stop`（或儀表板的 Stop 按鈕）會關閉代理、停止已安裝的背景服務，並將 Codex 恢復為原始配置。之後 `codex` 就像從沒安裝過 nexcode 一樣工作 —— 無殘留配置，無殭屍程序。
...
也可以用 `nxc init`（互動式 CLI）或直接編輯 `~/.nexcode/config.json` 來新增 provider。
...
省略 `provider/` 字首時，nexcode 會路由到預設 provider，或根據模型名模式自動匹配（例如 `claude-*`
...
除非 provider config 明確設定 alias，nexcode 會把 `xhigh` 與 `max` 保持為不同檔位。`ultra` 與上游
...
preview gate 限制；nexcode 只是準備好你的帳號/provider 可存取時所需的路由和目錄後設資料。

<p align="center">
  <img src="../assets/codex-app-picker.png" alt="Codex App 展示 nexcode 路由模型及 reasoning effort 選擇器" width="480">
</p>
...
- 目前 marker 為 `openaiProviderTierVersion: 2`，原配置備份到
  `~/.nexcode/config.json.pre-openai-tiers-v2.bak`。恢復命令：
...
- **現有會話保持 affinity。** 執行緒 id 綁定到所選帳號並在後續輪次複用，因此長請求或移動/SSH 連線的會話
  會繼續使用同一帳號。
- **新會話可自動路由。** 啟用自動切換後，nexcode 比較 5 小時、每週、30 天使用量中最熱的配額視窗，
...
此外還有 DeepSeek、Groq、OpenRouter、Together、Fireworks、Cerebras、Mistral、Hugging Face、NVIDIA NIM、MiniMax、Qwen Cloud、騰訊雲 Coding Plan、SiliconFlow 等等。完整清單可用 `nxc init` 檢視，或見[供應商文件](https://nexcode.me/zh-tw/reference/configuration/)。

## CLI

```bash
nxc init                       # 互動式初始化
nxc start [--port 10100]       # 啟動代理
nxc stop                       # 停止並恢復原生 Codex 配置
nxc restore                    # 僅恢復，不停止（別名：nxc eject）
nxc uninstall                  # 移除 service/shim/config 並恢復原生 Codex
nxc ensure                     # 按需啟動 + 重新整理 Codex config/cache
nxc sync                       # 重新整理模型列表 + 重新注入 Codex
nxc status                     # 檢視代理是否在執行
nxc login <provider>          # OAuth 登入（xai、anthropic、kimi、cursor 等）
nxc logout <provider>          # 移除已儲存的登入
nxc account <list|current|use> # 檢視/切換帳號與 API-key pool（脫敏；含 refresh/auto-switch/remove/add-key）
nxc gui                        # 開啟 Web 儀表板
nxc claude [args...]           # 啟動接入代理的 Claude Code（模型發現已開啟）
nxc claude desktop             # 儲存並套用 Claude Desktop 四系列配置
nxc codex-shim install         # 執行 codex 時自動啟動代理
nxc service [install|start|stop|status|uninstall]   # 安裝/更新/啟動背景服務
nxc update [--tag preview]     # 更新 nexcode；preview 安裝保持 @preview
```
...
```bash
nxc claude desktop [apply]                         # 儲存並套用目前設定
nxc claude desktop show [--json]                   # 檢視路由、系列和預設值
nxc claude desktop move <route> <family> [--default]
nxc claude desktop default <family> <route|none>
nxc claude desktop export <path|->                 # 使用 - 將 JSON 輸出到 stdout
nxc claude desktop import <path> [--apply]         # 驗證後儲存，可選擇立即套用
```
...
`nxc uninstall` 會停止代理、移除已安裝的 service、移除 Codex shim、恢復原生 Codex config/catalog/history，並刪除 `~/.nexcode`。

## 設定

配置檔案路徑：`~/.nexcode/config.json`。
...
> nexcode 會在傳送請求前剝離末尾的 `[1m]` 字尾，因為 OpenAI 相容端點會拒絕帶方括號的 id（Z.AI 400 code
...
本機 provider 的 `apiKey` 通常留空。只要本機服務有提供 OpenAI 相容的 Chat Completions 端點，nexcode 就能直接接上。

WebSocket 傳輸預設關閉。只有當你希望 Codex 使用 Responses WebSocket 而不是 HTTP/SSE 時，才需要設定 `"websockets": true`。

### 遠端存取

預設 nexcode 會綁定在 `127.0.0.1`（迴環），不必額外認證。
若你設定 `"hostname": "0.0.0.0"` 把代理暴露到區網，nexcode 會要求 bearer token，同時保護管理
...
綁定到非迴環地址時若缺少該環境變數，代理會拒絕啟動。若為區網存取安裝背景服務，請在 `nxc service install`
...
nexcode 會自動 remap Codex 的 resume 歷史，讓舊的 OpenAI 對話與 nexcode 建立的專案執行緒在代理運作期間仍能在
Codex App 中可見。原始 provider/source 後設資料紀錄在 `~/.nexcode/codex-history-backup.json`。`nxc stop` /
`nxc restore` 會把備份的 OpenAI 列還原到 OpenAI，並把其餘 nexcode 使用者執行緒也 eject 到 OpenAI，讓原生
...
各欄位詳細說明見 **[設定參考](https://nexcode.me/zh-tw/reference/configuration/)**。

## 文件

完整文件——安裝、供應商設定、路由、sidecar、Codex 整合、Codex App 模型選擇器、CLI／設定參考——由 [`docs-site/`](../docs-site) 目錄的 Astro 站點建置，發布於 **[nexcode.me](https://nexcode.me/zh-tw/)**。
...
見 **[貢獻指南](https://nexcode.me/zh-tw/contributing/)**。

## 免責聲明

nexcode 是獨立的社群維護專案，**與 OpenAI、Anthropic 或其他任何供應商無關，也未獲其背書。**

部分供應商——尤其是 Anthropic（Claude）——可能會暫停或限制經第三方代理路由 API 流量的帳號。**使用風險自負（UAYOR）。** 連線供應商前，請先查其服務條款，確認是否允許代理式存取。上游供應商對帳號採取的任何處置，nexcode 維護者概不負責。
