...
<p align="center">
  ...
  <a href="https://www.npmjs.com/package/@bitkyc08/nexcode"><img src="https://img.shields.io/npm/v/@bitkyc08/nexcode?color=cb3837&label=npm&logo=npm" alt="version npm"></a>
  <a href="https://github.com/lidge-jun/nexcode/blob/main/LICENSE"><img src="https://img.shields.io/npm/l/@bitkyc08/nexcode?color=blue" alt="licence"></a>
  <img src="https://img.shields.io/node/v/@bitkyc08/nexcode?logo=node.js&label=node" alt="version de Node">
</p>

```bash
npm install -g @bitkyc08/nexcode
nxc start        # proxy et tableau de bord sur le port par défaut localhost:10100
```

<table align="center">
  <tr>
    <td width="50%" align="center">
      <img src="../assets/claude-code-models.gif" alt="Claude Code exécutant un modèle routé par nexcode — la barre d'état affiche gpt-5.6-luna-medium comme modèle actif" width="410"><br>
    ...
    <td width="50%" align="center">
      <img src="../assets/demo.gif" alt="démonstration d'nexcode — exécution d'une tâche dans l'application Codex avec un modèle routé non-OpenAI" width="410"><br>
  ...
  <tr>
    <td width="50%" align="center">
      <img src="../assets/claude-desktop-subagent.gif" alt="Claude Desktop répondant avec Claude Opus 4.8, puis déléguant à un sous-agent GPT-5.6 Sol par l'intermédiaire d'nexcode" width="410"><br>
    ...
    <td width="50%" align="center">
      <img src="../assets/grok-build-subagent.gif" alt="Grok Build exécutant GPT-5.6 Sol par l'intermédiaire d'nexcode et appelant un sous-agent Kimi K3" width="410"><br>
...
<p align="center">
  <a href="../README.md">English</a> · <b>Français</b> · <a href="README.ko.md">한국어</a> · <a href="README.zh-CN.md">简体中文</a> · <a href="README.zh-TW.md">繁體中文</a> · <a href="README.ru.md">Русский</a> · <a href="README.ja.md">日本語</a> · <a href="README.tr.md">Türkçe</a> · 📖 <a href="https://nexcode.me/fr/"><b>Documentation complète →</b></a>
</p>

nexcode est un proxy local léger qui traduit l'API Responses de Codex vers le protocole utilisé
...
```bash
npm install -g @bitkyc08/nexcode   # Node 18+ ; le runtime Bun est inclus automatiquement
nxc start                            # ou `nxc service` pour l’exécuter en arrière-plan
```
...
cd nexcode && ~/.bun/bin/bun install
...
choisissez les modèles et gérez les comptes. `nxc gui` permet de rouvrir le tableau de bord à tout moment.
...
```bash
npm install -g @bitkyc08/nexcode
nxc start     # ou `nxc service`
nxc init      # configuration interactive : écrit ~/.nexcode/config.json et relie Codex
```

`nxc init` ne démarre jamais le proxy ; démarrez-le d'abord (ou après — les deux ordres fonctionnent,
mais les commandes sans interface comme `nxc provider add` et `nxc combo set` communiquent avec le proxy
**actif** et se terminent avec un code différent de zéro s'il est inaccessible). `nxc status` / `nxc doctor` /
`nxc health` indiquent l'état d'exécution.

> **Agents qui installent ou exécutent nexcode :** lisez
> [`AGENTS_INSTALL.md`](../AGENTS_INSTALL.md). Une exécution interactive de `nxc start` peut demander une
...
[documentation d'installation](https://nexcode.me/fr/getting-started/installation/).
...
- **Combos** — un identifiant de modèle virtuel avec basculement ou round-robin pondéré entre fournisseurs.
  Consultez le [guide des combos](https://nexcode.me/fr/guides/combos/).
- **Des sous-agents sur n'importe quel modèle** — affichez les modèles routés dans le sélecteur de sous-agents
  ...
  [guide des sous-agents](https://nexcode.me/fr/guides/sub-agent-surface/).
...
- **Arrêt propre, aucun résidu** — `nxc stop` restaure la configuration d'origine de Codex.
...
NexCode suit 36 catégories d'état conservé par le processus. Chacune possède une limite documentée :
...
[documentation sur le routage des modèles](https://nexcode.me/fr/guides/model-routing/).
...
Qwen Cloud, SiliconFlow et bien d'autres. Liste complète : `nxc init` ou la
[documentation des fournisseurs](https://nexcode.me/fr/guides/providers/).

## CLI

```bash
nxc init                       # configuration interactive (écrit la configuration, relie Codex, propose le shim)
nxc start [--port 10100]       # démarre le proxy au premier plan
nxc stop                       # arrête le proxy et restaure Codex natif
nxc service [install|start|stop|status|uninstall|remove]  # service en arrière-plan
nxc codex-shim install         # démarre le proxy à la demande lors du lancement de `codex`
nxc health [--json]            # vérifie immédiatement que le proxy répond
nxc ready [--json] [--wait [--timeout <seconds>]]  # vérifie l’état après synchronisation
nxc status                     # indique si le proxy est actif
nxc gui                        # ouvre le tableau de bord web
nxc provider <...>             # gère les fournisseurs (list/add/edit/test/remove)
nxc account <...>              # gère les comptes ChatGPT et les groupes de clés API
nxc combo <...>                # gère les combos de basculement ou de rotation
nxc v2 <...>                   # contrôle les surfaces multi-agents v1/v2
nxc update [--tag preview]     # met à jour nexcode
```
...
[documentation de la CLI](https://nexcode.me/fr/reference/cli/).
...
`nxc ready [--json] [--wait [--timeout <seconds>]]` effectue une seule sonde par défaut. `--wait` interroge
...
tandis que `nxc health` reste compatible.

### Démarrage automatique : service ou shim

Utilisez le **service** (`nxc service`) pour un proxy toujours actif qui redémarre après un plantage. Utilisez
le **shim** (`nxc codex-shim install`) pour un démarrage léger à la demande sans démon en arrière-plan.
Supprimez-les avec `nxc service uninstall` / `nxc codex-shim uninstall`.

### Désinstallation

```bash
nxc uninstall                  # arrête, supprime le service ou shim, restaure Codex natif et nettoie l’état
npm uninstall -g @bitkyc08/nexcode
```

## Accès distant

Par défaut, nexcode se lie à `127.0.0.1` et ne nécessite aucune authentification supplémentaire. Une liaison
au-delà de l'adresse de bouclage (`"hostname": "0.0.0.0"`) **nécessite** un jeton bearer — le proxy refuse de
démarrer sans `NEXCODE_API_AUTH_TOKEN`, et chaque requête cliente doit le fournir dans
`x-nexcode-api-key`. Détails :
[référence de configuration](https://nexcode.me/fr/reference/configuration/).
...
[`docs-site/`](../docs-site) et publiée sur **[nexcode.me](https://nexcode.me/fr/)**.
...
[signalement privé de vulnérabilités de GitHub](https://github.com/lidge-jun/nexcode/security/advisories/new),
...
nexcode est un projet indépendant maintenu par la communauté et **n'est affilié ni à OpenAI, ni à Anthropic,
...
mainteneurs d'nexcode ne sont pas responsables des mesures prises sur les comptes par les fournisseurs en amont.
