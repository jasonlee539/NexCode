...
nxc start        # прокси + дашборд: localhost:10100
```

<p align="center">
  <img src="../assets/claude-code-models.gif" alt="Claude Code работает на маршрутизированной модели через nexcode — в строке состояния активна gpt-5.6-luna-medium" width="820"><br>
  <sub><b>Claude Code на любой модели.</b> Селектор — обычный Claude Code, а вот модель за ним — какую захотите.</sub>
</p>

<p align="center">
  <img src="../assets/demo.gif" alt="Демонстрация nexcode — выполнение задачи в приложении Codex на маршрутизируемой модели не от OpenAI" width="820"><br>
  <sub><b>Codex на любой модели.</b> Выберите провайдера — и вперёд: тот же рабочий процесс Codex, другой «мозг».</sub>
</p>

<p align="center">
  <a href="../README.md">English</a> · <a href="README.fr.md">Français</a> · <a href="README.ko.md">한국어</a> · <a href="README.zh-CN.md">简体中文</a> · <a href="README.zh-TW.md">繁體中文</a> · <b>Русский</b> · <a href="README.ja.md">日本語</a> · <a href="README.tr.md">Türkçe</a> · 📖 <a href="https://nexcode.me/ru/"><b>Полная документация →</b></a>
</p>

<p align="center">
  <img src="../assets/architecture.png" alt="Архитектура nexcode — Codex CLI направляет запросы через прокси nexcode к любому LLM-провайдеру" width="820">
</p>
...
nexcode — это лёгкий локальный прокси, который транслирует Responses API Codex в протокол, понятный вашему провайдеру. Потоковая передача, вызовы инструментов, токены рассуждений, изображения — всё работает в обе стороны.

Кроме того, nexcode умеет управлять **пулом аккаунтов ChatGPT** для аутентификации Codex. Добавьте
...
```mermaid
flowchart LR
  codex[Сессия Codex<br/>CLI, App, SSH, мобильный] --> proxy[nexcode]
...
аккаунтами. `nxc gui` в любой момент снова откроет дашборд.
...
`nxc init` никогда не запускает прокси; сначала запустите его сами (или после — оба порядка
работают, но headless-команды вроде `nxc provider add` и `nxc combo set` обращаются к **живому**
прокси и завершаются с ненулевым кодом, если он недоступен). Состояние запущенного прокси
показывают `nxc status`, `nxc doctor` и `nxc health`.

> **Решение о GitHub star принимает пользователь, а не агент.** При интерактивном `nxc start`
> или `nxc service install` CLI может один раз спросить, нужно ли поставить star этому
...
Провайдеров также можно добавлять через `nxc init` (интерактивный CLI) или напрямую редактируя `~/.nexcode/config.json`.
...
Если префикс `provider/` опущен, nexcode направляет запрос провайдеру по умолчанию — либо автоматически подбирает провайдера по шаблону имени модели (например, `claude-*` уходит в Anthropic, `gpt-*` — в OpenAI).
...
`max` и `ultra`, если модель их объявляет. nexcode сохраняет `xhigh` и `max` как разные уровни,
...
стороне; nexcode лишь подготавливает маршрутизацию и метаданные каталога для аккаунтов и
провайдеров, которые могут их обслуживать.

<p align="center">
  <img src="../assets/codex-app-picker.png" alt="Codex App с маршрутизируемыми моделями nexcode и селектором уровня рассуждений" width="480">
</p>
...
- Устаревший публичный id провайдера `chatgpt` после миграции скрывается. Исходная конфигурация
  однократно сохраняется в `~/.nexcode/config.json.pre-openai-tiers-v2.bak`; восстановить её можно командой
...
обслужит следующую сессию Codex. nexcode гарантирует следующее поведение:
...
- **Новые сессии могут маршрутизироваться автоматически.** При включённом автопереключении nexcode
...
- **Любая LLM и в Claude Code.** Тот же демон обслуживает Anthropic Messages API (`/v1/messages` + `count_tokens`): `nxc claude` запускает Claude Code с полностью готовой конфигурацией, а маршрутизируемые модели появляются в его родном селекторе `/model` благодаря обнаружению моделей через шлюз (алиасы `claude-nxc-<provider>--<model>`, Claude Code 2.1.129+). Слоты и сопоставления моделей настраиваются на странице Claude панели управления.
...
- **Делегируйте задачи подходящей модели.** Через панель управления или конфигурацию можно вывести до пяти маршрутизируемых или нативных моделей в селектор подагентов Codex — сложные задачи отправляйте модели с развитыми рассуждениями, быстрые — дешёвой. На мультиагентной поверхности v2 (GPT-5.6 Sol/Terra) прокси внедряет компактные указания по делегированию: предпочтительную модель и уровень рассуждений подагента (`injectionModel` / `injectionEffort`), список отобранных моделей со шкалой уровней, которую поддерживает каждая из них, и правила `fork_turns`, позволяющие кросс-модельным вызовам `spawn_agent` применять свои переопределения. Известное ограничение: когда нативный родитель порождает маршрутизируемого потомка, тело задачи в настоящий момент может прийти зашифрованным на бэкенде и потеряться ([#92](https://github.com/lidge-jun/nexcode/issues/92)) — для надёжного делегирования между провайдерами используйте поверхность v1. Хотите свои формулировки? Задайте `injectionPrompt` с плейсхолдерами `{{model}}` / `{{effort}}` / `{{roster}}`.
...
- **Работает в фоне.** Установите как системную службу (launchd / systemd / Task Scheduler) и забудьте о ней. На macOS/Linux прокси стартует при входе в систему; на Windows бэкенд Task Scheduler по умолчанию запускается при входе (без окон), либо используйте `nxc service install --native`, чтобы получить полноценную службу Windows, стартующую при загрузке.
- **Чистый выход без следов.** `nxc stop` (или кнопка Stop в панели управления) завершает работу прокси, останавливает фоновую службу, если она установлена, и возвращает Codex к исходной конфигурации. Обычный `codex` работает ровно так же, как раньше, — без остатков конфигурации и осиротевших процессов.
...
А также DeepSeek, Groq, OpenRouter, Together, Fireworks, Cerebras, Mistral, Hugging Face, NVIDIA NIM, MiniMax, Qwen Cloud, Tencent Cloud Coding Plan, SiliconFlow и другие. Полный список — в `nxc init` или в [документации по провайдерам](https://nexcode.me/reference/configuration/).

Поддержка Cursor — поэтапный экспериментальный мост: он появляется в `nxc init` и в селекторе
...
не настроен, nexcode возвращает типизированные ответы об отсутствии исполнителя вместо
...
```bash
nxc init                       # интерактивная настройка
nxc start [--port 10100]       # запустить прокси; если порт занят, выбирается свободный
nxc stop                       # остановить + восстановить нативный Codex
nxc restore                    # восстановить без остановки (алиас: nxc eject)
nxc uninstall                  # удалить службу/shim/конфигурацию и восстановить нативный Codex
nxc ensure                     # запустить при необходимости + обновить конфигурацию/кэш Codex
nxc sync                       # обновить модели + заново встроиться в Codex
nxc codex-shim install         # выполнять `nxc ensure` при каждом запуске `codex`
nxc status                     # работает ли прокси?
nxc login <provider>          # вход через OAuth (xai, anthropic, kimi, cursor, ...)
nxc logout <provider>          # удалить сохранённый вход
nxc account <list|current|use> # просмотр/переключение аккаунтов и пулов API-ключей (маскировано; также refresh/auto-switch/remove/add-key)
nxc gui                        # открыть веб-панель управления
nxc claude [args...]           # запустить Claude Code, подключённый к прокси (обнаружение моделей включено)
nxc service [install|start|stop|status|uninstall]   # установить/обновить/запустить фоновую службу
nxc update [--tag preview]     # обновить nexcode; preview-установки остаются на @preview
```

### Автозапуск: служба или shim

У nexcode есть два способа автоматически запускать прокси:
...
| **Когда** | Всегда работает после входа в систему | По требованию — выполняет `nxc ensure` при запуске `codex` |
| **Перезапуск** | Автоматический перезапуск при сбое | Запускается один раз на каждый вызов `codex` |
| **Обновления Codex** | Не влияют | Стабильно заменённый лончер восстанавливается следующей обычной командой `nxc` |
| **Удаление** | `nxc service uninstall` | `nxc codex-shim uninstall` |

Используйте **службу**, если прокси должен работать постоянно (рекомендуется для машин разработчиков).
Если внешнее обновление Codex перезапишет установленный shim, следующая обычная команда `nxc`
...
к сбою запрошенной команды; ручной вариант — `nxc codex-shim install`. Для отключения установите
...
занят, `nxc start` автоматически выберет другой свободный локальный порт и обновит настройки Codex.
...
`nxc uninstall` останавливает прокси, удаляет установленную службу, удаляет shim для Codex,
восстанавливает нативные конфигурацию/каталог/историю Codex и удаляет `~/.nexcode`.

## Конфигурация

Конфигурация хранится в `~/.nexcode/config.json`. Если файл не удаётся разобрать (например,
JSON обрезан или испорчен вручную), nexcode сохраняет его резервную копию в
...
> и `glm-5.2[1m]` — nexcode отрезает завершающий суффикс `[1m]` перед отправкой
...
Локальные модели тоже работают. Направьте nexcode на любой OpenAI-совместимый сервер,
...
По умолчанию nexcode привязывается к `127.0.0.1` (loopback) и не требует дополнительной аутентификации.
Если вы задаёте `"hostname": "0.0.0.0"`, открывая прокси в локальной сети, nexcode требует bearer-токен
...
`nxc service install`, чтобы менеджер служб её получил.
...
nexcode автоматически перепривязывает историю возобновления Codex, чтобы старые чаты OpenAI и
созданные nexcode проектные треды оставались видимыми в Codex App, пока прокси активен. Исходные
метаданные provider/source nexcode записывает в `~/.nexcode/codex-history-backup.json`.
`nxc stop` / `nxc restore` возвращает сохранённые в резервной копии строки OpenAI обратно к OpenAI,
а оставшиеся пользовательские треды nexcode также переводит на OpenAI, чтобы нативный Codex
...
Описание всех полей — в **[справочнике по конфигурации](https://nexcode.me/reference/configuration/)**.

## Документация

Публичная документация — установка, провайдеры, маршрутизация, сайдкары, интеграция с Codex, селектор моделей Codex App и справочник по CLI/конфигурации — собирается из [`docs-site/`](../docs-site) и публикуется на **[nexcode.me](https://nexcode.me/)**.
...
nexcode — независимый проект, поддерживаемый сообществом; он **не аффилирован с OpenAI, Anthropic или каким-либо другим провайдером и не одобрен ими**.

Некоторые провайдеры — в частности Anthropic (Claude) — могут приостанавливать или ограничивать аккаунты, которые направляют API-трафик через сторонние прокси. **Используйте на свой страх и риск (UAYOR).** Прежде чем подключать провайдера, изучите его условия использования и убедитесь, что доступ через прокси разрешён. Мейнтейнеры nexcode не несут ответственности за какие-либо действия вышестоящих провайдеров в отношении аккаунтов.
