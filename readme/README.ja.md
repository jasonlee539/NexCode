...
<p align="center">
  ...
  <a href="https://www.npmjs.com/package/@bitkyc08/nexcode"><img src="https://img.shields.io/npm/v/@bitkyc08/nexcode?color=cb3837&label=npm&logo=npm" alt="npm version"></a>
  <a href="https://github.com/lidge-jun/nexcode/blob/main/LICENSE"><img src="https://img.shields.io/npm/l/@bitkyc08/nexcode?color=blue" alt="license"></a>
  <img src="https://img.shields.io/node/v/@bitkyc08/nexcode?logo=node.js&label=node" alt="node version">
</p>

```bash
npm install -g @bitkyc08/nexcode
nxc start        # プロキシ + ダッシュボード: localhost:10100
```

<p align="center">
  <img src="../assets/claude-code-models.gif" alt="nexcode 経由でルーティングされたモデルで動作する Claude Code — ステータスバーに gpt-5.6-luna-medium が有効なモデルとして表示" width="820"><br>
  <sub><b>Claude Code でどんなモデルでも。</b>ピッカーは純正 Claude Code のまま、動いているモデルは自由に。</sub>
</p>

<p align="center">
  <img src="../assets/demo.gif" alt="nexcode デモ — Codex アプリで非 OpenAI ルーティングモデルでタスクを実行" width="820"><br>
  <sub><b>Codex でどんなモデルでも。</b>プロバイダーを選ぶだけ — 同じ Codex ワークフローで、違う頭脳。</sub>
</p>

<p align="center">
  <a href="../README.md">English</a> · <a href="README.fr.md">Français</a> · <a href="README.ko.md">한국어</a> · <a href="README.zh-CN.md">简体中文</a> · <a href="README.zh-TW.md">繁體中文</a> · <a href="README.ru.md">Русский</a> · <b>日本語</b> · <a href="README.tr.md">Türkçe</a> · 📖 <a href="https://nexcode.me/ja/"><b>完全なドキュメント →</b></a>
</p>

<p align="center">
  <img src="../assets/architecture.png" alt="nexcode アーキテクチャ — Codex CLI が nexcode プロキシ経由で任意の LLM プロバイダーにルーティング" width="820">
</p>
...
nexcode は Codex の Responses API をプロバイダーが話すプロトコルに変換する、軽量なローカルプロキシです。ストリーミング、ツール呼び出し、推論トークン、画像 — すべて双方向で動作します。
...
```
Codex CLI / App / SDK ──/v1/responses──▶ nexcode ──▶ Any provider
                                              │
...
```mermaid
flowchart LR
  codex[Codex セッション<br/>CLI, App, SSH, モバイル] --> proxy[nexcode]
...
```bash
npm install -g @bitkyc08/nexcode   # Node 18+; the Bun runtime is bundled automatically
nxc start                            # or `nxc service` to run it in the background
```

**http://localhost:10100** を開き、Web ダッシュボードですべてを設定します。40 以上の組み込みプロバイダーまたは OpenAI 互換エンドポイントの追加、モデルの選択、アカウントの管理ができます。`nxc gui` を実行すれば、いつでもダッシュボードを開き直せます。

### エージェント向け

```bash
npm install -g @bitkyc08/nexcode
nxc start     # or `nxc service`
nxc init      # interactive setup: writes ~/.nexcode/config.json and wires Codex
```

`nxc init` 自体はプロキシを起動しません。先に起動しても後から起動しても構いませんが、`nxc provider add` や `nxc combo set` などのヘッドレスコマンドは**稼働中の**プロキシと通信し、接続できない場合は非ゼロで終了します。`nxc status` / `nxc doctor` / `nxc health` で稼働状態を確認できます。

> **GitHub のスタープロンプトはユーザーが判断するものであり、エージェントが判断するものではありません。** 対話型の `nxc start`
> または `nxc service install` では、このリポジトリにスターを付けるかどうかを一度だけ尋ねる場合があります。エージェントは
...
> （`lidge-jun/nexcode にスターを付けますか？ Yes / No`）。「よかったらスターをどうぞ」のような
> 曖昧な言い方や、長い返答の末尾に紛れ込ませる形は避けてください。無回答は何も決着しません。沈黙は保留であって Yes でも記録された No でもありませんが、以降の返答で同じ質問を繰り返さないでください。CLI はこの確認を nexcode のバージョンごとに最大一度しか再表示しないため、次のバージョンが自動的に再確認します。
...
`nxc init`(対話型 CLI)や `~/.nexcode/config.json` の直接編集からもプロバイダーを追加できます。
...
`provider/` 接頭辞を省略すると、nexcode はデフォルトプロバイダーにルーティングするか、モデル名のパターンで自動
...
コントロールを表示できます。nexcode はプロバイダー config が明示的にエイリアスを指定しない限り
...
利用可否は upstream preview gate に従い、nexcode はアカウント/プロバイダーが提供時に使う
ルーティング/カタログメタデータを準備しておきます。

<p align="center">
  <img src="../assets/codex-app-picker.png" alt="推論負荷ピッカーと共に nexcode ルーティングモデルを表示する Codex App" width="480">
</p>
...
- 現在のマーカーは `openaiProviderTierVersion: 2` で、オリジナルは
  `~/.nexcode/config.json.pre-openai-tiers-v2.bak` に保存されます。
  復元: `cp ~/.nexcode/config.json.pre-openai-tiers-v2.bak ~/.nexcode/config.json`
...
- **Claude Code でも任意の LLM を。** 同じデーモンが Anthropic Messages API(`/v1/messages` + `count_tokens`)を提供します: `nxc claude` が Claude Code を完全に接続された状態で起動し、ルーティングモデルがゲートウェイモデルディスカバリでネイティブ `/model` ピッカーに表示されます(`claude-nxc-<provider>--<model>` エイリアス、Claude Code 2.1.129+)。スロットとモデルマッピングはダッシュボードの Claude ページで設定します。
...
- **クリーンな終了、残留ゼロ。** `nxc stop`(またはダッシュボードの Stop ボタン)はプロキシを終了し、インストールされたバックグラウンドサービスを停止し、Codex を元の設定に復元します。その後 `codex` は残留設定やゾンビプロセスなしに以前と同じように動作します。
...
このほか DeepSeek、Groq、OpenRouter、Together、Fireworks、Cerebras、Mistral、Hugging Face、NVIDIA NIM、MiniMax、Qwen Cloud、Tencent Cloud Coding Plan、SiliconFlow などがあります。完全な一覧は `nxc init` または[プロバイダードキュメント](https://nexcode.me/ja/reference/configuration/)で確認してください。

Cursor サポートは段階的な実験的ブリッジです: `nxc init` とダッシュボードの Add Provider ピッカーに Cursor の静的公開モデルカタログを持つローカル config として表示されます。Cursor アクセストークンを設定するとライブ
...
nexcode はポリシーでブロックする代わりに型付きの no-executor 結果を返します。
...
```bash
nxc init                       # 対話型セットアップ
nxc start [--port 10100]       # プロキシ起動; ポートが使用中なら空きポートに自動切替
nxc stop                       # プロキシ停止 + Codex を元の設定に復元
nxc restore                    # 停止せずに復元(エイリアス: nxc eject)
nxc uninstall                  # service/shim/config を削除 + Codex をオリジナルに復元
nxc ensure                     # 必要時に起動 + Codex config/cache を更新
nxc sync                       # モデルを更新 + Codex に再注入
nxc status                     # プロキシは起動中か?
nxc login <provider>           # OAuth ログイン(xai, anthropic, kimi, cursor, ...)
nxc logout <provider>          # 保存されたログインを削除
nxc account <list|current|use> # アカウント/API キープールの一覧・切替(マスク済み; refresh/auto-switch/remove/add-key 含む)
nxc gui                        # ウェブダッシュボードを開く
nxc claude [args...]           # プロキシに接続した Claude Code を起動(モデルディスカバリ オン)
nxc codex-shim install         # codex 起動時に `nxc ensure` を実行
nxc service [install|start|stop|status|uninstall]   # バックグラウンドサービスのインストール/更新/開始
nxc update [--tag preview]     # nexcode を更新; preview インストールは @preview を維持
```

### 自動起動: service vs shim

nexcode にはプロキシを自動起動する方法が 2 つあります:

| | `nxc service` / `nxc service install` | `nxc codex-shim install` |
|---|---|---|
| **方式** | OS サービスマネージャー(launchd / systemd / schtasks) | `codex` スクリプトランチャーをラップし実際の `codex.exe` は触らない |
| **タイミング** | ログイン後に常時実行 | オンデマンド — `codex` 起動時に `nxc ensure` を実行 |
| **再起動** | クラッシュ時に自動再起動 | `codex` 呼び出しごとに 1 回起動 |
| **Codex 更新** | 影響なし | 安定して置換されたランチャーは次の通常の `nxc` コマンドで修復 |
| **削除** | `nxc service uninstall` | `nxc codex-shim uninstall` |

常にプロキシを起動しておくには **service**(開発マシン推奨)、軽くオンデマンドで使うには **shim** を使ってください。

外部の Codex 更新でインストール済み shim が上書きされた場合、次の通常の `nxc` コマンドが
安定した新しいランチャーをバックアップして shim を復元します。まだ変更中のランチャーには触れず、
後続のコマンドで再試行します。修復失敗は要求されたコマンドを失敗させず警告だけを表示し、手動の
代替手段は `nxc codex-shim install` です。自動修復を無効にするには
`codexShimAutoRestore` を `false` にするか、プロセスで
`NEXCODE_CODEX_SHIM_AUTO_RESTORE=0` を設定します。
shim 自動起動はデフォルトでオンで、GUI ダッシュボードからオフにできます。設定されたプロキシポートが既に使用
中の場合、`nxc start` が自動的に別の空きローカルポートを選び、Codex の設定もそのポートに更新します。
...
`nxc uninstall` はプロキシの停止、インストールされた service の削除、Codex shim の削除、Codex config/catalog/history の
復元、`~/.nexcode` の削除を行います。

## 設定

設定ファイルは `~/.nexcode/config.json` に保存されます。ファイルが壊れている場合(不正な JSON など)
nexcode は `config.json.invalid-<timestamp>` にバックアップし、警告を出力した上でデフォルトで起動します。
...
> 動作します — nexcode がリクエスト前に末尾の `[1m]` 接尾辞を削除するためです(OpenAI 互換エンドポイントは
...
ローカルモデルも動作します。nexcode をマシンで動いている OpenAI 互換サーバーに向けてください:
...
デフォルトで nexcode は `127.0.0.1`(ループバック)にバインドされ、追加の認証は不要です。
`"hostname": "0.0.0.0"` で LAN に公開する場合、nexcode は管理 API(`/api/*`)とデータプレーン
...
サービスをインストールする場合も、同じシェルでこの変数を先に設定してから `nxc service install` を実行してください。
クライアント(スクリプト、リモートマシン)はすべてのリクエストにトークンを含める必要があります:

```
x-nexcode-api-key: your-secret-token
```

トークンはタイミング攻撃を防ぐため定数時間で比較されます。

nexcode は Codex resume 履歴を自動でリマップし、古い OpenAI チャットと nexcode が作成したプロジェクト
スレッドがプロキシ有効中に Codex App に表示され続けるようにします。オリジナルの provider/source メタデータは
`~/.nexcode/codex-history-backup.json` に記録されます。`nxc stop` / `nxc restore` はバックアップされた OpenAI 行を
OpenAI に復元し、残った nexcode ユーザースレッドも OpenAI にイジェクトして、ネイティブ Codex が `config.toml` に
...
**[コントリビュート](https://nexcode.me/ja/contributing/)** を参照してください。

## 免責事項

nexcode は独立したコミュニティプロジェクトであり、**OpenAI、Anthropic などいかなるプロバイダーとも提携したり推奨を受けたりしていません。**

一部のプロバイダー — 特に Anthropic (Claude) — はサードパーティプロキシ経由の API トラフィックルーティングでアカウントを停止または制限する場合があります。**使用の責任は自己にあります(UAYOR)。** プロバイダーを接続する前に、該当する利用規約でプロキシベースのアクセスが許可されているか確認してください。nexcode メンテナは上流プロバイダーによるアカウント措置について責任を負いません。
