...
const [nexcodeHome, codexHome, capturePath] = Bun.argv.slice(2);
if (!nexcodeHome || !codexHome || !capturePath) {
  throw new Error("runtime child requires nexcode home, codex home, and capture path");
}

for (const key of Object.keys(process.env)) {
  if (/^(?:OPENAI_|CODEX_|NEXCODE_)/.test(key) || /^(?:http|https|all)_proxy$/i.test(key)) {
    delete process.env[key];
  }
}
process.env.NEXCODE_HOME = nexcodeHome;
process.env.CODEX_HOME = codexHome;
process.env.NEXCODE_API_AUTH_TOKEN = "fixture-admission";
...
function lifecycle(body: Record<string, unknown>): string {
  const item = {
    ...
    status: "completed",
    role: "assistant",
    content: [{ type: "output_text", text: "NXC_PROBE_OK", annotations: [] }],
  };
  const response = {
  ...
  const frames = [
    ...
    { type: "response.output_text.delta", item_id: item.id, output_index: 0, content_index: 0, delta: "NXC_PROBE_OK" },
    { type: "response.output_text.done", item_id: item.id, output_index: 0, content_index: 0, text: "NXC_PROBE_OK" },
