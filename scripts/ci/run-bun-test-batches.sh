...
# qualification lane sets NEXCODE_BUN_PATH so the batches actually execute on
...
readonly BUN_BIN="${NEXCODE_BUN_PATH:-bun}"
...
run_test_once() {
  ...
  log_file="$(mktemp -t nxc-bun-test-batch.XXXXXX)"
