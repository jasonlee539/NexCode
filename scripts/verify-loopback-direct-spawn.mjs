...
const UNIQUE_MODEL = `nxc-direct-spawn-${randomUUID()}`;
const steps = [];
    ...
    const srv = http.createServer((req, res) => {
      ...
      if (req.url.startsWith("/v1/models")) {
        ...
        res.end(JSON.stringify({
          object: "list",
          data: [{ id: UNIQUE_MODEL, object: "model", created: 0, owned_by: "nexcode" }],
        }));
        return;
      }
...
async function main() {
  ...
  const home = mkdtempSync(join(tmpdir(), "nxc-direct-spawn-"));
  ...
  try {
    // The provider block `nxc sync` writes when the loopback listener is enabled: loopback host,
    ...
    // `buildCatalogEntries` also means this script exercises the same bytes `nxc sync` writes.
    const catalogPath = join(codexHome, "nexcode-models.json");
    ...
    delete env.NEXCODE_API_AUTH_TOKEN;
    record("stripped NEXCODE_API_AUTH_TOKEN from the child environment", true);
    ...
    send({ id: 1, method: "initialize", params: { clientInfo: { name: "nxc-verify", version: "1", title: "NexCode verification" } } });
    ...
    send({
      id: 3,
      method: "thread/start",
      params: { cwd: home, model: UNIQUE_MODEL, provider: "nexcode" },
    });
