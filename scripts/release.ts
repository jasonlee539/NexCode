 ...
 * `NXC_RELEASE_SSH_KEY` to the private key of the dedicated write deploy key registered as a
 * `DeployKey` bypass actor on those rulesets, and the version-bump push (and only that push) uses
 * it. Override the SSH remote with `NXC_RELEASE_SSH_REPO` when releasing a fork. Unset, the push
 ...
 *
 * Opt-in by path: without `NXC_RELEASE_SSH_KEY` the push runs exactly as before over the configured
 ...
 * fork's release to the upstream repository. `NXC_RELEASE_SSH_REPO` still wins when a maintainer
...
async function releasePushCommand(branch: string): Promise<{ command: string[]; env?: Record<string, string> }> {
  const keyPath = process.env.NXC_RELEASE_SSH_KEY?.trim();
  if (!keyPath) return { command: ["git", "push", "origin", branch] };
  const configured = process.env.NXC_RELEASE_SSH_REPO?.trim();
  ...
  if (configured && !isSshRemote(configured)) {
    console.error("✗ NXC_RELEASE_SSH_REPO is not a credential-free ssh:// or git@host:owner/repo remote; refusing to push.");
    process.exit(1);
  }
  const slug = configured || sshTargetFromOrigin(await capture(["git", "remote", "get-url", "origin"]));
  if (!slug) {
    console.error("✗ NXC_RELEASE_SSH_KEY is set but no SSH push target could be derived from origin; set NXC_RELEASE_SSH_REPO.");
    process.exit(1);
  }
...
console.log(dryRun
  ...
  : "\n✓ Published. Try:  npm install -g @bitkyc08/nexcode");
