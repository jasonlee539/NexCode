...
const TEMPLATE_PLACEHOLDER = "__NXC_BASE_URL__";
...
export function renderDshSettings(template: string, nexCodeBaseUrl: string): string {
  let parsed: URL;
  try {
    parsed = new URL(nexCodeBaseUrl);
  } catch {
    throw new Error("DSH compatibility E2E requires a valid loopback NexCode URL");
  }
  if (parsed.protocol !== "http:" || !["127.0.0.1", "localhost", "::1", "[::1]"].includes(parsed.hostname)) {
    throw new Error("DSH compatibility E2E requires a loopback NexCode URL");
  }
  ...
  return template.replace(TEMPLATE_PLACEHOLDER, nexCodeBaseUrl.replace(/\/+$/, ""));
}
...
function nexcodeConfig(baseUrl: string): NxcConfig {
  return {
    port: 0,
    hostname: "127.0.0.1",
...
async function runCompatibilityE2E(): Promise<void> {
  ...
  const root = mkdtempSync(join(tmpdir(), "nxc-dsh-rc6-e2e-"));
  const nxcHome = join(root, "nxc-home");
  const childHome = join(root, "home");
  const dshHome = join(root, "dsh-home");
  const workspace = join(root, "workspace");
  for (const dir of [nxcHome, childHome, dshHome, workspace]) mkdirSync(dir, { recursive: true, mode: 0o700 });
  ...
  try {
    ...
    saveConfig(nexcodeConfig(new URL("/v1", upstream.url).toString()));
    proxy = startServer(0);
