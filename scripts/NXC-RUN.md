# nxc-run — remote long-job runner (installed on `lidge`)

`~/bin/nxc-run`. Use it for anything that takes minutes: suites, typechecks,
builds, long probes.
...
ssh lidge 'export PATH=$HOME/bin:$PATH
  nohup nxc-run suite ~/nxc-boundary/repo 40m bun run test > /dev/null 2>&1 &'

# check — RUNNING shows time since last output, so a wedge is visible
ssh lidge 'export PATH=$HOME/bin:$PATH; nxc-run status'

ssh lidge 'export PATH=$HOME/bin:$PATH; nxc-run tail suite 40'
ssh lidge 'export PATH=$HOME/bin:$PATH; nxc-run stop suite'
```
...
Pick a ceiling above the honest runtime: the nexcode suite is ~210s idle, so
...
Do not set `NXC_TEST_NO_QUEUE=1` to "go faster" — that bypass is what let four
