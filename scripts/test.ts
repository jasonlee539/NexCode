...
export function createIsolatedTestEnvironment(
  ...
  const root = mkdtempSync(join(tmpdir(), "nexcode-test-"));
  ...
  return {
    ...
    env: {
      ...
      NXC_REAL_HOME: baseEnv.NXC_REAL_HOME ?? homedir(),
 ...
 * worked around. `NXC_TEST_NO_QUEUE=1` opts out for anyone who really wants overlap.
 */
async function waitForExclusiveRun(selfPid: number): Promise<void> {
  if (process.env.NXC_TEST_NO_QUEUE === "1") return;
  const pollMs = 5_000;
  ...
  for (;;) {
    ...
    if (!announced) {
      ...
      console.warn(
        ...
        + "Set NXC_TEST_NO_QUEUE=1 to run concurrently anyway.",
      );
    }
