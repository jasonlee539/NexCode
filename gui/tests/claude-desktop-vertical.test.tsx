...
function familyToggle(name: string): HTMLButtonElement {
  const found = Array.from(container.querySelectorAll("button.nxc-group-toggle"))
...
test("families render as a vertical stack, Opus first", async () => {
  await mount();
  expect(container.querySelector(".nxc-group-stack")).not.toBeNull();
  // The old 4-column kanban container is gone.
  expect(container.querySelector(".claude-lanes")).toBeNull();
  const names = Array.from(container.querySelectorAll(".nxc-group-name")).map(n => n.textContent);
...
test("toggling a family hides its body and persists the preference", async () => {
  ...
  const stored = testWindow.localStorage.getItem("nxc.claudeDesktop.collapsedFamilies.v2");
...
test("a collapsed family still accepts a dropped model", async () => {
  ...
  const fableSection = Array.from(container.querySelectorAll("section.nxc-group"))
    .find(section => (section.querySelector(".nxc-group-name")?.textContent ?? "") === "Fable")!;
  ...
  const fableCount = fableSection.querySelector(".nxc-group-count")?.textContent ?? "";
...
test("the header count ignores the search", async () => {
  await mount();
  await click(familyToggle("Opus"));
  const opusSection = Array.from(container.querySelectorAll("section.nxc-group"))
    .find(section => (section.querySelector(".nxc-group-name")?.textContent ?? "") === "Opus")!;
  expect(opusSection.querySelector(".nxc-group-count")?.textContent).toContain("7");
...
test("moving a model does not re-fold the family the user is working in", async () => {
  ...
  const sonnetSection = Array.from(container.querySelectorAll("section.nxc-group"))
    .find(section => (section.querySelector(".nxc-group-name")?.textContent ?? "") === "Sonnet")!;
  expect(sonnetSection.querySelector("button.nxc-group-toggle")!.getAttribute("aria-expanded")).toBe("true");
  ...
  const sonnetAfter = Array.from(container.querySelectorAll("section.nxc-group"))
    .find(section => (section.querySelector(".nxc-group-name")?.textContent ?? "") === "Sonnet")!;
  expect(sonnetAfter.querySelector("button.nxc-group-toggle")!.getAttribute("aria-expanded")).toBe("true");
});
...
test("a stored preference wins over the load-time default", async () => {
  ...
  testWindow.localStorage.setItem("nxc.claudeDesktop.collapsedFamilies.v2", JSON.stringify(["opus"]));
  await mount();
  expect(familyToggle("Opus").getAttribute("aria-expanded")).toBe("false");
