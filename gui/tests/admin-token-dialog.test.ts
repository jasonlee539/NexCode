...
test("renders stable password-manager-compatible sign-in fields", async () => {
  ...
  const dialog = document.querySelector<HTMLDialogElement>("#nexcode-admin-token-dialog");
  const form = dialog?.querySelector<HTMLFormElement>("form");
  const username = form?.elements.namedItem("username") as HTMLInputElement | null;
  const password = form?.elements.namedItem("password") as HTMLInputElement | null;

  expect(dialog).not.toBeNull();
  expect(dialog?.querySelector("h3")?.textContent).toBe("NexCode admin token (NEXCODE_ADMIN_AUTH_TOKEN)");
  expect(form?.method).toBe("post");
  expect(form?.autocomplete).toBe("on");
  expect(username?.id).toBe("nexcode-admin-token-dialog-username");
  ...
  expect(username?.value).toBe("NexCode");
  expect(password?.id).toBe("nexcode-admin-token-dialog-password");
  expect(form?.querySelector(`label[for="${password?.id}"]`)?.textContent).toBe("Admin token");
  expect(password?.type).toBe("password");
  expect(password?.autocomplete).toBe("current-password");
  expect(password?.required).toBe(true);

  password!.value = "  nxc_admin_test  ";
  form!.dispatchEvent(new testWindow.Event("submit", { bubbles: true, cancelable: true }));

  expect(await pending).toBe("nxc_admin_test");
  expect(document.querySelector("#nexcode-admin-token-dialog")).toBeNull();
...
test("uses the active UI locale instead of re-detecting browser storage", async () => {
  localStorage.setItem("nxc-lang", "en");
  setActiveLocale("ko");
