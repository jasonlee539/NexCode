...
const LEGACY_TOKEN_KEY = "nexcode-api-token";
...
beforeEach(() => {
  ...
  resetApiAuthFetchForTests(async () => {
    return window.prompt("NexCode admin token (NEXCODE_ADMIN_AUTH_TOKEN)")?.trim() || null;
  });
...
test("prompted API tokens stay memory-only and are not written to sessionStorage", async () => {
  ...
  const mockFetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (headers.get("X-NexCode-API-Key") === "fresh-token") {
      authorized = true;
...
test("validates prompted tokens with a safe read before retrying the failed request", async () => {
  ...
  const mockFetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    const key = new Headers(init?.headers).get("X-NexCode-API-Key");
...
test("cross-origin /api/* requests do not receive the API key or token prompt", async () => {
  ...
  const stateful = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    const headers = new Headers(init?.headers);
    seenHeaders.push(headers.get("X-NexCode-API-Key"));
    if (phase === "seed") {
      if (headers.get("X-NexCode-API-Key") === "local-token") return new Response("{}", { status: 200 });
      return new Response("unauthorized", { status: 401 });
    }
    return new Response("unauthorized", { status: 401 });
  }) as typeof fetch;
...
test("concurrent 401s share one token prompt and all retry with the stored token", async () => {
  ...
  const mockFetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (new URL(_input instanceof Request ? _input.url : String(_input), "http://localhost/").pathname === "/nexcode-session") {
      return new Response("unauthorized", { status: 401 });
    }
    if (headers.get("X-NexCode-API-Key") === "shared-token") {
      return new Response("{}", { status: 200 });
    }
...
test("stale concurrent 401 does not clear a token refreshed by another request", async () => {
  ...
  const mockFetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    const headers = new Headers(init?.headers);
    const key = headers.get("X-NexCode-API-Key");
...
test("canceling the token prompt once does not reopen it for the rest of the 401 fan-out", async () => {
  ...
  const mockFetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (headers.get("X-NexCode-API-Key")) {
      return new Response("{}", { status: 200 });
    }
    await new Promise<void>((resolve) => {
      release401.push(resolve);
...
function injectSessionMeta(token: string, csrf: string, origin: string): void {
  for (const [name, content] of [
    ["nexcode-session-token", token],
    ["nexcode-session-csrf", csrf],
    ["nexcode-session-origin", origin],
  ] as const) {
...
function sessionDocumentHtml(token: string, csrf: string, origin: string): string {
  return [
    ...
    `<meta name="nexcode-session-token" content="${token}">`,
    `<meta name="nexcode-session-csrf" content="${csrf}">`,
    `<meta name="nexcode-session-origin" content="${origin}">`,
    "</head><body></body></html>",
  ].join("");
}
...
test("expired session silently re-bootstraps from the served document without prompting", async () => {
  ...
  injectSessionMeta("nxc_session_stale", "stale-csrf", "http://localhost");
  ...
  const mockFetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.pathname === "/nexcode-session") {
      bootstrapFetches += 1;
      return new Response(sessionDocumentHtml("nxc_session_fresh", "fresh-csrf", "http://localhost"), {
        status: 200,
        headers: { "Content-Type": "text/html" },
      });
    }
    seenApiKeys.push(headers.get("X-NexCode-API-Key"));
    seenGuiOrigins.push(headers.get("X-NexCode-GUI-Origin"));
    if (headers.get("X-NexCode-API-Key") === "nxc_session_fresh"
      && headers.get("X-NexCode-GUI-Origin") === "http://localhost") {
      return new Response("{}", { status: 200 });
    }
    return new Response("unauthorized", { status: 401 });
  }) as typeof fetch;
  ...
  expect(seenApiKeys).toEqual(["nxc_session_stale", "nxc_session_fresh"]);
...
test("a session minted for another origin is rejected and the prompt fallback stays", async () => {
  ...
  const mockFetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    const headers = new Headers(init?.headers);
    if (url.pathname === "/nexcode-session") {
      return new Response(sessionDocumentHtml("nxc_session_foreign", "foreign-csrf", "http://192.0.2.10:10100"), {
        status: 200,
        headers: { "Content-Type": "text/html" },
      });
    }
    if (headers.get("X-NexCode-API-Key") === "manual-admin-token") return new Response("{}", { status: 200 });
    return new Response("unauthorized", { status: 401 });
  }) as typeof fetch;
  window.prompt = () => {
    promptCalls += 1;
