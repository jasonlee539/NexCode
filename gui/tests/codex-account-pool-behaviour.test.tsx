...
test("the account controller never reads the legacy proxy-log usage summary", async () => {
  accounts = [
  ...
  const seen = await mountController();
  expect(seen.current!.accounts.map(account => account.id)).toEqual(["main", "pool"]);
  expect(calls.some(call => call.startsWith("GET usage?"))).toBe(false);
});

test("account reloads remain scoped to account and active-account endpoints", async () => {
  const seen = await mountController();
  expect(calls.some(call => call.startsWith("GET usage?"))).toBe(false);
