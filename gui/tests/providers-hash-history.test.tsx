...
describe("useAppRouteState (real hook)", () => {
  ...
  test("stale layout-preference keys are cleared on mount", async () => {
    ...
    seed.localStorage.setItem("nxc-global-view", "workspace");
    seed.localStorage.setItem("nxc-providers-view", "classic");
    seed.localStorage.setItem("keep-me", "yes");

    await mountAt("#dashboard", seed.localStorage as unknown as Storage);

    expect(seed.localStorage.getItem("nxc-global-view")).toBeNull();
    expect(seed.localStorage.getItem("nxc-providers-view")).toBeNull();
