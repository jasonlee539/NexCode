...
const OPENCODE_ENVELOPE_BASE = {
  ...
  apiKeyEnv: "NEXCODE_OPENCODE_API_KEY",
  exportHint: "export NEXCODE_OPENCODE_API_KEY=<your key>",
  modelCount: 2,
  ...
  config: {
    provider: {
      nexcode: {
        ...
        name: "NexCode",
        options: { baseURL: "http://127.0.0.1:10100/v1", apiKey: "{env:NEXCODE_OPENCODE_API_KEY}" },
...
const PI_ENVELOPE_BASE = {
  ...
  apiKeyEnv: "NEXCODE_PI_API_KEY",
  exportHint: "export NEXCODE_PI_API_KEY=<your key>",
  modelCount: 2,
  modelsWithoutLimits: 1,
  format: "json",
  mediaType: "application/json",
  // Pi keys its models as an ARRAY — the shape swap is what proves a real refetch.
  config: { providers: { nexcode: { models: [{ id: "gpt-5.4" }, { id: "claude-sonnet-4-6" }] } } },
};
...
const KIMI_ENVELOPE = {
  ...
  text: '[providers.nexcode]\ntype = "openai"\nbase_url = "http://127.0.0.1:10100/v1"\n',
  config: { providers: { nexcode: { type: "openai", base_url: "http://127.0.0.1:10100/v1" } } },
};
...
test("each row fetches its own client and its dialog renders that client's exact bytes", async () => {
  ...
  expect(Array.isArray(parsed.providers.nexcode.models)).toBe(true);
...
test("download emits the fetched config under the server-provided filename and never says applied", async () => {
  ...
  try {
    ...
    expect(tomlBytes.startsWith("[providers.nexcode]")).toBe(true);
...
test("no-key state is informational and leaves copy and download enabled", async () => {
  ...
  expect(container.querySelector(".awi-clientconfig-nokey")?.textContent)
    .toContain("NEXCODE_OPENCODE_API_KEY has no key behind it yet");
