...
const MINTED = () => new Response(sessionDocumentHtml("nxc_session_fresh", "fresh-csrf", "http://localhost"), {
  status: 200,
...
test("hung bootstrap fails the wave within the deadline and a later wave re-bootstraps to success", async () => {
  ...
  const mockFetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (path === "/nexcode-session") {
      bootstrapCalls += 1;
      if (bootstrapHangs) return hangUntilAborted(init?.signal);
      return MINTED();
    }
    const key = new Headers(init?.headers ?? (input instanceof Request ? input.headers : undefined)).get("X-NexCode-API-Key");
    if (key === "nxc_session_fresh") return new Response("{}", { status: 200 });
...
test("bootstrap timeout and 5xx never open the admin-token prompt; only refusal does", async () => {
  ...
  const mockFetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    if (pathnameOf(input) === "/nexcode-session") {
...
test("the retried request carries the caller signal", async () => {
  ...
  const mockFetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    if (pathnameOf(input) === "/nexcode-session") return MINTED();
