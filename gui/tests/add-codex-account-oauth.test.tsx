...
test("OAuth completion polling continues while the desktop WebView is hidden", async () => {
  ...
  expect(calls.filter((c) => c.path.includes("/api/codex-auth/login-status"))).toHaveLength(1);
  expect(statusHolders).toHaveLength(1);
});

test("native OAuth return event requests an immediate status check", async () => {
  await mountProbe(false);
  expect(statusHolders).toHaveLength(1);

  await act(async () => {
    statusHolders.shift()!.resolve(Response.json({ status: "pending" }));
    await new Promise((r) => setTimeout(r, 40));
  });
  const beforeEvent = calls.filter((c) => c.path.includes("/api/codex-auth/login-status")).length;

  await act(async () => {
    win.dispatchEvent(new win.Event("nexcode:oauth-complete"));
    await new Promise((r) => setTimeout(r, 40));
  });

  expect(calls.filter((c) => c.path.includes("/api/codex-auth/login-status"))).toHaveLength(beforeEvent + 1);
});
