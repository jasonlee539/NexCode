...
const proxyTarget = process.env.NEXCODE_PROXY_TARGET
...
export default defineConfig({
  ...
  /* [Decision Log]
  - 목적: 로컬 Vite GUI가 실행 중인 nexcode API를 same-origin으로 호출해 CORS 잡음 없이 실제 데이터를 보여준다.
  ...
  */
  server: proxyTarget ? {
    proxy: {
      ...
      '/nexcode-session': { target: proxyTarget, changeOrigin: false },
