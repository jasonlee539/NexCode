        ...
        <div key={a.id} className={`card ${isNext(a) ? "card-active" : ""}`} style={{ marginBottom: 8 }}>
          <div className="card-head">
            ...
            <span className="card-badges">
              ...
              {!simple && <AccountPriorityBadge value={a.priority} />}
              {!simple && a.id === pinnedId && !a.paused && <span className="badge badge-muted">{t("codexAuth.pinned")}</span>}
              {!simple && <CodexTicketBadge t={t} account={a} onClick={() => onOpenReset(a)} />}
              {healthLabel && (
            ...
            {!simple && onCopyDoctor && oauthHealthShowsDoctor(healthStatus) && (
          ...
          <div className="codex-account-identity">
            <div className="codex-account-identity-copy">{a.email}{a.plan ? ` · ${a.plan}` : ""}{simple ? "" : ` · ${t("prov.accountId")}: ${displayAccountId(a.id)}`}</div>
            {!simple && (
