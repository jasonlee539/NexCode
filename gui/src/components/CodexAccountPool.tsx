...
import DesktopAccountGrid from "./DesktopAccountGrid";
...
const DOCTOR_CMD = "nxc doctor";

async function fetchDesktopUsage(apiBase: string, signal: AbortSignal): Promise<DesktopUsageResponse> {
 ...
 */
export default function CodexAccountPool({ apiBase, accountModeState = null, banner = null, embedded = false, simple = false, onActiveNeedsReauthChange, controller: injectedController, advancedExtras = null }: {
  apiBase: string;
  ...
  const localUsage = useKeyedClientResource(
    `desktop-local-usage:${apiBase}`,
    [apiBase],
    signal => fetchDesktopUsage(apiBase, signal),
  ...
  const togglePaused = async (account: CodexAccountEntry) => {
    ...
    showActionFeedback(t(result.ok
      ...
      email: codexAccountDisplayLabel(accounts, account, t),
    }), result.ok ? "ok" : "err");
  };
  ...
  const remove = async (id: string) => {
    ...
    const label = target
      ? codexAccountDisplayLabel(accounts, target, t)
      : t("pws.accountOrdinal", { count: "1" });
  ...
  const showDoctorCopy = !embedded && !simple;
  ...
  return (
    <div>
      ...
      {simple && actionFeedback && (
        <div className={`desktop-inline-status is-${actionFeedbackTone ?? "ok"}`} role="status">
          {actionFeedback}
        </div>
      )}
      ...
      {simple && loadState === "loading" && accounts.length === 0 && (
        <div className="desktop-page-loading" role="status">{t("common.loading")}</div>
      )}
      {simple && loadState === "error" && accounts.length === 0 && (
        <div className="desktop-page-error" role="alert">
          <span>{t("accounts.loadFailed")}</span>
          <button type="button" className="btn btn-sm btn-ghost" onClick={() => { void load(); }}>{t("common.retry")}</button>
        </div>
      )}
      ...
      {!(loadState === "loading" && accounts.length === 0) && (
        simple ? (
          <DesktopAccountGrid
            ...
            refreshing={refreshingQuota || controller.refreshing}
            localUsage30d={localUsage.data?.ranges["30d"] ?? null}
            usageLoading={localUsage.loading && !localUsage.data}
            onRefresh={() => { void refreshQuotas(); }}
