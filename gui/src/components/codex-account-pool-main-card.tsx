    ...
    <div className={`card ${isMainActive ? "card-active" : ""}`} style={{ marginBottom: 12 }}>
      <div className="card-head">
        ...
        <span className="card-badges">
          {!simple && main && <CodexTicketBadge t={t} account={{ ...main, id: "__main__" } as CodexAccountEntry} onClick={() => onOpenReset({ ...main, id: "__main__" } as CodexAccountEntry)} />}
          {main?.paused && (
            <span className="badge badge-muted" title={t("codexAuth.pausedHint")}>
              {t("codexAuth.paused")}
            </span>
          )}
          {!simple && <AccountPriorityBadge value={mainSwitchEntry.priority} />}
          {!simple && pinnedId === "__main__" && !main?.paused && <span className="badge badge-muted">{t("codexAuth.pinned")}</span>}
          {healthLabel && (
        ...
        {!simple && onCopyDoctor && oauthHealthShowsDoctor(main?.health?.status) && (
...
export function CodexAccountPoolPageHead({
  ...
  return (
    ...
    >
      {!embedded && !simple && <h2 className="page-title">{t("nav.codexAuth")}</h2>}
