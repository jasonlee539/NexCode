  ...
  const startOAuth = useCallback(async (requestedId?: string) => {
    ...
    try {
      ...
      if (data.url) {
        ...
        // OAuth finishes in the system browser while the desktop WebView is hidden.
        // Keep this one poll alive in the background and also let the native app
        // request an immediate check when its custom callback brings NexCode forward.
        ...
        const stopVisibilityPoll = startVisibilityPoll(pollLoginStatus, 2000, {
          pauseWhenHidden: false,
          immediate: true,
        });
        const onDesktopOAuthComplete = () => { void pollLoginStatus(); };
        window.addEventListener("nexcode:oauth-complete", onDesktopOAuthComplete);
        pollRef.current = () => {
          stopVisibilityPoll();
          window.removeEventListener("nexcode:oauth-complete", onDesktopOAuthComplete);
        };
