...
const STATUS_CODES: Record<number, LocalizedInfo> = {
  ...
  401: {
    en: { label: "Unauthorized", description: "Credentials are missing, expired, or invalid. Re-login or refresh the account/provider credentials used by nexcode." },
    fr: { label: "Non autorisé", description: "Les identifiants sont absents, expirés ou non valides. Reconnectez-vous ou actualisez les identifiants du compte ou du fournisseur utilisés par nexcode." },
    ko: { label: "인증 필요", description: "자격 증명이 없거나 만료되었거나 유효하지 않습니다. nexcode에서 사용하는 계정 또는 제공자 자격 증명을 다시 로그인하거나 갱신해야 합니다." },
    zh: { label: "未授权", description: "凭据缺失、已过期或无效。请重新登录，或刷新 nexcode 使用的账号/提供商凭据。" },
    "zh-TW": { label: "未授權", description: "憑證缺失、已過期或無效。請重新登入，或重新整理 nexcode 使用的帳號/供應商憑證。" },
    de: { label: "Nicht autorisiert", description: "Anmeldedaten fehlen, sind abgelaufen oder ungültig. Melde dich erneut an oder aktualisiere die von nexcode genutzten Konto-/Anbieter-Zugangsdaten." },
    ru: { label: "Не авторизован", description: "Учётные данные отсутствуют, истекли или недействительны. Войдите заново или обновите учётные данные аккаунта или провайдера, которые использует nexcode." },
    ja: { label: "認証が必要", description: "認証情報が不在・期限切れ・無効です。nexcode が使用するアカウントまたはプロバイダー認証情報を再ログインまたは更新してください。" },
    tr: { label: "Yetkisiz erişim", description: "Kimlik bilgileri eksik, süresi dolmuş veya geçersiz. nexcode tarafından kullanılan hesap veya sağlayıcı kimlik bilgilerini yeniden doğrulayın." },
  },
  402: {
  ...
  404: {
    en: { label: "Not found", description: "The requested route, model, account, or upstream resource was not found. Verify the model name and nexcode provider configuration." },
    fr: { label: "Introuvable", description: "La route, le modèle, le compte ou la ressource en amont demandés sont introuvables. Vérifiez le nom du modèle et la configuration du fournisseur nexcode." },
    ko: { label: "찾을 수 없음", description: "요청한 경로, 모델, 계정 또는 업스트림 리소스를 찾을 수 없습니다. 모델 이름과 nexcode 제공자 설정을 확인해야 합니다." },
    zh: { label: "未找到", description: "找不到请求的路由、模型、账号或上游资源。请确认模型名称和 nexcode 提供商配置。" },
    "zh-TW": { label: "未找到", description: "找不到請求的路由、模型、帳號或上游資源。請確認模型名稱和 nexcode 供應商配置。" },
    de: { label: "Nicht gefunden", description: "Die angeforderte Route, das Modell, das Konto oder die Upstream-Ressource wurde nicht gefunden. Prüfe Modellname und nexcode-Anbieterkonfiguration." },
    ru: { label: "Не найдено", description: "Запрошенный маршрут, модель, аккаунт или вышестоящий ресурс не найден. Проверьте имя модели и конфигурацию провайдера в nexcode." },
    ja: { label: "見つかりません", description: "要求されたルート、モデル、アカウント、上流リソースが見つかりませんでした。モデル名と nexcode プロバイダー設定を確認してください。" },
    tr: { label: "Bulunamadı", description: "İstenen rota, model, hesap veya yukarı akış kaynağı bulunamadı. Model adını ve nexcode sağlayıcı yapılandırmasını doğrulayın." },
  },
  408: {
  ...
  424: {
    en: { label: "Provider dependency failed", description: "A required upstream dependency failed while nexcode was routing the request. Retry later or switch to another configured provider." },
    fr: { label: "Échec d’une dépendance du fournisseur", description: "Une dépendance en amont requise a échoué pendant le routage de la requête par nexcode. Réessayez plus tard ou sélectionnez un autre fournisseur configuré." },
    ko: { label: "제공자 의존성 실패", description: "nexcode가 요청을 라우팅하는 동안 필요한 업스트림 의존성이 실패했습니다. 나중에 재시도하거나 다른 설정된 제공자로 전환해야 합니다." },
    zh: { label: "提供商依赖失败", description: "nexcode 路由请求时，必需的上游依赖失败。请稍后重试，或切换到另一个已配置的提供商。" },
    "zh-TW": { label: "供應商依賴失敗", description: "nexcode 路由請求時，必需的上游依賴失敗。請稍後重試，或切換到另一個已配置的供應商。" },
    de: { label: "Anbieter-Abhängigkeit fehlgeschlagen", description: "Eine erforderliche Upstream-Abhängigkeit ist fehlgeschlagen, während nexcode die Anfrage geroutet hat. Später erneut versuchen oder zu einem anderen Anbieter wechseln." },
    ru: { label: "Сбой зависимости провайдера", description: "Необходимая вышестоящая зависимость дала сбой, пока nexcode маршрутизировал запрос. Повторите попытку позже или переключитесь на другого настроенного провайдера." },
    ja: { label: "プロバイダー依存の失敗", description: "nexcode がリクエストをルーティング中に必要な上流依存が失敗しました。後で再試行するか、別の設定済みプロバイダーに切り替えてください。" },
    tr: { label: "Sağlayıcı bağımlılığı başarısız", description: "nexcode isteği yönlendirirken gerekli bir yukarı akış bağımlılığı başarısız oldu. Daha sonra tekrar deneyin veya başka bir sağlayıcıya geçin." },
  },
  429: {
  ...
  499: {
    en: { label: "Client closed request", description: "The client disconnected or canceled the request before nexcode finished routing it. Retry if the cancellation was accidental." },
    fr: { label: "Requête fermée par le client", description: "Le client s’est déconnecté ou a annulé la requête avant la fin de son routage par nexcode. Réessayez si l’annulation était involontaire." },
    ko: { label: "클라이언트 취소", description: "nexcode가 라우팅을 끝내기 전에 클라이언트 연결이 끊기거나 요청이 취소되었습니다. 의도한 취소가 아니면 다시 시도해야 합니다." },
    zh: { label: "客户端已取消", description: "nexcode 完成路由前，客户端已断开连接或取消请求。如果不是有意取消，请重试。" },
    "zh-TW": { label: "客戶端已取消", description: "nexcode 完成路由前，客戶端已斷開連線或取消請求。如果不是有意取消，請重試。" },
    de: { label: "Client hat Anfrage geschlossen", description: "Der Client hat die Verbindung getrennt oder die Anfrage abgebrochen, bevor nexcode das Routing abgeschlossen hat. Bei versehentlichem Abbruch erneut versuchen." },
    ru: { label: "Запрос закрыт клиентом", description: "Клиент отключился или отменил запрос до того, как nexcode завершил его маршрутизацию. Если отмена была случайной, повторите попытку." },
    ja: { label: "クライアントがリクエストをクローズ", description: "nexcode がルーティングを終える前にクライアントが切断またはキャンセルしました。意図しないキャンセルなら再試行してください。" },
    tr: { label: "İstemci isteği kapattı", description: "nexcode yönlendirmeyi bitirmeden önce istemci bağlantıyı kesti veya isteği iptal etti. İptal kazara yapıldıysa tekrar deneyin." },
  },
  500: {
    en: { label: "Proxy error", description: "nexcode hit an internal error while handling the request. Retry once, then check proxy logs if it repeats." },
    fr: { label: "Erreur du proxy", description: "nexcode a rencontré une erreur interne lors du traitement de la requête. Réessayez une fois, puis consultez les journaux du proxy si l’erreur se reproduit." },
    ko: { label: "프록시 오류", description: "nexcode가 요청을 처리하는 동안 내부 오류가 발생했습니다. 한 번 재시도하고 반복되면 프록시 로그를 확인해야 합니다." },
    zh: { label: "代理错误", description: "nexcode 处理请求时发生内部错误。请先重试一次；如果重复出现，请检查代理日志。" },
    "zh-TW": { label: "代理錯誤", description: "nexcode 處理請求時發生內部錯誤。請先重試一次；如果重複出現，請檢查代理日誌。" },
    de: { label: "Proxy-Fehler", description: "nexcode ist bei der Anfragebearbeitung auf einen internen Fehler gestoßen. Einmal erneut versuchen, bei Wiederholung Proxy-Logs prüfen." },
    ru: { label: "Ошибка прокси", description: "В nexcode произошла внутренняя ошибка при обработке запроса. Повторите попытку один раз; если ошибка повторяется, проверьте логи прокси." },
    ja: { label: "プロキシエラー", description: "nexcode がリクエスト処理中に内部エラーに遭遇しました。1 回再試行し、繰り返す場合はプロキシログを確認してください。" },
    tr: { label: "Proxy hatası", description: "nexcode isteği işlerken dahili bir hatayla karşılaştı. Bir kez tekrar deneyin, tekrarlarsa proxy günlüklerini kontrol edin." },
  },
  502: {
...
const GENERIC_STATUS: { client: LocalizedInfo; server: LocalizedInfo } = {
  ...
  server: {
    en: { label: "Server or upstream error", description: "nexcode or an upstream provider failed while processing the request. Retry later or route the request to another provider." },
    fr: { label: "Erreur du serveur ou du fournisseur en amont", description: "nexcode ou un fournisseur en amont a échoué lors du traitement de la requête. Réessayez plus tard ou acheminez la requête vers un autre fournisseur." },
    ko: { label: "서버 또는 업스트림 오류", description: "nexcode 또는 업스트림 제공자가 요청 처리 중 실패했습니다. 나중에 재시도하거나 다른 제공자로 라우팅해야 합니다." },
    zh: { label: "服务器或上游错误", description: "nexcode 或上游提供商处理请求时失败。请稍后重试，或将请求路由到其他提供商。" },
    "zh-TW": { label: "伺服器或上游錯誤", description: "nexcode 或上游供應商處理請求時失敗。請稍後重試，或將請求路由到其他供應商。" },
    de: { label: "Server- oder Upstream-Fehler", description: "nexcode oder ein Upstream-Anbieter ist bei der Anfragebearbeitung fehlgeschlagen. Später erneut versuchen oder zu einem anderen Anbieter routen." },
    ru: { label: "Ошибка сервера или провайдера", description: "nexcode или вышестоящий провайдер завершил обработку запроса с ошибкой. Повторите попытку позже или направьте запрос другому провайдеру." },
    ja: { label: "サーバーまたは上流エラー", description: "nexcode または上流プロバイダーがリクエスト処理中に失敗しました。後で再試行するか、リクエストを別のプロバイダーにルーティングしてください。" },
    tr: { label: "Sunucu veya yukarı akış hatası", description: "nexcode veya bir yukarı akış sağlayıcısı isteği işlerken başarısız oldu. Daha sonra tekrar deneyin veya isteği başka bir sağlayıcıya yönlendirin." },
  },
};
