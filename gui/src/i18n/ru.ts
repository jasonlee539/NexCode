...
import { desktopRu } from "./desktop-translations";
...
export const ru: Record<TKey, string> = {
  ...
  "auth.adminTokenTitle": "Токен администратора NexCode (NEXCODE_ADMIN_AUTH_TOKEN)",
  ...
  // startup health
  "startup.title": "Служба управления",
  "startup.subtitle": "Настройте запуск локального интерфейса и средств управления аккаунтами NexCode. Запросы моделей Codex идут напрямую.",
  ...
  "startup.summary.native": "Запросы Codex идут напрямую",
  "startup.summary.protected": "Управление NexCode будет доступно после перезагрузки",
  "startup.summary.atRisk": "Управление NexCode может быть недоступно после перезагрузки",
  "startup.riskDetail": "Для службы управления NexCode не настроен постоянный запуск.",
  "startup.riskDetailCustomLocal": "Codex направлен на пользовательский локальный шлюз. nexcode не может управлять или проверять его перезапуск.",
  ...
  "startup.safeDetail": "Запуск службы управления настроен. Маршрут запросов Codex не меняется.",
  "startup.routing": "Подключение Codex",
  "startup.routing.proxy": "Устаревший локальный прокси",
  ...
  "startup.restartProtection": "Восстановление службы",
  ...
  "startup.serviceHint": "Запускается при входе и перезапускает службу управления после сбоя.",
  ...
  "startup.shimHint": "Запускает nxc ensure при запуске поддерживаемого скриптового лаунчера Codex.",
  ...
  "startup.tray.hint": "Добавляет значок для запуска, остановки, перезапуска, открытия панели и проверки службы управления.",
  ...
  "startup.tray.error": "Действие Windows tray завершилось ошибкой. Подробности: nxc tray status.",
  ...
  "startup.codexRuntime.clampHidden": "Некоторые уровни рассуждений скрыты, потому что NexCode использует Codex {version}.",
  "startup.codexRuntime.clampHiddenWithEfforts": "Некоторые уровни рассуждений скрыты, потому что NexCode использует Codex {version} (удалены: {efforts}).",
  "startup.codexRuntime.olderBinary": "NexCode использует более старый бинарник Codex ({version}). Доступна более новая установка.",

  // dashboard
  "dash.subtitle": "Управляйте аккаунтами Codex, моделями и локальными интеграциями. Запросы моделей не проходят через NexCode.",
  ...
  "dash.mem.restart": "Перезапустить службу управления",
  ...
  "dash.mem.reconnecting": "Служба управления перезапускается… ожидание подключения",
  "dash.mem.restartFailed": "Не удалось перезапустить службу управления. Проверьте, что служба запущена.",
  "dash.mem.restartNoSupervisor": "Восстановление службы не настроено. Возможно, её придётся запустить вручную.",
  ...
  "dash.cannotConnect": "Не удаётся подключиться к службе управления NexCode. Она запущена?",
  "dash.runStart": "Выполните {cmd}, чтобы запустить службу управления.",
  "dash.stop": "Остановить службу управления",
  "dash.stopConfirm": "Остановить службу управления NexCode? Запросы Codex продолжат идти напрямую.",
  "dash.stopFailed": "Не удалось остановить службу управления (HTTP {status}).",
  "dash.maSwitchFailed": "Не удалось переключить режим (HTTP {status}).",
  "dash.maNetworkError": "Ошибка сети — служба управления запущена?",
  "dash.stopping": "Остановка…",
  "dash.actions": "Служба управления",
  ...
  "dash.codexAutoStart": "Запускать nexcode вместе с Codex",
  "dash.codexAutoStartHint": "Разрешает установленному launcher shim выполнять nxc ensure. Эта настройка не устанавливает защиту перезапуска; проверьте фактическое состояние в разделе безопасности запуска.",
  ...
  "dash.shadowCallTooltip": "Codex App в фоновом режиме вызывает служебную модель для генерации заголовков тредов, сообщений коммитов и оркестрации навыков. Эта модель менялась между версиями клиента, поэтому nexcode перехватывает весь набор: {models}. Включите функцию, чтобы перенаправлять такие вызовы на выбранную вами модель.",
  ...
  "dash.maintenanceHint": "Обновите каталог моделей Codex или установите более новую версию nexcode.",
  ...
  "dash.projectConfigTitle": "Конфигурация Codex в проекте обходит NexCode",
  "dash.projectConfigHint": "Эти локальные настройки репозитория переопределяют прокси NexCode (например, направляют запросы напрямую в OpenCode Go). Удалите их, чтобы в этом проекте действовала маршрутизация из ~/.codex/config.toml.",
  "dash.checkUpdate": "Проверить обновления",
  "dash.updateTitle": "Обновление nexcode",
  "dash.updateDesc": "Проверьте npm для выбранного канала, затем решите, перезапускать ли службу управления после установки.",
  ...
  "dash.updateRestartHint": "Рекомендуется. Текущий GUI продолжает работать на старом коде, пока служба управления не перезапустится.",
  "dash.runUpdate": "Обновить",
  "dash.updateReconnecting": "Ожидание перезапущенной службы управления…",
  "dash.updateStatus.running": "Обновление nexcode.",
  "dash.updateStatus.restarting": "Обновление установлено. Перезапуск службы управления.",
  ...
  // providers
  "prov.subtitle": "Настройте вышестоящих провайдеров, которых nexcode маршрутизирует в Codex. Войдите в аккаунт, добавьте провайдера или отредактируйте конфигурацию вручную.",
  ...
  "prov.saved": "Сохранено! Перезапустите службу управления, чтобы применить изменения.",
  ...
  "oauthTos.anthropicBody": "Прямое повторное использование OAuth-токенов подписки Claude через сторонний прокси, такой как NexCode, не является поддерживаемой интеграцией Anthropic и может привести к ограничению доступа. Поддерживаемые интеграции Agent SDK, использующие подписки Claude, — это отдельный механизм.",
  "oauthTos.highBody": "NexCode подключает {provider} через сторонний механизм OAuth. Неподдерживаемое использование может привести к ограничению или приостановке доступа.",
  "oauthTos.elevatedBody": "NexCode подключает {provider} через неофициальный механизм OAuth. По возможности используйте официальный клиент; нетипичный или автоматизированный трафик может быть расценён как злоупотребление, и доступ может быть ограничен или приостановлен.",
  "oauthTos.saferPath": "Более безопасный вариант: вместо этого настройте API-ключ в NexCode.",
  ...
  "prov.networkError": "Ошибка сети. Проверьте, что служба управления запущена, и повторите попытку.",
  ...
  "models.subtitle": "Управляйте тем, какие модели видит Codex — нативные GPT (сквозной проброс) и модели маршрутизируемых провайдеров, сгруппированные по провайдеру (нажмите на заголовок, чтобы свернуть группу). Скрытые модели исчезают из каталога и селектора, но остаются вызываемыми по точному id. Изменения применяются на следующем ходе Codex — nexcode сбрасывает 5-минутный кэш моделей Codex, поэтому перезапуск не требуется.",
  ...
  "models.networkError": "Ошибка сети — запущена ли служба управления?",
  "models.loadFail": "Не удалось загрузить модели — запущена ли служба управления?",
  ...
  "sub.networkError": "Ошибка сети — запущена ли служба управления?",
  "sub.loadFail": "Не удалось загрузить модели — запущена ли служба управления?",
  ...
  "logs.subtitle": "Недавние запросы через локальный прокси nexcode, новые сверху.",
  ...
  "storage.policy.schedule.startup": "При запуске службы управления",
  ...
  "modal.networkError": "Ошибка сети — запущена ли служба управления?",
  ...
  "modal.invalidPreset": "Этот встроенный пресет провайдера неполный. Перезапустите службу управления и попробуйте ещё раз.",
  ...
  "pws.copyDoctor": "Скопировать nxc doctor",
  ...
  "integrations.subtitle": "Подключайте клиенты к nexcode, управляйте учётными данными и восстанавливайте конфигурацию клиентов.",
  ...
  "integrations.codex.body": "Подключением Codex управляет прокси-сервис. При запуске nexcode оно применяется, а при остановке сервиса восстанавливается нативная маршрутизация.",
  ...
  "integrations.dialog.desktop.changes": "Если {path} содержит профиль шлюза, управляемый nexcode, Desktop сначала выберет новый стандартный профиль без учётных данных, а затем удалит старый профиль и резервную копию.",
  "integrations.dialog.desktop.breakage": "Claude Desktop вернётся к обычному Claude вместо моделей, маршрутизируемых через nexcode.",
  "integrations.dialog.desktop.undo": "При повторном включении профиль nexcode будет создан заново из сохранённых назначений моделей.",
  ...
  "integrations.native.error.desktopCleanupIncomplete": "Claude Desktop указывает на стандартный режим, но старые файлы учётных данных nexcode остались в: {paths}.",
  ...
  "integrations.detail.grokAbsent": "В конфигурации нет блока nexcode",
  "integrations.dialog.grok.title": "Отключить интеграцию Grok Build?",
  "integrations.dialog.grok.changes": "Из {path} будет удалён только блок, отмеченный nexcode. Содержимое, добавленное вручную вне блока, останется без изменений.",
  "integrations.dialog.grok.breakage": "После отключения псевдонимы моделей nexcode исчезнут из Grok Build. Модели, использовавшиеся с учётной записью xAI, останутся доступны.",
  "integrations.dialog.grok.undo": "Если nexcode запущен на loopback-адресе, при повторном включении будет записан новый блок из списка доступных на тот момент моделей.",
  "integrations.dialog.grok.confirm": "Отключить",
  "integrations.native.msg.nonLoopbackRemoved": "Grok Build можно регистрировать автоматически, только когда nexcode запущен на loopback-адресе. Предыдущий блок, указывавший на loopback-адрес, удалён.",
  "integrations.native.msg.nonLoopbackRemovedNoop": "Grok Build можно регистрировать автоматически, только когда nexcode запущен на loopback-адресе. Предыдущего блока для удаления не было.",
  "integrations.native.msg.nonLoopbackSuperseded": "Grok Build можно регистрировать автоматически, только когда nexcode запущен на loopback-адресе. Тем временем другая программа записала в конфигурацию новый блок, поэтому текущий блок в файле создан не этим запросом.",
  "integrations.native.error.orphanedMarker": "В {path} есть начальная метка nexcode, но нет конечной. Файл не изменён, потому что невозможно надёжно определить конец блока.",
  ...
  "integrations.onboarding": "При применении сначала сохраняется резервная копия, а затем записывается один блок провайдера nexcode. При отключении удаляется только этот блок, а сохранённый снимок можно восстановить.",
  "integrations.empty.title": "Установленные клиенты не обнаружены",
  "integrations.empty.body": "Установите поддерживаемый клиент, затем вернитесь сюда и примените nexcode.",
  ...
  "integrations.error.conflict": "После записи nexcode конфигурация изменилась. Ничего не было удалено.",
  ...
  "integrations.bulk.body": "Будет удалён только блок, принадлежащий nexcode. Для каждого клиента предварительно сохраняется снимок исходного состояния.",
  ...
  "integrations.semantics.opencode": "Действует только при прямом запуске с диска; внедрение окружения через nxc opencode имеет приоритет.",
  ...
  "integrations.semantics.dsh": "NexCode управляет только llm-pi-ai.providers.nexcode в $DSH_HOME/settings.yaml. DSH применяет этот провайдер горячей перезагрузкой; модель по умолчанию и deepseek-official остаются без изменений. Сейчас поддерживается только loopback; реальные учётные данные не записываются.",
  "integrations.semantics.mcode": "Управляет только custom_provider.nexcode. Модель по умолчанию и вход MiniMax не меняются.",
  "integrations.semantics.zcode": "Управляет только provider.nexcode в ~/.zcode/v2/config.json. Вход Z.ai и другие провайдеры не меняются. Перезапустите ZCode после изменений.",
  "integrations.semantics.prime": "Управляет только providers.nexcode в models.json Prime Agent — ~/.prime/agent, если PRIME_AGENT_CODING_AGENT_DIR не переопределяет путь. Другие провайдеры и переопределения моделей не меняются. Применяется к новым сессиям.",
  ...
  "codexAuth.switchDesc": "После подтверждения NexCode закроет запущенные процессы Codex и переключит нативную учётную запись для Codex CLI и Codex App.",
  "codexAuth.cacheWarning": "При переключении запущенные процессы Codex App и CLI будут закрыты, затем будет заменена основная учётная запись Codex. Сначала сохраните работу.",
  "codexAuth.setAsNext": "Переключиться на аккаунт",
  "codexAuth.cancel": "Отмена",
  "codexAuth.switchBack": "Вернуться на основной аккаунт?",
  "codexAuth.switchBackDesc": "После подтверждения NexCode закроет запущенные процессы Codex и восстановит нативную учётную запись, сохранённую до начала управления аккаунтами.",
  ...
  "codexAuth.catalogRefreshPending": "Изменение сохранено, но обновление каталога моделей Codex ещё не завершено. Выполните nxc sync, чтобы повторить попытку.",
  ...
  "accountPool.strategyDesc": "Как NexCode назначает аккаунт новой/непривязанной задаче.",
  ...
  "api.subtitle": "Сгенерированные API-ключи дают внешним приложениям доступ к прокси nexcode. Аутентификация — через заголовок {authHeader}; какие заголовки принимает каждый эндпоинт, смотрите в таблице ниже.",
  ...
  "api.authLoopback": "Loopback-привязки (127.0.0.1 или ::1) обходят аутентификацию. Для удалённых привязок нужен сгенерированный nxc_-ключ или NEXCODE_API_AUTH_TOKEN.",
  ...
  "claude.injectAgentsDesc": "Регистрирует модели, выбранные на вкладке «Подагенты» (плюс текущую модель по умолчанию), как доступных для вызова агентов Claude Code (nxc-*). Применяется со следующей сессии.",
  ...
  "claude.smallFastModelNativeWarning": "Если модель не выбрана, NexCode не задаёт переопределения вспомогательной модели. Claude Code может использовать нативную модель Sonnet, что может привести к расходам у вашего нативного провайдера.",
  ...
  "grok.subtitle": "Модели, зарегистрированные nexcode в вашей конфигурации Grok.",
  ...
  "grok.notConfiguredHint": "Установите Grok и перезапустите службу управления — NexCode запишет управляемый блок в:",
  ...
  "cws.howBody": "Запросите у Codex публичное имя модели комбо. Если оно не задано, используется combo/<id>. NexCode выбирает цель и переключается на следующую только при сбоях вышестоящего провайдера, допускающих повтор. Если доступных целей не осталось, запрос завершается ошибкой, а не переходит на глобальный провайдер по умолчанию.",
