...
export default function Models({ apiBase, restartEpoch = 0, simple = false }: { apiBase: string; restartEpoch?: number; simple?: boolean }) {
   ...
   */
  const [tab, setTab] = useState<ModelsTab>(() => simple ? "catalog" : readModelsTab());
  const [mounted, setMounted] = useState<ReadonlySet<ModelsTab>>(() => new Set([simple ? "catalog" : readModelsTab()]));
  ...
  useEffect(() => {
    const syncFromHash = () => activateTab(simple ? "catalog" : readModelsTab());
  ...
  const t: TFn = useT();
  const cacheKey = `nxc.models.catalog.v1:${apiBase}`;
        ...
        <div className="modal-overlay" role="dialog" aria-modal="true" aria-label={t("models.v2Label")} onClick={() => setV2HelpOpen(false)} onKeyDown={e => { if (e.key === "Escape") setV2HelpOpen(false); }}>
          <div className="modal-card" onClick={e => e.stopPropagation()}>
            ...
            <div className="models-help-link">
              <a className="text-control" href="https://nexcode.me/guides/sub-agent-surface/" target="_blank" rel="noreferrer" style={{ color: "var(--accent)" }}>
                {t("models.v2DocsLink")}
              </a>
            </div>
    ...
    <div className="models-workspace-shell">
      ...
      <div className="models-workspace-root" aria-busy={catalogState.refreshing || undefined}>
        ...
        <section className="models-workspace-main" aria-label={t("models.workspace.mainAria")}>
          {!simple && controlsBlock}
          {!simple && collapseControls}
  ...
  return (
    <>
      ...
      */}
      {!simple && <p className="page-sub">{t(SUBTITLE_TKEY[tab])}</p>}
