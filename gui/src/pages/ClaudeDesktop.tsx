 ...
 */
const FAMILY_COLLAPSE = makeCollapseStore("nxc.claudeDesktop.collapsedFamilies.v2");
...
interface DesktopProfile {
  ...
  /** Written by the apply route; mirrors NxcClaudeDesktopProfile so a round-trip keeps them. */
...
export default function ClaudeDesktop({
  ...
  const cacheKey = `nxc.claude-desktop.v1:${apiBase}`;
  ...
  const statusCacheKey = `nxc.claude-desktop.status.v1:${apiBase}`;
  ...
  return (
    <>
      ...
      <div className="nxc-group-stack" aria-label={t("claudeDesktop.assignmentsLabel")}>
        {FAMILIES.map(family => {
          ...
          <section
            key={family}
            className={`nxc-group${isCollapsed ? " collapsed" : ""}`}
            aria-labelledby={`claude-lane-${family}`}
            onDragOver={event => event.preventDefault()}
            onDrop={event => dropOnLane(event, family)}
          >
            <header className={`nxc-group-head${isCollapsed ? "" : " open"}`}>
              ...
              <h3 id={`claude-lane-${family}`} className="nxc-group-heading">
                <button
                  type="button"
                  className="nxc-group-toggle"
                  aria-expanded={!isCollapsed}
                  aria-controls={`claude-lane-body-${family}`}
                  onClick={() => toggleFamily(family)}
                >
                  <IconChevron
                    className="nxc-chevron"
                    width={14}
                    height={14}
                    aria-hidden="true"
                    style={{ transform: isCollapsed ? "none" : "rotate(90deg)" }}
                  />
                  <span className="nxc-group-name">{t(FAMILY_KEYS[family])}</span>
                  <span className="nxc-group-count">
