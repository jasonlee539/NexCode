...
const STARTUP_PAGE_CACHE_PREFIX = "nxc.startup.page.v1:";
...
function deriveCodexRuntimeNotice(
  ...
  const doctorSync = shellChain(["nxc doctor --fix-codex-runtime", "nxc sync"], platform);
  if (clampActive) {
    return {
      warning: efforts
        ? t("startup.codexRuntime.clampHiddenWithEfforts", { version, efforts })
        : t("startup.codexRuntime.clampHidden", { version }),
      fix: newer ? doctorSync : "nxc sync",
    };
  }
  if (newer) {
    return {
