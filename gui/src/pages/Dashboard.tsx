...
export default function Dashboard({ apiBase, simple = false }: { apiBase: string; simple?: boolean }) {
  return simple ? <DesktopDashboard apiBase={apiBase} /> : <DashboardWorkspace apiBase={apiBase} />;
}

function DashboardWorkspace({ apiBase }: { apiBase: string }) {
  ...
  if (error) {
    return (
      <EmptyState style={{ marginTop: 40 }} icon={<IconAlert />}
        ...
        <Trans k="dash.runStart" cmd="nxc start" />
      </EmptyState>
    );
  }
