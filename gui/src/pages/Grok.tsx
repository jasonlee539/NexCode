...
const GROUP_COLLAPSE = makeCollapseStore("nxc.grok.collapsedGroups.v2");
 ...
 * by injectGrokConfig — the same path `nxc start`/`ensure`/`restart` use. Aliases shown
...
export default function Grok({ apiBase, active = true }: { apiBase: string; active?: boolean }) {
  const t = useT();
  const cacheKey = `nxc.grok.status.v1:${apiBase}`;
      ...
      {status && status.candidates.length > 0 && (
        <div className="nxc-group-stack">
          ...
          {GROUPS.map(group => {
            ...
            return (
              <section key={group.id} className={`nxc-group${isCollapsed ? " collapsed" : ""}`} aria-labelledby={`grok-group-${group.id}`}>
                <header className={`nxc-group-head${isCollapsed ? "" : " open"}`}>
                  <h3 id={`grok-group-${group.id}`} className="nxc-group-heading">
                    <button
                      type="button"
                    ...
                    >
                      ...
                      />
                      <span className="nxc-group-name">{t(group.tkey)}</span>
