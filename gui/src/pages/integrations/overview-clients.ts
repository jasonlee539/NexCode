 ...
 * Codex CLI.
 *
 * `routingInjected` — server-derived as `routingKind === "nexcode-local"` —
 * is the only field that answers "is nexcode in Codex's path right now".
...
function claudeDesktopRow(
  ...
  // "absent": Desktop is still routing through NexCode until cleanup lands.
  if (!toggleOn) {
