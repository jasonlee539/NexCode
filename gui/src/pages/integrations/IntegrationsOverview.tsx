...
export default function IntegrationsOverview({
  ...
  const statesResource = useDataSurface<IntegrationStatus[]>(
    ...
    [apiBase],
    fetchStates,
    { isEmpty: rows => rows.length === 0, enabled: active, sessionCacheKey: `nxc.integrations.states.v1:${apiBase}` },
  );
  const historyResource = useDataSurface<IntegrationJournalRow[]>(
    `integration-journal-all:${apiBase}`,
    [apiBase],
    fetchHistory,
    { isEmpty: rows => rows.length === 0, enabled: active, sessionCacheKey: `nxc.integrations.journal.v1:${apiBase}` },
  );
  const codexResource = useDataSurface(
    `integration-codex:${apiBase}`,
    [apiBase],
    fetchCodex,
    { isEmpty: value => value === null, enabled: active, sessionCacheKey: `nxc.integrations.codex.v1:${apiBase}` },
  );
  const keysResource = useDataSurface(
    ...
    { isEmpty: () => false, enabled: active, sessionCacheKey: `nxc.integrations.keys.v1:${apiBase}` },
  );
  const claudeResource = useDataSurface(
    `integration-claude:${apiBase}`,
    [apiBase],
    fetchClaude,
    { isEmpty: value => value === null, enabled: active, sessionCacheKey: `nxc.integrations.claude.v1:${apiBase}` },
  );
  const claudeDesktopResource = useDataSurface(
    `integration-claude-desktop:${apiBase}`,
    [apiBase],
    fetchClaudeDesktop,
    { isEmpty: value => value === null, enabled: active, sessionCacheKey: `nxc.integrations.claude-desktop.v1:${apiBase}` },
  );
  const grokResource = useDataSurface(
    `integration-grok:${apiBase}`,
    [apiBase],
    fetchGrok,
    { isEmpty: value => value === null, enabled: active, sessionCacheKey: `nxc.integrations.grok.v1:${apiBase}` },
  );
  const nativeResource = useDataSurface<NativeStatus[] | null>(
    `integration-native:${apiBase}`,
    [apiBase],
    fetchNative,
    { isEmpty: value => value === null, enabled: active, sessionCacheKey: `nxc.integrations.native.v1:${apiBase}` },
  );
