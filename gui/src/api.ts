 ...
 */
const SESSION_REBOOTSTRAP_PATH = "/nexcode-session";
...
function loadInjectedSession(): void {
  const token = takeMetaContent("nexcode-session-token");
  const csrfToken = takeMetaContent("nexcode-session-csrf");
  const origin = takeMetaContent("nexcode-session-origin");
...
function storeSession(token: string | null, csrfToken: string | null, origin: string | null): boolean {
  if (!token?.startsWith("nxc_session_") || !csrfToken || origin !== window.location.origin) return false;
  memoryToken = token;
...
async function reBootstrapSessionToken(): Promise<RebootstrapResult> {
  ...
  try {
    ...
    const stored = storeSession(
      metaContentFromHtml(html, "nexcode-session-token"),
      metaContentFromHtml(html, "nexcode-session-csrf"),
      metaContentFromHtml(html, "nexcode-session-origin"),
    );
...
function withToken(input: RequestInfo | URL, init: RequestInit | undefined, token: string): [RequestInfo | URL, RequestInit | undefined] {
  ...
  if (memorySessionOrigin && memoryCsrfToken && token.startsWith("nxc_session_")) {
    headers.set("X-NexCode-GUI-Origin", memorySessionOrigin);
    const method = (init?.method ?? (input instanceof Request ? input.method : "GET")).toUpperCase();
    if (method !== "GET" && method !== "HEAD") {
      headers.set("X-NexCode-CSRF-Token", memoryCsrfToken);
    }
  }
