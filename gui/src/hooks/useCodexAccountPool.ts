...
export function useCodexAccountPool(apiBase: string, enabled = true): CodexAccountPoolController {
  ...
  // One opportunistic quota refill per exact set of missing accounts. Three stacked
  // timers amplified an eight-second WHAM timeout into a permanently refreshing UI.
  const quotaFillAttemptedRef = useRef("");
  ...
  // Retry once for the exact set of healthy credentialed accounts missing quota.
  // Further refreshes are manual or the five-minute background poll; a failed WHAM
  // endpoint must not create a continuous request loop.
  const missingQuotaKey = JSON.stringify(accounts
  ...
  useEffect(() => {
    if (missingQuotaKey === "[]") {
      quotaFillAttemptedRef.current = "";
      return;
    }
    if (!enabled || pauseCount > 0 || quotaFillAttemptedRef.current === missingQuotaKey) return;
    const timer = window.setTimeout(() => {
      quotaFillAttemptedRef.current = missingQuotaKey;
      void load(false);
    }, 1_500);
    return () => window.clearTimeout(timer);
  }, [enabled, missingQuotaKey, pauseCount, load]);
