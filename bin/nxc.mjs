#!/usr/bin/env node
/**
 * nexcode npm bin launcher.
 *
 * The package source is TypeScript that runs on the Bun runtime. To let
 * `npm install -g @bitkyc08/nexcode` work without a separately-installed Bun,
...
const PKG = "@bitkyc08/nexcode";
const require = createRequire(import.meta.url);
const here = dirname(fileURLToPath(import.meta.url));
const cliPath = join(here, "..", "src", "cli", "index.ts");
const NODE_LAUNCH_CONTEXT_ENV = "NXC_NODE_LAUNCH_CONTEXT";
const NODE_LAUNCH_PROOF_PREFIX = "--nxc-internal-launch-proof=";
...
function configDir() {
  const raw = process.env.NEXCODE_HOME?.trim();
  return resolve(raw ? expandUserPath(raw) : join(homedir(), ".nexcode"));
}
...
function repairCodexShimIfNeeded() {
  ...
  if (res.status !== 0) {
    console.warn(`nexcode: Codex shim repair failed (${res.status ?? "unknown exit"}). Try: nxc codex-shim install`);
  }
}
...
function runNpmSelfUpdate() {
  ...
  if (!latestInvocation || !installInvocation) {
    console.error("nexcode: could not resolve npm from a trusted absolute PATH entry; aborting before stopping the proxy.");
    process.exit(1);
  }
  ...
  console.log(`nexcode v${current} (installed via npm, tag ${tag})`);
  ...
  if (!cachePreflight.ok) {
    console.error(`nexcode: ${npmCachePreflightFailureMessage(cachePreflight.reason)}. Aborting before stopping the proxy.`);
    process.exit(1);
  }

  // Remember whether a background service manages the proxy BEFORE stopping — `nxc stop`
   ...
   * non-admin `nxc update` does not have.
   */
  ...
  // Never replace package files under a live proxy — stop it first (full `nxc stop`
  ...
  // while nxc.pid is stale/missing.
  const launcher = fileURLToPath(import.meta.url);
  if (trayBeforeUpdate.stopBeforeReplacement) {
    ...
    try {
      ...
      console.error("nexcode: could not stop the Windows tray; aborting before package replacement.");
      process.exit(1);
    }
  }
  const hasRuntimeState =
    existsSync(join(configDir(), "nxc.pid")) || existsSync(join(configDir(), "runtime-port.json"));
  if (serviceWasInstalled || hasRuntimeState) {
    ...
    if (stopRes.status !== 0 || stillHasRuntimeState) {
      ...
      console.error("nexcode: could not stop the running proxy; aborting the update. Run 'nxc stop' and retry.");
      process.exit(1);
    }
    if (historyRestoreIncomplete()) {
      console.warn(
        "nexcode: WARNING — Codex resume history was NOT restored (history DB locked; Codex app/IDE open?).\n" +
        "  Routed threads stay hidden in the native Codex app until restored.\n" +
        "  After the update: close the Codex app, then run 'nxc stop' once to restore.",
      );
    }
  }
  ...
  try {
    ...
    if (tx.ok) {
      ...
      console.error(`nexcode: update aborted before touching the live install (${tx.phase}): ${tx.error}`);
      res = { status: 1 };
    } else {
      console.error(`nexcode: update failed (${tx.phase}): ${tx.error}${tx.rolledBack ? " — previous version restored." : ""}`);
      res = { status: 1 };
    }
  } catch (error) {
    ...
    console.error(`nexcode: transactional update failed unexpectedly (${error?.message ?? error}). ` +
      "The live install was not knowingly modified; run 'nxc update' again or reinstall with npm install -g.");
    res = { status: 1 };
  }
  if (res.status === 0) {
    ...
    if (trayBeforeUpdate.refreshAfterReplacement) {
      ...
      if (tray.status !== 0) {
        console.warn("nexcode: Windows tray refresh failed. Run: nxc tray install");
        if (trayBeforeUpdate.restoreOnFailure) runTrayLifecycle(launcher, "start");
      }
    }
    ...
    if (serviceWasInstalled) {
      ...
      process.env.NXC_BAKE_PORT = String(bakePort);
      try {
        ...
        if (needDirectStart) {
          ...
          console.warn(
            svc.status === 0
              ? "nexcode: service refresh left a non-viable manager — starting the proxy directly instead."
              : "nexcode: service refresh failed — starting the proxy directly instead.",
          );
          console.warn("  Run 'nxc service repair' to see why the background service could not restart.");
      ...
      console.log("Restart the proxy:  nxc start");
    }
    process.exit(0);
  }
  if (trayBeforeUpdate.restoreOnFailure) runTrayLifecycle(launcher, "start");
...
const BUN_OVERRIDE_ENV = "NEXCODE_BUN_PATH";
...
// imported; tests/nxc-launcher-source.test.ts pins the two together.
const BUN_RUNTIME_SOURCE_ENV = "NXC_BUN_RUNTIME_SOURCE";
const BUN_RUNTIME_PATH_ENV = "NXC_BUN_RUNTIME_PATH";
...
function resolveBun() {
  ...
  if (override) {
    ...
    console.error(
      `nexcode: ${BUN_OVERRIDE_ENV} is missing, unreadable, or not a complete Bun binary; falling back to the bundled runtime.`,
    );
  }
...
// `nxc update --help` prints usage and exits WITHOUT side effects. The npm launcher
...
if (updateHelpRequested) {
  console.log("Usage: nxc update [--tag latest|preview]\n\nUpdate nexcode. Preview installs stay on the preview tag unless overridden.");
  process.exit(0);
}
...
if (isNodeModulesInstall() && !isBunGlobalInstall()) {
  try {
    ...
    if (probe.action === "restored") {
      console.warn(`nexcode: previous update left a broken install — restored the backup from ${probe.from}.`);
    } else if (probe.action === "failed") {
      console.warn(`nexcode: a backup from a failed update exists but could not be restored automatically: ${probe.error}`);
    }
...
// auto-load a project `.env`/`.env.local`; the Bun child does, before any nexcode
...
// in argv, which project dotenv cannot modify during an ordinary `nxc` invocation.
...
child.on("error", err => {
  clearHandlers();
  console.error(`nexcode: failed to launch Bun runtime: ${err.message}`);
  process.exit(1);
});
