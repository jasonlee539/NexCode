...
// NOTE: there is deliberately no "nexcode anthropic OAuth" source. That is a
// PROVIDER credential in nexcode's own store which the Claude CLI never consumes,
// so it is not evidence the client can authenticate natively (audit 002 §5).

/**
 * The one nexcode-owned dummy token. Exported so the CLI, the system-env writer and
 * the tests all compare against the same literal — a second copy is how the marker
 * feedback loop (002 §1) sneaks back in.
 */
export const PROXY_MARKER = "nexcode-proxy";
...
export interface AuthDetectDeps {
  ...
  /**
   * Token values nexcode itself put into the environment. `system-env.ts` exports the
...
function detectExportedEnv(deps: AuthDetectDeps): AuthSourceResult {
  try {
    ...
    // Our own dummy is nexcode state, never user auth: counting it would make a
    // proxy-mode launch look authenticated on the NEXT launch (002 §1). The configured
    // admission key is nexcode state for exactly the same reason — the system-env
...
/**
 * The token values nexcode itself exports. Configured admission keys land in
