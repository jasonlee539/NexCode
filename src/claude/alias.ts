 ...
 * `claude-nxc-<provider>--<model>` with an honest display_name. Aliases must be
 * deterministic, reversible, and STABLE across releases (picker selections
 * persist to Claude Code's settings.json `model` field).
 *
 * Versioned prefixes:
 *  - `claude-nxc-` (v1) — legacy / plain model ids with no `/` or `~`. Decode
 ...
 *  - `claude-nxc2-` (v2) — used whenever the model id needs escape encoding
...
export const CLAUDE_ALIAS_PREFIX_V1 = "claude-nxc-";
/** Escape-encoded readable prefix (`~s`/`~t` expanded on decode). */
export const CLAUDE_ALIAS_PREFIX_V2 = "claude-nxc2-";
/**
...
export function resolveAlias(id: string): string | null {
  // Check v2 before v1 for clarity (prefixes are disjoint: nxc2 vs nxc-).
 ...
 *
 * The readable `claude-nxc*` form when representable; otherwise the desktop-3p
