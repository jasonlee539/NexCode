 ...
 *    providers carry reasoning in Responses items/nxcr1 envelopes instead.
 ...
 */
import type { NxcClaudeCodeConfig } from "../types";
 ...
 */
function configuredClassifierRoute(cc?: NxcClaudeCodeConfig): string | undefined {
...
export function resolveInboundModel(model: string, cc?: NxcClaudeCodeConfig): string {
...
export function effectiveBlockedSkillNames(cc?: Pick<NxcClaudeCodeConfig, "blockedSkills">): string[] {
...
/**
 * nxc-route directive (devlog 072): injected agent-definition bodies carry
 * `<!-- nxc-route: <model> -->` because Claude Code 2.1.207 ignores custom
 ...
 */
const NXC_ROUTE_RE = /<!--\s*nxc-route:\s*([^\s]+)\s*-->/;
const NXC_EFFORT_RE = /<!--\s*nxc-effort:\s*(low|medium|high|xhigh|max)\s*-->/;
...
export function extractNxcRouteDirective(body: unknown): string | null {
  const text = systemText(body);
  if (!text) return null;
  const match = NXC_ROUTE_RE.exec(text);
 ...
 * nxc-route so the inbound translator can restore `output_config.effort`.
 */
export function extractNxcEffortDirective(body: unknown): NonNullable<NxcClaudeCodeConfig["subagentEffort"]> | null {
  const text = systemText(body);
  if (!text) return null;
  const match = NXC_EFFORT_RE.exec(text);
  return match ? match[1] as NonNullable<NxcClaudeCodeConfig["subagentEffort"]> : null;
}
...
function maybeElideSkillText(text: string, names: readonly string[]): string {
  ...
  // basenaming (repo precedent: src/codex/inject.ts isNexcodeCatalogPath).
  const base = dir.replace(/\\/g, "/").split("/").filter(Boolean).pop()?.toLowerCase() ?? "";
  if (!names.includes(base)) return text;
  return `[nexcode] '${base}' skill document bundle (${text.length} chars) elided for routed models `
...
function skillElisionStub(callId: string): string {
  return "[nexcode] Skill document bundle elided for routed models (claudeCode.blockedSkills). "
 ...
 */
export function anthropicToResponsesBody(raw: unknown, cc?: NxcClaudeCodeConfig): Rec {
 ...
 */
export function anthropicToResponsesTranslation(raw: unknown, cc?: NxcClaudeCodeConfig): ClaudeInboundTranslation {
