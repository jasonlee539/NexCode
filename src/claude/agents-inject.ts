 ...
 * already covered) into ~/.claude/agents/nxc-*.md — one dispatchable
 ...
 * `nxc-*.md` inside the agents dir. User-authored agents are never touched.
 */
...
export interface ClaudeAgentDef {
  ...
  model: string;
  description: string;
  effort?: NonNullable<NxcConfig["claudeCode"]>["subagentEffort"];
  blockedSkills: readonly string[];
}

const OWNED_PREFIX = "nxc-";
/** Ownership proof (audit 071 #2): a file without this marker is NEVER touched. */
const GENERATED_MARKER = "generated-by: nexcode";
 ...
 * no-model nxc-self dispatch fell back to claude-fable-5 — devlog 072), so the
...
function entryParts(entry: string, config: NxcConfig): { alias: string; id: string; provider: string } {
...
export function buildClaudeAgentDefs(config: NxcConfig, windows: Record<string, number>, configDir = claudeConfigDir()): ClaudeAgentDef[] {
  ...
  for (const entry of roster.slice(0, 5)) {
    ...
    push(sanitizeName(id), alias, `Delegate work to ${id} (${provider}) via nexcode routing. General-purpose worker/explorer on that model. ${NO_MODEL_ARG}`);
  }
...
function renderAgentDef(def: ClaudeAgentDef): string {
  ...
  return [
    ...
    `<!-- nxc-route: ${def.model} -->`,
    ...(def.effort ? [`<!-- nxc-effort: ${def.effort} -->`] : []),
    "",
    `You are a delegated worker running on \`${def.model}\` through the local nexcode proxy.`,
    `IDENTITY: your ACTUAL underlying model is \`${def.model}\` — the nexcode proxy routes this`,
    ...
    "auto-generated by nexcode (`nxc claude`) from the featured subagent roster —",
...
/**
 * Sync owned agent files: write/overwrite current defs, prune stale nxc-*.md,
...
export function syncClaudeAgentDefs(defs: readonly ClaudeAgentDef[], configDir = claudeConfigDir()): string[] | null {
  try {
    ...
    for (const def of defs) {
      ...
      // A pre-existing nxc-* file WITHOUT our marker is user property: skip the def.
      try {
        lstatSync(target);
...
/** Launch-time hook: gate + build + sync in one call (used by nxc claude and systemEnv). */
export function injectClaudeAgentDefs(config: NxcConfig, windows: Record<string, number>, configDir?: string): string[] | null {
...
/**
 * Dispatcher directive appended to every nxc-* description. The nxc-route body
 ...
 */
const NO_MODEL_ARG = "NOTE: this agent's real model is pinned by the nexcode proxy — the `model` argument is ignored. Pass model: \"haiku\" as a placeholder (or omit it); routing is unaffected either way.";
