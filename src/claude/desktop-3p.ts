...
import type { NxcClaudeDesktopProfile } from "../types";
...
/** CLI arg parsing for `nxc claude desktop` mode flags (mutually exclusive). */
...
export interface Desktop3pLibraryInspection {
  ...
  /** Paths of nexcode-owned rows that are not selected by Desktop. */
...
function collectDesktop3pModels(
  ...
  profile?: NxcClaudeDesktopProfile,
  ...
  if (profile) {
    ...
    for (const model of [...rendered].sort((a, b) => a.route.localeCompare(b.route))) {
      ...
      if (existing && existing !== model.route) {
        console.warn(`[nexcode] Claude Desktop legacy alias collision: ${legacy} stays bound to ${existing}; ignoring ${model.route}`);
        continue;
      }
  ...
  for (const { provider, id, contextWindow } of candidates) {
    ...
    if (existingRoute !== undefined) {
      console.warn(`[nexcode] Claude Desktop 3P alias collision: ${alias} maps to both ${existingRoute} and ${route}; skipping ${route}`);
      continue;
    }
...
function isOwnedDesktopEntry(entry: Desktop3pMetadataEntry | undefined): boolean {
  return entry?.name === "nexcode" || entry?.name === "nexcode-standard";
}
 ...
 *
 * `gateway_drifted` is still an owned nexcode gateway (name + valid shape); the
...
export function removeDesktop3pStandardPivot(
  ...
  try {
    ...
    if (selectedOwnedGatewayActive) {
      ...
      const standardEntry: Desktop3pMetadataEntry = { id: standardId, name: "nexcode-standard" };
    ...
    // selection this only removes proven nexcode residues; appliedId is kept.
    atomicWriteFile(
      metadataPath,
...
/** Write and apply the nexcode config in Claude Desktop 3P's config library. */
export function writeDesktop3pConfig(
  ...
  try {
    ...
    const entry: Desktop3pMetadataEntry = existing ? { ...existing, id, name: "nexcode" } : { id, name: "nexcode" };
