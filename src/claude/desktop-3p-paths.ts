...
/**
 * The directory nexcode writes its 3P profile into.
 *
 * NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR wins over every other branch and is returned
...
export function resolveConfigLibraryDir(inputs: DesktopPathInputs): string {
  const override = inputs.env.NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR?.trim();
