...
export interface GatewayModelCacheRefreshOptions {
  timeoutMs?: number;
  configDir?: string;
  admissionConfig?: Pick<NxcConfig, "apiKeys">;
...
/**
 * Hardened service-token file, the same precedence `nxc opencode` uses. A service
 * install writes the admission token to disk rather than the interactive environment,
 * so an interactive `nxc claude` with neither env token nor configured key would
 * otherwise still get a 401 and keep a stale picker list.
 */
function serviceFileToken(env: NodeJS.ProcessEnv): string | null {
  const lookup = env.NXC_API_TOKEN_FILE?.trim()
    ? env
    : { ...env, NXC_API_TOKEN_FILE: serviceApiTokenFilePath() };
...
export async function refreshGatewayModelCacheFromProxy(
  ...
  try {
    ...
    const envToken = (options.env ?? process.env).NEXCODE_API_AUTH_TOKEN?.trim();
    ...
    if (admissionToken) headers.set("x-nexcode-api-key", admissionToken);

    // ?ids=cli pins the readable claude-nxc id family deterministically (audit 051
