...
/** Codex pool accounts are not a public `nxc login` provider; reauth is dashboard-driven. */
...
function actionFor(provider: string, health: OAuthAccountHealth): string | undefined {
  ...
  if (health.status === "warning" && health.reason === "refresh_conflict") {
    return "re-run `nxc doctor` after ensuring only one proxy process writes the credential store";
  }
  return undefined;
}
...
/** Shown by `nxc status` / `nxc doctor` when the proxy management API is unreachable. */
