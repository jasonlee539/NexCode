...
import type { NxcAccountPoolRotationStrategy, NxcConfig } from "../types";
...
export function anthropicAccountPoolConfig(config: NxcConfig): AnthropicAccountPoolConfig {
  const raw = config.anthropicAccountPool;
  if (!raw || typeof raw !== "object") return {};
  return raw;
}

export function isAnthropicAccountPoolEnabled(config: NxcConfig): boolean {
  return anthropicAccountPoolConfig(config).enabled === true;
}

export function anthropicAutoSwitchThreshold(config: NxcConfig): number {
...
function stickyLimitForPool(config: NxcConfig): number {
  return normalizeAccountPoolStickyLimit(anthropicAccountPoolConfig(config).stickyLimit);
}

function anthropicPoolStrategy(config: NxcConfig): NxcAccountPoolRotationStrategy {
  return normalizeAccountPoolStrategy(anthropicAccountPoolConfig(config).strategy);
}

function isActiveUnderFillFirstThreshold(config: NxcConfig, accountId: string): boolean {
 ...
 */
function pickFillFirstAnthropicAccount(config: NxcConfig, now: number): string | null {
