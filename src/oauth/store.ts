/**
 * OAuth token store at ~/.nexcode/auth.json, keyed by provider name.
 *
...
/**
 * Observe-only auth store read for diagnostics (`nxc doctor` / status).
 ...
 * still contains legacy single-credential entries, keep a pristine copy. An older nexcode
...
function normalizeCredential(cred: unknown): OAuthCredentials | null {
  ...
  if (typeof candidate.idToken === "string" && candidate.idToken.length > 0) normalized.idToken = candidate.idToken;
