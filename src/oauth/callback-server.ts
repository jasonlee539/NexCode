 ...
 *
 * Change vs source: the success/error page is an inline HTML constant. nexcode's GUI polls
...
export function oauthCallbackSuccessHtml(
  desktopApp = process.env.NEXCODE_DESKTOP_APP === "1",
): string {
  const desktopReturn = desktopApp
    ? "<p><a href='nexcode://oauth-complete' style='display:inline-block;margin-top:1rem;padding:.7rem 1.1rem;border-radius:.6rem;background:#111;color:#fff;text-decoration:none'>返回 NexCode</a></p>" +
      "<script>setTimeout(function(){window.location.href='nexcode://oauth-complete'},250)</script>"
    : "";
  return (
    "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width'><title>NexCode</title></head>" +
    "<body style='font-family:system-ui,sans-serif;text-align:center;padding:4rem 1.5rem;color:#111'>" +
    "<h2>&#9989; 授权已完成</h2><p>NexCode 正在完成账号验证，请返回软件稍候。</p>" +
    desktopReturn +
    "<p style='color:#666;font-size:.9rem'>此页面现在可以关闭。</p></body></html>"
  );
}
...
function errorHtml(message: string): string {
  return (
    "<!doctype html><html><head><meta charset='utf-8'><title>nexcode</title></head>" +
...
export abstract class OAuthCallbackFlow {
  ...
  #handleCallback(req: Request, expectedState: string): Response {
    ...
    return new Response(ok ? oauthCallbackSuccessHtml() : errorHtml(errMessage), {
