...
export function runningProxyUpdateHeaders(): Headers {
  ...
  if (adminToken) headers.set("X-NexCode-API-Key", adminToken);
  return headers;
}
...
async function handleOAuthLogin(name: string): Promise<void> {
  ...
  console.log(`\n✅ Logged in to ${name}. Try: nxc sync`);
  warnIfLiveReloadSkipped(reload);
}

export function providerConfigFromKeyLoginProvider(def: KeyLoginProvider, key: string, baseUrlOverride?: string): NxcProviderConfig {
  return {
    adapter: def.adapter,
...
export function mergeKeyLoginProviderRow(
  ...
  existing: NxcProviderConfig | undefined,
): NxcProviderConfig {
  return {
    ...provider,
...
async function handleKeyLogin(name: string): Promise<void> {
  ...
  console.log(`✅ ${def.label} added. Try: nxc sync`);
  warnIfLiveReloadSkipped(reload);
}
