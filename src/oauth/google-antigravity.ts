 ...
 *
 * OAuth client credentials are supplied through the environment and are never committed. User
 * tokens/refresh credentials are never logged.
 */
...
function oauthClientCredentials(): { clientId: string; clientSecret: string } {
...
class AntigravityOAuthFlow extends OAuthCallbackFlow {
  ...
  async generateAuthUrl(state: string, redirectUri: string): Promise<{ url: string; instructions?: string }> {
    const { clientId } = oauthClientCredentials();
