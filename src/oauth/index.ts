...
import type { NxcConfig, NxcProviderConfig, RefreshPolicy } from "../types";
...
function oauthConfig(id: string): NxcProviderConfig {
 ...
 */
export function resolveRefreshPolicy(provider: string, config: NxcConfig): RefreshPolicy {
...
export class OAuthLoginRequiredError extends Error {
  ...
  constructor(provider: string) {
    super(`Not logged in to ${provider}. Run: nxc login ${provider}`);
...
export function publicOAuthAuthenticationErrorMessage(error: unknown): string {
  ...
  return "OAuth authentication failed. Check the NexCode account status and retry.";
}
 ...
 */
export async function resolveModelsAuthToken(name: string, prov: NxcProviderConfig): Promise<string | undefined> {
...
function modelDiscoveryTransportSeed(providerName: string, prov: NxcProviderConfig): NxcProviderConfig {
 ...
 * configs on the next `nxc start`, instead of only fresh installs. The live `/models` fetch stays
...
const OAUTH_RECONCILE_FIELDS: (keyof NxcProviderConfig)[] = [
  "models",
  "contextWindow",
  "modelContextWindows",
...
function isLegacyCommandCodeStaticCatalog(provider: NxcProviderConfig): boolean {
...
function isLegacyAntigravityStaticCatalog(provider: NxcProviderConfig): boolean {
...
function migrateLegacyAntigravityStaticCatalog(config: NxcConfig): boolean {
...
export function reconcileOAuthProviders(config: NxcConfig): boolean {
...
function preservableApiKeyPool(value: unknown): NonNullable<NxcProviderConfig["apiKeyPool"]> | undefined {
  if (!Array.isArray(value)) return undefined;
  const pool: NonNullable<NxcProviderConfig["apiKeyPool"]> = [];
 ...
 */
export function upsertOAuthProvider(config: NxcConfig, provider: string): void {
  ...
  const next: NxcProviderConfig = { ...def.providerConfig };
...
/** Token-safe per-provider login state for the CLI `nxc status` logins section (no tokens, masked email). */
