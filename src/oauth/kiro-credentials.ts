...
const KIRO_CLI_RECOVERY_SUFFIX = ".nexcode-recovery";
const KIRO_CLI_RECOVERY_HEADER_V1 = Buffer.from("nexcode-kiro-session-v1\n", "utf8");
const KIRO_CLI_RECOVERY_HEADER = Buffer.from("nexcode-kiro-session-v2\n", "utf8");
...
function parseExpires(value: unknown, present: boolean, hasRefreshToken: boolean): number {
  ...
  if (present) {
    console.warn(
      `[nxc:kiro:credentials] credential expiry is present but unparseable; ${
...
export function restoreKiroCliSession(snapshot: KiroCliSessionSnapshot): void {
  const nonce = `${process.pid}.${Date.now()}.${Math.random().toString(16).slice(2)}`;
  const staged = `${snapshot.path}.nxc-restore.${nonce}.tmp`;
  const displacedBase = `${snapshot.path}.nxc-restore.${nonce}.new`;
