...
const TRAY_ICON_FILES = [
  "nexcode-tray-online.ico",
  "nexcode-tray-warning.ico",
  "nexcode-tray-offline.ico",
] as const;
...
function installedTrayScriptPath(): string {
  return join(getConfigDir(), "nexcode-tray.ps1");
}
...
export function windowsTrayStatePathsOwned(
  state: Pick<WindowsTrayEntry, "script" | "nexcodeHome"> & { launcherPath?: string },
  configDir = getConfigDir(),
): boolean {
  if (resolve(state.nexcodeHome) !== resolve(configDir)) return false;
  if (resolve(state.script) !== resolve(join(configDir, "nexcode-tray.ps1"))) return false;
  return state.launcherPath === undefined
    || resolve(state.launcherPath) === resolve(join(configDir, "nexcode-tray.vbs"));
}
...
function currentEntry(): WindowsTrayEntry {
  ...
  return {
    ...
    nexcodeHome: getConfigDir(),
  };
}

export function windowsTrayRunValue(nexcodeHome: string): string {
  const normalized = resolve(nexcodeHome).replace(/[\\/](?:\.)?[\\/]*$/, "").toLowerCase();
  return `NexCodeTray-${createHash("sha256").update(normalized).digest("hex").slice(0, 12)}`;
}
...
export function windowsTrayProcessArgs(entry: WindowsTrayEntry, mode: "Run" | "Stop" = "Run", hostPid?: number): string[] {
  const args = [
    ...
    "-NexCodeHome", safePath(entry.nexcodeHome),
    "-Mode", mode,
  ];
...
function installedTrayLauncherPath(): string {
  return join(getConfigDir(), "nexcode-tray.vbs");
}
...
export function buildWindowsTrayPowerShellCommand(entry: WindowsTrayEntry, powershell = windowsPowerShellPath()): string {
  return [
    ...
    "-NexCodeHome", quoteRunValue(entry.nexcodeHome),
    "-Mode", "Run",
  ].join(" ");
}
...
export function buildWindowsTrayLauncherScript(entry: WindowsTrayEntry, powershell = windowsPowerShellPath()): string {
  ...
  return [
    "' NexCode owned tray launcher — do not edit by hand.",
...
function readState(): WindowsTrayState | null {
  try {
    ...
    for (const key of ["bun", "cli", "script", "codexHome", "nexcodeHome", "runValue", "runCommand"] as const) {
    ...
    for (const value of [valid.bun, valid.cli, valid.script, valid.codexHome, valid.nexcodeHome]) safePath(value);
    ...
    if (valid.runValue !== windowsTrayRunValue(valid.nexcodeHome)) return null;
    return valid;
  } catch {
    return null;
  }
...
function assertWindows(): void {
  if (process.platform !== "win32") throw new Error(`The nexcode tray is Windows-only (current platform: ${process.platform}).`);
}

const DETACHED_TRAY_HOST_LAUNCHER = [
  "$startInfo = New-Object System.Diagnostics.ProcessStartInfo",
  "$startInfo.FileName = $env:NXC_TRAY_HOST_BUN",
  "$startInfo.Arguments = $env:NXC_TRAY_HOST_ARGS",
...
export function launchWindowsTrayHost(state: WindowsTrayEntry): void {
  ...
  execFileSync(windowsPowerShellPath(), [
    ...
    env: {
      ...process.env,
      NXC_TRAY_HOST_BUN: bun,
      NXC_TRAY_HOST_ARGS: `${quoteRunValue(cli)} __tray-host`,
      NXC_TRAY_ENTRY_B64: Buffer.from(JSON.stringify(state), "utf8").toString("base64"),
    },
  });
}
...
function parseTrayHostEntry(): WindowsTrayEntry {
  const encoded = process.env.NXC_TRAY_ENTRY_B64;
  ...
  for (const key of ["bun", "cli", "script", "codexHome", "nexcodeHome"] as const) {
...
export async function runWindowsTrayHost(): Promise<void> {
  ...
  delete process.env.NXC_TRAY_ENTRY_B64;
  delete process.env.NXC_TRAY_HOST_BUN;
  delete process.env.NXC_TRAY_HOST_ARGS;
...
export function installWindowsTray(startNow = true): WindowsTrayStatus {
  ...
  const runValue = windowsTrayRunValue(entry.nexcodeHome);
...
export async function windowsTrayCommand(args: string[]): Promise<void> {
  ...
  if (args.includes("--no-start") && sub !== "install" || values.length > 1 || !["install", "start", "stop", "status", "uninstall", "remove"].includes(sub)) {
    console.error("Usage: nxc tray <install|start|stop|status|uninstall|remove> [--json] [--no-start]");
    process.exitCode = 1;
    return;
  }
  try {
