param(
  ...
  [Parameter(Mandatory = $true)][string]$NexCodeHome,
...
$NexCodeHome = Normalize-HomePath $NexCodeHome
...
function Load-TrayIcon([string]$Name, [System.Drawing.Icon]$Fallback) {
  $path = Join-Path $NexCodeHome $Name
...
$onlineIcon = Load-TrayIcon "nexcode-tray-online.ico" ([System.Drawing.SystemIcons]::Information)
$warningIcon = Load-TrayIcon "nexcode-tray-warning.ico" ([System.Drawing.SystemIcons]::Warning)
$offlineIcon = Load-TrayIcon "nexcode-tray-offline.ico" ([System.Drawing.SystemIcons]::Error)
...
$stableHash = Get-StableHash $NexCodeHome
$stopEventCreated = $false
$stopEvent = New-Object System.Threading.EventWaitHandle($false, [System.Threading.EventResetMode]::AutoReset, "Local\NexCodeTrayStop-$stableHash", [ref]$stopEventCreated)
if ($Mode -eq "Stop") {
  [void]$stopEvent.Set()
  $stopEvent.Dispose()
  exit 0
...
$createdNew = $false
$mutex = New-Object System.Threading.Mutex($true, "Local\NexCodeTray-$stableHash", [ref]$createdNew)
if (-not $createdNew) {
  $stopEvent.Dispose()
  $mutex.Dispose()
  exit 0
...
$heartbeatPath = Join-Path $NexCodeHome "tray-heartbeat.json"
$actionLogPath = Join-Path $NexCodeHome "tray-actions.log"
...
function Start-NxcCommand([string[]]$CommandArgs, [switch]$TrackExit) {
  try {
    ...
    $psi.EnvironmentVariables["NEXCODE_HOME"] = $NexCodeHome
    if ($BunRuntimeSource) {
      $psi.EnvironmentVariables["NXC_BUN_RUNTIME_SOURCE"] = $BunRuntimeSource
      ...
      $psi.EnvironmentVariables["NXC_BUN_RUNTIME_PATH"] = $BunPath
    }
    ...
    $notify.ShowBalloonTip(5000, "nexcode action failed", "The action could not start. Open the logs folder or run nxc doctor.", [System.Windows.Forms.ToolTipIcon]::Error)
    return $false
  }
}

function Read-ListenTarget {
  foreach ($path in @((Join-Path $NexCodeHome "runtime-port.json"), (Join-Path $NexCodeHome "config.json"))) {
    try {
...
function Complete-PendingAction([bool]$Success) {
  ...
  if ($Success) {
    ...
    $notify.ShowBalloonTip(2500, "nexcode", "$action completed.", [System.Windows.Forms.ToolTipIcon]::Info)
  } else {
    Write-ActionLog "$action failed to reach the expected state"
    $notify.ShowBalloonTip(5000, "nexcode action failed", "$action did not reach the expected state. Open the logs folder or run nxc doctor.", [System.Windows.Forms.ToolTipIcon]::Error)
  }
}
...
$restartItem.add_Click({
  ...
  $restartProcess = Start-NxcCommand @("__tray-restart") -TrackExit
