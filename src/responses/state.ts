...
import type { NxcProviderContinuationState } from "../types";
import {
  deleteResponseSpill,
  noteStubSwapForTest,
  readResponseSpill,
...
const RESPONSE_STATE_TEMP_NAME = /^responses-state\.json\.nxc\.(\d+)\.(\d+)\.tmp$/;
...
interface ResidentResponseState {
  ...
  providers?: NxcProviderContinuationState;
  sizeBytes: number;
}
...
function schedulePersist(): void {
  // Resolve the target path NOW: tests (and anything else) may swap NEXCODE_HOME before the
...
/**
 * Report-only counterpart for `nxc doctor`: applies every selection gate and unlinks
...
export function previousResponseProviderState(responseId: string | undefined): NxcProviderContinuationState | undefined {
...
export function rememberResponseState(
  ...
  providerState?: NxcProviderContinuationState | string,
  ...
  ensureLoaded();
  const normalizedProviderState: NxcProviderContinuationState = typeof providerState === "string"
