 ...
 * signature (and any redacted blocks) inside a transparent `nxcr1:` + base64(JSON) envelope.
 *
 * Native OpenAI-encrypted blobs (no nxcr1 prefix) are left untouched by the decoder, and the
 * passthrough scrub strips nxcr1 envelopes before native forwarding.
 */

export const NXC_REASONING_PREFIX = "nxcr1:";
...
export function encodeReasoningEnvelope(envelope: ReasoningEnvelope): string {
  return NXC_REASONING_PREFIX + Buffer.from(JSON.stringify(envelope), "utf-8").toString("base64");
}

/** Decode an nxcr1 envelope; returns null for native (OpenAI-encrypted) blobs or garbage. */
export function decodeReasoningEnvelope(encryptedContent: string): ReasoningEnvelope | null {
  if (!encryptedContent.startsWith(NXC_REASONING_PREFIX)) return null;
  try {
    const parsed: unknown = JSON.parse(Buffer.from(encryptedContent.slice(NXC_REASONING_PREFIX.length), "base64").toString("utf-8"));
