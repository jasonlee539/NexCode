...
function replayThoughtSignatureMetadata(
  callId: string,
  scope: NxcReasoningReplayScopeRef | undefined,
...
function inputContentParts(blocks: unknown): string | NxcContentPart[] {
...
function outputTextOf(blocks: unknown): NxcTextContent[] {
  if (typeof blocks === "string") return blocks.length > 0 ? [{ type: "text", text: blocks }] : [];
  if (!Array.isArray(blocks)) return [];
  const out: NxcTextContent[] = [];
  for (const raw of blocks) {
...
function mapToolChoice(value: unknown): NxcRequestOptions["toolChoice"] {
...
function buildTools(tools: unknown[] | undefined): NxcTool[] | undefined {
  ...
  for (const t of tools) {
    ...
    if (t.type === "function" && typeof t.name === "string") {
      ...
      // Any OTHER named tool (e.g. a native/computer-use tool type nexcode doesn't explicitly
...
function ensureAssistantPlaceholder(messages: NxcMessage[], modelId: string, now: number): NxcAssistantMessage {
  const last = messages[messages.length - 1];
  if (last && last.role === "assistant") return last;
  const placeholder: NxcAssistantMessage = { role: "assistant", content: [], model: modelId, timestamp: now };
 ...
 */
function outputToToolResultContent(output: string | unknown[] | undefined): string | NxcContentPart[] {
...
function findToolById(messages: NxcMessage[], callId: string): { name: string; namespace?: string } {
...
function attachPendingReasoningToCallOwner(
  messages: NxcMessage[],
  callId: string,
  pendingReasoning: Array<{ part: NxcThinkingContent; envelopeSigned: boolean }>,
): void {
...
export function parseRequest(
  body: unknown,
  parseOptions?: { replayCacheScope?: NxcReasoningReplayScopeRef },
): NxcParsedRequest {
  ...
  const pendingReasoning: Array<{ part: NxcThinkingContent; envelopeSigned: boolean }> = [];
  ...
  const assistantHolderWithReasoning = (): NxcAssistantMessage => {
  ...
  if (typeof data.input === "string") {
    ...
    for (let inputIndex = 0; inputIndex < data.input.length; inputIndex++) {
      ...
      if (isCompactionItemType(effectiveType)) {
        // A stored summary from a previous compaction. Decode our nxc1 envelope into plain text so
      ...
      if (effectiveType === "reasoning") {
        ...
        // Native/non-nxcr1 encrypted-only reasoning is opaque here. Do not create a detached
        // assistant turn or invent replayable plaintext/signatures from the encrypted payload.
        if (thinkingText.length > 0) {
          const part: NxcThinkingContent = {
            type: "thinking",
  ...
  const wireOwners = new Map<string, NxcTool>();
  ...
  const mergedTools = [...declaredTools, ...loadedTools]
    ...
    .map(t => loadedToolNames.has(namespacedToolName(t.namespace, t.name))
      ...
      : t);
  const context: NxcContext = {
    ...(systemPrompt.length > 0 ? { systemPrompt } : {}),
    messages,
    ...(mergedTools.length > 0 ? { tools: mergedTools } : {}),
  };

  const options: NxcRequestOptions = {};
 ...
 */
function parseTextFormat(text: unknown): NxcRequestOptions["textFormat"] {
