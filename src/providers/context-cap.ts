...
export function providerContextCap(config: Pick<NxcConfig, "providerContextCaps">, provider: string): number | undefined {
  const value = config.providerContextCaps?.[provider];
  return isValidContextCap(value) ? value : undefined;
}

export function providerContextCaps(config: Pick<NxcConfig, "providerContextCaps">): Record<string, number> {
...
export function globalContextCapValue(config: Pick<NxcConfig, "contextCapValue">): number {
  const value = config.contextCapValue;
  return isValidContextCap(value) ? Math.floor(value) : DEFAULT_PROVIDER_CONTEXT_CAP;
}

export function setProviderContextCap(config: NxcConfig, provider: string, enabled: boolean, value?: number): void {
 ...
 */
export function setGlobalContextCapValue(config: NxcConfig, value: number, applyToAll: boolean): void {
...
export function setAllProviderContextCaps(config: NxcConfig, providerNames: string[], enabled: boolean): void {
  if (!enabled) {
