...
export function isKeyAuthProvider(provider: NxcProviderConfig): boolean {
...
function ensurePool(provider: NxcProviderConfig): NonNullable<NxcProviderConfig["apiKeyPool"]> {
...
function activeEntryId(provider: NxcProviderConfig): string | null {
  const pool = provider.apiKeyPool ?? [];
  if (pool.length === 0) return null;
  return (pool.find(e => e.key === provider.apiKey) ?? pool[0]!).id;
}

export function listProviderApiKeys(config: NxcConfig, name: string): { activeId: string | null; keys: ProviderApiKeyInfo[] } {
...
export function addProviderApiKey(config: NxcConfig, name: string, key: string, label?: string): { id: string } | { error: string } {
  const provider = config.providers[name];
...
export function setActiveProviderApiKey(config: NxcConfig, name: string, id: string): boolean {
  const provider = config.providers[name];
...
export function setProviderApiKeyLabel(config: NxcConfig, name: string, id: string, label: string | undefined): boolean {
  const provider = config.providers[name];
  if (!provider || !isKeyAuthProvider(provider)) return false;
  const entry = ensurePool(provider).find(e => e.id === id);
  if (!entry) return false;
...
export function removeProviderApiKey(config: NxcConfig, name: string, id: string): boolean {
  const provider = config.providers[name];
  if (!provider || !isKeyAuthProvider(provider)) return false;
  const pool = ensurePool(provider);
