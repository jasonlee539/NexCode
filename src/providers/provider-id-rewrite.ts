 ...
 */
export function rewriteProviderReferences(config: NxcConfig, from: string, to: string): ProviderRewriteResult {
  const prefix = `${from}/`;
   ...
   * than `keyof NxcConfig`: the latter also admits `customModels` and friends, so
 ...
 */
export function dropProviderCustomModels(config: NxcConfig, provider: string): number {
