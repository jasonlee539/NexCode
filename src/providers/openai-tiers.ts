import type { CodexAccountMode, NxcConfig, NxcProviderConfig, ProviderCostOverlay } from "../types";
...
function canonicalCodexForwardProvider(mode: CodexAccountMode): NxcProviderConfig {
  return {
...
export function isCanonicalOpenAiForwardProvider(provider: NxcProviderConfig): boolean {
 ...
 */
function resolvedResponsesEndpoint(provider: NxcProviderConfig): string | undefined {
  try {
...
function isOfficialOpenAiResponsesDestination(provider: NxcProviderConfig): boolean {
 ...
 */
export function isOpenAiOperatedResponsesDestination(provider: NxcProviderConfig): boolean {
  if (isCanonicalOpenAiForwardProvider(provider)) return true;
  return provider.adapter === "openai-responses"
...
/**
 * Whether this destination can decode a native (non-`nxc1:`) compaction blob.
 *
 ...
 */
export function destinationDecodesNativeCompactionBlob(provider: NxcProviderConfig): boolean {
...
function managedLegacyMultiOverlay(
  provider: NxcProviderConfig,
): Pick<NxcProviderConfig, "disabled" | "selectedModels" | "modelCosts"> | null {
...
function mergeLegacyOpenAiProviderRows(
  openai: NxcProviderConfig | undefined,
  legacyMulti: NxcProviderConfig | undefined,
  ...
  const formerRows = [openai, legacyMulti].filter((row): row is NxcProviderConfig => row !== undefined);
...
function hasKnownLegacyOpenAiReference(config: NxcConfig): boolean {
...
function rewriteLegacyOpenAiReferences(config: NxcConfig, warnings: string[]): void {
...
function unknownLegacyOpenAiWarnings(config: NxcConfig): string[] {
...
export function projectOpenAiTierMigration(config: NxcConfig): OpenAiTierMigrationProjection {
  ...
  const nextProviders: Array<[string, NxcProviderConfig]> = [];
  let inserted = false;
