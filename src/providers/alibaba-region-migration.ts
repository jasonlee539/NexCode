 ...
 * row is registry-derived Beijing metadata (`nxc provider add` and the GUI persist
 ...
 * what the user owns. `providerConfigSeed` is the same function `nxc provider add`
 * uses, so the migrated row is indistinguishable from one the user had created
 * against the international provider directly.
 */
function buildIntlRow(source: NxcProviderConfig): NxcProviderConfig {
 ...
 * migration's.
 */
export function projectAlibabaRegionMigration(config: NxcConfig): AlibabaRegionMigrationProjection {
