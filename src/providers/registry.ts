import type { CodexAccountMode, FastWire, NxcProviderConfig } from "../types";
...
export interface ProviderRegistryEntry {
  ...
  adapter: string;
  baseUrl: string;
  apiKeyTransport?: NxcProviderConfig["apiKeyTransport"];
   ...
   * `NxcProviderConfig.supportsServiceTier`. Registry-only: backfilled (never
  ...
  /** Registry default for plaintext reasoning replay; see `NxcProviderConfig.preserveResponsesReasoningContent`. Registry-only like `supportsServiceTier`. */
  ...
  reasoningWireFormat?: NxcProviderConfig["reasoningWireFormat"];
  noVisionModels?: string[];
  noReasoningModels?: string[];
  noTemperatureModels?: string[];
  noTopPModels?: string[];
  noPenaltyModels?: string[];
  /** Opt this provider into parallel tool calls (see NxcProviderConfig.parallelToolCalls). */
...
// nexcode's generic reasoning defaults. Audio is omitted because the current proxy
...
export const PROVIDER_REGISTRY: readonly ProviderRegistryEntry[] = [
  ...
  },
  {
    ...
    note: "Command Code Provider API (OpenAI-compatible); API access requires the Provider plan. Use `nxc login command-code` for OAuth account login (imports an existing local Command Code CLI credential when present). Docs: https://commandcode.ai/docs/provider.",
  },
  {
    id: "sambanova",
  ...
  },
  {
    ...
    note: "Keyed OpenCode Zen gateway. Free models on this tier are often short-window rate-limited at roughly 15-20 requests/minute (community-measured; OpenCode does not publish RPM). Zen may return generic 429s without Retry-After / X-RateLimit headers; when Retry-After is omitted, nexcode adds a synthetic backoff hint (upstream Retry-After still wins). Distinct from the keyless opencode-free desktop quota (~200 Big Pickle/free-model requests per 5 hours). Docs: https://opencode.ai/docs/zen/. Free-model prompts may be retained for training — do not send confidential material.",
    modelReasoningEfforts: Object.fromEntries(
...
export function registryModelServiceTierCapabilityApplies(
  ...
  provider: Pick<NxcProviderConfig, "baseUrl">,
): boolean {
...
export function providerMatchesRegistryTransport(
  id: string,
  provider: Pick<NxcProviderConfig, "baseUrl" | "adapter"> & Partial<Pick<NxcProviderConfig, "authMode">>,
): boolean {
 ...
 */
export function providerCodexAccountMode(id: string, provider?: NxcProviderConfig): CodexAccountMode | undefined {
