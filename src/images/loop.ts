...
import type { AdapterEvent, NxcMessage, NxcParsedRequest, NxcProviderContinuationState, NxcProviderOpaqueToolCallMetadata, NxcRequestOptions, NxcThinkingContent, NxcUsage, RateLimitRetryPolicy } from "../types";
...
function scanEventsForImageCall(events: AdapterEvent[], toolNames: Set<string>): {
  ...
  let pending: { name: string; id: string; argsBuf: string; events: AdapterEvent[]; providerMetadata?: NxcProviderOpaqueToolCallMetadata } | null = null;
...
export async function runWithImageBridge(deps: ImageBridgeDeps): Promise<Response> {
  ...
  let hiddenUsage: NxcUsage | undefined;

  const addUsage = (a: NxcUsage | undefined, b: NxcUsage | undefined): NxcUsage | undefined => {
    if (!a) return b;
    if (!b) return a;
    return {
  ...
  const sse = bridgeToResponsesSSE(
    ...
    }, 2_000,
    {
      ...
      ...(deps.onUsage ? {
        ...
        onUsage: (usage: NxcUsage | undefined) => deps.onUsage?.(usage),
      } : {}),
