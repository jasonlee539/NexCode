/**
 * `nxc service` — run the proxy as a background service that auto-starts on login and
 * auto-restarts on crash. macOS → launchd; Windows → Task Scheduler; Linux → systemd user unit.
 * The service sets NXC_SERVICE=1 so the proxy's shutdown handler does NOT restore native
...
const LABEL = "com.nexcode.proxy";
const TASK = "nexcode-proxy";

export type ServiceBackend = "scheduler" | "native";

function cliEntry(): { bun: string; bunRuntimeSource: BunRuntimeSource; cli: string } {
  // Bake the bundled Bun (npm global prefix, survives `nxc update`) rather than
...
function windowsServiceScriptPath(): string {
  return join(getConfigDir(), "nexcode-service.cmd");
}

function windowsLauncherVbsPath(): string {
  return join(getConfigDir(), "nexcode-service-launcher.vbs");
}

function windowsTaskXmlPath(): string {
  return join(getConfigDir(), "nexcode-service-task.xml");
}
...
function defaultNexCodeHome(): string {
  return resolve(join(homedir(), ".nexcode"));
}

function serviceStatePaths(): string[] {
  const paths = [serviceStatePath()];
  const defaultPath = join(defaultNexCodeHome(), "service-state.json");
...
function currentCodexSqliteHomeAbsolute(target: "native" | "windows" = "native"): string | undefined {
  ...
  //
  // CODEX_HOME and NEXCODE_HOME are carried through literally, so without this the same
...
function currentNexCodeHome(): string {
  // getConfigDir() already resolves NEXCODE_HOME with ~ expansion; keep the
...
export function parseServiceInstallState(value: unknown): ServiceInstallState | null {
  ...
  if (typeof state.nexcodeHome !== "string" || state.nexcodeHome.length === 0) return null;
...
function writeServiceInstallState(backend: ServiceBackend = "scheduler"): void {
  ...
  const state: ServiceInstallState = {
    ...
    nexcodeHome: currentNexCodeHome(),
    bunPath: bun,
    cliPath: cli,
    backend,
...
export function currentServiceHomes(deps: CodexHomeDeps = {}): { codexHome: string; nexcodeHome: string } {
  return { codexHome: currentCodexHome(deps), nexcodeHome: currentNexCodeHome() };
}
...
/**
 * The `nxc` argv that refreshes an already-installed service after an update.
 *
 ...
 * elevation — so an ordinary non-elevated `nxc update` used to stop a working proxy and
...
/** The `nxc` argv that registers a service from scratch, preserving the chosen backend. */
export function serviceInstallArgs(): string[] {
  return readServiceBackend() === "native" ? ["service", "install", "--native"] : ["service", "install"];
}

/**
 * The service was installed under a different CODEX_HOME/NEXCODE_HOME, so this process may not
 ...
 * CODEX_HOME/NEXCODE_HOME. Callers use it to decide whether they may tear down shared state
...
export function assertServiceEnvironmentMatchesInstall(): void {
  ...
  const expectedNexCodeHome = normalizePathForCompare(state.nexcodeHome);
  const actualNexCodeHome = normalizePathForCompare(currentNexCodeHome());
  if (expectedNexCodeHome !== actualNexCodeHome) {
    throw new ServiceOwnershipError(
      `Service was installed with NEXCODE_HOME=${state.nexcodeHome}, but current NEXCODE_HOME=${currentNexCodeHome()}. ` +
        "Run the service command from the same NexCode home so service state and secrets match.",
    );
  }
}
...
/**
 * The `nxc` command a user should rerun for the service state they actually have.
 *
...
export function serviceRetryCommand(
  ...
  return diag.installed && !diag.conflict ? "nxc service repair" : "nxc service install";
}

export function assertServiceAuthEnvironment(): void {
  const config = loadConfig();
  if (isLoopbackHostname(config.hostname)) return;
  if (process.env.NEXCODE_API_AUTH_TOKEN?.trim()) return;
  ...
  throw new Error(
    `NEXCODE_API_AUTH_TOKEN is required before ${diag.installed ? "refreshing" : "installing"} a service `
      + `for non-loopback hostname. Set it in the same shell, then rerun \`${retry}\`.`,
  );
}

function writeServiceApiTokenFile(): string | null {
  const token = process.env.NEXCODE_API_AUTH_TOKEN?.trim();
...
export function buildPlist(proxyEnv: { name: string; value: string }[] = resolvedProxyEnv()): string {
  ...
  const nexcodeHome = process.env.NEXCODE_HOME?.trim();
  const envLines = [
    `    <key>NXC_SERVICE</key><string>1</string>`,
    ...
    nexcodeHome ? `    <key>NEXCODE_HOME</key><string>${plistString(nexcodeHome)}</string>` : null,
 ...
 * Priority: explicit override → NXC_BAKE_PORT (update restart) → config.port → 10100.
...
export function resolveServiceListenPort(override?: number): number {
  ...
  const baked = process.env.NXC_BAKE_PORT?.trim();
...
function buildServiceShellCommand(bun: string, cli: string, port = resolveServiceListenPort()): string {
  ...
  return `if [ -f ${shellQuote(tokenFile)} ]; then NEXCODE_API_AUTH_TOKEN="$(cat ${shellQuote(tokenFile)})"; export NEXCODE_API_AUTH_TOKEN; fi; exec ${shellQuote(bun)} ${shellQuote(cli)} start --port ${port}`;
}
 ...
 * NXC_BAKE_PORT, or any later config.port edit, would otherwise leave launchd serving
 ...
 * the first two while `resolveServiceListenPort` (NXC_BAKE_PORT precedence, config.port
...
async function reportServiceServing(
  ...
  if (serving.ok) {
    console.log(`✅ nexcode service ${verb} and serving on port ${serving.port}.`);
    return;
  }
  console.error(
    ...
    + `   Log:       ${serviceLogPath()}\n`
    + `   Meanwhile: nxc start   (serves in the foreground)`,
  );
  process.exitCode = 1;
}
 ...
 *
 * `nxc service repair` reads the recorded backend itself, so it cannot silently switch a
 * WinSW install to Task Scheduler the way a plain `nxc service install` would, and on
 * Windows it needs no elevation because it never calls `schtasks /create`.
 */
function serviceRepairCommand(): string {
  return "nxc service repair";
}
 ...
 * same install driven through `nxc codex-shim` worked, because that path spawns with
 * `{ ...process.env }`.
 *
 ...
 * sat unused on disk. That is the 2026-08-02 report: `nxc service` prints a
...
export function formatWindowsSchedulerServiceStatus(
  ...
  if (task.status === "present") {
    if (proxy.status === "running") {
      return `✅ service installed (Task Scheduler); NexCode proxy running on port ${proxy.port}.`;
    }
    if (proxy.status === "not-running") {
      return "⚠️  service installed (Task Scheduler); NexCode proxy not running.";
    }
    return "⚠️  service installed (Task Scheduler); NexCode proxy status unknown.";
  }
  if (task.status === "absent") {
    if (proxy.status === "running") {
      return `❌ service not installed (Task Scheduler); NexCode proxy is running independently on port ${proxy.port}.`;
    }
    if (proxy.status === "unknown") {
      return "❌ service not installed (Task Scheduler); NexCode proxy status unknown.";
    }
    return "❌ service not installed (Task Scheduler).";
  }
  if (proxy.status === "running") {
    return `⚠️  Task Scheduler registration unknown; NexCode proxy running on port ${proxy.port}.`;
  }
  if (proxy.status === "not-running") {
    return "⚠️  service status unknown (Task Scheduler query failed); NexCode proxy not running.";
  }
...
/**
 * Probe whether the NexCode Task Scheduler task exists.
    ...
    : conflict
      ...
      : !assetsHealthy
        ...
        : !registrationHealthy
          ...
          : nativeStatusUnknown
            ? "The Task Scheduler task was created, but NexCode could not verify that the native WinSW service is absent."
            : "ok";
  return {
...
async function reconcileUnknownElevatedOutcome(exitCode: number): Promise<void> {
  ...
  const parts = [
    ...
    `Exit code: ${exitCode}.`,
    "NexCode could not prove whether task creation completed, so installation state was not written.",
  ];
  if (probe.status === "unknown") {
  ...
  if (probe.status === "absent") {
    parts.push("No NexCode Task Scheduler task was found after the elevated operation.");
    throwPartialInstall(parts);
  }
...
export async function finalizeWindowsSchedulerServiceRegistration(
  ...
  try {
    ...
    if (error instanceof WindowsElevationError && error.reason === "terminated") {
      try {
        await reconcileUnknownElevatedOutcome(NXC_ELEVATED_PROTOCOL_FAILED);
...
export function buildWindowsServiceScript(
  ...
  const lines = [
    ...
    windowsBatchSet("NEXCODE_HOME", process.env.NEXCODE_HOME?.trim(), "path"),
    ...proxyEnv.map(({ name, value }) => windowsBatchSet(name, value)),
    windowsBatchSet("NXC_API_TOKEN_FILE", serviceApiTokenFilePath(), "path"),
    windowsBatchSet("NXC_SERVICE_LOG", serviceLogPath(), "path"),
    windowsBatchSet("NXC_BUN", bun, "path"),
    windowsBatchSet("NXC_CLI", cli, "path"),
    ...
    'for %%I in ("%NXC_CLI%\\..\\..\\..") do set "NXC_PKG_DIR=%%~fI"',
    'if exist "%NXC_API_TOKEN_FILE%" (',
    '  set /p NEXCODE_API_AUTH_TOKEN=<"%NXC_API_TOKEN_FILE%"',
    ")",
    ":loop",
    '>>"%NXC_SERVICE_LOG%" echo [%DATE% %TIME%] nexcode service wrapper start',
    '>>"%NXC_SERVICE_LOG%" echo bun="%NXC_BUN%"',
    `>>"%NXC_SERVICE_LOG%" echo bun_source="${bunRuntimeSource}"`,
    '>>"%NXC_SERVICE_LOG%" echo cli="%NXC_CLI%"',
    '>>"%NXC_SERVICE_LOG%" echo nexcode_home="%NEXCODE_HOME%"',
    '>>"%NXC_SERVICE_LOG%" echo codex_home="%CODEX_HOME%"',
    '>>"%NXC_SERVICE_LOG%" echo token_file="%NXC_API_TOKEN_FILE%"',
    'if not exist "%NXC_BUN%" (',
    "  call :restore_backup",
    ")",
    'if not exist "%NXC_BUN%" (',
    '  >>"%NXC_SERVICE_LOG%" echo [%DATE% %TIME%] installation is incomplete: bundled Bun is missing; reinstall nexcode, then run nxc service repair',
    "  exit /b 3",
    ")",
    'if not exist "%NXC_CLI%" (',
    "  call :restore_backup",
    ")",
    'if not exist "%NXC_CLI%" (',
    '  >>"%NXC_SERVICE_LOG%" echo [%DATE% %TIME%] installation is incomplete: CLI entry is missing; reinstall nexcode, then run nxc service repair',
    "  exit /b 3",
    ")",
    `"%NXC_BUN%" "%NXC_CLI%" start --port ${port} >>"%NXC_SERVICE_LOG%" 2>&1`,
    "if %ERRORLEVEL% NEQ 0 (",
    '  >>"%NXC_SERVICE_LOG%" echo [%DATE% %TIME%] child exited with code %ERRORLEVEL%; restarting in 5s',
    ...
    // a sibling .nxc-backup-* holding the previous version. This wrapper lives OUTSIDE
    ...
    ":restore_backup",
    '>>"%NXC_SERVICE_LOG%" echo [%DATE% %TIME%] install incomplete - looking for a transactional-update backup to restore',
    'for /f "delims=" %%B in (\'dir /b /ad /o-n "%NXC_PKG_DIR%\\..\\.nxc-backup-*" 2^>nul\') do (',
    '  if exist "%NXC_PKG_DIR%\\..\\%%B\\nexcode\\package.json" (',
    '    if exist "%NXC_PKG_DIR%" rmdir /s /q "%NXC_PKG_DIR%" 2>nul',
    '    move "%NXC_PKG_DIR%\\..\\%%B\\nexcode" "%NXC_PKG_DIR%" >nul 2>&1',
    '    if exist "%NXC_PKG_DIR%\\package.json" (',
    '      >>"%NXC_SERVICE_LOG%" echo [%DATE% %TIME%] restored previous install from %%B',
    "      goto :eof",
    "    )",
    "  )",
    ")",
    '>>"%NXC_SERVICE_LOG%" echo [%DATE% %TIME%] no restorable backup found',
    "goto :eof",
...
export function buildWindowsLauncherVbs(script = windowsServiceScriptPath()): string {
  ...
  const lines = [
    "' NexCode service launcher — runs the batch wrapper with a hidden window.",
    "' Generated by `nxc service install`; do not edit.",
...
function windowsTaskDescription(attemptNonce?: string): string {
  return attemptNonce
    ? `NexCode proxy service wrapper; install-attempt=${attemptNonce}`
    : "NexCode proxy service wrapper";
}
...
function installLaunchd(): void {
  ...
  if (!loaded.ok || launchctlLoadFailed(loaded.stderr)) {
    ...
    throw new Error(
      ...
      + `then re-run '${wasInstalled ? "nxc service repair" : "nxc service install"}'.`,
    );
  }
...
export function startLaunchd(deps: {
  ...
  // throwing here would break `nxc service start` on every healthy service.
  ...
  throw new Error(
    ...
    + (live.loaded
      ...
      : "The job is not loaded. Run 'nxc service repair' to reload it."),
  );
}
...
const WINDOWS_SCHEDULER_STAGE_PREFIX = "nexcode-service-stage-";
...
export async function registerFreshWindowsSchedulerTask(
  ...
  if (
    ...
    throw new Error("The staged Task Scheduler registration failed NexCode ownership or shape validation.");
  }
  try {
  ...
  if (
    ...
    const rollback = await rollbackTask();
    throw new Error(
      "Task Scheduler registration was created but failed the NexCode action/trigger or attempt-ownership verification."
  ...
  if (diag.conflict) {
    throw new Error(
      "Cannot repair while Task Scheduler and native WinSW are both present. "
        + "Run 'nxc service uninstall' then reinstall one backend with 'nxc service install'.",
    );
  }
  if (!diag.installed) {
    throw new Error("Background service is not installed. Run 'nxc service install' first.");
  }
...
/**
 * Opt-in native backend (`nxc service install --native`). Transactional: removes the
  ...
  if (source?.toLowerCase() === "microsoftaccount") {
    throw new Error(
      "The native (WinSW) service backend cannot run under a Microsoft-account Windows login. "
        + "Keep the Task Scheduler backend (`nxc service install`) or sign in with a local/domain account before `nxc service install --native`.",
    );
  }
}
...
async function installWindowsNative(): Promise<void> {
  ...
  try {
    ...
    if (hadScheduler) console.error("⚠️  Native install failed AFTER removing the Task Scheduler backend — no service is installed now. Run `nxc service install` to restore the scheduler backend, or retry `--native`.");
    throw err;
  }
...
export function bakedServicePathsDiagnostic(): string | null {
  ...
  return `STALE baked paths (missing: ${missing.join(", ")}) — run 'nxc service repair' to re-bake`;
}
...
export function buildUnit(proxyEnv: { name: string; value: string }[] = resolvedProxyEnv()): string {
  ...
  const nexcodeHome = systemdEnvironmentAssignment("NEXCODE_HOME", process.env.NEXCODE_HOME?.trim());
  const envLines = [
    systemdEnvironmentAssignment("NXC_SERVICE", "1"),
  ...
  return `[Unit]
Description=NexCode Proxy Server
...
function isSystemd(): boolean {
  ...
  // systemd instance is available — so a first-time `nxc service install` isn't wrongly refused.
...
function startSystemd(): void {
  ...
  if (!existsSync(unitPath())) {
    console.error(`nexcode service is not installed: ${unitPath()}`);
    console.error("Run `nxc service install` first to create and enable the systemd user unit.");
    process.exit(1);
  }
...
function platformOps(backend: ServiceBackend = "scheduler"): ServiceOps | null {
  ...
  if (process.platform === "linux") {
    if (existsSync("/.dockerenv")) {
      console.error("Docker detected. Run 'nxc start' directly instead of using the service manager.");
      process.exit(1);
    }
    if (!isSystemd() && !existsSync(unitPath())) {
      console.error("systemd not found. Run 'nxc start' under your process supervisor.");
      if (isWslRuntime()) {
 ...
 * that returned success can still leave a live proxy — and `nxc service stop` then restored
...
async function stopTrackedProxyIfRunning(): Promise<TrackedProxyCleanupResult> {
  ...
  // a live proxy running — mirror `nxc stop`'s identity-checked findLiveProxy fallback.
...
export async function installFreshWindowsSchedulerSafely(
  ...
  try {
    ...
    if (!ownershipRecorded && configRootWasAbsent) {
      throw new Error(
        "The fresh NexCode config root could not be claimed for safe uninstall; "
    ...
    if (started) {
      throw new Error(
        ...
        + "The task was left in place; inspect `nxc service status` before retrying.",
      );
    }
    throw error;
  } finally {
...
/**
 * If a service is installed, stop it so the process manager doesn't respawn after `nxc stop`.
...
export function stopServiceIfInstalled(): boolean {
  ...
  if (process.platform === "darwin") {
    ...
    // two managers installed, and either one would respawn the proxy after `nxc stop`.
    let stopped = false;
    try {
...
/** Delete install-state files; stale state would make `nxc update` "reinstall" a service that no longer exists. */
...
/**
 * Best-effort service removal for full uninstall. Unlike `nxc service uninstall`, this is quiet
...
export function deriveWindowsServiceDiagnostic(inputs: WindowsServiceDiagnosticInputs): ServiceDiagnostic {
  ...
  const detail = conflict
    ? "CONFLICT: Task Scheduler and native WinSW are both present — run 'nxc service uninstall' then reinstall one"
    : stale
      ? "stale or missing service assets — run 'nxc service repair'"
      : schedulerInstalled
...
export async function serviceStatusReport(
  ...
  const stalePlist = deps.matchesPlist?.() ?? (process.platform === "darwin"
    ? (() => {
        ...
        // resolveServiceListenPort(), which reads NXC_BAKE_PORT/config.port, so after
  ...
  const staleLine = stalePlist && stalePlist.loaded && !stalePlist.matchesPlist
    ? "   launchd is running an OLDER plist than the one on disk.\n"
      + `   Fix:    launchctl bootout gui/$(id -u)/${LABEL} && nxc service repair\n`
    : "";
...
export function planServiceCommand(
  ...
  if (parsed.backend && parsed.sub !== "install") {
    return { ok: false, message: "--native/--scheduler apply to `nxc service install` only; other subcommands use the installed backend." };
  }
  ...
  if (!hasExplicitSubcommand && parsed.backend === null) {
    ...
    if (probe.state === "unknown") {
      ...
      return {
        ok: false,
        message: `Could not safely determine whether the service is installed${suffix}. Run 'nxc service status' and retry; use explicit 'nxc service install' only after confirming it is absent.`,
      };
    }
...
/**
 * `nxc service [sub] [--native|--scheduler]`. The first non-flag token is the
...
export async function serviceCommand(...args: (string | undefined)[]): Promise<void> {
  ...
  if (!ops) {
    console.error("nxc service supports macOS (launchd), Windows (Task Scheduler), and Linux (systemd).");
    process.exit(1);
  }
  switch (command) {
    case "install":
      ...
      // Service users never reach the `nxc start` prompt: the proxy they run is the
      // supervised child, which always carries NXC_SERVICE=1. This command, though, is
      ...
      await stopTrackedProxyForServiceCommand();
      {
        ...
        if (survivor) {
          console.error(
            ...
            + " both pointing at each other. Check for a second service backend (`nxc service status`)"
            + " or a manually started proxy, then re-run `nxc service stop`.",
          );
          process.exitCode = 1;
          break;
        }
    ...
    case "remove":
      ...
      try {
        ...
        console.error("The service may still be installed. Check with 'nxc service status' or remove manually.");
        process.exit(1);
      }
      {
        const restore = await restoreNativeCodexAsync();
        if (!restore.success) {
          console.error(`⚠️ native Codex restore FAILED: ${restore.message}\nRun \`nxc restore\` before using native Codex.`);
        }
        const grok = stripGrokConfig();
        if (grok.changed) console.log(`↩️  ${grok.message}`);
        else if (!grok.ok) console.error(`⚠️  ${grok.message}`);
      ...
      break;
    default:
      console.error("Usage: nxc service [install|repair|restart|start|stop|status|uninstall|remove] [--native|--scheduler]");
