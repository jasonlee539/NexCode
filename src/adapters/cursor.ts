...
import type { AdapterEvent, NxcProviderConfig } from "../types";
...
function safeCursorTransportError(err: unknown, sizeContext?: CursorSizeContext): string {
  ...
  if (err instanceof CursorMissingCredentialError) {
    return "Cursor live transport is enabled, but no Cursor access token is configured. Set provider.apiKey or NEXCODE_CURSOR_TEST_TOKEN.";
  }
...
export function createCursorAdapter(provider: NxcProviderConfig, deps: CursorAdapterDeps = {}): ProviderAdapter {
  return {
    name: "cursor",
    ...
    async runTurn(_parsed, incoming, emit) {
      ...
      try {
        ...
        const partialUsage = (err as { partialUsage?: import("../types").NxcUsage }).partialUsage;
        emit({
          type: "error",
