...
function toAnthropicContentPart(p: NxcContentPart): unknown {
  if (p.type === "image") {
...
function usesNativeAnthropicEndpoint(provider: NxcProviderConfig): boolean {
  try {
...
function anthropicKeyUsesBearer(provider: NxcProviderConfig): boolean {
...
function usageFromAnthropic(usage: Record<string, number> | undefined): NxcUsage | undefined {
  ...
  // canonical inclusive convention (types.ts NxcUsage / devlog 070).
  return {
...
function buildToolNameTransforms(provider: NxcProviderConfig): { toWire: (name: string) => string; fromWire: (name: string) => string } {
...
function toAnthropicToolResult(msg: NxcToolResultMessage, wireCallId: string): Record<string, unknown> {
  ...
  if (typeof msg.content === "string") {
    ...
    const parts = (msg.content as NxcContentPart[])
...
function unrepresentableToolCallText(tc: NxcToolCall, wireName: string): string {
  const args = typeof tc.arguments === "string" ? tc.arguments : JSON.stringify(tc.arguments);
  return `[tool_use without a usable id: ${wireName}]\n${args}`;
}

function orphanToolResultText(msg: NxcToolResultMessage): string {
...
function messagesToAnthropicFormat(
  ...
  for (const message of parsed.context.messages) {
    if (message.role === "assistant") {
      for (const part of (message as NxcAssistantMessage).content) {
        if (part.type === "toolCall") callIds.reserve((part as NxcToolCall).id);
      }
    } else if (message.role === "toolResult") {
      callIds.reserve((message as NxcToolResultMessage).toolCallId);
    }
  }
    ...
    switch (msg.role) {
      ...
      case "assistant": {
        ...
        for (const part of aMsg.content) {
          if (part.type === "text") {
            const text = (part as NxcTextContent).text;
            if (text) preface.push({ type: "text", text });
          } else if (part.type === "thinking") {
            const t = part as NxcThinkingContent;
        ...
        if (toolUseIds.length > 0) {
          ...
          while (j < parsed.context.messages.length && parsed.context.messages[j].role === "toolResult") {
            const tr = parsed.context.messages[j] as NxcToolResultMessage;
      ...
      case "toolResult": {
        ...
        messages.push({ role: "user", content: orphanToolResultText(msg as NxcToolResultMessage) });
        break;
      }
    }
  }
...
function toolsToAnthropicFormat(parsed: NxcParsedRequest, toolNames: { toWire: (name: string) => string }): unknown[] | undefined {
...
export function createAnthropicAdapter(provider: NxcProviderConfig, cacheRetention?: "none" | "short" | "long"): ProviderAdapter {
  ...
  return {
    ...
    async buildRequest(parsed: NxcParsedRequest, incoming?: IncomingMeta) {
      if (typeof provider.apiKey !== "string" || provider.apiKey.trim() === "") {
        if (isOAuth) {
          throw new Error("anthropic oauth token missing — run nxc login anthropic");
        }
