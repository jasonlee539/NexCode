...
function vertexReplaySessionId(parsed: NxcParsedRequest): string {
 ...
 */
function toolResultImageParts(content: string | NxcContentPart[]): unknown[] {
 ...
 */
function geminiToolResultText(content: string | NxcContentPart[]): string {
...
function geminiUnrepresentableToolCallPart(tc: NxcToolCall, wireName: string): unknown {
  const args = typeof tc.arguments === "string" ? tc.arguments : JSON.stringify(tc.arguments);
  return { text: `[tool_use without a usable id: ${wireName}]\n${args}` };
}

function geminiOrphanToolResultParts(msg: NxcToolResultMessage): unknown[] {
...
function messagesToGeminiFormat(
  ...
  for (const msg of parsed.context.messages) {
    if (msg.role === "assistant") {
      for (const part of (msg as NxcAssistantMessage).content) {
        if (part.type === "toolCall") callIds.reserve((part as NxcToolCall).id);
      }
    } else if (msg.role === "toolResult") {
      callIds.reserve((msg as NxcToolResultMessage).toolCallId);
    }
  }
    ...
    switch (msg.role) {
      ...
      case "developer": {
        if (typeof msg.content === "string") {
          ...
          const parts: unknown[] = [];
          for (const p of msg.content as NxcContentPart[]) {
            if (p.type === "image") {
              const data = parseDataUrl(p.imageUrl);
      ...
      case "assistant": {
        ...
        for (const p of aMsg.content) {
          if (p.type === "text") {
            const textPart = geminiTextPart((p as NxcTextContent).text);
            if (textPart) parts.push(textPart);
          } else if (p.type === "toolCall") {
            const tc = p as NxcToolCall;
        ...
        if (toolCalls.length > 0) {
          ...
          const resultsById = new Map<string, NxcToolResultMessage>();
          const orphanResults: NxcToolResultMessage[] = [];
          let j = i + 1;
          while (j < parsed.context.messages.length && parsed.context.messages[j].role === "toolResult") {
            const result = parsed.context.messages[j] as NxcToolResultMessage;
      ...
      case "toolResult": {
        ...
        contents.push({ role: "user", parts: geminiOrphanToolResultParts(msg as NxcToolResultMessage) });
        break;
      }
    }
  }
...
function toolsToGeminiFormat(parsed: NxcParsedRequest): unknown[] | undefined {
 ...
 */
function toolChoiceToGeminiToolConfig(parsed: NxcParsedRequest): Record<string, unknown> | undefined {
...
function usageFromGemini(usage: Record<string, number> | undefined): NxcUsage | undefined {
...
): { providerMetadata: NxcProviderOpaqueToolCallMetadata } | undefined {
...
export function createGoogleAdapter(provider: NxcProviderConfig): ProviderAdapter {
  ...
  return {
    ...
    async buildRequest(parsed: NxcParsedRequest) {
      ...
      if (provider.googleMode === "cloud-code-assist") {
        ...
        if (!token) throw new Error("google-antigravity oauth token missing — run nxc login google-antigravity");
        ...
        if (!project) throw new Error("Antigravity requires a discovered Cloud Code Assist project id (re-run `nxc login google-antigravity`).");
