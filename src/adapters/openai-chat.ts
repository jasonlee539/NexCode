...
import type { AdapterEvent, NxcAssistantMessage, NxcContentPart, NxcMessage, NxcParsedRequest, NxcProviderConfig, NxcTextContent, NxcThinkingContent, NxcToolCall, NxcUsage } from "../types";
...
function openAIChatTransport(provider: NxcProviderConfig): {
  url: string;
...
function invalidChoicesEvent(usage?: NxcUsage): Extract<AdapterEvent, { type: "error" }> {
  return {
    type: "error",
 ...
 */
function unnamedToolCallEvent(usage?: NxcUsage): Extract<AdapterEvent, { type: "error" }> {
  return {
    type: "error",
...
function developerSystemText(message: NxcMessage): string | undefined {
  if (message.role !== "developer") return undefined;
  if (typeof message.content === "string") return message.content;
  if (message.content.some(part => part.type === "image")) return undefined;
  return message.content.map(part => (part as NxcTextContent).text).join("");
}

function isNativeOpenAIChatTarget(provider: NxcProviderConfig): boolean {
  try {
 ...
 */
function toolResultTextForWire(content: string | NxcContentPart[]): string {
  if (typeof content === "string") return content;
  const text = content.filter((p) => p.type === "text").map((p) => (p as NxcTextContent).text).join("");
  if (text) {
...
function toolResultImageChatParts(content: string | NxcContentPart[]): unknown[] {
...
function messagesToChatFormat(parsed: NxcParsedRequest, provider: NxcProviderConfig): unknown[] {
  const out: unknown[] = [];
  ...
  const mintCallId = (): string => {
    let id = "";
    do {
      id = `call_nxc_minted_${++mintedIdSeq}`;
  ...
  const flushToolResultImages = (): void => {
    ...
    out.push({
      role: "user",
      content: [
        { type: "text", text: "[nxc] image output from the preceding tool result(s):" },
  ...
  const flushPendingToolCalls = (): void => {
    ...
    for (const call of pendingToolCalls) {
      out.push({
        ...
        content: `[nxc] no tool result was recorded for "${call.name}"; execution status unknown — do not treat this as success, failure, or user-provided input.`,
      });
    }
    pendingToolCalls = [];
  ...
  for (const msg of context.messages) {
    switch (msg.role) {
      case "user":
      case "developer": {
        const parts = typeof msg.content === "string" ? undefined : msg.content as NxcContentPart[];
      ...
      case "assistant": {
        const aMsg = msg as NxcAssistantMessage;
        const textParts = aMsg.content.filter(p => p.type === "text") as NxcTextContent[];
        const thinkingParts = aMsg.content.filter(p => p.type === "thinking") as NxcThinkingContent[];
        const toolCalls = aMsg.content.filter(p => p.type === "toolCall") as NxcToolCall[];
...
function shouldSanitizeZenToolParameters(provider: NxcProviderConfig): boolean {
  const baseUrl = provider.baseUrl.replace(/\/+$/, "");
  return baseUrl === "https://opencode.ai/zen/v1"
    || baseUrl === "https://opencode.ai/zen/go/v1";
}

function isXaiSchemaTarget(provider: NxcProviderConfig): boolean {
  try {
...
function isVolcengineArkPaygChatTarget(provider: NxcProviderConfig): boolean {
  try {
...
function emptyAssistantContent(provider: NxcProviderConfig): string | { type: "text"; text: string }[] {
...
function toolsToChatFormat(parsed: NxcParsedRequest, provider: NxcProviderConfig): unknown[] | undefined {
...
function toolsToChatFormatForProvider(parsed: NxcParsedRequest, provider: NxcProviderConfig): unknown[] | undefined {
...
function toolChoiceToChatFormat(
  tc: NxcParsedRequest["options"]["toolChoice"],
  tools: NxcParsedRequest["context"]["tools"],
  provider: NxcProviderConfig,
): unknown {
...
function usageFromOpenAIChat(usage: Record<string, unknown> | undefined): NxcUsage | undefined {
...
function resolveMaxTokens(provider: NxcProviderConfig, parsed: NxcParsedRequest): number | undefined {
...
function thinkingBudgetForEffort(parsed: NxcParsedRequest, reasoningEffort: string, maxOutputTokens?: number): number | undefined {
...
function canSerializeOpenAIChatServiceTier(
  ...
  modelId: string,
  serviceTier: unknown,
  tierDecision?: NxcParsedRequest["options"]["tierDecision"],
): boolean {
...
export function createOpenAIChatAdapter(provider: NxcProviderConfig): ProviderAdapter {
  return {
    name: "openai-chat",

    formatErrorBody: formatOpenAIChatErrorBody,

    buildRequest(parsed: NxcParsedRequest) {
      const { url, headers, hasCredential } = openAIChatTransport(provider);
    ...
    async *parseStream(
      ...
      let pendingUsage: NxcUsage | undefined;
