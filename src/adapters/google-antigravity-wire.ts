...
import type { NxcContentPart, NxcParsedRequest } from "../types";
...
/**
 * Whether a stored `NxcToolCall.thoughtSignature` is a REAL upstream Gemini signature versus a
...
function firstUserText(parsed: NxcParsedRequest): string | undefined {
  for (const msg of parsed.context.messages) {
    if (msg.role !== "user") continue;
    if (typeof msg.content === "string") return msg.content;
    const first = (msg.content as NxcContentPart[]).find(p => p.type === "text" && typeof p.text === "string");
 ...
 */
export function antigravitySessionId(parsed: NxcParsedRequest): string {
 ...
 */
function clientThreadAnchor(parsed: NxcParsedRequest): string | undefined {
