...
import { namespacedToolName, type AdapterEvent, type NxcParsedRequest, type NxcProviderConfig, type NxcUsage, type TierDecision } from "../types";
...
import { NXC_REASONING_PREFIX } from "../responses/reasoning-envelope";
...
export function sanitizeReasoningInputContent(
  ...
  const input = raw.input.map(item => {
    ...
    // nxcr1 envelopes are proxy-minted (Anthropic signatures), not OpenAI encryption — the native
    // backend cannot decrypt them and would reject the request. Strip regardless of content shape.
    const hasNxcEnvelope = typeof rec.encrypted_content === "string" && rec.encrypted_content.startsWith(NXC_REASONING_PREFIX);
    ...
    const stripEncryptedContent = hasNxcEnvelope
 ...
 * turn, and only the backend that minted it can decode it. Proxy-minted `nxc1:` envelopes are
 ...
 */
function scrubNxcCompactionItems(
  body: unknown,
 ...
 * is a defense-in-depth guard so stale on-disk catalogs don't break until the user runs `nxc sync`.
 */
...
const REPAIRED_CALL_ID_PREFIX = "call_nxc_";
    ...
    if (isFnCall || isCustomCall) {
      ...
      if (synthesizeMissingCallOutputs) {
        ...
        if (!hasOutput && callId) {
          ...
          const text = `[nxc] no tool result was recorded for "${name}"; execution status unknown — do not treat this as success, failure, or user-provided input.`;
 ...
 *   `max_output_tokens`). Codex only sends the id on WS turns, and nxc converts those to
...
function usageFromResponsesPayload(payload: unknown): NxcUsage | undefined {
...
export function createResponsesPassthroughAdapter(provider: NxcProviderConfig): ProviderAdapter & { passthrough: true } {
  return {
    name: "openai-responses",
    passthrough: true as const,

    buildRequest(parsed: NxcParsedRequest, incoming: IncomingMeta) {
      ...
      const sanitizedBody = normalizeToolSchemas(stripSparkCompatibility(stripUnsupportedReasoningParams(stripItemIdsWhenUnstored(stripInvalidItemIds(stripUnsupportedHostedTools(sanitizeReasoningInputContent(scrubNxcCompactionItems(
        outBody,
