...
function classifyKiroFailure(
  ...
  if (evidence.includes("profilearn") && evidence.includes("required")) {
    return {
      message: "kiro_profile_required: Kiro requires a CodeWhisperer profileArn for this account and model. Re-login or re-import the matching Kiro account (nxc account login kiro --reauth) so the profile is captured, then retry.",
      status: 400,
      errorType: "invalid_request_error",
