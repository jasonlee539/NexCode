...
function isCodexCodeModeExecTool(tool: Pick<NxcTool, "namespace" | "name" | "freeform">): boolean {
 ...
 * perform.
 */
function isBareShellBridgeTool(tool: Pick<NxcTool, "namespace" | "name">): boolean {
...
export function shouldInjectNonOpenAIToolCatalogNudge(provider: Pick<NxcProviderConfig, "baseUrl">): boolean {
  try {
...
export function buildNonOpenAIToolCatalogNudgeForTools(
  ...
  toWireName: (tool: Pick<NxcTool, "namespace" | "name">) => string = tool => namespacedToolName(tool.namespace, tool.name),
): string | undefined {
