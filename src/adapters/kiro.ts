...
function userContentText(content: string | NxcContentPart[]): string {
  if (typeof content === "string") return content;
  return content.map(p => (p.type === "text" ? p.text : "")).filter(Boolean).join("\n");
}

function usageContentText(content: string | NxcContentPart[]): string {
  if (typeof content === "string") return content;
  return content
    .map(p => {
...
function currentTurnUsageMessages(messages: NxcMessage[]): NxcMessage[] {
  return messages.slice(messages.map(m => m.role).lastIndexOf("assistant") + 1).filter(m => m.role !== "assistant");
}
function kiroPayloadMessages(parsed: NxcParsedRequest): NxcMessage[] {
  return parsed.context.messages;
}

function messageUsageText(msg: NxcMessage): string {
  switch (msg.role) {
    case "user":
    case "developer":
...
function messageLogText(msg: NxcMessage): string {
...
function shouldCountStablePromptOverhead(parsed: NxcParsedRequest): boolean {
  return !parsed.previousResponseId && !parsed.context.messages.some(m => m.role === "assistant");
}

function estimateKiroInputTokens(parsed: NxcParsedRequest): number {
...
function estimateKiroLogInputTokens(parsed: NxcParsedRequest): number {
...
function kiroRuntimeEndpoint(provider: NxcProviderConfig, region: string): string {
...
function kiroThinkingBudget(parsed: NxcParsedRequest): number | undefined {
...
function injectKiroThinkingTags(content: string, parsed: NxcParsedRequest): string {
...
function validateKiroCapabilities(parsed: NxcParsedRequest): void {
...
export function buildKiroPayload(
  ...
  for (const msg of kiroPayloadMessages(parsed)) {
    if (msg.role === "user" || msg.role === "developer") {
      const text = userContentText((msg as { content: string | NxcContentPart[] }).content);
      const images = extractKiroImages((msg as { content: string | NxcContentPart[] }).content);
      pushUser(text, images);
    } else if (msg.role === "assistant") {
      const aMsg = msg as NxcAssistantMessage;
      const text = (aMsg.content || [])
        .filter((b): b is NxcTextContent => b.type === "text")
        .map(b => b.text)
        .join("");
      const toolCalls = (aMsg.content || [])
        .filter((b): b is NxcToolCall => b.type === "toolCall");
      ...
      const tr = msg as NxcToolResultMessage;
...
function mergeKiroUsage(
  first: NxcUsage | undefined,
  second: NxcUsage | undefined,
  preserveFirstContextGrowth = false,
): NxcUsage | undefined {
  if (!first) return second;
  if (!second) return first;
  const sumOptional = (key: keyof NxcUsage): number | undefined => {
    const a = first[key];
    const b = second[key];
...
async function* parseKiroAttemptEvents(
  ...
  let authoritativeUsage: NxcUsage | undefined;
...
// Adapter
export function createKiroAdapter(provider: NxcProviderConfig): ProviderAdapter {
  ...
  let requestSnapshot: NxcParsedRequest | undefined;
  ...
  const build = async (
    ...
    if (typeof provider.apiKey !== "string" || provider.apiKey.trim() === "") {
      throw new Error("kiro token missing — run nxc login kiro");
    }
  ...
  return {
    name: "kiro",
    async buildRequest(parsed: NxcParsedRequest, incoming) {
