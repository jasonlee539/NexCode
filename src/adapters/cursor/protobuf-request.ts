...
import type { NxcAssistantContentPart, NxcMessage, NxcToolResultMessage } from "../../types";
...
function assistantRootText(
  message: Extract<NxcMessage, { role: "assistant" }>,
...
function contentText(message: NxcMessage): string {
...
function contentToText(content: NxcToolResultMessage["content"]): string {
...
function historyContentText(message: NxcMessage): string {
 ...
 *
 * `NxcImageContent.imageUrl` is either a `data:` URL or a remote https URL, so this cannot reuse
 ...
 */
function decodeResultParts(message: NxcToolResultMessage): DecodedResultPart[] | undefined {
 ...
 */
function normalizedToolResult(message: NxcToolResultMessage, text: string): NormalizedToolResult {
...
function toolCallStep(
  part: Extract<NxcAssistantContentPart, { type: "toolCall" }>,
  requestScope: CursorBlobRequestScopeToken,
  result?: NxcToolResultMessage,
): Uint8Array {
...
function toolResultPart(message: NxcToolResultMessage, decoded?: DecodedResultPart[], maxImages?: number) {
  const parts = decoded ?? decodeResultParts(message);
...
function assistantStep(part: NxcAssistantContentPart, requestScope: CursorBlobRequestScopeToken): Uint8Array | undefined {
...
function lastActionIndex(messages: readonly NxcMessage[] | undefined): number {
...
function conversationTurns(
  ...
  const pendingToolCalls = new Map<string, Extract<NxcAssistantContentPart, { type: "toolCall" }>>();
  const flush = () => {
    if (!current) return;
