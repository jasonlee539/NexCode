import type { NxcUsage } from "../../types";
...
export function usageFromContextTokens(state: CursorProtobufEventState, contextTokens: number): NxcUsage {
  return {
    ...state.usage,
...
export function mcpArgsFromToolCall(toolCall: ToolCall | undefined): McpArgs | undefined {
  ...
  return args?.providerIdentifier === NXC_RESPONSES_TOOL_PROVIDER ? args : undefined;
}
...
export function mapSyntheticMcpExecToToolEvents(
  ...
  if (args.providerIdentifier !== NXC_RESPONSES_TOOL_PROVIDER) return [];
 ...
 */
export function resolvedTurnUsage(state: CursorProtobufEventState): NxcUsage {
