...
/**
 * One MCP server nexcode starts/connects and exposes to the Cursor agent as callable tools.
 * Either `command` (stdio: nexcode spawns the server as a child process) or `url`
 * (streamable-http: nexcode connects to a remote MCP server) must be set.
 */
 ...
 */
export function resolveMcpServers(provider: NxcProviderConfig): ResolvedMcpServer[] {
