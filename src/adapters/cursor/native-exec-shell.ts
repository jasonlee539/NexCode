...
export function nativeShellDisabledMessage(): string {
  ...
  // Keep this host-shell-neutral: NexCode may run on a different OS than the Codex
  // client that executes the bridge (LAN/SSH remote-proxy).
  return (
    "Route this through the Codex bridge shell tool from the current catalog (`shell_command` or `exec_command`, including the long `mcp_nexcode-responses_*` display name if listed). "
