...
import { isAllowedToolChoice, namespacedToolName, toolChoiceAliases, type NxcTool, type NxcToolChoice } from "../../types";
...
function explicitlySelectedNames(choice: NxcToolChoice | undefined): Set<string> {
...
function toolPriority(tool: NxcTool, selectedNames: ReadonlySet<string>): number {
  // Execution path (bare or nexcode-responses `exec` / `exec_command` / `shell_command`)
...
function isPinnedCursorTool(tool: NxcTool, selectedNames: ReadonlySet<string>): boolean {
...
export function applyCursorToolBudget(
  ...
  toolChoice: NxcToolChoice | undefined,
  ...
  const kept: NxcTool[] = [];
  const keptSet = new Set<NxcTool>();
  let keptBytes = 0;

  const tryKeep = (tool: NxcTool): boolean => {
...
function catalogLimitNote(kept: readonly NxcTool[], omitted: readonly NxcTool[]): string | undefined {
  ...
  return recoverable
    ? `[nexcode] Cursor's transport limit allows ${kept.length} of ${kept.length + omitted.length} client tools this turn. Omitted: ${omittedSummary}. Use tool_search for a needed omitted tool; tools returned by tool_search are prioritized on the next turn.`
    : `[nexcode] Cursor's transport limit allows ${kept.length} of ${kept.length + omitted.length} client tools this turn. Omitted and unavailable this turn: ${omittedSummary}.`;
}
...
function contentPartToText(part: NxcContentPart | NxcAssistantContentPart): string | undefined {
  switch (part.type) {
    case "text":
      return part.text;
    case "thinking":
...
function toolResultToText(message: NxcToolResultMessage): string {
  return [
    "[tool_result]",
...
function contentToText(content: string | readonly (NxcContentPart | NxcAssistantContentPart)[]): string {
...
function requestMessage(message: NxcMessage): CursorRequestMessage | undefined {
  switch (message.role) {
    case "user":
    case "developer":
      {
...
export function cursorRequestMessagesFromRaw(
  messages: readonly NxcMessage[] | undefined,
...
export function cursorInstructionDigest(parsed: NxcParsedRequest): string {
  const hash = createHash("sha256").update("nxc:cursor:sys:");
...
export function cursorCoveredPrefixDigest(parsed: NxcParsedRequest, coveredMessageCount: number): string {
  const hash = createHash("sha256").update("nxc:cursor:prefix:");
