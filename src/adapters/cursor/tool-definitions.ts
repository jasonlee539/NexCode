...
import type { NxcRequestOptions, NxcTool } from "../../types";
...
export const NXC_RESPONSES_TOOL_PROVIDER = "nexcode-responses";
...
export const CURSOR_SHELL_ALIAS_SYSTEM_NOTE =
  'Shell commands use the Codex shell bridge tool shown in this turn\'s catalog (`shell_command` or `exec_command`) with JSON arguments like {"cmd":"..."}. The long `mcp_nexcode-responses_*` display name is the same tool. Prefer it over Cursor-native Shell.';
...
export function cursorToolChoiceAliases(tool: Pick<NxcTool, "namespace" | "name">): string[] {
...
function catalogHasBareCodexShellBridge(
  catalog: readonly Pick<NxcTool, "namespace" | "name">[],
): boolean {
...
function cursorToolChoiceMatches(
  tool: Pick<NxcTool, "namespace" | "name">,
  choiceName: string,
  catalog: readonly Pick<NxcTool, "namespace" | "name">[],
): boolean {
...
export function isBareCodexShellBridgeTool(tool: Pick<NxcTool, "namespace" | "name">): boolean {
...
function isCursorResponsesProvider(namespace: string | undefined): boolean {
  return !namespace || namespace === NXC_RESPONSES_TOOL_PROVIDER;
}
...
export function isCursorExecutionPathTool(tool: Pick<NxcTool, "namespace" | "name">): boolean {
...
export function isCursorWaitTool(tool: Pick<NxcTool, "namespace" | "name">): boolean {
...
export function isCursorCodeModeExecTool(
  tool: Pick<NxcTool, "namespace" | "name" | "freeform">,
): boolean {
  return isCursorResponsesProvider(tool.namespace)
...
export function cursorRequestUsesCodeMode(
  tools: readonly Pick<NxcTool, "namespace" | "name" | "freeform">[] | undefined,
  toolChoice?: NxcRequestOptions["toolChoice"],
): boolean {
  const catalog = tools ?? [];
...
function isBareCodexExecCommandTool(tool: Pick<NxcTool, "namespace" | "name">): boolean {
  return isBareCodexShellBridgeTool(tool);
}

export function cursorRequestHasShellAlias(tools: readonly Pick<NxcTool, "namespace" | "name">[] | undefined): boolean {
  return tools?.some(isBareCodexExecCommandTool) ?? false;
}

function cursorRequestHasExecutionPath(
  tools: readonly Pick<NxcTool, "namespace" | "name">[] | undefined,
): boolean {
...
export function isCursorSyntheticStructuredEditTool(
  tool: Pick<NxcTool, "namespace" | "name" | "cursorStructuredEdit">,
): boolean {
...
export function cursorToolWireName(tool: Pick<NxcTool, "namespace" | "name">): string {
 ...
 * sessions: `mcp_nexcode-responses_exec_command` / `mcp_nexcode-responses_shell_command`).
 ...
 */
const CURSOR_MCP_DISPLAY_PREFIX = `mcp_${NXC_RESPONSES_TOOL_PROVIDER}_`;
 ...
 * `mcp_${NXC_RESPONSES_TOOL_PROVIDER}_` prefix, never generic `mcp_`.
...
export function cursorToolInputSchema(tool: NxcTool): unknown {
 ...
 */
export function cursorToolArgNormalizeSchema(tool: NxcTool): unknown {
  if (isBareCodexShellBridgeTool(tool)) {
...
function shellBridgeArgNormalizeSchema(tool: NxcTool): unknown {
...
export function cursorToolsForActivePrompt<T extends Pick<NxcTool, "namespace" | "name">>(
...
export function cursorToolAllowedByChoice(
  tool: Pick<NxcTool, "namespace" | "name">,
  toolChoice: NxcRequestOptions["toolChoice"] | undefined,
  catalog: readonly Pick<NxcTool, "namespace" | "name">[] = [tool],
): boolean {
...
export function buildCursorToolGuidanceSystemNote(
  ...
  // the NexCode proxy OS (LAN/SSH remote-proxy). Always cover PowerShell 5.1 pitfalls.
  ...
  const notes = [
    ...
    hasBareExec
      ...
      : undefined,
    hasBareExec
      ? "Your tool list may display it under a longer `mcp_nexcode-responses_shell_command` / `mcp_nexcode-responses_exec_command` name; those are the SAME tool — call whichever your list shows, and do not comment on the naming difference to the user."
      : undefined,
    hasBareExec
    ...
    hasApplyPatch
      ? structuredEditNames.length > 0
        ? `For file edits, prefer the structured edit tools ${quotedNames(structuredEditNames)} — they take replacements that NexCode converts into Codex \`apply_patch\` changes. Include exact leading whitespace in old_string/new_string. Use \`apply_patch\` directly only with a \`*** Begin Patch\` envelope and bare \`@@\` hunks (never git-style \`@@ -n,m +n,m @@\`); never emit patch-like plain text as tool arguments.`
...
export function buildCursorToolDefinitions(
  tools: readonly NxcTool[] | undefined,
  toolChoice?: NxcRequestOptions["toolChoice"],
): McpToolDefinition[] {
  ...
  return tools.filter(tool => cursorToolAllowedByChoice(tool, toolChoice, tools)).map(tool => {
    ...
    return create(McpToolDefinitionSchema, {
      name: wireName,
      toolName: wireName,
      providerIdentifier: NXC_RESPONSES_TOOL_PROVIDER,
