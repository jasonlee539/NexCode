...
import type { NxcContentPart, NxcImageContent, NxcMessage } from "../../types";
import {
...
function isImagePart(part: NxcContentPart): part is NxcImageContent {
...
export function extractCursorImageUrls(content: string | readonly NxcContentPart[]): string[] {
...
export function extractCursorImageParts(
  content: string | readonly NxcContentPart[],
): CursorImagePartRef[] {
...
/**
 * Resolve NexCode image parts (data: URLs only) into bytes for SelectedImage.
...
async function prepareCursorContentParts(
  content: string | readonly NxcContentPart[],
  signal?: AbortSignal,
): Promise<{ content: string | readonly NxcContentPart[]; images: ResolvedCursorImage[] }> {
  if (typeof content === "string" || !Array.isArray(content)) {
    return { content, images: [] };
  }
  let changed = false;
  const next: NxcContentPart[] = [];
 ...
 */
export function cursorVisionPrepareStartIndex(messages: readonly NxcMessage[]): number {
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (
      ...
      if (prepared.content !== message.content) {
        changed = true;
        out.push({ ...message, content: prepared.content } as NxcMessage);
        continue;
      }
    }
    out.push(message);
