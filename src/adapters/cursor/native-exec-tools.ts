...
export async function mcpExec(execMsg: ExecServerMessage, deps: CursorNativeToolDeps): Promise<Uint8Array> {
  ...
  try {
    if (execMsg.message.value.providerIdentifier === NXC_RESPONSES_TOOL_PROVIDER) {
    ...
    const result = deps.mcp
      ...
      : create(McpResultSchema, { result: { case: "error", value: create(McpErrorSchema, { error: "No local MCP executor is configured inside nexcode." }) } });
  ...
  const result = deps.listMcpResources
    ...
    : create(ListMcpResourcesExecResultSchema, {
        result: { case: "error", value: create(ListMcpResourcesErrorSchema, {
          error: "No local MCP resource executor is configured inside nexcode.",
        }) },
      });
  ...
  try {
    const result = deps.readMcpResource
      ...
      : create(ReadMcpResourceExecResultSchema, {
        result: { case: "error", value: create(ReadMcpResourceErrorSchema, { uri: args.uri, error: "No local MCP resource executor is configured inside nexcode." }) },
      });
  ...
  try {
    const result = deps.computerUse
      ...
      : create(ComputerUseResultSchema, {
        result: { case: "error", value: create(ComputerUseErrorSchema, { error: "computer-use is not supported in this headless nexcode proxy. Configure provider.desktopExecutor.computerUseCommand to enable it.", actionCount: args.actions.length, durationMs: 0 }) },
      });
  ...
  try {
    const result = deps.recordScreen
      ...
      : create(RecordScreenResultSchema, {
        result: { case: "failure", value: create(RecordScreenFailureSchema, { error: "record-screen is not supported in this headless nexcode proxy. Configure provider.desktopExecutor.recordScreenCommand to enable it." }) },
      });
