...
import { namespacedToolName, type NxcProviderConfig, type NxcUsage } from "../../types";
...
import { NXC_RESPONSES_TOOL_PROVIDER } from "./tool-definitions";
import {
...
export class CursorMissingCredentialError extends Error {
  ...
  constructor() {
    super("Cursor live transport requires a Cursor access token in provider.apiKey, Authorization, or NEXCODE_CURSOR_TEST_TOKEN.");
    this.name = "CursorMissingCredentialError";
  }
}

export function resolveCursorToken(provider: NxcProviderConfig, headers?: Headers): string {
  ...
  const envToken = process.env.NEXCODE_CURSOR_TEST_TOKEN?.trim();
...
export function planMcpArgsHandling(
  ...
  if (args.providerIdentifier !== NXC_RESPONSES_TOOL_PROVIDER) {
 ...
 * (devlog 260702_cursor-live-stability-rca). nxc is a headless non-interactive client, so:
...
export function planInteractionQueryReply(query: InteractionQuery): { response: InteractionResponse; replyCase: string; planText?: string } {
  const NON_INTERACTIVE_REASON = "nexcode bridge is non-interactive; proceed without this interaction.";
  const q = query.query;
...
class LiveCursorTransport implements CursorTransport {
  ...
  // Per-turn diagnostic counters/timestamps when provider debug is on (`nxc debug provider on`). Stamped in open(), cleared on
 ...
 private firstFrameAt?: number;
  ...
  async *run(request: CursorRunRequest, signal?: AbortSignal): AsyncIterable<CursorServerMessage> {
    ...
    // frame. Gated behind provider debug (`nxc debug provider on`).
  ...
  private open(
    ...
    if (useHttp1) {
      const providerFetch = this.input.fetch
        ?? (this.input.provider as NxcProviderConfig & { fetch?: typeof globalThis.fetch }).fetch;
 ...
 */
export function partialUsageFromEventState(state: ReturnType<typeof createCursorProtobufEventState>): NxcUsage | undefined {
...
function attachPartialUsage(failure: Error, state: ReturnType<typeof createCursorProtobufEventState>): Error {
  ...
  if (usage) (failure as Error & { partialUsage?: NxcUsage }).partialUsage = usage;
  return failure;
}

/**
 * Compact frame descriptor for provider debug (`nxc debug provider on`): outer case plus the inner
 ...
 * Only frames whose inner ToolCall is an nxc-bridged Responses tool (`mcpToolCall` with our provider)
 ...
 * surfacing the (potentially secret-bearing) message — used for `[nxc:cursor:connect-end-stream]`
