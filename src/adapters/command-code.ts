...
import type { AdapterEvent, NxcContentPart, NxcMessage, NxcParsedRequest, NxcProviderConfig, NxcTool, NxcUsage } from "../types";
...
function toolResultText(content: string | NxcContentPart[]): string {
 ...
 */
function wireMessages(messages: NxcMessage[]): Array<Record<string, unknown>> {
  ...
  const closePendingCalls = (): void => {
    for (const call of pendingCalls) {
      out.push({ role: "tool", content: [{
        ...
        toolCallId: call.id,
        toolName: call.name,
        output: { type: "error-text", value: "[nxc] no tool result was recorded for this tool call; execution status unknown." },
      }] });
    }
...
function visibleTools(parsed: NxcParsedRequest): NxcTool[] {
...
function toolChoiceInstruction(parsed: NxcParsedRequest): string | undefined {
  const choice = parsed.options.toolChoice;
...
function wireTools(tools: NxcTool[]): Array<Record<string, unknown>> {
...
function usage(value: unknown): NxcUsage | undefined {
...
function supportedCommandCodeEffort(provider: NxcProviderConfig, modelId: string, requested: string | undefined): string | undefined {
...
export function createCommandCodeAdapter(provider: NxcProviderConfig): ProviderAdapter {
  const executor = (provider as NxcProviderConfig & { fetch?: typeof globalThis.fetch }).fetch ?? globalThis.fetch;
  return {
    name: "command-code",
    async buildRequest(parsed: NxcParsedRequest): Promise<AdapterRequest> {
      if (!provider.apiKey) throw new Error("Command Code credential missing — run nxc login command-code");
