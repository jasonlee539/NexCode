...
import type { AdapterEvent, NxcParsedRequest, NxcProviderConfig } from "../../types";
...
function parsedFromContext(vector: Record<string, unknown>): NxcParsedRequest {
...
function createHarnessAdapter(provider: NxcProviderConfig) {
...
async function runReasoningEffortMapping(
  ...
  const provider: NxcProviderConfig = {
    ...
    reasoningWireFormat: vector.reasoningWireFormat as NxcProviderConfig["reasoningWireFormat"],
  };
  const parsed = parseRequest({
    model: "fixture-model",
    input: "PING",
