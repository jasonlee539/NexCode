 ...
 *
 * `refreshUserCostOverlays` is process-local by design: `nxc config set`
...
const owners = new Map<symbol, NxcConfig | null>();
...
function liveConfigsForPreservation(extra?: NxcConfig | null): NxcConfig[] {
  const liveConfigs: NxcConfig[] = [];
  const seen = new Set<NxcConfig>();
  if (extra) {
    seen.add(extra);
 ...
 */
function adoptDiskModelCosts(live: NxcConfig, disk: NxcConfig): void {
 ...
 */
function rememberDiskOnlyProviders(liveConfigs: readonly NxcConfig[], disk: NxcConfig): void {
  const preserved: Record<string, NxcProviderConfig> = {};
 ...
 */
export function reconcileUserCostOverlaysFromDisk(liveConfig?: NxcConfig | null): boolean {
  const diagnostics = readConfigDiagnostics();
...
export function startUserCostOverlayReconciler(
  options: { intervalMs?: number; liveConfig?: NxcConfig | null } = {},
): { stop(): void } {
