 ...
 * (`providers.<name>.modelCosts` in config.json — per-model prices in nxc's
 ...
 */
import type { NxcConfig, NxcProviderConfig, ProviderCostOverlay } from "../types";
...
let version = 0;
let preservedDiskOnlyProviders: Record<string, NxcProviderConfig> | null = null;
 ...
 */
const PRESERVATION_OWNER_STATE = Symbol("nexcode.user-cost-overlay-preservation-owner");
const PERSISTED_PROVIDER_DELETIONS = Symbol("nexcode.persisted-provider-deletions");
...
type PreservationTaggedConfig = NxcConfig & {
...
function providerNames(config: NxcConfig): Set<string> {
...
export function registerPreservedProviderOwner(config: NxcConfig): void {
 ...
 */
export function refreshPreservedProviderOwner(config: NxcConfig, disk: NxcConfig): void {
...
export function unregisterPreservedProviderOwner(config: NxcConfig): void {
  const tagged = config as PreservationTaggedConfig;
...
export function setPreservedDiskOnlyProviders(
  providers: Record<string, NxcProviderConfig> | null,
): void {
 ...
 */
function commitPersistedProviderDeletions(config: NxcConfig): void {
  const tagged = config as PreservationTaggedConfig;
 ...
 */
export function withPreservedDiskOnlyProviders(config: NxcConfig): NxcConfig {
  const tagged = config as PreservationTaggedConfig;
  ...
  let preserved: Record<string, NxcProviderConfig> | null = null;
  if (preservedDiskOnlyProviders) {
 ...
 */
export function refreshUserCostOverlays(config: NxcConfig): void {
