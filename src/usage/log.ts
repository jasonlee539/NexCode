...
import type { AttemptTierOutcome, NxcUsage } from "../types";
...
export function usageTotalTokens(usage: NxcUsage | undefined): number | undefined {
...
export function usageForFinalLog(provider: string, usage: NxcUsage | undefined): NxcUsage | undefined {
  if (!usage) return undefined;
  if (usage.estimated || isEstimatedUsageProvider(provider)) return { ...usage, estimated: true };
  return usage;
}

export function usageStatusForFinalLog(usage: NxcUsage | undefined): UsageStatus {
  if (!usage) return "unreported";
  return usage.estimated ? "estimated" : "reported";
}

function normalizeUsageValue(usage: NxcUsage | undefined): NxcUsage | undefined {
  if (!usage) return undefined;
  return {
...
function normalizeAttemptUsage(raw: unknown): NxcUsage | null {
  ...
  return normalizeUsageValue(usage as unknown as NxcUsage) ?? null;
}
 ...
 * file into memory. Used by request-log hydration on `nxc start`.
 */
