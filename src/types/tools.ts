...
export function toolChoiceAliases(tool: Pick<NxcTool, "namespace" | "name">): string[] {
  const wireName = namespacedToolName(tool.namespace, tool.name);
  return tool.namespace ? [wireName, `${tool.namespace}.${tool.name}`] : [wireName];
}

function sameToolIdentity(
  left: Pick<NxcTool, "namespace" | "name">,
  right: Pick<NxcTool, "namespace" | "name">,
): boolean {
...
export function toolChoiceCandidates(
  ...
  name: string,
): Pick<NxcTool, "namespace" | "name">[] {
  if (!tools) return [];
  const candidates: Pick<NxcTool, "namespace" | "name">[] = [];
...
export function toolAllowedByChoice(
  ...
  tools?: readonly Pick<NxcTool, "namespace" | "name">[],
): boolean {
...
export function resolveToolChoiceWireName(tools: readonly Pick<NxcTool, "namespace" | "name">[] | undefined, name: string): string {
  const candidates = toolChoiceCandidates(tools, name);
...
export function isAllowedToolChoice(value: NxcToolChoice | undefined): value is { allowedTools: string[]; mode: "auto" | "required" } {
...
export function toolChoiceToolPredicate(
  choice: NxcToolChoice | undefined,
  tools?: readonly Pick<NxcTool, "namespace" | "name">[],
): (tool: Pick<NxcTool, "namespace" | "name">) => boolean {
