import type { NxcProviderConfig } from "./provider";
import type { CodexAccount } from "./accounts";

/**
 * Claude Code inbound settings (devlog/260711_claude_inbound). Consumed by the
 * /v1/messages surface, the `nxc claude` launcher, and the GUI Claude page.
 */
export interface NxcClaudeCodeConfig {
   ...
   */
  bodyMaxBytes?: number;
  /** Default model slot injected as ANTHROPIC_MODEL by `nxc claude`. */
  model?: string;
  ...
  * so plain `claude` commands route through the proxy without `nxc claude`. Reverted
   ...
   * ~/.claude/agents/nxc-*.md custom agent definitions at launch (devlog 260712
   ...
   * free. Only nxc-*.md files are owned/pruned. Default: enabled.
   */
  injectAgents?: boolean;
  /**
   * Optional Claude Code effort pinned in every generated nxc-* subagent
  ...
  desktopProfile?: NxcClaudeDesktopProfile;
...
export type NxcClaudeDesktopFamily = "opus" | "fable" | "sonnet" | "haiku";

export interface NxcClaudeDesktopAssignment {
  family: NxcClaudeDesktopFamily;
  alias: string;
}

export interface NxcClaudeDesktopProfile {
  version: 1;
  assignments: Record<string, NxcClaudeDesktopAssignment>;
  defaults: Record<NxcClaudeDesktopFamily, string | null>;
 ...
 * Persisted under `NxcConfig.storageCleanupPolicy`. Default `enabled: false`.
 */
...
export interface NxcCustomModel {
...
/**
 * A generated `nxc_` data-plane key. `key` is the secret itself and never leaves
 * the server except in the one-time POST /api/keys response; every other surface
 * sees only the masked prefix.
 */
export interface NxcApiKeyEntry {
  id: string;
  name: string;
  key: string;
  createdAt: string;
 ...
 */
export interface NxcClientIntegrationsConfig {
...
export interface NxcConfig {
  ...
  claudeCode?: NxcClaudeCodeConfig;
  /**
   ...
   */
  clientIntegrations?: NxcClientIntegrationsConfig;
  /**
   ...
   * when every routed row is listed (see nexcode_spawn_priority / effectiveSubagentRoster).
   */
  modelPickerOrder?: string[];
  /**
   * Priority-ordered fallback models for spawned sub-agents. When the requested
   * model is quota-exhausted or recently failed, nexcode rewrites the child
  ...
  * Opt-in; disabled by default. Matching maintenance/helper requests are forced to low.
   ...
   * wildcard, every caller needs `x-nexcode-api-key`, but a `codex app-server` spawned
   ...
   * so Codex App can show old OpenAI chats and nexcode-created exec chats under its default
   * interactive-source/provider filters. Default true; originals are backed up and restored by
   * `nxc stop` / `nxc restore`. Set false to opt out of history remapping.
   */
  ...
  accountPoolStrategy?: NxcAccountPoolRotationStrategy;
  ...
  anthropicAccountPool?: {
    ...
    strategy?: NxcAccountPoolRotationStrategy;
    /** Successful new-session binds retained on one round-robin selection. Default 1; range 1..100. */
    stickyLimit?: number;
  };
  /** Virtual `combo/<id>` models spanning concrete provider/model targets (issue #133). */
  combos?: Record<string, NxcComboConfig>;
  /**
   ...
   */
  routingProfiles?: Record<string, NxcRoutingProfileConfig>;
  /** Background proactive token refresh ("Token Guardian"). Off by default; see NxcTokenGuardianConfig. */
  tokenGuardian?: NxcTokenGuardianConfig;
  /** Additional exact origins allowed for CORS (e.g. HTTPS or chrome-extension://<id>). Loopback origins are always allowed. */
  corsAllowOrigins?: string[];
}

export type NxcAccountPoolRotationStrategy = "quota" | "round-robin" | "fill-first";

export type NxcComboStrategy = "failover" | "round-robin";
export type NxcComboDefaultEffort = "low" | "medium" | "high" | "xhigh" | "max" | "ultra";

export interface NxcComboTarget {
  provider: string;
  model: string;
...
export interface NxcComboConfig {
  targets: NxcComboTarget[];
  /** Ordered failover (default) or deterministic smooth weighted round-robin. */
  strategy?: NxcComboStrategy;
  ...
  defaultEffort?: NxcComboDefaultEffort | null;
  /**
...
export type NxcRoutingUnknownEvidenceMode = "allow" | "penalize" | "exclude";

export interface NxcRoutingProfileCandidate {
  provider: string;
  model: string;
}

export interface NxcRoutingProfileRequirements {
...
export interface NxcRoutingProfileOptimize {
  latency?: number;
  health?: number;
  cost?: number;
  quota?: number;
 ...
 */
export type NxcRoutingUnknownCostCapMode = "allow" | "exclude";

export interface NxcRoutingProfileLimits {
   ...
   */
  onUnknownCost?: NxcRoutingUnknownCostCapMode;
}

export interface NxcRoutingProfileUnknownEvidence {
  capability?: NxcRoutingUnknownEvidenceMode;
  health?: NxcRoutingUnknownEvidenceMode;
  quota?: NxcRoutingUnknownEvidenceMode;
  cost?: NxcRoutingUnknownEvidenceMode;
}

export interface NxcRoutingProfileCompatibilitySuite {
  suiteId: string;
  evidenceLayer: "protocol_conformance" | "live_route_compatibility";
}

export interface NxcRoutingProfileCompatibility {
  requiredSuites?: NxcRoutingProfileCompatibilitySuite[];
  minStatus?: "PROBED" | "VERIFIED";
  maxEvidenceAgeMs?: number;
  unknownEvidence?: NxcRoutingUnknownEvidenceMode;
  degradedEvidence?: NxcRoutingUnknownEvidenceMode;
}

export interface NxcRoutingProfileConfig {
  /**
   * Explicit candidate allowlist (`provider/model` refs). No implicit
   * expansion in v1.
   */
  candidates: NxcRoutingProfileCandidate[];
  /** Optional public model name replacing the default `policy/<id>` slug. */
  alias?: string;
  /** Hard requirements evaluated before scoring. */
  require?: NxcRoutingProfileRequirements;
  /** Optimization weights; normalized deterministically. */
  optimize?: NxcRoutingProfileOptimize;
  limits?: NxcRoutingProfileLimits;
  /** How unknown evidence is handled per dimension. */
  unknownEvidence?: NxcRoutingProfileUnknownEvidence;
  /** Optional Compatibility Lab policy (CL-06). */
  compatibility?: NxcRoutingProfileCompatibility;
}


export interface NxcTokenGuardianConfig {
...
export interface NxcImagesConfig {
...
export interface NxcSearchConfig {
  /**
...
export interface NxcVisionSidecarConfig {
...
export interface NxcWebSearchSidecarConfig {
