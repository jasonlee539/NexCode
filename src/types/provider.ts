 ...
 */
export interface NxcProviderConfig {
  adapter: string;
  /**
   ...
   * `nxcr1` envelopes are still stripped because no upstream can decrypt them.
   */
  ...
  */
   ...
   * opencode-style per-model pricing in nxc's flat `modelXxx` convention:
  ...
  location?: string;
  /**
   * Cursor adapter only: MCP servers nexcode starts/connects and exposes to the Cursor agent
  ...
  /**
   * Cursor adapter only: opt-in external executor for computer-use / record-screen. nexcode is
   ...
   * caller-controlled prose, and nexcode has no trustworthy per-request attestation that it
