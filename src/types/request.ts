...
import type { NxcTool, NxcToolChoice } from "./tools";
...
export interface NxcReasoningReplayIdentity {
  providerName: string;
 ...
 */
export interface NxcReasoningReplayScopeRef {
  readonly clientThreadId: string;
  current?: Readonly<NxcReasoningReplayIdentity>;
}

export interface NxcParsedRequest {
  modelId: string;
  ...
  _reasoningReplayScope?: NxcReasoningReplayScopeRef;
  /**
  ...
  _providerContinuation?: NxcProviderContinuationState;
  /** Persisted continuation considered only after the final physical route is known. */
  _providerContinuationCandidate?: NxcProviderContinuationState;
  /** Exact process-local route owner attached to newly persisted provider state. */
  _providerContinuationOwner?: NxcProviderContinuationOwner;
  /**
...
export interface NxcContext {
  systemPrompt?: string[];
  messages: NxcMessage[];
  tools?: NxcTool[];
}

export type NxcMessage =
  | NxcUserMessage
  | NxcAssistantMessage
  | NxcDeveloperMessage
  | NxcToolResultMessage;

export interface NxcUserMessage {
  role: "user";
  content: string | NxcContentPart[];
  timestamp: number;
}

export interface NxcAssistantMessage {
  role: "assistant";
  content: NxcAssistantContentPart[];
  /** Responses message phase, preserved when replaying translated provider output. */
  phase?: NxcMessagePhase;
  model?: string;
  timestamp: number;
  /**
...
export interface NxcDeveloperMessage {
  role: "developer";
  content: string | NxcContentPart[];
  timestamp: number;
}

export interface NxcToolResultMessage {
  role: "toolResult";
  toolCallId: string;
  toolName: string;
...
export interface NxcTextContent {
  type: "text";
  text: string;
}

export interface NxcImageContent {
  type: "image";
...
export type NxcContentPart = NxcTextContent | NxcImageContent;

export interface NxcThinkingContent {
  type: "thinking";
  thinking: string;
  signature?: string;
  itemId?: string;
...
export interface NxcToolCall {
   ...
   */
  providerMetadata?: NxcProviderOpaqueToolCallMetadata;
 ...
 */
export interface NxcProviderOpaqueToolCallMetadata {
  google?: {
    thoughtSignature?: string;
  };
}

export type NxcAssistantContentPart = NxcTextContent | NxcThinkingContent | NxcToolCall;
export interface NxcRequestOptions {
  maxOutputTokens?: number;
  temperature?: number;
  topP?: number;
  stopSequences?: string[];
  toolChoice?: NxcToolChoice;
...
export type NxcMessagePhase = "commentary" | "final_answer";

/** Non-secret, process-local owner fence for provider-private continuation state. */
export interface NxcProviderContinuationOwner {
 ...
 */
export interface NxcProviderContinuationState {
  /** Proxy-authored owner metadata; stripped before provider adapters receive the state. */
  __nxcOwner?: NxcProviderContinuationOwner;
  cursor?: {
...
export type AdapterEvent =
  | { type: "heartbeat" }
  | { type: "text_delta"; text: string; phase?: NxcMessagePhase }
  ...
  | { type: "tool_call_start"; id: string; name: string; providerMetadata?: NxcProviderOpaqueToolCallMetadata }
  ...
  | { type: "web_search_call_end"; id: string; queries: string[]; status?: "completed" | "failed"; sources?: NxcUrlCitation[] }
  | {
      type: "done";
      usage?: NxcUsage;
      stopReason?: string;
      endTurn?: boolean;
      providerState?: NxcProviderContinuationState;
    }
  | {
      type: "incomplete";
      reason: string;
 ...
 */
export interface NxcUrlCitation {
  url: string;
  title?: string;
}
