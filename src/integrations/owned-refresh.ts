/**
 * Refresh a file integration only when NexCode already owns its block.
 *
 * This is the safe bridge between an implicit operation such as `nxc sync`
...
export interface OwnedIntegrationRefreshInput {
  ...
  /** Lazy in `nxc sync`, so an unowned client does not even load the catalog. */
...
/**
 * Returns `null` when the client has never been connected by NexCode.
