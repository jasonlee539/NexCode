 ...
 *
 * `nxc stop` has honored `ServiceOwnershipError` since it started catching it
 ...
 * service running from another CODEX_HOME/NEXCODE_HOME — the installed
 ...
 * and `injectGrokConfig` already runs unguarded from `nxc start`/`ensure`.
 */
import {
 ...
 * this process is the only writer. Two foreground `nxc start` processes on one
...
function claimNamesDifferentHome(
  ...
  current: { codexHome: string; nexcodeHome: string },
): boolean {
  ...
  if (claim.homes.nexcodeHome !== null && !serviceHomeMatches(claim.homes.nexcodeHome, current.nexcodeHome)) return true;
  return false;
}
...
export interface OwnershipDeps extends ProbeDeps {
  ...
  readonly currentHomes?: { codexHome: string; nexcodeHome: string };
}
...
export function inspectNativeCodexOwnership(deps: OwnershipDeps = {}): OwnershipInspection {
  ...
  for (const one of valid) {
    for (const other of valid) {
      if (!serviceHomeMatches(one.state.codexHome, other.state.codexHome)
        || !serviceHomeMatches(one.state.nexcodeHome, other.state.nexcodeHome)) {
  ...
  const foreign = valid.find(e =>
    ...
    || !serviceHomeMatches(e.state.nexcodeHome, current.nexcodeHome));
  if (foreign) {
    return {
      ownership: "foreign",
      reason: `a service is installed for CODEX_HOME=${foreign.state.codexHome} / NEXCODE_HOME=${foreign.state.nexcodeHome}`,
    };
  }

  // The manager assets live under the effective NEXCODE_HOME. Production
  ...
  // falling back to <homedir>/.nexcode.
  const manager = inspectServiceManagerInstallation({
    ...deps,
    configDir: deps.configDir ?? current.nexcodeHome,
  });
