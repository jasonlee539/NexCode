/**
 * Apply, disable and restore an nexcode provider block in a client's config.
 *
...
function preflight(input: IntegrationWriteInput) {
  ...
  if (parsed === PARSE_FAILED) {
    return { failed: refuse(clientId, "unsafe", "unsafe",
      `${configPath} could not be parsed, or holds something nexcode cannot rewrite without changing it (a non-finite number, a large integer or a tiny one a rewrite would round, -0, a duplicate member, or nesting deeper than 1000 levels)`) } as const;
  }
...
function applyOrRefreshIntegration(input: IntegrationWriteInput, allowAbsent: boolean): WriteOutcome {
  ...
  if (classified.state === "conflict") {
    return refuse(clientId, "conflict", "conflict",
      classified.reason === "foreign-edit"
        ? `${configPath} changed after nexcode wrote it`
        : `${configPath} already contains an nexcode block we did not write`);
  }
  /*
  ...
  if (classified.state === "unsafe") {
    return refuse(clientId, "unsafe", "unsafe",
      classified.reason === "blocked-container"
        ? `${configPath} holds a value where nexcode would have to write a section, so applying would replace it`
  ...
  try {
    if (spec.sourcePreservingYaml && before !== null) {
      ...
      if (patched === null) {
        return refuse(clientId, "unsafe", "unsafe",
          `${configPath} uses YAML source nexcode cannot patch without risking unrelated comments or formatting, so it was left alone`);
      }
      text = patched;
    } else {
    ...
    return refuse(clientId, "unsafe", "unsafe",
      `${configPath} contains something nexcode cannot rewrite safely (${error.message}), so it was left alone`);
  }
...
export function disableIntegration(input: IntegrationWriteInput): WriteOutcome {
  ...
  if (classified.state === "conflict") {
    return refuse(clientId, "conflict", "conflict",
      classified.reason === "foreign-edit"
        ...
        : `${configPath} contains an nexcode block we did not write`);
  }
  /*
  ...
  if (classified.state === "unsafe") {
    return refuse(clientId, "unsafe", "unsafe",
      classified.reason === "blocked-container"
        ? `${configPath} holds a value where nexcode would have to read a section, so nothing can be removed safely`
        : `${configPath} cannot be changed safely`);
  }
  ...
  if (prunableCreated === null) {
    return refuse(clientId, "unsafe", "unsafe",
      `${configPath} uses YAML source nexcode cannot patch without risking unrelated comments or formatting, so nothing was removed`);
  }
  ...
  try {
    ...
    if (!(error instanceof UnserializableValueError)) throw error;
    return refuse(clientId, "unsafe", "unsafe",
      `${configPath} contains something nexcode cannot rewrite safely (${error.message}), so nothing was removed`);
  }
...
export function restoreIntegration(input: IntegrationRestoreInput): WriteOutcome {
   ...
   * that describes what nexcode had written. Overwriting the record's
