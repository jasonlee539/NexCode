/**
 * Data-access layer for `nxc account` (issue #180) — live-proxy HTTP client and
...
export function classifyAccount(config: NxcConfig, name: string): ClassifyResult {
...
export function proxyUnreachable(): number {
  console.error("Proxy not reachable. Start it with 'nxc start' or 'nxc ensure'.");
  return 1;
}
...
export function apiError(json: Record<string, unknown>, fallback: string): number {
  ...
  if (json.cleanupRequired === true) {
    console.error("Warning: native-login staging cleanup is still required; run 'nxc account main doctor'.");
  }
  return 1;
}
