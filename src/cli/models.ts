/**
 * `nxc models` subcommand — list configured models and manage custom models.
 */
...
import { modelInList, type NxcConfig, type NxcCustomModel } from "../types";

const ADD_USAGE = "Usage: nxc models add <provider> <modelId> [--display-name <name>] [--context-window <tokens>] [--modalities text,image,audio] [--reasoning-efforts <none,minimal,low,medium,high,xhigh,max,ultra>] [--default-reasoning-effort <level>]";
const REMOVE_USAGE = "Usage: nxc models remove <customId|provider/modelId> [--yes]";
const LIST_CUSTOM_USAGE = "Usage: nxc models list-custom [--json]";
const ALLOWED_MODALITIES = new Set(["text", "image", "audio"]);

/**
 * Parse and validate the reasoning flags shared by `nxc models add` (offline path).
...
function collectModels(config: NxcConfig, providerFilter?: string): ModelEntry[] {
...
async function handleCustomAdd(args: string[]): Promise<void> {
  ...
  if (!hasOwnProvider(config.providers, provider)) {
    fail(`provider "${provider}" is not configured. See: nxc provider list`);
  }
...
async function confirmCustomRemoval(model: NxcCustomModel): Promise<boolean> {
...
function customModelCells(model: NxcCustomModel): string[] {
  return [
    model.id.slice(0, 8),
    model.modelId,
    model.displayName ?? "-",
...
function printCustomModelGroup(provider: string, models: NxcCustomModel[]): void {
...
function handleCustomList(args: string[]): void {
  ...
  const byProvider = new Map<string, NxcCustomModel[]>();
...
function handleConfiguredModels(args: string[]): void {
  ...
  if (restArgs.length > 0) {
    ...
    console.error("Usage: nxc models [--provider <name>] [--json]");
    process.exit(1);
  }
  ...
  if (providerFilter && !hasOwnProvider(config.providers, providerFilter)) {
    console.error(`Provider "${providerFilter}" is not configured. See: nxc provider list`);
    process.exit(1);
  }
