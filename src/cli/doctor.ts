/**
 * `nxc doctor` - read-only environment diagnostics.
 *
...
import { countPendingNexcodeHistory } from "../codex/history-provider";
import {
...
function actionForDoctorEntry(entry: OAuthHealthEntry): string {
  ...
  if (entry.health.status === "warning" && entry.health.reason === "stale_credentials") {
    return `run \`nxc login ${entry.provider}\``;
  }
  if (entry.health.status === "warning" && entry.health.reason === "metadata_mismatch") {
    return `run \`nxc login ${entry.provider}\` to refresh credentials`;
  }
  return `run \`nxc doctor\` again after fixing OAuth state for ${entry.provider}`;
}
...
/**
 * OAuth reliability checks for `nxc doctor`. Observe-only: never mutates
...
export async function collectOAuthDoctorChecks(
  ...
  if (isOAuthCredentialStorageWritable()) {
    ...
    checks.push({
      level: "WARN",
      message:
        "OAuth credential storage directory is not writable. Action: fix permissions on NEXCODE_HOME so nxc can create temp files and rename auth.json",
    });
  }
  ...
  if (isOAuthRefreshSingleFlightReady()) {
    ...
    checks.push({
      level: "WARN",
      message:
        "Token refresh single-flight is unavailable. Action: fix permissions on NEXCODE_HOME so nxc can create refresh lock files",
    });
  }
  ...
  if (report.codexHealthSource === "unavailable") {
    checks.push({
      level: "WARN",
      message:
        "Codex account health unavailable (proxy not running). Action: start the proxy and re-run `nxc doctor` to inspect live cooldown/reauth",
    });
  } else if (report.codexHealthSource === "management-auth-failed") {
    checks.push({
      level: "WARN",
      message:
        "Codex account health unavailable (proxy running; management authentication failed). Action: verify the admin token configuration, restart the proxy, and re-run `nxc doctor`",
    });
  } else if (report.codexHealthSource === "management-api-unavailable") {
    checks.push({
      level: "WARN",
      message:
        "Codex account health unavailable (proxy running; management API response failed). Action: inspect the proxy service log, restart the proxy if needed, and re-run `nxc doctor`",
    });
  }
...
export function collectPaths(): PathRow[] {
  ...
  const nexcodeHome = getConfigDir();
  return [
    ...
    { label: "NEXCODE_HOME", path: nexcodeHome, exists: existsSync(nexcodeHome) },
    { label: "NEXCODE_HOME/config.json", path: getConfigPath(), exists: existsSync(getConfigPath()) },
  ];
}
...
/*
 * [Decision Log]
 * - Purpose: Make `nxc doctor` distinguish the current shell env from the already-running proxy process env.
...
export function formatResponseTempLines(
  ...
  const lines = [
    ...
    "      Reclaim them with: nxc doctor --reclaim-response-temps",
  ];
  ...
  switch (diagnostic.kind) {
    ...
    case "zero-byte":
      return [
        ...
        ...pathLine,
        ...evidenceLines,
        `       Action: stop the NexCode proxy/service, then run nxc doctor ${RECOVER_ZERO_BYTE_COORDINATOR_FLAG} --yes`,
      ];
    ...
    case "changed":
      return ["  --     native-write coordinator changed during diagnosis; re-run nxc doctor", ...pathLine];
    case "unsafe":
...
export function formatServiceMemoryLines(report: ServiceMemoryReport): string[] {
  ...
  if (report.status === "unauthorized") {
    lines.push("  --     local diagnostic capability unavailable — restart the running proxy with this NexCode version");
    return lines;
  }
  ...
  if (d.platform === "win32" && d.eagerRelay?.reason === "auto-known-bad") {
    ...
    // launch-time marker can answer. Telling someone to set NEXCODE_BUN_PATH
    // when it is already set is the bug this branch exists to avoid (#848).
    if (d.bunRuntimeSource === "override") {
      lines.push(`         NEXCODE_BUN_PATH is already active for this service — the override runtime is itself an affected version (unvalidated — own risk).`);
      ...
      lines.push("         this service records no runtime origin (installed before provenance tracking), so NexCode cannot tell whether an override is already active.");
      ...
      lines.push(`         the service is using ${origin}. Options: wait for a bundled runtime update, or set NEXCODE_BUN_PATH to a runtime you trust (unvalidated — own risk),`);
...
export function proxyDownRestartHint(input: {
  ...
  const restart = input.serviceViable
    ? "Restart it with 'nxc service start' (service installed) or 'nxc start'."
    : installedButBroken
      ? "Restart it with 'nxc start', or refresh the installed service: 'nxc service repair'."
      : "Restart it with 'nxc start', or install the persistent service: 'nxc service install'.";
  return `The nxc proxy is not running. Codex/Claude clients pinned to 127.0.0.1:${input.port} fail with errors like "error sending request for url (http://127.0.0.1:${input.port}/v1/responses)". ${restart}`;
}

export interface RunDoctorDeps {
  /** Test/embedding seam: doctor itself must not require live internet to verify local cleanup. */
  probeWhamFn?: () => Promise<WhamProbeResult>;
}

export async function runDoctor(args: string[] = [], deps: RunDoctorDeps = {}): Promise<void> {
  if (args.includes("--fix-codex-runtime")) {
    ...
    console.log("Run nxc sync to refresh the catalog against this runtime.");
    return;
  }

  if (args.includes(RECOVER_ZERO_BYTE_COORDINATOR_FLAG)) {
    if (!args.includes("--yes")) {
      console.log(`Recovery is explicit and creates a same-directory backup. Re-run: nxc doctor ${RECOVER_ZERO_BYTE_COORDINATOR_FLAG} --yes`);
      process.exitCode = 1;
      return;
    }
    ...
    if (live) {
      console.log(`Recovery refused: NexCode proxy pid ${live.pid} is still running. Stop the proxy/service and retry.`);
      process.exitCode = 1;
      return;
    }
    ...
    console.log("Run `nxc sync` to retry Codex config injection. The backup was preserved and no Codex config/catalog file was changed by recovery.");
    process.exitCode = 0;
    return;
  }

  console.log("nexcode doctor\n");
  ...
  console.log("\nCodex runtime selection");
  {
    ...
    if (resolved.newerAvailable) {
      ...
      console.log("       Suggested: set CODEX_CLI_PATH to the desired binary and run nxc sync.");
      console.log("       Optional: nxc doctor --fix-codex-runtime");
    }
    ...
    if (lastClamp && lastClamp.removedEfforts.length > 0) {
      ...
      console.log("       Suggested: set CODEX_CLI_PATH to a newer Codex binary and run nxc sync.");
    }
  }
  ...
  if (runningProxyEnv.status === "not_running") {
    console.log("  --     no running nxc proxy process found");
  ...
  console.log("\nMemory / runtime");
  {
    if (!live) {
      ...
      console.log("  --     no running nxc proxy found (no live pid/runtime record)");
    } else {
  ...
  console.log("\nWHAM reachability");
  const probe = await (deps.probeWhamFn ?? probeWham)();
  ...
  // Design B upgrade visibility: threads still tagged nexcode are invisible to the native
  ...
  const pending = countPendingNexcodeHistory();
  if (pending.failed) {
    ...
    console.log("  ok     no legacy nexcode-tagged threads pending");
  } else {
    console.log(`  --     ${pending.pendingRows} thread(s) still tagged nexcode, ${pending.backupEntries} backup manifest entr${pending.backupEntries === 1 ? "y" : "ies"}`);
  }
  ...
  if (tomlFallbackRoles.length === 0) {
    ...
    console.log("        Codex >= 0.146 rejects that field as unknown and skips the whole role. Move the chains to nexcode config `subagentModelFallbackByModel` (keyed by primary model) and remove the field from the TOML files.");
  }
  ...
  if (catalogState.state === "stale") {
    console.log(`  [WARN] Codex app-server (PID(s): ${catalogState.processes.map(p => p.pid).join(", ")}) started before the on-disk catalog changed; its in-memory model list disagrees with nxc. Action: restart Codex (or run \`nxc sync --restart-codex\`)`);
  ...
  if (!probe.ok) {
    if (probe.classification === "timeout" || probe.classification === "connect_error") {
      ...
      if (noProxy) {
        hints.push("No proxy is visible to this doctor process and config.proxy is unset or unresolved. If Windows uses a proxy/VPN, set config.proxy or start nxc from a shell with HTTP(S)_PROXY.");
      }
    }
  }
  if (pending.failed || pending.pendingRows > 0 || pending.backupEntries > 0) {
    hints.push("Legacy chat threads are still tagged nexcode (or the DB was locked). The running proxy retries the migration automatically; to force it now, close the Codex app and run 'nxc sync'.");
  }
  if (dual.dualInstall && !dual.effectiveIsWindowsMount) {
    hints.push(`Codex is installed on BOTH WSL and Windows. Each side keeps its own ~/.codex (logins, config, catalog are separate); nxc here manages the Linux one. To share a single home, set CODEX_HOME=${dual.windowsCodexHomes[0] ?? `${dual.automountRoot}/c/Users/<you>/.codex`} in WSL (drvfs file locking is less reliable).`);
  ...
  if (dual.interopCodexOnPath) {
    hints.push("The `codex` found on PATH is the Windows launcher reached through WSL interop; nxc will not shim it (a WSL shim breaks Windows invocations). Install codex inside WSL (npm i -g @openai/codex) or run 'nxc ensure' from Windows.");
  }
  if (hints.length > 0) {
    console.log("\nHints");
