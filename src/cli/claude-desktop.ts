...
function printDesktopHelp(): void {
  console.log(`Usage:
  nxc claude desktop [apply] [--static|--hybrid|--discovery-only]
  nxc claude desktop show [--json]
  nxc claude desktop move <provider/model> <opus|fable|sonnet|haiku> [--default]
  nxc claude desktop default <family> <provider/model|none>
...
export async function handleClaudeDesktopCommand(argv: string[], deps: ApplyProfileDeps = {}): Promise<number> {
  ...
  try {
    ...
    const state = await buildClaudeDesktopState(config);
    if (command === "show") {
      if (argv.length > 2 || (argv[1] && argv[1] !== "--json")) throw new Error("Usage: nxc claude desktop show [--json]");
    ...
    if (command === "move") {
      ...
      if (!route || !isFamily(familyRaw) || flags.some(flag => flag !== "--default")) throw new Error("Usage: nxc claude desktop move <route> <family> [--default]");
    ...
    if (command === "default") {
      ...
      if (!isFamily(familyRaw) || !routeRaw || argv.length !== 3) throw new Error("Usage: nxc claude desktop default <family> <route|none>");
    ...
    if (command === "export") {
      const target = argv[1];
      if (!target || argv.length !== 2) throw new Error("Usage: nxc claude desktop export <path|->");
    ...
    if (command === "import") {
      ...
      if (!source || flags.some(flag => flag !== "--apply")) throw new Error("Usage: nxc claude desktop import <path> [--apply]");
