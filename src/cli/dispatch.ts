...
const commandRunners: Record<string, CommandRunner> = {
  ...
  restore: async deps => {
    ...
    if (deps.args[1] === "back") {
      ...
      // lifecycle — the counterpart of `nxc restore`. Start/stop triggers are unchanged;
      // this only re-runs the same inject (config + catalog + history) `nxc start` does.
      const live = await deps.findLiveProxy();
      if (!live) {
        console.error("No running proxy found. Run 'nxc start' — it injects nexcode automatically.");
        return 1;
      }
      ...
      if (!synced.ok) {
        console.error("Plain `codex` was not switched back to nexcode. Fix the reported Codex config issue and retry.");
        return 1;
      }
      const target = collectOrcaCodexHomeDiagnostic();
      console.log(`Plain \`codex\` now routes through nexcode in ${target.effectiveCodexHome} (undo with: nxc restore).`);
      return 0;
    }
    ...
    if (r.success) {
      console.log("Codex integration is OFF and plain `codex` now runs natively. Switch back with: nxc restore back");
    } else {
  ...
  sync: async deps => {
    ...
    // `nxc sync` is a direct CLI path; it does not call the management
  ...
  "codex-shim": async deps => {
    ...
    switch (deps.args[1]) {
      ...
      default:
        console.error("Usage: nxc codex-shim <install|status|uninstall|remove>");
        return 1;
    }
    return 0;
  },
  update: async deps => {
    // `nxc update --help` must print usage and exit WITHOUT side effects — running the
  ...
  route: async deps => {
    if (deps.args[1] !== "combo" && deps.args[1] !== "policy") {
      console.error("Usage: nxc route <combo|policy> <subcommand>");
      return 2;
    }
  ...
  integration: async deps => {
    ...
    if (integration === "grok") {
      ...
      console.error("Usage: nxc integration <claude|grok|client> <subcommand>");
      return 2;
    }
  },
  system: async deps => {
  ...
  claude: async deps => {
    ...
    // "nxc claude desktop" → write Desktop 3P config
