/**
 * `nxc claude [claude args...]` — launch Claude Code wired to the local proxy.
 *
 ...
 * loopback nexcode base URL points at a different proxy port.
 */
 ...
 * nexcode launch. Unproven ambient values fail closed as project dotenv.
 */
export function buildClaudeEnv(
  ...
  // leaving the child with no token at all (audit R2-1). It is nexcode state, never
  ...
  // through `nxc` (the published bin) and genuine shell exports are preserved by proof.
  ...
  delete env.NXC_PRE_BUN_ANTHROPIC_ENV;
  delete env.NXC_NODE_LAUNCH_CONTEXT;
  ...
  if (existingBaseUrl) {
    try {
      ...
      if (parsed.protocol === "http:"
        ...
        console.error(`⚠ Replacing stale nexcode ANTHROPIC_BASE_URL ${parsed.origin} with ${replacement}.`);
  ...
  // env block cannot silently hijack proxy routing away from nexcode.
  ...
  // nxc claude runs — use the top-level settings "model" field or opt out.
  ...
  // so the guard is only safe when nexcode actually owns authentication.
 ...
 */
export async function fetchClaudeContextWindows(config: NxcConfig, port: number, timeoutMs = 3_000): Promise<Record<string, number>> {
  try {
...
async function ensureProxyForClaude(): Promise<number | null> {
  ...
  const child = spawn(process.execPath, selfLaunchArgv(["start", "--port", String(pinPort)]), {
    ...
    stdio: "ignore",
    windowsHide: true,
    env: withProcessRuntimeProvenance({ ...process.env, NXC_SERVICE: "1" }),
  });
  child.unref();
...
export async function cmdClaude(args: string[]): Promise<number> {
  ...
  // Sync roster agents (devlog 070): subagentModels + self -> ~/.claude/agents/nxc-*.md.
  try {
