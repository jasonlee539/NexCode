/** `nxc account` — list and switch provider credentials (issue #180). */
...
const ACCOUNT_USAGE = `Usage:
  ...
  nxc account alias <provider> <account-or-key-id> <display-name|-> [--json]
  nxc account priority <provider> <account-id|main> [<-100..100|first|earlier|normal|later|last|reset>] [--json]
  nxc account remove <provider> <account-or-key-id|main> --yes [--json]
  nxc account clear-cooldown <provider> <account-id|main> [--json]
  nxc account add-key <provider> [--label <label>] [--json]
  nxc account import <provider> --format <format> (--file <path>|--stdin) [--json]
  nxc account login <provider> [--id <account-id>] [--reauth] [--code -] [--no-wait] [--json]
  nxc account code <provider> [--flow <flow-id>] [--json]   (reads the code from stdin)
  nxc account cancel <provider> [--flow <flow-id>] [--json]
  nxc account reset-credits <account-id|main> [--consume --yes] [--json]
  nxc account main <doctor|list|register|add|switch|recover> ...
...
function candidateNames(config: NxcConfig): string {
