...
const USAGE = `Usage:
  nxc observe logs [--provider <name>] [--model <id>] [--status <code>]
      [--limit <n>] [--follow] [--json|--jsonl]
  nxc logs explain <request-id> [--json]
  nxc logs rebuild-index
  nxc logs index-status
  nxc observe usage [--range <7d|30d|all>] [--surface <all|codex|claude|grok>] [--json]
  nxc observe storage [codex-logs [status|protect|unprotect|repair|compact] [--mode <compat|quiet>]] [--json]
  nxc observe memory [--json]
  nxc observe debug [--json]
  nxc observe claude-inbound [--limit <n>] [--json]
  nxc observe injection [--limit <n>] [--json]`;
