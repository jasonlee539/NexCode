 ...
 *
 * `nxc mcode` uses the managed `custom_provider.nexcode` block written by the
 * existing file-integration subsystem. `nxc mmx` is intentionally text-only:
 ...
 * NexCode does not claim to implement.
 */
...
export function buildMmxEnv(
  ...
  // is present and does not apply NO_PROXY. Its NexCode destination is always
 ...
 * NexCode already exposes the canonical Anthropic data plane at
...
export function startMmxTextBridge(
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
      ...
      // The bridge is loopback-only and NexCode does not require a real key
      ...
      headers.delete("x-nexcode-api-key");
      ...
      try {
        ...
        return Response.json({
          type: "error",
          error: { type: "api_error", message: "NexCode proxy unavailable" },
        }, { status: 502 });
      } finally {
...
export function mcodeNexCodeBaseUrl(text: string): string | null {
  try {
    const parsed = Bun.YAML.parse(text) as {
      custom_provider?: { nexcode?: { options?: { baseURL?: unknown } } };
    };
    const baseURL = parsed?.custom_provider?.nexcode?.options?.baseURL;
...
async function ensureProxy(config: NxcConfig): Promise<LiveProxy | null> {
...
export function installMmxTerminationHandlers(
  ...
  const onTerminationSignal = (signal: "SIGINT" | "SIGTERM") => {
    ...
    // forwarded by bin/nxc.mjs. Keep the listener installed and coalesce the
...
export async function cmdMcode(args: string[]): Promise<number> {
  ...
  if (!isLoopbackHostname(config.hostname)) {
    console.error("❌ MiniMax Code integration is loopback-only; its config cannot carry NexCode's dedicated remote-admission header.");
    return 2;
  }
  ...
  try {
    configuredBase = mcodeNexCodeBaseUrl(readFileSync(mcodeConfigPath(process.env), "utf8"));
  } catch (error) {
  ...
  if (!configuredBase) {
    console.error("❌ MiniMax Code is not connected. Run: nxc integration client enable --client mcode");
    return 2;
  }
  ...
  if (normalizedMcodeBaseUrl(configuredBase) !== normalizedMcodeBaseUrl(expected)) {
    console.error("❌ MiniMax Code's NexCode provider points at a stale proxy address. Re-run: nxc integration client enable --client mcode");
    return 2;
  }
  console.error(`✅ MiniMax Code wired to ${expected}; select custom_provider:nexcode/<model> in MCode.`);
...
export async function cmdMmx(args: string[]): Promise<number> {
  ...
  if (unsafe) {
    console.error(`❌ ${unsafe} is not accepted by nxc mmx because it could bypass the proxy or expose a caller credential.`);
    return 2;
  }
  const commandPath = mmxCommandPath(args);
  if (commandPath[0] !== "text") {
    console.error("❌ nxc mmx supports only `mmx text` commands. Use plain `mmx` for MiniMax image, video, speech, music, vision, search, quota, auth, config, file, and update APIs.");
    return 2;
  }
  const config = loadConfig();
  if (!isLoopbackHostname(config.hostname)) {
    console.error("❌ nxc mmx is loopback-only; MMX has no field for NexCode's dedicated remote-admission header.");
    return 2;
  }
  const live = await ensureProxy(config);
  if (!live) {
  ...
  const configDir = mkdtempSync(join(tmpdir(), "nexcode-mmx-"));
