/**
 * `nxc opencode [opencode args...]` — launch opencode wired to the local proxy.
 *
 * Mirrors `nxc claude` (src/cli/claude.ts): ensure the proxy is running, then exec the
 ...
 * global/project config to detect an existing `provider.nexcode` override, then injects
 ...
 */
export function opencodeLaunchNativeSlugs(config: NxcConfig): string[] {
...
/** Default deadline for authenticated GET /api/models during `nxc opencode` launch. */
...
export async function fetchOpencodeProxyModels(
  ...
  if (token) headers.set("X-NexCode-API-Key", token);
...
/**
 * Merge inherited `OPENCODE_CONFIG_CONTENT` and override only `provider.nexcode`.
...
function serviceTokenLookupEnv(env: OpencodeLaunchEnv): OpencodeLaunchEnv {
  if (env.NXC_API_TOKEN_FILE?.trim()) return env;
  return { ...env, NXC_API_TOKEN_FILE: serviceApiTokenFilePath() };
}

/**
 * Child env for a detached `nxc start` from `nxc opencode`. When the admission token is
 * not already in the environment, pass through an existing `NXC_API_TOKEN_FILE` or the
...
export function opencodeProxyStartEnv(base: OpencodeLaunchEnv = process.env): OpencodeLaunchEnv {
  const withTokenFile = base.NEXCODE_API_AUTH_TOKEN?.trim()
    ? base
    : serviceTokenLookupEnv(base);
  return { ...withTokenFile, NXC_SERVICE: "1" };
}

/**
 * Env assembly (unit-tested). Inherited inline config is merged and only
 * `provider.nexcode` is replaced; disk config layers stay untouched. The admission
 ...
 */
export function opencodeApiKey(config: NxcConfig, env: OpencodeLaunchEnv = process.env): string {
  const envToken = env.NEXCODE_API_AUTH_TOKEN?.trim();
  if (envToken) return envToken;
  const serviceToken = loadServiceTokenFromFile(serviceTokenLookupEnv(env));
  if (serviceToken) return serviceToken;
  return config.apiKeys?.[0]?.key || "nxc";
}

async function ensureProxyForOpencode(config: NxcConfig): Promise<LiveProxy | null> {
...
export async function cmdOpencode(args: string[]): Promise<number> {
  ...
  if (providerOverride) {
    console.error(`ℹ ${providerOverride} also defines provider.${OPENCODE_PROVIDER_ID}; the runtime layer from nxc opencode overrides it for this launch.`);
  }
