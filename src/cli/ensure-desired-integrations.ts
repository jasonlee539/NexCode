/**
 * Align Grok and Claude Desktop files with the durable switches during `nxc ensure`.
 *
...
export function grokSyncFailureMessage(err: unknown): string {
  ...
  return `Grok Build config sync failed: ${detail}. `
    ...
    + "run 'nxc ensure' (or apply from the dashboard's Grok page) to repoint it.";
}
 ...
 *
 * `handleStart` already gates inject on `shouldSyncGrokOnStart`. `nxc ensure`
