...
const EXTENDED_USAGE = `Usage:
  ...
  nxc account alias <provider> <id|main> <display-name|-> [--json]
  nxc account priority <provider> <id|main> [<-100..100|first|earlier|normal|later|last|reset>] [--json]
  nxc account remove <provider> <id|main> --yes [--json]
  nxc account clear-cooldown <provider> <id|main> [--json]
...
const PIPE_GUIDANCE = `Pipe the API key on stdin, for example:
  nxc account add-key <provider> <<< "$MY_KEY"
  security find-generic-password -w <item> | nxc account add-key <provider>`;
...
export async function cmdRemove(args: string[], deps: AccountDeps): Promise<number> {
  ...
  if (!confirmed) {
    const message = `Confirmation required. Re-run: nxc account remove ${name} ${requestedId} --yes`;
  ...
  const baseUrl = await resolveBaseUrl(deps);
  if (!baseUrl) return fail("Proxy not reachable. Start it with 'nxc start' or 'nxc ensure'.");
  const before = await fetchRows(deps, baseUrl, name, classified.type);
  if (before.networkDown) return fail("Proxy not reachable. Start it with 'nxc start' or 'nxc ensure'.");
  ...
  if (response.status === 0) return fail("Proxy not reachable. Start it with 'nxc start' or 'nxc ensure'.");
