/**
 * `nxc v2 status|on|off` — toggle/report the codex `multi_agent_v2` feature that
...
export async function cmdV2(args: string[], deps: V2CliDeps = {}, findPort?: () => Promise<number | undefined>): Promise<number> {
  ...
  if (verb === "mode") {
    ...
    try {
      ...
      log.error(`catalog resync failed: ${err instanceof Error ? err.message : String(err)} — run 'nxc sync' manually.`);
      return 1;
    }
  ...
  try {
    ...
    log.error(`catalog resync failed (flag IS flipped): ${err instanceof Error ? err.message : String(err)} — run 'nxc sync' manually.`);
    return 1;
  }
