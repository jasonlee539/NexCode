...
const USAGE = `Usage:
  nxc models live [--provider <name>] [--json]
  nxc models edit <custom-id> [--model-id <id>] [--display-name <name|->]
  ...
  nxc models <enable|disable> <provider/model|native-model> [--native] [--json]
  nxc models provider <name> <on|off> [--json]
  nxc models selected <provider> [--set <id,id...>|--clear] [--json]
  nxc models context <status|value <tokens> [--set-all]|provider <name> on [--value <tokens>]|provider <name> off|all <on|off>> [--json]
  nxc models shadow <status|set> [model|-] [--enabled <on|off>] [--json]`;
...
async function context(argv: string[], deps: RuntimeApiDeps): Promise<void> {
  ...
  if (action === "value") {
    ...
    // Optional explicit cap value for this provider only (`nxc models context provider
