...
const CLAUDE_USAGE = `Usage:
  nxc claude config [status] [--json]
  nxc claude config set [--enabled <on|off>] [--auth-mode <auto|proxy|subscription>]
...
const GROK_USAGE = `Usage:
  nxc grok [status] [--json]
  nxc grok <exclude|include|set> <model,model...> [--json]
  nxc grok clear [--json]
  nxc grok apply [--json]`;

const CLIENT_USAGE = `Usage:
  nxc integration client [status] [--client <id>] [--json]
  nxc integration client <enable|disable> --client <id> [--json]
  nxc integration client history [--client <id>] [--json]
  nxc integration client restore --op <opId> [--confirm-drift] [--json]`;
...
const ZCODE_USAGE = `Usage:
  nxc zcode [status] [--json]
  nxc zcode <enable|disable> [--json]
  nxc zcode history [--json]
  nxc zcode restore --op <opId> [--confirm-drift] [--json]`;
 ...
 * `nxc mcode` there is no exec step: connecting the managed provider block is
...
export async function handleZcodeCommand(argv: string[], deps: RuntimeApiDeps = {}): Promise<number> {
  const args = [...argv];
  // Find the first non-flag token so `nxc zcode --json enable` still enables.
