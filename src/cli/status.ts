...
import { findLiveProxy, isNexcodeHealthz, probeHostname } from "../server/proxy-liveness";
...
export function selectListenTarget(
  config: Pick<NxcConfig, "port" | "hostname">,
  pid: number | null,
...
async function checkProxyHealth(target: ListenTarget): Promise<HealthCheck> {
  ...
  try {
    ...
    if (!isNexcodeHealthz(body)) {
      const message = "responded, but not an nexcode proxy";
      return { ok: false, url, message, label: `${url} ${message}` };
    }
...
export async function collectStatus(): Promise<CliStatusView> {
  ...
  // Prefer identity-verified liveness (runtime-port + /healthz) over nxc.pid alone (#618).
  ...
  const serviceSummary = service.installed && !live
    ? `${service.summary} — registered but NOT serving; see ${serviceLogPath()} and re-run 'nxc service repair'`
    : service.summary;
  ...
  if (
    ...
    warningParts.push(
      `Preferred Codex runtime is unavailable; using ${displayCodexRuntimePath(resolvedRuntime.runtime.command)} instead. Run nxc doctor for diagnosis and recovery.`,
    );
  } else if (
    ...
    warningParts.push(
      detail
        ? `No validated Codex runtime found (${detail}); falling back to \`codex\`. Run nxc doctor for diagnosis and recovery.`
        : "No validated Codex runtime found; falling back to `codex`. Run nxc doctor for diagnosis and recovery.",
    );
  }
  if (resolvedRuntime.newerAvailable) {
    warningParts.push("NexCode is using an older Codex binary. Run nxc doctor for diagnosis and recovery.");
  }
  if (clampActive) {
    warningParts.push(
      `Catalog clamp removed: ${lastClamp!.removedEfforts.join(", ")}. Run nxc doctor for diagnosis and recovery.`,
    );
  }
  ...
  const grokDrift = (() => {
    try {
      ...
      return null; // reading grok's config must never break `nxc status`
    }
  })();
  if (grokDrift) {
    warningParts.push(
      `Grok Build config points at port ${grokDrift.fencePort}, but the proxy is on `
      + `${grokDrift.livePort}; grok turns will retry against a closed port. Run 'nxc ensure' to repoint it.`,
    );
  }
  const codexRuntime = {
