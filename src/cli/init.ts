...
/**
 * The full CLI provider menu, derived from the canonical provider registry so `nxc init`,
...
const KIND_HEADING: Record<InitKind, string> = {
  ...
  oauth: "Account login (OAuth — then run: nxc login <id>)",
...
export async function runInit(): Promise<void> {
  ...
  try {
    ...
    let providerConfig: NxcProviderConfig;
    let oauthHint = false;
    ...
    // installation would make the next `nxc start` crash on a stale-backup
    ...
    if (/stdin (closed|reached EOF)/i.test(message)) {
      console.error(`\n❌ ${message}. Re-run \`nxc init\` in an interactive terminal.`);
      process.exitCode = 1;
      return;
    }
    throw error;
