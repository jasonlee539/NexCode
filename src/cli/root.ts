/**
 * CLI head: version/help early exits, `nxc ready` pre-parse, and the bounded
...
export function parseCliHead(argv: string[]): CliHead {
  ...
  if (command === undefined || command === "help" || command === "--help" || command === "-h") {
    // `nxc help <sub>` carries the subcommand; bare/flag help prints the full usage.
    return {
      kind: "help",
      command,
      args,
  ...
  if (command !== "help" && hasHelpFlag(args.slice(1))) {
    // `nxc <cmd> --help|-h|help` prints that command's usage, not the full list.
    return { kind: "help", command, args, helpTarget: command };
  }
  // P1: pre-parse `nxc ready` and reject invalid arguments with exit 64 BEFORE
...
export async function runCli(argv: string[]): Promise<CliHead> {
  ...
  switch (head.kind) {
    ...
    case "ready": {
      ...
      if (!head.readyArgs) {
        console.error("Usage: nxc ready [--json] [--wait [--timeout <seconds>]]");
