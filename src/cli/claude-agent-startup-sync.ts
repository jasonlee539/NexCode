 ...
 * stale `nxc-*.md` files. A small forwarding gate delays only the successful transition;
 ...
 * context-window map used by `nxc claude`; failure keeps startup available and falls back to an
...
export async function syncClaudeAgentDefsAtProxyStartup(
  ...
  try {
    ...
    try {
      ...
      // Startup remains best-effort. The next management mutation or `nxc claude` launch can
