/**
 * `nxc provider` subcommand — non-interactive provider management.
 *
 * Subcommands:
...
function handleList(args: string[]): void {
  ...
  rejectUnknownArgs(args, "Usage: nxc provider list [--json]");
  ...
  if (available.length > 0) {
    ...
    console.log(`\nAdd with: nxc provider add <name> [--api-key <key>]`);
  }
}
...
const ADD_USAGE = "Usage: nxc provider add <name> [--adapter <adapter>] [--base-url <url>] [--api-key <key>] [--api-key-transport <x-api-key|bearer>] [--default-model <model>] [--allow-private-network] [--set-default] [--force] [--json] [--sync]";
...
async function handleAdd(args: string[]): Promise<void> {
  ...
  let provConfig: NxcProviderConfig;
  const registryEntry = getProviderRegistryEntry(name);
  ...
  if (registryEntry) {
    ...
    if (apiKey) {
      if (registryEntry.authKind === "forward") {
        ...
        console.warn(`Warning: provider "${name}" uses OAuth auth; --api-key is ignored. Run: nxc login ${name}`);
      } else {
    ...
    if (!adapter || !baseUrl) {
      ...
      console.error("Usage: nxc provider add <name> --adapter <adapter> --base-url <url> [--api-key <key>]");
      process.exit(1);
    }
    provConfig = {
      adapter,
...
function handleRemove(args: string[]): void {
  ...
  if (!name || name.startsWith("-")) {
    console.error("Usage: nxc provider remove <name> [--json]");
    process.exit(1);
  }
  rejectUnknownArgs(restArgs.slice(1), "Usage: nxc provider remove <name> [--json]");
  ...
  if (name === config.defaultProvider) {
    console.error(`Cannot remove "${name}" — it is the default provider. Change the default first: nxc provider set-default <other>`);
    process.exit(1);
  }
...
function handleShow(args: string[]): void {
  ...
  const name = restArgs[0];
  if (!name || name.startsWith("-")) {
    console.error("Usage: nxc provider show <name> [--json]");
    process.exit(1);
  }
  rejectUnknownArgs(restArgs.slice(1), "Usage: nxc provider show <name> [--json]");
...
function handleSetDefault(args: string[]): void {
  ...
  const name = restArgs[0];
  if (!name || name.startsWith("-")) {
    console.error("Usage: nxc provider set-default <name> [--json]");
    process.exit(1);
  }
  rejectUnknownArgs(restArgs.slice(1), "Usage: nxc provider set-default <name> [--json]");

  const config = loadConfig();
  if (!hasOwnProvider(config.providers, name)) {
    console.error(`Provider "${name}" is not configured. Add it first: nxc provider add ${name}`);
    process.exit(1);
  }
...
const PROVIDER_USAGE = `Usage: nxc provider <subcommand>
...
Examples:
  nxc provider list
  nxc provider add anthropic --api-key sk-ant-...
  nxc provider add my-ollama --adapter openai-chat --base-url http://localhost:11434/v1
  nxc provider show anthropic --json
  nxc provider set-default anthropic
  nxc provider remove my-ollama`;
