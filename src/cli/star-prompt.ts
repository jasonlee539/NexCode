...
const REPO = "lidge-jun/nexcode";
/** Fires exactly once from the first interactive `nxc start`. */
const MARKER = ".star-prompted";
/**
 * Bounds the agent-facing deferral (issue #879): the relay fires at most once
 * per nexcode version, and never more than once per week while the version
...
/**
 * Shown instead of the prompt when an agent is driving `nxc start`. The agent
...
function printAgentDeferral(): void {
  ...
  // one ever is.
    console.log(
    `\n  \x1b[2m⭐ nexcode isn't starred on GitHub yet — agent-driven session ` +
    ...
    `       replies — this deferral fires at most once per nexcode version, and\n` +
    ...
    `\n` +
    `  Full rule: AGENTS_INSTALL.md in the nexcode package.\n` +
    `  </details>\x1b[0m\n`,
  );
}

/**
 * First interactive `nxc start`: a one-time "star on GitHub?" question with an
...
export async function maybeShowStarPrompt(): Promise<void> {
  try {
    if (process.env.NXC_SERVICE || !process.stdin.isTTY || !process.stdout.isTTY) return;
    ...
    const yes = await ask({
      question: "\n  \x1b[38;5;141m⭐ Enjoying nexcode? Star it on GitHub (via gh)?\x1b[0m",
      defaultYes: true,
    });
    if (!yes) return;
    const r = starRepo();
