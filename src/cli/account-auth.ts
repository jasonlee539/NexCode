...
visible to anyone who can run ps:
  pbpaste | nxc account code <provider> --flow <flow-id>
  nxc account login <provider> --code -   (same, for the login flow)`;
...
async function login(argv: string[], deps: RuntimeApiDeps): Promise<void> {
  ...
  // Only resolve when --code was actually given: a plain `nxc account login`
...
async function code(argv: string[], deps: RuntimeApiDeps): Promise<void> {
  ...
  // `nxc account code openai --flow f1` read `--flow` as the code and then
