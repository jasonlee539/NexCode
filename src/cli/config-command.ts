...
const USAGE = `Usage:
  nxc config [show] [--json] [--source]
  nxc config get <dot.path> [--json]
  nxc config set <dot.path> <json-or-string> [--json]
  nxc config unset <dot.path> [--json]
  nxc config validate [path|-] [--json]
  nxc config export <path|->
  nxc config import <path|-> --yes [--json]`;
...
function normalizeVisionConfig(config: NxcConfig): NxcConfig {
...
function validate(value: unknown): NxcConfig {
  ...
  return runCliAction(async () => {
    ...
    if (action === "set" || action === "unset") {
      ...
      const outcome = mutatePersistedConfig(fresh => {
        ...
        // `nxc account priority` or the management route, so it releases the manual pin
    ...
    if (action === "import") {
      ...
      printData({ ok: true, source: path }, wantsJson, [`Imported config from ${path}. Restart or run nxc sync if needed.`]);
      return;
    }
