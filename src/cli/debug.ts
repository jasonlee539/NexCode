...
function printScopeStatus(scope: DebugScope, view: DebugSettingsView): void {
  if (scope === "provider") {
    ...
    console.log("  Tail: nxc debug provider logs [-f]");
  } else if (scope === "usage") {
    ...
    console.log("  View: nxc observe claude-inbound");
  }
}

function envDebugEnabled(): boolean {
  return process.env.NXC_DEBUG === "1"
...
async function printUsageLogs(follow: boolean): Promise<void> {
  ...
  try {
    ...
    const entries = await res.json() as { seq: number; line: string }[];
    for (const entry of entries) console.log(entry.line);
    if (entries.length === 0) console.log("(empty — enable with: nxc debug usage on)");
    if (entries.length > 0) after = entries[entries.length - 1]!.seq;
  } catch (err) {
...
async function handleScopeCommand(scope: DebugScope, actionArgv: string[]): Promise<void> {
  ...
  if (action === "logs") {
    if (scope === "injection" || scope === "claude") {
      console.error(scope === "claude"
        ? "Use: nxc observe claude-inbound"
        : "Injection debug has no buffered log stream; use: nxc observe injection");
      process.exit(1);
    }
  ...
  console.error(scope === "injection" || scope === "claude"
    ? `Usage: nxc debug ${scope} on|off|status|reset`
    : `Usage: nxc debug ${scope} on|off|status|reset|logs [-f]`);
  process.exit(1);
}
...
function printTopLevelHelp(): void {
  ...
  console.log("  nxc debug injection on|off|status|reset");
  console.log("  nxc debug claude on|off|status|reset");
  console.log("");
  console.log("Env defaults on start:");
  console.log("  provider → NXC_DEBUG=1 (legacy NXC_DEBUG_FRAMES still works)");
