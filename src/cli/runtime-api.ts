 ...
 * tokens: mistyping `nxc account cancel <p> --code <secret>` on a command that
...
export interface RejectArgsOptions {
   ...
   * positional is plausibly the credential itself — `nxc account code <p>`
...
export async function readSecretLine(deps: RuntimeApiDeps, label: string): Promise<string> {
  ...
  // something-else | nxc account code <p>` reaches here that way.
