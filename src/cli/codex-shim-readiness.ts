  ...
  if (inputs.routingKind === "unknown") {
    warnings.push(
      inputs.externalProvider
        ? `Codex still selects ${provider}. The shim can start NexCode, but it does not redirect that provider; point it at the live NexCode /v1 endpoint with wire_api = "responses", or switch to the built-in openai provider and run 'nxc sync'.`
        : "Codex routing could not be verified. The shim can start NexCode, but it may not redirect Codex; run 'nxc doctor' before relying on autostart.",
    );
  } else if (inputs.routingKind === "custom-local") {
    warnings.push(
      `Codex uses ${provider} through a user-owned local gateway. The shim can start NexCode, but NexCode does not own that route; run 'nxc doctor' to verify its lifecycle.`,
    );
  } else if (inputs.routingKind === "custom-remote") {
    warnings.push(
      `Codex uses ${provider} through a remote gateway. The shim only starts a local NexCode proxy and will not affect those requests.`,
    );
  }
