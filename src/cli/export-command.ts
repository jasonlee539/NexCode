/**
 * `nxc export --client <id>` — print a client config for the live proxy.
 *
 ...
 *   so `nxc export --client pi --json > models.json` is safe to pipe. This is JSON for
...
const USAGE = `Usage:
  nxc export --client <${EXPORT_CLIENT_IDS.join("|")}> [--json] [--out <path>] [--force]`;
 ...
 * already throws the "Start it with: nxc start" error when nothing answers — so a config
