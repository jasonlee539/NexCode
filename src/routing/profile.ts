...
export const DEFAULT_UNKNOWN_EVIDENCE: Record<"capability" | "health" | "quota" | "cost", NxcRoutingUnknownEvidenceMode> = {
  capability: "exclude",
  health: "penalize",
  quota: "penalize",
  cost: "penalize",
...
export const DEFAULT_DEGRADED_EVIDENCE: NxcRoutingUnknownEvidenceMode = "penalize";
export const DEFAULT_COMPATIBILITY_UNKNOWN_EVIDENCE: NxcRoutingUnknownEvidenceMode = "exclude";
...
export interface NormalizedRoutingProfile {
  ...
  limits: { maxEstimatedCostUsd?: number; onUnknownCost?: NxcRoutingUnknownCostCapMode };
  unknownEvidence: Record<"capability" | "health" | "quota" | "cost", NxcRoutingUnknownEvidenceMode>;
...
export function resolvePolicyProfileId(
  config: { routingProfiles?: Record<string, NxcRoutingProfileConfig> },
  modelId: string,
): string | null {
...
function aliasIssues(
  id: string,
  alias: string,
  config: Pick<NxcConfig, "providers" | "combos" | "routingProfiles" | "codexAccountNamespaces">,
...
function normalizedRequirements(raw: NxcRoutingProfileConfig): NormalizedRoutingProfileRequirements {
...
function normalizedUnknownEvidence(raw: NxcRoutingProfileConfig): NormalizedRoutingProfile["unknownEvidence"] {
...
export function normalizeRoutingProfile(id: string, raw: NxcRoutingProfileConfig): NormalizedRoutingProfile {
  const alias = typeof raw.alias === "string" ? raw.alias.trim() : "";
...
export function listRoutingProfileIds(config: { routingProfiles?: Record<string, NxcRoutingProfileConfig> }): string[] {
