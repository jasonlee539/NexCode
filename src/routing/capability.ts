...
import { modelInList, type NxcConfig } from "../types";
...
function cachedCatalogModels(): CatalogModelRow[] {
  try {
    ...
    // Read ONLY `nexcode_capability_provenance` (written by
    ...
    const rows = models.flatMap((model): CatalogModelRow[] => {
      ...
      const provenance = (model as Record<string, unknown>).nexcode_capability_provenance;
