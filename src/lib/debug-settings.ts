 ...
 * Provider debug: `nxc debug provider on|off|status|reset|logs [-f]` (or NXC_DEBUG=1 on start).
 * Usage capture: `nxc debug usage on|off|status|reset|logs [-f]` (or NEXCODE_USAGE_DEBUG=1).
 * Injection log: `nxc debug injection on|off|status|reset` (or NXC_INJECTION_DEBUG=1) —
 * multi-agent guidance-injection console lines, default OFF.
 * Claude inbound capture: `nxc debug claude on|off|status|reset` (or NXC_CLAUDE_DEBUG=1) —
 * allowlist-scalar ring of inbound Anthropic request metadata, default OFF.
 * `/api/debug` and `nxc debug` override env defaults without restart.
 */

export const DEBUG_ENV = {
  debug: "NXC_DEBUG",
  usage: "NEXCODE_USAGE_DEBUG",
  injection: "NXC_INJECTION_DEBUG",
  claude: "NXC_CLAUDE_DEBUG",
} as const;

/** Legacy env var that still enables provider debug logging. */
const LEGACY_DEBUG_ENV = ["NXC_DEBUG_FRAMES"] as const;
