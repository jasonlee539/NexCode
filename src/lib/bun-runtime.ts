 ...
 *
 * nexcode ships the Bun runtime via the `bun` npm dependency (esbuild-style:
 ...
 * launcher (bin/nxc.mjs) and the durable service/shim integrations both need a
 ...
 */
export const BUN_RUNTIME_SOURCE_ENV = "NXC_BUN_RUNTIME_SOURCE";
 ...
 */
export const BUN_RUNTIME_PATH_ENV = "NXC_BUN_RUNTIME_PATH";
...
export function reportedBunRuntimeSource(
  ...
  // reported (a bare NXC_BUN_RUNTIME_SOURCE inherited from an unrelated
 ...
 * be silently dropped on `nxc ensure`, GUI start, restart, and update-relaunch, and the
 ...
 * shell nor its `NEXCODE_BUN_PATH`, and re-deriving would demote a correct `override`
...
export function durableBunRuntime(): DurableBunRuntime {
  ...
  // running executable. Re-reading NEXCODE_BUN_PATH here would let a project
 ...
 * npm global prefix and survives across `nxc update` — and fall back to the
