...
export const WINDOWS_SCHTASKS_CREATE_ACCESS_DENIED_MARKER =
  "NXC_ERROR_CODE=WINDOWS_SCHTASKS_CREATE_ACCESS_DENIED";

/**
 * Reserved elevated-scheduler protocol exit codes.
 * These are NexCode-owned values returned by the elevated PowerShell transaction.
 ...
 */
export const NXC_ELEVATED_SUCCESS = 0;
export const NXC_ELEVATED_CREATE_FAILED = 10;
export const NXC_ELEVATED_RUN_FAILED_ROLLED_BACK = 11;
export const NXC_ELEVATED_RUN_FAILED_ROLLBACK_FAILED = 12;
export const NXC_ELEVATED_PROTOCOL_FAILED = 13;
/** Windows ERROR_CANCELLED — reserved for UAC denial; never emitted by the elevated script. */
export const NXC_ELEVATED_UAC_CANCELLED = 1223;

export const NXC_ELEVATED_PROTOCOL_CODES = [
...
function isOwnedSchedulerCreate(args: string[]): boolean {
  ...
  return normalized[0] === "/create"
    && normalized[taskIndex + 1] === "nexcode-proxy"
    && taskIndex > 0
    && xmlIndex > 0
...
export function formatWindowsSchtasksError(error: unknown, args: string[]): string {
  ...
  const guidance = [
    ...
    "Approve the Windows UAC prompt to install the background service, or run `nxc service install` from an elevated PowerShell window.",
  ].join(" ");
...
export function classifyElevatedSchedulerExitCode(exitCode: number): ElevatedSchedulerOutcome {
  if (exitCode === NXC_ELEVATED_SUCCESS) return "success";
  if (exitCode === NXC_ELEVATED_CREATE_FAILED) return "create-failed";
  if (exitCode === NXC_ELEVATED_RUN_FAILED_ROLLED_BACK) return "run-failed-rolled-back";
  if (exitCode === NXC_ELEVATED_RUN_FAILED_ROLLBACK_FAILED) return "run-failed-rollback-failed";
  ...
  const completion = new Promise<WindowsElevationResult>((resolve, reject) => {
    ...
    child.once("close", (code, signal) => {
      ...
      if (typeof code === "number") {
        ...
        if (
          code === NXC_ELEVATED_UAC_CANCELLED
...
export function runWindowsElevated(file: string, args: string[]): Promise<number> {
  ...
  const script = [
    ...
    `if ($null -eq $p) { exit ${NXC_ELEVATED_UAC_CANCELLED} }`,
    "$null = $p.Handle;",
    // Missing ExitCode is a protocol failure for single-file elevation (not success).
    `if ($null -eq $p.ExitCode) { exit ${NXC_ELEVATED_PROTOCOL_FAILED} }`,
    "exit $p.ExitCode",
  ].join("");
 ...
 * Returns only NexCode protocol exit codes (never raw schtasks codes, never 1223).
...
export function buildElevatedSchtasksCreateAndRunScript(
  ...
  return [
    ...
    "function Invoke-NxcSchtasks([string]$ArgList) {",
    ...
    "}",
    `$createCode = Invoke-NxcSchtasks ${psSingleQuote(createList)}`,
    `if ($createCode -ne 0) { exit ${NXC_ELEVATED_CREATE_FAILED} }`,
    `$runCode = Invoke-NxcSchtasks ${psSingleQuote(runList)}`,
    `if ($runCode -eq 0) { exit ${NXC_ELEVATED_SUCCESS} }`,
    `$deleteCode = Invoke-NxcSchtasks ${psSingleQuote(deleteList)}`,
    `if ($deleteCode -eq 0) { exit ${NXC_ELEVATED_RUN_FAILED_ROLLED_BACK} }`,
    `exit ${NXC_ELEVATED_RUN_FAILED_ROLLBACK_FAILED}`,
  ].join("; ");
}
