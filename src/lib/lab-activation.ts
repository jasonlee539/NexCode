...
interface LabActivationRecord {
  ...
  seenRuntimeConfigs: WeakSet<NxcConfig>;
}
...
export function labActivationRequired(config: NxcConfig, configDir?: string): boolean {
 ...
 */
export function activateLab(config: NxcConfig, configDir?: string): void {
  ...
  const record: LabActivationRecord = {
    staticDetach: [],
    runtime: null,
    seenRuntimeConfigs: new WeakSet<NxcConfig>(),
  };
