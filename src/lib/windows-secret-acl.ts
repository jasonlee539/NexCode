 ...
 * into multi-minute startup stalls). Override with NEXCODE_ACL_TIMEOUT_MS
...
function resolveHardenDeadlineMs(): number {
  const raw = env["NEXCODE_ACL_TIMEOUT_MS"]?.trim();
...
function hardenEntry(
  ...
  if (isTimeoutError(lastErr)) {
    ...
    console.warn(`[nexcode] ${annotated} — continuing without NTFS ACL harden`);
