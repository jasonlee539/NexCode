/**
 * Fail-closed protection for the user's REAL NexCode home while tests run.
 *
 ...
 * ten-provider `~/.nexcode/config.json` with an 874-byte fixture on a real machine.
 ...
 *    like NODE_ENV would brick `NODE_ENV=test nxc ...` for a user who did nothing
 *    wrong — worse than the bug it prevents. Arming requires NXC_TEST_HOME_GUARD=1,
...
const GUARD_ENV = "NXC_TEST_HOME_GUARD";
/**
 ...
 */
const REAL_HOME_ENV = "NXC_REAL_HOME";
...
const PROTECTED_HOME = canonicalize(
  join(process.env[REAL_HOME_ENV]?.trim() || homedir(), ".nexcode"),
);
...
/**
 * Throw when an armed test process is about to write the real NexCode home.
 *
...
export function assertNotRealHomeUnderTest(dir: string): void {
  ...
  throw new Error(
    `refusing to write the real NexCode home (${PROTECTED_HOME}) from a test process. `
    + "Point NEXCODE_HOME at a temp directory for this test, or inject persistence "
