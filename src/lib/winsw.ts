/**
 * WinSW-backed native Windows service (opt-in via `nxc service install --native`).
 *
...
/** SCM service id — distinct from the Task Scheduler task name (nexcode-proxy). */
export const WINSW_SERVICE_ID = "nexcode-proxy-native";
 ...
 * NXC_API_TOKEN_FILE at startup (cli handleStart). PATH is baked for parity with the
...
export function buildWinswXml(entry: WinswEntry, env: NodeJS.ProcessEnv = process.env, port?: number): string {
  ...
  const listenPort = (() => {
    ...
    const baked = env.NXC_BAKE_PORT?.trim();
  ...
  // - NEXCODE_HOME so file-backed admin auth (`admin-api-token`) resolves
  // - NEXCODE_ACL_TIMEOUT_MS when set (not a secret)
  // Never embed NEXCODE_ADMIN_AUTH_TOKEN or NEXCODE_API_AUTH_TOKEN values in XML —
  // those stay file-pointer / generated-file only (uninstall retains the XML).
  const aclTimeout = env.NEXCODE_ACL_TIMEOUT_MS?.trim();
  const envLines = [
    `  <env name="NXC_SERVICE" value="1"/>`,
    ...
    `  <env name="NXC_API_TOKEN_FILE" value="${xmlEscape(serviceApiTokenFilePath())}"/>`,
    ...
    `  <env name="NEXCODE_HOME" value="${xmlEscape(getConfigDir())}"/>`,
    aclTimeout ? `  <env name="NEXCODE_ACL_TIMEOUT_MS" value="${xmlEscape(aclTimeout)}"/>` : null,
  ].filter((line): line is string => Boolean(line));
  return `<?xml version="1.0" encoding="UTF-8"?>
<service>
  <id>${WINSW_SERVICE_ID}</id>
  <name>NexCode Proxy (native)</name>
  <description>NexCode proxy running as a native Windows service (windowless, starts at boot).</description>
  ...
  if (!process.stdin.isTTY) {
    throw new Error(
      "WinSW install requires an interactive console to prompt for the service account password. "
        + "Run `nxc service install --native` from an elevated Command Prompt or PowerShell window, not a hidden or piped session.",
    );
  }
...
export function statusWinswRaw(): WinswStatus {
  ...
  // partial uninstall): a stale nexcode-proxy-native registration can outlive it.
  ...
  if (/localsystem/i.test(startName) || (user && !startName.toLowerCase().includes(user.toLowerCase()))) {
    ...
    throw new Error(
      `Native service was registered as "${startName || "unknown"}" instead of the current user; ` +
        "rolled back. Re-run `nxc service install --native` and enter the account credentials when prompted.",
    );
  }
}
  ...
  if (existing === "unknown") {
    throw new Error(
      "Could not query the native service state (WinSW status failed or returned an unexpected result). " +
        "Refusing to guess the install state — check 'nxc service status' and retry.",
    );
  }
...
export function winswStatusSummary(): string {
  ...
  if (status === "nonexistent") {
    ...
    return existsSync(winswXmlPath()) && !existsSync(winswExePath())
      ? "native assets present but WinSW binary missing — run 'nxc service repair'"
      : "";
  }
