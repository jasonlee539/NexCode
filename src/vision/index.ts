...
import type { NxcConfig, NxcContentPart, NxcMessage, NxcParsedRequest, NxcProviderConfig, NxcTextContent } from "../types";
 ...
 */
export function findAnthropicVisionProvider(config: NxcConfig): AnthropicVisionProvider | undefined {
...
export function resolveOpenAiVisionModel(config: Pick<NxcConfig, "visionSidecar">): string {
...
export function resolveEffectiveVisionModel(
  config: Pick<NxcConfig, "visionSidecar">,
...
function messagesHaveImage(parsed: NxcParsedRequest): boolean {
  return parsed.context.messages.some(m =>
    carriesImages(m.role) && Array.isArray(m.content) && (m.content as NxcContentPart[]).some(p => p.type === "image"));
}
...
export interface VisionPlan {
  ...
  routedConfig?: Pick<NxcConfig, "port" | "apiKeys">;
...
function renderDescription(out: { text: string; error?: string }): NxcTextContent {
  return {
    type: "text",
    text: out.error
 ...
 * untouched.
 */
function syncRawBodyImageDescriptions(parsed: NxcParsedRequest, descriptions: readonly string[]): void {
...
export async function describeImagesInPlace(
  ...
  const jobs: ImageJob[] = [];
  const targets: { msg: NxcMessage; parts: NxcContentPart[] }[] = [];
  for (const msg of parsed.context.messages) {
    ...
    const contextText = parts
      .filter((p): p is NxcTextContent => p.type === "text")
      .map(p => p.text)
      .join(" ")
 ...
 */
export function stripImagesInPlace(parsed: NxcParsedRequest, translatorBudget?: TranslatorBudget): boolean {
  let stripped = false;
  const descriptions: string[] = [];
  for (const msg of parsed.context.messages) {
    if (!carriesImages(msg.role) || !Array.isArray(msg.content)) continue;
    ...
    msg.content = parts.map(p => {
      ...
      const replacement = { type: "text", text: IMAGE_OMITTED_TEXT } as NxcContentPart;
      descriptions.push((replacement as NxcTextContent).text);
      const reservation = translatorBudget?.reserveTransient(
        descriptionEncoder.encode((replacement as NxcTextContent).text).byteLength,
        { kind: "request_copies" },
      );
      reservation?.commitRetained();
      return replacement;
