 ...
 *
 * Recursion fence: the request carries `x-nexcode-vision-describe: 1`.
 ...
 * `x-nexcode-api-key` — never Authorization (gateway-cache.ts rule: an
...
export const VISION_DESCRIBE_TERMINAL_HEADER = "x-nexcode-vision-describe";
...
export function routedDescribeAdmissionToken(config: Pick<NxcConfig, "apiKeys">): string | undefined {
...
export function routedDescribeBaseUrl(config: Pick<NxcConfig, "port">): string {
...
export async function describeImageRouted(
  ...
  contextText: string,
  routedModel: string,
  config: Pick<NxcConfig, "port" | "apiKeys">,
  ...
  if (admission) headers["x-nexcode-api-key"] = admission;
