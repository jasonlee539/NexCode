 ...
 *
 * Two invariants carried over from `nxc opencode` (src/cli/opencode.ts), which owned the
...
/**
 * One entry nexcode owns inside a client's config: the JSON path to it and
...
/** Everything nexcode contributes to one client's config, as one unit. */
...
export const OPENCODE_PROVIDER_ID = "nexcode";
 ...
 * load time.
 */
export const OPENCODE_API_KEY_ENV = "NEXCODE_OPENCODE_API_KEY";
 ...
 */
export const HERMES_API_KEY_ENV = "NEXCODE_HERMES_API_KEY";
...
export const OPENCLAW_API_KEY_ENV = "NEXCODE_OPENCLAW_API_KEY";
 ...
 */
export const LOOPBACK_API_KEY_PLACEHOLDER = "nexcode-loopback";
 ...
 */
export const GAJAE_API_KEY_ENV = "NEXCODE_GAJAE_API_KEY";
...
export const OPENCODE_PROVIDER_BLOCK_DEFAULT_CONFIG: NxcConfig = {
  port: 10100,
  hostname: "127.0.0.1",
...
function absoluteClientPath(raw: string, home: string, variable: string): string {
  ...
  if (!isAbsolute(trimmed)) {
    throw new ClientPathError(
      ...
      + "so nexcode and the client would disagree about which file it names.",
    );
  }
  return trimmed;
}
...
export function dshHomeDir(env: OpencodeLaunchEnv = process.env, home: string = homedir()): string {
  ...
  if (!isAbsolute(raw)) {
    throw new ClientPathError(
      ...
      + "so nexcode and DSH would disagree about which settings file it names.",
    );
  }
...
export interface ExportContext {
   ...
   * admission from `apiKey` to the `x-nexcode-api-key` header.
   */
  config?: NxcConfig;
}
...
export interface ExportClientSpec {
  ...
  /**
   * The fragments nexcode owns inside this client's config. Only the builder
   ...
   * dedicated `x-nexcode-api-key` header (AUTH_MATRIX in
 ...
 * value — Gajae reports `/providers/nexcode/models/N/input/2: Invalid option`
 ...
 * it does not have. That input is reachable three ways: `nxc models add
...
function opencodeProviderOptions(baseURL: string, config: NxcConfig): OpencodeProviderBlock["options"] {
  const options: OpencodeProviderBlock["options"] = { baseURL };
  // Non-loopback binds accept proxy admission only via x-nexcode-api-key so Authorization
  // stays free for Codex Direct upstream credentials when applicable.
  if (shouldInjectApiAuthHeader(config)) {
    options.headers = { "x-nexcode-api-key": OPENCODE_API_KEY_ENV_REF };
    return options;
  }
...
/**
 * `nexcode` provider block for a resolved base URL.
 *
...
/**
 * Build the `nexcode` provider block from proxy catalog rows keyed by each row's
 * canonical `namespaced` selector. Used by the `nxc opencode` launcher, which injects
...
export function buildOpencodeProviderBlockFromCatalog(
  ...
  catalogModels: readonly OpencodeCatalogModel[],
  hostname?: string,
  config: NxcConfig = OPENCODE_PROVIDER_BLOCK_DEFAULT_CONFIG,
): OpencodeProviderBlock {
...
export interface DshProviderBlock {
  ...
  api: "openai-responses";
  baseURL: string;
  headers: { Authorization: "Bearer nxc_data_dsh" };
...
function proxyAdmissionHeaders(config: NxcConfig | undefined, envRef: string): Record<string, string> | undefined {
  return shouldInjectApiAuthHeader(config) ? { "x-nexcode-api-key": envRef } : undefined;
}
...
function dshReasoningEfforts(model: ExportModel): DshModelEntry["reasoningEfforts"] {
  ...
  for (const effort of DSH_EFFORT_ORDER) {
    ...
    // wire. Preserve NexCode's `ultra` spelling when that is the only
...
function isKnownSafeDshCombo(model: ExportModel, config: NxcConfig): boolean {
...
/** The fragments nexcode owns in a client's config (writer-side). */
