 ...
 * `CODEX_HOME`/`NEXCODE_HOME` inside it, so an interrupted reinstall leaves a
...
export interface ServiceManagerClaim {
  ...
  readonly homes: {
    ...
    readonly nexcodeHome: string | null;
  };
...
export interface ProbeDeps {
  ...
  /** Effective NexCode config dir (NEXCODE_HOME). Overrides `<home>/.nexcode`. */
  ...
  return {
    ...
    claims: [{
      ...
      homes: {
        ...
        nexcodeHome: unitEnvValue(body, "NEXCODE_HOME"),
      },
      registration: "absent",
    }],
  };
  ...
  return {
    ...
    claims: [{
      ...
      homes: {
        ...
        nexcodeHome: plistEnvValue(body, "NEXCODE_HOME"),
      },
      registration,
    }],
  };
...
function inspectSystemd(deps: Required<Pick<ProbeDeps, "run" | "home">>): ServiceManagerInstallation {
  ...
  if (shown.status !== 0) {
    ...
    // with no session bus answers every native request with a 503 until `nxc restart`.
    //
 ...
 * `set "CODEX_HOME=..."` / `set "NEXCODE_HOME=..."` lines. Parsing the XML
 ...
 * doubling it, so a wrapper `C:\...\nexcode-service.cmd` is emitted as
...
function wrapperLooksGenerated(body: string): boolean {
  return /:loop\s*[\s\S]*^"%NXC_BUN%" "%NXC_CLI%" start\b[^\r\n]*$/im.test(body);
}
...
/** True only when a definition-provided path remains inside the effective NEXCODE_HOME. */
...
function inspectWindows(
  ...
  const taskXmlPath = join(configDir, "nexcode-service-task.xml");
  ...
  if (registration.registered === "unknown") {
    return unknown("Task Scheduler could not be asked whether nexcode-proxy is registered");
  }
  ...
  if (task === "absent") {
    return schedulerRegistered
      ? unknown("Task Scheduler holds nexcode-proxy but its task XML is missing")
      : { kind: "absent" };
  }
...
function homesEqual(
  a: { codexHome: string | null; nexcodeHome: string | null },
  b: { codexHome: string | null; nexcodeHome: string | null },
): boolean {
  const norm = (v: string | null): string | null => {
    if (v === null) return null;
    return v.replace(/[\\/]+$/, "").replace(/\//g, "\\").toLowerCase();
  };
  return norm(a.codexHome) === norm(b.codexHome) && norm(a.nexcodeHome) === norm(b.nexcodeHome);
}
 ...
 * paths are followed only inside the effective NEXCODE_HOME, preventing a
...
function walkWindowsChain(
  ...
  if (!wrapperLooksGenerated(wrapperBody)) {
    return unknown(`the launcher wrapper does not look like a generated nexcode service wrapper: ${wrapperPath}`);
  }

  const rawCodexHome = batchSetValue(wrapperBody, "CODEX_HOME");
  const rawNexcodeHome = batchSetValue(wrapperBody, "NEXCODE_HOME");
  ...
  return {
    ...
    claims: [{
      ...
      homes: {
        ...
        nexcodeHome: rawNexcodeHome === null ? null : decodeBatchPathValue(rawNexcodeHome),
      },
      registration: "absent",
    }],
  };
...
function walkWinswChain(
  ...
  // triggers behind #2108, where a scheduler-only install answers 503 until `nxc restart`.
  //
  ...
  if (!winswXmlLooksGenerated(body)) {
    return unknown(`the WinSW XML does not look like a generated nexcode service definition: ${xmlPath}`);
  }
  ...
  return {
    ...
    claims: [{
      ...
      homes: {
        ...
        nexcodeHome: envValue("NEXCODE_HOME"),
      },
      registration,
    }],
  };
...
function winswXmlLooksGenerated(body: string): boolean {
  return /<id>\s*nexcode-proxy-native\s*<\/id>/i.test(body)
