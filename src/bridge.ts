...
import { adapterFailureFromMessage, classifyError, CYBER_POLICY_ERROR_CODE, isCyberPolicyCode, type NxcErrorPayload } from "./lib/errors";
...
function responsesUsage(usage: NxcUsage | undefined): Record<string, unknown> {
...
function responseError(status: number, type: string, message: string): NxcErrorPayload {
...
function adapterFailureFromEvent(event: Extract<AdapterEvent, { type: "error" }>): { httpStatus: number; error: NxcErrorPayload } {
...
export function bridgeToResponsesSSE(
  ...
  options?: {
     ...
     * `{type:"compaction", encrypted_content:"nxc1:"+base64(text)}` output item before
    ...
    onCompletedResponse?: (response: Record<string, unknown>, providerState?: NxcProviderContinuationState) => void;
    /**
     ...
     */
    onUsage?: (usage: NxcUsage | undefined) => void;
     ...
     */
    replayCacheScope?: NxcReasoningReplayScopeRef;
    /**
  ...
  let stepping = false;
      ...
      const heartbeatFrame = options?.heartbeatStyle === "comment"
        ? encoder.encode(': nexcode heartbeat\n\n')
      ...
      let currentMsg: { itemId: string; outputIndex: number; text: string; textBytes: number; phase?: NxcMessagePhase } | null = null;
      ...
      // item as an nxcr1 encrypted_content envelope on close. hiddenThinkingText collects the
      ...
      // like native models — but the text still round-trips in a txt-only nxcr1 envelope so
      ...
      let currentToolCall: { itemId: string; outputIndex: number; callId: string; name: string; args: string; argsBytes: number; namespace?: string; freeform?: boolean; toolSearch?: boolean; inputEmitted?: string; providerMetadata?: NxcProviderOpaqueToolCallMetadata } | null = null;
      ...
      const closeCurrentMessage = (inferredPhase?: NxcMessagePhase) => {
...
function buildResponseJSONWithBudget(
  ...
  options?: {
    ...
    compaction?: boolean;
    onProviderState?: (state: NxcProviderContinuationState) => void;
  ...
  let currentTextPhase: NxcMessagePhase | undefined;
  ...
  let currentToolCallProviderMetadata: NxcProviderOpaqueToolCallMetadata | undefined;
  ...
  const flushText = (inferredPhase?: NxcMessagePhase) => {
