 ...
 * TOML admits equivalent header spellings for BOTH segments (`["model"."nxc-mine"]`,
 * `['model'.nxc-mine]`, `[ model . nxc-mine ]`); all of them redefine the same table, so each
 ...
 */
const NEXCODE_API_KEY = "nexcode-loopback";

/** A plain `[model.<alias>]` table outside the fence that nexcode itself wrote. */
...
/**
 * Model tables OUTSIDE the fence that nexcode itself wrote (#511).
 *
 ...
 */
function findNexcodeOrphans(content: string, region: ManagedRegion | null): OrphanTable[] {
  ...
  for (const [position, header] of headers.entries()) {
    ...
    if (keys.get("api_key") !== NEXCODE_API_KEY) continue;
...
function orphanedMarkerResult(action: string): GrokInjectResult {
  return {
    ok: false,
    changed: false,
    message: `Grok config ${action} refused: found the nexcode begin marker without its end marker. `
      + "The managed region boundary is ambiguous, so nothing was modified. "
      + "Repair ~/.grok/config.toml manually (see config.toml.bak-nexcode) and re-run.",
...
export function buildGrokManagedBlock(
  ...
  for (const model of models) {
    const baseAlias = `nxc-${model.id.replace(/[^A-Za-z0-9_-]/g, "-")}`;
    ...
    lines.push(
      ...
      `name = ${tomlString(model.name ?? `NXC ${model.id}`)}`,
      ...
      'extra_headers = { "x-nexcode-grok" = "1" }',
    );
...
export function injectGrokConfig(
  ...
  if (!isLoopbackHostname(opts.hostname)) {
    ...
    return {
      ...
      message: `Grok auto-registration skipped: nexcode is bound to the non-loopback host `
        ...
        + `add the models yourself OUTSIDE the nexcode markers (see the Grok Build guide).${cleanup}`,
    };
  }
  ...
  try {
    ...
    const orphans = findNexcodeOrphans(originalContent, originalRegion);
    ...
    if (output === rawContent) {
      return { ok: true, changed: false, message: "Grok config already contains the current nexcode managed block." };
    }
    ...
    return {
      ...
      changed: true,
      message: region
        ? "Updated the nexcode managed block in Grok config."
        : "Added the nexcode managed block to Grok config.",
    };
  } catch (error) {
...
export function stripGrokConfig(opts: { grokHome?: string } = {}): GrokInjectResult {
  ...
  try {
    ...
    if (!region) {
      return { ok: true, changed: false, message: "No nexcode managed block found in Grok config." };
    }
    ...
    return {
      ok: true,
      changed: true,
      message: "Removed the nexcode managed block from Grok config.",
    };
  } catch (error) {
