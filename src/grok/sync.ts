 ...
 * managed block into ~/.grok/config.toml. Used by `nxc start` (server process) and by
 * `nxc ensure` / `nxc restart` (parent process, after live discovery or child readiness)
...
async function defaultFetchAllModels(config: NxcConfig): Promise<CatalogModel[]> {
