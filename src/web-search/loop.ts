...
import type { AdapterEvent, NxcMessage, NxcParsedRequest, NxcProviderConfig, NxcProviderOpaqueToolCallMetadata, NxcThinkingContent, NxcUsage, RateLimitRetryPolicy } from "../types";
...
export function scanEventsForWebSearch(events: AdapterEvent[]): {
  ...
  let pending: { name: string; id: string; argsBuf: string; closed: boolean; events: AdapterEvent[]; providerMetadata?: NxcProviderOpaqueToolCallMetadata } | null = null;
 ...
 */
function extractIterationThinking(events: AdapterEvent[]): NxcThinkingContent[] {
  const parts: NxcThinkingContent[] = [];
  let thinking = "";
 ...
 */
function forcedAnswerNudge(): NxcMessage {
  return {
    role: "developer",
    content:
...
export async function runWithWebSearch(deps: WebSearchLoopDeps): Promise<Response> {
  ...
  const messages: NxcMessage[] = [...parsed.context.messages];
  ...
  const prepareIterationEvents = async function* (forceAnswer: boolean): AsyncGenerator<AdapterEvent, IterationResponse> {
    ...
    const iterMessages: NxcMessage[] = forceAnswer && executedSearchCount > 0
      ? [...messages, forcedAnswerNudge()]
      : messages;
    const iterParsed: NxcParsedRequest = {
      ...parsed, stream: true,
  ...
  async function* runSearchCall(call: WebSearchCall, precedingThinking: NxcThinkingContent[] = []): AsyncGenerator<AdapterEvent> {
