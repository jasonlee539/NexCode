...
import type { AdapterEvent, NxcConfig, NxcParsedRequest, NxcProviderConfig, NxcProviderContinuationState, NxcUsage } from "../../types";
import {
...
export function providerFetch(
  ...
  runtime: BunRuntimeGateInput | undefined = undefined,
  options: ProviderFetchOptions = {},
): ProviderFetch {
  const base = (provider as NxcProviderConfig & { fetch?: typeof globalThis.fetch }).fetch ?? globalThis.fetch;
  // The full suite deliberately mocks HTTP fetches in hundreds of route tests. Once
  // Bun gained the bounded WS transport those otherwise-local fixtures could dial the
  // real ChatGPT WebSocket before their mocked fetch, making results depend on network
  // reachability. The guarded preload override keeps ordinary tests on HTTP; dedicated
  // WS tests pass an explicit runtime and therefore still exercise the real branch.
  const guardedTestRuntime = process.env.NXC_TEST_HOME_GUARD === "1"
    ? process.env.NXC_TEST_CODEX_WS_RUNTIME?.trim()
    : undefined;
  const effectiveRuntime: BunRuntimeGateInput = runtime
    ?? (guardedTestRuntime || currentBunRuntimeIdentity());
  ...
  const unpaced = async (input: Parameters<typeof globalThis.fetch>[0], init?: RequestInit) => {
    if (typeof input === "string" && init && shouldUseCodexWsUpstream(input, init, effectiveRuntime)) {
      return codexWsUpstreamFetch(input, init, base, effectiveRuntime);
    }
