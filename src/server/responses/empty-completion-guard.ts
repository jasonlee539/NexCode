import type { AdapterEvent, NxcConfig, NxcUsage } from "../../types";
 ...
 * is a disable-only emergency override: NXC_EMPTY_COMPLETION_RETRY=0 restores
 * the previous relay behavior without editing the persisted config.
 */
export const EMPTY_COMPLETION_RETRY_ENV = "NXC_EMPTY_COMPLETION_RETRY";
...
export function emptyCompletionRetryEnabled(
  config: Pick<NxcConfig, "emptyCompletionRetry">,
...
export function emptyCompletionRetryFailedEvent(
  ...
  return {
    ...
    message: retryFailedUpstream
      ...
      : "The model returned an empty completion. nexcode retried once and the completion was empty again.",
    ...(usage ? { usage } : {}),
  };
}
...
export async function* guardEmptyCompletionEventStream(
  ...
  const withUsage = (event: AdapterEvent & { usage?: NxcUsage }): AdapterEvent => {
