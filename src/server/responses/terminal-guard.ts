...
function latestUserText(parsed: NxcParsedRequest): string {
...
function recentTurnHasToolActivity(parsed: NxcParsedRequest): boolean {
  ...
  let lastAssistant: NxcAssistantMessage | undefined;
...
export function analyzeTerminalTurn(parsed: NxcParsedRequest, events: readonly AdapterEvent[]): TerminalTurnAnalysis {
...
function assistantMessageFromEvents(events: readonly AdapterEvent[], timestamp: number): NxcAssistantMessage | undefined {
  let text = "";
  let thinking = "";
  ...
  const content: NxcAssistantContentPart[] = [];
...
export function buildContinuationRequest(parsed: NxcParsedRequest, events: readonly AdapterEvent[]): NxcParsedRequest {
...
export interface GuardedEventStreamOptions {
  ...
  continuation: (parsed: NxcParsedRequest) => AsyncIterable<AdapterEvent> | Promise<AsyncIterable<AdapterEvent>>;
  adapterName?: string;
...
function mergeUsage(first: NxcUsage | undefined, second: NxcUsage | undefined): NxcUsage | undefined {
...
export async function* guardTerminalEventStream(options: GuardedEventStreamOptions): AsyncGenerator<AdapterEvent> {
  ...
  let continuations = 0;
  let accumulatedUsage: NxcUsage | undefined;
