...
function backfillItemId(item: Record<string, unknown>, slot: ItemIdSlot): Record<string, unknown> {
  ...
  return { ...item, id: prefix + "nxc_" + (slot.kind === "index" ? String(slot.index) : "fallback_" + slot.ordinal) };
}
...
function rewriteEvent(event: Record<string, unknown>): Record<string, unknown> {
  ...
  if ((type === "response.output_item.added" || type === "response.output_item.done")
    ...
    // both synthesize `msg_nxc_0`, and duplicate ids are the very thing this backfill exists to
