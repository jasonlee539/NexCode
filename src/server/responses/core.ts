...
type ContinuationOwnerRead =
  | { kind: "missing" }
  | { kind: "invalid" }
  | { kind: "valid"; owner: NxcProviderContinuationOwner };

function readProviderContinuationOwner(
  state: NxcProviderContinuationState | undefined,
): ContinuationOwnerRead {
  if (!state || state.__nxcOwner === undefined) return { kind: "missing" };
  const owner = state.__nxcOwner;
...
function providerContinuationPayload(
  state: NxcProviderContinuationState | undefined,
): NxcProviderContinuationState | undefined {
  if (!state) return undefined;
  const cloned = structuredClone(state);
  delete cloned.__nxcOwner;
  return Object.keys(cloned).length > 0 ? cloned : undefined;
}

function bindProviderContinuationForRoute(
  parsed: NxcParsedRequest,
  currentOwner: NxcProviderContinuationOwner | undefined,
): void {
...
function bindRouteReasoningReplayScope(args: {
  ...
  const replayIdentity: NxcReasoningReplayIdentity | undefined = credentialIdentity && providerDestinationIdentity
    ? {
        providerName,
...
function prepareOpaqueBlobRecovery(parsed: NxcParsedRequest): void {
  parsed._stripReasoningEncryptedContent = true;
}
...
function nonEmptyProviderApiKey(provider: NxcProviderConfig): string | undefined {
...
interface CodexPoolAccountRetryArgs {
  req: Request;
  config: NxcConfig;
  route: { providerName: string; modelId: string; provider: NxcProviderConfig };
  parsed: NxcParsedRequest;
...
function applyCodexAccountGatedWireNormalization(parsed: NxcParsedRequest, route: RouteResult): void {
...
export interface HandleResponsesOptions {
  ...
  comboReplaySnapshot?: {
    ...
    providerContinuation: NxcProviderContinuationState | undefined;
  };
   ...
   * detects the raw `x-nexcode-vision-describe` header before its bridge
...
export function usageFromComboFailureText(text: string): NxcUsage | undefined {
  try {
...
function warnFastWireCapabilityGap(providerName: string, modelId: string): void {
  ...
  console.warn(
    `[nexcode] Fast policy for ${safeProvider}/${safeModel} has service-tier capability but no Fast wire; preserving only caller-permitted tier behavior`,
  );
}
...

  {
    ...
    if (guidance) {
      ...
      if (isInjectionDebugEnabled()) {
        injectionDebugLog(`[nexcode] ${route.modelId}: multi-agent guidance injected (surface=${collabSurface(parsed)}, guidanceEnabled=${multiAgentGuidanceEnabled(config)}, ${guidance.length} chars)`);
      }
    } else if (isInjectionDebugEnabled() && collabSurface(parsed) !== null) {
      injectionDebugLog(`[nexcode] ${route.modelId}: collab surface=${collabSurface(parsed)}, guidance silent (effort=${parsed.options.reasoning ?? "unset"}, injectionModel=${config.injectionModel ?? "unset"})`);
    }
  }

  {
    ...
    if (effortCapAppliesTo(surface, req.headers, config, parsed._compactionRequest === true)) {
      ...
      if (capped) {
        ...
        if (isInjectionDebugEnabled()) {
          injectionDebugLog(`[nexcode] ${route.modelId}: effort cap applied (${capped.from} -> ${capped.to}, ${capped.subagent ? "sub-agent" : "main"} turn)`);
        }
      }
    } else if (isInjectionDebugEnabled() && (config.effortCap || config.subagentEffortCap)) {
      injectionDebugLog(`[nexcode] ${route.modelId}: effort cap skipped (surface=${surface ?? "none"}, v2 feature only)`);
    }
  }
...
export async function handleComboResponses(
  ...
  if (scopeMismatch) {
    console.warn("[nexcode] dropped a previous_response_id with a mismatched client task scope; continuing fresh");
  }
...
async function handleResponsesInner(
  ...
  // verbatim). Genuine backend ciphertext is left byte-identical (looksLikeBackendCiphertext).
  {
    ...
    if (rewritten > 0)
      console.warn(
        `[nexcode] rewrote ${rewritten} plaintext encrypted_content part(s) to input_text (spawn-message compatibility)`,
      );
  }

  let parsed: NxcParsedRequest;
  ...
  if (threadSpawn && !options.comboAttempt && route.codexAccountId === undefined) {
    ...
    if (fallback) {
      ...
      if (isInjectionDebugEnabled()) {
        injectionDebugLog(`[nexcode] subagent model fallback ${fallback.from} -> ${fallback.to}`);
      }
    }
    ...
    if (recovered) {
      ...
      if (!unreadableEncryptedAgentTask) {
        try {
          ...
          const kept: Array<keyof NxcParsedRequest> = [
    ...
    try {
      ...
      if (err instanceof UnsupportedOAuthProviderError) {
        ...
        return formatErrorResponse(
          400,
          "invalid_request_error",
          `${redactSecretString(err.message)}. Remove or reconfigure provider '${safeProviderName}' in the NexCode configuration.`,
        );
      }
  ...
  const visionDescribeTerminal = options.visionDescribeTerminal === true
    || req.headers.get("x-nexcode-vision-describe") === "1";
  ...
  const continuationStateForResponse = (
    emitted?: NxcProviderContinuationState,
  ): NxcProviderContinuationState | undefined => {
    ...
    const merged = mergeProviderContinuationPayload(
      inherited ?? {},
      emittedPayload ?? {},
    ) as NxcProviderContinuationState;
    ...
    return parsed._providerContinuationOwner
      ? { ...merged, __nxcOwner: { ...parsed._providerContinuationOwner } }
      : merged;
  };
  ...
  if ("passthrough" in adapter && adapter.passthrough && !routedCompaction) {
    ...
    // previous_response_id, nxc converts them to internal HTTP requests, and the ChatGPT Codex
  ...
  // empty or fails. This is a top-level config opt-in; NXC_EMPTY_COMPLETION_RETRY=0 is a
  ...
  if (adapter.runTurn) {
    ...
    if (parsed.stream) {
      ...
      const sseStream = bridgeToResponsesSSE(
        ...
        }, 2_000,
        {
          ...
          onCompletedResponse: (response: Record<string, unknown>, providerState?: NxcProviderContinuationState) => {
            commitReasoningReplayServingRoute();
            if (!routedCompaction) {
              rememberResponseState(
                parsed._rawBody,
    ...
    let providerState: NxcProviderContinuationState | undefined;
  ...
  let sameTargetParsed: NxcParsedRequest | undefined = parsed;
