...
export function buildToolBridgeMaps(parsed: NxcParsedRequest, budget?: TranslatorBudget): {
...
export function isV1CollabSurface(parsed: NxcParsedRequest): boolean {
  return collabSurface(parsed) === "v1";
}



export function collabSurface(parsed: NxcParsedRequest): "v1" | "v2" | null {
...
async function defaultCollectCatalogState(): Promise<{ state: "fresh" | "stale" | "not_running" | "unknown" }> {
  ...
  const override = process.env.NEXCODE_APP_SERVER_CATALOG_STATE_OVERRIDE;
...
export async function multiAgentGuidanceText(
  ...
  if (surface === "v2") {
    ...
    // safe behaviour is to withhold NexCode's own disk-derived claims —
    ...
    if (isInjectionDebugEnabled() && effective.excluded.length > 0) {
      injectionDebugLog(`[nexcode] multi-agent guidance excluded: ${effective.excluded
        .map(item => `${item.configured}:${item.reason}`)
        .join(", ")}`);
    }
    const fallbackGuidance = subagentFallbackGuidanceText({ subagentModelFallback } as NxcConfig);
...
export function injectDeveloperMessage(parsed: NxcParsedRequest, text: string): void {
