...
import type { NxcContentPart, NxcParsedRequest, NxcProviderConfig } from "../../types";
...
function contentPartTokens(part: NxcContentPart, modelId: string): number {
  return part.type === "image" ? imageTokens(part.imageUrl) : estimateTokens(part.text, modelId);
}

function contentTokens(content: string | readonly NxcContentPart[], modelId: string): number {
 ...
 *
 * Walks the whole `NxcMessage` union rather than user text alone. Assistant turns carry
 * their content as `NxcAssistantContentPart[]` — text, thinking blocks, and tool calls whose
 ...
 * gate.
 */
export function estimateInputTokens(parsed: NxcParsedRequest, modelId: string): number {
