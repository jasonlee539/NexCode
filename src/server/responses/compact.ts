...
import type { AdapterEvent, CodexAccountMode, NxcConfig, NxcParsedRequest, NxcProviderConfig, NxcProviderContinuationState, NxcUsage } from "../../types";
import {
...
async function resolveAlternateCompactContext(args: {
  req: Request;
  config: NxcConfig;
  route: { provider: NxcProviderConfig; codexAccountMode?: CodexAccountMode };
  selectedModelId: string | undefined;
  excludeAccountId: string | null;
  turnAdmissionLease?: AdmissionLease;
}): Promise<{ authCtx: CodexAuthContext; provider: NxcProviderConfig; headers: Headers } | null> {
...
export async function handleResponsesCompact(
  ...
  if (supportsNativeResponsesCompactEndpoint(route.providerName, route.provider) && !accountGatedCompactWireModel) {
    ...
    const sendCompactAttempt = (
      sendProvider: NxcProviderConfig,
      sendHeaders: Headers,
  ...
  // tools) and decode the resulting nxc1 envelope into plain v1 replacement-history items.
  ...
  // The canonical Responses stream returns a real OpenAI-encrypted compaction item. NXC cannot
  ...
  // An empty `nxc1:` envelope decodes to "" rather than null, so length is what matters.
