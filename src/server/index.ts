...
export function startServer(port?: number, deps: StartServerDeps = {}): Server<WsData> {
  ...
  // /api/logs (and the GUI) survive `nxc stop` / `nxc start` process restarts.
  hydrateRequestLogsFromDisk();
  registerDefaultAppOwnedMemoryStores();
  registerDefaultAppOwnedObservedBuffers();
  registerAppOwnedMemorySweepFallback();
  ...
  const nativeMainLifecycle: NativeMainStartupLifecycle = shouldSyncCodexOnStart(config)
    ? nativeOwnership.ownership === "owned"
      ...
      : blockNativeMainStartupForUnownedServiceHome(
        ...
        // after boot reopens on its own instead of needing `nxc restart`. A `foreign`
  ...
  try {
    ...
    // External `nxc config set` / direct config.json edits run in other
    ...
    const serveOptions = {
      ...
      // The desktop management application has no model data plane. This guard
      ...
      if (url.pathname === "/v1/responses" && req.headers.get("upgrade")?.toLowerCase() === "websocket") {
        ...
        if (!admission) {
          return withCors(formatErrorResponse(401, "authentication_error", "nexcode API key required"), req, policy);
        }
      ...
      if (readyzPath !== undefined) {
        ...
        // sync). Report pending so `nxc ready --wait` and external supervisors
        ...
        // (and `nxc ready --wait`) back off instead of hot-looping.
      ...
      if (url.pathname === "/v1/models" && req.method === "GET") {
        ...
        // set (Authorization / x-api-key / x-nexcode-api-key) is safe here and required by
        ...
        if (!admission) return withCors(formatErrorResponse(401, "authentication_error", "nexcode API key required"), req, policy);
        if (!isAllowedRequestOrigin(req, policy)) {
        ...
        // aliases; legacy claude-nxc-* ids keep decoding via resolveAlias. Detection:
        ...
        if (wantsAnthropicList && !url.searchParams.has("client_version")) {
          ...
          // readable claude-nxc ids and every other client (Desktop 3P) keeps the
      ...
      if (req.method === "GET" && url.pathname.startsWith("/v1/nexcode/artifacts/")) {
        const admission = resolveApiAuth(req, policy);
        if (!admission) return withCors(formatErrorResponse(401, "authentication_error", "nexcode API key required"), req, policy);
        if (!isAllowedRequestOrigin(req, policy)) {
          return withCors(formatErrorResponse(403, "origin_rejected", "cross-origin data-plane request blocked"), req, policy);
        }
        const id = decodeURIComponent(url.pathname.slice("/v1/nexcode/artifacts/".length));
      ...
      if (url.pathname === "/v1/responses" && req.method === "POST") {
        ...
        if (req.headers.get("x-nexcode-grok") === "1") logCtx.surface = "grok";
        let logged = false;
      ...
      if (url.pathname === "/v1/messages/count_tokens" && req.method === "POST") {
        ...
        const admission = resolveApiAuth(req, policy);
        if (!admission) {
          return withCors(anthropicErrorResponse(401, "nexcode API key required", "authentication_error"), req, policy);
        }
        if (!isAllowedRequestOrigin(req, policy)) {
      ...
      if (url.pathname === "/nexcode-session" && guiSessionCandidate) {
