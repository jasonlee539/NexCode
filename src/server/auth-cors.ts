...
export function managementRequestOrigin(req: Request, config: NxcConfig): string | null {
...
export function isAllowedManagementOrigin(req: Request, config: NxcConfig): boolean {
...
const STATIC_ALLOWED_REQUEST_HEADERS =
  "Content-Type, Authorization, X-NexCode-API-Key, X-Api-Key, Anthropic-Version, Anthropic-Beta, ChatGPT-Account-Id, OpenAI-Alpha, X-Session-Id, Session-Id, Thread-Id, Originator, X-OAI-Attestation";
...
export function managementCorsHeaders(req?: Request, config?: NxcConfig): Record<string, string> {
...
export function withManagementCors(response: Response, req: Request, config: NxcConfig): Response {
  const headers = new Headers(response.headers);
...
// pass a config, so keep accepting one. Typed as `unknown` rather than `NxcConfig` so a narrow
// policy view can reach it too (#1102); widening to NxcConfig here would force every caller in
...
export function configuredAdminAuthToken(): string | undefined {
  const token = process.env.NEXCODE_ADMIN_AUTH_TOKEN?.trim();
  return token || undefined;
}
...
export function isApiAuthRequired(config: Pick<NxcConfig, "hostname">): boolean {
 ...
 */
export type RequestPolicyView = Pick<NxcConfig, "hostname" | "corsAllowOrigins" | "apiKeys">;

/** Derive the per-request policy view for a listener. Cheap enough to build per request. */
export function requestPolicyView(config: NxcConfig, bindHostname: string): RequestPolicyView {
  return {
...
export function assertServerAuthConfig(config: NxcConfig): void {
  ...
  if (isApiAuthRequired(config) && !hasConfiguredDataCredential) {
    throw new Error(
      "A data-plane credential (NEXCODE_API_AUTH_TOKEN or config.apiKeys) is required when binding nexcode to a non-loopback hostname",
    );
  }
}
...
export function isDataPlaneAdmissionSecret(token: string, config: NxcConfig): boolean {
...
export function isProxyAdmissionSecret(token: string, config: NxcConfig): boolean {
  const actual = token.trim();
  if (!actual) return false;
  if (/^nxc_(?:data|admin|session)_/.test(actual) || /^nxc_[0-9a-f]{40}$/.test(actual)) return true;
...
export class ForwardAdmissionCredentialError extends Error {
  constructor() {
    super("NexCode admission credentials cannot be forwarded upstream");
    this.name = "ForwardAdmissionCredentialError";
  }
}

export function validateForwardAdmissionCredential(headers: Headers, config: NxcConfig): void {
...
export function resolveApiAuth(req: Request, config: RequestPolicyView): DataPlaneAdmission | null {
  ...
  const dedicated = req.headers.get("x-nexcode-api-key")?.trim();
...
export function requireApiAuth(req: Request, config: RequestPolicyView, _kind: "data-plane"): Response | null {
  ...
  return formatErrorResponse(401, "authentication_error", "nexcode API key required");
}
...
function sameCanonicalProviderSeed(actual: Record<string, unknown>, expected: NxcProviderConfig): boolean {
...
export function providerManagementConfigError(name: unknown, provider: unknown): string | null {
  ...
  const typed = provider as unknown as NxcProviderConfig;
...
export function copyIfDefined<K extends keyof NxcProviderConfig>(
...
export function safeConfigDTO(config: NxcConfig): unknown {
