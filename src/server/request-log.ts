 ...
 * `nxc stop` / `nxc start` (process restart). Idempotent per process; no-ops when
...
export function nextRequestLogId(timestamp = Date.now()): string {
  ...
  return `nxc-${timestamp.toString(36)}-${requestLogSeq.toString(36)}`;
}
...
export function usageFromResponsesPayload(usage: unknown): NxcUsage | undefined {
