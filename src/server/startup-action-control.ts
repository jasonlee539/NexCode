...
const INDETERMINATE_MESSAGE =
  "Windows elevation timed out, but the elevated Task Scheduler transaction may still be running. New service installation attempts are temporarily blocked while NexCode reconciles its final state.";
...
const BLOCKED_PARTIAL_MESSAGE =
  "A previous elevated Windows service installation left a partial Task Scheduler state. Remove the NexCode scheduler task (or confirm it is absent), then clear the install block before retrying.";
...
export function clearStartupInstallPartialBlock(options: {
  ...
  if (options.probe.status === "present") {
    return {
      cleared: false,
      detail: "NexCode Task Scheduler task is still present; remove it before clearing the block.",
    };
  }
...
export interface CliInstallFailure {
  /** Machine marker such as NXC_ERROR_CODE=..., when present in any stream. */
  code: string | null;
  stdout: string;
  stderr: string;
  message: string;
...
function extractNxcErrorCode(text: string): string | null {
  const match = text.match(/NXC_ERROR_CODE=[A-Z0-9_]+/);
...
export function classifyCliInstallFailure(stdout: string, stderr: string, error: Error): CliInstallFailure {
  ...
  const code = extractNxcErrorCode(combined);
  ...
  return new Promise((resolve, reject) => {
    ...
    execFile(bun, argv, {
      ...
      if (error) {
        ...
        reject(Object.assign(new Error(failure.detail), { nxcInstallFailure: failure }));
        return;
      }
...
function installFailureCode(error: unknown): string | null {
  if (error && typeof error === "object" && "nxcInstallFailure" in error) {
    const failure = (error as { nxcInstallFailure?: CliInstallFailure }).nxcInstallFailure;
    if (failure?.code) return failure.code;
  }
  const detail = error instanceof Error ? error.message : String(error);
  return extractNxcErrorCode(detail);
}
 ...
 *
 * Repair mode (`options.repair`) runs `nxc service repair` — asset rewrite + restart
