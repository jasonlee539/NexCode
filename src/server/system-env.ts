...
/**
 * Does the nexcode dummy marker belong in the system environment?
 *
 * Keyed on the SAME resolver `nxc claude` uses, so an auto config with no Claude auth
 ...
 *
 * NOTE this is a SNAPSHOT: the file only changes when this runs (proxy start, `nxc
 * ensure`, or a settings save). `nxc claude` re-resolves live on every launch.
 */
function systemEnvMarkerMode(config: NxcConfig): "proxy" | "subscription" {
...
function writeShellEnvFile(port: number, config: NxcConfig, modelEnv: Record<string, string> = {}, auto?: AutoContextMode): void {
  const lines = [
    `# Generated by nexcode — do not edit manually`,
  ...
  // Auto-context (devlog 260712 020): same contract as `nxc claude` / launchctl.
...
const SHELL_HOOK_MARKER = "# nexcode claude-env hook";
 ...
 * remove the hook. That is the intended failure direction — removing an NexCode-owned block
 * is reversible on the next foreground `nxc start`, whereas leaving a hook pointing at an
 ...
 */
async function computeEffectiveModelEnv(config: NxcConfig, auto?: AutoContextMode): Promise<{ modelEnv: Record<string, string>; windows: Record<string, number> }> {
...
export async function injectSystemEnv(port: number, config: NxcConfig): Promise<SystemEnvResult> {
  ...
  try {
    ...
    if (config.apiKeys?.length) {
      ...
      // the nexcode-owned dummy token so a launchd-started Claude regains its own
...
export async function applySystemEnvToggle(config: NxcConfig, port: number): Promise<SystemEnvResult | RevertResult> {
