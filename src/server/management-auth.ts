...
function assertSafeDirectory(path: string): void {
  ...
  if (!hardened.ok) {
    throw new Error(
      "management token directory ACL hardening did not complete; set NEXCODE_ADMIN_AUTH_TOKEN to use an environment token instead of a file-backed token",
    );
  }
}
...
function readExistingToken(path: string): string {
  ...
  if (!hardened.ok) {
    throw new Error(
      "management token file ACL hardening did not complete; set NEXCODE_ADMIN_AUTH_TOKEN to use an environment token instead of a file-backed token",
    );
  }
  const token = readFileSync(path, "utf8").trim();
  if (!/^nxc_admin_[A-Za-z0-9_-]{43}$/.test(token)) throw new Error("management token file is invalid");
  return token;
}
...
function createTokenFile(path: string): string {
  ...
  const token = `nxc_admin_${randomBytes(32).toString("base64url")}`;
  ...
  try {
    ...
    if (!temporaryHardened.ok) {
      throw new Error(
        "management token temporary ACL hardening did not complete; set NEXCODE_ADMIN_AUTH_TOKEN to use an environment token instead of a file-backed token",
      );
    }
    try {
...
function ready(token: string, source: "environment" | "file", config: NxcConfig): ManagementAuthState {
...
export function initializeManagementAuthState(config: NxcConfig): ManagementAuthState {
  const environmentToken = process.env.NEXCODE_ADMIN_AUTH_TOKEN?.trim();
  if (environmentToken) {
...
function randomSessionSecret(prefix: "nxc_session_"): string {
...
export function issueGuiSession(
  ...
  const token = randomSessionSecret("nxc_session_");
...
export function managementPrincipal(
  ...
  const actual = req.headers.get("x-nexcode-api-key")?.trim()
...
export function requireManagementAuth(
  ...
  if (!state.available) {
    return Response.json({
      ...
      reason: state.reason,
      hint: "Set NEXCODE_ADMIN_AUTH_TOKEN to bypass file-backed admin token ACL hardening",
    }, { status: 503 });
  }
  const actual = req.headers.get("x-nexcode-api-key")?.trim()
    || req.headers.get("authorization")?.replace(/^Bearer\s+/i, "").trim();
  ...
  if (actual && config) {
    ...
    if (session) {
      ...
      const claimedOrigin = req.headers.get("x-nexcode-gui-origin");
      ...
      const csrf = req.headers.get("x-nexcode-csrf-token")?.trim();
      if (sameOrigin && (safeMethod || (browserOrigin === session.origin && !!csrf && equalSecret(csrf, session.csrfToken)))) {
        return null;
      }
    }
  }
  return Response.json({ error: "nexcode admin token required" }, { status: 401 });
}
