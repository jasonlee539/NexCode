...
import { AnthropicRequestError, anthropicToResponsesTranslation, extractNxcEffortDirective, extractNxcRouteDirective, resolveInboundModel, type ClaudeCacheKeySource } from "../claude/inbound";
...
export function buildClaudeReplayConfig(config: NxcConfig): NxcConfig {
  return {
    ...config,
    webSearchSidecar: {
...
function claudeInboundDisabled(config: NxcConfig): Response | null {
...
const PASSTHROUGH_STRIP_HEADERS = new Set([
  ...
  "accept-encoding", "x-nexcode-api-key", "origin",
]);
...
function hasAnthropicNativeCredential(req: Request, config: NxcConfig): boolean {
...
function shouldForwardNativeHeader(name: string, value: string, config: NxcConfig): boolean {
...
function anthropicUsageToNxc(usage: Rec | undefined): { inputTokens: number; outputTokens: number; cachedInputTokens?: number; cacheReadInputTokens?: number; cacheCreationInputTokens?: number } | undefined {
  ...
  // inclusive convention (types.ts NxcUsage / devlog 070). cached = READS only.
  return {
...
export function tapAnthropicSseForLog(
  ...
  const recordUsage = () => {
    logCtx.usage = anthropicUsageToNxc(Object.keys(usageAcc).length > 0 ? usageAcc : undefined);
  };
...
async function anthropicNativePassthrough(
  ...
  if (upstream.ok) {
    try {
      ...
      if (isRec(parsed?.usage)) logCtx.usage = anthropicUsageToNxc(parsed.usage);
 ...
 */
export function resolvePassthroughBodyGuard(config: NxcConfig, reqSignal?: AbortSignal): PassthroughBodyGuard {
...
async function handleClaudeMessagesWithBudget(
  ...
  let effortOverride: ReturnType<typeof extractNxcEffortDirective> = null;
  try {
    ...
    // nxc-route override (devlog 072): injected agent bodies pin their model via a
    ...
    if (isRec(anthropicBody)) {
      const routeOverride = extractNxcRouteDirective(anthropicBody);
      if (routeOverride && typeof anthropicBody.model === "string") {
        anthropicBody.model = stripOneMillionMarker(routeOverride);
        effortOverride = extractNxcEffortDirective(anthropicBody);
      }
    }
  ...
  for (const name of FORWARD_HEADERS) {
    // The caller's bearer is the proxy admission token (nxc claude placeholder), never a
...
export async function handleClaudeCountTokens(
  ...
  // nxc-route override (devlog 072): keep count_tokens consistent with messages.
  const countRoute = extractNxcRouteDirective(raw);
  if (countRoute) {
