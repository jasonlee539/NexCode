 ...
 *
 * When `NXC_LIVE_FRAME_LOG` is set to a file path, every relayed sideband frame appends one
 ...
 */
export const LIVE_FRAME_LOG_ENV = "NXC_LIVE_FRAME_LOG";
  ...
  if (candidates.forwardCandidates.length === 0 && !candidates.keyed) {
    return formatErrorResponse(
      ...
      "Built-in ChatGPT voice needs an OpenAI upstream (ChatGPT login or an OpenAI API-key provider), "
        + "but none is configured in nexcode. Routed providers cannot serve voice call-create.",
    );
  }
