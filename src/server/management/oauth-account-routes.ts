...
export async function handleOauthAccountRoutes(ctx: ManagementContext): Promise<Response | null> {
  ...
  if (url.pathname === "/api/keys" && req.method === "GET") {
    ...
    return jsonResponse({
      // 8 random hex past the fixed `nxc_data_` literal: enough to tell two keys
      // apart in a list, with 128 bits of the tail still unrevealed. Masking only
      // 8 characters showed `nxc_data...` for every key ever generated.
      keys: keys.map(k => ({
        id: k.id,
        name: k.name,
  ...
  if (url.pathname === "/api/keys" && req.method === "POST") {
    ...
    const key = "nxc_data_" + randomBytes(20).toString("hex");
