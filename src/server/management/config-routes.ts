...
async function sidecarVisionResponseSettings(config: NxcConfig): Promise<{
  model: string;
  reasoning: string;
 ...
 * NexCode ownership record is the operator's durable opt-in.
 *
 ...
 * next `nxc start`. The startup path already gates each client on its own toggle
...
export async function handleConfigRoutes(ctx: ManagementContext): Promise<Response | null> {
  ...
  if (url.pathname === "/api/settings" && req.method === "GET") {
    ...
    if (clampActive) {
      ...
      warningParts.push(
        `Some reasoning effort options were hidden because NexCode used Codex ${clampVersion}.${resolved.newerAvailable ? " A newer Codex installation is available." : ""}`,
      );
    } else if (resolved.newerAvailable) {
      warningParts.push(
        `NexCode is using an older Codex binary (${resolved.runtime.version ?? "unknown"}). A newer Codex installation is available.`,
      );
    }
    return jsonResponse({
