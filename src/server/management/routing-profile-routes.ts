...
import type { NxcConfig, NxcRoutingProfileConfig } from "../../types";
...
export async function handleRoutingProfileRoutes(ctx: ManagementContext): Promise<Response | null> {
  ...
  if (url.pathname === "/api/routing-profiles" && req.method === "PUT") {
    ...
    if (mode === "update" && previousProfile) {
      ...
      const newProfile = normalizeRoutingProfile(id, body.profile as NxcRoutingProfileConfig);
    ...
    nextProfiles[id] = storedProfile(id, body.profile as NxcRoutingProfileConfig);
