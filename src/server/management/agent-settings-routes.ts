 ...
 */
function mirrorDesiredEnabledOntoSnapshot(config: NxcConfig, client: "claude-desktop", enabled: boolean): void {
...
function persistDesktopProfileField(
  config: NxcConfig,
  desktopProfile: NonNullable<NxcConfig["claudeCode"]>["desktopProfile"],
...
export async function handleAgentSettingsRoutes(ctx: ManagementContext): Promise<Response | null> {
  ...
  if (url.pathname === "/api/claude-code" && req.method === "GET") {
    ...
    return jsonResponse({
      ...
      /** Does the nexcode dummy marker get injected — NOT a claim about native auth. */
  ...
  if (url.pathname === "/api/claude-code" && req.method === "PUT") {
    ...
    for (const field of ["webSearchSidecar", "visionSidecar"] as const) {
      ...
      const override = { ...next[field] } as NonNullable<NxcClaudeCodeConfig[typeof field]>;
    ...
    // Subscription has to remove the nexcode-owned dummy ANTHROPIC_AUTH_TOKEN
