 ...
 * other is the same as no gate at all. `nxc config set` raw JSON writes
...
export async function webSearchCandidateRows(config: NxcConfig): Promise<WebSearchCandidateRow[]> {
...
export function webSearchModelOptionsFrom(
  config: Pick<NxcConfig, "webSearchSidecar">,
