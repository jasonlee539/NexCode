...
import type { NxcClaudeCodeConfig, NxcConfig, NxcCustomModel, NxcProviderConfig } from "../../types";
...
function applyProviderPatchFields(
  ...
  const next: NxcProviderConfig = { ...provider };
  let touched = false;
  ...
  if (Object.hasOwn(rawBody, "requestPacing")) {
    ...
    if (value === null) {
      ...
      next.requestPacing = structuredClone(value) as unknown as NxcProviderConfig["requestPacing"];
    }
    touched = true;
  }
  if (Object.hasOwn(rawBody, "upstreamHttpVersion")) {
    ...
    if (value === null || value === "") {
      ...
      next.upstreamHttpVersion = value as NxcProviderConfig["upstreamHttpVersion"];
    }
    touched = true;
  }
...
export async function handleProviderRoutes(ctx: ManagementContext): Promise<Response | null> {
  ...
  if (url.pathname === LOCAL_PROVIDER_RELOAD_PATH && req.method === "POST") {
    ...
    let currentDiskConfig: NxcConfig | null = null;
  ...
  if (url.pathname === "/api/providers" && req.method === "POST") {
    ...
    const prov = body.provider ? stripCodexRuntimeProviderFields(body.provider as NxcProviderConfig) : undefined;
  ...
  if (url.pathname === "/api/providers/test" && req.method === "POST") {
    ...
    if (antigravity && !project) {
      return jsonResponse({ ok: false, latencyMs: 0, error: "Antigravity project unavailable — re-run `nxc login google-antigravity`" });
    }
