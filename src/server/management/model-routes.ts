 ...
 * be handed back to the GUI and CLI as if it were real, and the offline `nxc models add` path
...
function readInputModalities(raw: unknown): { values?: string[]; error?: string } {
  ...
  // array stays valid: that is how `nxc models edit --modalities -` clears the field.
...
export async function handleModelRoutes(ctx: ManagementContext): Promise<Response | null> {
  ...
  // ~/.nexcode/config.json with the `existing-uuid` test fixture.
  ...
  if (url.pathname === "/api/catalog" && req.method === "GET") {
    ...
    if (version) headers["x-nexcode-codex-version"] = version;
  ...
  /**
   * Client config document for OpenCode / Pi, built from the SAME function `nxc export`
