...
function grokStatus(config: ManagementContext["config"]): NativeStatus {
  ...
  if (seen.kind === "orphaned_marker") {
    disableBlocked = {
      ...
      message: "The managed block in the Grok config is ambiguous (a begin marker with no end marker), so nexcode cannot tell where it ends and will not touch it. Remove the nexcode markers by hand, then retry.",
    };
  } else {
 ...
 */
async function defaultFetchAllModels(config: NxcConfig) {
...
const ORPHANED_MARKER_MESSAGE =
  ...
  + "so nexcode cannot tell where it ends and will not touch it. "
  + "Remove the nexcode markers by hand, then retry.";
 ...
 *
 * THE PROXY STAYS UP. Turning Codex off is not `nxc stop`: other clients keep
 ...
 *   1. persist the desired state, so the decision survives the next `nxc start`;
  ...
  codexToggleFlight = (async (): Promise<Response> => {
    ...
    if (enabled) {
      ...
      return jsonResponse({
        ...
        desiredEnabled: enabled,
        message: applied.ok
          ? "Codex now routes through nexcode"
...
async function handleGrokToggle(ctx: ManagementContext): Promise<Response> {
  ...
  grokToggleFlight = (async (): Promise<Response> => {
     ...
     * records nothing, so the next `nxc start` calls syncGrokConfig
    ...
    if (!enabled) {
      ...
      return jsonResponse({
        ...
        message: result.changed
          ? "Grok integration disabled — the nexcode block was removed. Re-enabling regenerates it from the current model list."
    ...
    if (result.skippedReason === "non-loopback") {
      ...
      switch (after.kind) {
        ...
        case "present":
          ...
          // this read (`nxc ensure`, another proxy, a hand edit). It is not
          ...
          return jsonResponse({
            ...
            desiredEnabled,
            message: "nexcode is bound to a non-loopback address, so this request did not write a block — but a well-formed nexcode block is present in the Grok config, written by something else. The card shows what is on disk.",
          } satisfies NativeToggleEnvelope);
        case "not_installed":
          return postCommitRefusal(404, "grok", "not_installed", NOT_INSTALLED_MESSAGE, { desiredEnabled });
        case "absent":
          return jsonResponse({
            ...
            desiredEnabled,
            message: "nexcode is bound to a non-loopback address, so Grok cannot be auto-registered. The previously generated block was removed because it pointed at a loopback address that no longer serves.",
          } satisfies NativeToggleEnvelope);
        default: {
    ...
    return jsonResponse({
      ...
      state: "current",
      desiredEnabled,
      message: result.changed ? "Grok integration enabled — the nexcode block was regenerated from the current model list." : "Grok integration is already on",
    } satisfies NativeToggleEnvelope);
  })();
  try {
...
export async function handleNativeIntegrationRoutes(ctx: ManagementContext): Promise<Response | null> {
  ...
  if (url.pathname === "/api/native-integrations/claude" && req.method === "PUT") {
     ...
     * in-memory fixture cannot overwrite the developer's real NEXCODE_HOME.
     */
