...
export interface ManagementApiDeps {
  ...
  createManagementConvergeCodex?: (config: Readonly<NxcConfig>) => ConvergeCodex;
  /** Startup-health seam keeps route tests from launching platform probes. */
  getCachedStartupHealth?: (config: Pick<NxcConfig, "codexAutoStart">) => Promise<StartupHealth>;
  /**
   ...
   * NEXCODE_HOME (incident: devlog 260730.../070).
   */
  saveConfigPreservingClaudeCode?: (config: NxcConfig) => void;
  /**
   ...
   */
  fetchAllModels?: (config: NxcConfig) => Promise<CatalogModel[]>;
  /**
  ...
  primeCodexPoolQuotas?: (config: NxcConfig, reason: string) => Promise<void> | void;
  ...
  /**
   * Desktop-only filesystem/process seams. Production leaves these unset; route
   * tests inject a temporary Codex home and inert process controls so they never
   * inspect or terminate the developer's real Codex processes.
   */
