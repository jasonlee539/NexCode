 ...
 * port instead of hopping to an ephemeral one (Windows CLOSE_WAIT / leftover nxc).
 *
 * Killing is never the default. A process may be killed only when the caller
 * sets `killNxcHolders` and either supplies a non-empty `onlyKillPids` allowlist
 * (trusted teardown PIDs, including allowlisted holders that fail nxc revalidate)
 * or enables `killAllNxcOnPort` for revalidated nxc listeners. Unknown foreign
 * (non-nxc, non-allowlisted) processes are never killed.
 */
...
export type ReclaimListenPortOptions = WaitForPortOptions & {
   ...
   * When {@link killAllNxcOnPort} is also true, any nxc listener on this port
   * may be killed even if it is not in `onlyKillPids`.
   */
  killNxcHolders?: boolean;
  /**
   * Explicit PIDs the caller just stopped / hard-killed. An omitted or empty
   * list means no process may be killed — unless {@link killAllNxcOnPort} is set.
   */
  onlyKillPids?: number[];
  /**
   * When true with `killNxcHolders`, every live nxc listener on this port may be
   ...
   * Never kills foreign (non-nxc) processes — only allowlisted teardown PIDs
   * and revalidated nxc listeners.
   */
  killAllNxcOnPort?: boolean;
  /**
   ...
   * live foreign / protected nxc listener owns the port, and never runs when
 ...
 * Never kills a process unless `killNxcHolders === true` and either
 * `onlyKillPids` is a non-empty allowlist or `killAllNxcOnPort` is set — then
 * revalidates immediately before each kill.
 * Never kills foreign processes. Never drops TCP rows while a live foreign or
 * protected nxc listener owns the port, or when the listener scan failed.
 */
export async function reclaimListenPort(
  ...
  const killAllNxc = opts.killAllNxcOnPort === true;
  const mayKill = opts.killNxcHolders === true
    && (allowedKillPids.size > 0 || killAllNxc);
  ...
  const verifyNxcFn = opts.verifyNxcFn ?? verifyPidIdentity;
  ...
  for (;;) {
    ...
    if (Date.now() - lastScan >= scanIntervalMs) {
      ...
      let protectedNxcListener = false;
      ...
      for (const pid of scan.pids) {
        ...
        const isNxc = verifyNxcFn(pid) === pid;
          ...
          if (mayKill && allowlisted) {
            if (!killed.has(pid)) {
              try {
                ...
                protectedNxcListener = true;
              }
            }
            if (!isAliveFn(pid)) killed.delete(pid);
            else protectedNxcListener = true;
            continue;
          }
          foreignLive = true;
          continue;
        }
        const mayKillThis = allowlisted || killAllNxc;
        if (!mayKill || !mayKillThis) {
          // Healthy / intentional nxc proxy — never steal its port.
          protectedNxcListener = true;
          continue;
        }
        if (!killed.has(pid)) {
          // Revalidate immediately before termination.
          if (isAliveFn(pid) && verifyNxcFn(pid) === pid && mayKillThis) {
            try {
              killFn(pid);
              killed.add(pid);
            } catch {
      ...
      if (foreignLive || protectedNxcListener) {
        // Foreign app or an unprotected live nxc listener owns the port: never
