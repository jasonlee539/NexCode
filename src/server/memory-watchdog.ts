...
const DOCS_URL = "https://nexcode.me/troubleshooting/windows-memory/";
...
export function startMemoryWatchdog(opts?: {
  ...
  const tick = () => {
    ...
    if (s.observedBytes >= warnThresholdBytes && (lastWarnAt === null || now() - lastWarnAt >= WARN_INTERVAL_MS)) {
      ...
      warn(`⚠️  nexcode observed memory ${observedMb}MB (${s.observedMetric}) exceeds the ${thresholdMb}MB watch threshold. On Windows this is usually the upstream Bun runtime memory issue — see ${DOCS_URL}`);
    }
  };
