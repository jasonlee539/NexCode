 ...
 * the /healthz body to identify as nexcode.
 *
...
/**
 * True when a /healthz body identifies an nexcode proxy. Accepts the explicit
 * `service: "nexcode"` marker, plus the legacy `{status, version, uptime}` trio so a
 * still-running pre-identity proxy (e.g. right after `nxc update`) is not mistaken for a
 * foreign server and shadow-started over.
 */
export function isNexcodeHealthz(body: HealthzIdentity | null): boolean {
  if (!body) return false;
  if (body.service === "nexcode") return true;
...
//  - body.service must be exactly "nexcode".
...
export function validateReadyzBody(
  ...
  if (b.service !== "nexcode") return null;
