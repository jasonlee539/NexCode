...
async function tryCcaImageGeneration(
  ...
  try {
    ...
    if (errName === "OAuthLoginRequiredError") {
      return formatErrorResponse(401, "invalid_request_error", "Google Antigravity login required: run 'nxc login google-antigravity'");
    }
  ...
  if (!project) {
    ...
    return formatErrorResponse(
      ...
      "Antigravity requires a discovered Cloud Code Assist project id (re-run `nxc login google-antigravity`).",
    );
  }
  ...
  const envelope = {
    ...
    request: {
      ...
      sessionId: `nxc-img-${crypto.randomUUID().slice(0, 8)}`,
    },
  };
...
export async function handleImages(
  ...
  if (!canUseOpenAiForward && !candidates.keyed) {
    ...
    return formatErrorResponse(
      ...
      + "but none is configured in nexcode. Add a provider or disable the tool with `codex features disable image_generation`.",
    );
  }
