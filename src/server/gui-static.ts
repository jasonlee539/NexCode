...
/** nexcode version, read from the packaged package.json (same source as the server bootstrap). */
const VERSION = (() => {
  try {
...
function sessionBootstrapMeta(session: GuiSessionBootstrap): string {
  return [
    `<meta name="nexcode-session-token" content="${escapeHtmlAttribute(session.token)}">`,
    `<meta name="nexcode-session-csrf" content="${escapeHtmlAttribute(session.csrfToken)}">`,
    `<meta name="nexcode-session-origin" content="${escapeHtmlAttribute(session.origin)}">`,
  ].join("");
}
 ...
 * GUI (Vite) proxies /nexcode-session to the backend with the original host so the
