...
export function knownModelIdsForProvider(
  ...
  config?: Pick<NxcConfig, "customModels">,
): string[] {
...
export function routedProviderConfig(providerName: string, provider: NxcProviderConfig): NxcProviderConfig {
  ...
  const resolved: NxcProviderConfig = {
    ...provider,
...
function activeProviderEntries(config: NxcConfig): [string, NxcProviderConfig][] {
...
export class NoEnabledOpenAiProviderError extends Error {
  constructor(modelId: string) {
    super(
      ...
      + `Run: nxc provider add openai && nxc sync && nxc restart`,
    );
...
export function routeConcreteModel(config: NxcConfig, modelId: string): RouteResult {
  return routeModelInternal(config, modelId, true, undefined);
}

function routeByKnownModelPattern(config: NxcConfig, modelId: string): RouteResult | undefined {
