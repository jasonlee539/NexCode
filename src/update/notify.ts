...
function interactiveGuardOk(): boolean {
  return !(process.env.NXC_SERVICE || !process.stdin.isTTY || !process.stdout.isTTY);
}
...
export function triggerBackgroundRefreshIfStale(channel: Channel, cache: VersionCache | null): void {
  ...
  try {
    ...
    const child = spawn(process.execPath, args, {
      ...
      stdio: "ignore",
      windowsHide: true,
      env: { ...process.env, NXC_SERVICE: "1" }, // never let the helper prompt
    });
    child.unref();
  } catch {
    /* best-effort */
...
/**
 * Interactive-only update prompt for `nxc start`. Must be called BEFORE the
