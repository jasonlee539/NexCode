...
export const PKG = "@bitkyc08/nexcode";
const HERE = dirname(fileURLToPath(import.meta.url)); // .../nexcode/src/update

export type Installer = "bun" | "npm" | "source";
export type Channel = "latest" | "preview";

/** Infer how nexcode is installed from the running module's path. */
...
/**
 * The GUI update worker sets NXC_SERVICE=1 and has stdio ignored — inheriting that for
...
function updateChildStdio(): "inherit" | "pipe" {
  if (process.env.NXC_SERVICE === "1") return "pipe";
...
/** The global-install command nexcode would run to update on this channel. */
...
/**
 * `nxc update` fallback for source checkouts and Bun global installs. npm global installs are updated
...
export async function runUpdate(): Promise<void> {
  ...
  console.log(`nexcode v${current} (installed via ${installer}, tag ${tag})`);
  ...
  // service-managed or orphaned proxy can be live while nxc.pid is stale/missing.
  // Full `nxc stop` semantics (drain, service stop, restore).
  if (serviceWasInstalled || readPid() || readRuntimePort()) {
    ...
    if (stop.status !== 0 || readPid() || readRuntimePort()) {
      ...
      console.error("⚠️  Could not stop the running proxy; aborting the update. Run 'nxc stop' and retry.");
      process.exit(1);
    }
  ...
  if (r.status === 0) {
    ...
    if (trayWasInstalled) {
      ...
      if (tray.status === 0) {
        ...
        console.warn("⚠️  Windows tray refresh failed. Run 'nxc tray install'.");
    ...
    if (serviceWasInstalled) {
      ...
      const freed = await reclaimListenPort(capturedListen.port, capturedListen.hostname, {
        ...
        intervalMs: 100,
        scanIntervalMs: 500,
        killNxcHolders: capturedListen.oldPid != null,
      ...
      process.env.NXC_BAKE_PORT = String(capturedListen.port);
      try {
        ...
        if (!serviceRefreshed || !serviceViable) {
          ...
          if (!freed) {
            ...
            console.warn(process.platform === "win32"
              ? `   Run 'nxc service repair', then 'nxc start --port ${capturedListen.port}'.`
              : `   Run 'nxc service repair' to see the reason, then 'nxc start --port ${capturedListen.port}'.`);
          } else {
            console.warn(
              serviceRefreshed
            ...
            // reasons `nxc service repair` reports directly (since it now verifies
            // the service actually serves).
            console.warn(process.platform === "win32"
              ? "   Run 'nxc service repair' to refresh the background service."
              : "   Run 'nxc service repair' to refresh the background service and see why it failed.");
      ...
      console.log(`Restart the proxy:  nxc start --port ${capturedListen.port}`);
    }
  } else {
    if (trayWasRunning) {
      try {
