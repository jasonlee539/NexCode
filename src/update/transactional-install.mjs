 ...
 *   <scopeDir>/nexcode                      live package
 *   <scopeDir>/.nxc-staging-<ts>/             npm --prefix root (contains node_modules/...)
 *   <scopeDir>/.nxc-backup-<ts>/nexcode     previous live tree during/after the swap
 *   <scopeDir>/.nxc-recovery.json             double-fault marker with a one-line restore
 */
...
export function verifyInstallTree(packageDir, expectedVersion) {
  ...
  const launcher = join(packageDir, "bin", "nxc.mjs");
  try {
    const st = statSync(launcher);
    if (!st.isFile() || st.size < 1024) failures.push("bin/nxc.mjs missing or truncated");
  } catch {
    failures.push("bin/nxc.mjs absent");
  }
...
function recoveryMarkerPath(scopeDir) {
  return join(scopeDir, ".nxc-recovery.json");
}
...
export function bootRestoreProbe(packageDir, deps = {}) {
  ...
  let backups = [];
  try {
    backups = readdirSync(scopeDir).filter(name => name.startsWith(".nxc-backup-")).sort();
  } catch {
    return { action: "none" };
  }
  ...
  const newestBackup = join(scopeDir, backups[backups.length - 1], "nexcode");
  if (liveOk) {
...
export function transactionalNpmUpdate({
  ...
  const rename = deps.rename ?? renameSync;
  const scopeDir = dirname(packageDir);
  const stageRoot = join(scopeDir, stampedName(".nxc-staging"));
  ...
  const backupRoot = join(scopeDir, stampedName(".nxc-backup"));
  const backupPackage = join(backupRoot, "nexcode");
  try {
