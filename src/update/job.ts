...
import { isNexcodeHealthz, probeHostname, proxyIdentityAt, type HealthzIdentity } from "../server/proxy-liveness";
...
const RELEASE_NOTES_URL = "https://github.com/lidge-jun/nexcode/releases/latest";
 ...
 * Windows ghost-TCB failures under Bun) is busy — matches published `nxc start`
 ...
 * be executing from (`@bitkyc08/.nexcode-*`). Reject the tiny postinstall
...
/**
 * Port is free for post-update `nxc start` only when the runtime that will
...
async function waitForGhostListenClear(
  ...
  while (Date.now() < deadline) {
    ...
    // Never SetTcpEntry while a live process still owns the port (foreign or nxc).
...
function packageLauncherPath(): string {
  // This module lives at src/update/job.ts — the launcher is <pkg-root>/bin/nxc.mjs.
  // After `npm install -g`, import.meta.url can still point at npm's renamed temp
  // tree (`@bitkyc08/.nexcode-*`). Prefer the live package path when that happens.
  const fromMeta = join(dirname(fileURLToPath(import.meta.url)), "..", "..", "bin", "nxc.mjs");
  if (!/[\\/]\.nexcode-/i.test(fromMeta) && existsSync(fromMeta)) return fromMeta;
  const live = fromMeta.replace(/[\\/]@bitkyc08[\\/]\.nexcode-[^\\/]+/i, `${sep}@bitkyc08${sep}nexcode`);
 ...
 *
 * The real npm worker command is `node /Users/<name>/.../bin/nxc.mjs update --tag latest`, so
...
export function restartCommand(
  ...
  // .exe files), NOT the `nxc.cmd` shim. Spawning a `.cmd` shell-less throws EINVAL on Windows
...
const NPM_FIELD_VALIDATORS: Record<string, (value: string) => string | null> = {
  ...
  // still left the VERSION free: `@bitkyc08/nexcode@99.99.99-JaneDoe` is a valid-looking
...
/**
 * Tear down anything that would make `nxc start` exit 1 with "already running"
...
function preparePortForPinnedStart(
  ...
  verifyNxc: (pid: number) => number | null = verifyPidIdentity,
): void {
  ...
  for (const holder of listPids(port)) {
    ...
    updateJob(job, {}, `Stopping live nxc listen holder PID ${holder} on port ${port} before pinned start.`);
...
function spawnDetachedStart(
  ...
  // Foreground `nxc start` keeps the listen process; EADDRINUSE/ghost races exit quickly
...
export interface RestartIo {
   ...
   * nxc child that would otherwise be treated as a protected listener).
   */
  ...
  verifyNxcFn?: (pid: number) => number | null;
...
async function restartAfterUpdate(
  ...
  const verifyNxc = io.verifyNxcFn ?? verifyPidIdentity;
  const aliveFn = io.isAliveFn ?? isProcessAlive;
  // Pre-update PID plus any nxc still LISTENing on the captured port. After a
  ...
  const reclaimKillAllowlist = (): number[] => {
    ...
    for (const pid of listPids(port)) {
      ...
      if (verifyNxc(pid) === pid) allow.add(pid);
    }
    return [...allow];
  };
  const reclaimOptsFor = (onlyKillPids: number[]) => ({
    ...
    // killing every nxc listener on this port, not only the pre-wait snapshot.
    // npm rename trees under `@bitkyc08/.nexcode-*` are classified as nxc by
    // isNxcStartCommandLine — never kill unknown foreign claimants on this port.
    killAllNxcOnPort: true,
    onlyKillPids,
  });

  if (serviceInstalled) {
    // schtasks /end often leaves the hidden cmd/wscript wrapper alive; its :loop
    // respawns `nxc start` a few seconds later and races port reclaim. End the
    ...
    // This skip existed because the refresh ran `nxc service install`, whose Windows
    // scheduler path always reaches `schtasks /create` — elevation the GUI update worker
    // (NXC_SERVICE=1) never has. `service repair` rewrites the wrapper assets and
    ...
    if ((io.platform ?? process.platform) === "win32" && process.env.NXC_SERVICE === "1" && refreshRegisters) {
    ...
    if (!freed && !skipServiceInstall) {
      updateJob(
        ...
        `Port ${port} still busy after ${Math.trunc(RESTART_PORT_RECLAIM_MS / 1000)}s; refusing to hop — reinstall may fail until the port is free.`
          + ` ${formatPortHolders(port, listPids, verifyNxc, preServiceAllow)}`,
      );
    ...
    if (!skipServiceInstall) {
      const prevBake = process.env.NXC_BAKE_PORT;
      process.env.NXC_BAKE_PORT = String(Math.trunc(port));
      let serviceOk = false;
      try {
        ...
        if (!serviceOk) {
          // The refresh that just failed was `nxc service repair` (serviceReinstallArgs),
          ...
          updateJob(
            ...
            + " Run 'nxc service repair' by hand to see the reason, then 'nxc service status'.",
          );
        }
      } finally {
        if (prevBake === undefined) delete process.env.NXC_BAKE_PORT;
        else process.env.NXC_BAKE_PORT = prevBake;
      }
      if (serviceOk) {
  ...
  // killAllNxcOnPort covers wrapper-respawned bun PIDs minted during the wait.
  ...
  if (!freed) {
    ...
    updateJob(
      ...
      `Port ${port} still busy after ${Math.trunc(RESTART_PORT_RECLAIM_MS / 1000)}s (reclaim could not free the socket).`
        + ` ${formatPortHolders(port, listPids, verifyNxc, directAllow)}`,
    );
    if (liveHolders.length > 0) {
      updateJob(job, {}, `Live holder(s) remain on port ${port}; not starting on another port. Retry 'nxc start --port ${port}'.`);
      return;
    }
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    ...
    preparePortForPinnedStart(job, port, listPids, aliveFn, verifyNxc);
...
function formatPortHolders(
  port: number,
  listPids: (port: number) => number[],
  verifyNxc: (pid: number) => number | null,
  allow: number[],
): string {
  const allowSet = new Set(allow);
  const holders = listPids(port).map(pid => {
    const tags = [
      verifyNxc(pid) === pid ? "nxc" : "foreign",
 ...
 * wrapper belonging to a DIFFERENT NexCode home under the same account. The
...
function killWindowsServiceWrapperProcesses(): void {
  killWindowsSchedulerWrappers({
    scriptPath: join(getConfigDir(), "nexcode-service.cmd"),
    launcherPath: join(getConfigDir(), "nexcode-service-launcher.vbs"),
  });
}
...
function restartFailureHint(port: number): string {
  return `Update installed, but the restarted proxy did not stay healthy on port ${port}. `
    + `Try 'nxc start --port ${port}'. `
    + "If the update log shows bun postinstall or EPERM warnings, "
    + "reinstall with 'npm install -g --allow-scripts=bun @bitkyc08/nexcode'.";
}
...
async function defaultProbeProxyIdentity(
  ...
  try {
    ...
    if (!isNexcodeHealthz(body)) return null;
    return {
 ...
 *
 * npm installs run `node nxc.mjs update`, which already stops the proxy and reinstalls /
 ...
 *
 * Probe-first applies only to service-managed npm installs: without a service, `nxc.mjs`
 * only prints `nxc start` and never brings the proxy back, so waiting would always burn
 ...
 * path always falls through to a direct `nxc start --port` so /healthz can recover.
 */
 ...
 * evidence — not merely a healthy NexCode listener. A no-op restart or a
  ...
  try {
    ...
    */
    ...
    if (trayWasInstalled) {
      ...
      if (tray.status !== 0) {
        updateJob(job, {}, "Windows tray refresh failed; run 'nxc tray install'.");
