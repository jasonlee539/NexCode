import type { NxcComboTarget, NxcConfig } from "../types";
...
export interface ComboPick {
  comboId: string;
  target: Required<NxcComboTarget>;
  targetIndex: number;
  attempted: string[];
...
function targetProviderIsUsable(config: NxcConfig, target: NxcComboTarget): boolean {
...
function smoothWeightedIndex(
  targets: Required<NxcComboTarget>[],
  state: SelectionState,
  eligible: (target: Required<NxcComboTarget>) => boolean,
): number {
  let best = -1;
...
export function pickComboTarget(
  ...
  const eligible = (target: Required<NxcComboTarget>): boolean =>
...
export function tryPickComboModel(config: NxcConfig, modelId: string): ComboPick | null {
