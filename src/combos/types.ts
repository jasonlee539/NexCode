...
export function preservesPhysicalComboProvider(
  config: Pick<NxcConfig, "providers" | "combos">,
): boolean {
...
export interface NormalizedComboConfig {
  ...
  targets: Array<Required<NxcComboTarget>>;
}
...
export function targetKey(target: Pick<NxcComboTarget, "provider" | "model">): string {
...
export function resolveComboId(
  config: { combos?: Record<string, NxcComboConfig> },
  modelId: string,
): string | null {
...
export function comboAliasIssues(
  id: string,
  alias: string,
  combos: Record<string, NxcComboConfig> | undefined,
...
export function normalizeComboConfig(raw: NxcComboConfig): NormalizedComboConfig {
  const alias = typeof raw.alias === "string" ? raw.alias.trim() : "";
...
export function comboDefaultEffort(
  ...
  return typeof value === "string" && isCodexReasoningEffort(value)
    ? value as NxcComboDefaultEffort
    : null;
}
...
export function listComboIds(config: { combos?: Record<string, NxcComboConfig> }): string[] {
