import type { NxcComboDefaultEffort, NxcComboTarget, NxcConfig } from "../types";
...
export function comboIdFromRawBody(body: unknown, config: NxcConfig): string | null {
...
export function concreteComboRequestBody(
  body: unknown,
  target: Pick<NxcComboTarget, "provider" | "model">,
  ...
  if (!targetReasoningEfforts?.includes(defaultEffort)) {
    ...
    if (!warnedUnsupportedDefaults.has(key)) {
      ...
      console.debug("[nexcode] combo default effort omitted", {
