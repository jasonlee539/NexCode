...
export function deriveStartupHealth(inputs: StartupHealthInputs): StartupHealth {
  ...
  const routingInjected = inputs.routingKind === "nexcode-local";
  const localRoutingDependency = inputs.routingKind === "nexcode-local"
  ...
  const shimCoverage: ShimCoverage = !shimEffective
    ? "none"
    : "cli-only";
  // We can only credit an nexcode service/shim for routing that nexcode owns.
  // An arbitrary localhost gateway has an independent lifecycle that NXC cannot repair.
  const ownsLocalRouting = inputs.routingKind === "nexcode-local";
...
export function collectStartupHealth(
  config: Pick<NxcConfig, "codexAutoStart">,
...
export function startupHealthSummary(health: StartupHealth): string {
  if (health.status === "native") return health.routingKind === "custom-remote"
    ...
    : "native Codex routing (no nexcode restart dependency)";
  ...
  if (health.routingKind === "custom-local") return `AT RISK after restart (custom local gateway lifecycle is not managed by nexcode; run '${command}')`;
