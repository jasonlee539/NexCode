import type { NxcAccountPoolRotationStrategy } from "../types";
...
const DEFAULT_STRATEGY: NxcAccountPoolRotationStrategy = "quota";
const VALID_STRATEGIES = new Set<NxcAccountPoolRotationStrategy>(["quota", "round-robin", "fill-first"]);
...
export function parseAccountPoolStrategy(raw: unknown): NxcAccountPoolRotationStrategy | null {
  if (typeof raw === "string" && VALID_STRATEGIES.has(raw as NxcAccountPoolRotationStrategy)) {
    return raw as NxcAccountPoolRotationStrategy;
  }
  return null;
}
...
export function normalizeAccountPoolStrategy(raw: unknown): NxcAccountPoolRotationStrategy {
