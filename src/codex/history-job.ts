 ...
 * apply targets nexcode only in legacy mode, and legacy recovery must not
 ...
 * history TO nexcode; the ordinary apply migrates to native so a later restore
 * has nothing to undo.
 */
/**
...
export function deriveCodexHistoryOperation(intent: {
  ...
  return intent.legacyMode ? "apply-nexcode" : "migrate-openai";
}
...
export function describeHistoryJobFailure(
  ...
  if (outcome.kind === "converged") {
    return "the history job reported no failure; run 'nxc doctor' if this is unexpected.";
  }
  ...
  const busyText = surface === "apply"
    ? legacyMode
      ? "the history DB is locked (Codex app/IDE open?). Close it and rerun 'nxc start'."
      : "the history DB is locked (Codex app/IDE open?). It is retried automatically (while the proxy runs and on every 'nxc start'); to force it now, close the Codex app and run 'nxc sync'."
    : surface === "recover-legacy"
      ? "the Codex history DB is locked (Codex app/IDE open?). Close it and rerun this command."
      : "the Codex app appears to be holding the history database. Close Codex and run `nxc restore` again.";
  if (outcome.kind === "blocked") {
    if (outcome.reason === "busy") return busyText;
    switch (outcome.reason) {
      case "unsafe-path":
        return "nexcode refused its history lock path (unsafe coordinator namespace); this is not a Codex app lock. Run 'nxc doctor' and check the nexcode runtime directory.";
      case "database":
        return "the history coordinator database is unavailable; this is not a Codex app lock. Run 'nxc doctor'.";
  ...
  if (outcome.historyFailureReason === "permission") {
    return "permission was denied while writing Codex history; this is not a Codex app lock. Run 'nxc doctor'.";
  }
  switch (outcome.reason) {
    case "worker-error":
      return `the history worker failed (${outcome.message}). Run 'nxc doctor'.`;
    case "worker-died":
      return "the history worker exited unexpectedly; this is not a Codex app lock. Run 'nxc doctor'.";
    case "timeout":
      return "the history worker timed out; this is not a Codex app lock. Run 'nxc doctor'.";
  }
}
  ...
  return new Promise<CodexHistoryJobOutcome>(resolve => {
    ...
    worker.postMessage({
      ...
      env: {
        ...
        ...(process.env.NEXCODE_HOME ? { NEXCODE_HOME: process.env.NEXCODE_HOME } : {}),
      },
    });
  });
}
