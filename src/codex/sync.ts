...
export async function syncModelsToCodex(
  ...
  if (desiredDisabled && catalogEvenWhenNotInjected) {
    // Explicit `nxc sync` with the integration OFF: refresh the catalog/cache so
  ...
  if (externalProvider) {
    if (catalogEvenWhenNotInjected) {
      // External providers own config.toml, and the injector removes the NexCode
  ...
  applyProxyEnv(config); // `nxc ensure`/`nxc sync` fetch provider models outside the server process
  let added = 0;
