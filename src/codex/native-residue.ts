...
const JOURNAL_FILE_NAME = "nexcode-journal.json";
const ROUTED_CATALOG_DESCRIPTION_PREFIX = "Routed via nexcode → ";
...
function classifyToml(
  ...
  if (result === "indeterminate") {
    return indeterminate(surface, read.path, "NexCode-shaped TOML does not match a complete routed grammar");
  }
  return { kind: "clean" };
}
...
function inspectConfig(codexHome: string, path: string): ConfigObservation {
  ...
  if (hasInjectedCodexRouting(read.content)) {
    ...
    const hasMarker = read.content.includes(NXC_SECTION_MARKER);
    const provider = rootTomlString(read.content, "model_provider");
    const providerBaseUrl = providerTableString(read.content, "nexcode", "base_url");
    if (hasMarker || provider === "nexcode" || providerBaseUrl !== null) {
      classification = indeterminate(
        "config",
        read.path,
        "NexCode-shaped TOML does not match a complete routed grammar",
      );
    }
  }
...
function classifyProfile(path: string): NativeRoutedResidueResult {
  return classifyToml("profile", path, content => {
    const generatedFallback = content.startsWith("# NexCode proxy fallback config (Design B)")
      && rootTomlString(content, "openai_base_url") !== null;
    const generatedNamedProfile = content.startsWith("# NexCode proxy profile — use with:")
...
function isNxcRoutedCatalogEntry(entry: Record<string, unknown>): boolean {
...
function classifyCatalogLike(
  ...
  if ((catalog.models ?? []).some(isNxcRoutedCatalogEntry)) {
    return { kind: "residue", surface, path: read.path };
  }
  if (catalogHasRoutedEntries(catalog)) {
    return indeterminate(surface, read.path, "routed catalog rows lack the NexCode authorship signature");
  }
  return { kind: "clean" };
}
...
function classifyPartialWrites(targetPaths: string[]): NativeRoutedResidueResult {
  ...
  for (const target of targetsByParent.values()) {
    ...
    for (const name of names) {
      const match = /^(.*)\.nxc\.\d+\.\d+\.tmp$/.exec(name);
      if (match?.[1] && target.names.has(match[1])) {
        return indeterminate("partial-write", join(target.path, name), "NexCode atomic-write artifact is still present");
      }
    }
  }
  return { kind: "clean" };
...
function classifyReferencedRollout(
  ...
  for (const [position, payload] of [["first", first], ["latest", latest]] as const) {
    ...
    hasNexCodeProvider = hasNexCodeProvider || payload.model_provider === "nexcode";
  }
  return hasNexCodeProvider
...
function classifyHistoryDatabase(path: string): NativeRoutedResidueResult {
  ...
  try {
    ...
    return rows.some(row => row.model_provider === "nexcode")
...
/** Read-only, fail-closed observation of every NexCode-routed Codex surface. */
