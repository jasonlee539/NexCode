...
function bundledRuntimeKey(
  ...
  nexcodeHome: string = process.env.NEXCODE_HOME ?? "",
): string {
  return [runtime.command, runtime.version ?? "", nexcodeHome].join("\0");
}
...
export function setBundledCatalogCacheForTests(
  runtime: Pick<ResolvedCodexRuntime, "command" | "version">,
  value: RawCatalog | null,
  options: Readonly<{ expiresAt?: number; nexcodeHome?: string }> = {},
): void {
  publishBundledCatalogCache(
    bundledRuntimeKey(runtime, options.nexcodeHome),
...
export function loadBundledCodexCatalog(deps: BundledCatalogDeps = {}): ReadonlyRawCatalog | null {
  ...
  // than NexCode will launch. Tests may inject commandCandidates to stub probing.
  ...
  const candidates = deps.commandCandidates?.() ?? (() => {
    const resolved = resolveAndPersistCodexRuntime({
      ...
      // which alone can exceed the 3s budget `nxc claude` allows /api/claude-code. Priority
