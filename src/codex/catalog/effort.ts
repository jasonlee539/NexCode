...
function stampCapabilityProvenance(entry: RawEntry, model: CatalogModel): void {
  ...
  entry.nexcode_capability_provenance = {
  ...
  if (!deps.commandCandidates) {
    try {
      ...
      if (resolved.persistError) {
        console.warn(`[nexcode] Codex runtime selection could not be persisted; a later sync may pick a different binary.`);
      }
      if (
        resolved.replacedConfigured
        && resolved.replacedConfigured.from.command !== resolved.runtime.command
      ) {
        console.warn(`[nexcode] Preferred Codex runtime is unavailable.`);
        console.warn(
          `[nexcode] Falling back from ${displayCodexRuntimePath(resolved.replacedConfigured.from.command)} to ${displayCodexRuntimePath(runtimePath)}.`,
        );
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      const redacted = redactUserPath(redactSecretString(message)).slice(0, 200);
      console.warn(`[nexcode] Codex runtime resolve failed during catalog clamp: ${redacted}`);
    }
  }
