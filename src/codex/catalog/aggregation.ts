...
export function buildComboCatalogOmission(
  ...
  return {
    ...
    targets,
    reason,
    message: `[nexcode] Combo "${safeCatalogWarningLabel(id)}" is omitted from the catalog because ${comboCatalogOmissionDetail(reason)}: ${targets.join(", ")}.`,
  };
}
...
export function warnComboMasqueradeCollisionOnce(slug: string): void {
  ...
  console.warn(
    `[nexcode] combo alias collision on "${safeCatalogWarningLabel(slug)}": the combo wins and the shadowed provider model is omitted from the catalog.`,
  );
}
...
export function warnComboUnrestorableShadowOnce(slug: string): void {
  ...
  console.warn(
    `[nexcode] combo alias collision on "${safeCatalogWarningLabel(slug)}": the existing user-managed or foreign catalog row is retained because no pristine backup can restore it. Rename the combo alias to expose both models.`,
  );
}
...
export function warnAccountSelectorShadowedProviderOnce(slug: string): void {
  ...
  console.warn(
    `[nexcode] account selector collision on "${safeCatalogWarningLabel(slug)}": the account-bound native model wins and the shadowed provider model is omitted from the catalog. Rename the provider or account selector.`,
  );
}
...
export function resolveSlugAliasCollisions(goModels: CatalogModel[]): Set<CatalogModel> {
  ...
  for (const m of goModels) {
    ...
    if (!slugAliasCollisionWarnings.has(key)) {
      ...
      console.warn(
        `[nexcode] slug alias collision on "${key}": multiple native ids encode to the same Codex-facing slug; `
