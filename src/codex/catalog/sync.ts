...
// NexCode-private catalog field: the spawn_agent candidate priority a row would have WITHOUT
// modelPickerOrder. Codex ignores unknown catalog fields (same as nexcode_catalog_kind), so this
...
export const SPAWN_PRIORITY_FIELD = "nexcode_spawn_priority";
 ...
 *                   kept Luna out of nexcode's roster.
...
export function deriveEntry(
  ...
  if (template || codexForwardNativeCapabilityAlias) {
    ...
    if (isRouted) {
      ...
      if (typeof e.base_instructions === "string") {
        // Proxy-neutral: keep the GPT-5/OpenAI disclaimer but never advertise the nexcode proxy
      ...
      if (model?.catalogKind) e.nexcode_catalog_kind = model.catalogKind;
    } else {
  ...
  if (model?.catalogKind) entry.nexcode_catalog_kind = model.catalogKind;
...
export function buildCatalogEntriesFromObservedState({
   ...
   * NxcConfig.modelPickerOrder (route native ordering through subagentModels instead).
   */
  ...
  for (const model of goModels) {
    ...
    if (nativeAliasesBySlug.has(model.alias)) {
      ...
      if (!slugAliasCollisionWarnings.has(model.alias)) {
        ...
        console.warn(
          `[nexcode] native combo alias collision on "${model.alias}": keeping the first configured combo and omitting later duplicates from the catalog.`,
        );
      }
      continue;
    }
  ...
  for (const slug of gptSlugs) {
    ...
    const routed = deriveEntry(
      template,
      slug,
      `Routed via nexcode → ${nativeAlias.provider} (${nativeAlias.owned_by ?? nativeAlias.provider}).`,
      5,
      nativeAlias,
      exactComboSlugs,
    );
    routed.nexcode_catalog_kind = CODEX_NATIVE_ALIAS_CATALOG_KIND;
  ...
  for (const [selectorIndex, selector] of accountSelectors.entries()) {
    ...
    for (const [nativeIndex, native] of accountNativeEntries.entries()) {
      ...
      // Codex ignores this NexCode extension; preserve the native comp_hash unchanged.
      e.nexcode_catalog_kind = CODEX_ACCOUNT_BOUND_CATALOG_KIND;
  ...
  for (const m of goModels) {
    ...
    const e = deriveEntry(
      template,
      slug,
      `Routed via nexcode → ${m.provider} (${m.owned_by ?? m.provider}).`,
      5,
      m,
      exactComboSlugs,
    );
    if (m.provider === COMBO_NAMESPACE && m.nativeAlias === true && !slug.includes("/")) {
      e.nexcode_catalog_kind = CODEX_NATIVE_ALIAS_CATALOG_KIND;
    }
  ...
  for (const entry of out) {
    if (wsEnabled) entry.supports_websockets = true;
      ...
      // for an endpoint nxc has disabled.
...
/**
 * True when an existing catalog row was authored by NexCode routing (#855).
 ...
 * description prefix `Routed via nexcode → `; foreign rows from Cursor or
 * user tooling do not. `owned_by` cannot serve as the signal (upstream
 * ownership), and `comp_hash` defaults to "nexcode" for every normalized
 * row.
 */
function isNxcAuthoredRoutedEntry(entry: RawEntry): boolean {
  if (isNativeAliasCatalogEntry(entry)) return true;
  const desc = typeof entry.description === "string" ? entry.description : "";
  const slug = typeof entry.slug === "string" ? entry.slug : "";
  return slug.includes("/") && desc.startsWith("Routed via nexcode → ");
}
...
export function mergeCatalogEntriesFromObservedState({
  ...
  const wouldSurviveUnreplaced = (entry: RawEntry): boolean => {
    if (entry.owned_by === COMBO_NAMESPACE
      ...
      || entry.nexcode_catalog_kind === CODEX_CUSTOM_MODEL_CATALOG_KIND
      || isNxcAuthoredRoutedEntry(entry)
  ...
  for (const entry of admittedRoutedEntries) {
    const slug = typeof entry.slug === "string" ? entry.slug : "";
    if (!slug
      || entry.nexcode_catalog_kind !== undefined
      || entry.owned_by === COMBO_NAMESPACE
      || !isNxcAuthoredRoutedEntry(entry)
      || !legacyCustomModelKeys.has(slugEquivalenceKey(slug))) continue;
    entry.nexcode_catalog_kind = CODEX_PROVIDER_MODEL_CATALOG_KIND;
  }
  const freshExactComboEntries = new Set(admittedRoutedEntries.filter(entry => (
    ...
    && entry.description.startsWith(`Routed via nexcode → ${COMBO_NAMESPACE} (`)
  )));
  const rank = new Map(featured.map((slug, i) => [slug, i] as const));
  ...
  const nativeSourceEntries = includeNativeOpenAi
    ...
    .map(m => {
      ...
      // Fallback-quality entries (nxc synthesis / codex-rs model_info fallback: display_name
  ...
  const alignedAccountBoundEntries = detachedAccountBoundEntries.map(entry => {
    ...
    aligned.nexcode_catalog_kind = CODEX_ACCOUNT_BOUND_CATALOG_KIND;
    return aligned;
  });
  ...
  const preservedRoutedEntries = existingRoutedEntries.filter(entry => {
    ...
    if (entry.nexcode_catalog_kind === CODEX_CUSTOM_MODEL_CATALOG_KIND) return false;
    // Before custom rows had a marker, a config deletion could otherwise be mistaken for a
    // provider outage. Only explicit save-boundary evidence may classify an unmarked NexCode
    // row; foreign and future-marked rows fail closed and remain preserved.
    if (entry.nexcode_catalog_kind === undefined
      && entry.owned_by !== COMBO_NAMESPACE
      && isNxcAuthoredRoutedEntry(entry)
    ...
    // Deleted/disabled providers cannot retain NexCode-authored ghosts. Foreign catalog rows
    // remain outside provider ownership and survive unless a fresh row replaces their exact slug.
    return !isNxcAuthoredRoutedEntry(entry);
  });
  ...
  if (degradedPreservedCount > 0 && policy.warningPolicy === "emit") {
    console.warn(`[nexcode] catalog sync: provider discovery degraded; preserving ${degradedPreservedCount} existing routed entr${degradedPreservedCount === 1 ? "y" : "ies"} on disk.`);
  }
  ...
  for (const entry of versionedEntries) {
    const kind = entry.nexcode_catalog_kind;
    ...
    delete entry.nexcode_catalog_kind;
    entry.nexcode_catalog_kind = kind;
  }
 ...
 *
 * An explicit `nxc sync` is also the refresh path for side profiles that consume
 * the NexCode catalog without injection (for example a custom `model_provider`
 ...
 */
function readRetainedCatalogSync(config: NxcConfig): RetainedCatalogSyncRead | null {
...
function writeRetainedCatalogSync({
  ...
  try {
    // Once-only: preserve the PRISTINE pre-nexcode catalog as the native-priority baseline
  ...
  // time, so an ordinary `nxc start` — or any dashboard action that re-syncs an
  // unchanged model set — marked every already-running Codex as holding an outdated
  // in-memory catalog. Since #1407 that verdict silences nexcode's own model
  ...
  try {
    ...
    const observedAccountModels = observedAccountBoundNativeEntries(currentCache?.models ?? [])
      ...
      .map(entry => ({
        ...
        // next NexCode catalog sync consumes this marker and creates only selector-qualified
        // rows for the currently configured public account selectors.
        visibility: "hide",
        nexcode_account_observed_native: true,
        nexcode_account_observed_selectors: mainSelectors,
      }));
    const wrapper = {
