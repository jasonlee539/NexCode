...
type ModelsAuthResolver =
  ...
  | {
      ...
      readonly resolve: (name: string, provider: NxcProviderConfig) => ModelsAuthResolution;
    };
...
interface CapturedProviderGather {
  ...
  readonly provider: NxcProviderConfig;
   ...
   * them — combo targets that are also listed in providers.*.models (NXC-111).
...
const REQUEST_CREDENTIAL_SENTINEL = `nxc-catalog-credential-${randomBytes(16).toString("hex")}`;
...
/** Model ids each provider must retain for combo catalog derivation (NXC-111). */
export function configuredComboTargetModelsByProvider(
  config: Pick<NxcConfig, "combos">,
 ...
 */
function omitProviderTransportExecutor(provider: NxcProviderConfig): Record<string, unknown> {
...
function providerCatalogFingerprint(name: string, prov: NxcProviderConfig): Record<string, unknown> {
  return {
    n: name,
...
function gatherFlightKey(config: NxcConfig): string {
  const providers = Object.entries(config.providers)
...
export function configuredContextWindow(prov: NxcProviderConfig, id: string): number | undefined {
  const configured = modelRecordValue(prov.modelContextWindows, id) ?? prov.contextWindow;
  return typeof configured === "number" && configured > 0 ? configured : undefined;
}

export function configuredInputModalities(prov: NxcProviderConfig, id: string): string[] | undefined {
...
export function configuredMaxInputTokens(prov: NxcProviderConfig, id: string): number | undefined {
  const configured = modelRecordValue(prov.modelMaxInputTokens, id);
  return typeof configured === "number" && configured > 0 ? configured : undefined;
}

function configuredReasoningSummarySupport(prov: NxcProviderConfig | undefined, id: string): boolean | undefined {
...
export function applyProviderConfigHints(name: string, prov: NxcProviderConfig, model: CatalogModel, providerCap?: number): CatalogModel {
  void name;
...
export function catalogHintsFromProviderConfig(name: string, prov: NxcProviderConfig, id: string, contextCap?: number): Partial<CatalogModel> {
  const hinted = applyProviderConfigHints(name, prov, { id, provider: name }, contextCap);
  const { provider: _provider, id: _id, ...hints } = hinted;
  return hints;
}

export function applyConfigHintsToCachedModels(name: string, prov: NxcProviderConfig, models: CatalogModel[], contextCap?: number): CatalogModel[] {
...
export function resolveComboCatalogMember(
  ...
  providers: ReadonlyMap<string, NxcProviderConfig>,
  contextCap?: number,
...
export function warnDroppedConfiguredIdsOnce(name: string, droppedConfiguredIds: string[]): void {
  ...
  console.warn(
    `[nexcode] Provider model discovery for "${name}" omitted configured model ids; dropping them from the authoritative live catalog: ${droppedConfiguredIds.join(", ")}.`,
  );
}
...
async function fetchProviderModelsWithAuth(
  ...
  if (prov.adapter === "cursor") {
    ...
    const cursorFetch = (prov as NxcProviderConfig & { fetch?: typeof globalThis.fetch }).fetch;
    ...
    if (isCurrentCacheGeneration()) {
      ...
      console.warn(
        `[nexcode] Cursor model discovery for "${name}" failed [${liveResult.error}]${liveResult.detail ? `: ${liveResult.detail}` : ""}; using stale/static catalog degradation.`,
      );
    }
  ...
  try {
    ...
    if (redirectError) {
      ...
      if (shouldLog) {
        console.warn(
          `[nexcode] Provider model discovery for "${name}" ${redirectError} [urlClass=${urlClass}, fallback=${fallback}].`,
        );
      }
    ...
    if (!res.ok) {
      ...
      if (shouldLog) {
        console.warn(
          `[nexcode] Provider model discovery for "${name}" failed with HTTP ${res.status} [urlClass=${urlClass}, fallback=${fallback}].`,
        );
      }
      return observed(models, "degraded");
    }
    ...
    if (!bounded.ok) {
      ...
      if (shouldLog) {
        console.warn(
          `[nexcode] Provider model discovery for "${name}" ${diagnostic} [status=${res.status}, contentType=${contentType}, urlClass=${urlClass}, fallback=${fallback}].`,
        );
      }
      return observed(models, "degraded");
    }
    ...
    if (cloudCodeAssist && !antigravity) {
      ...
      if (shouldLog) {
        console.warn(
          `[nexcode] Provider model discovery for "${name}" returned malformed CCA model data [status=${res.status}, contentType=${contentType}, urlClass=${urlClass}, fallback=${fallback}].`,
        );
      }
      return observed(models, "degraded");
    }
    ...
    if (!extracted.ok) {
      ...
      if (shouldLog) {
        console.warn(
          `[nexcode] Provider model discovery for "${name}" ${diagnostic[extracted.reason]} [status=${res.status}, contentType=${contentType}, urlClass=${urlClass}, fallback=${fallback}].`,
        );
      }
      return observed(models, "degraded");
    }
    ...
    // current capture's retain set on read (warm-cache NXC-111 / #1308).
    const forCache = withConfiguredRetention(live, { retainComboTargets: false });
    ...
    if (returned.length === 0 && name !== OPENAI_API_PROVIDER_ID) {
      console.warn(
        `[nexcode] Provider model discovery for "${name}" returned an authoritative empty catalog; ${droppedConfiguredIds.length > 0 ? `dropping configured model ids: ${droppedConfiguredIds.join(", ")}` : "no models will be exposed"}.`,
      );
    }
    if (!setCached(name, forCache, Date.now(), cacheGeneration)) {
      return observed(withConfiguredRetention(configured), "degraded");
    ...
    if (error instanceof ProviderOutboundPolicyError) {
      ...
      if (shouldLog) {
        console.warn(
          `[nexcode] Provider model discovery for "${name}" was blocked by destination policy: ${error.message} [urlClass=${urlClass}, fallback=${fallback}].`,
        );
      }
      return observed(models, "degraded");
    }
    ...
    if (shouldLog) {
      console.warn(
        `[nexcode] Provider model discovery for "${name}" threw ${error instanceof Error ? error.name : "unknown"} [urlClass=${urlClass}, fallback=${fallback}].`,
      );
    }
    return observed(models, "degraded");
  }
 ...
 * surfaces the configured target (NXC-111 / #1308).
 *
...
export function filterCatalogVisibleModels(
  models: CatalogModel[],
  config: Pick<NxcConfig, "disabledModels" | "providers">,
): CatalogModel[] {
...
function augmentRoutedModelsWithCapturedOpenAiApiRows(
  ...
  for (const trusted of trustedRows) {
    ...
    console.warn(`[nexcode] replacing conflicting live OpenAI API metadata for ${trusted.provider}/${trusted.id} with trusted registry metadata`);
  }
...
export function augmentRoutedModelsWithMetadata(
  ...
  caps?: Pick<NxcConfig, "providerContextCaps">,
): CatalogModel[] {
  const out = [...models];
