...
export function activeDefaultCatalogPath(): string {
  const home = activeCodexHome();
  return home ? join(home, "nexcode-catalog.json") : DEFAULT_CATALOG_PATH;
}
...
export interface CatalogModel {
  ...
  /** Provider opted into parallel tool calls (NxcProviderConfig.parallelToolCalls). */
  ...
  capabilities?: string[];
  /** NexCode-only catalog ownership marker; Codex ignores the serialized extension field. */
...
export function findNativeTemplate(catalog: RawCatalog | null): RawEntry | null {
  return catalog?.models?.find(
    m => typeof m.slug === "string"
      ...
      && m.nexcode_catalog_kind !== CODEX_NATIVE_ALIAS_CATALOG_KIND
      && m.owned_by !== COMBO_NAMESPACE
      && !(typeof m.description === "string" && m.description.startsWith("Routed via nexcode → ")),
  ) ?? null;
}
...
export function ensureStrictCatalogFields(
  ...
  if (typeof entry.comp_hash !== "string") entry.comp_hash = "nexcode";
...
export function catalogEntryIsNativeChatGpt(entry: RawEntry): boolean {
  ...
  // NexCode. Keep those on v2 unless the row still carries the ChatGPT-forward
  // contract (`use_responses_lite`).
  if (entry.nexcode_catalog_kind === CODEX_NATIVE_ALIAS_CATALOG_KIND) {
 ...
 *   spawns nexcode exists to enable (option B, devlog
...
export function applyMultiAgentMode(
  ...
  if (mode === "default") {
    ...
    for (const entry of entries) {
      const slug = typeof entry.slug === "string" ? entry.slug : "";
      const nativeAlias = entry.nexcode_catalog_kind === CODEX_NATIVE_ALIAS_CATALOG_KIND;
      const routedNativeSlug = slug.startsWith(`${OPENAI_CODEX_PROVIDER_ID}/`)
        ? slug.slice(OPENAI_CODEX_PROVIDER_ID.length + 1)
        : "";
      const codexForwardCapabilityAlias = entry.nexcode_catalog_kind === CODEX_CUSTOM_MODEL_CATALOG_KIND
        && entry.use_responses_lite === true
        && isNativeOpenAiCapabilityAliasModel(routedNativeSlug)
        ? routedNativeSlug
        : undefined;
...
export function normalizeRoutedCatalogEntry(
  ...
  // Opt-in providers (NxcProviderConfig.parallelToolCalls, e.g. xAI) advertise it too: the
...
export function catalogHasRoutedEntries(catalog: RawCatalog | null): boolean {
  return (catalog?.models ?? []).some(m => typeof m.slug === "string"
    && (m.slug.includes("/") || m.nexcode_catalog_kind === CODEX_NATIVE_ALIAS_CATALOG_KIND));
}
