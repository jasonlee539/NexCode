...
export function desktopAllowlistSuppressedNativeSlugs(
  config: Pick<NxcConfig, "combos" | "disabledModels">,
): Set<string> {
...
export function isNativeAliasCatalogEntry(entry: RawEntry): boolean {
  return entry.nexcode_catalog_kind === CODEX_NATIVE_ALIAS_CATALOG_KIND;
}
 ...
 * the `nxc ensure` parent process, outside the server, so an injected global would never
...
export function nativeContextLimits(
  config: Pick<NxcConfig, "providers" | "providerContextCaps">,
): NativeContextLimits {
...
export function disabledNativeSlugs(config: Pick<NxcConfig, "disabledModels">): Set<string> {
  return new Set((config.disabledModels ?? []).filter(id => !id.includes("/")));
}

export function visibleNativeSlugs(config: Pick<NxcConfig, "disabledModels" | "combos">): string[] {
  const disabled = disabledNativeSlugs(config);
...
export function shouldIncludeNativeOpenAi(config: Pick<NxcConfig, "providers">): boolean {
...
export function desktopVisibleNativeSlugs(
  config: Pick<NxcConfig, "claudeCode" | "disabledModels" | "combos" | "providers"
    | "codexAccounts" | "codexAccountNamespaces" | "codexAccountPickerEnabled">,
): string[] {
...
export function nativeModelRows(config: Pick<NxcConfig, "disabledModels" | "combos" | "providerContextCaps" | "providers">): Array<{ slug: string; disabled: boolean; contextWindow?: number; maxInputTokens?: number }> {
  const disabled = disabledNativeSlugs(config);
  const shadowed = configuredNativeAliasSlugs(config);
...
const ACCOUNT_BOUND_OBSERVED_NATIVE_MARKER = "nexcode_account_observed_native";
const ACCOUNT_BOUND_OBSERVED_SELECTORS_MARKER = "nexcode_account_observed_selectors";
 ...
 * able to rewrite it can already edit `config.json` or run `nxc` directly, and `router.ts`
