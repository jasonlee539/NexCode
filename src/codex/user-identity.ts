...
function windowsIdentityPowerShellCommand(expression: string): string[] {
  ...
  const deterministicOutput = [
    ...
    `$nxcValue = [string](${expression})`,
    "$nxcBytes = [System.Text.Encoding]::Unicode.GetBytes($nxcValue)",
    "[Console]::Out.Write([Convert]::ToBase64String($nxcBytes))",
  ].join("; ");
  return [
...
function windowsLocalAppDataExpression(): string {
  ...
  const statements = [
    `$nxcShell = Add-Type -MemberDefinition '${signature}'`
      + " -Name NxcKnownFolder -Namespace NxcIdentity -PassThru",
    `$nxcFolderId = [System.Guid]'${WINDOWS_LOCAL_APPDATA_FOLDER_ID}'`,
    "$nxcPathPtr = [System.IntPtr]::Zero",
    "$nxcHr = $nxcShell::SHGetKnownFolderPath([ref]$nxcFolderId, "
      + `${WINDOWS_KF_FLAG_DEFAULT_PATH}, [System.IntPtr]::Zero, [ref]$nxcPathPtr)`,
    "if ($nxcHr -ne 0) { throw 'SHGetKnownFolderPath failed' }",
    "try { [System.Runtime.InteropServices.Marshal]::PtrToStringUni($nxcPathPtr) }"
      + " finally { [System.Runtime.InteropServices.Marshal]::FreeCoTaskMem($nxcPathPtr) }",
  ];
...
function resolvePosixRuntimeRoot(uid: number): string {
  ...
  const root = join(realTmp, `nexcode-runtime-v1-${uid}`);
...
/**
 * Read-only namespace probe for diagnostics (`nxc doctor`).
 *
...
export function probeCodexCoordinatorNamespace(identity: UserIdentity): CoordinatorNamespaceProbe {
  if (identity.platform === "posix") {
    const root = join(resolveTrustedPosixTmp(), `nexcode-runtime-v1-${identity.uid}`);
    let entry;
    try {
  ...
  const root = resolve(localAppData, "NexCode", "Runtime", "v1", identity.sid.toUpperCase());
  let entry;
  try {
    entry = lstatSync(root);
  } catch (cause) {
