...
const COMPAT_TRIGGER = "nexcode_log_guard_compat_v1";
const QUIET_TRIGGER = "nexcode_log_guard_quiet_v1";
...
function openReadWrite(databasePath: string): Database {
  ...
  // reason for NexCode to materialise a new foreign database.
...
function performMutation(
  ...
  // user with an active NexCode trigger and no in-product way to remove it.
  ...
  try {
    locked = withLock(codexHome, databasePath, () => {
      ...
      // Keep L held through this write so another NexCode process cannot
