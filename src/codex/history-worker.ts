 ...
 * targets nexcode only in legacy mode, and legacy recovery is a different
...
export interface HistoryWorkerRunMessage {
  ...
  readonly env?: { readonly CODEX_HOME?: string; readonly NEXCODE_HOME?: string };
}
...
export function runHistoryUnitUnderLock(
  ...
  const acquired = withHistoryWriteSerialization(
    ...
    permit => {
      ...
      // apply-nexcode routes history to nexcode; migrate/restore return it to
      // native. The provider is derived from the operation, never from a caller.
      const provider = operation === "apply-nexcode" ? "nexcode" : "openai";
...
if (typeof self !== "undefined" && typeof (self as { onmessage?: unknown }) === "object") {
  self.onmessage = (event: MessageEvent<unknown>) => {
    ...
    try {
      ...
      if (message.env?.NEXCODE_HOME) process.env.NEXCODE_HOME = message.env.NEXCODE_HOME;
