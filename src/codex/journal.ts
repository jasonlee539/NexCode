 ...
 */
export const JOURNAL_PATH = join(CODEX_HOME, "nexcode-journal.json");
 ...
 *
 * Only native (non-nexcode-owned) config may be journaled, and native config
 * always supersedes an older snapshot. The first half stops a re-inject from
 * recording nexcode's own routing as the user's original — which would survive
 * `nxc stop` and make the injection unremovable. The second half is the #477 fix:
...
export function writeJournal(options: WriteJournalOptions = {}): void {
  ...
  // otherwise make nexcode's own routing the user's permanent "original".
...
export function markJournalInjectedState(
  ...
  // The profile file is wholly generated by NexCode, so its latest exact hash remains safe
