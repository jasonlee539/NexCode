/**
 * Ownership predicates for `~/.codex/config.toml`: does nexcode own the routing
...
export const NXC_SECTION_MARKER = "# Auto-injected by nexcode";
...
export function stripJournaledOpenaiBaseUrl(content: string, injectedUrl: string | null): string {
  ...
  for (let i = 0; i < rootEnd; i++) {
    ...
    if (i > 0 && lines[i - 1]!.includes(NXC_SECTION_MARKER)) drop.add(i - 1);
  }
...
export function hasInjectedOpenaiBaseUrl(content: string): boolean {
  ...
  for (let i = 1; i < rootEnd; i++) {
    if (isRootOpenaiBaseUrlLine(lines[i]!) && lines[i - 1]!.includes(NXC_SECTION_MARKER)) return true;
  }
  return false;
}

/**
 * True when the active Codex config is owned by nexcode routing. Covers the
...
export function hasInjectedCodexRouting(content: string): boolean {
  ...
  return rootTomlString(content, "model_provider") === "nexcode"
    && providerTableString(content, "nexcode", "base_url") !== null;
}
