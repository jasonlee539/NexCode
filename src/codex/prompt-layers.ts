 ...
 *    `nexcode-prompt.json`, which we own outright; config.toml receives a
...
import { NXC_SECTION_MARKER } from "./injected-marker";
import {
  durableWrite,
...
export function activeStorePath(opts?: Paths): string {
  return opts?.storePath ?? join(activeCodexHome(), "nexcode-prompt.json");
}
...
export function inspectOwnership(configBytes: string | null): Ownership {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    const marked = i > 0 && lines[i - 1]!.includes(NXC_SECTION_MARKER);
...
export interface ToggleState {
  ...
  /**
   * userFileValue ?? default. NOT the resolved Codex value: nexcode reads one
...
function setProjection(content: string | null, projection: string | null): string {
  ...
  let markerAt = -1;
  for (let i = 0; i < limit; i += 1) {
    if (i > 0 && lines[i - 1]!.includes(NXC_SECTION_MARKER) && ANY_DEV_INSTRUCTIONS.test(lines[i]!)) {
      markerAt = i - 1;
      break;
    }
  }
  ...
  lines.splice(0, 0, NXC_SECTION_MARKER, `${DEV_INSTRUCTIONS_KEY} = ${encodeBasicString(projection)}`);
  return joinLines(lines, eol);
}
