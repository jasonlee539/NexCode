 ...
 *
 * Scope boundary: this module mirrors only the flags nexcode has to READ
 * directly from config.toml:
 *   - `multi_agent_v2`, because nexcode migrates its concurrency value across
 ...
 * under-development flags were added. Delegation is what keeps nexcode out of
 * that churn.
 *
 * Used by the catalog v2-gated-ultra policy (devlog/260709_v2_gated_ultra) and the
 * `nxc v2` toggle surface. The FLAG itself is never written here — toggling goes
 ...
 * feature config (`-c` is per-invocation only), so nxc does a scoped,
 ...
 * `nxc v2 on` flow warns about it instead of editing config itself.
 */
...
export function setMaxConcurrentThreads(value: number, configPath?: string, migratedComment?: string): { ok: true; changed: boolean } | { ok: false; error: string } {
  ...
  if (headerIdx === -1) {
    ...
    if (featuresHeader === -1) return { ok: false, error: "multi_agent_v2 feature config not found — enable v2 first (nxc v2 on)" };
    ...
    return { ok: false, error: "multi_agent_v2 feature config not found — enable v2 first (nxc v2 on)" };
  }
...
export function setMultiAgentModeHintText(value: string | null, configPath?: string): ConfigEditResult {
  ...
  // surface nexcode already manages; an older Codex build rejects the unknown
 ...
 * nexcode shim or the npm JS wrapper (`codex.nexcode-real`), neither of
...
function codexNativeBinaryCandidates(command: string): string[] {
  ...
  // Follow the resolved command to its real location. The nexcode shim and
...
/** Backing paths tied to this exact NXC shim entry in codex-shim.json. */
function selectedShimBackingPaths(commandPath: string): string[] {
  try {
    ...
    // Not an NXC-owned shim, or no readable state.
  }
  return [];
}
 ...
 * slot is already below float precision, and crashing `nxc v2 status` or
...
function applyConfigEditsAtomically(path: string, edit: (tempPath: string) => ConfigEditResult): ConfigEditResult {
  const content = readConfigText(path);
  if (content === null) return { ok: false, error: `config.toml not readable at ${path}` };
  const tempPath = `${path}.nxc-migration.${process.pid}.${++migrationEditSeq}`;
