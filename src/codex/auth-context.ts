...
import type { CodexAccountMode, NxcConfig, NxcProviderConfig } from "../types";
...
export function codexPoolAffinityKey(headers: Headers): string | undefined {
  ...
  return `app:${createHmac("sha256", CODEX_APP_AFFINITY_KEY)
    .update("nexcode-app-pool-affinity-v1\0")
    .update(sessionId)
    .update("\0")
    .update(threadId)
    .digest("base64url")}`;
...
export type NxcRuntimeProviderConfig = NxcProviderConfig & {
...
export const CODEX_MAIN_PROFILE_MAINTENANCE_MESSAGE =
  "NexCode local native-main profile maintenance is active; retry this request";
...
/**
 * #2108: a reboot could leave this fence closed until `nxc restart`, and the report was
...
export function cooldownErrorMessage(err: CodexAccountCooldownError, accountSelector?: string): string {
  ...
  const recovery = accountSelector
    ...
    : " Run 'nxc account list openai' to find the id, then"
      + " 'nxc account clear-cooldown openai <id>' to lift it, or switch accounts with 'nxc account use openai <id>'.";
...
export interface ResolveCodexAuthContextOptions {
  ...
  primeCodexPoolQuotas?: (config: NxcConfig, reason: string) => Promise<void>;
  /** Test seam for account-gated native model discovery. */
  resolveCodexModelEntitlements?: (
    config: Pick<NxcConfig, "codexAccounts">,
...
export function isCodexAuthContextUsable(ctx: CodexAuthContext, config: NxcConfig): boolean {
...
export function stripCodexRuntimeProviderFields(provider: NxcProviderConfig): NxcProviderConfig {
  const {
    _codexAccountOverride: _override,
    _codexAccountRequired: _required,
    ...safeProvider
  } = provider as NxcRuntimeProviderConfig;
  return safeProvider;
}
