...
function probeVersion(
  ...
  // read-only `nxc status` would dirty the user's CODEX_HOME. Redirect it to a
  ...
  try {
    probeHome = mkdtempSync(join(tmpdir(), "nxc-codex-probe-"));
...
function resolveCacheKey(deps: ResolveCodexRuntimeDeps): string | null {
  ...
  return JSON.stringify({
    ...
    home: process.env.NEXCODE_HOME ?? "",
    persisted: persistedRuntimeCacheStamp(deps),
  });
}

/**
 * Resolve the single Codex runtime NexCode should use for sync, clamp, and probes.
 */
...
export function resolveAndPersistCodexRuntime(
  ...
  if (result.runtime.command && result.runtime.source !== "fallback" && !selectionUnchanged) {
    try {
      ...
      console.warn(`[nexcode] Failed to persist Codex runtime selection: ${persistError}`);
...
export function formatRuntimeLogLine(runtime: ResolvedCodexRuntime): string {
  ...
  return `[nexcode] Codex runtime: ${path} (version=${runtime.version ?? "unknown"}, source=${runtime.source})`;
}

export function formatClampLogLines(diagnostic: EffortClampDiagnostic): string[] {
  const efforts = diagnostic.removedEfforts.join(", ");
  return [
    `[nexcode] Removed unsupported reasoning efforts: ${efforts}`,
    "[nexcode] Run nxc doctor for diagnosis and recovery.",
  ];
}
