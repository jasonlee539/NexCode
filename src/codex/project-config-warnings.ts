...
const NXC_SECTION_MARKER = "# Auto-injected by nexcode";
...
export interface ProjectCodexConfigWarning {
  ...
  /** Effective provider id that bypasses NexCode. */
  detail: string;
...
function hasInjectedOpenaiBaseUrl(content: string): boolean {
  ...
  for (let i = 1; i < rootEnd; i++) {
    if (/^\s*openai_base_url\s*=/.test(lines[i]) && lines[i - 1].includes(NXC_SECTION_MARKER)) return true;
  }
  return false;
}
...
function isProxyCompatibleProvider(provider: string): boolean {
  return provider === "nexcode" || provider === "openai";
}
...
/** True when global Codex config routes through the nexcode proxy. */
export function isGlobalNexcodeRoutingActive(
  ...
  if (readRootTomlString(text, "model_provider") === "nexcode") return true;
  return false;
}
...
export function analyzeProjectCodexConfig(content: string, configPath: string): ProjectCodexConfigWarning[] {
  ...
  if (hasModelProviderTable(sections, provider)) {
    return [{
      ...
      message:
        ...
        + `[model_providers.${provider}] (${rel}). That routes this trusted project away from the NexCode proxy.`,
    }];
  }
  ...
  if (routing.via === "profile" && routing.profileName) {
    return [{
      ...
      message:
        ...
        + "That routes this trusted project away from the NexCode proxy.",
    }];
  }
  ...
  return [{
    ...
    message:
      ...
      + "Use global ~/.codex/config.toml for NexCode routing instead of a project-local provider override.",
  }];
}
...
export function collectProjectCodexConfigWarnings(options: {
  cwd?: string;
  codexConfigPath?: string;
  requireNexcodeRouting?: boolean;
} = {}): ProjectCodexConfigWarning[] {
  const codexConfigPath = options.codexConfigPath ?? resolveCodexConfigPath();
  const requireRouting = options.requireNexcodeRouting ?? true;
  if (requireRouting && !isGlobalNexcodeRoutingActive(codexConfigPath)) return [];
...
function humanizeProviderDetail(detail: string): string {
  ...
  if (detail === "nexcode") return "NexCode";
  return detail;
}
...
export function explainProjectConfigBypass(warnings: ProjectCodexConfigWarning[]): string {
  ...
  return `Overrides NexCode — Codex uses ${via} for this repo instead of the proxy (~/.codex/config.toml).`;
}
...
export function formatProjectCodexConfigWarningsForDoctor(warnings: ProjectCodexConfigWarning[]): string[] {
  ...
  lines.push("       fix: remove those entries so NexCode proxy routing applies in this project");
  return lines;
}
...
export function formatProjectCodexConfigWarningsForConsole(warnings: ProjectCodexConfigWarning[]): string[] {
  const grouped = groupProjectCodexConfigWarningsByPath(warnings);
  if (grouped.length === 0) return [];
  const lines = ["⚠️  Project Codex config bypasses NexCode:"];
  for (const { path, issues, bypass } of grouped) {
    lines.push(`    ${relPath(path)} — ${issues.join(", ")}`);
    lines.push(`    ${bypass}`);
  }
