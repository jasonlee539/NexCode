...
function restoreRuntimeConfig(target: NxcConfig, snapshot: NxcConfig): void {
  for (const key of Object.keys(target) as Array<keyof NxcConfig>) delete target[key];
 ...
 */
export function deleteCodexAccount(runtimeConfig: NxcConfig, accountId: string): boolean {
