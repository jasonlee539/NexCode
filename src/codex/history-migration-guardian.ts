 ...
 *
 * Most upgrades run `nxc start` while the Codex app still holds `state_5.sqlite`,
 * so the inject-time migration often fails on the FIRST start — exactly the moment
 * every legacy thread is still tagged `nexcode` and invisible to the app. Instead
 ...
 * - Started ONLY from `nxc start` (cli handleStart), never from injectCodexConfig —
  ...
  const tick = async () => {
    ...
    try {
      ...
      if (!result.failed) {
        ...
        if (moved > 0) {
          log.log(`🩹 history-migration: ${moved} legacy nexcode thread(s) migrated back to openai.`);
        }
    ...
    if (ticks >= maxTicks) {
      stopped = true;
      log.log("⚠️ history-migration: Could not verify that legacy threads were migrated; the history database may be busy, unavailable, or not yet ready. Run 'nxc sync' (or check 'nxc doctor').");
      return;
    }
    schedule();
  };
