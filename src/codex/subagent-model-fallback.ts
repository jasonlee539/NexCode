...
import type { NxcParsedRequest, NxcConfig } from "../types";
...
type SubagentQuotaPrimeFn = (config: NxcConfig, reason: string) => Promise<void>;
...
function tryRouteFallbackModel(config: NxcConfig, model: string): RouteResult | null {
  try {
...
function isDisabledFallbackModel(model: string, config: NxcConfig): boolean {
...
function pollIntervalMs(config: NxcConfig): number {
...
function quotaThreshold(config: NxcConfig): number {
 ...
 */
function activeCodexAccountId(config: NxcConfig): string | null {
...
function isRoutableFallbackModel(model: string, config: NxcConfig): boolean {
  const slash = model.indexOf("/");
  if (slash > 0) {
...
function rewriteParsedModel(parsed: NxcParsedRequest, model: string): void {
...
function subagentQuotaPrimeBlockedByHostCircuit(config: NxcConfig): boolean {
...
/**
 * Per-primary-model fallback chains from nexcode config (#1190).
 *
...
export function subagentFallbackGuidanceText(config: NxcConfig): string {
  const chain = config.subagentModelFallback ?? [];
  if (chain.length === 0) return "";
  const quoted = chain.map(model => `"${model}"`).join(", ");
  return ` Subagent model fallback chain (priority order): ${quoted}. When the primary model is quota-exhausted, nexcode rewrites thread_spawn requests to the next available model automatically.`;
}
...
export function shouldPrimeSubagentQuota(config: NxcConfig, now = Date.now()): boolean {
