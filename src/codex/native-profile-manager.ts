...
export class NativeProfileManager {
  ...
  private async withLock<T>(operation: () => Promise<T>): Promise<T> {
    ...
    while (!database) {
      ...
      try {
        ...
        candidate.exec("CREATE TABLE IF NOT EXISTS nxc_native_profile_lock (singleton INTEGER PRIMARY KEY CHECK (singleton = 1), claim TEXT NOT NULL)");
        candidate.query("INSERT INTO nxc_native_profile_lock (singleton, claim) VALUES (1, ?) ON CONFLICT(singleton) DO UPDATE SET claim = excluded.claim").run(claim);
        candidate.exec("COMMIT");
        candidate.exec("BEGIN IMMEDIATE");
        const candidateClaim = candidate.query("SELECT claim FROM nxc_native_profile_lock WHERE singleton = 1").get() as { claim?: unknown } | null;
        ...
        try {
          ...
          pathClaim = verifier.query("SELECT claim FROM nxc_native_profile_lock WHERE singleton = 1").get() as { claim?: unknown } | null;
        } finally {
  ...
  private assertNoPendingRecovery(): void {
    ...
    throw new NativeProfileError(
      "RECOVERY_REQUIRED",
      "A native-profile recovery journal is pending. Run `nxc account main recover` or `nxc account main recover --rollback --yes` before registering or adding profiles.",
      409,
    );
  }
  ...
  private assertCurrentIdentity(vault: NativeMainProfileVaultV1, envelope: NativeEnvelopeSnapshot, key: NativeProfileKey): NativeMainProfileRecordV1 {
    ...
    if (nativeIdentityHash(key.key, envelope.accountId) !== active.identityHash) {
      throw new NativeProfileError(
        ...
        "The physical native login changed outside NexCode; recover or register the expected login before continuing.",
        409,
      );
    }
    return active;
  ...
  private assertStagingLayout(requireRoot = false): void {
    ...
    const configDir = assertSafeDirectory(this.context.configDir, "The NexCode configuration root");
    const stagingBase = dirname(this.context.stagingRoot);
    if (!samePath(stagingBase, join(configDir, "native-main-profile-staging"))) {
      throw new NativeProfileError("PROFILE_STORAGE_UNSAFE", "The native-login staging root escaped NEXCODE_HOME.", 409);
    }
    ...
    if (!samePath(dirname(this.context.stagingRoot), canonicalBase)) {
      throw new NativeProfileError("PROFILE_STORAGE_UNSAFE", "The native-login staging root is not contained by NEXCODE_HOME.", 409);
    }
