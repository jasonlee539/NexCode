...
const CODEX_SHIM_REENTRY_DIAGNOSTIC = "nexcode: saved Codex launcher resolved back to the autostart shim; run nxc codex-shim uninstall and reinstall Codex before enabling codexAutoStart.";
...
function cliEntry(): { bun: string; bunRuntimeSource: BunRuntimeSource; cli: string } {
  // Bundled Bun path (survives `nxc update`); all three shim builders
...
export function findCodexOnPath(deps: CodexPathScanDeps = {}): string | null {
  ...
  if (skippedInterop) {
    lastShimDiscoveryError = truncateRetainedUtf8(
      ...
      "Install codex inside WSL (npm i -g @openai/codex), or run 'nxc ensure' from Windows to shim the Windows side.",
      ...
      try {
        if (!lstatSync(exe).isDirectory()) {
          lastShimDiscoveryError = truncateRetainedUtf8(
            ...
            "install a codex.cmd/codex.ps1 launcher or use `nxc service install` for autostart.",
            MAX_DIAGNOSTIC_VALUE_BYTES,
          );
          return null;
        }
...
function backupPathFor(path: string): string {
  ...
  return ext ? `${path.slice(0, -ext.length)}.nexcode-real${ext}` : `${path}.nexcode-real`;
}
...
if [ "\${NXC_SHIM_PROBE:-}" = "1" ]; then
  if [ "\${NXC_SHIM_PROBE_ACTIVE:-}" = "1" ]; then
    if [ -n "\${NXC_SHIM_PROBE_REENTRY_PATH:-}" ]; then
      (umask 077; printf '%s\n' recursive > "$NXC_SHIM_PROBE_REENTRY_PATH") 2>/dev/null || true
    fi
    printf '%s\n' ${shQuote(CODEX_SHIM_REENTRY_DIAGNOSTIC)} >&2
    exit ${CODEX_SHIM_REENTRY_EXIT_CODE}
  fi
  NXC_SHIM_PROBE_ACTIVE=1
  export NXC_SHIM_PROBE_ACTIVE
fi
if [ "\${NXC_SHIM_ACTIVE_PID:-}" = "$$" ]; then
  printf '%s\n' ${shQuote(CODEX_SHIM_REENTRY_DIAGNOSTIC)} >&2
  exit ${CODEX_SHIM_REENTRY_EXIT_CODE}
fi
case "\${NXC_SHIM_ACTIVE_DEPTH:-0}" in
  0)
    NXC_SHIM_ACTIVE_DEPTH=1
    ;;
  1)
    NXC_SHIM_ACTIVE_DEPTH=2
    ;;
  *)
    printf '%s\n' ${shQuote(CODEX_SHIM_REENTRY_DIAGNOSTIC)} >&2
    exit ${CODEX_SHIM_REENTRY_EXIT_CODE}
...
NXC_SHIM_ACTIVE_PID=$$
export NXC_SHIM_ACTIVE_PID NXC_SHIM_ACTIVE_DEPTH
if [ -z "$NEXCODE_API_AUTH_TOKEN" ] && [ -f ${shQuote(tokenFile)} ]; then
  NEXCODE_API_AUTH_TOKEN="$(cat ${shQuote(tokenFile)})"
  export NEXCODE_API_AUTH_TOKEN
fi
nxc_subcommand=""
nxc_skip_next=0
for nxc_arg in "$@"; do
  if [ "$nxc_skip_next" -eq 1 ]; then
    nxc_skip_next=0
    continue
  fi
  case "$nxc_arg" in
...
case "$nxc_subcommand" in
  ${internalCommands}|--help|-h|--version|-V)
    ;;
  *)
    if [ -z "$NXC_SHIM_BYPASS" ]; then
...
function probeUnixShimInstall(wrapperPath: string): UnixShimProbeResult {
  ...
  const probeDir = mkdtempSync(join(tmpdir(), "nexcode-shim-probe-"));
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    NXC_SHIM_BYPASS: "1",
    NXC_SHIM_PROBE: "1",
    NXC_SHIM_PROBE_REENTRY_PATH: reentryPath,
  };
    ...
    try {
      if (entry.originalMovedToBackup && existsSync(target.backupPath)) {
        ...
        if (sourceOccupied) {
          ...
          // `codex.nexcode-real` is recoverable, a deleted launcher is not.
...
export function buildWindowsCodexShim(realCodexPath: string, bunPath: string, cliPath: string, bunRuntimeSource: BunRuntimeSource): string {
  ...
  return `@echo off\r
rem ${SHIM_MARKER}\r
${windowsBatchSet("NXC_REAL_CODEX", realCodexPath)}\r
${windowsBatchSet("NXC_BUN", bunPath)}\r
${windowsBatchSet("NXC_CLI", cliPath)}\r
${windowsBatchSet("NXC_API_TOKEN_FILE", serviceApiTokenFilePath())}\r
if "%NEXCODE_API_AUTH_TOKEN%"=="" if exist "%NXC_API_TOKEN_FILE%" set /p NEXCODE_API_AUTH_TOKEN=<"%NXC_API_TOKEN_FILE%"\r
if not "%NXC_SHIM_BYPASS%"=="" goto run_codex\r
goto scan_codex_args\r
:scan_codex_args\r
if "%~1"=="" goto ensure_nxc\r
if "%~1"=="--" goto ensure_nxc\r
${valueOptionChecks}\r
...
set "NXC_SCAN_ARG=%~1"\r
if "%NXC_SCAN_ARG:~0,1%"=="-" goto shift_codex_arg\r
goto ensure_nxc\r
:skip_option_value\r
shift\r
if "%~1"=="" goto ensure_nxc\r
...
"%NXC_BUN%" "%NXC_CLI%" ensure >nul 2>nul\r
endlocal\r
:run_codex\r
"%NXC_REAL_CODEX%" %*\r
`;
...
# ${SHIM_MARKER}
if (-not $env:NEXCODE_API_AUTH_TOKEN -and (Test-Path -LiteralPath ${psString(tokenFile)})) {
  $env:NEXCODE_API_AUTH_TOKEN = (Get-Content -Raw -LiteralPath ${psString(tokenFile)}).Trim()
}
...
$skipEnsure = $env:NXC_SHIM_BYPASS -or $internalCommands -contains $subcommand -or @("--help", "-h", "--version", "-V") -contains $subcommand
if (-not $skipEnsure) {
...
function writeShim(wrapperPath: string, realCodexPath: string): { dev: number; ino: number } | undefined {
  ...
  if (process.platform === "win32") {
    ...
    const staged = join(dirname(wrapperPath), `.${basename(wrapperPath)}.nexcode-staging.${process.pid}.${randomUUID()}`);
    let renamed = false;
    try {
    ...
    if (unsafe || wrapperChangedDuringProbe) {
      ...
      return {
        ...
        message: wrapperChangedDuringProbe
          ? `Refusing Codex autostart shim because ${reason}. The concurrent launcher was preserved, and your previous launcher was kept alongside it as \`<codex>.nexcode-real\`; retry after the Codex update finishes, and remove that backup once you are satisfied the launcher on PATH is the one you want.`
...
export function diagnoseCodexShim(): CodexShimDiagnostic {
  ...
  const summary = files.map(file => {
    const wrapper = existsSync(file.wrapperPath)
      ? isShim(file.wrapperPath)
        ? "shim present"
        : "present but not an nexcode shim"
      : "missing";
