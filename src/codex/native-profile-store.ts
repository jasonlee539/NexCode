...
const DOMAIN_HOME = "nexcode-native-profile-home-v1\0";
const DOMAIN_INSTANCE = "nexcode-native-profile-instance-v1\0";
const DOMAIN_IDENTITY = "nexcode-native-profile-identity-v1\0";
const KEYRING_SERVICE = "nexcode.native-main-profile.v1";
const SHARED_METADATA_DIR = ".nexcode-native-main-profiles";
  ...
  for (;;) {
    try {
      ...
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") {
        throw new NativeProfileError("PROFILE_STORAGE_UNSAFE", "The NexCode configuration root is not safely accessible.", 409);
      }
...
export function assertNoLegacyNativeProfileState(context: NativeProfileContext): void {
  ...
  throw new NativeProfileError(
    ...
    "Legacy native-profile state exists under NEXCODE_HOME. Stop every NexCode proxy sharing this CODEX_HOME, then migrate or reset the preview state as documented.",
    409,
  );
}
...
export function assertNativeProfileLockPath(context: NativeProfileContext): void {
  const canonicalHome = assertCanonicalDirectory(context.codexHome, "The effective CODEX_HOME");
  const expected = join(canonicalHome, ".nexcode-native-profile.lock.sqlite");
...
export function resolveNativeProfileContext(options: { codexHome?: string; configDir?: string } = {}): NativeProfileContext {
  ...
  return {
    ...
    lockPath: join(codexHome, ".nexcode-native-profile.lock.sqlite"),
  };
}
