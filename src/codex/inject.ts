...
import type { NxcConfig } from "../types";
...
export function externalCodexModelProvider(content: string): string | null {
  ...
  return provider && provider !== "openai" && provider !== "nexcode"
    ? provider
    : null;
}
 ...
 * `model_provider = "nexcode"` + a `[model_providers.nexcode]` table, we set the official
 ...
 * built-in provider cannot carry the `x-nexcode-api-key` env header.
 */
...
export interface InjectCodexOptions {
   ...
   *
   * Bounded by default so a stuck holder cannot wedge `nxc start`; an explicit
...
/**
 * The `[model_providers.nexcode]` TABLE only. A table is position-independent in TOML, so it is
 * safe to append at EOF. The bare root key `model_provider = "nexcode"` is NOT included here —
...
export function shouldInjectApiAuthHeader(
  config: Pick<NxcConfig, "hostname" | "unauthenticatedLoopbackListener"> | undefined,
): boolean {
  ...
  // directly-spawned app-server this exists for has no NEXCODE_API_AUTH_TOKEN in its
...
export function buildProviderTableBlock(
  ...
  if (includeApiAuthHeader) {
    ...
    lines.push('env_key = "NEXCODE_API_AUTH_TOKEN"');
  }
...
/**
 * Design B root-key injection: place `NXC_SECTION_MARKER` + `openai_base_url` at the document
...
export function setRootOpenaiBaseUrl(
  ...
  for (let i = 0; i < rootEnd; i++) {
    ...
    const markerOwned = i > 0 && lines[i - 1].includes(NXC_SECTION_MARKER);
  ...
  lines.splice(insertAt, 0, NXC_SECTION_MARKER, key);
  return { content: lines.join("\n"), keptUserBaseUrl: false };
}
...
export function stripInjectedOpenaiBaseUrl(content: string): string {
  ...
  for (let i = 0; i < rootEnd; i++) {
    if (!lines[i].includes(NXC_SECTION_MARKER)) continue;
...
export type CodexRoutingKind =
  "native" | "nexcode-local" | "custom-local" | "custom-remote" | "unknown";
...
/** Classify actual routing dependency separately from nexcode ownership. */
export function classifyCodexRouting(content: string): CodexRoutingKind {
  ...
  if (rootBaseUrl) {
    ...
    if (hasInjectedOpenaiBaseUrl(content)) return "nexcode-local";
  ...
  if (rootProvider) {
    ...
    if (providerBaseUrl) {
      ...
      if (endpoint === "unknown") return "unknown";
      if (rootProvider === "nexcode") return "nexcode-local";
      return endpoint === "local" ? "custom-local" : "custom-remote";
    }
    if (
      rootProvider === "nexcode" ||
      providerTableExists ||
      rootProvider !== "openai"
    )
      return "unknown";
 ...
 * "nexcode" (wherever it sits — including a previously mis-nested one under a table), plus any
 * ROOT-level model_provider (before the first table) of any value, since we override the global.
 * A `model_provider` legitimately inside a user table/profile with a non-nexcode value is left
 * untouched.
 */
function stripExistingModelProvider(content: string): string {
  ...
  lines.forEach((line, i) => {
    if (/^\s*model_provider\s*=/.test(line)) {
      const isOurs = /^\s*model_provider\s*=\s*"nexcode"\s*$/.test(line);
...
/**
 * Insert `model_provider = "nexcode"` at the document ROOT — immediately before the first table
...
function setRootModelProvider(content: string): string {
  const lines = content.split("\n");
  const firstTable = lines.findIndex((l) => /^\s*\[/.test(l));
  const key = 'model_provider = "nexcode"';
  if (firstTable === -1) {
...
function readRootModelCatalogPath(content: string): string | null {
  ...
  for (let index = 0; index < rootEnd; index += 1) {
    ...
    if (!isNexcodeCatalogPath(catalogPath)) return catalogPath;
...
function setRootModelCatalogPath(content: string, catalogPath: string): string {
  ...
  for (let i = 0; i < rootEnd; i++) {
    ...
    if (isNexcodeCatalogPath(existing)) {
...
function removeProfileSection(content: string): string {
  ...
  for (const line of lines) {
    if (line.trim() === "[profiles.nexcode]") {
      inProfile = true;
      continue;
    }
    if (inProfile) {
      if (/^\s*\[/.test(line) && line.trim() !== "[profiles.nexcode]") {
        inProfile = false;
        filtered.push(line);
      }
      continue;
...
function ensureFastModeFeature(content: string, fastMode?: boolean): string {
  // Tri-state fast mode (see NxcConfig.fastMode): true forces `fast_mode = true`,
...
function isNexcodeCatalogPath(path: string): boolean {
  return path.replace(/\\/g, "/").split("/").pop() === "nexcode-catalog.json";
}

function stripNexcodeCatalogPath(content: string): string {
  const modelCatalogAssignment = tomlStringPattern("model_catalog_json");
  const lines = content.split("\n");
  const firstTable = lines.findIndex((line) => /^\s*\[/.test(line));
  const rootEnd = firstTable === -1 ? lines.length : firstTable;
  return lines
    .filter((line, index) => {
      ...
      return !m || !isNexcodeCatalogPath(parseTomlString(m[1]));
    })
    .join("\n");
}
...
export function buildProfileFile(port: number, catalogPath?: string | null, supportsWebsockets = false, includeApiAuthHeader = false, hostname?: string, fastMode?: boolean): string {
  ...
  // the x-nexcode-api-key env header).
  if (!includeApiAuthHeader) {
    const lines = [
      "# NexCode proxy fallback config (Design B)",
  ...
  const lines = [
    "# NexCode proxy profile — use with: codex --profile nexcode",
    `# Routes all model requests through the nexcode proxy at ${host}:${port}`,
    'model_provider = "nexcode"',
  ];
  if (catalogPath) lines.push(`model_catalog_json = ${tomlString(catalogPath)}`);
  if (fastMode !== undefined) lines.push("", "[features]", `fast_mode = ${fastMode ? "true" : "false"}`);
...
export function chooseCatalogPathForInjection(
  ...
  if (existing) {
    ...
    if (!isNexcodeCatalogPath(resolved) || existsSync(resolved))
      return existing;
  }
...
export async function injectCodexConfig(
  ...
  // port and the config together: startup sync, `nxc sync`, and the ensure path would each
  ...
  if (activeProvider) {
    ...
    return {
      ...
      message:
        ...
        `  NexCode preserves external provider configuration so existing ${tomlString(activeProvider)} session history stays visible.\n` +
        `  Configure that provider for Responses passthrough at http://${providerBaseHost(config?.hostname)}:${port}/v1` +
        `${shouldInjectApiAuthHeader(config) ? ` with x-nexcode-api-key from NEXCODE_API_AUTH_TOKEN` : ""}.\n` +
        `  For direct injection, switch to the built-in openai provider, remove any user-owned root openai_base_url, and rerun 'nxc start'.`,
    };
  }

  // Marker-owned native defaults are NexCode residue, never part of the
  ...
  if (!nativeDefaultsBaseline.ok) {
    return {
      success: false,
      message:
        `Codex config injection refused: existing NexCode-managed native sub-agent defaults are ambiguous: ${nativeDefaultsBaseline.error}. ` +
  ...
  // Design B form FIRST: removeNxcSection also keys on the marker line, so a root-level
  ...
  if (hasNxcProviderTable(content)) {
    content = removeNxcSection(content);
  }
  ...
  content = catalogPath
    ...
    : stripNexcodeCatalogPath(content);
  ...
  if (legacyMode) {
    ...
    // x-nexcode-api-key env header, so keep the nexcode provider table + root re-tag.
  ...
  const routingOwnershipWarning =
    keptUserBaseUrl && desiredSubagentDefaults
      ? "Native Codex sub-agent defaults were not injected: a user-owned root openai_base_url prevents NexCode from managing active Codex routing."
      : undefined;
  ...
  const catalogMessage = catalogPath
    ...
    : `  Codex model catalog not injected because no nexcode catalog file exists yet.\n`;
  ...
  const historyMessage =
    config?.syncResumeHistory === false
      ...
      : history.failed
        ...
        : legacyMode
          ? `  Codex resume history: ${history.rows} thread(s) made visible for nexcode; originals backed up for restore.\n`
          : migratedRows > 0
            ? `  Codex resume history: ${migratedRows} legacy nexcode-tagged thread(s) migrated back to openai (one-time).\n`
  ...
  if (keptUserBaseUrl) {
    return {
      ...
      ...(nativeSubagentDefaultsWarning
        ...
        : {}),
      message:
        `⚠️ Codex routing NOT injected: your config already sets a root openai_base_url, and nexcode never overwrites a user-owned override.\n` +
        catalogMessage +
        historyMessage +
        managedDefaultsMessage +
        `  To route plain codex through the proxy, remove your openai_base_url line from ~/.codex/config.toml and rerun 'nxc start'.\n` +
        `  Reference config: ${CODEX_PROFILE_PATH}`,
    };
  }
  const headline = legacyMode
    ? `Injected nexcode as default provider into Codex config.\n`
    : `Pointed Codex's built-in openai provider at the nexcode proxy (openai_base_url).\n`;
  return {
    success: true,
    ...
    message:
      ...
      historyMessage +
      managedDefaultsMessage +
      `  All models now route through nexcode proxy (like OpenRouter).\n` +
      `  OpenAI models (gpt-5.5, etc.) are passed through to OpenAI.\n` +
      `  Custom models route to their configured providers.\n` +
      (legacyMode
        ? `  Fallback: codex --profile nexcode (same behavior)`
        : `  Fallback reference: ${CODEX_PROFILE_PATH}`),
  };
}

/**
 * Sub-table headers like `[model_providers.nexcode.env_http_headers]` appear when a Codex app
 * config rewrite re-serializes the provider's inline `env_http_headers` table. They define the
 * same `model_providers.nexcode` provider, so cleanup must remove them too — otherwise the
 ...
 * `[model_providers.nexcode_backup]`-style tables out of scope.
 */
function isNxcProviderHeaderLine(trimmedLine: string): boolean {
  // Root form matched by regex, not equality: TOML v1.0 allows a trailing comment
  // (`[model_providers.nexcode] # comment`), and an exact compare would miss that form.
  // The sub-table prefix check already tolerates trailing comments by construction.
  return (
    /^\[model_providers\.nexcode\]\s*(?:#.*)?$/.test(trimmedLine) ||
    trimmedLine.startsWith("[model_providers.nexcode.")
  );
}

function hasNxcProviderTable(content: string): boolean {
  return content
    .split("\n")
    .some((line) => isNxcProviderHeaderLine(line.trim()));
}

function removeNxcSection(content: string): string {
  const lines = content.split("\n");
  const filtered: string[] = [];
  let inNxcSection = false;
  for (const line of lines) {
    if (
      line.includes(NXC_SECTION_MARKER) ||
      isNxcProviderHeaderLine(line.trim())
    ) {
      inNxcSection = true;
      continue;
    }
    if (inNxcSection) {
      ...
      // "[model_providers.nexcode_backup]" (or similar) is preserved, not swallowed.
      if (/^\s*\[/.test(line) && !isNxcProviderHeaderLine(line.trim())) {
        inNxcSection = false;
        filtered.push(line);
      }
      continue;
...
interface StripNexcodeConfigResult {
  content: string;
 ...
 */
function stripNexcodeConfigResult(
  content: string,
  journaledBaseUrl: string | null = null,
): StripNexcodeConfigResult {
  let out = content;
  const hadRootNxcProvider =
    readRootTomlString(out, "model_provider") === "nexcode";
  ...
  out = stripInjectedOpenaiBaseUrl(out); // before removeNxcSection — it keys on the marker line too
  out = stripJournaledOpenaiBaseUrl(out, journaledBaseUrl);
  if (hasNxcProviderTable(out)) {
    out = removeNxcSection(out);
  }
  out = removeProfileSection(out);
  // Regex (not exact-string) removal so compact `model_provider="nexcode"` is stripped too —
  // must match the detection regex above, or a detected line could survive un-removed.
  out = out
    .split("\n")
    .filter((l) => !/^\s*model_provider\s*=\s*"nexcode"\s*$/.test(l))
    .join("\n");
...
/** Pure transform: strip the nexcode provider block + `model_provider = "nexcode"` lines. */
export function stripNexcodeConfig(content: string): string {
  return stripNexcodeConfigResult(content).content;
}

function hasNexcodeRouting(content: string): boolean {
  return (
    hasNxcProviderTable(content) ||
    /^\s*model_provider\s*=\s*"nexcode"/m.test(content) ||
...
export function removeCodexConfig(
  ...
  if (!existsSync(CODEX_CONFIG_PATH)) {
    ...
    return {
      success: true,
      message: `Codex config not found; no native restore was needed${options.preserveProfile ? "." : ", and the nexcode profile was removed if present."}`,
    };
  }
  const rawContent = readFileSync(CODEX_CONFIG_PATH, "utf-8");
  ...
  const had = hasNexcodeRouting(content)
    || (journaledBaseUrl !== null && rootTomlString(content, "openai_base_url") === journaledBaseUrl);
  const stripped = stripNexcodeConfigResult(content, journaledBaseUrl);
  ...
  const removedMessage = had
    ? `Removed nexcode routing from Codex config${options.preserveProfile ? "." : " + profile."}`
    : "nexcode not present in Codex config.";
  if (stripped.managedDefaultsError) {
    const routingMessage = had
      ? removedMessage
      : "No nexcode routing was present in Codex config.";
    return {
      success: false,
      message:
    ...
    message: reason === "permission"
      ...
      : reason === "busy"
        ...
        : detail
          ...
          : "Codex resume history could NOT be restored; the reason was not recorded. Run 'nxc doctor'.",
  };
}
...
function restoreCodexConfigInline(): CodexRestoreConfigResult {
  try {
    ...
    const restored = journal.configRestored
      ? { success: true, message: "Codex config restored from nexcode journal." }
  ...
  const history: CodexRestoreHistoryResult = outcome.kind === "converged"
    ? {
        ...
        message: outcome.rows > 0
          ? `Resume history restored from nexcode backup (${outcome.rows} thread(s)).`
    ...
    : rawHistory.failed
      ...
      : {
          ...
          message: rawHistory.rows > 0
            ? `Resume history restored from nexcode backup (${rawHistory.rows} thread(s)).`
            : "Codex resume history was already native.",
        };
