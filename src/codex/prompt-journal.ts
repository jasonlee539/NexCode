 ...
 *
 * Two files must move together: `nexcode-prompt.json` (the source of truth for
...
export function durableWrite(path: string, content: string): void {
  const tmp = `${path}.nxc.${process.pid}.${randomBytes(4).toString("hex")}.tmp`;
  let fd: number | undefined;
  try {
...
// Envelope
//
// Line 1 is `nxc-journal-v1 <sha256 of line 2>`; line 2 is the record. The
...
const ENVELOPE_TAG = "nxc-journal-v1";
...
export function recoverIfNeeded(
  ...
  if (config === "unknown" || store === "unknown") {
    ...
    return {
      ok: false,
      error: "recovery_required",
      detail: `${which} matches neither the pre- nor post-image; it was modified outside nexcode`,
    };
  }
