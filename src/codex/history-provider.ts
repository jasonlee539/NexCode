 ...
 * `nexcode` after a native restore, that clone re-appends `nexcode` and last-writer-wins
 ...
 * Only length-preserving shrinks are handled (e.g. "nexcode" -> "openai"); callers that would
 * grow the value fall back to append-only, which is correct for the nexcode direction.
 *
...
export type CodexHistoryProvider = "openai" | "nexcode";
...
function toNativeRestoreTarget(entry: BackupEntry): NativeRestoreTarget {
  if (entry.modelProvider !== "nexcode") {
    return {
...
function ejectRemainingNexcodeHistory(db: Database): { rows: number; files: number } {
  const rows = db
    .query<ThreadRow, []>(`
      SELECT id, rollout_path, model_provider, source, has_user_event
      FROM threads
      WHERE model_provider = 'nexcode'
 ...
 * zero threads still tagged nexcode AND an empty backup manifest. Used to skip the
...
function openaiRestoreIsNoop(stateDbPath: string, backupPath: string): boolean {
  const pending = countPendingNexcodeHistory(stateDbPath, backupPath);
...
function syncCodexHistoryProviderUnsafe(provider: CodexHistoryProvider, stateDbPath: string, backupPath: string): CodexHistorySyncResult {
  ...
  try {
    ...
    for (const row of openaiRows) {
      try {
        if (updateSessionMeta(row.rollout_path, row.id, { provider: "nexcode" })) files++;
      } catch {
...
function restoreCodexHistoryProvider(stateDbPath: string, backupPath: string): CodexHistorySyncResult {
  ...
  try {
    if (entries.length === 0) {
      const ejected = ejectRemainingNexcodeHistory(db);
...
export function restoreLegacyOpenaiHistory(stateDbPath = resolveCodexStateDbPath()): CodexHistorySyncResult {
  ...
  const retried = withHistoryRetryResult(() => {
    const db = openStateDb(stateDbPath);
    try {
      return ejectRemainingNexcodeHistory(db);
    } finally {
      db.close();
    }
  });
 ...
 * nexcode-tagged threads to openai. Thin wrapper over the restore path with a
...
export interface PendingHistoryCount {
  /** Threads still tagged nexcode that the eject path WOULD move (mirrors its WHERE). */
  pendingRows: number;
...
/**
 * Read-only migration progress probe for the guardian and `nxc doctor`. Opens sqlite
 * readonly with a SHORT busy timeout so a locked DB cannot stall a daemon tick. The
 * pending predicate mirrors ejectRemainingNexcodeHistory exactly — rows eject ignores
 * (empty first_user_message) are not counted, so 0 really means "migration done".
 */
export function countPendingNexcodeHistory(
  stateDbPath = resolveCodexStateDbPath(),
  backupPath = historyBackupPathFor(stateDbPath),
): PendingHistoryCount {
  let backupEntries = 0;
