...
import type { CodexAccount, CodexAccountCredentials, NxcConfig } from "../types";
...
function configuredPoolAccount(config: NxcConfig, accountId: string): CodexAccount | null {
...
async function verifyCodexAccountWarmup(
  ...
  | { ok: true; validatedAt: number; validationFailureReason?: undefined }
  | { ok: true; validatedAt?: undefined; validationFailureReason: string }
...
> {
  try {
    ...
    // OAuth already proved that the credential can be issued. A temporary Codex
    // transport/model/rate-limit failure should not make the freshly authorized
    // account disappear from the app; publish it as unvalidated so later health
    // checks (and the token guardian when enabled) can retry. Only an explicit
    // upstream auth rejection blocks admission.
    if (err instanceof CodexWarmupError
      && !(err.code === "http_status" && (err.status === 401 || err.status === 403))) {
      return { ok: true, validationFailureReason: reason };
    }
    return {
      ok: false,
...
function isRuntimeConfig(config: NxcConfig): boolean {
  return !!config && typeof config === "object" && !!config.providers;
}

function getRuntimeConfig(config: NxcConfig): NxcConfig {
  return isRuntimeConfig(config) ? config : loadConfig();
}

function saveRuntimeConfig(sourceConfig: NxcConfig, nextConfig: NxcConfig): void {
  saveConfigPreservingClaudeCode(nextConfig);
  if (sourceConfig === nextConfig || !isRuntimeConfig(sourceConfig)) return;
  for (const key of Object.keys(sourceConfig) as Array<keyof NxcConfig>) {
...
function persistNewCodexAccount(
  ...
  return withConfigMutationLockSync(() => {
    ...
    try {
      ...
      for (const key of Object.keys(runtimeConfig) as Array<keyof NxcConfig>) {
    ...
    try {
      ...
      if (staged.validatedAt !== undefined) {
        markCodexAccountValidated(addedAccount.id, staged.validatedAt);
      } else if (staged.validationFailureReason) {
        markCodexAccountValidationFailed(addedAccount.id, staged.validationFailureReason);
      }
...
/**
 * Keep the main-account card useful when WHAM is temporarily unreachable. Pool
 * accounts already fall back to getAccountQuota(); main must follow the same rule
 * or one failed network probe erases only its bar from an otherwise healthy list.
 */
function fallbackMainAccountInfo(localTokenEmail: string | null): MainAccountInfo {
  const cached = getMainAccountInfoCache();
  const stored = getAccountQuota(MAIN_CODEX_ACCOUNT_ID);
  const storedQuota = stored
    ? (({ updatedAt: _updatedAt, ...quota }) => quota)(stored)
    : null;
  return {
    email: cached?.email ?? localTokenEmail,
    plan: cached?.plan ?? getMainAccountPlan() ?? null,
    quota: cached?.quota ?? storedQuota,
  };
}
...
async function fetchMainAccountInfoWhileOwned(
  ...
  if (tokenRead.status !== "ok") {
    ...
    return { info: fallbackMainAccountInfo(null), credentialChecked: true, hasCredential: false };
  }
  ...
  // The local credential's ID token carries the account identity even when the WHAM
  // quota endpoint is temporarily unavailable. Use it only as a display-name
  // fallback; the DTO still masks the email and health remains authoritative.
  const localTokenEmail = extractEmail(tokens.id_token, tokens.access_token) ?? null;
  const cached = getMainAccountInfoCache();
  if (!forceRefresh && cached && Date.now() - cached.ts < MAIN_CACHE_TTL) {
    const fallback = fallbackMainAccountInfo(localTokenEmail);
    return {
      info: {
        email: cached.email ?? fallback.email,
        plan: cached.plan ?? fallback.plan,
        quota: cached.quota ?? fallback.quota,
      },
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return {
        info: terminalAuthFailure
          ? { ...EMPTY_MAIN_ACCOUNT_INFO, email: localTokenEmail }
          : fallbackMainAccountInfo(localTokenEmail),
        credentialChecked: true,
        hasCredential: true,
      };
    }
    ...
    const result = {
      email: data.email ?? localTokenEmail,
      plan,
      quota,
      ts: Date.now(),
    };
    ...
    const retried = await retryMainAccountInfoIfIdentityChanged(requestAccountId, retriesRemaining, nativeMainLease, explicitRefresh);
    return retried ?? {
      info: fallbackMainAccountInfo(localTokenEmail),
      credentialChecked: true,
      hasCredential: true,
    };
  }
 ...
 */
function reconcileFreshPoolAccountPlans(runtimeConfig: NxcConfig, updates: FreshPoolPlanUpdate[]): void {
...
export async function runCodexCooldownRecoveryProbes(config: NxcConfig, now = Date.now()): Promise<void> {
...
export function registerCodexCooldownRecoveryProbeWorker(config: NxcConfig): void {
...
export async function primeCodexPoolQuotas(
  ...
  primeInFlight = (async () => {
    ...
    if (process.env.NEXCODE_DEBUG_QUOTA === "1") {
...
export function effectiveCodexAuthAccountId(config: NxcConfig): string {
...
export async function listCodexAuthAccounts(config: NxcConfig, forceRefresh = false): Promise<CodexAuthAccountDto[]> {
...
function selectFallbackAfterPause(config: NxcConfig, pausedActiveId: string): void {
            ...
            if (st.done && st.loggedIn) {
              ...
              if (cred) {
                ...
                if (existingIdx >= 0) {
                  ...
                  if (warmup.validatedAt !== undefined) {
                    markCodexAccountValidated(accountId, warmup.validatedAt);
                  } else {
                    markCodexAccountValidationFailed(accountId, warmup.validationFailureReason);
                  }
                  ...
                  newAccountPersistence = persistNewCodexAccount(
                    ...
                    addedAccount,
                    {
                      ...
                      validationFailureReason: warmup.validationFailureReason,
                    },
                  );
