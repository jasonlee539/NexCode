 ...
 *
 * The switch itself was never the hard part — `nxc restore` already returns Codex
 * to its native path without stopping the proxy. What was missing is that the
 * decision did not survive: `nxc start` force-synced unconditionally, so an OFF
 ...
 * `tmpdir()/nexcode-native-locks/sha256(home).sqlite`; that keys on the home
...
export function integrationEnabled(
  config: Pick<NxcConfig, "clientIntegrations">,
  client: DurableIntentClientId,
): boolean {
  return config.clientIntegrations?.[client] !== false;
}

export function codexIntegrationEnabled(config: Pick<NxcConfig, "clientIntegrations">): boolean {
 ...
 * `~/.grok/config.toml` and records nothing, so the next `nxc start` calls
 * `syncGrokConfig` unconditionally and writes the fence straight back.
 */
export function grokIntegrationEnabled(config: Pick<NxcConfig, "clientIntegrations">): boolean {
...
export function claudeDesktopIntegrationEnabled(config: Pick<NxcConfig, "clientIntegrations">): boolean {
 ...
 *
 * `nxc start` used to call `syncModelsToCodex(port).catch(() => {})`
 ...
 *
 * Grok's toggle shipped and then `nxc start` called `syncGrokConfig`
 ...
 */
export function shouldSyncGrokOnStart(config: Pick<NxcConfig, "clientIntegrations">): boolean {
