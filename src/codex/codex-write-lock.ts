...
import { nexCodeCoordinatorTransaction } from "./transition-state";
import {
...
export function canonicalizeCodexHome(
  ...
  const lockId = createHash("sha256")
    .update("nexcode-codex-write-lock-v1\0")
    .update(normalized)
    .digest("hex");
...
export async function withCodexWriteLock<T>(
  ...
  for (;;) {
    ...
    let transaction: ReturnType<typeof nexCodeCoordinatorTransaction> | undefined;
    try {
      transaction = nexCodeCoordinatorTransaction(databasePath);
    } catch (error) {
