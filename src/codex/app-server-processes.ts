 ...
 * in-memory model catalog after `nxc sync` rewrites on-disk files (#476).
 *
...
export const STALE_CODEX_APP_SERVER_HINT =
  "If Codex still shows an older model list, restart its long-lived app-server process after sync (nxc sync --restart-codex).";
...
export function parseWindowsSnapshotOutput(output: string): ProcessSnapshot[] {
  ...
  for (const line of output.split(/\r?\n/)) {
    ...
    if (line.trim() === "__NXC_ENUM_INCOMPLETE__") throw new Error("windows_enum_incomplete");
...
function windowsSnapshotPowerShellCommand(): string {
  ...
  return [
    ...
    "    if($null -eq $o -or $o.ReturnValue -ne 0 -or [string]::IsNullOrWhiteSpace($o.User)){\"__NXC_ENUM_INCOMPLETE__\"; return}",
...
export function formatStaleCodexAppServerWarning(
  ...
  return (
    ...
    + "Re-run with `nxc sync --restart-codex` (or `nxc sync-cache --restart-codex`) to send SIGTERM only to matching app-server processes. "
 ...
 * an in-memory copy that disagrees with what nxc advertises.
 *
 ...
 *   `nxc sync --restart-codex` is consenting to that; a login is not.
