...
NexCode can inspect Codex's persistent diagnostic-log database and, when you opt in, reduce which diagnostic rows Codex persists. Inspection stays read-only; protection is an explicit mutation that is refused unless the known Codex log schema is present and Codex is stopped.

## What Inspect reports

NexCode resolves Codex's effective `sqlite_home` using Codex's existing precedence and inspects the canonical `logs_2.sqlite` database there. A higher-numbered or legacy `logs_N.sqlite` file is never substituted as the mutation-capable target.
...
NexCode does not select or expose `feedback_log_body` while producing these diagnostics. Log levels are reduced to the fixed known level set plus `OTHER`, and target names are not serialized.

## Protect modes

Protection is **off by default**. Enabling it installs one NexCode-owned `BEFORE INSERT` trigger in Codex's canonical `logs_2.sqlite` database. NexCode never replaces an unknown trigger using its reserved names and removes only triggers whose SQL matches the NexCode-owned version.
...
Before Protect, Disable, or Repair changes the foreign database, NexCode:
...
7. changes only NexCode-owned Log Guard triggers and reads the result back before commit; and
8. persists the requested mode in NexCode configuration while the Log Guard lock is still held.

If process enumeration is uncertain, the database is busy, the schema is unknown, or a reserved trigger name belongs to different SQL, the mutation fails closed. NexCode does not terminate Codex automatically.

## Drift and Repair

The requested protection mode is stored in NexCode configuration separately from Codex's log database. This matters because a Codex migration can rebuild the `logs` table, and SQLite drops triggers attached to a table that is replaced.

When the saved mode is `compat` or `quiet` but the corresponding owned trigger is no longer observed, Log Guard reports **drifted**. `nxc doctor` reports the drift but never repairs it automatically.

Repair is explicit:

```bash
nxc storage codex-logs repair
```

NexCode deliberately does not recreate protection on every startup. A later release can reconsider automatic repair only after there is enough field evidence that doing so is safe across Codex migrations.
...
```bash
nxc storage codex-logs status
nxc storage codex-logs status --json
nxc doctor
```
...
```bash
nxc storage codex-logs protect
```

Choose quiet mode explicitly:

```bash
nxc storage codex-logs protect --mode quiet
```

Disable NexCode protection or repair drift:

```bash
nxc storage codex-logs unprotect
nxc storage codex-logs repair
```
