---
title: Integrations
description: Connect nexcode to OpenCode, Pi, OMP, Hermes, OpenClaw, Kimi Code, Gajae Code, DeepSeek Harness, MiniMax Code and Prime Agent from the dashboard — one switch per client, with a backup taken before every write.
---

The **Integrations** tab writes nexcode's provider block into a client's own config
...
|---|---|---|---|---|
| OpenCode | `~/.config/opencode/opencode.json` | JSON | next direct launch | `NEXCODE_OPENCODE_API_KEY` |
| Pi | `~/.pi/agent/models.json` | JSON | new sessions | loopback placeholder |
| OMP | `~/.omp/agent/models.yml` | YAML | after restarting OMP | `nexcode-loopback` placeholder |
| Hermes | `~/.hermes/config.yaml` | YAML | new sessions | `NEXCODE_HERMES_API_KEY` |
| OpenClaw | `~/.openclaw/openclaw.json` | JSON5 | immediately, on a running gateway | `NEXCODE_OPENCLAW_API_KEY` |
| Kimi Code | `~/.kimi-code/config.toml` | TOML | on restart, or `/reload` | loopback placeholder |
| Gajae Code | `~/.gjc/agent/models.yml` | YAML | new sessions, or when you open `/model` |`NEXCODE_GAJAE_API_KEY` |
...
Managed DSH support has a compatibility floor of **DSH 0.1.0-rc.6**. NexCode owns only
`llm-pi-ai.providers.nexcode`; Apply and Refresh replace that fragment, Disable removes only that
...
back to `~/.minimax`. Its managed block owns only `custom_provider.nexcode`.
It does not change `defaultModel`, the selected MiniMax credential source, or
the user's MiniMax login. Choose a `custom_provider:nexcode/<provider/model>`
...
`providers.nexcode`, so other providers and any `modelOverrides` you have set
...
is deliberately loopback-only; remote `x-nexcode-api-key` wiring is deferred. Relocated
...
nexcode reads these from its own environment. If your gateway runs with a profile
or a relocated home, start nexcode with the same variables set, or it will
correctly follow a different installation.

## The other four surfaces are not switches

**API Keys** manages nexcode's own credentials and is not a client at all. **Codex
CLI** is wired by the proxy service itself — starting nexcode applies it, stopping it
...
Disable removes only the entries nexcode recorded as its own. If your file changed
...
patches only its own `providers.nexcode` range byte-wise, so the rest of the
...
OMP and DSH are the exceptions: their YAML writers patch only `providers.nexcode` and
`llm-pi-ai.providers.nexcode`, respectively, preserving
...
The first four have no config field for the `x-nexcode-api-key` header a non-loopback bind
...
`x-nexcode-api-key` credential wiring. Manual remote OMP configuration is outside the
managed integration for now.

**Kimi Code cannot hold an environment reference,** so its config carries an
`nexcode-loopback` placeholder rather than a key. No real credential is ever written
into any client config.

**For `nxc opencode`, the launcher's provider block wins.** That launcher injects
`provider.nexcode` through `OPENCODE_CONFIG_CONTENT`, which outranks the same entry on
...
Once connected, `nxc sync` also refreshes the owned MCode block with current context
...
commands use MiniMax's Anthropic-compatible endpoint, so NexCode provides a
