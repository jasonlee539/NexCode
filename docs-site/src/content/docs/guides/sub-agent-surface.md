...
own context and tools, so several independent tasks can run in parallel. nexcode controls which
...
nexcode applies this as the final pass to both the live `/v1/models` catalog and the catalog synced
...
- `injectionModel` is the preferred worker model named in nexcode guidance.
...
`multiAgentGuidanceEnabled` defaults to on and is the master switch for nexcode-authored guidance
on both surfaces. Turning it off suppresses both the v2 designation block and v1 proactive text.

For array-form stateless Responses requests, nexcode places generated guidance after leading
...
The built-in v2 guidance has a 700-character budget. If it would exceed the budget, nexcode drops
...
On v1, nexcode injects only the upstream-style proactive delegation guidance at `max` or `ultra`
effort. It does not add a preferred model, roster, fallback list, or custom prompt on v1.

The default-off `syncCodexSubagentDefaults` option is separate from guidance. When nexcode owns
...
nexcode updates or removes only fields bearing its markers. If either target field is user-owned,
...
For a spawned worker, nexcode builds this priority order:

1. The requested primary model.
2. A per-model chain from `subagentModelFallbackByModel` in nexcode config, keyed by
   the requested primary model.
3. The global `subagentModelFallback` list in nexcode config.

Per-role fallback chains belong in nexcode config, not in
...
(#1190). nexcode can still read a legacy `model_fallback` line from the TOML for
backwards compatibility, but `nxc doctor` warns about it and Codex itself will ignore
the affected role.

Duplicate model ids are removed while preserving the first occurrence. During selection, nexcode
...
known [#92 limitation](https://github.com/lidge-jun/nexcode/issues/92).

nexcode fails safely instead of forwarding an empty or unreadable task:
...
Use `nxc v2` for the collaboration surface and native feature settings:
...
Use `nxc agent` for delegation, roster, effort-cap, and fallback settings:
...
Pass `-` to clear a nullable `nxc agent injection` value, or use the relevant `clear` action for a
...
catalog state, run `nxc sync` and restart that Codex surface.

### What happens when nexcode cannot trust the catalog?

nexcode compares the on-disk model catalog against the start time of every Codex app-server owned
...
| `stale` | At least one app-server predates the catalog | **No nexcode-authored model guidance** |
| `unknown` | The comparison could not be made | **No nexcode-authored model guidance** |

For `stale` and `unknown`, nexcode withholds its own disk-derived claims — preferred model, roster,
...
separately by `nxc doctor`. `stale` clears only after every detected Codex app-server starts after
...
slow or failing CIM query can delay or suppress only NexCode-authored model guidance; it does not
...
tier that Codex converts to `max`; nexcode then maps or clamps the value for the selected provider.
