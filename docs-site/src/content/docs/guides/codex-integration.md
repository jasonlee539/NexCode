...
description: How nexcode injects itself into Codex, syncs the model catalog, installs shims, and restores cleanly.
---

nexcode makes Codex route through the proxy by editing two things Codex reads: its config
...
`nxc init`, `nxc start`, and `nxc sync` call the injector. On the default loopback bind, it keeps
Codex's built-in `openai` provider id and points that provider at nexcode:
...
points at nexcode, the proxy relays those calls to the OpenAI upstream.
...
- **Google Antigravity (CCA) fallback:** when neither an OpenAI forward candidate nor a keyed
  ...
  requires `nxc login google-antigravity`; the OAuth token is sent only to the pinned CCA registry
...
providers, nexcode lowers Codex's private `image_gen` namespace to an upstream-safe
`image_gen__<inner-name>` alias (for example `image_gen__imagegen`). When that usable alias replaces
the client declaration, nexcode removes a duplicate hosted `image_generation` declaration. It maps
...
When NexCode owns routing, both modes write `$CODEX_HOME/nexcode.config.toml` as a
...
Codex CLI, TUI, App, and SDK all read the same Codex home. nexcode resolves that directory from
...
On WSL, if `CODEX_HOME` is unset and the Linux `~/.codex/config.toml` is absent, nexcode also
...
Codex can keep SQLite-backed thread state in a separate directory. NexCode history operations use
...
If `config.toml` or its root `sqlite_home` key is absent, NexCode continues to the
...
home while the ChatGPT/Codex app still reads `%USERPROFILE%\\.codex`. `nxc status` and `nxc doctor`
...
aligned with native Codex. nexcode also serves `/v1/responses` over WebSocket. The dedicated
...
resume history needs no remapping. On first sync it also migrates threads tagged by older nexcode
builds back to `openai`. Non-loopback dedicated-provider mode still mirrors history under the
`nexcode` provider while active and restores the backed-up metadata on exit. Set
`syncResumeHistory: false` to leave history untouched.

## Model catalog sync

Codex shows models from an on-disk catalog (`$CODEX_HOME/nexcode-catalog.json` by default). On
start and on `nxc sync`, nexcode:

1. **Backs up** the pristine catalog once to `~/.nexcode/catalog-backup.json` (so featuring is
   reversible).
...
though it contains no authoritative transition row. `nxc doctor` reports the exact coordinator path
...
For a state that doctor proves is a zero-byte creation remnant, stop the NexCode proxy/service
and run:
...
`exec` entrypoint and nested MCP tools, including Browser and Computer Use, while nexcode routes
only the model's ordinary function call. Tool execution, permissions, and confirmations remain
local to Codex; nexcode does not implement a second browser or desktop-control executor.

For key-auth Responses providers that do not accept Codex's `exec` custom-tool grammar, nexcode
...
After `nxc sync` changes this metadata, restart Codex App and open a fresh task. Existing app-server
...
The response is the raw `nexcode-catalog.json` document (no provider credentials). When
available, the `x-nexcode-codex-version` header reports the Codex runtime version on the
...
The display name is **display-only and stable across regeneration**. Every `nxc sync` and catalog
...
offline login, the previously persisted catalog is retained and the next successful `nxc sync`
...
If `config.toml` already selects a provider other than `openai` or `nexcode`, NexCode leaves the
...
Keep one tool as the owner of Codex provider configuration. To use NexCode behind an existing
...
enabled, also pass `x-nexcode-api-key` from `NEXCODE_API_AUTH_TOKEN`, matching the non-loopback
provider form above. To let NexCode inject routing directly, first switch Codex back to its
built-in `openai` provider and remove any user-owned root `openai_base_url`, then rerun `nxc start`.
...
Separately, `tool_search` is a client-executed Codex discovery surface. It is not an NexCode
feature flag, and an upstream `tool_choice: "auto"` value does not create or enable it. NexCode can
...
For routed chat/local models, NexCode exposes that declaration as a normal function named
`tool_search`. If the model calls it, NexCode converts the call back to a Responses
...
1. **No `type: "tool_search"` in the incoming request:** the active Codex client/session did not
   advertise the explicit `tool_search` surface. NexCode cannot invent that declaration. This
...
2. **The incoming declaration exists, but no `tool_search` function reaches the routed request:**
   capture only the redacted tool-type/name list and open an NexCode bug. Never attach the bearer,
...
4. **A call is emitted repeatedly or loaded tools never become usable:** capture the redacted
   `tool_search_call` / `tool_search_output` item types and call ids. NexCode preserves both in
...
3. **`liveModels: false` with empty `models`** — when live discovery is off and `models` is empty or
   omitted, nexcode exposes no routed models for that provider.
...
5. **Cache and `nxc sync`** — live catalogs are cached for about five minutes (`modelCacheTtlMs`,
   default `300000`). Run `nxc sync` to force a fresh fetch and rewrite the catalog immediately.
6. **Running Codex `app-server`** — rewriting the on-disk catalog is not enough while a long-lived
   Codex `app-server` (Desktop / CLI background host) keeps the previous list in memory. `nxc sync`
   and `nxc sync-cache` warn when those processes are detected. Restart them with
   `nxc sync --restart-codex` (or stop the matching `app-server` processes yourself), then let Codex
   recreate them so the new list appears.

:::caution[Other local writers]
Catalog writes (`nexcode-catalog.json`, `config.toml`) are atomic **inside** nexcode, which only
prevents half-written files when two nexcode-owned writers race. That does **not** stop another
local process, file watcher, or sync agent from rewriting catalog visibility or order after nexcode
has written. Codex keeps its separate `models_cache.json` and can refresh it independently, changing
the visible list without rewriting `nexcode-catalog.json`. If models flip unexpectedly while the
proxy is running, stop or reconfigure the competing writers, then run `nxc sync` — this is an
external-writer hazard, not a confirmed nexcode defect.
:::
...
— or Claude Code reports a similar connection failure — the nexcode proxy is not
...
`nxc status` shows whether the proxy is running and prints the same restart hint when
it is not; `nxc doctor` reports restart safety (service/shim coverage).
...
When a ChatGPT account is added to the Codex account pool, nexcode verifies it before persistence
...
nexcode never traps you. **`nxc stop` is the single command that fully reverts to native Codex** — it
stops the proxy, stops the background service if one is installed, and strips every injected line and
routed catalog entry so plain `codex` works exactly as if nexcode was never there:
...
When nexcode runs as a managed [background service](/reference/cli/#nxc-service), it sets
`NXC_SERVICE=1` so a service-driven restart does **not** thrash the Codex config — only an explicit
`nxc stop` / `nxc service stop` restores native Codex.
