...
routed by nexcode. When enabled, a synthetic `video_gen` tool is injected into the conversation.
The model calls it like any function tool; nexcode intercepts the call, submits a video generation
job to xAI, polls until completion, and downloads the result.

## Prerequisites

- An `xai` provider entry with an **API key** (`nxc login xai` alone is not sufficient — the video bridge requires key auth, not OAuth)
- A non-OpenAI model as your routed provider (e.g. Anthropic Claude, Google Gemini)
- nexcode configured to route through the non-OpenAI provider
...
> ```
>
> If you onboarded via `nxc login xai` (OAuth), the provider stays in `authMode: "oauth"`
...
1. nexcode detects a non-OpenAI routed model with `videoBridgeEnabled: true`
2. A synthetic `video_gen` function tool is injected into the conversation
3. When the model calls `video_gen`, nexcode submits a job to xAI's `/videos/generations`
