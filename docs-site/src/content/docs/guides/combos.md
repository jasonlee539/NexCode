...
client requests `combo/<id>`; nexcode chooses a target, rewrites the request to that concrete
...
`anthropic/claude-opus-4-8`. If that attempt has a retryable failure, nexcode can hop to
`openai/gpt-5.6-sol`.
...
The combo id in `nxc combo set <id>` must start with a letter or number. It may then contain
...
The alias must be one of the native model ids supported by this nexcode release; a native-family
...
valid `Retry-After` value, nexcode uses it instead. Numeric seconds and HTTP-date values are
...
If the request has no `reasoning` object, nexcode creates one. If `reasoning` exists without an
`effort` property, it preserves the other fields and adds the default. A caller-provided effort is
never overwritten.

When target capability is unknown or does not include the configured effort, nexcode omits the
...
There is one important limitation for Codex v2 sub-agents ([issue #92](https://github.com/lidge-jun/nexcode/issues/92)).
...
including after a retryable failure. If the combo has no decrypt-capable target, nexcode stops
...
`remove`; and the same subcommands are available under `nxc route combo`.
...
`nxc combo list`, check spelling and case, and confirm your management command wrote to the same
running nexcode instance that receives model requests.
