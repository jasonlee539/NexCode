---
title: opencode
description: Use any routed model from opencode — nexcode injects a runtime provider block and leaves your own opencode config untouched.
---

opencode reads its providers from merged JSON config layers rather than environment
variables, so there is no `ANTHROPIC_BASE_URL`-style slot to inject. `nxc opencode`
...
`provider.nexcode` block injected for that process. Extra arguments pass through:
`nxc opencode run "hello"`.

Routed models appear in the picker under the `nexcode` provider:
...
read global or project config to detect a `provider.nexcode` override, while your
existing providers, agents, keybinds, MCP entries, and relative `{file:…}` references
keep resolving from their original files.

For this launch only, nexcode adds the generated `provider.nexcode` block through
...
| Layer | Behavior with `nxc opencode` |
| --- | --- |
| Global / custom / project config | Left on disk exactly as you wrote it |
| Inline runtime (`OPENCODE_CONFIG_CONTENT`) | Receives only the generated `provider.nexcode` block |
| Relative `{file:…}` paths | Still resolve against the config file that originally defined them |

If a global or project config also defines `provider.nexcode`, the launcher prints an
informational note: the runtime layer from `nxc opencode` overrides it for that launch.

## Putting the block into your own config

`nxc opencode` injects the provider block for one launch only, which means plain `opencode` still
knows nothing about the proxy. When you want routed models available from plain `opencode` — or
from an editor extension that never goes through the launcher — `nxc export` prints the same
...
Merge the `provider.nexcode` block into your existing config. Replacing the whole file with the
exported one destroys your other providers, agents, keybinds, and MCP entries. `nxc export --out`
...
catalog. Re-run `nxc export` after you add a provider or change model visibility.
...
`apiKey`; non-loopback binds send it only through `x-nexcode-api-key` so proxy
...
`NEXCODE_API_AUTH_TOKEN` takes precedence, then the hardened service token file, then
...
see [Remote access](/reference/configuration/#remote-access). This admission key is nexcode's
own, and is unrelated to the upstream provider keys configured under
[Providers](/guides/providers/).

## Reverting

Nothing to undo — no generated config file is written under `~/.nexcode`. Run plain
...
The `nexcode` provider block is regenerated on every launch, so per-model tweaks made inside it
