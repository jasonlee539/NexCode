---
title: Pi
description: Use any routed model from Pi — nxc export writes a custom provider block for Pi's models.json, wired to the running proxy.
---

Pi reads its providers from a single global JSON file rather than environment variables, so
nexcode does not launch it. Instead, `nxc export` serializes the `nexcode` provider block —
...
`nxc export` never writes that file. Merge the `providers.nexcode` block into it — replacing the
...
The exported block is a static snapshot, not a live view. Re-run `nxc export` after adding a
...
| --- | --- | --- |
| Proxy admission key | nexcode's own credential, generated on the dashboard's **API** tab | referenced by `apiKey` as `$NEXCODE_API_KEY`; the value stays in your environment |
| Provider key | your Anthropic / OpenAI / OpenRouter key | nexcode's own config, per [Providers](/guides/providers/) |
...
(`NEXCODE_OPENCODE_API_KEY`, in `{env:…}` form) — see the [opencode guide](/guides/opencode/).

**A loopback proxy needs no key at all.** nexcode binds `127.0.0.1` by default and authenticates
nothing there, so the `$NEXCODE_API_KEY` reference is inert and you can leave the variable unset.
...
`nxc export` prints how many rows fell into that case.
...
Two fields are deliberately absent. `cost` requires all four price fields and nexcode has no
...
reasoning-free. Pi then offers its effort control for exactly the models nexcode will accept it
...
[open an issue](https://github.com/lidge-jun/nexcode/issues) with what Pi reported.
:::

## Requirements

A running nexcode proxy (`nxc start`) and Pi installed. `nxc export` reads the live catalog
