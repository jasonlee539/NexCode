---
title: Web Dashboard
description: The nexcode GUI for proxy health, providers, models, delegation guidance, auth pools, usage, and logs.
---

nexcode ships a local web dashboard (a Vite/React app under `gui/`) served from the proxy. It is the
...
the admin token (`NEXCODE_ADMIN_AUTH_TOKEN`, or the auto-generated
`~/.nexcode/admin-api-token` file).
...
| **Sub-agent delegation** | Choose a native or routed model and optional reasoning effort shared by NexCode delegation guidance and the separate native-default opt-in. This is not a proxy-side per-spawn router; see below. |
...
| **Codex autostart** | Allow an already-installed Codex launcher shim to run `nxc ensure`. This toggle does not install a shim or background service. |
...
`injectionEffort`. **NexCode multi-agent guidance** independently controls the delegation
...
native `[agents]` defaults on the next sync/restart when NexCode manages the active Codex routing.
...
:::caution
Neither control is a proxy-side cross-model spawn router. NexCode guidance asks Codex to pass
...
use pattern. NexCode does not endorse using additional accounts to circumvent rate limits, quotas,
...
pattern. Provider restrictions, suspension, or termination are outside NexCode's control;
...
- Each account card carries a **Selection order** control (First, Earlier, Normal, Later, Last).
  ...
  be set to **Last** and kept as the reserve. An order set from `nxc account priority` outside those
...
- Account add, remove, and picker-setting changes are saved before the model catalog is refreshed.
  ...
  run `nxc sync` to retry. The account or setting change itself remains saved.
...
The sidebar's star button — and the one-time question `nxc start` asks in an interactive
terminal — goes through **your own `gh` login**. nexcode holds no GitHub token, and the
...
- `nxc start` and `nxc service install` **skip the prompt entirely** when an agent or CI
