---
title: Claude Code
description: Use any routed model from Claude Code — nexcode serves the Anthropic Messages API and gateway model discovery on the same port.
---

nexcode serves `POST /v1/messages` (plus `count_tokens`) alongside `/v1/responses`, so Claude
...
You can log in multiple Claude accounts via the Providers dashboard (`nxc login anthropic` /
...
`nxc claude` ensures the proxy is running, then launches Claude Code with the environment wired:
...
Variables you export yourself always win. Extra arguments pass through: `nxc claude -p "hello"`.
...
silently disabled a healthy claude.ai subscription in favour of API billing. `nxc claude` now
...
depends on something nexcode can look up, so by default it does.

Leave **Auth mode** on **Auto** (the default) in **Claude → Claude Code** and nexcode
...
next `nxc claude` with nothing to reconfigure.
...
`claude` launched outside `nxc` behaves the same way. That file is a snapshot refreshed when
the proxy starts or you save settings, while `nxc claude` always resolves live.
...
`nxc claude desktop` and `apply` both write the current profile to Claude Desktop. `show` gives a
...
`NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR` for an explicit library override or
...
When `claudeCode.systemEnv` is set to `true` (default: **off**), `nxc start` uses `launchctl setenv`
...
requiring the `nxc claude` wrapper. Already-open shells are unaffected and must be reopened.

`nxc stop` and proxy shutdown **unset the injected keys** (it does not restore previous values —
only the keys nexcode injected are removed). The proxy also writes `~/.nexcode/claude-env.sh`;
`nxc start` installs a `.zshrc` source hook that loads it automatically only when an executable
Claude Code CLI is present on `PATH`. Startup and `nxc ensure` remove the NexCode-owned hook when
...
feature is macOS-only; on other platforms, use `nxc claude`.
...
`x-nexcode-api-key`, and `origin` are always stripped before forwarding. On a non-loopback bind,
native passthrough also requires a valid proxy credential in `x-nexcode-api-key`; `Authorization`
...
"claude.ai connectors are disabled" warning no longer appears with `nxc claude`.
...
with `claude` or `anthropic`, nexcode exposes routed models as stable, reversible aliases:

| Surface | Format | Example |
| --- | --- | --- |
| Claude Code CLI | `claude-nxc-<provider>--<model>` (plain) or `claude-nxc2-…` (escaped) | `claude-nxc-native--gpt-5.6-sol` |
...
and `claude-nxc-<provider>--<model>` ids from older configs still resolve.

If Claude Desktop's footer picker does not change the model for an already-running 3P
conversation, use `/model <id>` in that conversation. NexCode cannot observe picker state; it
...
Plain model ids (no `/` or `~`) keep the v1 prefix `claude-nxc-…`. Model ids that contain `/` or
`~` mint the v2 prefix `claude-nxc2-…` with escapes (`/` → `~s`, `~` → `~t`), e.g.
...
`effectiveModelEnv` computes six slots injected by `nxc claude` / system env / shell file:
...
When both `tierModels.haiku` and `smallFastModel` are absent, NexCode leaves both helper variables unset; Claude Code then chooses its native helper model (currently Sonnet), which may incur native-provider charges.

## Roster agents (injectAgents)

Proxy startup/ensure, `nxc claude`, and relevant dashboard saves sync your featured subagent roster
(Subagents tab, up to 5 models) plus `nxc-self` into `~/.claude/agents/nxc-*.md`.

- **`nxc-self`** pins your `/model` picker default (falling back to `claudeCode.model`); omitted
  when neither exists. It does NOT use model inheritance.
- Each agent body contains an `<!-- nxc-route: <model> -->` directive — the proxy uses this to
...
- Only marker-verified `nxc-*.md` files containing `generated-by: nexcode` are ever
...
Dispatch: `subagent_type: "nxc-gpt-5-6-sol"`. 1M-capable targets carry `[1m]` automatically.
...
default nexcode replaces the skill's content with a short stub on **routed** requests. Native
...
Routed models do not all have the same hosted tools or image support. nexcode fills those gaps
...
`anthropic` without a usable credential **fails closed**: nexcode does not silently borrow
...
`nxc debug claude on|off|status|reset`, `NXC_CLAUDE_DEBUG=1`, or `PUT /api/debug {"claude": true}`
...
- Quickstart (`nxc claude`) and manual env block
...
including `usage.server_tool_use.web_search_requests`. Update nexcode if an older build completed
...
in your shell. `nxc claude` deliberately does NOT set `ANTHROPIC_API_KEY`; if you have it exported,
unset it. `nxc claude` injects `ANTHROPIC_BASE_URL`, discovery, auto-context, and configured model slots — but never `ANTHROPIC_API_KEY`.

**Models not showing in /model picker** — Verify `CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY=1` is
set (automatic with `nxc claude`). Run `nxc claude` to refresh the gateway model cache at
...
`ANTHROPIC_BASE_URL`. Open a new terminal, or re-run `nxc claude`.
...
on Claude model mentions. This is normal for native passthrough; on routed models, nexcode stubs
it by default (`blockedSkills: ["claude-api"]`).

**Subagent dispatches to wrong model** — Roster agents (`nxc-*`) use `<!-- nxc-route: ... -->`
