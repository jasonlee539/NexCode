...
description: Connect Factory Droid models to nexcode through a local Responses-compatible bridge.
---
...
nexcode adapter or adding provider headers does not make that private route a supported public API.
...
This keeps the Factory credential inside the official Droid client. NexCode receives a separate,
...
| `${DROID_BRIDGE_TOKEN}` is unresolved | The variable is missing from the nexcode service environment | Inject it into the service process, not only an interactive shell |
...
## NexCode provider configuration
...
SDK. Treat `nxc access test` success as text-path verification, not Codex agent or tool-path
verification.
