---
title: Providers
description: Every way nexcode authenticates and talks to an LLM provider — OAuth, API key, ChatGPT forward, and local.
---

A **provider** is one upstream LLM endpoint plus how to reach it: an adapter, a base URL, an auth
mode, and an optional model list. Providers live under `providers` in `~/.nexcode/config.json`.
...
is retained once at `~/.nexcode/config.json.pre-openai-tiers-v2.bak`; restore it with
...
device-flow bridge. nexcode stores their credentials in
`~/.nexcode/auth.json` and refreshes them automatically. `chatgpt` is also accepted by the login
CLI; it acquires a ChatGPT credential while creating a `forward`-mode provider entry.

```bash
nxc login xai          # xAI Grok
nxc login anthropic    # Anthropic Claude (Pro/Max)
nxc login kimi         # Moonshot Kimi
nxc login nous         # Nous Portal (device grant; free + paid models)
nxc login kiro         # import kiro-cli credentials (or token fallback)
nxc login google-antigravity
nxc login cursor       # standalone Cursor PKCE login
nxc login command-code # Command Code browser OAuth (or import ~/.commandcode/auth.json)
nxc login github-copilot  # GitHub device flow → Copilot token (Copilot Pro/Business)
nxc login chatgpt      # standalone ChatGPT OAuth login
nxc logout <provider>
```
...
After a terminal Nous refresh failure, run `nxc login nous` to reauthenticate.

For the canonical Kimi Coding Plan presets (`kimi` account login and `kimi-code` API key),
nexcode forwards only a caller-supplied stable `prompt_cache_key` to the Chat Completions request;
...
field, nexcode does not strip it and retry or mutate saved configuration. Other providers remain
deny-by-default.
...
Tokens stay in `~/.nexcode/auth.json`; `/api/oauth/accounts` returns masked metadata only.

### Google Antigravity OAuth client setup

NexCode does not ship Google OAuth client credentials in the repository. Before starting an
Antigravity login, provide both `GOOGLE_ANTIGRAVITY_CLIENT_ID` and
`GOOGLE_ANTIGRAVITY_CLIENT_SECRET` in the proxy process environment. Login and token refresh fail
closed without both values; do not commit either value to your project.

### Cockpit Tools Antigravity import

For v1, NexCode imports only a **Cockpit Tools Antigravity** JSON export for the `google-antigravity` provider. In the Providers dashboard, choose the local JSON file from that provider's Accounts tab. The dashboard does not show the file contents or credential values; it reports only imported, updated, failed, and unsupported counts. Other Cockpit providers are rejected in v1.

The CLI accepts the export from a file or standard input only — never paste it into a command argument:

```bash
nxc account import google-antigravity --format cockpit-tools --file <path> [--json]
cat accounts.json | nxc account import google-antigravity --format cockpit-tools --stdin [--json]
```
...
nexcode coordinates token refresh and Codex pool routing so concurrent requests do not race the
...
`chatgpt-account-id` to match the selected credential. nexcode does **not** fabricate official
client identity (for example `originator`, session, or thread headers) when the caller did not send
them.

For account-switch compatibility diagnosis, enabling provider debug (`nxc debug provider on`) adds
one `[nxc:codex:affinity]` line per canonical ChatGPT forward response. The line contains header
...
change after every proxy restart. Use `nxc debug provider logs -f` while
reproducing the two requests, then run `nxc debug provider off`. This capture is observation-only and
does not strip metadata, retry a request, switch accounts, reset a thread, or otherwise affect routing.

**Diagnostics and reauth.** Human `nxc status` prints an OAuth health block (redacted account ids,
no tokens). `nxc doctor` adds an OAuth reliability section with writable-store / single-flight checks
and WARN rows that include a recovery Action. When an OAuth provider account needs reauthentication, run
`nxc login <provider>` (or use Reauthenticate in the dashboard). Codex pool accounts are not an
`nxc login` provider — reauthenticate via the dashboard Codex account pool. See
[`nxc status` / `nxc doctor`](/reference/cli/) in the CLI reference.
...
Without a `kiro-cli` session, `nxc login kiro` falls
back to a pasted access token or the `KIRO_ACCESS_TOKEN` environment variable.

The `nxc login kiro` import path searches the platform Kiro CLI stores and opens SQLite databases
...
- `KIROCLI_DB_PATH` selects a nonstandard Kiro CLI SQLite database. The path must already exist;
  during this import path, nexcode does not create or modify the database, WAL, or SHM files.
...
also needs the local CLI binary: nexcode first uses `PATH`, then falls back to
`%LOCALAPPDATA%\Kiro-Cli\kiro-cli.exe` and `C:\Program Files\Kiro-Cli\kiro-cli.exe`.

After a successful import, nexcode persists the imported credential to
`~/.nexcode/auth.json`.
...
and imports the fresh browser login. If the login is cancelled or fails, including while NexCode
...
nexcode ships 79 built-in presets: 67 key-based, eight OAuth, three local, and one default
...
intentional: the first segment selects the nexcode provider, while `cline-pass/kimi-k3` is the
...
accepts `low`, `medium`, `high`, `xhigh`, and `max` at the gateway input boundary. nexcode
...
`Retry-After` on such a 429, nexcode adds provider guidance to the client error and a
...
Responses event, nexcode applies a five-second model-scoped grace repair; malformed or partial
...
> nexcode is the documented use; pointing other automation at a plan key is not. The
...
rows. `nxc login command-code` supports OAuth via browser sign-in (with optional local CLI credential
...
**Command Code quota.** The dashboard and `nxc account refresh` probe Command Code's
...
the dashboard and from `nxc account refresh <provider>`. The provider name is arbitrary; detection
...
Use `nxc account list`, `nxc account current`, and `nxc account use` to inspect or switch the same
Codex, OAuth, and API-key pools without opening the dashboard. See the
[CLI reference](/reference/cli/#nxc-account-subcommand) for commands, JSON output, and
new-session behavior.

### GPT-5.6 preview paths

GPT-5.6 Sol/Terra/Luna are seeded in provider fallback lists so `nxc sync` can keep the models
...
A provider is included when nexcode has a matching wire adapter, **not** based on whether it is an
...
**GitHub Copilot** is an OAuth provider (`nxc login github-copilot`) that exchanges a GitHub
...
`/chat/completions` for agent traffic, so nexcode routes those models over the
...
Cursor is tracked separately as an experimental adapter. `adapter: "cursor"` appears in `nxc init`
and the dashboard Add Provider picker as an experimental local config entry with Cursor's static
fallback model catalog metadata. When a Cursor access token is configured, nexcode uses Cursor's
...
`unsafeAllowNativeLocalExec: true` on the `providers.cursor` object in `~/.nexcode/config.json`
...
configured local executor, nexcode returns typed no-executor results instead of policy-blocking
...
from [ollama.com/settings/keys](https://ollama.com/settings/keys). nexcode classifies its cloud
...
Point nexcode at a local OpenAI-compatible server — usually with a blank key:
...
dashboard or `custom` in `nxc init` and enter the base URL. See the
...
their matching bars. NexCode does not reconstruct dollar caps from local usage logs, and a
