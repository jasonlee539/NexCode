...
NexCode performs a bounded offline maintenance sequence:
...
A busy Log Guard lock, busy SQLite writer, or busy initial checkpoint is returned as an explicit refusal rather than being retried in the background. If checkpoint contention appears only after an incremental-vacuum batch has already committed, NexCode reports the work already completed as a successful partial result with `stopReason: "busy"` instead of claiming that nothing changed.
...
```bash
nxc storage codex-logs compact
```

For machine-readable before/after metrics:

```bash
nxc storage codex-logs compact --json
```

If the result reports that more reclaimable space remains, stop there unless you explicitly want another bounded pass. NexCode does not loop indefinitely or schedule a follow-up pass for you.
