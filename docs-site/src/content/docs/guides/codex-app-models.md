...
description: How nexcode models appear in Codex App, Codex CLI, and Codex TUI through the shared Codex catalog.
---

nexcode does not patch Codex App. It writes the same Codex configuration and model catalog that
...
nexcode adds separate `<selector>/<native-openai-model>` rows for the mapped accounts and hides
...
that is not yet in nexcode's static set, the exact id is preserved as a selector-qualified row
...
`gpt-daybreak-blue-latest` is account-gated. nexcode checks each authenticated ChatGPT account's
...
the stored main credential when an NexCode admission bearer is substituted). A
...
metadata. The request still sends `gpt-daybreak-blue-latest`; nexcode does not rewrite it to Sol
...
retain the shipped v1 source at `~/.nexcode/config.json.pre-openai-tiers-v2.bak`; restore it with:
...
For an equivalent routed model, nexcode provides an explicit, default-off native-alias combo mode.
...
`nxc init`, `nxc start`, and `nxc sync` wire the shared Codex config and catalog into the proxy; see
...
Codex's model picker expects Codex-shaped catalog entries. nexcode builds routed entries by cloning
...
base instructions. nexcode then removes native-only capabilities that the route cannot honor,
...
`gpt-5.3-codex-spark`, and GPT-5.6 Sol/Terra/Luna. For the GPT-5.5/5.4 family, nexcode preserves
...
But the model catalog and runtime request tier id use `priority`. nexcode preserves that split.
...
exact ids; saving the page replaces the list with dashboard-visible choices. nexcode assigns low
...
rendering. nexcode has no hook into that list; the upstream bug is tracked at
...
nexcode rewrites `models_cache.json` with a deliberately stale cache wrapper whenever catalog
