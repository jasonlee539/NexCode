...
description: How nexcode determines model order in the Codex picker and spawn_agent model overrides.
---

The Codex model picker does not preserve the order of provider declarations or model arrays in the
nexcode configuration. Its final order comes from catalog priorities, with a deterministic
...
nexcode therefore controls featured placement by assigning lower priorities, not by relying on
...
priority (normally `9` for entries built by nexcode), and the routed group remains provider/id
alphabetical.
...
`nxc agent subagents set` or edit the nexcode configuration for exact
...
Codex-visible picker priority while nexcode retains each moved row's natural priority for
