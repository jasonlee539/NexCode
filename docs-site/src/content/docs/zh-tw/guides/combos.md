...
**combo** 是一個虛擬模型，背後代表一個有序的真實供應商/模型目標清單。你的客戶端請求 `combo/<id>`；nexcode 選擇一個目標，將請求改寫為該具體的 `provider/model`，並可在第一個目標發生可重試失敗時嘗試另一個目標。
...
預設策略為 failover，因此正常請求會送往 `anthropic/claude-opus-4-8`。若該次嘗試發生可重試失敗，nexcode 可跳到 `openai/gpt-5.6-sol`。
...
`nxc combo set <id>` 中的 combo id 必須以字母或數字開頭。其後可含字母、數字、`.`、`_` 或 `-`，總長最多 64 字元。其規範模型 id 始終為 `combo/<id>`；例如 id `main` 變成 `combo/main`。
...
由 [nexcode #241](https://github.com/lidge-jun/nexcode/issues/241) 追蹤。

當你控制一個等效的路由目標時，combo 可以明確接管一個原生 slug：

```bash
nxc combo set nova-sol \
...
nexcode 版本支援的原生模型 id 之一；僅有原生系列前綴不會被接受，因為移除時必須能恢復具權威性的
...
跳轉的目標預設進入 60 秒冷卻。若上游回應包含有效的 `Retry-After` 值，nexcode 改用它。接受數字秒與 HTTP-date 值，且每次冷卻上限為 10 分鐘。
...
若請求沒有 `reasoning` 物件，nexcode 建立一個。若 `reasoning` 存在但無 `effort` 屬性，它保留其他欄位並加入預設值。呼叫者提供的 effort 永不被覆寫。

當目標能力未知或不包含設定的 effort 時，nexcode 省略預設值並保持目標自身行為不變。支援的值為 `low`、`medium`、`high`、`xhigh`、`max` 與 `ultra`；省略欄位或設為 `null` 可將 effort 完全交給呼叫者與目標。

## 加密的 v2 子代理任務

Codex v2 子代理有一個重要限制（[issue #92](https://github.com/lidge-jun/nexcode/issues/92)）。原生父代只能將新生成 worker 的任務以為原生 ChatGPT 後端鑄造的密文發送。外部供應商無法讀取該 payload。

對於此類請求，combo 將其合格目標過濾為規範的原生 ChatGPT 路由，包括可重試失敗之後。若 combo 無可解密目標，nexcode 在分派前停止並回傳 HTTP 400：
...
`set` 也接受 `--strategy`、`--sticky`、`--effort`、`--alias` 與 `--rename-from`。用 `-` 作為 `--effort` 或 `--alias` 的值可清除該欄位。`create` 與 `update` 為 `set` 的別名；`delete` 為 `remove` 的別名；且相同子指令在 `nxc route combo` 下也可用。
...
Combo id 未知。回應為 HTTP 404 並帶 type `invalid_request_error`。執行 `nxc combo list`、檢查拼字與大小寫，並確認你的管理指令寫入的是同一個接收模型請求的執行中 nexcode 實例。
