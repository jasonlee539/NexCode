...
Video Bridge 讓你透過 nexcode 路由的任何非 OpenAI 模型，使用 xAI 的 Grok Imagine Video 生成。啟用後，對話中會注入一個合成的 `video_gen` 工具。模型像呼叫一般函式工具一樣呼叫它；nexcode 攔截該呼叫、向 xAI 提交影片生成工作、輪詢直到完成，並下載結果。

## 前置條件

- 一個帶有 **API key** 的 `xai` 供應商項目（單靠 `nxc login xai` 不足夠 — video bridge 需要 key 認證，而非 OAuth）
- 一個非 OpenAI 模型作為你的路由供應商（例如 Anthropic Claude、Google Gemini）
- nexcode 設定為透過該非 OpenAI 供應商路由
...
> ```
>
> 若你是透過 `nxc login xai`（OAuth）接入，供應商會停留在 `authMode: "oauth"`，bridge 不會悄悄啟用。請在環境中設定 `XAI_API_KEY`，**或**如上所示直接寫入 key。
...
1. nexcode 偵測到帶有 `videoBridgeEnabled: true` 的非 OpenAI 路由模型
2. 對話中注入一個合成的 `video_gen` 函式工具
3. 當模型呼叫 `video_gen` 時，nexcode 向 xAI 的 `/videos/generations` 提交工作
