...
description: nexcode 模型如何透過共享 Codex 目錄出現在 Codex App、Codex CLI 和 Codex TUI 中。
---

nexcode 不會修改 Codex App。它會寫入 Codex CLI/TUI 所使用的同一套 Codex 設定和模型目錄。
...
帳號存在的合格選擇器時，nexcode 會為這些對應帳號新增獨立的 `<selector>/<native-openai-model>`
...
`~/.nexcode/config.json.pre-openai-tiers-v2.bak` 保留出廠的 v1 原始檔；用以下指令恢復：
...
對於等效的路由模型，nexcode 提供一個明確、預設關閉的 native-alias combo 模式。它會發布
...
`nxc init`、`nxc start` 和 `nxc sync` 會把共享的 Codex 設定與目錄接入代理；設定注入、目錄同步、
shim、WebSocket 回退與恢復機制請見 [Codex 整合](/zh-tw/guides/codex-integration/)。

## 為什麼路由模型會顯示

Codex 模型選擇器要求條目符合 Codex 目錄結構。nexcode 會克隆一個原生 Codex 模型模板，然後
替換路由模型的身份資訊：
...
所需欄位。隨後，nexcode 會移除該路由無法兌現的原生專屬能力，例如 OpenAI service-tier 後設資料。

## 目前穩定模型涵蓋範圍

原生回退列表包含 `gpt-5.5`、`gpt-5.4`、`gpt-5.4-mini`、
`gpt-5.3-codex-spark` 以及 GPT-5.6 Sol/Terra/Luna。對於 GPT-5.5/5.4 系列，nexcode 會
...
模型目錄和執行環境請求使用的 tier id 則是 `priority`。nexcode 會保留這一差異。原生 OpenAI
...
儀表板不提供這些精確 id；儲存頁面時會把列表替換為儀表板可見的選擇。nexcode 會按所選順序
...
allowlist 上的項目。nexcode 無法介入該清單；上游錯誤追蹤於
...
當目錄的可見性、priority 或後設資料發生變化時，nexcode 會用一個刻意標記為過期的快取 wrapper
