...
不同路由模型對託管 **Web Search** 和原生**圖像輸入**的支援並不相同。nexcode 透過兩個
...
當 Codex 為非透傳的路由模型請求託管 `web_search` 時，nexcode 會：

1. **移除**託管的 `web_search` 工具，改為向路由模型提供一個合成的
   `web_search(query)` function 工具。原託管工具的選項會保留並用於 sidecar 呼叫。
2. 讓路由模型在一個小型 **agentic 迴圈**中執行。模型呼叫 `web_search` 時，nexcode 使用所選
...
路由模型的每次迭代都會向上遊請求 `stream: true`，但 nexcode 會在決定搜尋還是回傳最終答案前，
...
當路由模型列在其 provider 的 `noVisionModels` 中，並且請求包含圖像時，nexcode 會在主呼叫
