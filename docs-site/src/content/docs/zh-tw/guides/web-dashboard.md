---
title: Web 儀表板
description: 用於管理代理健康狀態、provider、模型、委派指引、認證池、usage 和日誌的 nexcode GUI。
---

nexcode 內建了一個由代理提供服務的本機 web 儀表板（`gui/` 下的 Vite/React 應用）。你可以在
...
儀表板才需要 admin token（`NEXCODE_ADMIN_AUTH_TOKEN`，或自動產生的
`~/.nexcode/admin-api-token` 檔案）。
...
| **Codex 自動啟動** | 允許已安裝的 Codex launcher shim 執行 `nxc ensure`。此開關不會安裝 shim 或後臺服務。 |
...
`injectionEffort`。在 v1 turn 中，nexcode 會注入一段指引，告訴父代理呼叫 `spawn_agent` 時應
...
側邊欄的星標按鈕——以及 `nxc start` 在互動式終端機中詢問的一次性問題——都透過 **你自己的
`gh` 登入** 執行。nexcode 不持有任何 GitHub token，它唯一得知的是你的 yes 或 no。

由於這會寫入你的 GitHub 帳號，agent 驅動的呼叫者會被拒絕，而不是被允許替你回答：

- `nxc start` 與 `nxc service install` 在 agent 或 CI harness 驅動時 **完全略過該提示**
