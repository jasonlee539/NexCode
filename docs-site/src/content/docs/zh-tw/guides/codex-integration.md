---
title: Codex 整合
description: nexcode 如何將自身注入 Codex、同步模型目錄、安裝 shim，並乾淨地恢復。
---

nexcode 透過修改 Codex 會讀取的兩項內容，讓 Codex 經由 proxy 路由：其設定
...
`nxc init`、`nxc start` 與 `nxc sync` 都會呼叫注入器。在預設 loopback 繫結下，它會保留 Codex
內建的 `openai` provider id，並將該 provider 指向 nexcode：
...
認證。由於注入的 `base_url` 指向 nexcode，proxy 會把這些呼叫中繼到 OpenAI 上游。
...
- **Google Antigravity（CCA）fallback：** 若既沒有 OpenAI forward 候選，也沒有設定 keyed provider，
  ...
  `nxc login google-antigravity`；OAuth token 只會傳送到固定的 CCA registry host，絕不會傳到設定層級
...
工具宣告仍會跟著模型的 Responses 請求傳送。對 API-key Responses provider，nexcode 會把 Codex
私有的 `image_gen` namespace 降為上游安全的 `image_gen__<inner-name>` alias，例如
`image_gen__imagegen`。當可用 alias 取代 client 宣告時，nexcode 會移除重複的 hosted
...
當 NexCode 擁有路由時，兩種模式都會把 `$CODEX_HOME/nexcode.config.toml` 寫成參考／fallback
...
Codex CLI、TUI、App 與 SDK 都讀取同一個 Codex home。nexcode 會從 `CODEX_HOME` 解析該目錄，
...
在 WSL 中，如果未設定 `CODEX_HOME`，且 Linux 的 `~/.codex/config.toml` 不存在，nexcode 也會檢查
...
Codex 可將 SQLite 支援的 thread state 放在另一個目錄。NexCode 的歷史操作採用與 Codex 相同的
...
資料庫。若 `config.toml` 或其根級 `sqlite_home` 不存在，NexCode 會繼續使用環境變數／home fallback。
...
home，而 ChatGPT/Codex App 仍讀取 `%USERPROFILE%\\.codex`。`nxc status` 與 `nxc doctor` 會警告這個
...
Codex 保持一致。nexcode 也透過 WebSocket 提供 `/v1/responses`。專用 provider 只會在
...
history 不需要重新對映。第一次同步時，也會把舊版 nexcode 改過標記的 thread 遷回 `openai`。
non-loopback 專用 provider 模式在啟用期間仍會把歷史映射到 `nexcode` provider，退出時再恢復已備份的
metadata。設定 `syncResumeHistory: false` 可完全不修改歷史。

## 模型目錄同步

Codex 從磁碟上的目錄顯示模型，預設為 `$CODEX_HOME/nexcode-catalog.json`。啟動時與執行
`nxc sync` 時，nexcode 會：

1. **備份**一次原始目錄到 `~/.nexcode/catalog-backup.json`，讓置頂操作可逆。
...
工具，包括 Browser 與 Computer Use，同時 nexcode 只路由模型的一般 function call。工具執行、權限
與確認仍留在 Codex 本機；nexcode 不會實作第二套瀏覽器或桌面控制 executor。

對不接受 Codex `exec` custom-tool grammar 的 key-auth Responses provider，nexcode 會把該宣告與其
...
`nxc sync` 變更這份 metadata 後，請重新啟動 Codex App 並開啟新任務。既有 app-server process 與任務
...
回應是原始的 `nexcode-catalog.json` 文件，不包含 provider 憑證。若可用，
`x-nexcode-codex-version` 標頭會回報伺服器上的 Codex runtime 版本，讓 client 能辨識版本差異。

也可以透過管理 API（`POST /api/custom-models`、`PUT /api/custom-models/<id>`，搭配 `displayName`
字串）與 web 儀表板設定或編輯。`/` 會被拒絕，因為它會與路由 slug 的分隔符衝突。

顯示名稱**只用於顯示，且在重新產生時保持穩定**。每次 `nxc sync` 與目錄 refresh 都會從
...
離線登入，會保留先前已持久化的目錄，並在下一次成功的 `nxc sync` 重新套用設定名稱。真正的上游原生
名稱，例如 `gpt-5.6-sol` → "GPT-5.6-Sol"，來自固定的上游 snapshot，絕不會被自訂顯示名稱覆寫。

### 外部 provider 管理器

若 `config.toml` 已選用非 `openai` 或 `nexcode` 的 provider，NexCode 會保持檔案不變，並跳過
...
請讓單一工具負責 Codex provider 設定。若要在既有 provider manager 後方使用 NexCode，請把該
...
`NEXCODE_API_AUTH_TOKEN` 傳入 `x-nexcode-api-key`，形式與上方 non-loopback provider 相同。若要讓
NexCode 直接注入路由，請先將 Codex 切回內建 `openai` provider，移除任何使用者自有的根級
`openai_base_url`，再重新執行 `nxc start`。
...
2. **`disabledModels`（頂層）**：會同時從目錄與 `/v1/models` 隱藏模型，並把裸原生 GPT slug 設為
   `visibility: "hide"`。
3. **`liveModels: false` 且 `models` 為空**：當即時探索關閉，且 `models` 為空或省略時，nexcode
...
5. **cache 與 `nxc sync`**：即時目錄約快取五分鐘（`modelCacheTtlMs`，預設 `300000`）。執行
   `nxc sync` 可強制重新抓取並立即重寫目錄。
6. **正在執行的 Codex `app-server`**：長時間執行的 Codex `app-server`（Desktop／CLI 背景 host）可能
   仍在記憶體保留舊列表，因此只重寫磁碟目錄還不夠。`nxc sync` 與 `nxc sync-cache` 偵測到這些
   process 時會警告。可執行 `nxc sync --restart-codex` 重新啟動，或自行停止對應的 `app-server`
   process，再讓 Codex 重新建立它們，讓新列表出現。

:::caution[其他本機寫入者]
目錄寫入（`nexcode-catalog.json`、`config.toml`）在 nexcode **內部**是原子的；這只避免兩個
nexcode 擁有的寫入者競爭時出現半寫入檔案。它**不會**阻止其他本機 process、file watcher 或 sync
agent 在 nexcode 寫入後改寫目錄可見性或順序。Codex 另有自己的 `models_cache.json`，可獨立 refresh，
因此可能在不重寫 `nexcode-catalog.json` 的情況下改變可見列表。若 proxy 執行中模型卻意外跳動，請
先停止或重新設定競爭的寫入者，再執行 `nxc sync`。這是外部寫入者風險，不是已確認的 nexcode
缺陷。
:::
...
的錯誤，或 Claude Code 出現類似連線失敗，代表 nexcode proxy 沒有執行：設定埠上沒有任何監聽，
client 只能顯示原始連線錯誤。請重新啟動 proxy：

```bash
nxc start              # 前景執行
nxc service install    # 常駐：登入時自動啟動，崩潰後自動重新啟動
```

`nxc status` 可檢視 proxy 是否執行，未執行時也會給出相同的重啟提示；`nxc doctor` 會回報重啟安全性
（service／shim 覆蓋情況）。
...
向 Codex 帳號池新增 ChatGPT 帳號時，nexcode 會先用一個小型 streaming 請求向 Codex Responses
...
nexcode 絕不會把你困住。**`nxc stop` 是完整恢復原生 Codex 的單一命令**。它會停止 proxy、停止
背景服務（若已安裝），並移除所有注入行與路由目錄條目，讓普通的 `codex` 就像從未安裝 nexcode 一樣
運作：

```bash
nxc stop       # 停止 proxy + service，恢復原生 Codex
nxc restore    # 不停止 proxy，只恢復原生設定（alias: nxc eject）
nxc restore back # 讓普通 Codex 再次指向仍在執行的 proxy
```

當 nexcode 作為受管的 [背景服務](/zh-tw/reference/cli/#nxc-service) 執行時，會設定 `NXC_SERVICE=1`，
因此 service 驅動的 restart **不會**反覆改寫 Codex 設定；只有明確執行 `nxc stop` 或
`nxc service stop` 才會恢復原生 Codex。