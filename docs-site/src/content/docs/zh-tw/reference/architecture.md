---
title: 架構
description: nexcode 內部機制 —— 模組圖、請求解析器、AdapterEvent 橋接與快取。
---

nexcode 執行在單個 Bun 程序中。請求以 OpenAI Responses 格式進入，規範化為內部模型後完成
...
- **訊息（Messages）** —— `input` 條目會變成規範化的 `NxcMessage[]`：user / developer /
...
（`: nexcode heartbeat`）來重新啟動 Codex 的空閒計時器。註解行會被每個
...
`/api/*` 和 `/v1/*` 都提供 `NEXCODE_API_AUTH_TOKEN`；設定的 `corsAllowOrigins` 會擴充套件本機
origin allowlist。
...
thread affinity 位於 `codex/` 下，不會出現在管理 API 回應中。請求用量會規範化為 `NxcUsage`，
...
嘗試 Responses WebSocket upgrade 時，nexcode 會回傳 `426 upgrade_required`，Codex 隨後在該
...
內部模型位於 `types.ts`：`NxcParsedRequest`、`NxcContext`、`NxcMessage` 聯合型別、
`NxcContentPart`（text / image）、`NxcToolCall`、`NxcTool`、`AdapterEvent`，以及設定型別
（`NxcConfig`、`NxcProviderConfig`）。兩個常用 helper 是 `namespacedToolName()` 和
