...
nexcode 以多種客戶端方言呈現一個本機代理。Codex 客戶端可說 Responses API，OpenAI 相容的 app 可說 Chat Completions，而 Claude Code 可說 Anthropic Messages，而不需要每個上游供應商實作每種格式。
...
這是原生 nexcode data-plane 結構。請求 body 必須是帶有非空 `model` 的 JSON 物件。`input` 可為字串或 Responses 項目陣列。
...
- 在非回環 listener 上，請求還只透過 `x-nexcode-api-key` 攜帶有效代理許可；且
...
`POST /v1/realtime/calls` 接受 OpenAI Realtime call-creation 介面。nexcode 選擇一個合格的 OpenAI 家族路由、為上游認證模式正規化 call-creation 請求，並中繼有界的回應。
...
| 其他路由模型 | 執行一個內部、非串流、無工具的 compaction 回合，帶有 `compaction_trigger`；需要恰好一個合成的 `compaction` 項目，其 `encrypted_content` 為 `nxc1:` 封裝；將該摘要解碼為 v1 取代歷史 |
...
| 502 | `invalid_response_error` | 合成回合未產生恰好一個有效、非空的 `nxc1:` compaction 項目 |

## 認證矩陣

在僅回送綁定上，data-plane 許可不需要設定的金鑰。在遠端綁定上，請使用下方矩陣。「專屬」意指 `X-NexCode-API-Key`；其他欄意指 `Authorization: Bearer ...` 與 `x-api-key`。
...
代理將真實的後端密文視為不透明。結構有效的密文被逐位元組保留：nexcode 不解密它、轉譯其內容，或為另一個供應商重新加密它。

某些 agent hook 在歷史上曾將明文控制文字放入 `encrypted_content` 插槽。為相容性，代理將該明文分離為 text 部分，同時保留任何結構有效的 Fernet run 不變。若 `agent_message` 在該修復期間失去所有加密部分，它成為普通使用者訊息。若目前的 v2 task 保持真正加密但所選路由目標無法讀取原生 ChatGPT 密文，nexcode 以 `unreadable_encrypted_agent_task` 失敗，而非發送不可讀的位元組給該供應商。關於 worker task 周圍的客戶端行為，請見[子代理介面](/zh-tw/guides/sub-agent-surface/)。
