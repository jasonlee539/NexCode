...
---
title: 設定參考
description: nexcode 存放設定檔的位置、編輯方式的套用規則，以及每個設定領域的連結。
---

nexcode 把持久化設定存放在 `$NEXCODE_HOME/config.json`，通常是
`~/.nexcode/config.json`。在 Windows 上預設為
...
- **CLI：** `nxc init` 建立初始檔案，`nxc provider`、`nxc models`、`nxc combo`、
  `nxc agent`、`nxc config` 等命令會更新或檢查各自擁有的設定。
...
如果檔案無法解析，nexcode 會把它備份為 `config.json.invalid-<timestamp>`、在 console
警告，並以預設值啟動。檔案缺失時也使用全新安裝的預設：一個 `openai` forward provider。

## 優先順序與預設值

`config.json` 中的有效值會覆寫內建預設。缺失的選用欄位使用各領域頁面記載的預設值。
`NEXCODE_HOME` 優先於預設的設定目錄。接受環境變數引用的欄位（例如
...
:::note[原子寫入]
nexcode 透過臨時檔加上重新命名（`atomicWriteFile`）寫入受管的 `config.toml` 與
`nexcode-catalog.json`。當 `nxc stop` 與代理的 shutdown handler 這類同時寫入者
