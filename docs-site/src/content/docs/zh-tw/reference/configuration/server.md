...
| `hostname?` | `string` | `"127.0.0.1"` | 綁定位址。非回送綁定需要 `NEXCODE_API_AUTH_TOKEN`。 |
| `proxy?` | `string` | — | 對外 HTTP(S) 代理 URL 或 `${ENV_VAR}`。僅在那些變數未設定時套用至 `HTTP_PROXY` / `HTTPS_PROXY`；回送保留在 `NO_PROXY` 中。 |
| `emptyCompletionRetry?` | `boolean` | `false` | 明確啟用：當 Responses 完成時沒有文字或工具呼叫，以相同請求重試一次。重試可能產生費用。`NXC_EMPTY_COMPLETION_RETRY=0` 可在不變更設定的情況下停用；combo 與 routed-compaction turn 不適用。 |
...
| `apiKeys?` | `NxcApiKey[]` | `[]` | 生成的 `nxc_…` 憑證，在非回送綁定上被管理與 data-plane 認證接受。由儀表板管理。 |
...
| `codexAutoStart?` | `boolean` | `true` | 讓 Codex shim 在啟動 Codex 前執行 `nxc ensure`。False 使 ensure 為 no-op。 |
| `codexShimAutoRestore?` | `boolean` | `true` | 在完成的外部 Codex 更新取代已安裝的 shim 後還原它。環境退出：`NEXCODE_CODEX_SHIM_AUTO_RESTORE=0`。 |
| `syncResumeHistory?` | `boolean` | `true` | 可逆的 Codex App 歷史相容性。原始中繼資料由 `nxc stop` / `nxc restore` 備份並還原。 |
...
| `webSearchSidecar?` | `NxcWebSearchSidecarConfig` | 可用時開啟 | 網頁搜尋 sidecar 選項。 |
| `visionSidecar?` | `NxcVisionSidecarConfig` | 可用時開啟 | 圖片描述 sidecar 選項。 |
| `images?` | `NxcImagesConfig` | 自動 OpenAI 選擇 | Codex `image_gen` 的獨立 Images 中繼選項。 |

若較舊的開發組建在備份支援存在前變更了 resume-history 中繼資料，請執行 `nxc recover-history --legacy-openai` 以強制原生供應商復原。
...
代理在沒有此變數時拒絕遠端綁定。對於背景服務，請在 `nxc service install` 前匯出它，以便 launchd、systemd 或 Task Scheduler 接收它。客戶端應發送：

```text
x-nexcode-api-key: your-secret-token
```

| 端點 | `Authorization: Bearer` | `x-nexcode-api-key` | `x-api-key` |
| --- | --- | --- | --- |
...
`x-nexcode-api-key` 接受代理許可，並將 `Authorization` 與 `x-api-key` 保留給 Anthropic
...
永遠不會經過生成的 `codex` shim，因此它不會繼承 `NEXCODE_API_AUTH_TOKEN`，每次模型呼叫都會在
stream 開啟前以 `401` 失敗。
...
接著 `nxc sync` 會把 `base_url = "http://127.0.0.1:10200/v1"` 寫入受管的 Codex provider 區塊，
...
任何本機連接埠皆可。Host 解析為 `localhost`、`127.0.0.1` 或 `::1` 的請求，不論連接埠皆保持回送，因此 `http://localhost:20100/v1` 可運作。在客戶端設定該 base URL；`nxc` 僅將預設的本機 `127.0.0.1` 位址寫入受管客戶端設定。
...
這些設定治理 `/v1/messages`、`/v1/messages/count_tokens`、`nxc claude` 啟動器與 Claude 儀表板頁面。
...
| `claudeCode.subagentEffort?` | `"low" \| "medium" \| "high" \| "xhigh" \| "max"` | 繼承 | 寫入生成的 `~/.claude/agents/nxc-*.md` 的 effort；與 Codex guidance 與代理上限分開。透過 `nxc claude` 重啟以重新生成。 |
...
Anthropic OAuth sidecar 重用 nexcode 既有的 Claude Code OAuth 指紋。請對預期帳號與工作負載進行浸泡測試。
