...
**adapter** 負責在 nexcode 的內部請求/回應模型與某個 provider 的 wire 格式之間轉換。每個
...
`buildRequest` 把 `NxcParsedRequest` 轉成上游 HTTP 請求；`parseStream` / `parseResponse` 把 provider
...
只有完全沒有 stop reason 時，nexcode 才新增私有的 `codex_kiro_final_answer` 工具並做一次續寫。
