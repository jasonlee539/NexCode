---
title: 貢獻指南
description: nexcode 的開發環境、結構、約定，以及新增 provider 或 adapter 的方法。
---
...
公開文件釋出到 GitHub Pages：<https://nexcode.me/zh-tw/>。
...
- **Cross-platform CI**（`.github/workflows/ci.yml`）會在改動 runtime、test、package、script、
  ...
  build 和 `nxc help`。另一個三系統 lane 使用 package 內建 runtime，驗證無需單獨安裝 Bun 也能
...
[`MAINTAINERS.md`](https://github.com/lidge-jun/nexcode/blob/main/MAINTAINERS.md)。repository 與
...
`src/providers/derive.ts` 會把該條目提供給 `nxc init`、`nxc provider`、儀表板 preset、API-key
...
registry 條目是一項被維護的承諾：nexcode 會把使用者的 API key 送到這個目的地。因此 preset
...
`unverified`）且是惰性的：使用者仍可透過自訂 OpenAI-compatible flow 使用該服務，而 nexcode
...
nexcode 傾向於小而可驗證的 commit，而不是大批次改動。
