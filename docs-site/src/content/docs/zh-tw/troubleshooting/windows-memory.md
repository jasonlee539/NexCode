---
title: Windows 記憶體成長
description: 為何 Windows 上的 bun 程序可能成長到數 GB 的 RAM、nexcode 目前如何處理，以及上游 Bun 修復釋出前你有哪些選項。
---

部分 Windows 使用者會看到 nexcode 背後的 `bun` 程序，在長時間串流工作階段中成長到數 GB 的 RSS（回報為 issue [#314](https://github.com/lidge-jun/nexcode/issues/314)）。本頁誠實說明實際發生的狀況，以及你可以怎麼做。

## 根本原因：上游 Bun runtime 問題

nexcode 內嵌 Bun runtime（目前為 **1.3.14**）。記憶體成長由已知的上游 Bun 問題驅動，而非代理程式中 JavaScript 層級的洩漏：
...
在 Windows 上，nexcode 必須讓串流回應維持在保守的程式路徑，以避免 #32111 當機，而該路徑最容易暴露於 backpressure 問題：緩慢或停滯的用戶端可能讓 runtime 在原生記憶體中緩衝上游資料，而 JavaScript 無法加以限制。
...
- **`nxc doctor`** — 「Memory / runtime」區段會顯示*服務*程序的 Bun 版本、RSS、JS-heap 占比與 stream-mode 決策，並告訴你成長看起來是原生端（上游問題）還是 JS 端（你應回報的 nexcode 錯誤）。
...
1. **等待內嵌 runtime 更新。** 一旦某個 Bun 發行版可驗證承載這些修復，nexcode 會升級內嵌 runtime，並自動啟用較安全的串流路徑。

2. **以 `NEXCODE_BUN_PATH` 執行你信任的 Bun runtime。** 這是未驗證領域——你是在我們尚未測試的 runtime 上執行 nexcode；風險自負。對服務安裝很重要：覆寫是在**服務產物產生時**讀取，而非服務啟動時。請設定環境變數，然後從同一個 shell 重新執行 `nxc service repair`，讓路徑寫入持久的服務定義。僅設定環境變數對已安裝的服務無效。
...
若你在真實 Windows 工作負載上嘗試上述任一作法，請在 [#314](https://github.com/lidge-jun/nexcode/issues/314) 回報前後的 `nxc doctor` 記憶體區段——這正是此緩解等待的驗證。
