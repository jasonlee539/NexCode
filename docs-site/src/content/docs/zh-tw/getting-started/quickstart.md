---
title: 快速入門
description: 設定你的第一個 provider,用三條命令讓 OpenAI Codex 透過 nexcode 進行路由。
---
...
4. **代理埠** —— 預設為 `10100`。
5. **注入到 Codex？** —— 在通常的迴環地址設定中，nexcode 會在
...
6. **安裝自動啟動 shim？** —— 啟用後，每次啟動 `codex` 都會先執行 `nxc ensure`。

結果會儲存到 `$NEXCODE_HOME/config.json`（預設 `~/.nexcode/config.json`）。
...
- 將其 PID 寫入 `~/.nexcode/nxc.pid`(並拒絕重複啟動),
- 在 provider 支援時發現即時模型，並**把原生與已路由條目同步進 Codex 的模型目錄**，以及
- 在 `http://localhost:<port>/v1` 上監聽。

如果請求的埠已被佔用，`nxc start` 會選擇一個空閒埠，將其寫入 `runtime-port.json`，並更新
Codex 設定以使用實際監聽埠。
...
`gpt-5.6-terra`、`gpt-5.6-luna` 和 `gpt-5.4-mini`。開啟 `nxc gui` 即可替換或重新排序最多五個
...
```bash
nxc login xai          # 也可使用 anthropic、kimi、kiro、google-antigravity、cursor
nxc logout xai
```
...
```bash
nxc stop          # 停止代理並恢復原生 Codex
nxc restore       # 不停止代理，僅恢復原生 Codex（別名：nxc eject）
nxc restore back  # 讓 Codex 再次使用仍在執行的代理
```
