---
title: Agent 快速上手
description: 從 agent 驅動或腳本化終端機安裝並操作 nexcode，而不跨越使用者同意邊界。
---
...
在互動終端機執行 `nxc init`。若 `nxc start` 佔用了前景，請用第二個終端機：

```bash
nxc init
```

精靈會寫入 `$NEXCODE_HOME/config.json`（通常為
`~/.nexcode/config.json`）。它也可將代理位址注入 Codex 的 `config.toml`，並安裝可選的 Codex 自動啟動 shim。`nxc init` 永不啟動代理。若要完全非互動的設定，請改用下方所示的 `nxc provider add` 設定供應商，而非驅動精靈。
...
`nxc status` 回報代理與服務狀態。`nxc doctor` 診斷本機環境、網路、Codex 執行階段與帳號健康問題。`nxc health` 在代理健康時離開 `0`，否則 `1`；`--json` 回傳結構化輸出。

由管理 API 支援的指令（例如 `nxc combo set`）會聯繫即時代理。若找不到即時代理或 API 不可達，CLI 將其視為 `503` 失敗並以非零離開。重試前請啟動前景代理或背景服務。完整指令與端點介面請見
...
`nxc provider add` 寫入本機設定。若已有即時代理在執行且你想立即將模型同步到 Codex，請加上 `--sync`；否則稍後執行 `nxc sync`。不在 registry 中的自訂供應商需要同時提供 `--adapter` 與 `--base-url`。
...
`NEXCODE_API_AUTH_TOKEN`；代理在沒有它時拒絕啟動。請在 `nxc start` 前，或在 `nxc service install` 前設定該變數，以便服務接收它：
...
客戶端隨後必須認證其管理與模型請求。在將 nexcode 暴露到本機以外之前，請閱讀[設定](/zh-tw/reference/configuration/)中的遠端存取規則。
