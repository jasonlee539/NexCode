---
title: 運作原理
description: nexcode 的完整請求生命週期 —— 解析、路由、適配、橋接與 streaming。
---

import { Steps } from '@astrojs/starlight/components';

Codex 使用 OpenAI **Responses API**。nexcode 接收透過 HTTP 與 Server-Sent Events 傳送的
...
當選中的供應商是 ChatGPT/Codex 直通時，nexcode 可在請求轉發到上游前，先從已儲存的帳號池中選帳。
規則刻意拆成兩部分：

- **既有 thread id 維持親和性。** thread 會綁定啟動它的帳號世代，因此長時間的 SSH、tmux 或
  行動裝置掛載的 Codex session 會繼續使用同一個帳號，而不是在對話中途被重新平衡。
- **新 session 可以重新平衡。** 對新 thread，nexcode 會比較 5 小時、每週與 30 天視窗的已知配額
...
1. **解析** —— `responses/parser.ts` 使用 Zod schema(`responses/schema.ts`)校驗請求,
   並將其降級為內部的 `NxcParsedRequest`:系統提示詞、一份規範化的訊息列表
...
3. **認證** —— 對於 `oauth` 型別的 provider,nexcode 會換入一個全新、自動重新整理的 access
...
4. **Vision sidecar（可選）** —— 如果已路由的模型被列在 `provider.noVisionModels` 中，且
   請求攜帶了圖像，nexcode 會透過已設定的 vision sidecar 描述每張圖像並替換為文字，讓純文字
...
5. **直通快速路徑** —— 對於 Responses 直通 adapter（`openai-responses` 或 `azure-openai`），
   nexcode 會保留 Responses body，只執行必要的路由與相容性改寫，然後直接轉發 provider 回應，
   不再轉換為 `AdapterEvent`。

6. **網路搜尋 sidecar（可選）** —— 如果 Codex 啟用了託管的 `web_search`，但已路由的模型
   並非 OpenAI，nexcode 會暴露一個合成的 `web_search` 函式工具，並在一個小型
...
8. **適配** —— 否則,所選 adapter 的 `buildRequest()` 會以 provider 的原生格式生成上游 HTTP 請求
   (URL、headers、body),由 nexcode 對其執行 `fetch`。
...
Codex 把 Responses API 硬編碼在內部。透過在協議邊界處進行翻譯,nexcode 可以與
