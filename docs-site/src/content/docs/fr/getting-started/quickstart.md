...
description: Configurez votre premier fournisseur et acheminez OpenAI Codex via nexcode en trois commandes.
---
...
`nxc init` vous accompagne dans les étapes suivantes :
...
5. **Injecter dans Codex ?** — avec une configuration de bouclage normale, nexcode ajoute une valeur racine `openai_base_url` à
...
6. **Installer le shim de démarrage automatique ?** — lorsqu’il est activé, le lancement de `codex` exécute d’abord `nxc ensure`.

Le résultat est enregistré dans `$NEXCODE_HOME/config.json` (par défaut `~/.nexcode/config.json`).
...
```bash
nxc start            # utilise le port 10100 par défaut
nxc start --port 8080
```

Au démarrage, nexcode :

- écrit son PID à `~/.nexcode/nxc.pid` (et refuse de démarrer deux fois),
...
Si le port demandé est occupé, `nxc start` sélectionne un port libre, l'enregistre dans `runtime-port.json`,
...
```bash
nxc status
nxc gui       # ouvre le tableau de bord sur le port actif
```

## 3. Utiliser Codex

Codex parle maintenant à nexcode de manière transparente :
...
`gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna` et `gpt-5.4-mini`. Ouvrez `nxc gui` pour remplacer ou
...
```bash
nxc login xai          # ou : anthropic, kimi, kiro, google-antigravity, cursor
nxc logout xai
```
...
nxc restore       # restaure Codex natif sans arrêter le proxy (alias : nxc eject)
nxc restore back  # route de nouveau Codex par le proxy toujours actif
```
