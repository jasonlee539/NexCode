...
description: Pourquoi le processus bun peut utiliser plusieurs gigaoctets de RAM sous Windows, les mesures actuellement prises par nexcode et les solutions possibles dans l’attente des correctifs de Bun.
---

Sous Windows, certains utilisateurs constatent que le processus `bun` derrière nexcode atteint plusieurs gigaoctets de RSS au cours de longues sessions en flux continu (problème signalé dans [#314](https://github.com/lidge-jun/nexcode/issues/314)). Cette page explique avec transparence ce qui se produit réellement et les mesures que vous pouvez prendre.

## Cause première : problèmes en amont dans le runtime Bun

nexcode intègre le runtime Bun (actuellement en version **1.3.14**). Cette croissance de la mémoire provient de problèmes connus en amont dans Bun, et non de fuites au niveau du code JavaScript du proxy :
...
Sous Windows, nexcode doit conserver un chemin de code prudent pour diffuser les réponses afin d’éviter le plantage #32111. Or, ce chemin est le plus exposé au problème de contre-pression : lorsqu’un client est lent ou bloqué, le runtime peut continuer à mettre en mémoire tampon des données en amont dans une zone native que JavaScript ne peut pas limiter.

## Mesures actuellement prises par nexcode
...
- **`nxc doctor`** — une section « Memory / runtime » affiche la version de Bun du processus de *service*, les compteurs RSS, external/ArrayBuffers, le contexte du tas JS et la décision relative au mode de flux. Avec le runtime Bun 1.3.14 intégré, `heapUsed` / `jscHeap` ne permettent pas à eux seuls de caractériser une fuite ; comparez la mémoire observée à `responseState` et à plusieurs échantillons avant de conclure à une fuite au niveau de l’application.
...
1. **Attendre une mise à jour du runtime intégré.** Dès qu’une version de Bun inclura les correctifs de manière vérifiable, nexcode mettra à niveau son runtime intégré et activera automatiquement le chemin de flux plus sûr sous Windows (macOS continuera d’exiger l’activation explicite ci-dessous).

2. **Utiliser un runtime Bun de confiance avec `NEXCODE_BUN_PATH`.** Cette configuration n’est pas validée : vous exécutez nexcode avec un runtime que nous n’avons pas testé, à vos risques et périls. Point important pour les installations en tant que service : la valeur de remplacement est lue **lors de la génération de l’artefact du service**, et non au démarrage de celui-ci. Définissez la variable d’environnement, puis réexécutez `nxc service repair` depuis le même shell afin d’inscrire ce chemin dans la définition persistante du service. Définir uniquement la variable d’environnement ne change rien pour un service déjà installé.
...
Si vous essayez l’une de ces solutions avec une charge Windows réelle, veuillez publier les sections de mémoire de `nxc doctor` avant et après dans [#314](https://github.com/lidge-jun/nexcode/issues/314) — c’est précisément la vérification attendue pour cette mesure d’atténuation.
