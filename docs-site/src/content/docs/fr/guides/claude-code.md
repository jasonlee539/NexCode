---
title: Claude Code
description: "Utilisez n'importe quel modèle routé depuis Claude Code : nexcode fournit l'API Anthropic Messages et la découverte des modèles de passerelle sur le même port."
---

nexcode sert `POST /v1/messages` (ainsi que count_tokens) parallèlement à `/v1/responses`. Claude Code peut
...
Vous pouvez vous connecter à plusieurs comptes Claude via le tableau de bord des fournisseurs (`nxc login anthropic` /
...
`nxc claude` s'assure que le proxy est en cours d'exécution, puis lance Claude Code avec l'environnement câblé :
...
Les variables que vous exportez vous-même gagnent toujours. Les arguments supplémentaires passent par : `nxc claude -p "hello"`.
...
remplacer silencieusement un abonnement claude.ai valide par la facturation API. Désormais, `nxc claude`
...
nexcode peut détecter ; la détection automatique constitue donc le comportement par défaut.

Laissez le **Mode Auth** activé **Auto** (valeur par défaut) dans **Claude → Claude Code** et nexcode
...
appel de `nxc claude`, sans reconfiguration.
...
`claude` lancée sans passer par `nxc` se comporte donc de la même manière. Le fichier d'environnement est un instantané actualisé au
démarrage du proxy ou lors de l'enregistrement des paramètres, tandis que `nxc claude` effectue toujours une résolution immédiate.
...
```bash
nxc claude desktop [apply]
nxc claude desktop show [--json]
nxc claude desktop move <route> <opus|fable|sonnet|haiku> [--default]
nxc claude desktop default <opus|fable|sonnet|haiku> <route|none>
nxc claude desktop export <path|->
nxc claude desktop import <path> [--apply]
```

`nxc claude desktop` et `apply` écrivent tous deux le profil actuel dans Claude Desktop. `show` affiche un
...
`NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR` pour remplacer explicitement la bibliothèque, ou
...
Lorsque `claudeCode.systemEnv` est réglé sur `true` (par défaut : **off**), `nxc start` utilise `launchctl setenv`
...
par le proxy sans nécessiter le wrapper `nxc claude`. Les shells déjà ouverts ne sont pas concernés et doivent
être rouverts.

`nxc stop` et l'arrêt du proxy **suppriment les variables injectées** (ils ne restaurent pas les valeurs précédentes —
seules les clés injectées par nexcode sont supprimées). Le proxy écrit également `~/.nexcode/claude-env.sh` ;
`nxc start` installe le hook source `.zshrc` uniquement lorsqu’un exécutable Claude Code CLI est
présent dans le `PATH`. Au démarrage et avec `nxc ensure`, le hook appartenant à NexCode est supprimé
...
La fonctionnalité est réservée à macOS ; sur les autres plateformes, utilisez `nxc claude`.
...
`x-nexcode-api-key` et `origin` sont toujours supprimés avant le transfert. Sur une liaison sans bouclage,
le relais natif nécessite également des informations d'identification de proxy valides dans `x-nexcode-api-key` ; `Authorization`
...
« Les connecteurs claude.ai sont désactivés » n'apparaît plus avec `nxc claude`.
...
par `claude` ou `anthropic`, nexcode expose les modèles routés sous forme d'alias stables et réversibles :

| Surface | Format | Exemple |
| --- | --- | --- |
| Claude Code CLI | `claude-nxc-<provider>--<model>` (simple) ou `claude-nxc2-…` (échappé) | `claude-nxc-native--gpt-5.6-sol` |
...
anciens alias hachés et les identifiants `claude-nxc-<provider>--<model>` des configurations antérieures sont
toujours résolus.

Si le sélecteur situé au bas de Claude Desktop ne modifie pas le modèle d'une conversation 3P déjà en cours,
utilisez `/model <id>` dans cette conversation. NexCode ne peut pas observer l'état du sélecteur ; il
...
Les identifiants de modèle simples, sans `/` ni `~`, conservent le préfixe v1 `claude-nxc-…`. Ceux qui contiennent `/` ou
`~` utilisent le préfixe v2 `claude-nxc2-…` avec des échappements (`/` → `~s`, `~` → `~t`), par exemple :
`openrouter/anthropic/claude-opus-4-8` → `claude-nxc2-openrouter--anthropic~sclaude-opus-4-8`.
...
`effectiveModelEnv` calcule six emplacements injectés par `nxc claude`, l'environnement système ou le fichier d'environnement du shell :
...
Lorsque `tierModels.haiku` et `smallFastModel` sont absents, NexCode laisse les deux variables auxiliaires non définies ; Claude Code choisit ensuite son modèle d'assistance natif (actuellement Sonnet), qui peut entraîner des frais de fournisseur natif.

## Agents de la liste (injectAgents)

`nxc claude` (ainsi que le démon d'environnement système) synchronise la liste des sous-agents exposés (onglet Sous-agents,
jusqu'à 5 modèles) ainsi que `nxc-self` dans `~/.claude/agents/nxc-*.md`.

- **`nxc-self`** épingle le modèle par défaut de votre sélecteur `/model` (avec repli sur `claudeCode.model`) ; il est omis
  quand ni l’un ni l’autre n’existe. Il utilise l'héritage de modèle.
- Chaque corps d'agent contient une directive `<!-- nxc-route: <model> -->` — le proxy l'utilise pour
...
- Seuls les fichiers `nxc-*.md` vérifiés par marqueur contenant `generated-by: nexcode` sont toujours
...
Utilisation : `subagent_type: "nxc-gpt-5-6-sol"`. Les cibles compatibles avec un contexte de 1M portent automatiquement `[1m]`.
...
par défaut, nexcode remplace donc le contenu de la compétence par un bref contenu de remplacement dans les requêtes **routées**.
...
Les modèles routés ne disposent pas tous des mêmes outils hébergés ou de la même prise en charge des images. nexcode comble ces lacunes
...
Une valeur `backend` explicite est toujours prioritaire. Si elle est omise, nexcode sélectionne `anthropic` lorsqu'un
compte OAuth Anthropic enregistré existe ; sinon, il sélectionne `openai`. Une sélection explicite de
`anthropic` sans identifiants utilisables **échoue de manière sûre** : nexcode n'emprunte pas silencieusement les
...
`nxc debug claude on|off|status|reset`, `NXC_CLAUDE_DEBUG=1` ou `PUT /api/debug {"claude": true}`
...
- Démarrage rapide (`nxc claude`) et bloc d'environnement manuel
...
y compris `usage.server_tool_use.web_search_requests`. Mettez à jour nexcode si une ancienne version terminait
...
dans votre shell. `nxc claude` s'abstient délibérément de définir `ANTHROPIC_API_KEY` ; si vous l'avez exportée,
supprimez-la de l'environnement. `nxc claude` injecte `ANTHROPIC_BASE_URL`, la découverte, le contexte automatique et les modèles configurés, mais jamais `ANTHROPIC_API_KEY`.
...
défini (automatique avec `nxc claude`). Exécutez `nxc claude` pour actualiser le cache des modèles de la passerelle dans
...
une valeur `ANTHROPIC_BASE_URL` obsolète. Ouvrez un nouveau terminal ou réexécutez `nxc claude`.
...
quand un modèle Claude est mentionné. Ce comportement est normal avec le transfert natif ; pour les modèles routés, nexcode la remplace
par défaut par un contenu minimal (`blockedSkills: ["claude-api"]`).

**Les sous-agents sont envoyés au mauvais modèle** — Les agents de la liste (`nxc-*`) utilisent les directives
`<!-- nxc-route: ... -->`, et non l'argument `model` de l'outil Agent. Vérifiez que la directive désigne la route voulue.
