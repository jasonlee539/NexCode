...
- Configurez un fournisseur `xai` avec une **clé API**. Le pont envoie systématiquement les requêtes au
  ...
  configuré est ignoré pour les appels d'images. OAuth ou `nxc login xai` seul n'active **pas** le pont
...
`~/.nexcode/config.json`. Le pont est **facultatif** : vous devez définir `bridgeEnabled: true`
...
Les images générées sont enregistrées dans `~/.nexcode/artifacts/`. Pour éviter une croissance illimitée
...
1. Lorsqu'une requête Responses répertorie `image_generation` dans `tools`, NexCode le détecte pendant le
...
3. Lorsque le modèle appelle cet outil, NexCode intercepte l'appel et envoie l'invite à l'API de génération
   d'images de xAI.
4. Les images générées sont enregistrées dans `~/.nexcode/artifacts/`, puis leur **chemin de fichier local**
