---
title: opencode
description: Utilisez n’importe quel modèle routé depuis opencode — nexcode injecte un bloc de fournisseur à l’exécution sans modifier votre propre configuration opencode.
---

opencode lit ses fournisseurs dans des couches de configuration JSON fusionnées plutôt que dans des variables
d’environnement ; il n’existe donc aucun emplacement de type `ANTHROPIC_BASE_URL` dans lequel injecter une valeur. `nxc opencode`
...
`provider.nexcode` généré pour ce processus. Les arguments supplémentaires sont transmis tels quels :
`nxc opencode run "hello"`.

Les modèles acheminés apparaissent dans le sélecteur sous le fournisseur `nexcode` :

```text
nexcode/kiro/glm-5
nexcode/gpt-5.6-sol      # native slugs stay unprefixed
```
...
lire la configuration globale ou celle du projet afin de détecter une redéfinition de `provider.nexcode`, tandis que vos
...
Pour ce lancement uniquement, nexcode ajoute le bloc `provider.nexcode` généré via
...
| Couche | Comportement avec `nxc opencode` |
| --- | --- |
| Configuration globale/personnalisée/de projet | Conservée sur disque exactement telle que vous l’avez écrite |
| Exécution en ligne (`OPENCODE_CONFIG_CONTENT`) | Reçoit uniquement le bloc `provider.nexcode` généré |
| Chemins relatifs `{file:…}` | Toujours résolus par rapport au fichier de configuration qui les a définis à l’origine |

Si une configuration globale ou de projet définit également `provider.nexcode`, le lanceur affiche une
note d’information : la couche d’exécution de `nxc opencode` la remplace pour ce lancement.

## Ajouter le bloc à votre propre configuration

`nxc opencode` injecte le bloc fournisseur pour un seul lancement, ce qui signifie simplement `opencode` toujours
ne sait rien du proxy. Lorsque vous souhaitez que les modèles acheminés soient disponibles à partir de `opencode` — ou
depuis une extension d'éditeur qui ne passe jamais par le lanceur — `nxc export` imprime la même chose
...
Fusionnez le bloc `provider.nexcode` dans votre configuration existante. Remplacer tout le fichier par le
celui exporté détruit vos autres fournisseurs, agents, raccourcis clavier et entrées MCP. `nxc export --out`
...
catalogue. Réexécutez `nxc export` après avoir ajouté un fournisseur ou modifié la visibilité du modèle.
...
```bash
export NEXCODE_OPENCODE_API_KEY=<your key>
```

## La clé d’admission n’est pas écrite sur disque

La configuration enregistre la référence `{env:NEXCODE_OPENCODE_API_KEY}`, jamais le secret lui-même.
Sur une liaison de bouclage, cette référence est utilisée comme valeur `apiKey`. Sur une liaison hors
bouclage, OpenCode résout la variable et n’envoie sa valeur que dans `x-nexcode-api-key`, afin que
...
`NEXCODE_API_AUTH_TOKEN` est prioritaire, puis le fichier de jeton de service renforcé, puis
...
voir [Accès à distance](/fr/reference/configuration/server/#accès-à-distance). Cette clé d'admission est celle de nexcode
propre et n'est pas lié aux clés du fournisseur en amont configurées sous
[Prestataires](/fr/guides/providers/).

## Rétablissement

Rien à annuler — aucun fichier de configuration généré n'est écrit sous `~/.nexcode`. Courez simplement
...
aucune limite de sortie faisant autorité par modèle, nexcode émet également un budget `output` de `32000`, limité
...
Le bloc fournisseur `nexcode` est régénéré à chaque lancement, donc des ajustements par modèle y sont apportés
