---
title: Fournisseurs
description: Toutes les méthodes utilisées par nexcode pour s'authentifier auprès d'un fournisseur de LLM et communiquer avec lui — OAuth, clé API, transfert ChatGPT et exécution locale.
---
...
définis sous `providers` dans `~/.nexcode/config.json`.
...
`~/.nexcode/config.json.pre-openai-tiers-v2.bak` ; restaurez-la avec
...
expérimental et non officiel reposant sur un flux d'autorisation d'appareil. nexcode enregistre leurs identifiants dans
`~/.nexcode/auth.json` et les actualise automatiquement. La CLI de connexion accepte également `chatgpt` ;
...
Après un échec définitif d'actualisation de Nous, exécutez `nxc login nous` pour vous réauthentifier.

Pour les préréglages canoniques du forfait Kimi Coding (`kimi` pour la connexion au compte et `kimi-code`
pour la clé API), nexcode ne transmet à la requête Chat Completions qu'un `prompt_cache_key` stable fourni
...
restent. Si un service en amont explicitement activé rejette ce champ, nexcode ne le retire pas avant de
...
registre distinct. Les jetons restent dans `~/.nexcode/auth.json` ; `/api/oauth/accounts` ne renvoie que des
métadonnées masquées.

### Importation Cockpit Tools Antigravity

Dans la v1, NexCode importe uniquement les exportations JSON **Cockpit Tools Antigravity** destinées au
...
nexcode coordonne l'actualisation des jetons et le routage du pool Codex afin que les requêtes simultanées
...
nexcode ne fabrique **aucune** identité de client officiel, comme les en-têtes `originator`, de session ou
de fil, si l'appelant ne les a pas fournis.

**Diagnostics et réauthentification.** La sortie de `nxc status` destinée aux utilisateurs affiche un bloc d'état OAuth —
identifiants de compte expurgés, aucun jeton. `nxc doctor` ajoute une section sur la fiabilité OAuth, avec des
...
`nxc login <provider>` ou utilisez **Réauthentifier** dans le tableau de bord. Les comptes du pool Codex ne
constituent pas un fournisseur `nxc login` : réauthentifiez-les dans le groupe de comptes Codex du tableau de bord. Consultez
[`nxc status` / `nxc doctor`](/fr/reference/cli/) dans la référence CLI.
...
En l'absence de session `kiro-cli`, `nxc login kiro` se rabat sur un jeton d'accès collé ou sur la variable
d'environnement `KIRO_ACCESS_TOKEN`.

Le parcours d'importation `nxc login kiro` recherche les magasins de la CLI Kiro propres à la plateforme et
...
- `KIROCLI_DB_PATH` sélectionne une base de données Kiro CLI SQLite non standard. Le chemin doit déjà exister ;
  pendant ce chemin d'importation, nexcode ne crée ni ne modifie la base de données, ni les fichiers WAL ou SHM.
...
**Ajouter un compte** nécessite également le binaire local de la CLI : nexcode consulte d'abord `PATH`, puis se rabat sur
`%LOCALAPPDATA%\Kiro-Cli\kiro-cli.exe` et `C:\Program Files\Kiro-Cli\kiro-cli.exe`.

Après une importation réussie, nexcode conserve les informations d'identification importées dans
`~/.nexcode/auth.json`.
...
**Ajouter un compte** suit un parcours d'écriture distinct : nexcode crée un instantané de la session en
cours, déconnecte `kiro-cli`, puis importe la nouvelle connexion effectuée dans le navigateur. Si la connexion
est annulée ou échoue, y compris pendant l'enregistrement des identifiants par NexCode, la restauration remplace
...
nexcode fournit 79 préréglages intégrés : 67 à clé, huit OAuth, trois locaux et un préréglage par défaut de
...
est intentionnel : le premier segment sélectionne le fournisseur nexcode, tandis que `cline-pass/kimi-k3`
...
l'entrée de la passerelle. nexcode conserve les niveaux demandés ; toute normalisation propre au service en
...
`opencode-free`). Lorsque Zen omet `Retry-After` sur une telle réponse 429, nexcode ajoute à l'erreur client
...
Responses final, nexcode applique une réparation après un délai de grâce de cinq secondes, limitée à ce
...
> l'abonnement ou le bannissement du compte. Le routage de Codex ou Claude Code par nexcode correspond à
...
256 lignes brutes. `nxc login command-code` prend en charge OAuth par connexion dans le navigateur, avec
...
**Quota Command Code.** Le tableau de bord et `nxc account refresh` sondent les fenêtres
...
de bord et dans la sortie de `nxc account refresh <provider>`. Le nom du fournisseur est libre ; la détection
...
Utilisez `nxc account list`, `nxc account current` et `nxc account use` pour consulter ou changer les mêmes
groupes de comptes Codex, de comptes OAuth et de clés API sans ouvrir le tableau de bord. Consultez la
[référence de la CLI](/fr/reference/cli/providers-accounts/#nxc-account-subcommand) pour les commandes, la sortie JSON et le
comportement lors de l'ouverture d'une nouvelle session.

### Routes de préversion GPT-5.6

GPT-5.6 Sol/Terra/Luna sont préchargés dans les listes de repli des fournisseurs afin que `nxc sync` puisse
...
Un fournisseur est inclus lorsque nexcode dispose d'un adaptateur de protocole correspondant, **et non** selon
...
**GitHub Copilot** est un fournisseur OAuth (`nxc login github-copilot`) qui échange une connexion GitHub par
...
`/chat/completions` pour le trafic d'agent. nexcode route donc ces modèles sur l'API Responses par défaut,
...
Cursor est géré séparément comme adaptateur expérimental. `adapter: "cursor"` apparaît dans `nxc init` et dans
...
nexcode utilise le transport HTTP/2 direct de Cursor. Sa liste de repli intégrée comprend `gpt-5.6-sol` /
...
Codex ; définissez
`unsafeAllowNativeLocalExec: true` sur l'objet `providers.cursor` dans `~/.nexcode/config.json`
...
forme de points d'intégration pour un exécuteur ; sans exécuteur local configuré, nexcode renvoie des résultats
...
[ollama.com/settings/keys](https://ollama.com/settings/keys). nexcode classe les modèles cloud selon leurs
...
Faites pointer nexcode vers un serveur local compatible OpenAI, généralement avec une clé vide :
...
**Personnalisé** dans le tableau de bord ou `custom` dans `nxc init`, puis saisissez l'URL de base. Consultez la
