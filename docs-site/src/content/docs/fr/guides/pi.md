---
title: Pi
description: Utilisez n’importe quel modèle routé depuis Pi — nxc export produit un bloc de fournisseur personnalisé pour le fichier models.json de Pi, relié au proxy en cours d’exécution.
---

Pi lit ses fournisseurs dans un fichier JSON global unique plutôt que dans des variables d’environnement ;
nexcode ne lance donc pas Pi. À la place, `nxc export` sérialise le bloc du fournisseur `nexcode` —
...
`nxc export` n’écrit jamais dans ce fichier. Fusionnez-y le bloc `providers.nexcode` : remplacer le
...
```bash
nxc export --client pi --out ~/nexcode-pi-models.json
nxc export --client pi --json > ~/nexcode-pi-models.json   # or redirect the byte-exact JSON
```
:::

Le bloc exporté est un instantané statique et non une vue en direct. Réexécutez `nxc export` après avoir ajouté un
...
| --- | --- | --- |
| Clé d'admission proxy | Les propres informations d'identification de nexcode, générées dans l'onglet **API** du tableau de bord | référencé par `apiKey` comme `$NEXCODE_API_KEY` ; la valeur reste dans votre environnement |
| Clé du fournisseur | votre touche Anthropic / OpenAI / OpenRouter | La propre configuration de nexcode, selon [Fournisseurs](/fr/guides/providers/) |

La configuration exportée ne contient que la référence, jamais le secret. Pi interpole une valeur simple de la forme `$NAME` ;
la variable est :

```bash
export NEXCODE_API_KEY=<your key>
```

Ce nom est propre à Pi. opencode utilise une autre variable
(`NEXCODE_OPENCODE_API_KEY`, sous la forme `{env:…}`) — voir le [guide opencode](/fr/guides/opencode/).

**Un proxy lié à l’interface de bouclage n’a besoin d’aucune clé.** Par défaut, nexcode se lie à `127.0.0.1` et n’y exige
aucune authentification ; la référence `$NEXCODE_API_KEY` est donc inerte et la variable peut rester indéfinie.
...
`nxc export` affiche le nombre de lignes concernées.
...
Le champ `cost` est volontairement absent. Il exige les quatre champs de prix, alors qu’NexCode ne possède
...
contrôle de l’effort exactement pour les modèles auxquels NexCode permet de l’envoyer. L’export produit
...
[ouvrir un ticket](https://github.com/lidge-jun/nexcode/issues) en indiquant le message renvoyé par Pi.
:::

## Exigences

Un proxy nexcode en cours d'exécution (`nxc start`) et Pi installés. `nxc export` lit le catalogue en direct
