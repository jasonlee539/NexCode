...
son propre contexte et ses propres outils, afin que plusieurs tâches indépendantes puissent s'exécuter en parallèle. nexcode contrôle
...
de v2, pas un quatrième mode de catalogue. CLI : `nxc v2 mode v2` puis `nxc v2 keep-native-v1 on`.
...
nexcode applique cela comme passe finale à la fois au catalogue `/v1/models` en direct et au catalogue synchronisé
...
- `injectionModel` est le modèle de travailleur préféré nommé dans le guide nexcode.
...
`multiAgentGuidanceEnabled` est activé par défaut et constitue le réglage principal des directives produites par nexcode
sur les deux surfaces. Sa désactivation supprime à la fois le bloc de désignation v2 et le texte proactif v1.

Pour les requêtes Responses sans état dont l’entrée est un tableau, nexcode place les instructions générées après les
...
Le guide v2 intégré dispose d’un budget de 700 caractères. S’il devait dépasser ce budget, nexcode réduit
...
Sur la v1, nexcode injecte uniquement les conseils de délégation proactive de style amont à `max` ou `ultra`
effort. Il n’ajoute aucun modèle préféré, aucune liste, aucune chaîne de repli ni aucune invite personnalisée en v1.

L'option `syncCodexSubagentDefaults` désactivée par défaut est distincte du guidage. Quand nexcode possède
...
nexcode met à jour ou supprime uniquement les champs portant ses marqueurs. Si l'un des champs cibles appartient à l'utilisateur,
...
Pour un travailleur généré, nexcode construit cet ordre de priorité :

1. Le modèle primaire demandé.
2. Une chaîne par modèle de `subagentModelFallbackByModel` dans nexcode config, saisie par
   le modèle primaire demandé.
3. La liste `subagentModelFallback` globale dans nexcode config.

Les chaînes de secours par rôle appartiennent à nexcode config, pas à
...
(#1190). nexcode peut toujours lire une ligne `model_fallback` héritée du TOML pour
compatibilité ascendante, mais `nxc doctor` en avertit et Codex lui-même ignorera
le rôle concerné.

Les identifiants de modèle en double sont supprimés tout en préservant la première occurrence. Lors de la sélection, nexcode
...
limitation connue [#92](https://github.com/lidge-jun/nexcode/issues/92).

nexcode échoue en toute sécurité au lieu de transférer une tâche vide ou illisible :
...
Utilisez `nxc v2` pour les paramètres de la surface de collaboration et des fonctionnalités natives :
...
nxc v2 mode default
nxc v2 mode v2
nxc v2 keep-native-v1 on
nxc v2 keep-native-v1 off
nxc v2 threads 8
```

Utilisez `nxc agent` pour les paramètres de délégation, de liste, de plafond d'effort et de secours :

```bash
nxc agent status
nxc agent injection set --model anthropic/claude-sonnet-5 --effort xhigh
nxc agent subagents set gpt-5.6-sol,anthropic/claude-sonnet-5
nxc agent fallback set gpt-5.4-mini,xai/grok-4.5 --poll-ms 60000
nxc agent effort set --subagent max
```

Passez `-` pour effacer une valeur `nxc agent injection` nullable, ou utilisez l'action `clear` appropriée pour un
...
état du catalogue, exécutez `nxc sync` et redémarrez cette surface Codex.

### Que se passe-t-il lorsqu’nexcode ne peut pas considérer le catalogue comme fiable ?

nexcode compare le catalogue de modèles sur disque à l'heure de démarrage de chaque Codex serveur d'applications détenu
...
| `stale` | Au moins un serveur d'applications est antérieur au catalogue | **Aucune directive sur le modèle rédigé par nexcode** |
| `unknown` | La comparaison n'a pas pu être faite | **Aucune directive sur le modèle rédigé par nexcode** |

Pour `stale` et `unknown`, nexcode omet ses propres indications dérivées du disque — modèle préféré, liste,
...
séparément par `nxc doctor`. `stale` s'efface uniquement après chaque démarrage du serveur d'applications Codex détecté après
...
niveau que Codex convertit en `max` ; nexcode mappe ou bloque ensuite la valeur du fournisseur sélectionné.
