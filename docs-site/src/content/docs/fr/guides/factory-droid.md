...
| `${DROID_BRIDGE_TOKEN}` n’est pas résolu | La variable est absente de l’environnement du service NexCode | Injectez-la dans le processus de service, pas seulement dans un terminal interactif |
...
nxc doctor
nxc access test droid/glm-5.2 --protocol responses
```
