---
title: Intégrations
description: Connectez nexcode à OpenCode, Pi, OMP, Hermes, OpenClaw, Kimi Code, Gajae Code, DeepSeek Harness et MiniMax Code depuis le tableau de bord — un commutateur par client, avec une sauvegarde avant chaque écriture.
---

L'onglet **Intégrations** écrit le bloc fournisseur d'nexcode dans le fichier de configuration du client,
...
|---|---|---|---|---|
| OpenCode | `~/.config/opencode/opencode.json` | JSON | au prochain lancement direct | `NEXCODE_OPENCODE_API_KEY` |
| Pi | `~/.pi/agent/models.json` | JSON | dans les nouvelles sessions | valeur fictive de bouclage |
| OMP | `~/.omp/agent/models.yml` | YAML | après le redémarrage d'OMP | valeur fictive `nexcode-loopback` |
| Hermes | `~/.hermes/config.yaml` | YAML | dans les nouvelles sessions | `NEXCODE_HERMES_API_KEY` |
| OpenClaw | `~/.openclaw/openclaw.json` | JSON5 | immédiatement, sur une passerelle en cours d'exécution | `NEXCODE_OPENCLAW_API_KEY` |
| Kimi Code | `~/.kimi-code/config.toml` | TOML | au redémarrage ou avec `/reload` | valeur fictive de bouclage |
| Gajae Code | `~/.gjc/agent/models.yml` | YAML | dans les nouvelles sessions ou à l'ouverture de `/model` |`NEXCODE_GAJAE_API_KEY` |
...
La prise en charge gérée de DSH exige au minimum **DSH 0.1.0-rc.6**. NexCode ne possède que le fragment
`llm-pi-ai.providers.nexcode` : **Appliquer** et **Actualiser** remplacent ce fragment, **Désactiver** ne
...
`~/.minimax`. Son bloc géré ne possède que `custom_provider.nexcode`. Il ne modifie ni `defaultModel`, ni
la source d’identification MiniMax sélectionnée, ni la connexion MiniMax de l’utilisateur. Après l’avoir
connecté, choisissez dans MCode une entrée `custom_provider:nexcode/<provider/model>`.
...
au bouclage ; la configuration distante de `x-nexcode-api-key` est reportée. Les chemins déplacés définis
...
nexcode lit ces variables dans son propre environnement. Si votre passerelle utilise un profil ou un
dossier personnel déplacé, lancez nexcode avec les mêmes variables ; sinon, il suivra correctement une
autre installation.

## Les quatre autres surfaces ne sont pas des commutateurs

**Clés API** gère les propres identifiants d'nexcode et n'est donc pas un client. **Codex CLI** est relié
par le service du proxy lui-même : démarrer nexcode applique ce routage et l'arrêter restaure le routage
...
La désactivation ne supprime que les entrées enregistrées par nexcode comme lui appartenant. Si votre
...
d'écriture ne modifie, octet par octet, que sa propre plage `providers.nexcode` ; le reste du fichier
...
d'nexcode ont été modifiées, le commutateur se verrouille et la désactivation est refusée plutôt que de
...
OMP et DSH font exception : leurs outils d'écriture YAML ne modifient que `providers.nexcode` et
`llm-pi-ai.providers.nexcode`, respectivement, tout en préservant octet par octet les commentaires et le
...
bouclage.** Les quatre premiers n'ont aucun champ de configuration pour l'en-tête `x-nexcode-api-key`
...
`x-nexcode-api-key`. Pour l'instant, la configuration manuelle d'OMP à distance sort du périmètre de
l'intégration gérée.

**Kimi Code ne peut pas contenir de référence à une variable d'environnement** ; sa configuration reçoit
donc la valeur fictive `nexcode-loopback` plutôt qu'une clé. Aucun véritable identifiant n'est écrit dans
une configuration cliente.

**Pour `nxc opencode`, le bloc fournisseur du lanceur l'emporte.** Le lanceur injecte
`provider.nexcode` par `OPENCODE_CONFIG_CONTENT`, qui est prioritaire sur la même entrée enregistrée sur
...
```bash
nxc integration client status
nxc integration client enable --client hermes
nxc integration client disable --client hermes
nxc integration client history --client hermes
...
Une fois l’intégration connectée, `nxc sync` actualise également le bloc MCode géré avec les fenêtres de
...
commandes textuelles utilisent le point de terminaison compatible avec Anthropic de MiniMax ; NexCode
fournit donc un lanceur isolant les identifiants et limité à l’adresse locale :

```bash
nxc mmx text chat --model anthropic/claude-opus-5 --message "Hello"
nxc mmx text repl --model openai/gpt-5.6-sol
```
