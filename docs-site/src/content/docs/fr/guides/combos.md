...
demande `combo/<id>` ; nexcode choisit une cible, réécrit la requête vers le sélecteur concret
...
`anthropic/claude-opus-4-8`. Si cette tentative rencontre un échec autorisant une nouvelle tentative, nexcode peut basculer vers
`openai/gpt-5.6-sol`.
...
L'identifiant du combo dans `nxc combo set <id>` doit commencer par une lettre ou un chiffre. Il peut alors contenir
...
[nexcode #241](https://github.com/lidge-jun/nexcode/issues/241).
...
valeur `Retry-After` valide, nexcode l’utilise à la place. Les secondes numériques et les valeurs de date HTTP sont
...
Si la requête n'a pas d'objet `reasoning`, nexcode en crée un. Si `reasoning` existe sans
`effort`, il préserve les autres champs et ajoute la valeur par défaut. Un effort fourni par l’appelant n’est
jamais écrasé.

Lorsque la capacité cible est inconnue ou n'inclut pas l'effort configuré, nexcode omet le
...
Il existe une limitation importante pour les sous-agents Codex v2 ([issue #92](https://github.com/lidge-jun/nexcode/issues/92)).
...
y compris après un échec autorisant une nouvelle tentative. Si le combo ne comporte aucune cible capable de déchiffrer la tâche, nexcode s’arrête
...
`remove` ; et les mêmes sous-commandes sont disponibles sous `nxc route combo`.
...
`nxc combo list`, vérifiez l'orthographe et la casse, et confirmez que votre commande de gestion a écrit sur le même
exécution d'une instance nexcode qui reçoit des requêtes de modèle.
