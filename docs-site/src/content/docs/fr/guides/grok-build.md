---
title: Grok Build
description: Utilisez n’importe quel modèle routé par nexcode depuis la CLI Grok Build de xAI — les modèles sont automatiquement enregistrés dans ~/.grok/config.toml pendant l’exécution du proxy.
---

nexcode expose un point de terminaison compatible OpenAI `POST /v1/chat/completions` (ainsi que `/v1/responses`) sur son
...
cette intégration, nexcode enregistre automatiquement l’intégralité de son catalogue visible dans Grok Build :
aucune modification manuelle de la configuration n’est nécessaire.

## Enregistrement automatique

Lorsque `~/.grok` existe, `nxc start` (et `nxc ensure` / `nxc restart`) écrit un bloc géré
en `~/.grok/config.toml` :
...
- **Idempotent :** chaque exécution de `nxc start` (ainsi que de `nxc ensure` lorsque le démarrage automatique est activé) remplace
  le bloc délimité par le catalogue actuel.
- **Supprimé à l’arrêt :** `nxc stop`, `nxc eject`, `nxc uninstall` et l’arrêt normal
  du démon hors service suppriment le bloc délimité et restaurent votre fichier
  octet pour octet. Sous un gestionnaire de service, le démontage passe par `nxc stop`/`nxc
...
- **Les alias en conflit** déjà définis dans vos propres tables `[model.*]` sont respectés
  (nexcode ajoute un suffixe à ses propres entrées) ; un bloc délimité endommagé (marqueur de début sans marqueur de fin)
...
`reasoning_efforts`. Pour les entrées de modèles routés, nexcode reflète les niveaux configurés pour le fournisseur
...
Grok Build communique avec nexcode au moyen de Chat Completions et envoie `reasoning_effort` lorsque
...
injectées contiennent une valeur fictive (`nexcode-loopback`) ; nexcode ignore les clés d’admission pour les
connexions de bouclage, de sorte qu’aucun véritable secret n’est utilisé.

**L’enregistrement automatique est réservé au bouclage.** Lorsque nexcode se lie à un hôte hors bouclage, y compris
...
`nxc start`/`ensure`/`restart`. Donc nexcode n’écrit rien du tout dans ce cas (et supprime
tout bloc restant d'une liaison de bouclage précédente), et vous configurez les modèles vous-même
en dehors des marqueurs gérés, où rien de ce que nexcode fait ne peut les écraser. Voir
...
donc les tours contre nexcode n'ont pas besoin de connexion Grok supplémentaire. Gardez votre `grok login` /
...
Si vous gérez `~/.grok/config.toml` vous-même — ou si nexcode est sur une liaison sans bouclage — ajoutez
tables par modèle avec **champs directs**, en dehors des marqueurs `# >>> nexcode managed block` :
...
- **Installé par le service `nxc restart` :** le proxy en cours d'exécution possède l'autorisation de redémarrage et la vidange
...
- **Moment de lecture de la configuration :** démarrez d’abord nexcode, puis lancez `grok` pour obtenir les résultats les plus
  ...
  invalide *l'intégralité* de la couche de configuration utilisateur, c'est pourquoi nexcode écrit le fichier
...
- **Mises à jour du catalogue :** le bloc délimité reflète le catalogue au moment de l’injection. Après
  l’ajout de fournisseurs ou de modèles, exécutez `nxc ensure` (ou redémarrez le proxy) pour l’actualiser.
