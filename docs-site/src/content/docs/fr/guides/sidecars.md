...
Tous les modèles routés ne proposent pas une **recherche web** hébergée ni une **entrée d’image** native. nexcode complète
...
Lorsque Codex demande un hébergement `web_search` pour un modèle routé sans passage, nexcode :
...
2. Exécute le modèle routé dans une petite **boucle d'agent**. Lorsqu'il appelle `web_search`, nexcode utilise le
...
Chaque itération du modèle routé envoie `stream: true` en amont, mais, par défaut, nexcode met entièrement
...
nexcode décrit chaque image **avant** l'appel principal et la remplace par du texte. Quand
