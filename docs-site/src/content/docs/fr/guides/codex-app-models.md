...
description: Comment les modèles nexcode apparaissent dans Codex App, Codex CLI et Codex TUI par l'intermédiaire du catalogue Codex partagé.
---

nexcode ne modifie pas Codex App. Il écrit la même configuration et le même catalogue de modèles Codex
...
dont les comptes associés existent toujours, nexcode ajoute une ligne
...
la famille OpenAI, pris en charge par l'API mais absent de l'ensemble statique d'nexcode, cet identifiant
...
`gpt-daybreak-blue-latest` ; nexcode ne le réécrit pas en Sol, ne crée aucune ligne non qualifiée et
...
configuration v1 livrée dans `~/.nexcode/config.json.pre-openai-tiers-v2.bak`. Restaurez cette copie avec :
...
Pour un modèle routé équivalent, nexcode propose un mode explicite de combinaison avec alias natif,
...
`nxc init`, `nxc start` et `nxc sync` relient au proxy la configuration et le catalogue Codex partagés.
...
Le sélecteur de modèles de Codex attend des entrées ayant la structure de son propre catalogue. nexcode
...
de shell, les indicateurs de prise en charge de l'API et les instructions de base. nexcode retire ensuite
...
Sol/Terra/Luna. Pour la famille GPT-5.5/5.4, nexcode conserve les entrées dynamiques plus riches du
...
`priority`. nexcode préserve cette distinction. Les modèles OpenAI natifs transférés conservent la prise en
...
la page remplace la liste par les choix visibles dans le tableau de bord. nexcode attribue des priorités de
...
nexcode n'a aucun accès à cette liste ; le défaut amont est suivi dans
...
nexcode réécrit `models_cache.json` avec une enveloppe de cache volontairement périmée chaque fois que la
