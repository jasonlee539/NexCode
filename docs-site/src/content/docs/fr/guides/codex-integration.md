...
description: Comment nexcode s'intègre à Codex, synchronise le catalogue de modèles, installe des intercepteurs et restaure proprement la configuration d'origine.
---

nexcode fait passer Codex par le proxy en modifiant deux éléments lus par Codex : sa configuration
...
`nxc init`, `nxc start` et `nxc sync` appellent l'injecteur. Sur la liaison de bouclage par défaut, il conserve
l'identifiant du fournisseur `openai` intégré à Codex et fait pointer ce fournisseur vers nexcode :

```toml
# root keys, before the first table
model_catalog_json = "/absolute/path/to/nexcode-catalog.json"
# Auto-injected by nexcode
...
nexcode, le proxy relaie ces appels au service OpenAI en amont.
...
- **Repli Google Antigravity (CCA) :** lorsqu'aucun candidat au transfert OpenAI ni fournisseur à clé n'est
  ...
  et pas seulement lorsqu'aucun candidat OpenAI n'est configuré. Il nécessite `nxc login google-antigravity` ;
...
clé API, nexcode convertit l'espace de noms privé `image_gen` de Codex en un alias accepté en amont,
`image_gen__<inner-name>` (par exemple `image_gen__imagegen`). Lorsque cet alias exploitable remplace la
déclaration du client, nexcode retire toute déclaration hébergée `image_generation` en double. Il remappe
...
env_key = "NEXCODE_API_AUTH_TOKEN"
# supports_websockets = true   # only when config.websockets is true
```

Lorsque NexCode gère le routage, les deux modes écrivent `$CODEX_HOME/nexcode.config.toml` comme
...
La CLI, la TUI, l'application et le SDK Codex utilisent tous le même répertoire personnel Codex. nexcode le
détermine à partir de `CODEX_HOME`, avec `~/.codex` comme valeur de repli, et gère les fichiers suivants :

```text
$CODEX_HOME/config.toml
$CODEX_HOME/nexcode.config.toml
$CODEX_HOME/nexcode-catalog.json
$CODEX_HOME/models_cache.json
```

Sous WSL, si `CODEX_HOME` n'est pas défini et que `~/.codex/config.toml` n'existe pas côté Linux, nexcode
...
NexCode applique le même ordre de priorité que Codex : la clé racine `sqlite_home` de `config.toml`, puis
...
ou sa clé racine `sqlite_home` est absent, NexCode poursuit avec les valeurs de repli de l'environnement et du
...
`%USERPROFILE%\\.codex`. `nxc status` et `nxc doctor` signalent précisément cette divergence et affichent des
...
Codex soumises à un compte, comme dans Codex natif. nexcode sert également `/v1/responses` par WebSocket. Le
...
d'nexcode. Hors bouclage, le mode fournisseur dédié continue de refléter l'historique sous le fournisseur
`nexcode` tant qu'il est actif, puis restaure les métadonnées sauvegardées lorsqu'il prend fin. Définissez
...
`$CODEX_HOME/nexcode-catalog.json`. Au démarrage et lors de `nxc sync`, nexcode :

1. **Sauvegarde** une fois le catalogue d'origine dans `~/.nexcode/catalog-backup.json`, afin que la mise en
...
qu'nexcode ne route que l'appel de fonction ordinaire du modèle. L'exécution des outils, les autorisations et
les confirmations restent locales à Codex ; nexcode n'implémente pas un second navigateur ni un second
...
nexcode encode cette déclaration et son historique sous forme d'outil de fonction en amont, puis restaure le
...
Après toute modification de ces métadonnées par `nxc sync`, redémarrez l'application Codex et ouvrez une nouvelle
...
```bash
nxc models add deepseek deepseek-v4 --display-name "DeepSeek V4" --context-window 128000
```
...
```bash
dest="${CODEX_HOME:-$HOME/.codex}/nexcode-catalog.json"
tmp="$(mktemp "${dest}.XXXXXX")"
curl -fsS -H "x-nexcode-api-key: $NEXCODE_ADMIN_AUTH_TOKEN" \
  "https://proxy.example.com/api/catalog" > "$tmp" \
  && mv "$tmp" "$dest"
nxc sync-cache
```

La réponse contient le document `nexcode-catalog.json` brut, sans identifiants de fournisseur. Lorsqu'il est
disponible, l'en-tête `x-nexcode-codex-version` indique la version de l'environnement d'exécution Codex du
...
Le nom d'affichage sert **uniquement à l'affichage et reste stable entre les régénérations**. À chaque `nxc sync`
et à chaque actualisation du catalogue, nexcode reconstruit les entrées routées depuis `config.json`, y compris
...
`nxc sync` réussi réapplique le nom configuré. Les véritables noms natifs du service en amont, par exemple
...
Si `config.toml` sélectionne déjà un fournisseur autre que `openai` ou `nexcode`, NexCode ne modifie pas le
...
Confiez la configuration des fournisseurs Codex à un seul outil. Pour placer NexCode derrière un gestionnaire
...
Lorsque l'authentification de l'API du proxy est activée, transmettez aussi l'en-tête `x-nexcode-api-key`
depuis `NEXCODE_API_AUTH_TOKEN`, comme dans la configuration hors bouclage ci-dessus. Pour autoriser NexCode
à injecter directement le routage, rétablissez d'abord le fournisseur `openai` intégré de Codex, supprimez toute
clé racine `openai_base_url` appartenant à l'utilisateur, puis relancez `nxc start`.
...
3. **`liveModels: false` avec `models` vide** — lorsque la découverte en direct est désactivée et que `models`
   est vide ou absent, nexcode n'expose aucun modèle routé pour ce fournisseur.
...
5. **Cache et `nxc sync`** — les catalogues en direct sont mis en cache pendant environ cinq minutes (`modelCacheTtlMs`,
   par défaut `300000`). Exécutez `nxc sync` pour forcer une nouvelle récupération et réécrire le catalogue immédiatement.
6. **Processus Codex `app-server` actif** — réécrire le catalogue sur disque ne suffit pas tant qu'un processus
   ...
   liste en mémoire. `nxc sync` et `nxc sync-cache` émettent un avertissement lorsqu'ils détectent ces processus.
   Redémarrez-les avec `nxc sync --restart-codex`, ou arrêtez vous-même les processus `app-server` concernés,
   puis laissez Codex les recréer afin que la nouvelle liste apparaisse.

:::caution[Autres processus d'écriture locaux]
Les écritures du catalogue (`nexcode-catalog.json`, `config.toml`) sont atomiques **au sein** d'nexcode.
Cette garantie évite les fichiers partiellement écrits lorsque deux processus appartenant à nexcode entrent
en concurrence ; elle n'empêche **pas** un autre processus local, un observateur de fichiers ou un agent de
synchronisation de modifier la visibilité ou l'ordre du catalogue après l'écriture d'nexcode. Codex conserve
un fichier `models_cache.json` distinct et peut l'actualiser indépendamment, ce qui modifie la liste visible sans
réécrire `nexcode-catalog.json`. Si la visibilité des modèles change de façon inattendue pendant l'exécution du
proxy, arrêtez ou reconfigurez les autres processus d'écriture, puis exécutez `nxc sync`. Il s'agit d'un risque
lié à un processus d'écriture externe, et non d'un défaut confirmé d'nexcode.
:::
...
— ou si Claude Code signale un échec de connexion comparable — le proxy nexcode n'est pas actif : aucun
processus n'écoute sur le port configuré, et le client affiche donc lui-même cette erreur de connexion brute.
Redémarrez le proxy :

```bash
nxc start              # foreground
nxc service install    # persistent: auto-starts on login and respawns on crash
```

`nxc status` indique si le proxy est actif et affiche la même suggestion de redémarrage lorsqu'il ne l'est pas ;
`nxc doctor` évalue la sûreté du redémarrage — couverture par le service ou l'intercepteur.
...
Lorsqu'un compte ChatGPT est ajouté au groupe de comptes Codex, nexcode le vérifie avant de l'enregistrer
...
`gpt-5.4-mini`. Si ce modèle renvoie HTTP 400, nexcode réessaie avec `gpt-5.5` ; les détails structurés de
...
nexcode ne vous enferme jamais dans sa configuration. **`nxc stop` est l'unique commande qui restaure
...
exactement comme si nexcode n'avait jamais été installé :

```bash
nxc stop       # stop the proxy + service, restore native Codex
nxc restore    # restore without stopping  (alias: nxc eject)
nxc restore back # point plain Codex at the running proxy again
```

Lorsque nexcode s'exécute comme [service d'arrière-plan géré](/fr/reference/cli/lifecycle/#nxc-service-installrepairstartstopstatusuninstallremove), il définit
`NXC_SERVICE=1` afin qu'un redémarrage déclenché par le service ne modifie **pas** sans cesse la configuration
Codex. Seule l'exécution explicite de `nxc stop` ou `nxc service stop` restaure Codex natif.
