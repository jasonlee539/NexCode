...
description: Découvrez comment nexcode détermine l’ordre des modèles dans le sélecteur Codex et les substitutions de modèle de spawn_agent.
---
...
configuration nexcode. L’ordre final découle des priorités du catalogue ; les modèles routés qui partagent la même priorité
...
nexcode contrôle donc la mise en avant en attribuant des priorités plus faibles, et non en s’appuyant sur la
...
priorité (normalement `9` pour les entrées construites par nexcode), et le groupe routé reste provider/id
alphabétique.
...
qualifiés et les identifiants routés. Utilisez `nxc agent subagents set` ou modifiez la configuration
NexCode pour définir des choix exacts de la forme `<selector>/<native-openai-model>` ; le tableau de bord
...
priorité visible par Codex dans le sélecteur, tandis qu’NexCode conserve la priorité naturelle de chaque
