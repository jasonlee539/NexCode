...
n'importe quel modèle autre qu'OpenAI acheminé par nexcode. Lorsqu'il est activé, un outil synthétique
`video_gen` est injecté dans la conversation. Le modèle l'appelle comme n'importe quel outil de fonction ;
nexcode intercepte l'appel, soumet une tâche de génération vidéo à xAI, interroge son état jusqu'à son
achèvement, puis télécharge le résultat.

## Prérequis

- Une entrée de fournisseur `xai` avec une **clé API** (`nxc login xai` seul ne suffit pas : le pont vidéo exige une authentification par clé, et non OAuth)
- Un modèle autre qu'OpenAI comme fournisseur routé (par exemple Anthropic Claude ou Google Gemini)
- nexcode configuré pour acheminer les requêtes vers ce fournisseur autre qu'OpenAI
...
> ```
>
> Si vous avez configuré xAI avec `nxc login xai` (OAuth), le fournisseur reste en `authMode: "oauth"`
...
1. nexcode détecte qu'un modèle routé autre qu'OpenAI est utilisé avec `videoBridgeEnabled: true`.
2. Un outil de fonction synthétique `video_gen` est injecté dans la conversation.
3. Lorsque le modèle appelle `video_gen`, nexcode soumet une tâche à `/videos/generations` chez xAI.
