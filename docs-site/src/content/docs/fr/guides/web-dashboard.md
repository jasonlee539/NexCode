...
description: L'interface graphique d'nexcode pour l'état du proxy, les fournisseurs, les modèles, les consignes de délégation, les groupes d'authentification, l'utilisation et les journaux.
---

nexcode fournit un tableau de bord web local — une application Vite/React située sous `gui/` — servi par
...
nom d'hôte hors bouclage exige le jeton administrateur (`NEXCODE_ADMIN_AUTH_TOKEN`, ou le fichier généré
automatiquement `~/.nexcode/admin-api-token`).
...
| **Délégation de sous-agent** | Choisissez un modèle natif ou routé et, facultativement, un effort de raisonnement partagés entre les consignes de délégation NexCode et l'option distincte de valeurs par défaut natives. Il ne s'agit pas d'un routeur par création de sous-agent côté proxy ; voir ci-dessous. |
...
| **Démarrage automatique de Codex** | Autorisez un lanceur intermédiaire Codex déjà installé à exécuter `nxc ensure`. Ce commutateur n'installe ni lanceur ni service d'arrière-plan. |
...
`injectionEffort`. L'option **Consignes multi-agents NexCode** contrôle indépendamment les instructions de
...
redémarrage suivant, lorsque NexCode gère le routage Codex actif. Les configurations de fournisseurs externes
...
NexCode demandent à Codex de transmettre les remplacements à `spawn_agent` ; les valeurs par défaut natives
...
- Les changements d'ajout, de suppression et de réglage des sélecteurs de compte sont enregistrés avant l'actualisation du catalogue de modèles. Si cette actualisation limitée dans le temps n'aboutit pas, le tableau de bord affiche un avis orange indiquant la réussite et la procédure de récupération ; exécutez `nxc sync` pour réessayer. Le changement de compte ou de réglage reste enregistré.
...
Le bouton étoile de la barre latérale — et la question unique posée par `nxc start` dans un terminal
interactif — utilise **votre propre connexion `gh`**. nexcode ne détient aucun jeton GitHub et apprend
