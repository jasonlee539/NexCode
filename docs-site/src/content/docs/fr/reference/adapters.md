...
Un **adaptateur** traduit les échanges entre le modèle interne de requête/réponse d’nexcode et le protocole d’un fournisseur. Chaque adaptateur implémente l’interface `ProviderAdapter` (`src/adapters/base.ts`) :

```ts
interface ProviderAdapter {
  name: string;
  buildRequest(parsed: NxcParsedRequest, incoming: IncomingMeta): AdapterRequest | Promise<AdapterRequest>;
