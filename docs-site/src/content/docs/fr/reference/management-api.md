...
description: Authentification, erreurs et référence des points de terminaison du plan de contrôle d'nexcode.
---

L'API de gestion constitue le plan de contrôle d'nexcode. Le tableau de bord accessible à
`http://localhost:10100` en est l'un des clients ; les commandes `nxc` sans interface graphique qui gèrent les fournisseurs, les modèles, les combinaisons, les comptes,
...
nexcode le détermine dans l'ordre suivant :

1. `NEXCODE_ADMIN_AUTH_TOKEN`, lorsqu'il est défini.
2. Un jeton `nxc_admin_*` généré dans un fichier secret renforcé.
...
:::caution
Le jeton d'administrateur doit différer de tout identifiant du plan de données. Au démarrage, nexcode rejette un
...
Sur une liaison de bouclage, l'amorçage du tableau de bord peut recevoir un identifiant `nxc_session_*` de courte durée.
...
| --- | --- | --- |
| 401 | `nexcode admin token required` | Le jeton d'administrateur ou la session d'interface graphique est absent, invalide, expiré, associé à une autre origine, ou la preuve CSRF est manquante |
...
Pour les hôtes sans interface graphique et l'automatisation, utilisez les commandes `nxc` correspondantes : elles appellent cette
