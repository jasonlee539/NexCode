...
nexcode présente un proxy local dans plusieurs dialectes clients. Un client Codex peut parler le
...
Il s'agit de la forme native du plan de données nexcode. Le corps de la requête doit être un objet JSON avec un
...
`POST /v1/realtime/calls` accepte la surface de création d'appel OpenAI Realtime. nexcode sélectionne un
...
| Autre modèle routé | Exécute un tour de compactage interne, sans flux et sans outils avec un `compaction_trigger` ; nécessite exactement un élément `compaction` synthétique dont `encrypted_content` est une enveloppe `nxc1:` ; décode ce résumé dans l'historique de remplacement de la v1 |
...
| 502 | `invalid_response_error` | Le tour synthétique n’a pas produit exactement un élément de compactage `nxc1:` valide et non vide |
...
utilisez la matrice ci-dessous. « Dédié » signifie `X-NexCode-API-Key` ; les autres colonnes signifient
...
octet par octet : nexcode ne le déchiffre pas, ne traduit pas son contenu et ne le rechiffre pas pour un autre
...
mais la cible routé sélectionnée ne peut pas lire le texte chiffré natif ChatGPT, nexcode échoue avec
