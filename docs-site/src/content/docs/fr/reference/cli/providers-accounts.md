...
sous `~/.nexcode/` des informations d’identification actualisées automatiquement. Les fournisseurs à clé API
...
Utilisez la même commande pour **réauthentifier** après `nxc status` / `nxc doctor` rapports
réauthentification requise ou échec de l'actualisation du terminal (ou utilisez Réauthentifier dans le tableau de bord).
Les comptes du groupe Codex ne constituent pas un fournisseur public pour `nxc login` : réauthentifiez-vous
plutôt depuis le groupe de comptes Codex du tableau de bord (**Réauthentifier**) ou avec le flux non
interactif `nxc account reauth`.
...
⚠️  A proxy is running but could not reload this provider (unattested-target).
   ...
   Restart it to pick this up: nxc restart
```
...
la commutation basée sur l'utilisation est désactivée. NexCode rejoue la conversation après un changement de compte, mais le
...
Sur un **429**, nexcode respecte `Retry-After`, démarre la temporisation du compte, efface l'affinité et peut
...
Pour le groupe de comptes Codex, utilisez `nxc account refresh openai [--json]`. Cette commande force l'actualisation des quotas de compte et
...
n’importe quel autre compte du groupe : `nxc account priority openai main last` permet de la conserver comme réserve.
...
`nxc account --help` pour la forme de commande spécifique au fournisseur. Si une connexion au compte Codex est enregistrée mais
...
`nxc sync` conseils de récupération sur stderr. `--json` garde la sortie standard analysable et transporte
...
déjà enregistré; la sortie humaine imprime des conseils de récupération génériques `nxc sync` sur stderr et quitte toujours 0.
...
Gérez les profils de connexion principale natifs nommés Codex sans modifier le routage du groupe de comptes NexCode :
...
La matrice de récupération v1 couvre un processus NexCode sortant après qu'un fichier de transaction ait été
...
dans le répertoire canonique `<real CODEX_HOME>/.nexcode-native-main-profiles`. Toutes les instances NexCode
qui partagent ce répertoire Codex observent ainsi un propriétaire et un état de récupération uniques. La préparation
des identifiants en clair reste isolée sous chaque répertoire `<NEXCODE_HOME>/native-main-profile-staging`.
...
`auth.json.nxc.<pid>.<sequence>.tmp`. Chaque candidat doit rester un fichier ordinaire à lien unique sous le
...
plantages coopératifs d’NexCode, pas un processus malveillant déjà exécuté par le même utilisateur du système.
...
Aperçu des builds utilisés `<NEXCODE_HOME>/native-main-profiles`. Cette mise en page n'est jamais importée silencieusement.
Si `doctor` signale l’état du profil hérité, arrêtez tous les proxy NexCode partageant le même `CODEX_HOME`.
...
autorisations, ou supprimez l'ancien jeu d'aperçus et exécutez à nouveau `nxc account main register`. Ne choisissez pas
...
`nxc model` est un alias de `nxc models`. Sans sous-commande, répertoriez les modèles implantés statiquement dans
...
proxy en cours d'exécution (`nxc start` ou un service installé).
