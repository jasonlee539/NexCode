...
Ces commandes contrôlent la politique et le routage de l'agent, inspectent le proxy actif et connectent les clients pris en charge à nexcode.
...
La sous-commande `mode` écrit `multiAgentMode` dans la configuration nexcode et resynchronise le catalogue Codex.
...
(`nxc v2 mode v2`); `nxc v2 on` à lui seul ne satisfait pas à cette porte du tableau de bord.

## Routage combiné

### `nxc combo <list|show|set|remove> ...` · `nxc route combo ...`

Gérez les modèles virtuels de basculement combiné et de round robin. `nxc route combo` est l'alias hiérarchique ;
...
Sans portée, `nxc debug` imprime l'utilisation et, lorsque le proxy est arrêté, l'environnement de prochain démarrage
valeurs par défaut. Les valeurs par défaut du débogage du fournisseur sont `NXC_DEBUG=1` (l'héritage `NXC_DEBUG_FRAMES=1` fonctionne également) ; utilisation
les valeurs par défaut du débogage sont `NEXCODE_USAGE_DEBUG=1`.

## Accès à l’API

### `nxc access <key|endpoints|models|test> ...`

Gérez les clés d'admission NexCode et examinez les points de terminaison et les modèles externes. `nxc api-key
<list|create|remove> ...` est un alias de `nxc access key`.
...
restent soutenus. Utilisez `nxc claude config <status|set> ...` pour les réglages Claude Code.

### `nxc opencode [opencode args...]`

Vérifiez que le proxy est actif, puis lancez opencode avec un bloc `provider.nexcode` généré dans la couche d’exécution intégrée d’OpenCode (`OPENCODE_CONFIG_CONTENT`). La configuration intégrée existante est préservée et seul `provider.nexcode` est remplacé pour ce lancement. Les fichiers `opencode.json` globaux ou propres au projet peuvent être lus afin de signaler une substitution existante, mais les fichiers sur disque ne sont jamais modifiés. Les modèles routés apparaissent sous la forme `nexcode/<provider>/<model>`. Un lancement ultérieur de `opencode` sans intermédiaire se comporte exactement comme auparavant.
...
bloc fournisseur `nexcode` — URL de base, liste de modèles et référence d’identifiant du client
...
| --- | --- | --- | --- |
| `opencode` | `~/.config/opencode/opencode.json` (`XDG_CONFIG_HOME` gagne une fois défini) | `opencode.json` | `NEXCODE_OPENCODE_API_KEY` |
| `pi` | `~/.pi/agent/models.json` (`PI_CODING_AGENT_DIR` l'emporte une fois défini ; une valeur relative est refusée) | `pi-models.json` | none — le bloc porte le littéral `nexcode-loopback` |
...
`llm-pi-ai.providers.nexcode`. DSH recharge à chaud ce fournisseur ; le modèle par défaut de l'utilisateur et
...
opencode interpole `{env:NEXCODE_OPENCODE_API_KEY}`. Les exports Pi et OMP générés font
ne nécessitent pas de variable d’environnement : chacune porte l’espace réservé littéral `nexcode-loopback`.
...
l'intégration reste délibérément un bouclage uniquement ; le câblage à distance `x-nexcode-api-key` est différé.

:::caution[Fusionner, ne jamais remplacer]
`nxc export` n'écrit jamais votre vraie configuration client. La destination est imprimée pour que vous puissiez la fusionner
...
Gajae est l'exception : `NEXCODE_GAJAE_API_KEY` remplit ses informations d'identification de fournisseur à partir du
...
Inspectez et modifiez en toute sécurité la configuration NexCode validée. `show` et `get` masquent les secrets. Importer
