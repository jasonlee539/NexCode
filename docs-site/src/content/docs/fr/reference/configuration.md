...
description: Où nexcode stocke la configuration, comment les modifications sont appliquées et les liens vers chaque domaine de configuration.
---

nexcode stocke sa configuration persistante dans `$NEXCODE_HOME/config.json`, normalement
`~/.nexcode/config.json`. Sous Windows, la valeur par défaut est
...
- **Interface en ligne de commande :** `nxc init` crée le fichier initial, tandis que des commandes telles que `nxc provider`, `nxc models`,
  `nxc combo`, `nxc agent` et `nxc config` mettent à jour ou consultent les paramètres qui leur sont propres.
...
Si le fichier ne peut pas être analysé, nexcode le sauvegarde en tant que
...
documentées dans les pages de chaque domaine. `NEXCODE_HOME` est prioritaire sur le répertoire de configuration
...
nexcode écrit les fichiers gérés `config.toml` et `nexcode-catalog.json` dans un fichier temporaire,
puis le renomme (`atomicWriteFile`).
Cela évite les fichiers partiels lorsque plusieurs processus d'écriture, comme `nxc stop` et le gestionnaire d'arrêt du proxy,
