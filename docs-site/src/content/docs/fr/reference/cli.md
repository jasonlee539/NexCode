...
description: Répartition des commandes, codes de sortie et liens vers toutes les familles de commandes nxc.
---

L’interface en ligne de commande d’nexcode est `nxc`. La première commande détermine l’opération à exécuter. Les alias documentés, tels que `setup`/`init`, `restore`/`eject` et `models`/`model`, déclenchent la même opération. Une commande inconnue ou une syntaxe de commande non valide produit une erreur.

Exécutez `nxc help` (ou `nxc --help` / `nxc -h`) pour afficher l’aide générale. Pour une commande enregistrée dans la table d’aide, exécutez `nxc help <command>`, `nxc <command> --help` ou `nxc <command> -h`. Les commandes d’aide et de version sont en lecture seule : elles ne démarrent, n’arrêtent, n’installent, ne désinstallent ni ne réécrivent l’état de Codex ou d’nexcode.
...
`nxc --version`, `nxc -v` et `nxc version` affichent une seule ligne de version exploitable par un script, puis se terminent.
