...
Un fournisseur indique à nexcode où se trouve un modèle, quel adaptateur de protocole il utilise et comment les requêtes sont
authentifiées.
...
| --- | --- | --- | --- |
| `providers` | `Record<string, NxcProviderConfig>` | — | Mappage du nom du fournisseur avec la configuration du fournisseur. |
...
| `tokenGuardian?` | `NxcTokenGuardianConfig` | désactivé | Politique facultative d'actualisation proactive OAuth et de préchauffage des comptes Codex. |

Les noms des sélecteurs sont des étiquettes publiques choisies par l'utilisateur ; nexcode ne leur attribue aucune sémantique de rôle de compte.
...
configuration v1 distribuée, nexcode crée `config.json.pre-openai-tiers-v2.bak` sans remplacer une
...
## Entrées de fournisseur (`NxcProviderConfig`)
...
| `preserveResponsesReasoningContent?` | `boolean` | Conserve le contenu de raisonnement en texte brut dans les éléments de raisonnement Responses relus, au lieu de l'effacer comme l'exige le moteur ChatGPT. Activez cette option pour les services en amont dont le contrat accepte la relecture du raisonnement, comme DeepSeek. Les enveloppes `nxcr1` créées par le proxy sont toujours supprimées. |
...
magasin d'identifiants alimenté par `nxc login` ; le comportement de lancement de Claude Code avec abonnement est
...
proxy sortant, nexcode résout le nom d'hôte une seule fois et se connecte exclusivement à l'adresse ainsi validée.
...
nexcode ne peut donc ni épingler ni vérifier cet homologue. Il s'agit d'une limitation de sécurité explicite.
...
Lors d'un **429**, nexcode respecte `Retry-After`, place le compte en temporisation, efface l'affinité et peut
...
préférez le changement manuel avec `nxc account use anthropic <id>`.
:::
...
Lorsque le routage ignore `baseUrl`, nexcode consigne le point de terminaison du registre et uniquement l'origine configurée ;
...
Le pont Cursor est expérimental. Après `nxc login cursor`, ajoutez ou modifiez `providers.cursor`.
...
Les clés de modèle sont les identifiants OpenRouter natifs exacts, sans le préfixe externe du fournisseur nexcode. La sélection de
