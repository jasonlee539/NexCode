...
| `hostname?` | `string` | `"127.0.0.1"` | Adresse de liaison. Les liaisons hors bouclage nécessitent `NEXCODE_API_AUTH_TOKEN`. |
...
| `emptyCompletionRetry?` | `boolean` | `false` | Active une nouvelle tentative Responses identique lorsqu’une réponse ne contient ni texte ni appel d’outil. Cette tentative peut être facturée. `NXC_EMPTY_COMPLETION_RETRY=0` la désactive sans modifier la configuration ; les combinaisons et les tours de compactage routés restent exclus. |
...
| `apiKeys?` | `NxcApiKey[]` | `[]` | Identifiants `nxc_…` générés, acceptés par l'API de gestion et l'authentification du plan de données sur les liaisons hors bouclage. Gérés depuis le tableau de bord. |
...
| `codexAutoStart?` | `boolean` | `true` | Autorise le lanceur intermédiaire Codex à exécuter `nxc ensure` avant de démarrer Codex. Avec la valeur false, cette vérification ne fait rien. |
| `codexShimAutoRestore?` | `boolean` | `true` | Restaure le lanceur intermédiaire installé après son remplacement par une mise à jour externe de Codex terminée. Désactivation par variable d'environnement : `NEXCODE_CODEX_SHIM_AUTO_RESTORE=0`. |
| `syncResumeHistory?` | `boolean` | `true` | Compatibilité historique Codex App réversible. Les métadonnées originales sont sauvegardées et restaurées par `nxc stop` / `nxc restore`. |
...
| `webSearchSidecar?` | `NxcWebSearchSidecarConfig` | activé lorsqu'il est utilisable | Options du service auxiliaire de recherche Web. |
| `visionSidecar?` | `NxcVisionSidecarConfig` | activé lorsqu'il est utilisable | Options du service auxiliaire de description d'images. |
| `images?` | `NxcImagesConfig` | sélection automatique OpenAI | Options de relais d'images autonomes pour Codex `image_gen`. |
...
`nxc recover-history --legacy-openai` pour forcer la récupération du fournisseur natif.

## Accès à distance

La liaison par défaut à `127.0.0.1` est limitée au bouclage. Une adresse hors bouclage telle que `0.0.0.0`
exige un identifiant pour le plan de données : `NEXCODE_API_AUTH_TOKEN` ou au moins une entrée `apiKeys`
...
spécifiquement `NEXCODE_API_AUTH_TOKEN` ; exportez-le avant `nxc service install` afin que launchd, systemd
...
| Point de terminaison | `Authorization: Bearer` | `x-nexcode-api-key` | `x-api-key` |
| --- | --- | --- | --- |
...
`x-nexcode-api-key`, tandis que `Authorization` et `x-api-key` sont réservés aux identifiants Anthropic.
...
donc il n'hérite jamais de `NEXCODE_API_AUTH_TOKEN` et chaque appel de modèle échoue avec `401` avant un
le flux s’ouvre.
...
`nxc sync` écrit ensuite `base_url = "http://127.0.0.1:10200/v1"` dans le bloc du fournisseur Codex géré
...
`nxc` écrit uniquement l'adresse locale `127.0.0.1` par défaut dans la configuration du client géré.
...
Si un port de rappel enregistré est déjà utilisé et que la surface de connexion propose une saisie manuelle, NexCode
conserve l'URI de redirection enregistrée et renvoie tout de même l'URL d'autorisation du fournisseur. Terminez la
connexion au fournisseur, puis collez dans NexCode l'URL de redirection finale affichée dans la barre d'adresse du navigateur ou le
...
Ces paramètres régissent `/v1/messages`, `/v1/messages/count_tokens`, le lanceur `nxc claude` et la page du tableau de bord Claude.
...
| `claudeCode.subagentEffort?` | `"low" \| "medium" \| "high" \| "xhigh" \| "max"` | hériter | Effort écrit pour générer `~/.claude/agents/nxc-*.md` ; distinct des plafonds d’orientation et de proxy Codex. Redémarrez par `nxc claude` pour régénérer. |
...
Les services auxiliaires Anthropic OAuth réutilisent l'empreinte OAuth Claude Code existante d'nexcode. Effectuez un test d'endurance avec le
