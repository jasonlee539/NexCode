...
**combo** 是一个虚拟模型，它前置了一组按顺序排列的真实 provider/model 目标。你的客户端请求 `combo/<id>`；nexcode 会选择一个目标，将请求重写为那个具体的 `provider/model`，并且在第一个目标出现可重试失败时，可以改试另一个目标。
...
默认策略是故障切换，所以正常请求会发往 `anthropic/claude-opus-4-8`。如果这次尝试出现可重试失败，nexcode 可以切换到 `openai/gpt-5.6-sol`。
...
`nxc combo set <id>` 中的 combo id 必须以字母或数字开头。之后可以包含字母、数字、`.`、`_` 或 `-`，总长度最多 64 个字符。其规范模型 id 始终是 `combo/<id>`；例如，id `main` 会变成 `combo/main`。
...
该模式默认关闭，同时要求 `--native-alias` 和非空显示名称。alias 必须是当前 nexcode 版本
...
被跳过的目标默认会进入 60 秒冷却。如果上游响应包含有效的 `Retry-After` 值，nexcode 会改用该值。数字秒数和 HTTP-date 值都可以接受，而且每次冷却最多只会封顶到 10 分钟。
...
如果请求没有 `reasoning` 对象，nexcode 会创建一个。如果 `reasoning` 存在但没有 `effort` 属性，它会保留其他字段并添加默认值。调用方提供的 effort 永远不会被覆盖。

当目标能力未知，或者不包含配置的 effort 时，nexcode 会省略默认值，并保持目标自身行为不变。支持的值是 `low`、`medium`、`high`、`xhigh`、`max` 和 `ultra`；省略该字段或将其设为 `null`，就会把 effort 完全交给调用方和目标。
...
对于 Codex v2 子代理，有一个重要限制（[issue #92](https://github.com/lidge-jun/nexcode/issues/92)）。原生父进程只能把新启动 worker 的任务，以为原生 ChatGPT 后端生成的密文形式发送出去。外部 provider 无法读取那段负载。

对于这类请求，combo 会把合格目标筛选为规范的原生 ChatGPT 路由，即使在一次可重试失败之后也是如此。如果 combo 没有任何具备解密能力的目标，nexcode 会在分发前停止，并返回 HTTP 400：
...
combo id 不存在。响应是 HTTP 404，类型为 `invalid_request_error`。运行 `nxc combo list`，检查拼写和大小写，并确认你的管理命令写入的是同一个正在运行、并接收模型请求的 nexcode 实例。
