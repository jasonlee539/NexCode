---
title: 模型排序
description: nexcode 如何确定 Codex 模型选择器和 spawn_agent 模型 override 的顺序。
---

Codex 模型选择器不会保留 nexcode 配置中 provider 的声明顺序或模型数组顺序。最终顺序由目录
...
因此，nexcode 通过分配更低的 priority 控制置顶位置，而不依赖数组位置。本表中的固定值及下例适用于
...
（nexcode 创建的条目通常为 `9`），路由组内部仍按 provider/id 字母排序。
...
裸原生和路由 id 的顺序。配置和 `nxc agent subagents set` 也接受精确的账户限定
...
目前 `NxcConfig` 中没有通用的 `modelOrder`、`providerOrder` 或 priority map 设置。受支持的排序
