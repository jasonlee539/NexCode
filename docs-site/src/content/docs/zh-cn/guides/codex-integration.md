---
title: Codex 集成
description: nexcode 如何将自身注入 Codex、同步模型目录、安装 shim，并干净地恢复。
---

nexcode 通过修改 Codex 读取的两样东西，让 Codex 经由 proxy 路由：它的 config
...
`nxc init`、`nxc start` 和 `nxc sync` 都会调用注入器。在默认的 loopback 绑定下，它会保留
Codex 内置的 `openai` provider id，并将该 provider 指向 nexcode：
...
ChatGPT bearer auth。由于注入的 `base_url` 指向 nexcode，proxy 会把这些调用中继到 OpenAI
上游。
...
- **Google Antigravity（CCA）fallback：** 当既没有 OpenAI forward 候选，也没有已配置的 keyed
  ...
  **Cloud Code Assist** endpoint，并使用 `gemini-3.1-flash-image` 模型。该 fallback 也会在
  ...
  这需要 `nxc login google-antigravity`；OAuth token 只会发送到固定的 CCA registry host，
...
工具声明仍会随模型的 Responses 请求一同发送。对于 API-key Responses provider，nexcode 会把
Codex 私有的 `image_gen` namespace 降级为上游安全的 `image_gen__<inner-name>` alias（例如
`image_gen__imagegen`）。当这个可用 alias 取代客户端声明时，nexcode 会移除重复的 hosted
...
当 NexCode 负责路由时，两种模式都会把 `$CODEX_HOME/nexcode.config.toml` 写成参考/回退配置。
...
Codex CLI、TUI、App 和 SDK 都读取同一个 Codex home。nexcode 会从 `CODEX_HOME` 解析该目录，
回退到 `~/.codex`，并管理：
...
在 WSL 中，如果未设置 `CODEX_HOME`，且 Linux 侧的 `~/.codex/config.toml` 不存在，nexcode 还会检查
...
而 ChatGPT/Codex app 仍然读取 `%USERPROFILE%\\.codex`。`nxc status` 和 `nxc doctor` 会提示这个确切的不一致，
...
nexcode 也会通过 WebSocket 提供 `/v1/responses`。专用 provider 只有在 `"websockets": true` 时才会声明
...
重映射。首次 sync 时，它还会把旧版 nexcode 标记过的线程迁回 `openai`。非 loopback 的专用 provider 模式
在运行期间仍会把历史记录镜像到 `nexcode` provider 名下，并在退出时恢复已备份的 metadata。
如需保持历史记录完全不变，请设置 `syncResumeHistory: false`。

## 模型目录同步

Codex 显示的模型来自一个磁盘上的 catalog（默认是 `$CODEX_HOME/nexcode-catalog.json`）。在启动时以及执行
`nxc sync` 时，nexcode 会：

1. **备份**一次干净的 catalog 到 `~/.nexcode/catalog-backup.json`（这样 featuring 是可逆的）。
...
嵌套的 MCP 工具，包括 Browser 和 Computer Use；nexcode 只负责路由模型发起的普通 function call。
工具执行、权限和确认仍由 Codex 在本地处理；nexcode 不会实现另一套浏览器或桌面控制 executor。

对于不接受 Codex `exec` custom-tool grammar 的 key-auth Responses provider，nexcode 会把该工具声明及其
...
`nxc sync` 修改这些 metadata 后，请重启 Codex App 并打开一个新任务。现有 app-server process 和任务可能仍会
...
响应就是原始的 `nexcode-catalog.json` 文档（不含 provider 凭据）。当可用时，
`x-nexcode-codex-version` header 会报告服务器上的 Codex runtime 版本，方便客户端发现版本偏移。
...
display name 是 **仅用于显示且在重新生成时保持稳定的**。每一次 `nxc sync` 和 catalog refresh 都会从
...
之前持久化下来的 catalog 会保留，而下一次成功的 `nxc sync` 会重新应用已配置的名称。真实的上游原生名称
（例如 `gpt-5.6-sol` → "GPT-5.6-Sol"）来自固定的上游快照，绝不会被自定义 display name 覆盖。

### 外部 provider 管理器

如果 `config.toml` 已经选择了 `openai` 或 `nexcode` 之外的 provider，NexCode 会保持文件不变，
...
只保留一个工具作为 Codex provider 配置的 owner。若要在现有 provider 管理器之后使用 NexCode，
...
provider 形式一样，从 `NEXCODE_API_AUTH_TOKEN` 传入 `x-nexcode-api-key`。如果要让 NexCode 直接注入路由，
请先把 Codex 切回其内置的 `openai` provider，并移除任何用户拥有的 root `openai_base_url`，然后重新运行 `nxc start`。
...
2. **`disabledModels`**（顶层） - 会同时隐藏 catalog 和 `/v1/models` 中的模型，并把裸原生 GPT slug
   切成 `visibility: "hide"`。
3. **`liveModels: false` 且 `models` 为空** - 当 live discovery 关闭而 `models` 为空或省略时，nexcode
...
5. **缓存和 `nxc sync`** - live catalog 的缓存时间大约是五分钟（`modelCacheTtlMs`，默认 `300000`）。
   运行 `nxc sync` 可以强制立即重新抓取并重写 catalog。
6. **正在运行的 Codex `app-server`** - 当长生命周期的 Codex `app-server`（Desktop / CLI 后台宿主）还在
   内存中保留旧列表时，只重写磁盘上的 catalog 还不够。`nxc sync` 和 `nxc sync-cache` 会在检测到这些进程时给出
   警告。请用 `nxc sync --restart-codex` 重新启动它们（或者你自己停掉匹配的 `app-server` 进程），然后让 Codex
   重新创建它们，这样新列表才会出现。

:::caution[其他本地写入者]
在 nexcode 内部，catalog 写入（`nexcode-catalog.json`、`config.toml`）是原子的，这只能防止两个
nexcode 所有的 writer 竞争时写出半截文件。它**不能**阻止其他本地进程、文件 watcher 或同步 agent 在
nexcode 写入之后再次改写 catalog 的可见性或顺序。Codex 还有自己独立的 `models_cache.json`，并且可以独立刷新它，
从而在不重写 `nexcode-catalog.json` 的情况下改变可见列表。如果 proxy 正在运行时模型突然翻转，请停掉或重新配置
竞争中的写入者，然后运行 `nxc sync` —— 这是外部写入者风险，不是已确认的 nexcode 缺陷。
:::
...
的错误——或者 Claude Code 报告类似的连接失败——说明 nexcode proxy 没有在运行：
...
`nxc status` 会显示 proxy 是否在运行，并在未运行时打印相同的重启提示；`nxc doctor` 会报告
...
当把一个 ChatGPT 账号加入 Codex 账号池时，nexcode 会在持久化前向 Codex Responses backend
...
nexcode 绝不会把你困住。**`nxc stop` 是完全恢复原生 Codex 的单一命令** —— 它会停止 proxy、
停止后台服务（如已安装），并剥除所有注入的行和路由的目录条目，使普通的 `codex` 完全像 nexcode
从未存在过一样工作：
...
当 nexcode 作为受管的 [background service](/reference/cli/#nxc-service) 运行时，它会设置
`NXC_SERVICE=1`，这样由服务驱动的重启**不会**反复改写 Codex config——只有显式的
`nxc stop` / `nxc service stop` 才会恢复原生 Codex。
