...
Video Bridge 允许你通过 nexcode 路由的任意非 OpenAI 模型使用 xAI 的 Grok Imagine Video 生成功能。启用后，系统会在对话中注入一个合成的 `video_gen` 工具。模型会像调用普通函数工具一样调用它；nexcode 拦截该调用，向 xAI 提交视频生成任务，轮询直到完成，然后下载结果。

## 前提条件

- 一个带有 **API key** 的 `xai` provider 条目（仅执行 `nxc login xai` 不够，视频桥接需要 key 认证，而不是 OAuth）
- 作为路由目标的非 OpenAI 模型（例如 Anthropic Claude、Google Gemini）
- 已配置 nexcode 通过该非 OpenAI provider 路由
...
> ```
>
> 如果你是通过 `nxc login xai` 接入的（OAuth），provider 会保持在 `authMode: "oauth"`，桥接就会静默不启用。请在环境中设置 `XAI_API_KEY`，或者像上面那样直接硬编码密钥。
...
1. nexcode 检测到一个已路由的非 OpenAI 模型，并且 `videoBridgeEnabled: true`
2. 系统会在对话中注入一个合成的 `video_gen` 函数工具
3. 当模型调用 `video_gen` 时，nexcode 会向 xAI 的 `/videos/generations` 提交任务
