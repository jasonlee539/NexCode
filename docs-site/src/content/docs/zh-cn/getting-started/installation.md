---
title: 安装
description: 安装 nexcode(nxc)代理及其前置条件,并验证它能够运行。
---

安装 nexcode 后会得到 `nxc` 和 `nexcode` 两个等价命令，它们都指向同一个基于 Bun 的
...
| 要求 | 原因 |
| --- | --- |
| **[Node](https://nodejs.org) ≥ 18** | `nxc` 运行在 Bun 运行时上，但运行时会在 `npm install` 时自动打包，你**无需**自己安装 Bun。 |
| **[OpenAI Codex](https://openai.com/codex)**(CLI、App 或 SDK) | nexcode 所代理的客户端。nexcode 会写入 `$CODEX_HOME/config.toml`（默认 `~/.codex/config.toml`）。 |
...
nexcode 状态文件位于 `$NEXCODE_HOME`（默认 `~/.nexcode`），Codex 集成文件位于
`$CODEX_HOME`（默认 `~/.codex`）。

| 路径 | 用途 |
| --- | --- |
| `$NEXCODE_HOME/config.json` | 你的 provider、默认 provider、端口及选项。 |
| `$NEXCODE_HOME/nxc.pid` | 正在运行的代理的 PID（单实例保护）。 |
| `$NEXCODE_HOME/runtime-port.json` | 当前 PID、主机名和端口，包括自动选择的备用端口。 |
| `$NEXCODE_HOME/auth.json` | 执行 `nxc login` 后保存的 OAuth 凭据。 |
| `$NEXCODE_HOME/catalog-backup*.json` | nexcode 修改 Codex 模型目录前创建的备份。 |
| `$CODEX_HOME/config.toml` | 仅监听回环地址时，nexcode 会添加由自身标记管理的根级 `openai_base_url`；监听非回环地址时，则使用 `model_provider = "nexcode"` 和 `[model_providers.nexcode]`，以便 Codex 发送 API 认证 header。 |
| `$CODEX_HOME/nexcode.config.toml` | 与 Codex 主配置一同写入的备用/参考 profile。 |
| `$CODEX_HOME/nexcode-catalog.json` | 供 Codex 使用的原生与已路由模型目录。 |

:::note
nexcode 绝不会删除你的 Codex 配置。每次注入都是可逆的 —— `nxc stop`、`nxc restore`
或 `nxc eject` 会精确剥离 nexcode 所添加的那些行,并恢复原生 Codex。
:::
