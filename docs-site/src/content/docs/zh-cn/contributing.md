---
title: 贡献指南
description: nexcode 的开发环境、结构、约定，以及添加 provider 或 adapter 的方法。
---
...
公开文档发布到 GitHub Pages：<https://nexcode.me/zh-cn/>。
...
- **Cross-platform CI**（`.github/workflows/ci.yml`）会在改动 runtime、test、package、script、
  ...
  build 和 `nxc help`。另一个三系统 lane 使用 package 内置 runtime，验证无需单独安装 Bun 也能
...
`src/providers/derive.ts` 会把该条目提供给 `nxc init`、`nxc provider`、仪表盘 preset、API-key
...
nexcode 倾向于小而可验证的 commit，而不是大批量改动。
