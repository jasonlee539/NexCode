...
| `hostname?` | `string` | `"127.0.0.1"` | 绑定地址。非回环绑定需要 `NEXCODE_API_AUTH_TOKEN`。 |
| `proxy?` | `string` | — | 出站 HTTP(S) 代理 URL，或 `${ENV_VAR}`。仅当 `HTTP_PROXY` / `HTTPS_PROXY` 未设置时才会应用；回环地址始终保留在 `NO_PROXY` 中。 |
| `emptyCompletionRetry?` | `boolean` | `false` | 显式启用：当 Responses 完成时既无文本也无工具调用，使用相同请求重试一次。重试可能产生费用。`NXC_EMPTY_COMPLETION_RETRY=0` 可在不修改配置的情况下禁用；combo 与 routed-compaction turn 不参与。 |
...
| `apiKeys?` | `NxcApiKey[]` | `[]` | 管理平面和非回环绑定上的数据平面身份验证可接受的已生成 `nxc_…` 凭据。由仪表板管理。 |
...
| `codexAutoStart?` | `boolean` | `true` | 允许 Codex shim 在启动 Codex 之前运行 `nxc ensure`。设为 false 会让 ensure 变成无操作。 |
| `codexShimAutoRestore?` | `boolean` | `true` | 在完成外部 Codex 更新并覆盖安装的 shim 之后恢复该 shim。环境退出开关：`NEXCODE_CODEX_SHIM_AUTO_RESTORE=0`。 |
| `syncResumeHistory?` | `boolean` | `true` | 可逆的 Codex App 历史兼容性。原始元数据会被备份，并由 `nxc stop` / `nxc restore` 恢复。 |
...
| `webSearchSidecar?` | `NxcWebSearchSidecarConfig` | 在可用时启用 | Web 搜索侧车选项。 |
| `visionSidecar?` | `NxcVisionSidecarConfig` | 在可用时启用 | 图像描述侧车选项。 |
| `images?` | `NxcImagesConfig` | 自动选择 OpenAI | 用于 Codex `image_gen` 的独立 Images 转发选项。 |

如果较旧的开发版本在尚未提供备份支持之前修改过了 resume-history 元数据，请运行
`nxc recover-history --legacy-openai` 强制使用原生提供方恢复。
...
`nxc service install` 之前导出它，这样 launchd、systemd 或 Task Scheduler 都能接收到。客户端应发送：

```text
x-nexcode-api-key: your-secret-token
```

| 端点 | `Authorization: Bearer` | `x-nexcode-api-key` | `x-api-key` |
| --- | --- | --- | --- |
...
`x-nexcode-api-key` 接受代理准入，并把 `Authorization` 和 `x-api-key` 保留给 Anthropic
...
`nxc` 只会把默认的本地 `127.0.0.1` 地址写入已管理的客户端配置。
...
这些设置控制 `/v1/messages`、`/v1/messages/count_tokens`、`nxc claude` 启动器，以及 Claude 仪表板页面。
...
| `claudeCode.subagentEffort?` | `"low" \| "medium" \| "high" \| "xhigh" \| "max"` | inherit | 写入生成的 `~/.claude/agents/nxc-*.md` 的努力级别；与 Codex 指引和代理上限相互独立。需通过 `nxc claude` 重新启动以重新生成。 |
...
Anthropic OAuth 侧车会复用 nexcode 现有的 Claude Code OAuth 指纹。请对目标账户和负载进行 soak 测试。
