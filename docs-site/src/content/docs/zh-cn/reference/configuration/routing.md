...
| `combos?` | `Record<string, NxcComboConfig>` | `{}` | 由有序的提供方/模型目标构建出来的虚拟 `combo/<id>` 模型。 |

## 模型解析顺序

nexcode 按以下顺序解析请求的模型：
...
`nxc sync` 警告只表示 picker 目录仍需收敛，并不表示路由变更丢失。
...
即使某个 combo 不能被列出，它仍然可以直接路由。只有当所有目标都暴露出可以交集的能力时，`nxc sync`、`/v1/models` 和 Codex 选择器才会列出它：
