---
title: 架构
description: nexcode 内部机制 —— 模块图、请求解析器、AdapterEvent 桥接与缓存。
---

nexcode 运行在单个 Bun 进程中。请求以 OpenAI Responses 格式进入，规范化为内部模型后完成
...
- **消息（Messages）** —— `input` 条目会变成规范化的 `NxcMessage[]`：user / developer /
...
（`: nexcode heartbeat`）来重新启动 Codex 的空闲计时器。注释行会被每个
...
`/api/*` 和 `/v1/*` 都提供 `NEXCODE_API_AUTH_TOKEN`；配置的 `corsAllowOrigins` 会扩展本地
origin allowlist。
...
thread affinity 位于 `codex/` 下，不会出现在管理 API 响应中。请求用量会规范化为 `NxcUsage`，
...
尝试 Responses WebSocket upgrade 时，nexcode 会返回 `426 upgrade_required`，Codex 随后在该
...
内部模型位于 `types.ts`：`NxcParsedRequest`、`NxcContext`、`NxcMessage` 联合类型、
`NxcContentPart`（text / image）、`NxcToolCall`、`NxcTool`、`AdapterEvent`，以及配置类型
（`NxcConfig`、`NxcProviderConfig`）。两个常用 helper 是 `namespacedToolName()` 和
