...
**adapter** 负责在 nexcode 的内部请求/响应模型与某个 provider 的 wire 格式之间转换。每个
...
`buildRequest` 把 `NxcParsedRequest` 转成上游 HTTP 请求；`parseStream` / `parseResponse` 把 provider
...
启用工具时，nexcode 会添加私有 `codex_kiro_final_answer`。重试不会制造空的 assistant/user 回合，
