---
title: 配置参考
description: nexcode 配置的存放位置、如何应用编辑，以及各配置域的链接。
---

nexcode 会把持久化配置存放在 `$NEXCODE_HOME/config.json`，通常是
`~/.nexcode/config.json`。在 Windows 上，默认路径是
...
- **CLI：** `nxc init` 会创建初始文件，而 `nxc provider`、`nxc models`、
  `nxc combo`、`nxc agent` 和 `nxc config` 等命令会更新或检查它们所负责的设置。
...
`NEXCODE_HOME` 的优先级高于默认配置目录。支持环境引用的字段，例如
...
:::note[原子写入]
nexcode 会通过临时文件再重命名（`atomicWriteFile`）的方式写入托管的 `config.toml` 和 `nexcode-catalog.json` 文件。
这可以避免在并发写入时留下半写入文件，例如 `nxc stop` 和代理 shutdown handler 同时恢复 Codex 的情况。
:::
