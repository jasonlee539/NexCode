...
description: What responses-state.json.nxc.*.tmp files are, why they could accumulate, and how to reclaim them.
---

Some users found many gigabytes of files named like
`responses-state.json.nxc.<pid>.<seq>.tmp` in their nexcode home
(`~/.nexcode` by default), growing after every reboot.

## What these files are

nexcode keeps a continuation cache so `previous_response_id` chains survive a
