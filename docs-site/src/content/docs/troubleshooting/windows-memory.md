...
description: Why the bun process can grow to many gigabytes of RAM on Windows, what nexcode does about it today, and your options until the upstream Bun fixes ship.
---

Some Windows users see the `bun` process behind nexcode grow to many
gigabytes of RSS during long streaming sessions (reported as issue
[#314](https://github.com/lidge-jun/nexcode/issues/314)). This page explains
what is actually happening and what you can do about it, honestly.

## Root cause: upstream Bun runtime issues

nexcode bundles the Bun runtime (currently **1.3.14**). The memory growth is
...
On Windows, nexcode must keep streaming responses on a conservative code
...
- **Memory watchdog** — the proxy samples its own memory every minute and logs a
  ...
  retention.
- **`nxc doctor`** — a "Memory / runtime" section shows the *service*
...
1. **Wait for a bundled runtime update.** Once a Bun release verifiably
   carries the fixes, nexcode will bump the bundled runtime and the safer
   stream path turns on automatically on Windows (macOS keeps requiring the
   explicit opt-in below).

2. **Run a Bun runtime you trust with `NEXCODE_BUN_PATH`.** This is
   unvalidated territory — you are running nexcode on a runtime we have not
   ...
   the environment variable, then re-run `nxc service repair` from that same
...
and after `nxc doctor` memory sections on
[#314](https://github.com/lidge-jun/nexcode/issues/314) — that is exactly
