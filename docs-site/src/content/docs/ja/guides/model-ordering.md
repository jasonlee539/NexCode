---
title: モデルの並び順について
description: nexcode が Codex モデルピッカーと spawn_agent モデルオーバーライドの順序を決める方式。
---

Codex モデルピッカーは nexcode 設定に書かれたプロバイダー宣言順やモデル配列順を保存しません。
...
そのため nexcode は配列位置ではなくより低い priority を付与してフィーチャー位置を制御します。
...
(nexcode が作った項目は通常 `9`)を使います。ルーティンググループ内部は引き続きプロバイダー/ID
アルファベット順です。
...
設定と `nxc agent subagents set` は exact account-qualified
...
現在 `NxcConfig` には一般 `modelOrder`、`providerOrder`、priority map 設定はありません。サポートされるソート
