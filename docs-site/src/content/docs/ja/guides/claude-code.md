...
description: Claude Code でルーティングされたすべてのモデルを使います。nexcode は同じポートで Anthropic Messages API とゲートウェイモデル検索を提供します。
---

nexcode は `/v1/responses` と共に `POST /v1/messages`(`count_tokens` も)を提供します。そのため Claude
...
`nxc claude` はプロキシが実行中か確認した後、環境を接続して Claude Code を実行します。
...
直接 export した変数が常に優先します。追加引数はそのまま渡されます: `nxc claude -p "hello"`。

## システム環境統合(macOS)

`claudeCode.systemEnv` を `true` に設定すると(デフォルト: **オフ`)`nxc start` が `launchctl setenv` を
使い `ANTHROPIC_BASE_URL` と関連 Claude Code 環境変数をシステム全体に注入します。そのため新規
ターミナルのウィンドウとタブでは `nxc claude` ラッパーなしでも通常の `claude` コマンドがプロキシを経由します。すでに開いている
シェルには適用されないので開き直す必要があります。

`nxc stop` とプロキシ終了は**注入されたキーを解除します**。以前の値を復元せず、nexcode が
注入したキーのみ削除します。プロキシは `~/.nexcode/claude-env.sh` も書き出し、`nxc start` はこのファイルを
自動で読み込む `.zshrc` source hook を、実行可能な Claude Code CLI が `PATH` にある場合にのみインストールします。
Claude Code が存在しない場合、またはシステム環境連携が無効な場合、起動処理と `nxc ensure` は NexCode が追加した
...
専用で、他のプラットフォームでは `nxc claude` を使ってください。
...
`x-nexcode-api-key`、`origin` は常に転送前に削除します。非ループバック bind のネイティブ
パススルーでは、有効なプロキシ認証情報を `x-nexcode-api-key` でも要求し、
...
では専用プロキシ admission ヘッダーも有効であること。そのため `nxc claude` を
...
受け付けるため、nexcode はルーティングモデルを安定で元に戻せるエイリアスとして公開します。

| 画面 | 形式 | 例 |
| --- | --- | --- |
| Claude Code CLI | `claude-nxc-<provider>--<model>` (plain) または `claude-nxc2-…` (escaped) | `claude-nxc-native--gpt-5.6-sol` |
...
`/model <id>` を使用してください。NexCode はピッカーの状態を直接参照できず、各リクエストに
含まれるモデル ID をルーティングします。結果は **Logs → requestedModel** で確認できます。

**エイリアス構文ルール:** provider には `/` や `--` を含められず `native` と同じでもいけません。
`/` も `~` も含まない plain な model ID は v1 接頭辞 `claude-nxc-…` のままです。`/` または `~` を含む
model ID は v2 接頭辞 `claude-nxc2-…` で発行し、エスケープします(`/` → `~s`、`~` → `~t`)。例:
...
`effectiveModelEnv` は `nxc claude` / システム環境 / シェルファイルが注入するスロット 6 つを計算します。
...
`tierModels.haiku` と `smallFastModel` の両方がない場合、NexCode は 2 つのヘルパーモデル変数を未設定のままにします。その後 Claude Code がネイティブのヘルパーモデル（現在は Sonnet）を選択し、ネイティブプロバイダーで料金が発生する可能性があります。

## ロスターエージェント(injectAgents)

プロキシの起動/ensure、`nxc claude`、関連するダッシュボード保存は推奨サブエージェントロスター(Subagents タブ、最大 5 モデル)と
`nxc-self` を `~/.claude/agents/nxc-*.md` に同期します。

- **`nxc-self`** は `/model` ピッカーのデフォルトを固定し、値がない場合は `claudeCode.model` を使います。
  両方ない場合は作成しません。モデル継承は使いません。
- 各エージェント本文には `<!-- nxc-route: <model> -->` ディレクティブが含まれます。プロキシはこのディレクティブで
  実際のルートを固定します。そのため Agent ツールの `model` 引数は機能せず、プレースホルダとして
  `"haiku"` を渡してください。
- frontmatter にはエイリアスが入り、ルーティングはディレクティブに従います。
- `generated-by: nexcode` が含まれる標識検証済み `nxc-*.md` ファイルのみ上書きまたは整理します。
  ユーザー作成のエージェントは触りません。
...
ディスパッチ例: `subagent_type: "nxc-gpt-5-6-sol"`。1M をサポートする対象には `[1m]` が自動で
付きます。
...
nexcode はデフォルトで**ルーティングされた**リクエストのスキル内容を短いスタブに差し替えます。ネイティブ
...
ルーティングモデルごとに使えるホスト型ツールと画像サポート範囲が異なります。nexcode はメインモデルが
...
`nxc debug claude on|off|status|reset`、`NXC_CLAUDE_DEBUG=1` または
...
バージョンを使っている場合は nexcode を更新してください。
...
`ANTHROPIC_AUTH_TOKEN` が設定されています。`nxc claude` は意図的に `ANTHROPIC_API_KEY` を
設定しないため、直接 export していれば解除してください。`nxc claude` 使用時は
...
設定されているか確認してください(`nxc claude` では自動)。`nxc claude` を実行して
...
`ANTHROPIC_BASE_URL` が古い値の可能性があります。新規ターミナルを開くか `nxc claude` を再実行してください。
...
言及すると自動で読み込まれます。ネイティブパススルーでは正常で、ルーティングモデルでは nexcode が
デフォルトでスタブに差し替えます(`blockedSkills: ["claude-api"]`)。

**サブエージェントが誤ったモデルにディスパッチされる** — ロスターエージェント(`nxc-*`)は Agent ツールの `model`
引数ではなく `<!-- nxc-route: ... -->` ディレクティブを使います。ディレクティブが希望ルートと一致するか確認し、
