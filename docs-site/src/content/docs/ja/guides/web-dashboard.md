---
title: ウェブダッシュボード
description: プロキシ状態、プロバイダー、モデル、委任ガイド、認証プール、使用量、ログを管理する nexcode GUI。
---

nexcode はプロキシが提供するローカルウェブダッシュボード(`gui/` 配下の Vite/React アプリ)を含みます。
...
`localhost` や `127.0.0.1` などのループバックアドレスで開いたダッシュボードは、短時間有効な GUI セッションを自動的に受け取るため、通常はトークン入力が不要です。ループバック以外のホストで公開する場合は、`NEXCODE_ADMIN_AUTH_TOKEN`、または自動生成される `~/.nexcode/admin-api-token` ファイルの管理トークンが必要です。
...
| **サブエージェント委任** | NexCode の委任ガイダンスとオプションの Codex ネイティブサブエージェント既定値で共有するネイティブ/ルーティングモデルと任意の推論強度を選びます。スポーンごとのルーターではありません。下記を参照してください。 |
...
| **Codex 自動起動** | インストール済み Codex launcher shim に `nxc ensure` の実行を許可します。このトグルは shim やバックグラウンドサービスをインストールしません。 |
...
保存します。選択値は NexCode が作成する委任ガイダンスで使われ、そのガイダンスは
`multiAgentGuidanceEnabled` で別に制御されます。モデルを消去すると保存済み effort も消去され、
ネイティブ既定値の同期も無効になります。

**Codex ネイティブサブエージェント既定値として使用**を有効にすると、NexCode が有効な Codex
...
:::caution
2 つのトグルは独立しています。NexCode の委任ガイダンスを無効にしてもネイティブ既定値の同期は
...
- 各アカウントカードには **選択順序** のコントロール（最初 / 早め / 標準 / 遅め / 最後）があります。
  ...
  並べ替えられるので、**最後** にして予備に回せます。`nxc account priority` でプリセット以外の値を設定した場合も、カード上に
  選択肢として残ります。
...
- account の追加・削除と picker 設定は catalog refresh より先に保存されます。refresh が完了できない場合は
  amber の回復案内が表示されます。変更自体は保存済みなので、`nxc sync` で refresh を再試行してください。
