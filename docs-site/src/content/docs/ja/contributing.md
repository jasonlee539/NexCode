---
title: コントリビュート
description: nexcode の開発環境、構成、規約、プロバイダーとアダプターの追加方法。
---
...
公開ドキュメントは GitHub Pages の <https://nexcode.me/ja/> に公開されます。
...
- **Cross-platform CI**(`.github/workflows/ci.yml`)はランタイム、テスト、パッケージ、スクリプト、
  ...
  `nxc help` を検査します。別途 3 OS レーンはバンドルランタイムを使い、Bun を別途インストールしなくても
...
`src/providers/derive.ts` はこのエントリを `nxc init`、`nxc provider`、ダッシュボード preset、API キーログイン、
...
実行します。nexcode は大きな batch より小さく検証可能な commit を好みます。
