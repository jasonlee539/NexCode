...
| `hostname?` | `string` | `"127.0.0.1"` |バインドアドレス。非ループバック バインドには `NEXCODE_API_AUTH_TOKEN` が必要です。 |
| `proxy?` | `string` | — |送信 HTTP(S) プロキシ URL または `${ENV_VAR}`。これらの変数が設定されていない場合にのみ、`HTTP_PROXY` / `HTTPS_PROXY` に適用されます。ループバックは `NO_PROXY` に残ります。 |
| `emptyCompletionRetry?` | `boolean` | `false` | テキストもツール呼び出しもない Responses 完了を、同一リクエストで 1 回再試行するよう明示的に有効化します。再試行は課金対象になる場合があります。`NXC_EMPTY_COMPLETION_RETRY=0` で設定を変更せず無効化できます。combo と routed-compaction turn は対象外です。 |
...
| `apiKeys?` | `NxcApiKey[]` | `[]` |生成された `nxc_…` 資格情報は、非ループバック バインドでの管理およびデータ プレーン認証によって受け入れられました。ダッシュボードで管理。 |
...
| `codexAutoStart?` | `boolean` | `true` | Codex を起動する前に、Codex シムで `nxc ensure` を実行させます。 False を指定すると、操作が行われないことが保証されます。 |
| `codexShimAutoRestore?` | `boolean` | `true` |完了した外部 Codex アップデートによってインストールされたシムが置き換えられた後、インストールされているシムを復元します。環境オプトアウト: `NEXCODE_CODEX_SHIM_AUTO_RESTORE=0`。 |
| `syncResumeHistory?` | `boolean` | `true` | Codex App 履歴の互換性を元に戻すことができます。元のメタデータは `nxc stop` / `nxc restore` によってバックアップおよび復元されます。 |
...
| `webSearchSidecar?` | `NxcWebSearchSidecarConfig` |使用可能な場合はオン | Web 検索サイドカー オプション。 |
| `visionSidecar?` | `NxcVisionSidecarConfig` |使用可能な場合はオン |画像説明サイドカー オプション。 |
| `images?` | `NxcImagesConfig` | OpenAI の自動選択 | Codex `image_gen` のスタンドアロン イメージ リレー オプション。 |

バックアップ サポートが存在する前に古い開発ビルドで再開履歴メタデータが変更された場合は、`nxc recover-history --legacy-openai` を実行してネイティブ プロバイダーの回復を強制します。
...
プロキシは、この変数がないとリモート バインドを拒否します。バックグラウンド サービスの場合は、`nxc service install` の前にエクスポートして、launchd、systemd、またはタスク スケジューラがそれを受信できるようにします。クライアントは以下を送信する必要があります:

```text
x-nexcode-api-key: your-secret-token
```

|エンドポイント | `Authorization: Bearer` | `x-nexcode-api-key` | `x-api-key` |
| --- | --- | --- | --- |
...
`x-nexcode-api-key` のみを使い、`Authorization` と `x-api-key` は Anthropic 認証情報用に
...
任意のローカル ポートが機能します。ホストが `localhost`、`127.0.0.1`、または `::1` に解決されるリクエストは、ポートに関係なくループバックのままであるため、`http://localhost:20100/v1` が機能します。そのベース URL をクライアントに設定します。 `nxc` は、デフォルトのローカル `127.0.0.1` アドレスのみを管理対象クライアント設定に書き込みます。
...
これらの設定は、`/v1/messages`、`/v1/messages/count_tokens`、`nxc claude` ランチャー、および Claude ダッシュボード ページを制御します。
...
| `claudeCode.subagentEffort?` | `"low" \| "medium" \| "high" \| "xhigh" \| "max"` |継承 |生成された `~/.claude/agents/nxc-*.md` に書き込まれる作業量。 Codex のガイダンスおよびプロキシの上限とは別のものです。 `nxc claude` を通じて再起動して再生成します。 |
...
Anthropic OAuth サイドカーは、nexcode の既存のクロード コード OAuth フィンガープリントを再利用します。対象のアカウントとワークロードをソークテストします。
