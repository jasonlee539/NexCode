...
| `combos?` | `Record<string, NxcComboConfig>` | `{}` |注文されたプロバイダー/モデル ターゲットから構築された仮想 `combo/<id>` モデル。 |

## モデルの解決順序

nexcode は、要求されたモデルを次の順序で解決します。
...
同じ公開 label が復元されます。mutation は bounded catalog refresh より先に保存され、`nxc sync` warning は
...
コンボは、リストに表示できない場合でも、直接ルーティング可能です。 `nxc sync`、`/v1/models`、および Codex ピッカーは、すべてのターゲットが交差できる機能を公開している場合にのみリストします。
