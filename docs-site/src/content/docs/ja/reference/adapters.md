...
**アダプター**は nexcode の内部リクエスト/レスポンスモデルとプロバイダーの wire 形式の間を変換します。すべてのアダプターは `ProviderAdapter` インターフェース（`src/adapters/base.ts`）を実装します。
...
`buildRequest` は `NxcParsedRequest` を上流の HTTP リクエストに落とし、`parseStream` /
