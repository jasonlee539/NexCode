...
description: Windows'ta bun sürecinin neden çok sayıda gigabayt RAM'e kadar büyüyebildiği, nexcode'in bugün bu konuda ne yaptığı ve yukarı akış Bun düzeltmeleri gelene kadar seçenekleriniz.
---

Bazı Windows kullanıcıları, uzun akış oturumları sırasında nexcode'in
arkasındaki `bun` sürecinin çok sayıda gigabayt RSS'ye kadar büyüdüğünü
görmektedir ([#314](https://github.com/lidge-jun/nexcode/issues/314) numaralı
...
nexcode, Bun çalışma zamanını (şu anda **1.3.14**) paketler. Bellek büyümesi,
...
Windows'ta nexcode, #32111 çökmesini önlemek için akış yanıtlarını muhafazakar
...
- **`nxc doctor`** — bir "Bellek / çalışma zamanı" bölümü *servis* sürecinin Bun
...
1. **Paketlenmiş bir çalışma zamanı güncellemesini bekleyin.** Bir Bun sürümü
   düzeltmeleri doğrulanabilir şekilde taşıdığında nexcode paketlenmiş çalışma
...
2. **`NEXCODE_BUN_PATH` ile güvendiğiniz bir Bun çalışma zamanını
   çalıştırın.** Bu doğrulanmamış bir alandır — nexcode'i test etmediğimiz bir
   ...
   yolun dayanıklı servis tanımına yerleştirilmesi için aynı kabuktan `nxc
...
[#314](https://github.com/lidge-jun/nexcode/issues/314) üzerinde önceki ve
sonraki `nxc doctor` bellek bölümlerini bildirin — bu azaltmanın beklediği
