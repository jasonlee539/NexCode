...
Video Köprüsü, nexcode tarafından yönlendirilen herhangi bir OpenAI harici
...
Model bunu herhangi bir fonksiyon aracı gibi çağırır; nexcode çağrıyı yakalar,
xAI'a bir video üretim görevi gönderir, tamamlanana kadar yoklar ve sonucu
indirir.

## Ön Koşullar

- Bir **API anahtarına** sahip bir `xai` sağlayıcı kaydı (`nxc login xai` tek
...
- nexcode'in OpenAI harici sağlayıcı üzerinden yönlendirilecek şekilde
...
> ```
>
> `nxc login xai` (OAuth) ile katıldıysanız sağlayıcı `authMode: "oauth"` modunda kalır ve köprü sessizce devreye girmez. Ortamda `XAI_API_KEY` ayarlayın **veya** anahtarı yukarıda gösterildiği gibi sabit kodlayın.
...
1. nexcode, `videoBridgeEnabled: true` etkinken OpenAI harici yönlendirilen
   bir modeli algılar
2. Sohbete sentetik bir `video_gen` fonksiyon aracı enjekte edilir
3. Model `video_gen`'i çağırdığında nexcode xAI'ın `/videos/generations` uç
