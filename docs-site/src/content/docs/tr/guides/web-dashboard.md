...
description: Proxy sağlığı, sağlayıcılar, modeller, yetkilendirme rehberliği, kimlik doğrulama havuzları, kullanım ve günlükler için nexcode GUI'si.
---

nexcode, proxy'den sunulan yerel bir web kontrol paneli (`gui/` altında bir
...
paneli yönetici belirtecini (`NEXCODE_ADMIN_AUTH_TOKEN` veya otomatik olarak
oluşturulan `~/.nexcode/admin-api-token` dosyası) gerektirir.
...
| **Alt ajan yetkilendirmesi** | NexCode yetkilendirme rehberliği ve ayrı yerel varsayılan katılımı tarafından paylaşılan yerel veya yönlendirilen bir model ve isteğe bağlı akıl yürütme çabası seçin. Bu, proxy tarafında spawn başına bir yönlendirici değildir; aşağıya bakın. |
...
| **Codex otomatik başlatma** | Zaten kurulu bir Codex başlatıcı dolgusunun `nxc ensure` çalıştırmasına izin verin. Bu anahtar bir dolgu veya arka plan servisi kurmaz. |
...
isteğe bağlı olarak `injectionEffort`'ı saklar. **NexCode çoklu ajan
...
kullan (Use as native Codex subagent defaults)** anahtarı, NexCode aktif Codex
...
NexCode rehberliği Codex'ten `spawn_agent`'a geçersiz kılmalar iletmesini
...
- Her hesap kartı bir **Seçim sırası (Selection order)** kontrolü taşır (İlk,
  ...
  yedek olarak tutulabilir. Bu beş önayarın dışındaki `nxc account priority`'den
...
- Hesap ekleme, kaldırma ve seçici ayarı değişiklikleri model kataloğu
  ...
  için `nxc sync` çalıştırın. Hesabın veya ayarın kendisi kaydedilmiş olarak
  kalır.
...
Kenar çubuğunun yıldız düğmesi — ve etkileşimli bir terminalde `nxc start`'ın
sorduğu tek seferlik soru — **kendi `gh` girişinizden** geçer. nexcode hiçbir
...
- `nxc start` ve `nxc service install`, bir ajan veya CI donanımı onları
