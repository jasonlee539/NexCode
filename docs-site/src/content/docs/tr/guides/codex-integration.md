...
description: nexcode'in kendisini Codex'e nasıl enjekte ettiği, model kataloğunu nasıl senkronize ettiği, dolguları (shims) nasıl kurduğu ve nasıl temiz bir şekilde geri yüklediği.
---

nexcode, Codex'in okuduğu iki şeyi düzenleyerek Codex'in proxy üzerinden
...
`nxc init`, `nxc start` ve `nxc sync` enjektörü çağırır. Varsayılan geri döngü
(loopback) bağlantısında, Codex'in yerleşik `openai` sağlayıcı kimliğini korur
ve bu sağlayıcıyı nexcode'e yönlendirir:
...
nexcode'i işaret ettiğinden proxy bu çağrıları OpenAI yukarı akışına iletir.
...
- **Google Antigravity (CCA) geri dönüşü:** ne bir OpenAI iletme adayı ne de
  ...
  eksik bir ChatGPT kimlik bilgisi) tetiklenir. Bu, `nxc login
...
anahtarlı Responses sağlayıcıları için nexcode, Codex'in özel `image_gen` ad
...
bildiriminin yerini aldığında nexcode yinelenen bir barındırılan
...
NexCode yönlendirmeye sahip olduğunda, her iki mod da bir referans/geri dönüş
yapılandırması olarak `$CODEX_HOME/nexcode.config.toml` dosyasını yazar. Geri
...
Codex CLI, TUI, App ve SDK'nın tümü aynı Codex evini okur. nexcode bu dizini
...
değilse, nexcode `/mnt/c/Users/*/.codex/config.toml` konumunda tek bir Windows
...
NexCode geçmiş işlemleri Codex ile aynı önceliği kullanır: `config.toml`
...
`sqlite_home` anahtarı yoksa NexCode ortam/ev geri dönüşüne devam eder. Dosya
...
zamanı evine ayarlayabilir. `nxc status` ve `nxc doctor` bu tam uyumsuzluk
...
geçişli yüzeylerini yerel Codex ile hizalı tutar. nexcode ayrıca WebSocket
...
yeniden eşlenmesi gerekmez. İlk senkronizasyonda daha eski nexcode derlemeleri
tarafından etiketlenen iş parçacıklarını da `openai`'ye geri geçirir. Geri döngü
olmayan özel sağlayıcı modu etkinken geçmişi yine de `nexcode` sağlayıcısı
...
`$CODEX_HOME/nexcode-catalog.json`). Başlangıçta ve `nxc sync` sırasında
nexcode:

1. Öne çıkarmanın geri alınabilir olması için bozulmamış kataloğu
   `~/.nexcode/catalog-backup.json` konumuna bir kez **yedekler**.
...
sağlarken, nexcode yalnızca modelin sıradan fonksiyon çağrısını yönlendirir.
Araç yürütme, izinler ve onaylar Codex'e yerel kalır; nexcode ikinci bir
...
Responses sağlayıcıları için nexcode bu bildirimi ve geçmişini bir yukarı akış
...
`nxc sync` bu meta verileri değiştirdikten sonra Codex App'i yeniden başlatın ve
...
Yanıt ham `nexcode-catalog.json` belgesidir (sağlayıcı kimlik bilgisi yoktur).
Kullanılabilir olduğunda `x-nexcode-codex-version` başlığı sunucudaki Codex
...
kararlıdır**. Her `nxc sync` ve katalog yenilemesi yönlendirilen girdileri
...
katalog korunur ve bir sonraki başarılı `nxc sync` yapılandırılan adı yeniden
...
`config.toml` zaten `openai` veya `nexcode` dışında bir sağlayıcı seçiyorsa
NexCode dosyayı değiştirmeden bırakır ve profil yazmalarını, katalog/önbellek
...
Codex sağlayıcı yapılandırmasının sahibi olarak tek bir araç tutun. NexCode'i
...
sağlayıcı formuyla eşleşecek şekilde `NEXCODE_API_AUTH_TOKEN`'dan
`x-nexcode-api-key` değerini de iletin. NexCode'in yönlendirmeyi doğrudan
...
ardından `nxc start`'ı yeniden çalıştırın.
...
3. **Boş `models` ile `liveModels: false`** — canlı keşif kapalı olduğunda ve
   `models` boş veya atlandığında nexcode bu sağlayıcı için hiçbir
...
5. **Önbellek ve `nxc sync`** — canlı kataloglar yaklaşık beş dakika önbelleğe
   alınır (`modelCacheTtlMs`, varsayılan `300000`). Yeni bir getirmeye zorlamak
   ve kataloğu hemen yeniden yazmak için `nxc sync` çalıştırın.
6. **Çalışan Codex `app-server`** — uzun ömürlü bir Codex `app-server` (Desktop
   ...
   diskteki kataloğu yeniden yazmak yeterli değildir. `nxc sync` ve `nxc
   sync-cache` bu süreçler algılandığında uyarır. Bunları `nxc sync
...
Katalog yazmaları (`nexcode-catalog.json`, `config.toml`) nexcode **içinde**
atomiktir, bu da yalnızca nexcode'e ait iki yazıcı yarıştığında yarı yazılmış
dosyaları önler. Bu, nexcode yazdıktan sonra başka bir yerel sürecin, dosya
...
ve bunu bağımsız olarak yenileyebilir, `nexcode-catalog.json` dosyasını
...
yapılandırın, ardından `nxc sync` çalıştırın — bu onaylanmış bir nexcode
...
nexcode proxy'si çalışmıyordur: yapılandırılmış portu hiçbir şey
dinlemiyordur, bu nedenle istemci bu ham bağlantı hatasını kendisi işler.
Proxy'yi yeniden başlatın:

```bash
nxc start              # ön plan
nxc service install    # kalıcı: girişte otomatik başlar ve çökmede yeniden doğar
```

`nxc status` proxy'nin çalışıp çalışmadığını gösterir ve çalışmadığında aynı
yeniden başlatma ipucunu yazdırır; `nxc doctor` yeniden başlatma güvenliğini
...
Codex hesap havuzuna bir ChatGPT hesabı eklendiğinde nexcode, Codex Responses
...
nexcode sizi asla tuzağa düşürmez. **`nxc stop`, yerel Codex'e tamamen geri
...
böylece düz `codex` sanki nexcode hiç var olmamış gibi tam olarak çalışır:

```bash
nxc stop       # proxy'yi + servisi durdurun, yerel Codex'i geri yükleyin
nxc restore    # durdurmadan geri yükleyin  (takma ad: nxc eject)
nxc restore back # düz Codex'i çalışan proxy'ye yeniden yönlendirin
```

nexcode yönetilen bir [arka plan servisi](/tr/reference/cli/#nxc-service)
olarak çalıştığında `NXC_SERVICE=1` ayarlar, böylece servis odaklı bir yeniden
başlatma Codex yapılandırmasını **bozmaz** — yalnızca açık bir `nxc stop` / `nxc
