...
description: nexcode modellerinin paylaşılan Codex kataloğu aracılığıyla Codex App, Codex CLI ve Codex TUI içinde nasıl göründüğü.
---

nexcode, Codex App'i yamalamaz. Codex CLI/TUI'nin kullandığı aynı Codex
...
hala mevcut olan uygun seçicilere sahip olduğunda, nexcode eşlenen hesaplar
...
başarısız olur. Codex'in hesap kapsamlı kataloğu henüz nexcode'in statik
...
`gpt-daybreak-blue-latest` gönderir; nexcode bunu Sol'a yeniden yazmaz, yalın
...
kaynağını `~/.nexcode/config.json.pre-openai-tiers-v2.bak` konumunda tutar; şu
...
Eşdeğer bir yönlendirilmiş model için nexcode açık, varsayılan olarak kapalı
...
`nxc init`, `nxc start` ve `nxc sync` paylaşılan Codex yapılandırmasını ve
...
Codex'in model seçicisi Codex biçimli katalog girdileri bekler. nexcode,
...
talimatlar gibi katı ayrıştırıcı alanlarını korur. nexcode daha sonra OpenAI
...
ailesi için nexcode, kurulu Codex kataloğunun daha zengin canlı girdilerini
...
kullanır. nexcode bu ayrımı korur. Yerel OpenAI doğrudan geçiş modelleri hızlı
...
panelinde görünen seçenekleri koyar. nexcode seçilen sırada düşük katalog
...
nexcode'in bu listeye hiçbir kancası yoktur; yukarı akış hatası
...
nexcode, katalog görünürlüğü, önceliği veya meta verileri her değiştiğinde
