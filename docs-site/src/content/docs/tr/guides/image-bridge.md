...
- **API anahtarına** sahip bir `xai` sağlayıcı kaydı. Köprü, istekleri doğrudan
  ...
  Yalnızca OAuth / `nxc login xai` köprüyü **etkinleştirmez** (Grok CLI OAuth
...
Görsel Köprüsü seçenekleri `~/.nexcode/config.json` dosyasındaki `images`
...
Üretilen görseller `~/.nexcode/artifacts/` dizinine kaydedilir. Uzun
...
1. Bir Responses isteği `tools` içinde `image_generation` içerdiğinde, NexCode
...
3. Model bu aracı çağırdığında, NexCode çağrıyı yakalar ve prompt'u xAI'ın
   görsel üretim API'sine iletir.
4. Üretilen görseller `~/.nexcode/artifacts/` dizinine kaydedilir ve **yerel
