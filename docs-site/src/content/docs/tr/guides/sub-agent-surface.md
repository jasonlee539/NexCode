...
bağımsız görev paralel olarak çalışabilir. nexcode bu çalışanları hangi Codex
...
nexcode bunu hem canlı `/v1/models` kataloğuna hem de diske senkronize edilen
...
- `injectionModel`, nexcode rehberliğinde adı geçen tercih edilen çalışan
  modelidir.
...
nexcode tarafından yazılan rehberlik için ana anahtardır. Kapatılması hem v2
atama bloğunu hem de v1 proaktif metnini bastırır.

Dizi biçimli durum bilgisi olmayan Responses istekleri için nexcode,
...
nexcode temel spawn talimatlarını kesmek yerine önce kadroyu bırakır. Yerleşik
...
v1'de nexcode yalnızca `max` veya `ultra` çabada yukarı akış tarzı proaktif
...
ayrıdır. nexcode aktif Codex yönlendirmesine sahip olduğunda senkronizasyon
...
girdileri olarak yazabilir. nexcode yalnızca kendi işaretçilerini taşıyan
...
Oluşturulan bir çalışan için nexcode şu öncelik sırasını oluşturur:

1. İstenen birincil model.
2. nexcode yapılandırmasındaki `subagentModelFallbackByModel`'dan, istenen
   birincil modelle anahtarlanan model başına bir zincir.
3. nexcode yapılandırmasındaki genel `subagentModelFallback` listesi.

Rol başına geri dönüş zincirleri `$CODEX_HOME/agents/*.toml` içinde değil,
nexcode yapılandırmasında yer alır. Codex 0.146+ ajan rol dosyalarını katı bir
şekilde serileştirir ve `model_fallback`'i bilinmeyen bir alan olarak reddeder,
bu da tüm rol tanımını atlar (#1190). nexcode geriye dönük uyumluluk için
TOML'dan eski bir `model_fallback` satırını yine de okuyabilir, ancak `nxc
...
nexcode devre dışı bırakılmış, yönlendirilemez, devre dışı bırakılmış bir
...
bilinen [#92 sınırlamasıdır](https://github.com/lidge-jun/nexcode/issues/92).

nexcode boş veya okunamayan bir görevi iletmek yerine güvenli bir şekilde
başarısız olur:
...
İşbirliği arayüzü ve yerel özellik ayarları için `nxc v2` kullanın:
...
Yetkilendirme, kadro, çaba sınırı ve geri dönüş ayarları için `nxc agent`
kullanın:
...
Boş bırakılabilir bir `nxc agent injection` değerini temizlemek için `-` iletin
...
çalışan bir App ana bilgisayarı hala eski katalog durumunu gösteriyorsa `nxc
sync` çalıştırın ve bu Codex arayüzünü yeniden başlatın.

### nexcode kataloğa güvenemediğinde ne olur?

nexcode, diskteki model kataloğunu geçerli kullanıcıya ait her Codex
...
| `stale` | En az bir app-server katalogdan önceye dayanıyor | **nexcode kaynaklı model rehberliği yok** |
| `unknown` | Karşılaştırma yapılamadı | **nexcode kaynaklı model rehberliği yok** |

`stale` ve `unknown` için nexcode diskten türetilen kendi iddialarını — tercih
...
süreç başlangıç zamanı veya başarısız bir süreç numaralandırması — ve `nxc
...
bir üst katmandır; nexcode daha sonra seçilen sağlayıcı için değeri eşler veya
sabitler.
