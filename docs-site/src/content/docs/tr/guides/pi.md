...
description: "Inflection Pi ve hafif sohbet modellerini NexCode üzerinden bağlama rehberi."
---

Pi reads its providers from a single global JSON file rather than environment
variables, so
nexcode does not launch it. Instead, `nxc export` serializes the `nexcode`
provider block —
...
`nxc export` never writes that file. Merge the `providers.nexcode` block into
it — replacing the
...
The exported block is a static snapshot, not a live view. Re-run `nxc export`
after adding a
...
(`NEXCODE_OPENCODE_API_KEY`, in `{env:…}` form) — see the [opencode
guide](/guides/opencode/).

**A loopback proxy needs no key at all.** nexcode binds `127.0.0.1` by default
and authenticates
nothing there, so the `$NEXCODE_API_KEY` reference is inert and you can leave
the variable unset.
...
[open an issue](https://github.com/lidge-jun/nexcode/issues) with what Pi
reported.
:::

## Requirements

A running nexcode proxy (`nxc start`) and Pi installed. `nxc export` reads the
live catalog
