...
sunan tek bir sanal modeldir. İstemciniz `combo/<kimlik>` ister; nexcode bir
...
olursa nexcode `openai/gpt-5.6-sol`'a geçebilir.
...
`nxc combo set <kimlik>` komutundaki kombo kimliği bir harf veya rakamla
...
[nexcode #241](https://github.com/lidge-jun/nexcode/issues/241) tarafından
...
bir görüntüleme etiketi gerektirir. Takma ad, bu nexcode sürümü tarafından
...
Yukarı akış yanıtı geçerli bir `Retry-After` değeri içeriyorsa nexcode bunun
...
İstekte bir `reasoning` nesnesi yoksa nexcode bir tane oluşturur. Bir `effort`
...
Hedef yeteneği bilinmediğinde veya yapılandırılan çabayı içermediğinde nexcode
...
#92](https://github.com/lidge-jun/nexcode/issues/92)). Yerel bir üst ajan,
...
Kombonun şifre çözme yeteneğine sahip hiçbir hedefi yoksa nexcode göndermeden
...
```bash
nxc combo list
nxc combo show <kimlik>
nxc combo set <kimlik> --targets saglayici/model[:agirlik],...
nxc combo remove <kimlik> --yes
```
...
adlarıdır; `delete`, `remove`'un takma adıdır; ve aynı alt komutlar `nxc route
...
`nxc combo list` çalıştırın, yazımı ve büyük/küçük harf durumunu kontrol edin ve
yönetim komutunuzun model isteklerini alan çalışan aynı nexcode örneğine
yazdığını onaylayın.
