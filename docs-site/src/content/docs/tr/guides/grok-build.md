---
title: Grok Build
description: xAI Grok Build CLI içerisinden nexcode ile yönlendirilen herhangi bir modeli kullanın — proxy çalışırken modeller ~/.grok/config.toml içine otomatik olarak kaydedilir.
---

nexcode, yerel portunda OpenAI uyumlu bir `POST /v1/chat/completions` (ve
`/v1/responses`) sunar ve Grok Build, OpenAI uyumlu sunuculara karşı özel
modelleri destekler. Bu entegrasyonla başlayarak nexcode, görünür kataloğunun
tamamını otomatik olarak Grok Build'e kaydeder — manuel yapılandırma düzenlemesi
gerekmez.

## Otomatik Kayıt (Auto-registration)

`~/.grok` dizini mevcut olduğunda `nxc start` (ve `nxc ensure` / `nxc restart`),
...
# ... görünür model başına bir [model.nxc-*] tablosu ...
...
- **Eklemelidir (Additive):** çitin dışındaki kendi yapılandırmanıza asla
  ...
  `~/.grok/config.toml.bak-nexcode` konumuna tek seferlik bir yedek yazılır.
- **Eşkuvvetlidir (Idempotent):** her `nxc start` (ve otomatik başlatma
  etkinleştirildiğinde `nxc ensure`), çitle çevrili bloğu geçerli katalogla
  değiştirir.
- **Kaldırıldığında temizlenir:** `nxc stop`, `nxc eject`, `nxc uninstall` ve
  ...
  `nxc stop`/`nxc uninstall` üzerinden gerçekleşir (servis modu süreçleri
  kasıtlı olarak yeniden başlatmalar boyunca bloğu korur).
- **Çakışma güvenlidir:** kendi `[model.*]` tablolarınız tarafından zaten
  tanımlanmış takma adlara saygı duyulur (nexcode kendi girdilerine sonek
...
```bash
grok models          # yerel grok modellerinin yanında nxc-* girdilerini listeler
grok -m nxc-anthropic-claude-opus-4-8 -p "hello"
# veya TUI içinde: /model nxc-anthropic-claude-opus-4-8
```
...
girdileri için nexcode, yapılandırılmış sağlayıcı katmanlarını
...
(`nexcode-loopback`) taşır — nexcode geri döngü bağlantıları için kabul
anahtarlarını yok sayar, bu nedenle gerçek bir sır söz konusu değildir.

**Otomatik kayıt yalnızca geri döngü içindir.** nexcode geri döngü olmayan bir
...
sırrınızı `~/.grok/config.toml` dosyasına koyar ve bir sonraki `nxc
start`/`ensure`/`restart` sırasında orada ayarladığınız her şeyin üzerine yazar.
Bu nedenle nexcode bu durumda hiçbir şey yazmaz (ve daha önceki bir geri döngü
bağlantısından kalan tüm blokları kaldırır) ve modelleri yönetilen işaretçilerin
dışında kendiniz yapılandırırsınız, burada nexcode'in yaptığı hiçbir şey
...
`api_key` (`NEXCODE_API_AUTH_TOKEN` değeriniz) ayarlayın.
...
zincirinde ilk sırada yer alır; bu nedenle nexcode'e karşı yapılan dönüşler ek
...
`~/.grok/config.toml` dosyasını kendiniz yönetiyorsanız — veya nexcode geri
döngü olmayan bir bağlantıdaysa — `# >>> nexcode managed block`
...
api_backend = "responses"
api_key = "NEXCODE_API_AUTH_TOKEN_DEGERINIZ"
```
...
- **Servis kurulu `nxc restart`:** çalışan proxy yeniden başlatma
...
- **Yapılandırma okuma zamanlaması:** en öngörülebilir sonuçlar için önce
  nexcode'i başlatın, ardından `grok`'u başlatın. Grok Build
  ...
  geçersiz kıldığını unutmayın; bu nedenle nexcode dosyayı atomik olarak yazar
  — Grok asla yarı yazılmış bir yapılandırma görmez.
- **Katalog güncellemeleri:** çitle çevrili blok, enjeksiyon anındaki kataloğu
  yansıtır. Sağlayıcılar veya modeller ekledikten sonra yenilemek için `nxc
