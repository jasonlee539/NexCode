---
title: Sağlayıcılar
description: nexcode'in bir LLM sağlayıcısıyla kimlik doğrulaması yapmasının ve konuşmasının her yolu — OAuth, API anahtarı, ChatGPT iletme ve yerel.
---
...
listesi. Sağlayıcılar `~/.nexcode/config.json` içinde `providers` altında
bulunur.
...
`~/.nexcode/config.json.pre-openai-tiers-v2.bak` konumunda bir kez tutulur;
...
bir cihaz akışı köprüsü aracılığıyla GitHub Copilot. nexcode bunların kimlik
bilgilerini `~/.nexcode/auth.json` içinde saklar ve otomatik olarak yeniler.
...
nxc login nous         # Nous Portal (cihaz yetkisi; ücretsiz + ücretli modeller)
nxc login kiro         # kiro-cli kimlik bilgilerini içe aktarın (veya belirteç geri dönüşü)
nxc login google-antigravity
nxc login cursor       # bağımsız Cursor PKCE girişi
nxc login command-code # Command Code tarayıcı OAuth (veya ~/.commandcode/auth.json içe aktarma)
nxc login github-copilot  # GitHub cihaz akışı → Copilot belirteci (Copilot Pro/Business)
nxc login chatgpt      # bağımsız ChatGPT OAuth girişi
nxc logout <saglayici>
```
...
Uç bir Nous yenileme hatasından sonra yeniden kimlik doğrulamak için `nxc login
nous` çalıştırın.

Kurallı Kimi Coding Plan önayarları için (`kimi` hesap girişi ve `kimi-code` API
anahtarı), nexcode Chat Completions isteğine yalnızca arayan tarafından
...
edilmiş bir yukarı akış alanı reddederse nexcode bunu kaldırmaz ve yeniden
...
ayrı bir defteri vardır. Belirteçler `~/.nexcode/auth.json` içinde kalır;
`/api/oauth/accounts` yalnızca maskelenmiş meta verileri döndürür.

### Cockpit Tools Antigravity içe aktarma

v1 için NexCode, yalnızca `google-antigravity` sağlayıcısı için bir **Cockpit
...
```bash
nxc account import google-antigravity --format cockpit-tools --file <yol> [--json]
...
nexcode, eşzamanlı isteklerin kimlik bilgisi deposuyla yarışmaması için
...
yazar. nexcode, arayan kişi bunları göndermediğinde resmi istemci kimliği
(örneğin `originator`, oturum veya iş parçacığı başlıkları) **uydurmaz**.

**Tanılama ve yeniden kimlik doğrulama.** İnsan tarafından okunabilir `nxc
status`, bir OAuth sağlık bloğu yazdırır (maskelenmiş hesap kimlikleri, belirteç
yok). `nxc doctor`, yazılabilir depo / tek uçuş denetimleri ve bir kurtarma
Eylemi içeren UYARI satırlarıyla bir OAuth güvenilirlik bölümü ekler. Bir OAuth
sağlayıcı hesabının yeniden kimlik doğrulaması gerektiğinde `nxc login
<saglayici>` çalıştırın (veya kontrol panelinde Yeniden Kimlik Doğrula'yı
kullanın). Codex havuz hesapları bir `nxc login` sağlayıcısı değildir — kontrol
paneli Codex hesap havuzu aracılığıyla yeniden kimlik doğrulaması yapın. CLI
referansında [`nxc status` / `nxc doctor`](/tr/reference/cli/) bölümüne bakın.
...
ile oturum açın. Bir `kiro-cli` oturumu olmadan `nxc login kiro`, yapıştırılmış
bir erişim belirtecine veya `KIRO_ACCESS_TOKEN` ortam değişkenine geri döner.

`nxc login kiro` içe aktarma yolu platform Kiro CLI depolarını arar ve SQLite
...
- `KIROCLI_DB_PATH` standart olmayan bir Kiro CLI SQLite veritabanını seçer. Yol
  zaten mevcut olmalıdır; bu içe aktarma yolu sırasında nexcode veritabanı,
...
nexcode önce `PATH`'i kullanır, ardından
`%LOCALAPPDATA%\Kiro-Cli\kiro-cli.exe` ve `C:\Program
Files\Kiro-Cli\kiro-cli.exe`'ye geri döner.

Başarılı bir içe aktarmadan sonra nexcode içe aktarılan kimlik bilgisini
`~/.nexcode/auth.json` dosyasına kalıcı hale getirir. Bu değişkenleri ve
...
açma iptal edilirse veya başarısız olursa (NexCode kimlik bilgisini kalıcı
...
nexcode 79 yerleşik önayar ile birlikte gelir: 67 anahtar tabanlı, sekiz
...
kasıtlıdır: ilk segment nexcode sağlayıcısını seçerken, `cline-pass/kimi-k3`
...
doğrulamıştır. nexcode bu talep edilen katmanları korur; arka uca özgü
...
nexcode istemci hatasına sağlayıcı rehberliği ve sentetik bir `Retry-After`
...
bitirir ancak son Responses olayını atlarsa nexcode beş saniyelik model
...
ve keşfi 256 KiB ve 256 ham satırla sınırlar. `nxc login command-code`, tarayıcı
...
sağlayıcısı, kontrol panelinde ve `nxc account refresh <saglayici>` komutundan
...
veya değiştirmek için `nxc account list`, `nxc account current` ve `nxc account
use` komutlarını kullanın. Komutlar, JSON çıktısı ve yeni oturum davranışı için
[CLI referansına](/tr/reference/cli/#nxc-account-subcommand) bakın.

### GPT-5.6 önizleme yolları

GPT-5.6 Sol/Terra/Luna, canlı kataloglar geride kalsa bile `nxc sync`'in
...
Bir sağlayıcı, bir "ajan" ürünü olup olmadığına göre **değil**, nexcode'in
...
ömürlü bir Copilot API belirteci ile değiştiren bir OAuth sağlayıcısıdır (`nxc
...
trafiği için `/chat/completions`'ı reddeder, bu nedenle nexcode yerleşik
...
Cursor deneysel bir adaptör olarak ayrı izlenir. `adapter: "cursor"`, `nxc
...
olarak görünür. Bir Cursor erişim belirteci yapılandırıldığında nexcode
...
aracılığıyla) `~/.nexcode/config.json` içindeki `providers.cursor` nesnesinde
...
olarak mevcuttur; yapılandırılmış bir yerel yürütücü olmadan nexcode isteği
...
barındırılan (yerel olmayan) bir Ollama'dır. nexcode, bulut serisini vizyon
...
nexcode'i yerel bir OpenAI uyumlu sunucuya yönlendirin — genellikle boş bir
anahtarla:
...
kontrol panelinde **Özel (Custom)**'ı veya `nxc init`'te `custom`'ı seçin ve
