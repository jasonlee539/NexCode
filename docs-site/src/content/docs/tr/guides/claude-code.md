---
title: Claude Code
description: Claude Code içerisinden yönlendirilen herhangi bir modeli kullanın — nexcode aynı port üzerinden Anthropic Messages API'sini ve ağ geçidi model keşfini sunar.
---

nexcode, `/v1/responses` uç noktasının yanında `POST /v1/messages` (artı
...
Sağlayıcılar kontrol panelinden (`nxc login anthropic` / hesap ekle) birden
...
`nxc claude`, proxy'nin çalıştığından emin olur, ardından ortam değişkenleri
...
doğrudan iletilir: `nxc claude -p "hello"`.
...
`nxc claude` artık yalnızca bir proje dotenv dosyasının getirdiği Anthropic
...
hangisini istediğiniz nexcode'in bakabileceği bir şeye bağlıdır, bu nedenle
varsayılan olarak bunu yapar.

**Claude → Claude Code** altında **Kimlik doğrulama modu**'nu **Otomatik**
(varsayılan) olarak bırakın; nexcode her başlatmada karar verir:
...
çıkış yapmak, yeniden yapılandırılacak hiçbir şey olmadan bir sonraki `nxc
...
bu nedenle `nxc` dışında başlatılan düz bir `claude` aynı şekilde davranır. Bu
dosya, proxy başladığında veya ayarları kaydettiğinizde yenilenen bir anlık
görüntüdür; `nxc claude` ise her zaman canlı olarak çözümler.
...
`nxc claude desktop` ve `apply`, geçerli profili Claude Desktop'a yazar. `show`
...
kütüphane geçersiz kılma için `NEXCODE_CLAUDE_DESKTOP_CONFIG_DIR` veya
...
**kapalı**), `nxc start`, `ANTHROPIC_BASE_URL` ve ilgili Claude Code ortam
değişkenlerini sistem genelinde enjekte etmek için `launchctl setenv` kullanır.
Yeni terminal pencereleri ve sekmeleri, bu nedenle `nxc claude` sarmalayıcısına
...
`nxc stop` ve proxy'nin kapatılması **enjekte edilen anahtarları kaldırır**
(önceki değerleri geri yüklemez — yalnızca nexcode'in enjekte ettiği
anahtarlar kaldırılır). Proxy ayrıca `~/.nexcode/claude-env.sh` dosyasını
yazar; `nxc start`, bunu otomatik olarak yükleyen `.zshrc` kaynak kancasını
...
ve `nxc ensure`, NexCode tarafından eklenen kancayı kaldırır. Claude Desktop ayrı
...
bırakın. Bu özellik yalnızca macOS içindir; diğer platformlarda `nxc claude`
kullanın.
...
`accept-encoding`, `x-nexcode-api-key` ve `origin` iletmeden önce her zaman
kaldırılır. Geri döngü olmayan bir bağlantıda, yerel doğrudan geçiş
`x-nexcode-api-key` içinde geçerli bir proxy kimlik bilgisi de gerektirir;
...
aynı zamanda "claude.ai connectors are disabled" uyarısının artık `nxc claude`
...
kimlikleri kabul ettiğinden, nexcode yönlendirilen modelleri kararlı, tersine
çevrilebilir takma adlar olarak sunar:

| Yüzey | Format | Örnek |
| --- | --- | --- |
| Claude Code CLI | `claude-nxc-<provider>--<model>` (düz) veya `claude-nxc2-…` (kaçışlı) | `claude-nxc-openai--gpt-5.6-sol` |
...
takma adlar ve eski yapılandırmalardan gelen `claude-nxc-<provider>--<model>`
kimlikleri hala çözümlenir.

Claude Desktop'ın altbilgi seçicisi zaten çalışan bir 3P görüşmesi için modeli
değiştirmezse, o görüşmede `/model <id>` komutunu kullanın. NexCode seçici
...
önekini `claude-nxc-…` korur. `/` veya `~` içeren model kimlikleri, kaçışlarla
(`/` → `~s`, `~` → `~t`) v2 önekini `claude-nxc2-…` basar, örn.
`openrouter/anthropic/claude-opus-4-8` →
`claude-nxc2-openrouter--anthropic~sclaude-opus-4-8`. v1 takma adları harfi
...
`effectiveModelEnv`, `nxc claude` / sistem ortamı / kabuk dosyası tarafından
...
Hem `tierModels.haiku` hem de `smallFastModel` bulunmadığında, NexCode her iki
...
`nxc claude` (ve sistem ortamı arka plan programı), öne çıkan alt ajan kadronuzu
(Alt Ajanlar sekmesi, en fazla 5 model) artı `nxc-self`'i
`~/.claude/agents/nxc-*.md` içine senkronize eder.

- **`nxc-self`**, `/model` seçici varsayılanınızı sabitler (`claudeCode.model`'e
  geri döner); ikisi de olmadığında atlanır. Model kalıtımı KULLANMAZ.
- Her ajan gövdesi bir `<!-- nxc-route: <model> -->` yönergesi içerir — proxy
...
- Yalnızca `generated-by: nexcode` içeren işaretçi doğrulamalı `nxc-*.md`
...
Dağıtım: `subagent_type: "nxc-gpt-5-6-sol"`. 1M yetenekli hedefler `[1m]`
...
eğitilmemiştir, bu nedenle varsayılan olarak nexcode **yönlendirilen**
...
sahip değildir. nexcode, ana model yanıt vermeden önce bu boşlukları doldurur:
...
Açık bir `backend` her zaman kazanır. Atlandığında nexcode, kullanılabilir
...
seçmek **kapalı olarak başarısız olur (fail closed)**: nexcode sessizce
...
`nxc debug claude on|off|status|reset`, `NXC_CLAUDE_DEBUG=1` veya `PUT
...
- Hızlı başlangıç (`nxc claude`) ve manuel ortam bloğu
...
veya `ANTHROPIC_AUTH_TOKEN` ayarlanmıştır. `nxc claude` kasıtlı olarak
`ANTHROPIC_API_KEY` AYARLAMAZ; dışa aktardıysanız kaldırın. `nxc claude`,
...
doğrulayın (`nxc claude` ile otomatik). `~/.claude/cache/gateway-models.json`
konumundaki ağ geçidi model önbelleğini yenilemek için `nxc claude` komutunu
...
veya `nxc claude` komutunu yeniden çalıştırın.
...
modellerde nexcode varsayılan olarak bunu taslakla değiştirir (`blockedSkills:
["claude-api"]`).

**Alt ajan yanlış modele gönderiliyor** — Kadro ajanları (`nxc-*`), Agent
aracının `model` argümanını değil, `<!-- nxc-route: ... -->` yönergelerini
