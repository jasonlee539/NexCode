...
description: Kontrol panelinden OpenCode, Pi, OMP, Hermes, OpenClaw, Kimi Code, Gajae Code, DeepSeek Harness ve MiniMax Code'u nexcode'e bağlayın — istemci başına tek bir anahtar ve her yazmadan önce alınan bir yedek.
---

**Entegrasyonlar** sekmesi, nexcode'in sağlayıcı bloğunu istemcinin kendi
...
|---|---|---|---|---|
| OpenCode | `~/.config/opencode/opencode.json` | JSON | sonraki doğrudan başlatmada | `NEXCODE_OPENCODE_API_KEY` |
| Pi | `~/.pi/agent/models.json` | JSON | yeni oturumlarda | geri döngü (loopback) yer tutucusu |
| OMP | `~/.omp/agent/models.yml` | YAML | OMP yeniden başlatıldıktan sonra | `nexcode-loopback` yer tutucusu |
| Hermes | `~/.hermes/config.yaml` | YAML | yeni oturumlarda | `NEXCODE_HERMES_API_KEY` |
| OpenClaw | `~/.openclaw/openclaw.json` | JSON5 | hemen, çalışan bir ağ geçidinde | `NEXCODE_OPENCLAW_API_KEY` |
| Kimi Code | `~/.kimi-code/config.toml` | TOML | yeniden başlatmada veya `/reload` ile | geri döngü (loopback) yer tutucusu |
| Gajae Code | `~/.gjc/agent/models.yml` | YAML | yeni oturumlarda veya `/model` açtığınızda | `NEXCODE_GAJAE_API_KEY` |
...
Yönetilen DSH desteğinin en düşük uyumlu sürümü **DSH 0.1.0-rc.6**'dır. NexCode yalnızca
`llm-pi-ai.providers.nexcode` bölümünü yönetir: Uygula ve Yenile bu bölümü değiştirir, Devre Dışı
...
`custom_provider.nexcode` alanına sahiptir; `defaultModel` değerini, seçilen
MiniMax kimlik bilgisi kaynağını veya kullanıcının MiniMax oturumunu değiştirmez.
Bağladıktan sonra MCode içinde bir `custom_provider:nexcode/<provider/model>` girdisi seçin.
...
olarak yalnızca geri döngü (loopback) içindir; uzaktan `x-nexcode-api-key`
...
nexcode bunları kendi ortamından okur. Ağ geçidiniz bir profil veya taşınmış
bir ev dizini ile çalışıyorsa, nexcode'i aynı değişkenler ayarlanmış olarak
başlatın; aksi takdirde doğru şekilde farklı bir kurulumu takip eder.

## Diğer dört yüzey anahtar değildir

**API Anahtarları (API Keys)** nexcode'in kendi kimlik bilgilerini yönetir ve
hiçbir şekilde bir istemci değildir. **Codex CLI**, proxy servisinin kendisi
tarafından bağlanır — nexcode'i başlatmak uygular, durdurmak yerel
...
Devre dışı bırakma, yalnızca nexcode'in kendisine ait olarak kaydettiği
...
`providers.nexcode` aralığını bayt bayt yamalar, dosyanın geri kalanı hiçbir
...
istisnadır: YAML yazıcıları sırasıyla yalnızca `providers.nexcode` ve
`llm-pi-ai.providers.nexcode` kısımlarını yamalar,
...
`x-nexcode-api-key` başlığı için alan yoktur. DSH genel bir headers haritası sunar, ancak rc.6
...
`x-nexcode-api-key` kimlik bilgisi bağlantısını yayınlamaz. Manuel uzak OMP
...
yerine `nexcode-loopback` yer tutucusu taşır. Hiçbir istemci yapılandırmasına
asla gerçek bir kimlik bilgisi yazılmaz.

**`nxc opencode` için başlatıcının sağlayıcı bloğu kazanır.** Bu başlatıcı,
`provider.nexcode`'i diskteki aynı girdiden üstün olan
...
Bağlandıktan sonra `nxc sync`, yönetilen MCode bloğunu güncel bağlam pencereleri ve
...
Metin komutları MiniMax'ın Anthropic uyumlu uç noktasını kullandığı için NexCode,
