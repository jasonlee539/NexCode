...
description: nexcode'in Codex seçicisindeki ve spawn_agent model geçersiz kılmalarındaki model sırasını nasıl belirlediği.
---

Codex model seçicisi, nexcode yapılandırmasındaki sağlayıcı bildirimlerinin
...
Bu nedenle nexcode, dizi konumuna güvenmek yerine daha düşük öncelikler
...
girdileri normal önceliklerini kullanır (nexcode tarafından oluşturulan
...
`<seçici>/<yerel-openai-modeli>` seçimleri için `nxc agent subagents set`
kullanın veya nexcode yapılandırmasını düzenleyin; kontrol paneli bu seçimleri
...
Şu anda `NxcConfig` içinde genel bir `modelOrder`, `providerOrder` veya öncelik
