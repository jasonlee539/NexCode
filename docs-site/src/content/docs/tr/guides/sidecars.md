...
girişi** sunmaz. nexcode bu yetenekleri iki sidecar ile doldurur. Her biri bir
...
`web_search` istediğinde nexcode:
...
2. Yönlendirilen modeli küçük bir **ajan döngüsünde** çalıştırır. Model
   `web_search`'i çağırdığında nexcode seçilen sidecar arka ucunu kullanır:
...
varsayılan olarak nexcode arama yapmaya veya nihai yanıtı döndürmeye karar
...
bir istek görsel taşıdığında, nexcode ana çağrıdan **önce** her görseli
