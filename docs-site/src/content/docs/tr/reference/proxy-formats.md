...
nexcode, tek bir yerel proxy'yi çeşitli istemci lehçelerinde sunar. Bir Codex
...
Bu, yerel nexcode veri düzlemi şeklidir. İstek gövdesi boş olmayan bir `model`
...
- istek yerel bir Anthropic taşıyıcısı veya `x-api-key` kimlik bilgisi
  taşıyorsa;
- geri döngü olmayan bir dinleyicide istek yalnızca `x-nexcode-api-key` içinde
...
eder. nexcode uygun bir OpenAI ailesi rotası seçer, yukarı akış kimlik
...
| Diğer yönlendirilen model | Bir `compaction_trigger` ile dahili, akışsız, araçsız bir sıkıştırma turu çalıştırır; `encrypted_content`'i bir `nxc1:` zarfı olan tam olarak bir sentetik `compaction` öğesi gerektirir; bu özeti v1 değiştirme geçmişine çözer |
...
| 502 | `invalid_response_error` | Sentetik tur tam olarak bir geçerli, boş olmayan `nxc1:` sıkıştırma öğesi üretmedi |
...
`X-NexCode-API-Key` anlamına gelir; diğer sütunlar `Authorization: Bearer ...`
...
geçerli şifreli metin bayt bayt korunur: nexcode şifresini çözmez, içeriğini
...
okuyamazsa nexcode bu sağlayıcıya okunamayan baytlar göndermek yerine
