---
title: Mimari
description: nexcode iç yapısı — modül haritası, AdapterEvent köprüsü, istek ayrıştırıcısı ve önbellekleme.
---

nexcode tek bir Bun sürecidir. Bir istek OpenAI Responses olarak girer, dahili
...
```
src/
├── cli/                # nxc komut dağıtımı, başlatma, durum, sağlayıcı komutları
...
├── config.ts           # ~/.nexcode/config.json, varsayılanlar, PID, ortam çözümleme
...
├── responses/
│   ├── parser.ts       # Responses isteği → NxcParsedRequest
...
ardından bir `NxcParsedRequest` oluşturur:

- **Mesajlar** — `input` öğeleri normalleştirilmiş bir `NxcMessage[]` haline
...
`: nexcode heartbeat` SSE yorum satırı yayar. Yorum satırı, olay üretmeden her
...
hem de `/v1/*` için `NEXCODE_API_AUTH_TOKEN` gerektirir; yapılandırılmış
...
Paylaşılan bir sağlık projeksiyonu (`oauth/health.ts`) `nxc status`, `nxc
...
kalıcı değildir. İstek kullanımı `NxcUsage` olarak normalleştirilir, Responses
...
nexcode `426 upgrade_required` döndürür; Codex daha sonra bu oturum için
...
Dahili model `types.ts` içinde yer alır: `NxcParsedRequest`, `NxcContext`,
`NxcMessage` birleşimi, `NxcContentPart` (metin / görsel), `NxcToolCall`,
`NxcTool`, `AdapterEvent` ve yapılandırma tipleri (`NxcConfig`,
`NxcProviderConfig`). İki yardımcı yaygın olarak kullanılır:
