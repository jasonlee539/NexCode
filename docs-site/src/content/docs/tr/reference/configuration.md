...
description: nexcode'in yapılandırmayı nerede sakladığı, düzenlemelerin nasıl uygulandığı ve her yapılandırma alanına bağlantılar.
---

nexcode kalıcı yapılandırmasını `$NEXCODE_HOME/config.json` içinde saklar,
normalde `~/.nexcode/config.json` konumundadır. Windows'ta varsayılan
`%USERPROFILE%\.nexcode\config.json`'dır.
...
- **CLI:** `nxc init` başlangıç dosyasını oluştururken `nxc provider`, `nxc
  models`, `nxc combo`, `nxc agent` ve `nxc config` gibi komutlar sahip
...
Dosya ayrıştırılamazsa nexcode dosyayı `config.json.invalid-<zaman-damgasi>`
...
kullanır. `NEXCODE_HOME` varsayılan yapılandırma dizinine göre önceliklidir.
...
nexcode, yönetilen `config.toml` ve `nexcode-catalog.json` dosyalarını
geçici bir dosya ve ardından yeniden adlandırma (`atomicWriteFile`) yoluyla
yazar. Bu, `nxc stop` ve proxy kapatma işleyicisi gibi eşzamanlı yazıcılar
