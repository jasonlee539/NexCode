...
Bir **adaptör**, nexcode'in dahili istek/yanıt modeli ile bir sağlayıcının hat
...
`buildRequest`, bir `NxcParsedRequest`'i bir yukarı akış HTTP isteğine indirir;
...
- **Satır içi görsel çıktısı:** model açık görsel yetenekli sohbet
  ...
  edilmez. Döndürülen `inlineData` parçaları yapılandırılmış NexCode
  `artifacts/` dizini altında somutlaştırılır ve kimliği doğrulanmış donuk
  `/v1/nexcode/artifacts/<id>` rotasına markdown görsel bağlantıları olarak
...
Sıradan bir istemci aracı mevcut olduğunda nexcode yukarı akış isteğine özel
...
Kiro modelleri şu anda öykünülmüş akıl yürütme kullanır: nexcode yerel çaba
