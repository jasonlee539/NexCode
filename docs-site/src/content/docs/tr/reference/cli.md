---
title: CLI Referansı
description: Komut dağıtımı, çıkış kodları ve her nxc komut ailesine bağlantılar.
---

nexcode CLI'sı `nxc`'tir. `setup`/`init`, `restore`/`eject` ve
...
Üst düzey kullanım için `nxc help` (veya `nxc --help` / `nxc -h`) çalıştırın.
Yardım tablosunda kayıtlı bir komut için `nxc help <komut>`, `nxc <komut>
--help` veya `nxc <komut> -h` çalıştırın. Yardım ve sürüm komutları salt
okunurdur: Codex veya nexcode durumunu başlatmaz, durdurmaz, kurmaz, kaldırmaz
...
görüntüler için `--json` ve akışlı bir istek günlüğü akışı için `nxc observe
...
olmayan bir çıkış yapar. `nxc health` özellikle yalnızca proxy sağlıklı
...
`nxc --version`, `nxc -v` ve `nxc version` betik dostu tek bir sürüm satırı
yazdırır ve çıkar.
