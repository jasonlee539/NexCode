...
Ajan ayarları hangi Codex işbirliği yüzeyinin tanıtılacağını ve nexcode'in
...
| `multiAgentGuidanceEnabled?` | `boolean` | `true` | Yalnızca nexcode kaynaklı v1/v2 geliştirici rehberliğini denetler; yerel ajan varsayılanlarını, araçları, yönlendirmeyi, kadroları veya çaba sınırlarını değiştirmez. |
...
Arayüzü kontrol paneli veya `nxc v2 status|on|off|mode <v1|default|v2>|threads
...
bir `PUT /api/v2` alanıdır; `nxc v2 threads <n>`, v2 etkinleştirildikten sonra
...
`multiAgentModeHintText` ve `nxc v2 mode-hint`), Codex'in
`$CODEX_HOME/config.toml` dosyasında
`features.multi_agent_v2.multi_agent_mode_hint_text` yazar. CLI `nxc v2
...
yüzeyini (`multiAgentMode: "v2"`, `nxc v2 mode v2` ile eşdeğer) gerektirir; tek
başına `nxc v2 on` bu kontrol paneli geçidini karşılamaz.
...
Rol başına geri dönüş zincirleri nexcode yapılandırmasında yer almalıdır.
...
için hala okunur, ancak `nxc doctor` bunu bayraklar.

nexcode devre dışı bırakılmış, yönlendirilemez, sağlıksız, soğumada olan veya
...
okunamayan bir Fernet yükü içerdiğinde, nexcode iletme modu kimlik
...
görevini zorunlu bir fonksiyon çağrısı aracılığıyla döndürür; nexcode daha
...
- `x-nexcode-api-key`, `x-api-key`, genel API kimlik bilgileri veya bir proxy
...
- kurtarma isteği yalnızca `authorization`, eşleşen `chatgpt-account-id`,
  ...
  iletir; nexcode `content-type` ve `accept`'i kendisi ayarlar ve başka hiçbir
...
hiçbir basamak uymuyorsa nexcode çabayı kaldırır ve sağlayıcı varsayılanının
