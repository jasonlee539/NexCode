...
| `combos?` | `Record<string, NxcComboConfig>` | `{}` | Sıralı sağlayıcı/model hedeflerinden oluşturulmuş sanal `combo/<id>` modelleri. |
| `routingProfiles?` | `Record<string, NxcRoutingProfileConfig>` | `{}` | Kesin yetenek gereksinimlerini ve belirleyici puanlamayı kullanarak açık bir aday izin listesi arasından seçim yapan sanal `policy/<id>` modelleri. |

## Model çözümleme sırası

nexcode istenen modeli şu sırayla çözer:
...
gözlemlenen hesap kapsamlı kimlikler, henüz nexcode'in statik kümesinin bir
...
bir katalog yenilemesinden önce kalıcı hale getirilir; bir `nxc sync` uyarısı
...
CLI: `nxc route policy list [--json]`, `nxc route policy show <id> [--json]` ve
`nxc route policy dry-run <id> [--model-context <tokens>] [--tools] [--image]
...
Bir kombo listelenemediğinde bile doğrudan yönlendirilebilir kalır. `nxc sync`,
...
CLI: `nxc logs explain <request-id>`, `nxc logs rebuild-index`, `nxc logs
index-status`, `nxc route policy list | show | dry-run | evaluate`.
...
otomatik bir yeniden oluşturmayı tetikler; `nxc logs rebuild-index` bunu zorlar.
