...
| `hostname?` | `string` | `"127.0.0.1"` | Bağlama adresi. Geri döngü olmayan bağlamalar `NEXCODE_API_AUTH_TOKEN` gerektirir. |
...
| `emptyCompletionRetry?` | `boolean` | `false` | Metin veya araç çağrısı içermeyen bir Responses tamamlamasını aynı istekle bir kez yeniden denemeyi açıkça etkinleştirir. Yeniden deneme ücretlendirilebilir. `NXC_EMPTY_COMPLETION_RETRY=0`, yapılandırmayı değiştirmeden devre dışı bırakır; combo ve routed-compaction turları hariçtir. |
...
| `apiKeys?` | `NxcApiKey[]` | `[]` | Geri döngü olmayan bağlamalarda yönetim ve veri düzlemi kimlik doğrulaması tarafından kabul edilen oluşturulmuş `nxc_…` kimlik bilgileri. Kontrol paneli tarafından yönetilir. |
...
| `codexAutoStart?` | `boolean` | `true` | Codex dolgusunun Codex'i başlatmadan önce `nxc ensure` çalıştırmasına izin verin. False, ensure'ı bir işlem yapmayan (no-op) hale getirir. |
| `codexShimAutoRestore?` | `boolean` | `true` | Tamamlanan harici bir Codex güncellemesi değiştirdikten sonra kurulu bir dolguyu geri yükleyin. Ortam vazgeçmesi: `NEXCODE_CODEX_SHIM_AUTO_RESTORE=0`. |
| `syncResumeHistory?` | `boolean` | `true` | Tersine çevrilebilir Codex App geçmişi uyumluluğu. Orijinal meta veriler yedeklenir ve `nxc stop` / `nxc restore` tarafından geri yüklenir. |
...
| `webSearchSidecar?` | `NxcWebSearchSidecarConfig` | kullanılabilir olduğunda açık | Web arama sidecar seçenekleri. |
| `visionSidecar?` | `NxcVisionSidecarConfig` | kullanılabilir olduğunda açık | Görsel açıklama sidecar seçenekleri. |
| `images?` | `NxcImagesConfig` | otomatik OpenAI seçimi | Codex `image_gen` için bağımsız Görseller aktarma seçenekleri. |
...
`nxc recover-history --legacy-openai` çalıştırın.
...
için launchd, systemd veya Görev Zamanlayıcı'nın alması amacıyla `nxc service
...
| Uç nokta | `Authorization: Bearer` | `x-nexcode-api-key` | `x-api-key` |
| --- | --- | --- | --- |
...
olmayan bir bağlamada daha katıdır: proxy kabulü `x-nexcode-api-key`
...
dolgusundan asla geçmez, bu nedenle asla `NEXCODE_API_AUTH_TOKEN`'ı devralmaz
...
`nxc sync` daha sonra yönetilen Codex sağlayıcı bloğuna `base_url =
...
`http://localhost:20100/v1` çalışır. Bu temel URL'yi istemcide ayarlayın; `nxc`
...
girdi sunuyorsa NexCode kayıtlı yönlendirme URI'sini tutar ve yine de
...
kodunu NexCode'e yapıştırın. Bekleyen akış durumu ve PKCE doğrulamasını korur.
...
Bu ayarlar `/v1/messages`, `/v1/messages/count_tokens`, `nxc claude` başlatıcısı
...
| `claudeCode.subagentEffort?` | `"low" \| "medium" \| "high" \| "xhigh" \| "max"` | inherit | Oluşturulan `~/.claude/agents/nxc-*.md` dosyasına yazılan çaba; Codex rehberliğinden ve proxy sınırlarından ayrıdır. Yeniden oluşturmak için `nxc claude` üzerinden yeniden başlatın. |
...
Anthropic OAuth sidecar'ları nexcode'in mevcut Claude Code OAuth parmak izini
