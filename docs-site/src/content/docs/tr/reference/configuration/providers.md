...
Bir sağlayıcı, nexcode'e bir modelin nerede yaşadığını, hangi hat adaptörünü
...
| --- | --- | --- | --- |
| `providers` | `Record<string, NxcProviderConfig>` | — | Sağlayıcı adından sağlayıcı yapılandırmasına eşleme haritası. |
...
| `tokenGuardian?` | `NxcTokenGuardianConfig` | kapalı | İsteğe bağlı proaktif OAuth yenileme ve Codex hesabı ısınma politikası. |

Seçici adları kullanıcı tarafından seçilen genel etiketlerdir; nexcode bunlara
...
nexcode, sevk edilen bir v1 yapılandırmasını geçirmeden önce farklı bir yedeği
...
## Sağlayıcı girdileri (`NxcProviderConfig`)
...
tutabilir. OAuth sağlayıcıları `nxc login` tarafından doldurulan kimlik bilgisi
...
aktarımı kullanır. Giden bir proxy olmadan nexcode ana bilgisayar adını bir
...
ancak proxy son rotayı, DNS yanıtını ve eşi seçer, bu nedenle nexcode bu eşi
...
ve yeniden kimlik doğrulama gerektirir. Bir **429** durumunda nexcode
...
Emin olmadığınızda manuel `nxc account use anthropic <id>` geçişini tercih edin.
:::
...
Yönlendirme `baseUrl`'i attığında nexcode kayıt defteri uç noktasını ve
...
Cursor köprüsü deneyseldir. `nxc login cursor` komutundan sonra
...
Model anahtarları, dış nexcode sağlayıcı öneki olmadan tam yerel OpenRouter
