...
açar ve otomatik olarak yenilenen kimlik bilgilerini `~/.nexcode/` altında
...
`nxc status` / `nxc doctor` yeniden kimlik doğrulama gerektiğini veya bir
...
Doğrula'yı kullanın). Codex havuz hesapları genel bir `nxc login` sağlayıcısı
değildir — bunun yerine kontrol paneli Codex hesap havuzu (Yeniden Kimlik
Doğrula) veya başsız `nxc account reauth` akışı aracılığıyla yeniden kimlik
doğrulaması yapın.
...
geçiş kapalı olduğunda da aktif kalır. NexCode bir hesap değişikliğinden sonra
...
Bir **429** durumunda, nexcode `Retry-After`'a uyar, hesap soğuma süresini
...
Codex havuzu için `nxc account refresh openai [--json]` kullanın. Hesap
...
Codex Desktop girişini hedefler — `nxc account priority openai main last`, onu
...
doğrulamasını çalıştırın. Sağlayıcıya özgü komut şekli için `nxc account --help`
kullanın. Bir Codex hesap girişi kaydedilirse ancak model kataloğu yenilemesi
beklemede kalırsa insan çıktısı yine de başarıyla çıkar ve stderr'e sabit `nxc
...
hesap silme işlemi zaten kaydedilmiştir; insan çıktısı stderr'e genel `nxc sync`
...
### `nxc account main <alt-komut>`

NexCode hesap havuzu yönlendirmesini değiştirmeden adlandırılmış yerel Codex
ana giriş profillerini yönetin:

```text
nxc account main doctor [--json]
nxc account main list [--json]
nxc account main register <etiket> [--json]
nxc account main add <etiket>
nxc account main switch <profil-id-veya-etiket> --yes [--json]
...
sonra çıkan bir NexCode sürecini kapsar. Bir işletim sistemi veya çekirdek
...
kurallı `<gercek CODEX_HOME>/.nexcode-native-main-profiles` dizininde bulunur,
böylece bu Codex ana dizinini paylaşan her NexCode örneği tek bir sahip ve tek
bir kurtarma durumu gözlemler. Düz metin giriş hazırlığı her
`<NEXCODE_HOME>/native-main-profile-staging` dizini altında yalıtılmış olarak
kalır.
...
`auth.json.nxc.<pid>.<sequence>.tmp` çökme kalıntılarını kaldırır. Her aday
...
karşı değil, işbirlikçi NexCode çökmelerine karşı koruma sağlar. Bu kullanıcı
...
Önizleme derlemeleri `<NEXCODE_HOME>/native-main-profiles` kullanmıştır. Bu
düzen asla sessizce içe aktarılmaz. `doctor` eski profil durumunu bildirirse
aynı `CODEX_HOME`'u paylaşan her NexCode proxy'sini durdurun. Ardından eşleşen
...
kurallı dizine yedekleyip taşıyın ya da eski önizleme kümesini kaldırıp `nxc
...
### `nxc models [alt-komut]` · `nxc model <alt-komut>`

`nxc model`, `nxc models`'ın bir takma adıdır. Alt komut olmadan yapılandırılmış
...
yönetim API'si ile konuşur ve proxy'nin çalışmasını gerektirir (`nxc start` veya
kurulu bir servis).
...
```bash
nxc models live --json                                  # Codex'in şu anda gerçekte görebildikleri
nxc models disable anthropic/claude-haiku-4             # bir yönlendirilen modeli gizle
nxc models enable gpt-5.6-sol                           # eğik çizgi yok, bu yüzden yerel olarak kabul edilir
nxc models provider zenmux off                          # gürültülü bir sağlayıcıyı toptan gizle
nxc models selected anthropic --set claude-opus-5,claude-fable-5
nxc models selected anthropic --clear                   # izin listesini tekrar kaldır
nxc models add deepseek deepseek-v4 --display-name 'DeepSeek V4' --context-window 128000 --modalities text,image
nxc models list-custom --json                           # düzenleme/kaldırma için custom-id'yi oku
