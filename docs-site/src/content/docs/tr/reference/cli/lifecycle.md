...
Bu komutlar, yerel nexcode proxy'sini ve Codex entegrasyonunu kurar,
...
model ve proxy portu sorar; `~/.nexcode/config.json` dosyasını kaydeder;
...
nexcode başka bir kullanılabilir port seçer ve kaydeder. PID/çalışma zamanı
...
— yönetilen bir servis olarak başlatılmadığı sürece (`NXC_SERVICE=1`) — yerel
Codex'i geri yükler.
...
geri yükleyin. Yönetilen bir arka plan servisi kuruluysa `nxc stop` proxy'yi
...
olur. Sahipliği onayladıktan sonra bağımsız bir proxy için `nxc stop` ardından
`nxc start` kullanın. Servis tarafından yönetilen bir proxy için denetimin geri
yüklenmesi amacıyla `nxc stop` ardından `nxc service start` kullanın.
...
nexcode yerel yapılandırmasını kaldırın. `remove`, `uninstall`'ın bir takma
...
olmadığını, `NEXCODE_HOME` altında yenileme tek uçuş/kilit dosyalarının
...
Sağlayıcı keşfinden veya katalog/önbellek değiştirmeden önce `nxc sync`,
...
`nxc restore back`, yönlendirmeyi yeniden etkinleştirmeden önce aynı yazmasız ön
kontrolü kullanır.

Uzun ömürlü Codex `app-server` süreçleri hala çalışıyorsa `nxc sync`,
`nexcode-catalog.json` / `models_cache.json` güncellenmiş olsa bile önceki
...
Codex'in yerel model seçici önbelleğini geçersiz kılın, böylece aktif nexcode
kataloğundan yeniden oluşturulur. `nxc sync` ile aynı eski `app-server` uyarısı
...
nexcode'i oturum açmada otomatik başlayan ve çökmede otomatik yeniden başlayan
...
çalıştırmaları `NXC_SERVICE=1` ayarlar, böylece bir yeniden başlatma Codex
...
hizmet vermek için `nxc start` kullanın.

`nxc service status` ham yönetici çıktısı yerine aynı üç durumu bildirir:
...
Windows'ta `nxc service status`, Görev Zamanlayıcı kaydını kimliği doğrulanmış
NexCode proxy erişilebilirliğinden ayrı olarak bildirir. Yerelleştirilmiş
...
okunamıyorsa geri dönüş `/create /tn nexcode-proxy /xml <bos-olmayan-yol> /f`
...
yükseltilmiş bir PowerShell penceresinde `nxc service install`'ı yeniden
çalıştırın.

NexCode zamanlayıcı görevinin kesinlikle bulunmadığı yeni bir kurulum için UAC
...
NexCode eski dinleyiciyi durdurur, servis varlıklarını yayınlar ve zamanlanmış
...
Bir kurulum veya onarım uygulanmadan önce NexCode servis başlangıcı atlanırken
...
yerine `nxc service install` kullanın. Yükseltmeler sırasında geçerli doğrulama
korumasından yoksun kurulu bir Unix dolgusu yeniden oluşturulur ve araştırılır.
Kaydedilen başlatıcısı güvenli değilse NexCode güvensiz sarmalayıcıyı kurulu
bırakmak yerine eski dolguyu kaldırır ve orijinal başlatıcıyı geri yükler.

Yalnızca başlatıcı kurulumu Codex isteklerinin NexCode kullanacağını
...
yazdırmaz; otomatik başlatmaya güvenmeden önce bildirilen devri çözün ve `nxc
doctor` çalıştırın.

Tamamlanan harici bir Codex güncellemesi kurulu bir dolgunun üzerine yazarsa
sonraki sıradan `nxc` komutu kararlı yeni başlatıcıyı yedekler ve dağıtımdan
...
başarısız kılmadan uyarır; manuel geri dönüş: `nxc codex-shim install`. Süreç
düzeyinde bir vazgeçme için `codexShimAutoRestore`'u `false` olarak ayarlayın
veya `NEXCODE_CODEX_SHIM_AUTO_RESTORE=0` ayarlayın.
...
Her zaman açık bir arka plan proxy'si için `nxc service` kullanın (önerilir).
Bir arka plan programı olmadan hafif, isteğe bağlı başlatma için `nxc
...
nexcode'i npm'den kendi kendine güncelleyin. Kararlı kurulumlar `@latest`
...
başlatılır, ön plan kurulumu ise sonraki adım olarak `nxc start` yazdırır.
...
akışı](https://github.com/lidge-jun/nexcode/actions/workflows/release.yml)
