...
description: nexcode kontrol düzlemi için kimlik doğrulama, hatalar ve uç nokta referansı.
---

Yönetim API'si, nexcode'in kontrol düzlemidir. `http://localhost:10100`
adresindeki kontrol paneli onun bir istemcisidir; başsız `nxc` sağlayıcı, model,
...
kimlik bilgisi vardır. Başlangıçta nexcode bunu şu sırayla çözer:

1. Ayarlandığında `NEXCODE_ADMIN_AUTH_TOKEN`.
2. Güçlendirilmiş bir gizli dosyada oluşturulan bir `nxc_admin_*` belirteci.
...
```http
X-NexCode-API-Key: <yonetici-belirteci>
```
...
`nxc_session_*` kimlik bilgisi alabilir. Her oturum beş dakika sürer ve tam
...
| --- | --- | --- |
| 401 | `nexcode admin token required` | Yönetici belirteci veya GUI oturumu eksik, geçersiz, süresi dolmuş, kaynak uyumsuz veya CSRF kanıtı eksik |
...
`nxc` komutlarını kullanın: aynı canlı API'yi çağırırlar ve proxy erişilemez
