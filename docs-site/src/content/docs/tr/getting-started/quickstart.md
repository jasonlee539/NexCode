...
description: İlk sağlayıcınızı yapılandırın ve OpenAI Codex'i üç komutta nexcode üzerinden yönlendirin.
---
...
`nxc init` adım adım size rehberlik eder:
...
5. **Codex'e enjekte edilsin mi?** — normal bir geri döngü kurulumunda
   nexcode, `$CODEX_HOME/config.toml` (varsayılan `~/.codex/config.toml`)
...
6. **Otomatik başlatma dolgusu (shim) kurulsun mu?** — etkinleştirildiğinde,
   `codex` başlatıldığında önce `nxc ensure` çalışır.

Sonuç `$NEXCODE_HOME/config.json` (varsayılan `~/.nexcode/config.json`)
...
```bash
nxc start            # varsayılan port 10100
nxc start --port 8080
```

Başlatıldığında nexcode:

- PID'sini `~/.nexcode/nxc.pid` dosyasına yazar (ve iki kez başlamayı
  reddeder),
...
İstenen port meşgulse `nxc start` boş bir port seçer, bunu `runtime-port.json`
...
```bash
nxc status
nxc gui       # kontrol panelini canlı port üzerinde açın
```

## 3. Codex'i kullanın

Codex artık nexcode ile şeffaf bir şekilde konuşur:
...
için `nxc gui`'yi açın. Kontrol paneli ayrıca tercih edilen bir alt ajan
...
```bash
nxc login xai          # veya: anthropic, kimi, kiro, google-antigravity, cursor
nxc logout xai
```
...
```bash
nxc stop          # proxy'yi durdurun ve yerel Codex'i geri yükleyin
nxc restore       # durdurmadan yerel Codex'i geri yükleyin (takma ad: nxc eject)
nxc restore back  # Codex'i hala çalışan proxy üzerinden tekrar yönlendirin
```
