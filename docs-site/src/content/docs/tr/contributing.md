...
description: nexcode geliştirme — kurulum, düzen, kurallar ve yeni bir sağlayıcı veya adaptör ekleme.
---
...
Genel dokümantasyon GitHub Pages üzerinde <https://nexcode.me/> adresinde
...
- **Çapraz platform CI** (`.github/workflows/ci.yml`), çalışma zamanı, testler,
  ...
  yardımcısı duman testi, GUI derlemesi ve `nxc help` adımlarını kapsar. İkinci
...
[`MAINTAINERS.md`](https://github.com/lidge-jun/nexcode/blob/main/MAINTAINERS.md)
...
`src/providers/derive.ts` bu girdiyi `nxc init`, `nxc provider`, kontrol paneli
...
Bir kayıt defteri girdisi sürdürülen bir taahhüttür: nexcode, kullanıcının API
...
de ulaşabilirken, nexcode arkasında duramayacağı bir önayarın tanıtımını
...
nexcode büyük partiler yerine küçük, doğrulanabilir commit'leri tercih eder.
