---
title: Contributing
description: Develop nexcode — setup, layout, conventions, and how to add a provider or adapter.
---
...
The public docs publish to GitHub Pages at <https://nexcode.me/>. The
...
- **Cross-platform CI** (`.github/workflows/ci.yml`) runs on pull requests and `main` pushes that
  ...
  build, and `nxc help`. A second three-OS lane proves npm global install works without a separately
...
[`MAINTAINERS.md`](https://github.com/lidge-jun/nexcode/blob/main/MAINTAINERS.md). GitHub review
...
`src/providers/derive.ts` feeds that entry into `nxc init`, `nxc provider`, dashboard presets,
...
A registry entry is a maintained promise: nexcode ships the destination that a user's API key is
...
reach the service through the custom OpenAI-compatible flow, while nexcode avoids advertising a
...
the affected surface. nexcode favors small, verifiable commits over large batches.
