...
description: Установите и используйте nexcode из агентного или сценарного терминала, не переходя границы пользовательского согласия.
---
...
Установите опубликованный пакет и убедитесь, что `nxc` доступен в `PATH`:
...
Запустите `nxc init` в интерактивном терминале. Если `nxc start` уже занимает передний план,
...
Мастер записывает `$NEXCODE_HOME/config.json` (обычно `~/.nexcode/config.json`). Он также может
вставить адрес прокси в `config.toml` Codex и установить необязательный shim автозапуска Codex.
`nxc init` никогда не запускает прокси. Для полностью неинтерактивной настройки вместо мастера
настройте провайдеров через `nxc provider add`, как показано ниже.
...
`nxc status` сообщает состояние прокси и службы. `nxc doctor` диагностирует локальную среду,
сеть, рантайм Codex и проблемы со здоровьем аккаунтов. `nxc health` завершаетcя с кодом `0`,
когда прокси исправен, и с `1` в противном случае; `--json` возвращает структурированный вывод.

Команды, работающие через management API, например `nxc combo set`, обращаются к живому прокси.
...
`nxc provider add` записывает локальную конфигурацию. Добавьте `--sync`, если живой прокси уже
работает и вы хотите сразу синхронизировать модели в Codex; иначе позже выполните `nxc sync`.
...
`0.0.0.0`, требуется `NEXCODE_API_AUTH_TOKEN`; без него прокси откажется запускаться. Задайте
эту переменную перед `nxc start` или перед `nxc service install`, чтобы служба тоже её получила:
...
Прежде чем открывать nexcode за пределы локальной машины, прочитайте правила удалённого доступа
