...
description: Разработка nexcode — настройка окружения, структура, конвенции и добавление провайдера или адаптера.
---
...
Публичная документация публикуется на GitHub Pages по адресу <https://nexcode.me/ru/>.
...
- **Cross-platform CI** (`.github/workflows/ci.yml`) запускается на pull request и push в `main`,
  ...
  release-helper, сборка GUI и `nxc help`. Отдельная линия на тех же трёх ОС подтверждает, что
...
`src/providers/derive.ts` передаёт эту запись в `nxc init`, `nxc provider`, пресеты дашборда, вход
...
широкие проверки, соответствующие затронутой области. nexcode предпочитает небольшие проверяемые
