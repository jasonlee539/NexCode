---
title: Провайдеры
description: Все способы, которыми nexcode аутентифицируется и общается с LLM-провайдером — OAuth, API-ключ, форвард ChatGPT и локальные серверы.
---
...
`~/.nexcode/config.json` в секции `providers`.
...
Исходная конфигурация один раз сохраняется в `~/.nexcode/config.json.pre-openai-tiers-v2.bak`;
...
экспериментальный неофициальный мост device flow. nexcode хранит их учётные данные в
`~/.nexcode/auth.json` и обновляет их автоматически. CLI входа также принимает `chatgpt`: эта
...
nxc login nous         # Nous Portal (device grant; модели free + paid)
nxc login kiro         # импорт учётных данных kiro-cli (с фолбэком на токен)
nxc login google-antigravity
nxc login cursor       # отдельный PKCE-вход Cursor
nxc login command-code # браузерный OAuth Command Code (или импорт ~/.commandcode/auth.json)
nxc login github-copilot  # device flow GitHub → токен Copilot (Copilot Pro/Business)
nxc login chatgpt      # отдельный OAuth-вход ChatGPT
nxc logout <provider>
```
...
После терминального сбоя обновления Nous выполните `nxc login nous`, чтобы пройти повторную аутентификацию.

Для канонических пресетов Kimi Coding Plan (вход через аккаунт `kimi` и API-ключ `kimi-code`)
nexcode передаёт в запрос Chat Completions только стабильный `prompt_cache_key`, предоставленный
...
Если включённый провайдер отклоняет поле, nexcode не удаляет его для повторной попытки и не
...
Для входа Kiro требуется Kiro CLI: в Unix установите его командой `curl -fsSL https://cli.kiro.dev/install | bash`, в Windows PowerShell — `irm 'https://cli.kiro.dev/install.ps1' | iex`, затем сначала выполните `kiro-cli login`. Если сессии `kiro-cli` нет, `nxc login kiro` использует вставленный токен доступа или переменную окружения `KIRO_ACCESS_TOKEN`.

Обычный импорт `nxc login kiro` открывает базу SQLite CLI только для чтения и не изменяет базу, WAL или SHM.
...
Импортированные учётные данные сохраняются в `~/.nexcode/auth.json`. Откат **Добавить аккаунт** — отдельная операция: при восстановлении предыдущего снимка она заменяет базу и удаляет текущие sidecar-файлы WAL, SHM и journal.
...
nexcode поставляется с 79 встроенными пресетами: 67 на основе ключей, восемь OAuth, три локальных и
...
намеренный: первая часть выбирает провайдера nexcode, а полный slug `cline-pass/kimi-k3`
...
принимают на входе шлюза `low`, `medium`, `high`, `xhigh` и `max`. nexcode сохраняет запрошенный tier без изменения;
...
`opencode-free`). Когда Zen опускает `Retry-After` на таком 429, nexcode добавляет пояснение
...
256 KiB и 256 исходными строками. `nxc login command-code` поддерживает вход через OAuth в браузере
...
**Квота Command Code.** Дашборд и `nxc account refresh` опрашивают окна `/alpha/billing/credits` (5 часов и неделя) на каноническом хосте `https://api.commandcode.ai`. OAuth-пресет (`command-code`) использует сохранённый bearer аккаунта; пресет Provider-API ключа (`commandcode`) — активный настроенный ключ. Пользовательски изменённый похожий base URL не опрашивается. Если Command Code также сообщает расход за период, оставшиеся monthly / purchased / free credits показываются как USD-окно.
...
дашборде и в `nxc account refresh <provider>`. Имя провайдера может быть любым. Единицы токенов
...
Используйте `nxc account list`, `nxc account current` и `nxc account use`, чтобы просматривать и
...
[Справочник CLI](/ru/reference/cli/#nxc-account-subcommand).

### Превью-маршруты GPT-5.6

GPT-5.6 Sol/Terra/Luna заранее внесены в резервные списки провайдеров, чтобы `nxc sync` сохранял
...
Провайдер попадает в список, когда у nexcode есть подходящий wire-адаптер, а **не** в зависимости
...
**GitHub Copilot** — это OAuth-провайдер (`nxc login github-copilot`), который обменивает вход
...
отклоняет `/chat/completions` для агентного трафика, поэтому nexcode по умолчанию
...
`nxc init` и в селекторе Add Provider дашборда как экспериментальная запись локальной конфигурации
с метаданными статического резервного каталога моделей Cursor. Когда настроен токен доступа Cursor,
nexcode использует живой транспорт HTTP/2 Cursor. Его встроенный резервный список включает
...
объекте `providers.cursor` файла `~/.nexcode/config.json` только для доверенных локальных
...
исполнителя nexcode возвращает типизированные результаты «нет исполнителя», а не блокирует запрос
...
[ollama.com/settings/keys](https://ollama.com/settings/keys). nexcode классифицирует её облачную
...
Направьте nexcode на локальный OpenAI-совместимый сервер — обычно с пустым ключом:
...
**Custom** в дашборде или `custom` в `nxc init` и введите базовый URL. Все поля провайдера
