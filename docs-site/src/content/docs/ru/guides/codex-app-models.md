...
description: Как модели nexcode появляются в Codex App, Codex CLI и Codex TUI через общий каталог Codex.
---

nexcode не патчит Codex App. Он записывает ту же конфигурацию Codex и тот же каталог моделей,
...
nexcode добавляет для сопоставленных аккаунтов отдельные строки
...
`~/.nexcode/config.json.pre-openai-tiers-v2.bak`; вернуть его можно так:
...
`nxc init`, `nxc start` и `nxc sync` подключают общий конфиг и каталог Codex к прокси; подробности
...
Picker моделей Codex ожидает записи каталога в формате Codex. nexcode строит маршрутизируемые
...
базовые инструкции. После этого nexcode убирает нативные возможности, которые данный маршрут
...
`gpt-5.3-codex-spark` и GPT-5.6 Sol/Terra/Luna. Для семейства GPT-5.5/5.4 nexcode сохраняет
...
Но каталог моделей и id tier'а во время выполнения используют `priority`. nexcode сохраняет это
...
дашборде. nexcode назначает им низкие приоритеты каталога в выбранном порядке; при включённых
...
чего нет в этом списке, ограниченном нативными моделями, ещё до отрисовки. nexcode не может
...
Каждый раз, когда меняются видимость, priority или metadata каталога, nexcode переписывает
