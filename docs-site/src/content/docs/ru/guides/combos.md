...
provider/model. Клиент запрашивает `combo/<id>`, nexcode выбирает цель, переписывает запрос на
...
`anthropic/claude-opus-4-8`. Если эта попытка завершится retryable-сбоем, nexcode сможет
...
Идентификатор combo в `nxc combo set <id>` должен начинаться с буквы или цифры. Дальше он может
...
содержит корректный `Retry-After`, nexcode использует его. Поддерживаются и числовые секунды, и
...
Если в запросе нет объекта `reasoning`, nexcode создаёт его. Если `reasoning` есть, но в нём нет
...
Если возможности цели неизвестны или не включают настроенный effort, nexcode опускает значение
...
([issue #92](https://github.com/lidge-jun/nexcode/issues/92)). Нативный родитель может отправить
...
том числе после retryable-сбоя. Если в combo нет цели, способной расшифровать задачу, nexcode
...
те же подкоманды доступны через `nxc route combo`.
...
Id combo неизвестен. Ответ — HTTP 404 с типом `invalid_request_error`. Запустите `nxc combo list`,
проверьте написание и регистр, а также убедитесь, что management-команда записывала в тот же
запущенный экземпляр nexcode, который принимает запросы к моделям.
