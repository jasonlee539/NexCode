---
title: Claude Code
description: Используйте любую маршрутизируемую модель из Claude Code — nexcode обслуживает Anthropic Messages API и обнаружение моделей шлюза на одном и том же порту.
---

nexcode обслуживает `POST /v1/messages` (а также `count_tokens`) наряду с `/v1/responses`, поэтому
...
`nxc claude` убеждается, что прокси запущен, а затем запускает Claude Code с уже настроенным окружением:
...
Переменные, которые вы экспортируете сами, всегда имеют приоритет. Дополнительные аргументы передаются как есть: `nxc claude -p "hello"`.

## Интеграция с системным окружением (macOS)

Когда `claudeCode.systemEnv` установлен в `true` (по умолчанию: **выключено**), `nxc start`
...
обычные команды `claude` через прокси без обёртки `nxc claude`. Уже открытые оболочки не
затрагиваются — их нужно открыть заново.

`nxc stop` и остановка прокси **снимают внедрённые ключи** (прежние значения не восстанавливаются —
удаляются только ключи, внедрённые nexcode). Прокси также записывает `~/.nexcode/claude-env.sh`;
`nxc start` устанавливает source-хук в `.zshrc` только когда исполняемый Claude Code CLI найден в
`PATH`. При запуске и выполнении `nxc ensure` хук, принадлежащий NexCode, удаляется, если файл Claude
...
Функция доступна только на macOS; на других платформах используйте `nxc claude`.
...
`accept-encoding`, `x-nexcode-api-key` и `origin` всегда удаляются перед пересылкой. На
non-loopback bind нативный passthrough также требует валидный proxy credential именно в
`x-nexcode-api-key`; после этого `Authorization` и `x-api-key` принадлежат только Anthropic.
...
что предупреждение «claude.ai connectors are disabled» с `nxc claude` больше не появляется.
...
начинающиеся с `claude` или `anthropic`, nexcode публикует маршрутизируемые модели как
стабильные обратимые алиасы:

| Интерфейс | Формат | Пример |
| --- | --- | --- |
| Claude Code CLI | `claude-nxc-<provider>--<model>` (plain) или `claude-nxc2-…` (escaped) | `claude-nxc-native--gpt-5.6-sol` |
...
используйте `/model <id>` внутри этого диалога. NexCode не видит состояние селектора и
...
`native`. Обычные id моделей (без `/` и `~`) остаются с префиксом v1 `claude-nxc-…`. Id с `/`
или `~` выпускаются с префиксом v2 `claude-nxc2-…` и экранированием (`/` → `~s`, `~` → `~t`),
например `openrouter/anthropic/claude-opus-4-8` →
`claude-nxc2-openrouter--anthropic~sclaude-opus-4-8`. Алиасы v1 декодируются литерально (исторические
...
`effectiveModelEnv` вычисляет шесть слотов, внедряемых `nxc claude`, системным окружением и
...
Если отсутствуют и `tierModels.haiku`, и `smallFastModel`, NexCode оставляет обе переменные вспомогательной модели незаданными. Затем Claude Code выбирает нативную вспомогательную модель (сейчас Sonnet), что может привести к расходам у нативного провайдера.

## Агенты из ростера (injectAgents)

Запуск/ensure прокси, `nxc claude` и сохранение связанных настроек в панели синхронизируют ваш ростер избранных подагентов
(вкладка Subagents, до 5 моделей) плюс `nxc-self` в `~/.claude/agents/nxc-*.md`.

- **`nxc-self`** закрепляет модель по умолчанию из селектора `/model` (с откатом на
  `claudeCode.model`); если нет ни того, ни другого, файл не создаётся. Наследование модели
  НЕ используется.
- В теле каждого агента есть директива `<!-- nxc-route: <model> -->` — по ней прокси закрепляет
...
- Перезаписываются или удаляются только проверенные по маркеру файлы `nxc-*.md`, содержащие
  `generated-by: nexcode`; ваши собственные агенты никогда не затрагиваются.
...
Диспетчеризация: `subagent_type: "nxc-gpt-5-6-sol"`. Цели с поддержкой 1M автоматически получают `[1m]`.
...
этом пакете не обучались, поэтому по умолчанию nexcode подменяет содержимое скилла короткой
...
изображений. nexcode закрывает эти пробелы до того, как ответит основная модель:
...
Явно указанный `backend` всегда имеет приоритет. Если он не задан, nexcode выбирает
...
nexcode не заимствует втихую учётные данные ChatGPT и не переключает бэкенды. Бэкенд OpenAI
...
Захват входящих запросов управляется командой `nxc debug claude on|off|status|reset`, переменной
`NXC_CLAUDE_DEBUG=1` или запросом `PUT /api/debug {"claude": true}`.
...
- Быстрый старт (`nxc claude`) и блок ручной настройки окружения
...
сборке поиск выполнялся, но Claude Code всё равно насчитывал ноль, обновите nexcode.
...
`ANTHROPIC_AUTH_TOKEN`. `nxc claude` намеренно НЕ устанавливает `ANTHROPIC_API_KEY`; если вы
экспортировали его сами, снимите переменную. `nxc claude` внедряет `ANTHROPIC_BASE_URL`,
...
`CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY=1` (с `nxc claude` — автоматически). Запустите
`nxc claude`, чтобы обновить кеш моделей шлюза в `~/.claude/cache/gateway-models.json`.
...
для маршрутизируемых моделей nexcode по умолчанию подставляет заглушку
(`blockedSkills: ["claude-api"]`).

**Подагент отправляется не в ту модель** — агенты из ростера (`nxc-*`) используют директивы
`<!-- nxc-route: ... -->`, а не аргумент `model` инструмента Agent. Убедитесь, что директива
