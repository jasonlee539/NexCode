---
title: Веб-дашборд
description: GUI nexcode для состояния прокси, провайдеров, моделей, инструкций делегирования, пулов аутентификации, использования и логов.
---

nexcode включает локальный веб-дашборд (Vite/React-приложение в каталоге `gui/`), который раздаёт
...
| **Sub-agent delegation** | Выбор нативной/маршрутизируемой модели и необязательного уровня рассуждений, общих для руководства NexCode по делегированию и опциональных нативных значений подагентов Codex по умолчанию. Это не маршрутизатор отдельных порождений; см. ниже. |
...
| **Автозапуск Codex** | Разрешает уже установленному launcher shim Codex выполнять `nxc ensure`. Переключатель не устанавливает shim или фоновую службу. |
...
`injectionEffort`. Выбранные значения используются в добавляемом NexCode руководстве по
...
когда активной маршрутизацией Codex управляет NexCode. Внешняя пользовательская конфигурация
...
:::caution
Эти два переключателя независимы. Отключение руководства NexCode по делегированию не отключает
...
- На карточке каждого аккаунта есть элемент управления **Порядок выбора** (Первым, Раньше,
  ...
  `nxc account priority` вне этих пяти пресетов, остаётся видимым и выбираемым на карточке.
...
- Добавление/удаление аккаунта и picker-настройка сохраняются до catalog refresh. Если refresh не
  ...
  выполните `nxc sync`, чтобы повторить refresh.
