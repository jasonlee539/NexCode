...
изображений**. nexcode восполняет эти возможности двумя сайдкарами. Каждый может работать через
...
2. Запускает маршрутизируемую модель в небольшом **агентном цикле**. Когда она вызывает
   `web_search`, nexcode использует выбранный бэкенд сайдкара: OpenAI выполняет hosted
...
nexcode полностью буферизует семантические события внутри, прежде чем решить, искать дальше или
...
изображение, nexcode описывает каждое изображение **до** основного вызова и заменяет его текстом.
