...
description: Как nexcode внедряется в Codex, синхронизирует каталог моделей, устанавливает shim'ы и чисто восстанавливает исходное состояние.
---

nexcode заставляет Codex маршрутизировать запросы через прокси, редактируя две сущности,
...
`nxc init`, `nxc start` и `nxc sync` вызывают injector. На loopback-привязке по умолчанию он
сохраняет встроенный id провайдера Codex `openai` и направляет его на nexcode:
...
`base_url` указывает на nexcode, прокси ретранслирует эти вызовы в upstream OpenAI.
...
- **Fallback Google Antigravity (CCA):** если не настроен ни один OpenAI forward-candidate и ни
  ...
  `nxc login google-antigravity`; OAuth-токен отправляется только на закреплённый registry-host
...
API-ключу nexcode понижает приватное пространство имён Codex `image_gen` до безопасного для
upstream alias `image_gen__<inner-name>` (например, `image_gen__imagegen`). Когда этот рабочий
alias заменяет клиентское объявление, nexcode удаляет дублирующее hosted-объявление
...
Когда маршрутизацией владеет NexCode, оба режима пишут `$CODEX_HOME/nexcode.config.toml` как
...
Codex CLI, TUI, App и SDK читают один и тот же Codex home. nexcode определяет этот каталог из
...
В WSL, если `CODEX_HOME` не задан и Linux-файл `~/.codex/config.toml` отсутствует, nexcode
...
`nxc status` и `nxc doctor` предупреждают именно об этом рассогласовании и печатают замаскированные
...
в согласии с нативным Codex. nexcode также обслуживает `/v1/responses` по WebSocket.
...
перемещает треды, помеченные более старыми сборками nexcode, обратно на `openai`. В режиме
выделенного не-loopback-провайдера история во время работы зеркалируется под провайдером
`nexcode` и при выходе восстанавливает сохранённые метаданные. Задайте
`syncResumeHistory: false`, если не хотите трогать историю.

## Синхронизация каталога моделей

Codex показывает модели из каталога на диске (`$CODEX_HOME/nexcode-catalog.json` по
умолчанию). При старте и при `nxc sync` nexcode:

1. **Создаёт резервную копию** исходного каталога один раз в `~/.nexcode/catalog-backup.json`
...
и вложенные MCP-инструменты, включая Browser и Computer Use, а nexcode маршрутизирует только
обычный function call модели. Выполнение инструментов, разрешения и подтверждения остаются в
Codex; nexcode не реализует второй executor для браузера или управления рабочим столом.

Для key-auth Responses provider'ов, которые не принимают custom-tool grammar `exec` от Codex,
nexcode кодирует объявление и историю как function tool для upstream, а затем восстанавливает
...
После того как `nxc sync` изменит эти metadata, перезапустите Codex App и откройте новую задачу.
...
Ответ — это сырой документ `nexcode-catalog.json` (без credential'ов провайдеров). Если
доступен заголовок `x-nexcode-codex-version`, он сообщает версию рантайма Codex на сервере,
...
Display name — это **только отображение, и оно устойчиво к перегенерации**. Каждый `nxc sync` и
...
предыдущий каталог, а следующий успешный `nxc sync` снова применит настроенное имя. Настоящие
...
Если `config.toml` уже выбирает провайдера, отличного от `openai` или `nexcode`, NexCode
...
использовать NexCode позади уже существующего provider manager'а, направьте этот провайдер на
...
`x-nexcode-api-key` из `NEXCODE_API_AUTH_TOKEN`, то есть ровно так, как в форме
не-loopback-провайдера выше. Чтобы снова дать NexCode самому внедрить routing, сначала верните
Codex на встроенный провайдер `openai` и удалите любой user-owned root `openai_base_url`, после
чего снова выполните `nxc start`.
...
3. **`liveModels: false` и пустой `models`** — если живое обнаружение выключено, а `models` пуст
   или отсутствует, nexcode не показывает ни одной маршрутизируемой модели этого провайдера.
...
5. **Кэш и `nxc sync`** — живые каталоги кэшируются примерно на пять минут (`modelCacheTtlMs`,
   по умолчанию `300000`). Выполните `nxc sync`, чтобы принудительно обновить список и немедленно
   переписать каталог.
6. **Запущенный Codex `app-server`** — переписать каталог на диске недостаточно, если
   ...
   `nxc sync` и `nxc sync-cache` предупреждают, когда находят такие процессы. Перезапустите их
   через `nxc sync --restart-codex` (или остановите подходящие процессы `app-server` вручную), а
   затем дайте Codex создать их заново.

:::caution[Другие локальные writer'ы]
Записи каталога (`nexcode-catalog.json`, `config.toml`) атомарны **только внутри** nexcode, то
есть защищают лишь от полузаписанных файлов, когда гоняются два writer'а самого nexcode. Это
**не** мешает другому локальному процессу, file watcher'у или sync-agent'у переписать видимость или
порядок каталога после того, как nexcode уже записал свой вариант. У Codex есть отдельный
`models_cache.json`, и он может обновить его независимо, меняя видимый список без перезаписи
`nexcode-catalog.json`. Если модели неожиданно «перещёлкиваются», пока прокси работает,
остановите или перенастройте конкурирующих writer'ов, а затем выполните `nxc sync` — это риск
внешнего writer'а, а не подтверждённый дефект nexcode.
:::
...
— или Claude Code сообщает о похожем connection failure — прокси nexcode просто не запущен:
...
`nxc status` показывает, запущен ли прокси, и печатает ту же подсказку о перезапуске, если он не
работает; `nxc doctor` сообщает, насколько безопасен перезапуск (покрытие service/shim).
...
Когда аккаунт ChatGPT добавляется в пул аккаунтов Codex, nexcode проверяет его до сохранения
...
nexcode не запирает вас внутри себя. **`nxc stop` — это единственная команда, которая полностью
...
`codex` снова работает так, будто nexcode никогда не существовал:
...
Когда nexcode работает как управляемая [фоновая служба](/reference/cli/#nxc-service), он
устанавливает `NXC_SERVICE=1`, чтобы service-driven restart **не** дёргал конфигурацию Codex —
только явный `nxc stop` / `nxc service stop` восстанавливает нативный Codex.
