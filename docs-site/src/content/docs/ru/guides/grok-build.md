---
title: Grok Build
description: Используйте любую модель, маршрутизируемую nexcode, из CLI xAI Grok Build — пока прокси работает, модели автоматически регистрируются в ~/.grok/config.toml.
---

nexcode отдаёт OpenAI-совместимый `POST /v1/chat/completions` (и `/v1/responses`) на своём
локальном порту, а Grok Build поддерживает custom-модели поверх OpenAI-совместимых серверов.
Начиная с этой интеграции, nexcode автоматически регистрирует весь свой видимый каталог в
Grok Build — вручную редактировать конфигурацию не нужно.

## Авторегистрация

Когда существует `~/.grok`, `nxc start` (а также `nxc ensure` и `nxc restart`) записывает
...
- **Idempotent:** каждый `nxc start` (и `nxc ensure`, когда включён автозапуск) заменяет fenced-блок
  текущим каталогом.
- **Removed on teardown:** `nxc stop`, `nxc eject`, `nxc uninstall` и корректное завершение
  не-service-демона удаляют fenced-блок и побайтно восстанавливают ваш файл. Под service manager
  teardown выполняется через `nxc stop`/`nxc uninstall` (процессы service-mode намеренно
  сохраняют блок между respawn'ами).
- **Conflict-safe:** alias, уже объявленные в ваших `[model.*]`, уважаются (nexcode добавляет
...
placeholder (`nexcode-loopback`) — nexcode игнорирует admission key для loopback-подключений,
так что реальный секрет тут не используется.

**Авторегистрация работает только на loopback.** Когда nexcode привязывается к не-loopback-хосту
...
при каждом `nxc start`/`ensure`/`restart`. Поэтому в этом случае nexcode вообще ничего не
записывает (и удаляет блок, оставшийся от прежней loopback-привязки), а вы настраиваете модели
сами, вне managed-маркеров, где nexcode ничего не сможет затереть. Точный пример таблицы см. в
...
поэтому ходам через nexcode не нужен дополнительный `grok login`. Обычную настройку
...
Если вы управляете `~/.grok/config.toml` сами — либо nexcode привязан не к loopback, —
...
- **`nxc restart` при установленной службе:** работающий прокси сам управляет drain и заменой,
...
- **Время чтения конфигурации:** для наиболее предсказуемого поведения сначала запускайте
  nexcode, а затем `grok`. Grok Build отслеживает `~/.grok/config.toml` и перезагружает его,
  ...
  конфигурации, поэтому nexcode пишет файл атомарно — Grok никогда не увидит полузаписанный
  `config.toml`.
- **Обновления каталога:** fenced-блок отражает каталог на момент внедрения. После добавления
  провайдеров или моделей выполните `nxc ensure` (или перезапустите прокси), чтобы его обновить.
