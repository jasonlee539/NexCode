...
идти параллельно. nexcode управляет тем, какая collaboration surface Codex раскрывает для этих
воркеров, какие модели Codex предлагает для них и как выполняется fallback при сбое модели. Когда
именно основной агент должен делегировать задачу, nexcode не решает.
...
nexcode применяет это как финальный проход и к живому каталогу `/v1/models`, и к каталогу,
...
- `injectionModel` — предпочитаемая модель воркера, которую nexcode упоминает в guidance.
...
guidance-сообщений, которые nexcode пишет сам, на обеих поверхностях. Если выключить его,
...
Встроенное guidance для v2 ограничено 700 символами. Если оно не укладывается, nexcode сначала
...
На v1 nexcode внедряет только upstream-style proactive guidance о делегировании на уровнях
...
nexcode владеет активной маршрутизацией Codex, sync или restart могут записать выбранные
значения как marker-owned поля `[agents] default_subagent_model` и
`default_subagent_reasoning_effort` в TOML Codex. nexcode обновляет или удаляет только те поля,
...
Для порождённого воркера nexcode строит такой порядок приоритета:

1. Запрошенная основная модель.
2. Модельная цепочка из `subagentModelFallbackByModel` в конфигурации nexcode, ключ — запрошенная основная модель.
3. Глобальный список `subagentModelFallback` в конфигурации nexcode.

Модельные цепочки fallback для ролей должны храниться в конфигурации nexcode, а не в
...
nexcode по-прежнему читает устаревшую строку `model_fallback` из TOML для обратной
совместимости, но `nxc doctor` предупреждает о ней, а сам Codex игнорирует затронутую роль.

Дубликаты id моделей удаляются с сохранением первого вхождения. При выборе nexcode пропускает
...
провайдер. Это известное ограничение [#92](https://github.com/lidge-jun/nexcode/issues/92).

nexcode завершаетcя безопасно и не пересылает пустую или нечитаемую задачу:
...
Для collaboration surface и native feature settings используйте `nxc v2`:
...
Для делегирования, ростера, effort cap и fallback settings используйте `nxc agent`:
...
Чтобы очистить nullable-значение у `nxc agent injection`, передайте `-`, либо используйте
...
устаревшее состояние каталога, выполните `nxc sync` и перезапустите нужную поверхность Codex.
...
`ultra` — это верхний client-facing tier, который Codex переводит в `max`; затем nexcode
