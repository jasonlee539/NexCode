...
маршрутизируемую nexcode. Когда мост включён, в разговор добавляется синтетический tool
`video_gen`. Модель вызывает его как обычный function tool; nexcode перехватывает вызов,
отправляет job на генерацию видео в xAI, опрашивает её до завершения и скачивает результат.

## Предварительные требования

- Нужна запись провайдера `xai` с **API-ключом** (`nxc login xai` сам по себе недостаточен —
...
- Активной маршрутизируемой моделью должна быть не-OpenAI модель (например, Anthropic Claude или
  Google Gemini)
- nexcode должен быть настроен на маршрутизацию через этого не-OpenAI провайдера
...
> ```
>
> Если вы подключали его через `nxc login xai` (OAuth), провайдер останется в
...
1. nexcode обнаруживает не-OpenAI маршрутизируемую модель с `videoBridgeEnabled: true`
2. В разговор внедряется синтетический function tool `video_gen`
3. Когда модель вызывает `video_gen`, nexcode отправляет job в `/videos/generations` xAI
