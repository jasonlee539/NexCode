...
| `hostname?` | `string` | `"127.0.0.1"` | Адрес bind'а. Не-loopback bind требует `NEXCODE_API_AUTH_TOKEN`. |
...
| `emptyCompletionRetry?` | `boolean` | `false` | Явно включает один идентичный повтор Responses, если completion не содержит ни текста, ни tool call. Повтор может тарифицироваться. `NXC_EMPTY_COMPLETION_RETRY=0` отключает его без изменения config; combo и routed-compaction turn исключены. |
...
| `apiKeys?` | `NxcApiKey[]` | `[]` | Сгенерированные credentials `nxc_…`, принимаемые для management и data-plane auth на не-loopback bind'ах. Управляются через дашборд. |
...
| `codexAutoStart?` | `boolean` | `true` | Разрешает shim'у Codex запускать `nxc ensure` перед стартом Codex. При false `ensure` становится no-op. |
| `codexShimAutoRestore?` | `boolean` | `true` | Восстанавливает установленный shim после завершённого внешнего обновления Codex, которое заменило его. Для отключения через окружение: `NEXCODE_CODEX_SHIM_AUTO_RESTORE=0`. |
| `syncResumeHistory?` | `boolean` | `true` | Обратимый режим совместимости истории Codex App. Исходные metadata резервируются и восстанавливаются через `nxc stop` / `nxc restore`. |
...
| `webSearchSidecar?` | `NxcWebSearchSidecarConfig` | on when usable | Настройки sidecar'а web-search. |
| `visionSidecar?` | `NxcVisionSidecarConfig` | on when usable | Настройки sidecar'а описания изображений. |
| `images?` | `NxcImagesConfig` | automatic OpenAI selection | Настройки standalone Images relay для Codex `image_gen`. |

Если более старая development-сборка изменила metadata resume-history до появления резервного
backup'а, выполните `nxc recover-history --legacy-openai`, чтобы принудительно вернуть
...
экспортируйте её до `nxc service install`, чтобы launchd, systemd или Task Scheduler получили
...
| Эндпоинт | `Authorization: Bearer` | `x-nexcode-api-key` | `x-api-key` |
| --- | --- | --- | --- |
...
`x-nexcode-api-key`, а `Authorization` и `x-api-key` резервирует под credentials Anthropic.
...
работает. Укажите этот base URL клиенту; сам `nxc` продолжает записывать в managed client config
...
Эти настройки управляют `/v1/messages`, `/v1/messages/count_tokens`, launcher'ом `nxc claude` и страницей Claude в дашборде.
...
| `claudeCode.subagentEffort?` | `"low" \| "medium" \| "high" \| "xhigh" \| "max"` | inherit | Effort, записываемый в сгенерированные `~/.claude/agents/nxc-*.md`; это отдельно от guidance Codex и proxy cap'ов. Чтобы перегенерировать файлы, перезапускайте через `nxc claude`. |
...
nexcode. Перед использованием прогоните soak-test на нужном аккаунте и ожидаемой нагрузке.
