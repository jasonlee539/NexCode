...
Настройки агентов управляют тем, какая collaboration surface Codex рекламируется и как nexcode
...
| `multiAgentGuidanceEnabled?` | `boolean` | `true` | Управляет только developer-guidance, написанным самим nexcode, для v1/v2; не меняет native default'ы агентов, tools, routing, roster'ы и effort cap'ы. |
...
Управляйте surface через дашборд или `nxc v2 status|on|off|mode <v1|default|v2>|threads <n>`.
Смена режима применяется к новым сессиям. `maxConcurrentThreadsPerSession` — это поле
`PUT /api/v2`, а не ключ `config.json`; `nxc v2 threads <n>` записывает
...
Модельные цепочки fallback для ролей должны храниться в конфигурации nexcode. Запись
...
по-прежнему читается для обратной совместимости, но `nxc doctor` помечает её.

nexcode пропускает кандидатов, которые отключены, не маршрутизируются, unhealthy, находятся в
...
помещается под cap, nexcode убирает поле effort и позволяет провайдеру применить собственный
