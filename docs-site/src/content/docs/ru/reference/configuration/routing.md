...
| `combos?` | `Record<string, NxcComboConfig>` | `{}` | Виртуальные модели `combo/<id>`, построенные из упорядоченных целей provider/model. |

## Порядок разрешения модели

nexcode разрешает запрошенную модель в следующем порядке:
...
предупреждение с `nxc sync` означает только, что каталог picker'а ещё нужно привести в согласованное
...
Combo остаётся доступной для прямой маршрутизации, даже если её нельзя вывести в списке. `nxc sync`,
