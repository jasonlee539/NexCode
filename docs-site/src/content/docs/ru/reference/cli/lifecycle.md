...
nexcode и его интеграцию с Codex.
...
сохраняет `~/.nexcode/config.json`; при желании внедряет прокси в
...
Запустить proxy server (предпочтительный порт `10100`). Если этот порт занят, nexcode выбирает и
...
был запущен как managed service (`NXC_SERVICE=1`).
...
установлена managed background service, `nxc stop` сначала останавливает и её, чтобы она не
...
выполните `nxc stop`, затем один раз `nxc start`.
...
удалить локальную конфигурацию nexcode только если все шаги восстановления завершились успешно.
...
создавать refresh single-flight/lock file'ы в `NEXCODE_HOME`, есть ли нездоровые OAuth- или
...
Если всё ещё работают долгоживущие процессы Codex `app-server`, `nxc sync` предупредит, что они
могут продолжать отдавать старый in-memory список моделей, хотя файлы
`nexcode-catalog.json` / `models_cache.json` уже обновлены. Передайте `--restart-codex`, чтобы
...
nexcode. Предупреждение о stale-`app-server` и optional `--restart-codex` работают так же, как
и у `nxc sync`.
...
Запустить nexcode как login-managed background service (macOS **launchd**, Linux **systemd user
unit**, Windows **Task Scheduler**), которая автоматически стартует при логине и сама
перезапускается при crash. Запуски службы выставляют `NXC_SERVICE=1`, чтобы restart не дёргал
конфиг Codex.
...
На Windows bare `nxc service` выполняет путь установки только после того, как отсутствие подтверждено и для Task Scheduler, и для WinSW. Если любой из запросов статуса не даёт определённого ответа, он отказывается что-либо регистрировать и предлагает выполнить `nxc service status`; явный `nxc service install` используйте только после подтверждения отсутствия.

На Windows `nxc service status` отдельно показывает регистрацию в Task Scheduler и
identity-проверенную достижимость прокси NexCode. Он не печатает локализованную таблицу
...
владение command-shape `/create /tn nexcode-proxy /xml <non-empty-path> /f`, status 1 и
...
Либо подтвердите UAC через дашборд, либо заново выполните `nxc service install` в elevated
окне PowerShell.
...
Перед фиксацией установки или repair NexCode запускает сохранённый launcher с `--version`,
...
менеджера команд не проходит эти проверки, используйте вместо него `nxc service install`.
При обновлении установленный Unix shim без текущей validation-защиты пересоздаётся и проверяется.
Если сохранённый launcher небезопасен, NexCode удаляет устаревший shim и восстанавливает исходный
...
команда `nxc` сохранит новый стабильный launcher и восстановит shim перед выполнением запроса.
...
`nxc codex-shim install`. Чтобы отключить автоматику, задайте `codexShimAutoRestore: false` или
установите `NEXCODE_CODEX_SHIM_AUTO_RESTORE=0`.
...
:::tip[Service vs Shim]
Используйте `nxc service` для всегда работающего фонового прокси (рекомендуется). Используйте
`nxc codex-shim` для лёгкого on-demand запуска без демона — в этом случае прокси стартует только
...
Самообновить nexcode из npm. Стабильные установки используют `@latest`; preview-установки
...
запускается заново, а для foreground-установки печатается подсказка `nxc start`. В записях обновления
...
[Release workflow](https://github.com/lidge-jun/nexcode/actions/workflows/release.yml)
публикует их в npm.
