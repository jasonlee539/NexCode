---
title: Архитектура
description: Внутреннее устройство nexcode — карта модулей, мост AdapterEvent, парсер запросов и кэширование.
---

nexcode — это один процесс Bun. Запрос приходит как OpenAI Responses, нормализуется во
...
- **Сообщения** — элементы `input` становятся нормализованным `NxcMessage[]`: user / developer /
...
2 секунды генерирует комментарий-строку SSE (`: nexcode heartbeat`), чтобы перезапускать
...
`NEXCODE_API_AUTH_TOKEN` и для `/api/*`, и для `/v1/*`, когда прокси привязан за пределами
...
`NxcUsage`, отражается в терминальных событиях Responses и агрегируется модулем `usage/` для
...
выполнить WebSocket-апгрейд Responses, пока `websockets` равно `false`, nexcode возвращает
...
Внутренняя модель живёт в `types.ts`: `NxcParsedRequest`, `NxcContext`, объединение `NxcMessage`,
`NxcContentPart` (text / image), `NxcToolCall`, `NxcTool`, `AdapterEvent` и типы конфигурации
(`NxcConfig`, `NxcProviderConfig`). Широко используются два хелпера: `namespacedToolName()` и
