...
**Адаптер** выполняет преобразование между внутренней моделью запросов/ответов nexcode и
...
`buildRequest` понижает `NxcParsedRequest` до HTTP-запроса к вышестоящему провайдеру;
...
В ходе с инструментами nexcode добавляет приватный `codex_kiro_final_answer`. Повторная попытка не
