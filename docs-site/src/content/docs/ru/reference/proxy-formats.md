...
nexcode предоставляет один локальный прокси сразу в нескольких клиентских диалектах. Клиент
...
Это нативная форма data plane для nexcode. Тело запроса должно быть JSON-объектом с непустым
...
`POST /v1/realtime/calls` принимает surface call-creation OpenAI Realtime. nexcode выбирает
...
| Любая другая routed-модель | Запускает внутренний, не-streaming, без-tool'овый compaction-turn с `compaction_trigger`; требует ровно один синтетический item `compaction`, чей `encrypted_content` — это envelope `nxc1:`; затем декодирует это summary обратно в replacement history v1 |
...
| 502 | `invalid_response_error` | Synthetic-turn не создал ровно один корректный непустой item compaction `nxc1:` |

## Матрица аутентификации

На bind'е только для loopback data-plane admission не требует настроенного ключа. На удалённой
привязке используйте матрицу ниже. «Dedicated» означает `X-NexCode-API-Key`; остальные столбцы —
...
ciphertext сохраняется байт в байт: nexcode его не расшифровывает, не переводит содержимое и не
...
ChatGPT, nexcode завершит запрос ошибкой `unreadable_encrypted_agent_task`, вместо того чтобы
