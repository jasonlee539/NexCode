...
description: Аутентификация, ошибки и справочник эндпоинтов плоскости управления nexcode.
---

Management API — это control plane nexcode. Дашборд на `http://localhost:10100` — лишь один из
его клиентов; headless-команды `nxc` для провайдеров, моделей, combo, аккаунтов, настроек,
...
nexcode разрешает его в таком порядке:

1. `NEXCODE_ADMIN_AUTH_TOKEN`, если переменная задана.
2. Сгенерированный токен `nxc_admin_*` в hardened secret file.
...
На loopback-привязке bootstrap дашборда может получить short-lived credential `nxc_session_*`.
...
| --- | --- | --- |
| 401 | `nexcode admin token required` | Admin-token или GUI-session отсутствуют, неверны, просрочены, не совпадают по origin или не содержат CSRF-подтверждение |
...
соответствующие команды `nxc`: они обращаются к тому же живому API и возвращают ненулевой код,
