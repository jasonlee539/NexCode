---
title: Architecture
description: nexcode internals — module map, the AdapterEvent bridge, the request parser, and caching.
---

nexcode is a single Bun process. A request enters as OpenAI Responses, is normalized to an internal
...
- **Messages** — `input` items become a normalized `NxcMessage[]`: user / developer / assistant /
...
comment line (`: nexcode heartbeat`) every 2 seconds to re-arm Codex's idle timer. Comment lines
...
requires `NEXCODE_API_AUTH_TOKEN` for both `/api/*` and `/v1/*` when the proxy binds beyond
...
(`oauth/health.ts`) feeds `nxc status`, `nxc doctor`, the management API, and the dashboard.
...
and is not persisted across restarts. Request usage is normalized to `NxcUsage`, surfaced in
...
WebSocket upgrade while `websockets` is `false`, nexcode returns `426 upgrade_required`; Codex then
...
The internal model lives in `types.ts`: `NxcParsedRequest`, `NxcContext`, the `NxcMessage` union,
`NxcContentPart` (text / image), `NxcToolCall`, `NxcTool`, `AdapterEvent`, and the config types
(`NxcConfig`, `NxcProviderConfig`). Two helpers are widely used: `namespacedToolName()` and
