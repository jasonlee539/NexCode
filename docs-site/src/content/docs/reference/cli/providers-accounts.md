...
credentials under `~/.nexcode/`; API-key login providers open their key dashboard, prompt for the
...
Use the same command to **reauthenticate** after `nxc status` / `nxc doctor` reports
reauthentication required or a terminal refresh failure (or use Reauthenticate in the dashboard).
Codex pool accounts are not a public `nxc login` provider — reauthenticate via the dashboard Codex
account pool (Reauthenticate) or the headless `nxc account reauth` flow instead.
...
usage-based switching is off. NexCode replays the conversation after an account change, but the
...
On a **429**, nexcode honors `Retry-After`, starts the account cooldown, clears affinity, and may
...
For the Codex pool, use `nxc account refresh openai [--json]`. It force-refreshes account quotas and
...
account — `nxc account priority openai main last` is how you keep it as the reserve.
...
`nxc account --help` for the provider-specific command shape. If a Codex account login is saved but
its model-catalog refresh remains pending, human output still exits successfully and prints fixed
`nxc sync` recovery guidance on stderr. `--json` keeps stdout parseable and carries
...
already saved; human output prints generic `nxc sync` recovery guidance on stderr and still exits 0.
...
Manage named native Codex main-login profiles without changing NexCode account-pool routing:
...
The v1 recovery matrix covers an NexCode process exiting after a transaction file has been
...
`<real CODEX_HOME>/.nexcode-native-main-profiles` directory, so every NexCode instance sharing
that Codex home observes one owner and one recovery state. Plaintext login staging remains isolated
under each `<NEXCODE_HOME>/native-main-profile-staging` directory.

Before native-main traffic or journal recovery is admitted, the lifetime owner takes the exclusive
credential claim and removes only exact `auth.json.nxc.<pid>.<sequence>.tmp` crash residues. Each
...
This protects against cooperative NexCode crashes, not a malicious process already running as the
...
Preview builds used `<NEXCODE_HOME>/native-main-profiles`. That layout is never imported silently.
If `doctor` reports legacy profile state, stop every NexCode proxy sharing the same `CODEX_HOME`.
...
permissions, or remove the old preview set and run `nxc account main register` again. Do not choose
...
`nxc model` is an alias of `nxc models`. With no subcommand, list the models statically seeded in
...
proxy to be running (`nxc start`, or an installed service).
