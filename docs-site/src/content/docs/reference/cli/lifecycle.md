...
These commands install, run, inspect, repair, and update the local nexcode proxy and its Codex integration.
...
API key (literal or `${ENV}`), default model, and proxy port; saves `~/.nexcode/config.json`;
...
Start the proxy server (preferred port `10100`). If that port is occupied, nexcode selects and
...
native Codex — unless it was launched as a managed service (`NXC_SERVICE=1`).
...
background service is installed, `nxc stop` also stops it first so it cannot respawn the proxy.
...
closed without an `ensure` or stop/start fallback. After confirming ownership, use `nxc stop` then
`nxc start` for a standalone proxy. For a service-managed proxy, use `nxc stop` followed by
`nxc service start` so supervision is restored.
...
nexcode local config only if all restore steps succeeded. `remove` is an alias of `uninstall`.
...
After stopping the NexCode proxy/service, explicitly preserve and move a proven non-authoritative
...
single-flight/lock files can be created under `NEXCODE_HOME`, non-healthy OAuth or Codex pool
...
Before provider discovery or catalog/cache replacement, `nxc sync` validates that the managed
...
`nxc restore back` uses the same no-write preflight before it re-enables routing.

If long-lived Codex `app-server` processes are still running, `nxc sync` warns that they may keep
serving the previous in-memory model list even though `nexcode-catalog.json` / `models_cache.json`
...
Invalidate Codex's local model picker cache so it is rebuilt from the active nexcode catalog. The
same stale-`app-server` warning and optional `--restart-codex` behavior as `nxc sync` apply.

## Background service

### `nxc service [install|repair|restart|start|stop|status|uninstall|remove]`

Run nexcode as a login-managed background service (macOS **launchd**, Linux **systemd user unit**,
Windows **Task Scheduler**) that auto-starts on login and auto-restarts on crash. Service runs set
`NXC_SERVICE=1` so a restart does not churn the Codex config.
...
stops instead of retrying the same missing executable every five seconds. Reinstall nexcode, then
run `nxc service repair` to refresh the task with the restored package paths.
...
On Windows, a bare `nxc service` runs the install path only after both Task Scheduler and WinSW are
proven absent. If either status query is inconclusive, it refuses to register anything and asks you
to run `nxc service status`; use explicit `nxc service install` only after confirming absence.
...
log named in the message, and use `nxc start` to serve in the foreground meanwhile.

`nxc service status` reports the same three states rather than raw manager output:
...
On Windows, `nxc service status` reports Task Scheduler registration separately from
identity-verified NexCode proxy reachability. It does not print the localized `schtasks` table,
...
requires the owned command shape `/create /tn nexcode-proxy /xml <non-empty-path> /f`, status 1,
...
dashboard UAC prompt or rerun `nxc service install` in an elevated PowerShell window.

For a fresh install where the NexCode scheduler task is confirmed absent, UAC approval now
happens before the installer stops any existing proxy. Its unique registration XML is staged in
an ACL-hardened private directory outside the NexCode config root, and the task is registered
without being run. Only after
registration succeeds does NexCode remove that XML, require ownership metadata for a genuinely
...
Before an install or repair is committed, NexCode runs the saved launcher with `--version` while
...
use `nxc service install` instead when a dynamic command-manager launcher cannot meet these checks.
During upgrades, an installed Unix shim that lacks the current validation guard is regenerated and
probed. If its saved launcher is unsafe, NexCode removes the obsolete shim and restores the
original launcher instead of leaving the unsafe wrapper installed.

Launcher installation alone does not prove that Codex requests will use NexCode. After a healthy
...
never print proxy values; resolve the reported handoff and run `nxc doctor` before relying on
autostart.

If a completed external Codex update overwrites an installed shim, the next ordinary `nxc` command
...
command; manual fallback: `nxc codex-shim install`. Set `codexShimAutoRestore` to `false`, or set
`NEXCODE_CODEX_SHIM_AUTO_RESTORE=0` for a process-level opt-out.
...
:::tip[Service vs Shim]
Use `nxc service` for an always-on background proxy (recommended). Use `nxc codex-shim` for
...
Self-update nexcode from npm. Stable installs use `@latest`; preview installs stay on `@preview`
...
while a foreground installation prints `nxc start` as the next step. Dashboard update records
...
New versions become available when the [Release workflow](https://github.com/lidge-jun/nexcode/actions/workflows/release.yml)
