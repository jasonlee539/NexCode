...
nexcode presents one local proxy in several client dialects. A Codex client can speak the
...
This is the native nexcode data-plane shape. The request body must be a JSON object with a
...
`POST /v1/realtime/calls` accepts the OpenAI Realtime call-creation surface. nexcode selects an
...
| Other routed model | Runs an internal, non-streaming, no-tools compaction turn with a `compaction_trigger`; requires exactly one synthetic `compaction` item whose `encrypted_content` is an `nxc1:` envelope; decodes that summary into v1 replacement history |
...
| 502 | `invalid_response_error` | The synthetic turn did not produce exactly one valid, non-empty `nxc1:` compaction item |

## Authentication matrix

On a loopback-only bind, data-plane admission does not require a configured key. On a remote bind,
use the matrix below. “Dedicated” means `X-NexCode-API-Key`; the other columns mean
...
byte for byte: nexcode does not decrypt it, translate its contents, or re-encrypt it for another
provider.
...
but the selected routed target cannot read native ChatGPT ciphertext, nexcode fails with
