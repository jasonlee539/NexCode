...
An **adapter** translates between nexcode's internal request/response model and one provider wire
...
`buildRequest` lowers an `NxcParsedRequest` into an upstream HTTP request; `parseStream` /
...
- **Inline image output:** when the model is one of the explicit image-capable chat IDs
  ...
  `inlineData` parts are materialized under the configured NexCode `artifacts/` directory and
  surfaced as markdown image links to the authenticated opaque route
  `/v1/nexcode/artifacts/<id>` (not `file:` URIs or host filesystem paths). Each image is capped
...
When an ordinary client tool exists, nexcode adds a private
...
use emulated reasoning: nexcode converts the selected level into bounded thinking instructions in
...
- After a successful no-tool turn, the adapter keeps Cursor's returned ConversationStateStructure
  ...
  authoritative cache_read_tokens, so NexCode usage is not a cache-hit counter.
