...
description: Authentication, errors, and endpoint reference for the nexcode control plane.
---

The Management API is nexcode's control plane. The dashboard at
`http://localhost:10100` is one client of it; headless `nxc` provider, model, combo, account,
...
nexcode resolves it in this order:

1. `NEXCODE_ADMIN_AUTH_TOKEN`, when set.
2. A generated `nxc_admin_*` token in a hardened secret file.
...
On a loopback bind, the dashboard bootstrap can receive a short-lived `nxc_session_*` credential.
...
| --- | --- | --- |
| 401 | `nexcode admin token required` | The admin token or GUI session is missing, invalid, expired, origin-mismatched, or missing CSRF evidence |
...
workflow. For headless hosts and automation, use the corresponding `nxc` commands: they call this
