...
description: Where nexcode stores configuration, how edits are applied, and links to every configuration domain.
---

nexcode stores its persistent configuration in `$NEXCODE_HOME/config.json`, normally
`~/.nexcode/config.json`. On Windows, the default is
...
- **CLI:** `nxc init` creates the initial file, while commands such as `nxc provider`, `nxc models`,
  `nxc combo`, `nxc agent`, and `nxc config` update or inspect their owned settings.
...
If the file cannot be parsed, nexcode backs it up as
...
documented on the domain pages. `NEXCODE_HOME` takes precedence over the default configuration
...
:::note[Atomic writes]
nexcode writes managed `config.toml` and `nexcode-catalog.json` files through a temporary file
followed by rename (`atomicWriteFile`).
This prevents partial files when concurrent writers, such as `nxc stop` and the proxy shutdown handler,
