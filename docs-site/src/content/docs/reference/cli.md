---
title: CLI Reference
description: Command dispatch, exit codes, and links to every nxc command family.
---

The nexcode CLI is `nxc`. It dispatches on the first command name, with documented aliases such
...
Run `nxc help` (or `nxc --help` / `nxc -h`) for top-level usage. Run `nxc help <command>`,
`nxc <command> --help`, or `nxc <command> -h` for a command registered in the help table. Help and
...
`nxc observe logs --follow --jsonl` for a streaming request-log feed. Theme, language, navigation,
...
and unavailable required services exit nonzero. `nxc health` specifically exits 0 only when the
...
`nxc --version`, `nxc -v`, and `nxc version` print one script-friendly version line and exit.
