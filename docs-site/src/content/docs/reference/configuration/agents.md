...
Agent settings control which Codex collaboration surface is advertised and how nexcode guides,
...
| `multiAgentGuidanceEnabled?` | `boolean` | `true` | Controls only nexcode-authored v1/v2 developer guidance; it does not change native agent defaults, tools, routing, rosters, or effort caps. |
...
`config.json` key; `nxc v2 threads <n>` writes `max_concurrent_threads_per_session` under
...
`multiAgentModeHintText`, and `nxc v2 mode-hint`) writes
`features.multi_agent_v2.multi_agent_mode_hint_text` in Codex's
`$CODEX_HOME/config.toml`. The CLI `nxc v2 mode-hint` command persists this key even
...
an explicit v2 surface (`multiAgentMode: "v2"`, equivalent to `nxc v2 mode v2`);
`nxc v2 on` alone does not satisfy that dashboard gate.
...
Per-role fallback chains must live in nexcode config. Writing `model_fallback` into
...
read for backwards compatibility, but `nxc doctor` flags it.

nexcode skips disabled, unroutable, unhealthy, cooling-down, or quota-threshold candidates. The
...
contains an otherwise unreadable Fernet payload, nexcode uses a raw Responses passthrough request
to the fixed `https://chatgpt.com/backend-api/codex/responses` endpoint with forward-mode
authentication. ChatGPT returns the plaintext assignment through a forced function call; nexcode
...
- callers using `x-nexcode-api-key`, `x-api-key`, generic API credentials, or a proxy admission
...
- the recovery request forwards only `authorization`, the matching `chatgpt-account-id`,
  `originator`, and optional `openai-beta` and `user-agent` metadata; nexcode sets `content-type`
...
no effort control or no supported rung fits, nexcode removes the effort and lets the provider default
