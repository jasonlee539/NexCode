...
| `combos?` | `Record<string, NxcComboConfig>` | `{}` | Virtual `combo/<id>` models built from ordered provider/model targets. |
| `routingProfiles?` | `Record<string, NxcRoutingProfileConfig>` | `{}` | Virtual `policy/<id>` models that select among an explicit candidate allowlist using hard capability requirements and deterministic scoring. |

## Model resolution order

nexcode resolves the requested model in this order:
...
also be preserved exactly when they are not yet part of nexcode's static set; the observation must
...
labels. Account and setting mutations are persisted before a bounded catalog refresh; an `nxc sync`
...
CLI: `nxc route policy list [--json]`, `nxc route policy show <id> [--json]`, and
`nxc route policy dry-run <id> [--model-context <tokens>] [--tools] [--image] [--structured-output] [--json]`.
...
A combo remains directly routable even when it cannot be listed. `nxc sync`, `/v1/models`, and the
...
CLI: `nxc logs explain <request-id>`, `nxc logs rebuild-index`,
`nxc logs index-status`, `nxc route policy list | show | dry-run | evaluate`.
...
an automatic rebuild from `usage.jsonl` on the next query; `nxc logs
