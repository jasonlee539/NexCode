...
A provider tells nexcode where a model lives, which wire adapter it speaks, and how requests are
authenticated.
...
| --- | --- | --- | --- |
| `providers` | `Record<string, NxcProviderConfig>` | — | Map of provider name to provider config. |
...
| `tokenGuardian?` | `NxcTokenGuardianConfig` | off | Optional proactive OAuth refresh and Codex-account warmup policy. |

Selector names are user-chosen public labels; nexcode assigns no account-role semantics to them.
...
shipped v1 config, nexcode creates `config.json.pre-openai-tiers-v2.bak` without replacing a
differing backup and rewrites known legacy namespaced selected ids to bare ids.

## Provider entries (`NxcProviderConfig`)
...
| `preserveResponsesReasoningContent?` | `boolean` | Keep plaintext reasoning content on replayed Responses reasoning items instead of blanking it (blanking is the ChatGPT backend's rule). Enable for upstreams whose contract accepts reasoning replay, such as DeepSeek. Proxy-minted `nxcr1` envelopes are always stripped. |
...
[provider-field definitions](#provider-entries-nxcproviderconfig) above remain the authoritative
...
sends `service_tier: "priority"` through Chat Completions. `nxc login xai`
...
credential store populated by `nxc login`; subscription-backed Claude Code launch behavior is
...
outbound proxy, nexcode resolves the hostname once and connects only to that validated address.
...
so nexcode cannot pin or verify that peer. This is an explicit security limitation.
...
`nxc account use anthropic <id>` switching when unsure.
:::
...
When routing discards `baseUrl`, nexcode logs the registry endpoint and only the configured origin;
...
The Cursor bridge is experimental. After `nxc login cursor`, add or edit `providers.cursor`.
...
Model keys are exact native OpenRouter ids, without the outer nexcode provider prefix. Selecting
