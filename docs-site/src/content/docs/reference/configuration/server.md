...
| `hostname?` | `string` | `"127.0.0.1"` | Bind address. Non-loopback binds require `NEXCODE_API_AUTH_TOKEN`. |
...
| `emptyCompletionRetry?` | `boolean` | `false` | Opt in to one identical Responses retry when a completion has no text or tool call. The retry may be billable. `NXC_EMPTY_COMPLETION_RETRY=0` disables it without changing config; combo and routed-compaction turns remain excluded. |
...
| `apiKeys?` | `NxcApiKey[]` | `[]` | Generated `nxc_…` credentials accepted by management and data-plane auth on non-loopback binds. Dashboard-managed. |
...
| `codexAutoStart?` | `boolean` | `true` | Let the Codex shim run `nxc ensure` before launching Codex. False makes ensure a no-op. |
| `codexShimAutoRestore?` | `boolean` | `true` | Restore an installed shim after a completed external Codex update replaces it. Environment opt-out: `NEXCODE_CODEX_SHIM_AUTO_RESTORE=0`. |
| `syncResumeHistory?` | `boolean` | `true` | Reversible Codex App history compatibility. Original metadata is backed up and restored by `nxc stop` / `nxc restore`. |
...
| `webSearchSidecar?` | `NxcWebSearchSidecarConfig` | on when usable | Web-search sidecar options. |
| `visionSidecar?` | `NxcVisionSidecarConfig` | on when usable | Image-description sidecar options. |
| `images?` | `NxcImagesConfig` | automatic OpenAI selection | Standalone Images relay options for Codex `image_gen`. |

If an older development build changed resume-history metadata before backup support existed, run
`nxc recover-history --legacy-openai` to force native-provider recovery.
...
`nxc service install` so launchd, systemd, or Task Scheduler receives it. Clients should send:

```text
x-nexcode-api-key: your-secret-token
```

| Endpoint | `Authorization: Bearer` | `x-nexcode-api-key` | `x-api-key` |
| --- | --- | --- | --- |
...
`x-nexcode-api-key`, while `Authorization` and `x-api-key` are reserved for Anthropic credentials.
...
so it never inherits `NEXCODE_API_AUTH_TOKEN` and every model call fails with `401` before a
stream opens.
...
`nxc sync` then writes `base_url = "http://127.0.0.1:10200/v1"` into the managed Codex provider block
...
`nxc` writes only the default local `127.0.0.1` address into managed client config.
...
If a registered callback port is already in use and the login surface offers manual input, NexCode
...
code into NexCode. The pending flow preserves state and PKCE validation. Callers without manual
...
These settings govern `/v1/messages`, `/v1/messages/count_tokens`, the `nxc claude` launcher, and the Claude dashboard page.
...
| `claudeCode.subagentEffort?` | `"low" \| "medium" \| "high" \| "xhigh" \| "max"` | inherit | Effort written to generated `~/.claude/agents/nxc-*.md`; separate from Codex guidance and proxy caps. Restart through `nxc claude` to regenerate. |
...
Anthropic OAuth sidecars reuse nexcode's existing Claude Code OAuth fingerprint. Soak-test the
