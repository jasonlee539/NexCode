---
title: Quickstart
description: Configure your first provider and route OpenAI Codex through nexcode in three commands.
---
...
5. **Inject into Codex?** — on a normal loopback setup, nexcode adds a root `openai_base_url` to
...
6. **Install the autostart shim?** — when enabled, launching `codex` runs `nxc ensure` first.

The result is saved to `$NEXCODE_HOME/config.json` (default `~/.nexcode/config.json`).
...
- writes its PID to `~/.nexcode/nxc.pid` (and refuses to start twice),
...
If the requested port is busy, `nxc start` selects a free port, records it in `runtime-port.json`,
...
Codex now talks to nexcode transparently:
...
`gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, and `gpt-5.4-mini`. Open `nxc gui` to replace or
