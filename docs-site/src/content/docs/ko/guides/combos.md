...
**콤보**는 정해진 순서로 나열된 실제 공급자/모델 대상 목록 앞에 서는 하나의 가상 모델입니다. 클라이언트는 `combo/<id>`로 요청하고, nexcode는 대상을 하나 선택해 요청을 그 구체적인 `provider/model`로 다시 쓰며, 첫 번째 대상이 재시도 가능한 실패를 내면 다른 대상을 시도할 수 있습니다.
...
```bash
nxc combo set main --targets anthropic/claude-opus-4-8,openai/gpt-5.6-sol
```

기본 전략은 페일오버이므로 일반 요청은 `anthropic/claude-opus-4-8`으로 갑니다. 그 시도가 재시도 가능한 실패를 내면 nexcode는 `openai/gpt-5.6-sol`로 넘어갈 수 있습니다.
...
`nxc combo set <id>`의 콤보 ID는 문자나 숫자로 시작해야 합니다. 그 뒤에는 문자, 숫자, `.`, `_`, `-`를 포함할 수 있고, 전체 길이는 64자 이하여야 합니다. 정식 모델 ID는 항상 `combo/<id>`입니다. 예를 들어 ID `main`은 `combo/main`이 됩니다.
...
홉된 대상은 기본적으로 60초 동안 쿨다운에 들어갑니다. 상위 응답에 유효한 `Retry-After` 값이 있으면 nexcode는 그 값을 대신 사용합니다. 숫자 초와 HTTP-date 값이 모두 허용되며, 모든 쿨다운은 최대 10분으로 제한됩니다.
...
요청에 `reasoning` 객체가 없으면 nexcode가 새로 만듭니다. `reasoning`은 있지만 `effort` 속성이 없으면 다른 필드는 그대로 두고 기본값만 추가합니다. 호출자가 준 effort는 절대 덮어쓰지 않습니다.

대상 기능을 알 수 없거나 설정한 effort를 포함하지 않으면 nexcode는 기본값을 생략하고 대상의 동작은 그대로 둡니다. 지원 값은 `low`, `medium`, `high`, `xhigh`, `max`, `ultra`입니다. effort를 호출자와 대상에 완전히 맡기려면 이 필드를 생략하거나 `null`로 설정하십시오.

## 암호화된 v2 서브에이전트 작업

Codex v2 서브에이전트에는 중요한 제한이 하나 있습니다([issue #92](https://github.com/lidge-jun/nexcode/issues/92)). 네이티브 부모 프로세스는 새로 생성된 작업자에게 보낼 작업을 네이티브 ChatGPT 백엔드용으로 생성한 암호문으로만 전달할 수 있습니다. 외부 공급자는 그 페이로드를 읽을 수 없습니다.

이런 요청에서 콤보는 재시도 가능한 실패가 나더라도 정식 네이티브 ChatGPT 경로만 적합 대상으로 남깁니다. 콤보에 복호화 가능한 대상이 하나도 없으면 nexcode는 전송 전에 멈추고 HTTP 400을 반환합니다.
...
nxc combo list
nxc combo show <id>
nxc combo set <id> --targets provider/model[:weight],...
nxc combo remove <id> --yes
```
...
별칭이고 `delete`는 `remove`의 별칭입니다. 모든 combo 하위 명령은 `nxc route combo ...`에서도 사용할 수 있습니다.
...
`nxc combo list`를 실행하고, 철자와 대소문자를 확인하고, 관리 명령이 모델 요청을 받는 것과 동일한
nexcode 인스턴스에 기록했는지 확인하세요.
