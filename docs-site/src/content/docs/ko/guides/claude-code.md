...
description: Claude Code에서 라우팅된 모든 모델을 사용해요. nexcode는 같은 포트에서 Anthropic Messages API와 게이트웨이 모델 검색을 제공해요.
---

nexcode는 `/v1/responses`와 함께 `POST /v1/messages`(및 `count_tokens`)를 제공해요. 따라서 Claude
...
`nxc claude`는 프록시가 실행 중인지 확인한 다음, 환경을 연결해 Claude Code를 실행해요.
...
직접 내보낸 변수가 항상 우선해요. 추가 인자는 그대로 전달돼요: `nxc claude -p "hello"`.
...
로그인이 있느냐에 달려 있고, 그건 nexcode가 직접 확인할 수 있어요.
...
`nxc claude`부터 알아서 반영돼요.

고정하고 싶다면 **구독** 또는 **프록시**를 직접 선택하세요. 직접 고른 값은 `claudeCode.authMode`에
저장되고, 이후에 로그인 상태가 바뀌어도 자동 감지가 이 값을 덮어쓰지 않아요. 다시 자동으로
돌리면 판단을 nexcode에 넘겨요.

macOS의 자동 연결(`claudeCode.systemEnv`)도 같은 방식으로 판단하므로, `nxc` 없이 실행한 `claude`도
동일하게 동작해요. 다만 이쪽은 프록시가 시작하거나 설정을 저장할 때 갱신되는 스냅숏이고,
`nxc claude`는 실행할 때마다 실시간으로 판단해요.

## 시스템 환경 통합(macOS)

`claudeCode.systemEnv`를 `true`로 설정하면(기본값: **꺼짐**) `nxc start`가 `launchctl setenv`를
사용해 `ANTHROPIC_BASE_URL`과 관련 Claude Code 환경 변수를 시스템 전체에 주입해요. 따라서 새
터미널 창과 탭에서는 `nxc claude` 래퍼 없이 일반 `claude` 명령도 프록시를 거쳐요. 이미 열려
있는 셸에는 적용되지 않으므로 다시 열어야 해요.

`nxc stop`과 프록시 종료는 **주입된 키를 해제해요**. 이전 값을 복원하지는 않고 nexcode가
주입한 키만 제거해요. 프록시는 `~/.nexcode/claude-env.sh`도 작성하고, `nxc start`는 이 파일을
자동으로 불러오는 `.zshrc` source hook을 실행 가능한 Claude Code CLI가 `PATH`에 있을 때만 설치해요.
Claude Code가 없거나 시스템 환경 연동이 비활성화되어 있으면 시작 과정과 `nxc ensure`가 NexCode가 추가한
...
전용이며, 다른 플랫폼에서는 `nxc claude`를 사용하세요.
...
`x-nexcode-api-key`, `origin`은 항상 전달 전에 제거해요. 비루프백 바인드의 네이티브 패스스루는
유효한 프록시 자격 증명을 `x-nexcode-api-key`로도 요구하고, 이때 `Authorization`과
...
프록시 admission 헤더도 유효해야 해요. 그래서 `nxc claude`를
...
해시 별칭과 `claude-nxc-<provider>--<model>` 별칭도 계속 해석됩니다. 컨텍스트가 1M인 모델에는
...
받으므로, nexcode는 라우팅 모델을 안정적이고 되돌릴 수 있는 별칭으로 노출해요.

| 화면 | 형식 | 예시 |
| --- | --- | --- |
| Claude Code CLI | `claude-nxc-<provider>--<model>` (plain) 또는 `claude-nxc2-…` (escaped) | `claude-nxc-native--gpt-5.6-sol` |
...
`/model <id>`를 사용하세요. NexCode는 선택기 상태를 따로 볼 수 없고 각 요청에 실린 모델 ID를
라우팅해요. 적용 결과는 **Logs → requestedModel**에서 확인할 수 있어요.

**별칭 문법 규칙:** provider에는 `/`나 `--`를 넣을 수 없고 `native`와 같아도 안 돼요. `/`와 `~`가
없는 plain model ID는 v1 접두사 `claude-nxc-…`를 유지해요. `/` 또는 `~`가 있는 model ID는 v2
접두사 `claude-nxc2-…`로 만들고 이스케이프해요(`/` → `~s`, `~` → `~t`). 예:
...
`effectiveModelEnv`는 `nxc claude` / 시스템 환경 / 셸 파일이 주입할 슬롯 여섯 개를 계산해요.
...
`tierModels.haiku`와 `smallFastModel`이 모두 없으면 NexCode는 두 보조 모델 변수를 설정하지 않아요. 그러면 Claude Code가 네이티브 보조 모델(현재 Sonnet)을 선택하며, 네이티브 프로바이더 요금이 발생할 수 있어요.

## 로스터 에이전트(injectAgents)

프록시 시작/ensure, `nxc claude`, 관련 대시보드 저장은 추천 서브에이전트 로스터(Subagents 탭, 최대 5개 모델)와
`nxc-self`를 `~/.claude/agents/nxc-*.md`에 동기화해요.

- **`nxc-self`**는 `/model` 선택기의 기본값을 고정하고, 값이 없으면 `claudeCode.model`을 사용해요.
  둘 다 없으면 만들지 않아요. 모델 상속은 사용하지 않아요.
- 각 에이전트 본문에는 `<!-- nxc-route: <model> -->` 지시문이 들어 있어요. 프록시는 이 지시문으로
  실제 라우트를 고정해요. 따라서 Agent 도구의 `model` 인자는 작동하지 않으며, 자리 표시자로
  `"haiku"`를 전달하세요.
- frontmatter에는 별칭이 들어가고, 라우팅은 지시문을 따라요.
- `generated-by: nexcode`가 들어 있는 표식 검증된 `nxc-*.md` 파일만 덮어쓰거나 정리해요.
...
디스패치 예시: `subagent_type: "nxc-gpt-5-6-sol"`. 1M을 지원하는 대상에는 `[1m]`이 자동으로
붙어요.
...
nexcode는 기본적으로 **라우팅된** 요청에서 스킬 내용을 짧은 스텁으로 바꿔요. 네이티브
...
라우팅 모델마다 쓸 수 있는 호스팅 도구와 이미지 지원 범위가 달라요. nexcode는 메인 모델이
...
`nxc debug claude on|off|status|reset`, `NXC_CLAUDE_DEBUG=1` 또는
...
`ANTHROPIC_AUTH_TOKEN`이 설정되어 있어요. `nxc claude`는 의도적으로 `ANTHROPIC_API_KEY`를
설정하지 않으므로, 직접 내보냈다면 해제하세요. `nxc claude`를 사용할 때는
...
설정되어 있는지 확인하세요(`nxc claude`에서는 자동). `nxc claude`를 실행해
...
`ANTHROPIC_BASE_URL`이 오래된 값일 수 있어요. 새 터미널을 열거나 `nxc claude`를 다시 실행하세요.
...
언급하면 자동으로 불러와요. 네이티브 패스스루에서는 정상이며, 라우팅 모델에서는 nexcode가
기본적으로 스텁으로 바꿔요(`blockedSkills: ["claude-api"]`).

**서브에이전트가 잘못된 모델로 디스패치됨** — 로스터 에이전트(`nxc-*`)는 Agent 도구의 `model`
인자가 아니라 `<!-- nxc-route: ... -->` 지시문을 사용해요. 지시문이 원하는 라우트와 일치하는지
