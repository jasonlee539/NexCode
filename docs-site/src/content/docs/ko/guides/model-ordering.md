---
title: 모델 정렬에 관하여
description: nexcode가 Codex 모델 선택기와 spawn_agent 모델 override의 순서를 정하는 방식.
---

Codex 모델 선택기는 nexcode 설정에 적힌 프로바이더 선언 순서나 모델 배열 순서를 보존하지
...
따라서 nexcode는 배열 위치가 아니라 더 낮은 priority를 부여해 featured 위치를 제어합니다.
...
(nexcode가 만든 항목은 보통 `9`)를 사용합니다. 라우팅 그룹 내부는 계속 프로바이더/id
알파벳순입니다.
...
`nxc agent subagents set`은 exact account-qualified `<selector>/<native-openai-model>` id도
...
현재 `NxcConfig`에는 일반 `modelOrder`, `providerOrder`, priority map 설정이 없습니다. 지원되는 정렬
