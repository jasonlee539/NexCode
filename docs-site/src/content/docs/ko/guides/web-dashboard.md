---
title: 웹 대시보드
description: 프록시 상태, 프로바이더, 모델, 위임 안내, 인증 풀, 사용량, 로그를 관리하는 nexcode GUI.
---

nexcode는 프록시가 제공하는 로컬 웹 대시보드(`gui/` 아래의 Vite/React 앱)를 포함합니다.
...
`localhost`나 `127.0.0.1` 같은 loopback 주소에서 연 대시보드는 짧게 유지되는 GUI 세션을 자동으로 받으므로 보통 토큰을 입력할 필요가 없습니다. loopback이 아닌 호스트로 공개한 대시보드에는 `NEXCODE_ADMIN_AUTH_TOKEN` 또는 자동 생성되는 `~/.nexcode/admin-api-token` 파일의 관리자 토큰이 필요합니다.
...
| **Sub-agent delegation** | NexCode 위임 가이드와 선택적인 Codex 네이티브 서브에이전트 기본값이 함께 사용할 네이티브/라우팅 모델과 선택적 reasoning 강도를 고릅니다. 스폰별 라우터는 아닙니다. 아래 설명을 확인하세요. |
...
| **Codex 자동 시작** | 이미 설치된 Codex launcher shim이 `nxc ensure`를 실행하도록 허용합니다. 이 토글은 shim이나 백그라운드 서비스를 설치하지 않습니다. |
...
저장합니다. 선택한 값은 NexCode가 작성하는 위임 가이드에 사용되고, 이 가이드는
`multiAgentGuidanceEnabled`가 별도로 제어합니다. 모델을 지우면 저장된 강도도 지워지고 네이티브
기본값 동기화도 꺼집니다.

**Codex 네이티브 서브에이전트 기본값으로 사용**을 켜면 NexCode가 활성 Codex 라우팅을 관리하는
...
:::caution
두 토글은 서로 독립적입니다. NexCode 위임 가이드를 꺼도 네이티브 기본값 동기화는 꺼지지 않고,
...
- 각 계정 카드에는 **선택 순서** 컨트롤(가장 먼저 / 먼저 / 기본 / 나중에 / 가장 마지막)이 있습니다.
  ...
  있습니다. `nxc account priority`로 프리셋 밖의 값을 지정해도 카드에서 그대로 보이고 선택할 수
  있습니다.
...
- 계정 추가·삭제와 picker 설정은 catalog refresh보다 먼저 저장됩니다. refresh가 끝나지 않으면 amber
  복구 안내가 표시됩니다. 변경 자체는 저장되어 있으므로 `nxc sync`로 refresh를 다시 시도하십시오.
...
사이드바의 스타 버튼, 그리고 `nxc start`가 대화형 터미널에서 한 번 묻는 질문은 모두
**사용자 본인의 `gh` 로그인**을 씁니다. nexcode는 GitHub 토큰을 따로 갖고 있지 않고,
예/아니오 답만 알게 됩니다.

이 동작이 사용자 GitHub 계정에 쓰기를 하기 때문에, 에이전트가 대신 답하지 못하도록 막아둡니다.

- 에이전트나 CI가 실행 중이면(`CLAUDECODE`, `CODEX_THREAD_ID`, `CURSOR_TRACE_ID`, `CI` 등)
  `nxc start`와 `nxc service install`은 질문 자체를 띄우지 않습니다. 1회용 마커도 남기지 않으니
