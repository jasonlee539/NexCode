...
description: 공유 Codex 카탈로그를 통해 nexcode 모델이 Codex App, Codex CLI, Codex TUI에 표시되는 방식.
---

nexcode는 Codex App을 직접 고치지 않습니다. Codex CLI/TUI와 같은 Codex 설정과 모델 카탈로그를
...
nexcode는 매핑된 계정별로 `<selector>/<native-openai-model>` 행을 추가하고 선택기에서 bare native 행을
...
출하된 v1 소스를 `~/.nexcode/config.json.pre-openai-tiers-v2.bak`에 보관합니다. 복원하려면 다음을
실행합니다:
...
`nxc init`, `nxc start`, `nxc sync`는 공유 Codex 설정과 카탈로그를 프록시에 연결합니다. 설정 주입,
카탈로그 동기화, shim, WebSocket 폴백, 복원 메커니즘은 [Codex 통합](/guides/codex-integration/)을
참고하세요.

## 라우팅 모델이 표시되는 이유

Codex의 모델 선택기는 Codex 형식의 카탈로그 항목을 기대합니다. nexcode는 네이티브 Codex 모델
...
필드가 그대로 남습니다. 그다음 nexcode는 해당 라우트가 감당할 수 없는 OpenAI service-tier 메타데이터
...
하지만 모델 카탈로그와 런타임 요청 tier id는 `priority`를 씁니다. nexcode는 이 분리를 그대로
...
교체됩니다. nexcode는 선택한 순서대로 낮은 카탈로그 priority를 부여합니다. 계정 한정 선택기 행이
...
렌더러는 표시 전에 이 네이티브 전용 허용 목록에 없는 항목을 버립니다. nexcode는 이 목록에
개입할 수 없습니다. 업스트림 버그는
...
nexcode는 카탈로그의 visibility, priority, metadata가 바뀔 때마다 `models_cache.json`을 의도적으로
