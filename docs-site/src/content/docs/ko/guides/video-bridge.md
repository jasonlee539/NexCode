...
Video Bridge는 nexcode가 라우팅한 OpenAI가 아닌 모델로 xAI의 Grok Imagine Video 생성을 사용할 수 있게 합니다. 활성화하면 대화에 합성 `video_gen` 도구가 주입됩니다. 모델은 이를 일반 함수 도구처럼 호출하고, nexcode는 이 호출을 가로채 xAI에 영상 생성 작업을 제출한 뒤 완료될 때까지 폴링하고 결과를 내려받습니다.

## 사전 조건

- API 키가 있는 `xai` provider entry (`nxc login xai`만으로는 충분하지 않습니다. 비디오 브리지는 OAuth가 아니라 키 인증이 필요합니다)
- 라우팅 대상 provider로 비OpenAI 모델 사용 예시: Anthropic Claude, Google Gemini
- 비OpenAI provider를 거치도록 nexcode 설정
...
> ```
>
> `nxc login xai`(OAuth)로 연결했다면 provider는 계속 `authMode: "oauth"`
...
1. nexcode는 `videoBridgeEnabled: true`가 켜진 비OpenAI 라우팅 모델을 감지합니다.
2. 합성 `video_gen` 함수 도구가 대화에 주입됩니다.
3. 모델이 `video_gen`을 호출하면, nexcode는 xAI의 `/videos/generations`로 작업을 제출합니다.
