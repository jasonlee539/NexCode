---
title: Pi
description: Pi에서 라우팅된 모델을 그대로 쓸 수 있습니다. `nxc export`가 Pi의 `models.json`에 맞는 커스텀 provider 블록을 내보내고, 실행 중인 프록시에 연결합니다.
---

Pi는 provider를 환경 변수 대신 하나의 전역 JSON 파일에서 읽기 때문에,
nexcode가 Pi를 직접 실행하지 않습니다. 대신 `nxc export`가 `nexcode` provider 블록,
...
`nxc export`는 그 파일을 절대 쓰지 않습니다. `providers.nexcode` 블록을 그 안에
...
가시성을 바꾼 뒤에는 `nxc export`를 다시 실행하고, 새 블록을 옛 블록 위에 병합하세요.
...
| 키 | 무엇인지 | 어디에 있는지 |
| --- | --- | --- |
| Proxy admission key | nexcode의 자체 인증 정보이며, 대시보드의 **API** 탭에서 생성됩니다 | `apiKey`로 `$NEXCODE_API_KEY`를 참조하며, 값은 환경 변수에 둡니다 |
| Provider key | Anthropic / OpenAI / OpenRouter 키입니다 | nexcode의 자체 config에 있으며, [Providers](/guides/providers/)마다 따로 둡니다 |
...
(`NEXCODE_OPENCODE_API_KEY`, `{env:…}` 형식) - 자세한 내용은 [opencode 가이드](/guides/opencode/)를 보세요.

**루프백 프록시는 키가 전혀 필요 없습니다.** nexcode는 기본적으로 `127.0.0.1`에
바인드하고 그곳에서는 아무 것도 인증하지 않으므로, `$NEXCODE_API_KEY` 참조는
...
적용합니다. `nxc export`는 그 경우가 몇 줄이었는지도 함께 출력합니다.
...
하는데, nexcode는 라우팅된 모델의 가격 데이터를 갖고 있지 않습니다. 0을 넣으면
...
[issue를 열어주세요](https://github.com/lidge-jun/nexcode/issues).
:::

## 요구 사항

실행 중인 nexcode 프록시(`nxc start`)와 설치된 Pi가 필요합니다. `nxc export`는
