---
title: 기여하기
description: nexcode 개발 환경, 구조, 컨벤션, 프로바이더와 어댑터 추가 방법.
---
...
공개 문서는 GitHub Pages의 <https://nexcode.me/ko/>에 게시됩니다.
...
- **Cross-platform CI**(`.github/workflows/ci.yml`)는 런타임, 테스트, 패키지, 스크립트,
  ...
  `nxc help`를 검사합니다. 별도의 3개 OS lane은 번들 런타임을 사용해 Bun을 따로 설치하지 않아도
...
함께 끝났습니다. 히스토리는
[lidge-jun/nexcode-go-archive](https://github.com/lidge-jun/nexcode-go-archive)에
...
`src/providers/derive.ts`는 이 항목을 `nxc init`, `nxc provider`, 대시보드 preset, API 키 로그인,
...
실행합니다. nexcode는 큰 batch보다 작고 검증 가능한 commit을 선호합니다.
