...
nexcode는 하나의 로컬 프록시를 여러 클라이언트 방언으로 제공합니다. Codex 클라이언트는
...
이것이 nexcode의 기본 데이터 평면 형식입니다. 요청 본문은 비어 있지 않은 `model`을 가진 JSON 객체여야
...
- 비루프백 listener에서는 `x-nexcode-api-key`에만 유효한 프록시 admission이 들어 있습니다.
...
`POST /v1/realtime/calls`는 OpenAI Realtime call-creation 표면을 받습니다. nexcode는 적절한 OpenAI 계열
...
| 다른 라우팅된 모델 | `compaction_trigger`가 있는 내부 비스트리밍 no-tools compaction 턴을 실행합니다. `encrypted_content`가 `nxc1:` envelope인 synthetic `compaction` 항목이 정확히 하나 있어야 하며, 그 요약을 v1 replacement history로 디코딩합니다 |
...
| 502 | `invalid_response_error` | synthetic 턴이 유효하고 비어 있지 않은 `nxc1:` compaction 항목을 정확히 하나 만들지 못했습니다 |

## 인증 매트릭스

loopback 전용 bind에서는 data-plane admission에 설정된 key가 필요하지 않습니다. remote bind에서는 아래
매트릭스를 사용하십시오. “Dedicated”는 `X-NexCode-API-Key`를 뜻하고, 다른 열은 `Authorization: Bearer ...`
와 `x-api-key`를 뜻합니다.
...
그대로 보존됩니다. nexcode는 이를 복호화하거나, 내용을 번역하거나, 다른 프로바이더용으로 다시
암호화하지 않습니다.
...
nexcode는 읽을 수 없는 바이트를 프로바이더에 보내는 대신 `unreadable_encrypted_agent_task`로
