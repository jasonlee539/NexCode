...
**어댑터**는 nexcode의 내부 요청/응답 모델과 프로바이더 wire 형식 사이를 변환합니다. 모든
...
`buildRequest`는 `NxcParsedRequest`를 업스트림 HTTP 요청으로 내리고, `parseStream` /
...
- 도구 없이 정상 완료된 턴 뒤에는 Cursor가 돌려준 ConversationStateStructure를 프로세스 로컬
  ...
  않으므로 NexCode usage만 보고 cache hit라고 단정하지 않습니다.
