...
이 명령들은 로컬 nexcode 프록시와 Codex 연동을 설치, 실행, 점검, 복구, 업데이트합니다.
...
API 키(리터럴 또는 `${ENV}`), 기본 모델, 프록시 포트를 묻고 `~/.nexcode/config.json`에 저장합니다.
...
백그라운드 서비스가 설치되어 있으면 `nxc stop`이 먼저 그 서비스를 중지하므로 프록시가 다시
...
stop/start 대체 동작 없이 안전하게 실패합니다. 소유권을 확인한 뒤 `nxc stop`과 `nxc start`를
순서대로 한 번 실행하세요.
...
복원 단계가 모두 성공했을 때만 nexcode 로컬 설정을 제거합니다. `remove`는 `uninstall`의
...
**OAuth 안정성** 섹션은 자격 증명 저장소에 쓰기 가능한지, `NEXCODE_HOME` 아래에 refresh
...
오래 실행 중인 Codex `app-server` 프로세스가 아직 살아 있으면, `nexcode-catalog.json` /
...
Codex의 로컬 모델 선택기 캐시를 무효화하여, 활성 nexcode 카탈로그에서 다시 빌드되게 합니다.
`nxc sync`와 같은 오래된 `app-server` 경고와 선택적 `--restart-codex` 동작이 적용됩니다.

## 백그라운드 서비스

### `nxc service [install|repair|restart|start|stop|status|uninstall|remove]`

로그인 관리형 백그라운드 서비스로 nexcode를 실행합니다(macOS **launchd**, Linux **systemd** 사용자
유닛, Windows **Task Scheduler**). 로그인 시 자동 시작하고 충돌 시 자동 재시작합니다. 서비스 실행은
`NXC_SERVICE=1`을 설정하므로 재시작해도 Codex 설정이 흔들리지 않습니다.
...
Windows에서는 bare `nxc service`가 Task Scheduler와 WinSW 양쪽 모두 부재가 입증된 후에만 설치
경로를 실행합니다. 상태 조회 중 하나라도 불확실하면 아무것도 등록하지 않고 `nxc service status`
실행을 안내합니다. 부재를 확인한 뒤에만 명시적인 `nxc service install`을 사용하세요.

Windows에서는 `nxc service status`가 Task Scheduler 등록 상태를 ID가 검증된 NexCode 프록시
...
명령 형태 `/create /tn nexcode-proxy /xml <non-empty-path> /f`, 상태 1, 그리고 상승하지
...
상승된 PowerShell 창에서 `nxc service install`을 다시 실행해 주세요.
...
설치나 복구를 확정하기 전에 NexCode는 서비스 시작을 우회한 상태에서 저장된 런처를
...
명령 관리자의 런처가 이 검증을 충족할 수 없다면 대신 `nxc service install`을 사용하세요.
업그레이드할 때 현재 검증 가드가 없는 기존 Unix shim은 다시 생성하고 검증합니다. 저장된 런처가
안전하지 않으면 NexCode는 위험한 wrapper를 그대로 두지 않고 구버전 shim을 제거한 뒤 원래
런처를 복원합니다.

완료된 외부 Codex 업데이트가 설치된 shim을 덮어쓰면, 다음 일반 `nxc` 명령이 안정적인 새 런처를
백업하고 명령을 처리하기 전에 shim을 복원합니다. 아직 변경 중인 런처는 건드리지 않고 나중에 다시 시도합니다.
복구 실패는 요청한 명령을 실패시키지 않고 경고만 표시합니다. 수동 대체 수단은 `nxc codex-shim install`
입니다. `codexShimAutoRestore`를 `false`로 설정하거나, 프로세스 수준에서 제외하려면
`NEXCODE_CODEX_SHIM_AUTO_RESTORE=0`을 설정합니다.
...
:::tip[서비스와 shim]
항상 켜져 있는 백그라운드 프록시에는 `nxc service`를 사용합니다(권장). 데몬 없이 가볍게 필요할
때만 시작하려면 `nxc codex-shim`을 사용합니다. 이 경우 프록시는 `codex`를 실행할 때만 시작됩니다.
:::
...
npm에서 nexcode를 자체 업데이트합니다. 안정판 설치는 `@latest`를 사용하고, 미리보기 설치는
...
서비스는 자동으로 다시 빌드해 시작하며, 포그라운드 설치에서는 다음 단계로 `nxc start`를 출력합니다.
...
새 버전은 [Release workflow](https://github.com/lidge-jun/nexcode/actions/workflows/release.yml)가
