...
| `combos?` | `Record<string, NxcComboConfig>` | `{}` | 순서가 정해진 provider/model 대상들로 구성한 가상 `combo/<id>` 모델입니다. |

## 모델 해석 순서

nexcode는 요청된 model을 다음 순서로 해석합니다:
...
`nxc sync` warning은 picker catalog convergence만 보류 중이고 routing 변경은 손실되지 않았음을 뜻합니다.
...
combo는 목록에 오를 수 없더라도 계속 직접 라우팅할 수 있습니다. `nxc sync`, `/v1/models`, 그리고 Codex picker는 다음 조건을 모두 만족할 때만 이를 나열합니다:
...
CLI: `nxc route policy list`, `nxc route policy show <id>`, `nxc route policy dry-run <id> --model-context <tokens> --tools`, `nxc route policy evaluate <id>`.
...
CLI: `nxc logs explain <request-id>`, `nxc logs rebuild-index`, `nxc logs index-status`.
