---
title: 아키텍처
description: nexcode 내부 구조 — 모듈 맵, AdapterEvent 브리지, 요청 파서, 그리고 캐싱.
---

nexcode는 단일 Bun 프로세스입니다. 요청은 OpenAI Responses로 들어와 내부 모델로 정규화되고,
...
`NxcParsedRequest`를 구성합니다:

- **Messages** — `input` 항목은 정규화된 `NxcMessage[]`가 됩니다: user / developer / assistant /
...
파서가 무시하는 `: nexcode heartbeat` SSE 주석 줄을 보내 Codex의 유휴 타이머를 다시 시작합니다.
...
상태에서 Codex가 Responses WebSocket 업그레이드를 시도하면 nexcode는 `426 upgrade_required`를
...
내부 모델은 `types.ts`에 있습니다: `NxcParsedRequest`, `NxcContext`, `NxcMessage` 유니온,
`NxcContentPart`(text / image), `NxcToolCall`, `NxcTool`, `AdapterEvent`, 그리고 설정 타입
(`NxcConfig`, `NxcProviderConfig`). 두 가지 헬퍼가 널리 사용됩니다: `namespacedToolName()`과
