...
**Goal:** Make every non-OpenAI model exposed through the nexcode Codex catalog use Codex's official code-mode executor so Computer Use and Browser remain callable, while preserving native OpenAI metadata and the existing vision, image-generation, and DeepSeek streaming fixes.

**Architecture:** Add one explicit routed-catalog compatibility policy, `tool_mode: "code_mode_only"`, at the common normalization boundary. Apply the same policy to the no-template fallback path. Do not add local UI executors to nexcode; the proxy continues translating Codex's `exec` custom tool into ordinary provider function calling and relaying `custom_tool_call` events back to Codex.

**Tech Stack:** TypeScript, Bun, `bun:test`, Codex model catalog JSON, nexcode Responses adapters, launchd background service.
...
Use, while nexcode routes only the model's ordinary function call. Tool execution,
...
**Files:**
- Back up: `~/.nexcode/config.json`
- Back up: `~/.codex/config.toml`
- Back up: `~/.codex/nexcode-catalog.json`
- Back up: `~/Library/LaunchAgents/com.nexcode.proxy.plist`
- Replace: installed `@bitkyc08/nexcode` package/runtime

**Step 1: Capture provenance and create a timestamped backup**

Record the installed `nxc` path/version, service PID/runtime path, current git SHA, and SHA-256 hashes of the four configuration/service files. Copy present files into one timestamped directory under `/private/tmp` without printing credentials.
...
Install the local artifact into the same global prefix that owns the current `nxc`, then run:
...
Read `~/.codex/nexcode-catalog.json` and `~/.codex/models_cache.json`. Assert:
