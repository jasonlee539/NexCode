...
describe("logOAuthEvent", () => {
  test("emits redacted account and never prints a token-looking field value", () => {
    ...
    expect(lines[0]).toContain("[nexcode]");
...
export function logOAuthEvent(
  ...
  const parts = [`[nexcode] ${event}`, `provider=${fields.provider}`];
...
Use the existing test helpers that point `NEXCODE_HOME` at a temp dir and stub `OAUTH_PROVIDERS[provider].refresh` / fetch. Follow `tests/oauth-refresh.test.ts` setup patterns for auth store isolation.
...
Set `action` strings:
- reauth: `run \`nxc login <provider>\``
- cooldown: `wait until <local time> or start a new session with another eligible account`
- warning refresh_conflict: `re-run \`nxc doctor\` after ensuring only one proxy process writes the credential store`
...
### Task 5: `nxc status` OAuth health output
...
git commit -m "$(cat <<'EOF'
feat(cli): show OAuth health in nxc status
...
### Task 6: `nxc doctor` OAuth checks
...
- Produces: doctor rows like:
  ...
  - `[WARN] Account account-…42 requires reauthentication. Action: run \`nxc login <provider>\``
...
git commit -m "$(cat <<'EOF'
feat(cli): add OAuth reliability checks to nxc doctor
...
Attach projected health to account DTOs. In GUI, show badge + short explanation (what happened, provider/account redacted, blocked?, next action). Actions: Reauthenticate button (existing), copy `nxc doctor`, disable probe messaging during cooldown. No “anti-ban” copy.
...
- How to use `nxc status` and `nxc doctor` for OAuth health
