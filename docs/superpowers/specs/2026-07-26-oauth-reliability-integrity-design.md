...
- **`nxc status`:** concise OAuth health block (provider, redacted account, status, reason/action or retry-after).
- **`nxc doctor`:** checks for writable credential store, single-flight/lock readiness, reauth, cooldown, incomplete credentials, refresh conflicts; each WARN includes recovery action.
- **Dashboard:** health badge on provider/account views with explanation + actions (reauthenticate, copy `nxc doctor`, retry after cooldown). Copy must say reliability/diagnostics — never “anti-ban”.
