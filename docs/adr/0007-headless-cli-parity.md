...
- 장점, 단점 및 영향: Headless parity improves and future operations share consistent errors. Live management commands require a running proxy; offline config inspection/import remains available through a separately validated `nxc config` path.

## Command structure

```text
nxc setup                  interactive first-run flow (`init` remains an alias)
nxc provider ...           provider config, test, quota, selected models
nxc account ...            Codex/OAuth/key-pool login and lifecycle
nxc models ...             live/custom models, visibility, context, shadow calls
nxc route combo ...        failover and round-robin virtual models
nxc agent ...              subagents, fallback, injection, effort, sidecars
nxc observe ...            logs, usage, storage, memory, debug captures
nxc access ...             external API keys, endpoints, model tests
nxc integration ...        Claude and Grok client integrations
nxc system ...             settings, startup, diagnostics, sync, update jobs
nxc config ...             masked inspection and validated offline import/edit
```
...
- Live mutations go through the running management API. `nxc config import/set` validates
