...
The dashboard needs buttons for `nxc sync` and nexcode self-update. `nxc sync` is safe to run in
the proxy process because it refreshes Codex config/catalog state. `nxc update` is different: npm
...
API endpoints that create an update job in `NEXCODE_HOME/update-job.json`. The proxy starts a
detached hidden CLI worker, and the worker performs the install command and optional restart.

For npm installs, the worker runs the Node launcher path (`node bin/nxc.mjs update --tag <tag>`) so
...
For npm installs specifically, `node nxc.mjs update` already stops the proxy and reinstalls /
...
npm installs skip the probe-first path entirely, because the launcher only prints `nxc start`
...
- Update status survives a proxy restart because it is stored in the nexcode config directory.
...
- A completed install can still finish with `status: "failed"` when the replacement proxy never
  ...
  `nxc start` and the Bun `--allow-scripts` reinstall path.
