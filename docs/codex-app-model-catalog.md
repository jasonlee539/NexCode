...
> provider table name is `"NexCode Proxy"` (see `src/codex/inject.ts`).

This document records why nexcode routed models can appear in Codex App's model picker without
patching Codex App itself.

## Summary

Codex CLI, TUI, and App share the Codex home configuration surface. nexcode integrates by writing
...
When Codex App reads the same config/catalog state, routed nexcode models are visible because they
...
nexcode does not generate minimal JSON entries. It clones a native Codex model catalog entry and
then changes the routed fields:

```text
slug = "<provider>/<model>"
display_name = "<provider>/<model>"
description = "Routed via nexcode -> <provider> ..."
...
Codex's models-manager metadata lookup tolerates exactly one "/", so nexcode aliases
...
- `ultra` is a client-facing selection: maximum reasoning plus proactive multi-agent delegation
  ...
  boundary; nxc mirrors that in two places — the Responses parser normalizes `ultra -> max` at
...
- Native `gpt-5.6-*` slugs are emitted from the pinned upstream models.json snapshot
  ...
  `multi_agent_version` (`sol`/`terra` v2, `luna` v1). nxc adaptations: `minimal_client_version`
...
After changing providers, hidden models, featured models, or service-tier metadata, nexcode should
...
- active model provider is `nexcode`
