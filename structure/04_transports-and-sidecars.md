...
A bare `nxc service` is an idempotent install-or-repair command. Argument validation happens before
...
existing task. Explicit `nxc service install` remains the operator-owned registration request.
...
and the client replays it on every later turn. The proxy's own `nxc1:` envelopes are transparent
...
- 기존 구현 및 제약 조건: Compaction handling was binary — `nxc1:` envelopes were ours, everything
...
- 장점, 단점 및 영향: A cross-backend session degrades one compaction summary to a note instead of
  ...
  other routed gateways see a note because routed compaction produces an `nxc1:` envelope.
...
or `x-nexcode-api-key` because the provider key replaces caller authorization before fetch. The
...
an explicit `CLAUDE_USER_DATA_DIR` (or the nexcode override) wins; on Windows
...
- 다른 대안 대신 이 방식을 선택한 이유: nexcode has no trustworthy per-request sandbox attestation in request text or headers, so any prompt-carried marker is spoofable by data-plane callers.
...
`websocketsEnabled(config)` is true only for an explicit `true`. When false, nexcode removes
...
The HTTP/SSE bridge emits an SSE comment-line keep-alive (`: nexcode heartbeat`) during upstream
...
because the replay may be billable; `NXC_EMPTY_COMPLETION_RETRY=0` is a disable-only emergency
...
(Kimi/Moonshot 400 `nxc-mrqaiw05-269`; unit `devlog/_fin/260718_dangling_toolcall_hardening`).
...
- 기존 구현 및 제약 조건: Reasoning-input sanitation already handled raw content and `nxcr1:` envelopes; the replay cache already supplied a bounded, thread-scoped physical-route identity, but no record connected that identity to native `encrypted_content` provenance.
...
clients do not request them as flat model slugs. NexCode sends the matching Grok base id through
...
```text
[Decision Log]
- 목적과 의도: Reuse Cursor's returned ConversationStateStructure on validated linear continuations so NexCode does not rebuild the full root history every turn.
...
- 선택한 방식: Keep an opaque process-local checkpointRef on NxcProviderContinuationState.cursor, bind the snapshot to conversation/account/model affinity, pin referenced blobs for the checkpoint lifetime, and fall back to the existing full-replay path for isolation, compaction, restart, missing refs, and invalid_argument recovery. Tool-result turns reuse the last completed checkpoint plus an uncovered suffix. previous_response_id is a branch anchor, never a Cursor conversation ownership key.
...
from `NxcProviderConfig.openRouterRouting`, with exact model-id replacements in
...
src/providers/key-failover.ts). If an opted-in upstream rejects the field, NexCode does not strip it and retry or mutate the
...
- **Reasoning folding:** the Responses parser folds `reasoning` items into the FOLLOWING
  ...
  siblings newline-join; `nxcr1`-signed siblings stay separate parts (Anthropic replay keeps
...
- The blob rides the existing `nxcr1:` envelope as `krc` (`src/responses/reasoning-envelope.ts`) on
...
- The blob lives on `NxcAssistantMessage.kiroRedactedReasoning`, not on a thinking content part, so
...
`supports_parallel_tool_calls`. `NxcProviderConfig.parallelToolCalls: false` is the per-provider
...
`/anthropic/v1/messages`. `nxc mmx` adapts that hard-coded client path with a temporary
...
`nxc mmx` exposes only the text resource because the other MMX resources use MiniMax-specific
...
the separate reversible `custom_provider.nexcode` file integration and is likewise
...
falling back to NexCode guesses, and the integration does not write the removed
...
item (`summary: []`, txt-only `nxcr1:` `encrypted_content`, no text deltas) — invisible in the
...
| Mimo Free | `src/adapters/mimo-free.ts` | Client identity and JWT handling are transport-local; the per-install client id lives in the nexcode state root. |
