...
On the default `nexcode-catalog.json` path, sync deliberately uses two catalog sources: Codex's
...
- 장점, 단점 및 영향: Concurrent v2 turns do not multiply CIM walks and the event loop remains responsive. A cold request can still await the bounded advisory check, and collection failure suppresses NexCode-authored model guidance as `unknown`.
...
is `nxc ready [--json] [--wait [--timeout <seconds>]]`. The probe validates the service, version,
...
`NxcConfig.multiAgentMode` controls the `multi_agent_version` field stamped on catalog entries:
...
CLI: `nxc v2 mode v1|default|v2`. GUI: segmented control on the Models page. API: `GET/PUT /api/v2`
...
nexcode already matches this: `effectiveSubagentRoster` filters with
`surface !== "v2" || isEligibleV2SubagentEntry(entry)`, so the V1 path skips the eligibility
filter exactly as upstream does. nexcode also injects no roster on V1
...
through `nxc agent subagents set` or the nexcode configuration.
...
`multiAgentGuidanceEnabled` controls only NexCode-authored delegation guidance.
`syncCodexSubagentDefaults` is a separate, default-off opt-in that applies the selected values to
Codex's native `[agents]` defaults on sync/restart for newly created Codex tasks when NexCode owns
...
Claude Code `nxc-*` agent definitions consume the same effective `claudeCode.blockedSkills` policy
...
identity. The only unguarded `nxc-self` case is an identity-resolved `claude|anthropic` model while
...
[Decision Log]
- 목적과 의도: keep generated Claude Code `nxc-*.md` roster files synchronized when the proxy is
  started or ensured on Linux, Windows, and macOS, including background service restarts.
- 기존 구현 및 제약 조건: explicit `nxc claude` launches and Management API writes reconciled the
...
- 장점, 단점 및 영향: stale NexCode-owned definitions converge on every daemon start, disabled integration
  prunes them without provider discovery, and catalog failure falls back to unmarked definitions so
  startup remains available. A later dashboard save or `nxc claude` launch restores missing context
