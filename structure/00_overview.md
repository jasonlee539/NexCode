...
nexcode is a local proxy for Codex. It does not patch Codex binaries. It changes local Codex
...
Codex CLI / TUI / App / SDK
  ...
  -> nexcode routing + adapter bridge
  -> upstream provider
```
...
`~/.nexcode/` is the default state root and `NEXCODE_HOME` overrides it; the GUI and the
...
and grows as nexcode claims further paths at runtime — so the manifest, not this table, is what
...
`$CODEX_HOME` is a separate root with a separate owner, and nexcode writes there too: removing the
nexcode state root does not undo those writes. Putting native Codex back is the job of
`nxc restore`/`eject` and the injection journal, not of deleting a directory.

| Path | Owner | Notes |
| --- | --- | --- |
| `~/.nexcode/config.json` | nexcode | Main config written by `nxc init` and the dashboard. Atomic temp-then-rename. |
| `~/.nexcode/auth.json` | nexcode | OAuth tokens; not committed. Multiauth shape: `provider -> { activeAccountId, accounts[] }` (legacy single-credential values normalize on load; a one-time `auth.json.pre-multiauth` backup guards downgrades). ChatGPT scratch OAuth stays separate from the Codex account store. For multi-slot providers, credentials without `accountId`/email replace the active slot on a normal login; an explicit add-account login preserves the prior slot and appends a distinct one. Single-slot providers such as ChatGPT remain replacement-only. |
| `~/.nexcode/codex-accounts.json` | nexcode | Hardened main-plus-added credential store used by `openai` in Pool mode. |
| `~/.nexcode/catalog-backup.json` | nexcode | One-time pristine Codex catalog backup for restore; per-catalog copies are hashed variants (see [`03_catalog-and-subagents.md`](03_catalog-and-subagents.md)). |
| `~/.nexcode/usage.jsonl` | nexcode | Append-only request usage log (0o600); request metadata + token counts only, never prompts or auth. |
| `~/.nexcode/nxc.pid`, `runtime-port.json`, `system-env-port` | nexcode runtime | Live process identity and the port a client should reach; rewritten on start. `runtime-port.json` also carries the protected per-process listener-attestation key used before CLI diagnostics attach a management bearer. |
| `~/.nexcode/codex-runtime.json`, `codex-runtime-clamp.json` | nexcode Codex runtime | Selected Codex executable/version state and effort-clamp diagnostics. Not process identity: these persist a resolved choice and a diagnostic, so losing them changes behavior until re-resolved. |
| `~/.nexcode/service-state.json`, `service.log`, `service-api-token`, `nexcode-service-launcher.vbs`, `nexcode-service-task.xml`, `nexcode-service.cmd`, `winsw`, `tray-state.json`, `tray-heartbeat.json`, `nexcode-tray.ps1`, `nexcode-tray-*.ico`, `update-job.json` | nexcode operators | Installed-service, Windows tray, and self-update artifacts and bookkeeping. The update record carries its worker PID so a dead worker recovers instead of blocking later runs. |
| `~/.nexcode/responses-state.json`, `usage-debug.jsonl`, `crash.log`, `artifacts/` | nexcode diagnostics and artifacts | Bounded caches, diagnostics, and generated image/video artifacts served locally. |
| `~/.nexcode/codex-shim.json`, `*.lock`, `kimi-device-id`, `mimo-client-id`, `.star-prompted` | nexcode bookkeeping | Shim restore obligations, cross-process locks, per-install client identifiers, one-shot UI flags. |
| `~/.nexcode/.nexcode-owner.json`, `.nexcode-uninstall.json` | nexcode | Ownership marker and the manifest that bounds what uninstall may remove. Both live in the NexCode state root, not in `$CODEX_HOME`. |
| `$CODEX_HOME/config.toml` | Codex, edited by nexcode | Active provider and provider table. |
| `$CODEX_HOME/nexcode.config.toml` | nexcode | Optional profile for explicit Codex opt-in. |
| `$CODEX_HOME/nexcode-catalog.json` | nexcode | Shared native+routed model catalog. |
| `$CODEX_HOME/nexcode-journal.json` | nexcode | Injection journal used by restore to strip only marker-owned values while preserving later user edits. |
| `$CODEX_HOME/models_cache.json` | Codex, invalidated by nexcode | Cache invalidated after model/catalog changes. |
...
- `nxc stop`, `nxc restore`, and service stop/uninstall must leave native Codex usable.
