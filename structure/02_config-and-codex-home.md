...
$CODEX_HOME/nexcode-journal.json
$CODEX_HOME/models_cache.json
$CODEX_HOME/.nexcode-native-main-profiles/
```
...
Native-main profile ownership is bound to the real `CODEX_HOME`, not to an NexCode instance.
Its encrypted vault, transaction journal, recovery marker, and referenced quarantine files live in
the owner-only `.nexcode-native-main-profiles` directory. The unchanged
`.nexcode-native-profile.lock.sqlite` beside that directory serializes every process sharing the
home. Only plaintext login staging is instance-local under
`$NEXCODE_HOME/native-main-profile-staging`; a stage from one instance is invalid in another.
These paths and the OS keyring are owner-only: the operating-system account that owns them is the
trust boundary and already has direct access to active native credentials. NexCode detects and
...
fresh zero-byte creator stays on the coordinated path so its lock cannot be bypassed. `nxc doctor` inspects the
...
- 장점, 단점 및 영향: A stale zero-byte file no longer wedges sync, valid/unrecognized databases remain fail-closed, and recovery requires the proxy to be stopped before `nxc sync` retries injection.

NexCode never overrides an explicit `CODEX_HOME`. On Windows, `nxc doctor` and `nxc status`
...
display and JSON paths redact the OS username. The diagnostic tells users to invoke NexCode with
...
`atomicWriteFile` uses a temp file named `{path}.nxc.{pid}.{seq}.tmp` (process ID + incrementing
sequence number) to avoid collisions when concurrent writers (e.g. `nxc stop` and the proxy's own
...
matches regular files named `responses-state.json.nxc.<pid>.<sequence>.tmp`, waits at least 15
...
being re-tagged. History that an earlier legacy injection re-tagged as `nexcode` is migrated back
...
`x-nexcode-api-key` env header, so this form re-tags the root provider and appends the table:
...
both shapes — nexcode blocks, injected root base-url overrides, stale root context-window
...
If the root config selects a provider other than `openai` or `nexcode`, injection must leave the
...
`nxc sync` and `nxc restore back` run the injector's non-writing preflight before provider
...
- Project-level Codex config that bypasses managed routing
  (`src/codex/project-config-warnings.ts`), surfaced by `nxc doctor` as a warning rather than an
  override.

## Profile and fast tier

When nexcode owns routing, it also writes `$CODEX_HOME/nexcode.config.toml` as an explicit profile
...
`NxcProviderConfig.defaultMaxOutputTokens` and `modelMaxOutputTokens` are OpenAI Chat wire defaults,
...
`nxc stop`, `nxc restore` / `nxc eject`, `nxc service stop`, and `nxc service uninstall` must strip
nexcode config and routed catalog entries without damaging native Codex state.

Full `nxc uninstall` config cleanup is ownership-manifest based. A fresh config directory receives a
