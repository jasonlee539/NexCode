...
The bundled React dashboard is built into `gui/dist` and served by the same Bun proxy. `nxc gui`
...
NexCode uses three mutually exclusive reusable admission credential classes:

| Credential class | Sources | Allowed surface |
| --- | --- | --- |
| Data plane | `NEXCODE_API_AUTH_TOKEN`, the `service-api-token` file loaded through `NXC_API_TOKEN_FILE`, and `config.apiKeys` | `/v1/*` HTTP endpoints and new data-plane WebSocket handshakes only |
| Management plane | `NEXCODE_ADMIN_AUTH_TOKEN` or the independent protected `admin-api-token` file | `/api/*` only |
...
mechanism, not a reusable credential class. `nxc doctor` and OAuth health derive these capabilities
...
`nxc_data_`, `nxc_admin_`, and `nxc_session_` prefixes, historical keys matching
`^nxc_[0-9a-f]{40}$`, both environment tokens by constant-time comparison, and manually configured
...
| Logs & Debug | One sidebar entry (`/#logs`) with two tabs. Logs tab: request/runtime logs for local diagnosis. Debug tab (`/#logs/debug`; legacy `/#debug` deep links redirect there): provider + usage toggles, refresh/follow log viewer. `GET/PUT /api/debug`; `GET /api/debug/logs` and `GET /api/debug/usage-logs` (monotonic `after` cursor, legacy `since` accepted). CLI: `nxc debug provider|usage …` (both streams via running proxy API). |
| Usage | `GET /api/usage` aggregate read-only summary derived from `~/.nexcode/usage.jsonl`; measured / reported / unreported / unsupported / estimated counts, daily zero-filled grid, model and provider breakdowns. Never exposes prompts. |
| System | `POST /api/system/restart` restarts the proxy in place. Local CLI/tray callers first attest the exact runtime PID and port, then send a process-scoped HMAC capability bound to that method, path, PID, and port; the capability authorizes no other management route and is invalid after replacement. The caller observes one absolute deadline and accepts success only after a different runtime PID is healthy on the same port. `GET /api/system/memory` — service-process runtime/memory identity (pid, Bun version/revision, optional `bunRuntimeSource` provenance, platform, RSS/heap/external/ArrayBuffers scalars, observed memory = max(RSS, external, ArrayBuffers), `bun:jsc` heap context, streamMode + eager-relay gate decision, watchdog snapshot sliced to the last 60 samples) plus privacy-safe `appOwnedBytes` retained-store totals/counters under static store ids. Scalar-only payload; dashboard/admin callers use the standard management gate, while `nxc doctor` may use only the exact process-scoped local-read capability. It must never move to unauthenticated `/healthz`. |
...
`nxc account alias <provider> <id> <display-name|->` (`rename` is accepted as a synonym).
...
it. The matching CLI is `nxc account priority <provider> <id|main> [<value>]`, reading the current order
...
`NXC_BUN_RUNTIME_SOURCE` plus `NXC_BUN_RUNTIME_PATH`, the binary it was minted for — by whichever
...
Launchers that re-exec `process.execPath` instead of resolving a binary — `nxc ensure`, GUI/Claude/
...
neither that shell nor its `NEXCODE_BUN_PATH`, so re-deriving would demote a correct `override`
...
the answer matters, such as a `doctor` run in a shell whose `NEXCODE_BUN_PATH` differs from the
...
unknown rather than infer one. `nxc doctor` relies on this to avoid its previous behavior of
telling a user to set `NEXCODE_BUN_PATH` when the override was already active (#848). An
...
copyable repair commands (`nxc service repair` for an installed service or `nxc service install` when none is registered, `nxc codex-shim install`, and `nxc restore`). On
...
When the owned fixed-shape `/create /tn nexcode-proxy /xml ... /f` command exits with status 1,
...
first phase. NexCode writes a unique temporary XML definition in an ACL-hardened private directory
...
- 선택한 방식: Preserve text detection, then use the native token probe only for status-1 creation of the owned `nexcode-proxy` XML task. Unknown probe results fail closed.
...
keeps the saved state and renders fixed `nxc sync` guidance without server/account detail.

## Usage accounting

`src/usage/log.ts` writes append-only JSONL to `~/.nexcode/usage.jsonl` with file mode `0o600`.
...
For diagnosing upstream-shape / usage-extraction issues run `nxc debug usage on` (or set
`NEXCODE_USAGE_DEBUG=1` before start). The proxy then writes a rolling debug record per finalized
request to `~/.nexcode/usage-debug.jsonl` (mode `0o600`, auto-trimmed to the most-recent 100 lines
...
`nxc debug provider on` / `nxc debug provider off` on the running proxy, the Debug-page toggle, or `NXC_DEBUG=1` on
the next start (legacy `NXC_DEBUG_FRAMES` still enables the same path). Lines
use the `[nxc:<adapter>:<event>]` prefix, go to the proxy terminal, and are buffered for
`nxc debug provider logs` / `nxc debug provider logs -f`. Usage JSONL tails with
`nxc debug usage logs [-f]`. Separate from provider buffered logs above.
