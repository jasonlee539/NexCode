...
Before the first v2 projection, nexcode creates a mode-0600, no-replace byte snapshot:
...
- Account-gated native rows use each account's authenticated Codex `/models` roster as the
  ...
  entitled account still receives the exact pre-stream unsupported-model 400, nexcode invalidates
...
- `gpt-daybreak-blue-latest` remains the catalog and entitlement identity, but the canonical
  ...
  turn stays streaming as required by the canonical ChatGPT backend, and NXC returns the opaque
...
Provider debug capture includes one `[nxc:codex:affinity]` record for each canonical ChatGPT
