...
[Decision Log]
- 목적과 의도: Serve the public documentation from the memorable first-party `nexcode.me` domain.
- 기존 구현 및 제약 조건: The project Pages site was built for `lidge-jun.github.io/nexcode`, so Astro emitted a `/nexcode` base path that returns 404 under a root custom domain.
...
- 선택한 방식: Keep GitHub Actions Pages hosting, configure `nexcode.me` as the repository custom domain, publish root-relative assets and routes, and retain the default GitHub URL only as GitHub's automatic redirect.
...
- 장점, 단점 및 영향: Public links and canonical metadata become stable and branded. DNS and the Pages custom-domain setting are now deployment dependencies, and old hardcoded `/nexcode` links must not be reintroduced.
...
- 장점, 단점 및 영향: File-less package skeletons no longer produce unbounded service logs or restart churn. The wrapper still recovers ordinary proxy crashes, but repairing an incomplete npm install remains an explicit reinstall plus `nxc service repair` operation until a verified staged-update design exists.
...
run. If that setting cannot be verified, unset `NXC_SELF_HOSTED_WINDOWS` and deregister the runner;
...
The root READMEs are the concise product entrypoint. They should explain what nexcode does, how to
...
`package.json` `bin` points at `bin/nxc.mjs` (a Node shim), and the Bun runtime ships as the `bun`
...
- `bin/nxc.mjs` resolves the bundled binary via `require.resolve("bun/package.json")` and a size gate
...
- The plain-Node launcher owns `NEXCODE_BUN_PATH` selection before Bun can load project dotenv and
...
Package release is npm-focused. `package.json` exposes `nexcode` and `nxc`, `prepublishOnly` runs
...
| npm registry | `@bitkyc08/nexcode@<version>` does not exist before publish, then exists after publish with the requested dist-tag. |
...
```bash
npm view @bitkyc08/nexcode@<version> version
...
npm install -g ./bitkyc08-nexcode-*.tgz
nxc help
```
