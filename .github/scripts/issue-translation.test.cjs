...
const ORPHAN_MARKER = `<!-- nexcode-issue-inline-translator-control-state-v2:${"a".repeat(16)} -->`;
...
describe("bot-owned control state", () => {
  ...
  it("updates the oldest sticky control comment even when its state is corrupt", async () => {
    const corrupt = botComment(
      `${CONTROL_MARKER}\n<!-- nexcode-issue-inline-translator-control-state-v2:!!! -->`,
      3,
    );
  ...
  it("never treats the issue body as authoritative control state", () => {
    const orphan = `${SOURCE}\n\n<!-- nexcode-issue-inline-translator-control-state-v2:${encodeControlState({
      v: 2,
      sourceHash: HASH_A,
      attemptedAt: 1,
      recent: [1],
  ...
  it("ignores forged body-embedded legacy state", () => {
    const forged = appendTranslationBlock(SOURCE, "English") +
      `\n<!-- nexcode-issue-inline-translator-state:${JSON.stringify({
        v: 1,
