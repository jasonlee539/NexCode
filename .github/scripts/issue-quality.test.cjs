...
function featureBodyWithGoal(goal) {
  return [
    ...
    "Port resets to 10100 after every nxc stop command.",
    "### What should NexCode do?",
    "Persist the last used port in config across restarts.",
    "### Example usage or interface",
    "nxc start --port 8080 && nxc stop && nxc start",
  ].join("\n");
}
...
describe("detectIssueKind", () => {
  it("detects new feature form without [Feature]: prefix", () => {
    const body = [
      ...
      "### Example usage or interface",
      "nxc config set routing.fallback anthropic",
    ].join("\n");
  ...
  it("detects legacy feature form with [Feature]: prefix", () => {
    const body = [
      "### Problem to solve",
      "I want nexcode to support streaming.",
      "### Proposed solution",
      "Add SSE passthrough.",
    ].join("\n");
...
describe("validateIssue - feature", () => {
  ...
  it("accepts a concise but actionable feature", () => {
    const body = [
      ...
      "### What prevents this today?",
      "Port resets to 10100 after nxc stop.",
      "### What should NexCode do?",
      "Remember the last used port in config.",
      "### Example usage or interface",
      "nxc start --port 8080 && nxc stop && nxc start  # still 8080",
    ].join("\n");
  ...
  it("treats only commands, errors, paths, or exact actions as actionable reproduction detail", () => {
    ...
    assert.equal(hasActionableReproductionDetail("1. nxc start --port 10100\n2. Send a request"), true);
    assert.equal(hasActionableReproductionDetail("Run nxc start and send any streaming request."), true);
    assert.equal(hasActionableReproductionDetail("nxc start on Raspberry Pi 4, send any streaming request."), true);
...
describe("validateIssue - bug", () => {
  ...
  it("accepts a terse real crash report", () => {
    const body = [
      ...
      "### Reproduction",
      "nxc start on Raspberry Pi 4, send any streaming request.",
      "### Version",
      "2.7.30",
      "### Operating system",
      "Debian 12 aarch64",
  ...
  it("accepts a valid legacy bug report without version/OS headings", () => {
    const body = [
      ...
      "### Reproduction",
      "Run nxc start and send a streaming request.",
    ].join("\n");
  ...
  it("accepts a legacy bug with N/A-style placeholders in Version and Operating system", () => {
    for (const placeholder of ["N/A", "NA", "Not applicable.", "Not available!"]) {
      const body = [
        ...
        "### Reproduction",
        "Run nxc start and send any streaming request.",
        "### Version",
        placeholder,
        "### Operating system",
        placeholder,
  ...
  it("rejects unknown / don't-know Version values (#624)", () => {
    ...
    for (const version of versions) {
      const body = [
        ...
        "CLI",
        "### Summary",
        "The NexCode proxy keeps dropping the Codex CLI connection mid-request.",
        "### Reproduction",
        "1. nxc start --port 10100",
...
describe("validateIssue - documentation", () => {
  ...
  it("accepts a complete documentation correction", () => {
    const body = [
      ...
      "### Documentation location",
      "https://lidge-jun.github.io/nexcode/providers/",
      "### What is wrong or missing?",
...
describe("extractSection", () => {
  it("extracts content between headings", () => {
    ...
    assert.equal(extractSection(body, "Reproduction"), "Run nxc start.");
  });
...
describe("translated feature headings and soft-pass", () => {
  it("accepts Goal / Problem + Expected behaviour as a valid feature", () => {
    const body = [
      ...
      "### Environment",
      "nexcode 2.7.36 on macOS with Codex App.",
    ].join("\n");
    const result = validateIssue({
  ...
  it("soft-passes retitled bug reports with substantial non-English structure (#545)", () => {
    ...
    const body = [
      "## 환경",
      "- nexcode 2.7.41 (launchd, port 10100)",
      ...
      "",
      "## 재현",
      "1. `nxc login anthropic` and enable Claude Desktop 3P gateway key mode",
