...
// ---------------------------------------------------------------------------
// Pure issue-quality validation for NexCode.
...
function protectIndentedCodeLines(text) {
  const lines = [];
  let markerPrefix = "\u0000NXC_ISSUE_CODE_";
...
const AREA_HEURISTICS = [
  ...
  },
  {
    label: "catalog",
    scope: "full",
    re: /\b(model catalog|nexcode-catalog|model list|model visibility|virtual model|routed (catalog|entries|slug)|model slug)\b/i,
  },
  {
    label: "catalog",
    scope: "title",
  ...
  },
  {
    label: "cli",
    scope: "title",
    re: /\b(nxc\b|config\.toml|config inject)\b/i,
  },
  {
    label: "proxy",
    scope: "full",
  ...
  },
  {
    label: "install",
    scope: "full",
    re: /\b(npm (global )?install|packaging|release asset|npx nxc)\b/i,
  },
  {
    label: "service",
    scope: "full",
    re: /\b(nxc service|winsw|scheduler backend|launchd service)\b/i,
  },
  {
    label: "provider",
    scope: "full",
...
function hasConcreteDetail(text) {
  ...
  return (
    /\d/.test(c) ||
    /[`{}\[\]<>/\\]/.test(c) ||
    /\b(nxc|config|api|cli|dashboard|provider|proxy|route|endpoint|workflow|command)\b/i.test(c)
  );
}
 ...
 */
// Commands and exact technical actions, e.g. "nxc start", "run bun",
// "send a streaming request", "curl https://...".
const REPRO_COMMAND_RE = new RegExp([
    "\\b(?:run|start|stop|restart|install|launch|execute|reproduce|trigger|invoke)\\s+(?:(?:the|an|a)\\s+)?(?:nxc|bun|npm|pnpm|yarn|curl|node|codex|proxy|server|dashboard|plugin)\\b",
    "\\b(?:nxc|bun|npm|pnpm|yarn|curl|node|codex)\\s+(?:start|run|stop|restart|install|config|--[a-z-]+)\\b",
...
// File, config, and log paths such as ~/.codex/config.toml or C:\\logs\\nxc.log.
...
function validateIssue(issue) {
  ...
  if (kind === "bug") {
    ...
    if (!softPass && version !== null && isUnusableVersion(version)) {
      ...
      guidance.push("Report the installed `@bitkyc08/nexcode` version (for example `2.7.42`) or a commit SHA from `nxc --version`.");
    } else if (
      !softPass &&
      isNewBugForm &&
      ...
      guidance.push("Add your NexCode version so we can reproduce the environment.");
    }
    ...
    if (
      ...
      guidance.push("Add your NexCode version and OS so we can reproduce the environment.");
    }
  ...
  if (kind === "provider-compatibility") {
    ...
    const version = extractSection(body, "NexCode version");
    ...
    if (version !== null && isRawPlaceholder(version) === false && isEmpty(version)) emptyCore.push("NexCode version");
