...
### Debug evidence (nxc debug provider)
Run \`nxc debug provider cursor\` and send the same request through the Claude Code integration.
...
### Debug evidence
Run nxc debug provider cursor; the request fails with \`resource_exhausted\` after stream start.
`,
  });
...
test("does not duplicate case-insensitive reproduction aliases", () => {
  const repeated = "Run `nxc start` and observe the proxy error after the request fails.";
  const result = validateIssue({
...
test("still rejects an unknown NexCode version from Environment", () => {
  const result = validateIssue({
...
### Debug evidence
Run \`nxc debug provider cursor\`; it returns \`resource_exhausted\` after stream start.
`,
  });
