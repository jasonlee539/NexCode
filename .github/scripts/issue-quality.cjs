...
function normalizeEquivalentBugEvidence(issue) {
  ...
  if (core.extractSection(normalized, "Version") === null) {
    ...
    // specifically needs the NexCode install version.
    const version = extractEnvironmentField(environment, ["NexCode", "NexCode version"]);
...
function reproductionOnlyEchoesSummary(summary, reproduction) {
  ...
  const explicitOcxAction =
    /\b(?:run|execute|invoke|retry)\s+[`'"*_~]*nxc\s+(?:sync|restore|update|doctor|start|stop|restart)\b/i.test(
      independent,
    );
