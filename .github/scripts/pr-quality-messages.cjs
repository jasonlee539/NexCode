...
function buildFailureSections(failures, { pr, allowedBases, defaultBase }) {
  ...
  if (failures.some(failure => failure.code === "wrong_base")) {
    sections.push(
      ...
      "",
      `@${pr.user.login} Please retarget this PR to ${inlineCode(defaultBase)}. All contributions go to ${inlineCode(defaultBase)}; \`main\` receives only release promotions. See our [Contributing guide](https://lidge-jun.github.io/nexcode/contributing/) for details. Thanks! 🙏`
    );
  }
