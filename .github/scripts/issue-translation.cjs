...
const MARKER = "<!-- nexcode-issue-inline-translator -->";
const END_MARKER = "<!-- /nexcode-issue-inline-translator -->";
const LEGACY_STATE_RE = /<!-- nexcode-issue-inline-translator-state:([\s\S]*?) -->\s*/;
const CONTROL_MARKER = "<!-- nexcode-issue-inline-translator-control -->";
const CONTROL_STATE_V2_RE =
  /<!-- nexcode-issue-inline-translator-control-state-v2:([A-Za-z0-9_-]+) -->/;
const CONTROL_STATE_LEGACY_RE =
  /<!-- nexcode-issue-inline-translator-control-state:([\s\S]*?) -->/;
/** Trailing standalone marker (+ optional final whitespace). Never mid-body. */
const TRAILING_ORPHAN_BODY_STATE_RE =
  /<!-- nexcode-issue-inline-translator-control-state-v2:[A-Za-z0-9_-]+ -->[ \t]*(?:\r?\n)?[ \t]*$/;
...
function findTranslationBlockRange(text) {
  ...
  const legacyState = afterMarker.match(/^\s*<!-- nexcode-issue-inline-translator-state:[\s\S]*? -->\s*/);
  if (legacyState) {
...
function buildTranslationControlComment(state) {
  ...
  return [
    CONTROL_MARKER,
    `<!-- nexcode-issue-inline-translator-control-state-v2:${encoded} -->`,
    "",
